...
function ue_eliminar()
{
  ...
  var la_mensajes=new Array ("el código de personal. Seleccione una Evaluación de Requisitos Mínimos del Catalago");
  lb_valido = valida_datos_llenos(la_objetos,la_mensajes);
  if(lb_valido)
  {
...
function ue_nuevo()
{
  ...
  f.action="sigesp_srh_p_requisitos_minimos.php";
  f.submit();  
 }
...
function consultar_items () {
	 ...
	 if  ( ($('txtcodeval').value=="") ) 		
	 {
	  ...
	  f.operacion.value="CONSULTAR";
  	  f.action="../pantallas/sigesp_srh_p_requisitos_minimos.php";
	  f.existe.value="TRUE";			
	  f.submit();	
	 }
...
function ue_buscar()
{
	...
	if(li_leer==1)
	{
		window.open("../catalogos/sigesp_srh_cat_requisitos_minimos.php?valor_cat=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=518,height=400,left=50,top=50,location=no,resizable=yes");
	}
	else
	{
...
function ue_imprimir()
{
	...
	{	
		...
		fecha=f.txtfecha.value;
				   	pagina="../../../reporte/sigesp_srh_rpp_resultados_reqmin.php?codeval="+codeval+"&codper="+codper+"&codcon="+codcon+"&fecha="+fecha+"";
