...
{
  lb_valido=true;
  var la_objetos=new Array ("txtnroreg","txtfecelab", "txtcodper", "txtcodenf", "txtfecini", "txtobs");
  
 
 var la_mensajes=new Array ("el numero de registro", "la fecha de elaboracion","el código personal",   "el código de la enfermedad", "la fecha de inicio de la enfermedad",  "la observacion de la enfermedad");
  ...
  if(lb_valido)
  {
	  ...
	  var objeto = JSON.stringify(enfermedad);
...
function ue_eliminar()
{
  ...
  var la_mensajes=new Array ("el número de Registro de la Enfermedad. Seleccione un Registro del Catalago");
  lb_valido = valida_datos_llenos(la_objetos,la_mensajes);
  if(lb_valido)
  {
...
function catalogo_tipoenfermedad()
{
    
   pagina="../catalogos/sigesp_srh_cat_tipoenfermedad.php?valor_cat=0";
...
function ue_buscar()
{
	...
	if(li_leer==1)
	{
		window.open("../catalogos/sigesp_srh_cat_enfermedades.php?valor_cat=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=518,height=400,left=50,top=50,location=no,resizable=yes");
	}
	else
	{
