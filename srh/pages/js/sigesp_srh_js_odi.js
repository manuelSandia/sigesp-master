...
function ue_guardar()
{
  ls_control=$('controlguard').value;
  if (ls_control==0)
  {
  	  $('controlguard').value=1;
	  lb_valido=true;
	  lb_correcto=false;
	  ...
	  if($F('estperso')==3)
	  {
		  lb_valido_esp= ue_comparar_fechas($('fecegre').value,$('txtfecha').value,$('txtfecha'));
		  if (lb_valido_esp)
		  {
			 alert("El Evaluador esta egresado y la fecha del ODI no puede ser mayor a su fecha de egreso");
			 lb_valido= false;
		  }
	  }
	  ...
	  if (lb_valido)
	  {
		  ...
		  lb_valido2= ue_comparar_fechas($('txtfecini1').value,$('txtfecfin1').value,$('txtfecfin1'));  
			...
			if (lb_valido2)
			{
			 ...
			 lb_valido4= ue_comparar_fechas($('txtfecfin1').value,$('txtfecini2').value,$('txtfecini2'));  	
				if ((lb_valido3)&&(lb_valido4)&&($('txtfecfin1').value!=$('txtfecini2').value))
				{
				...
				{
				 alert ('Por favor chequee las fechas, hay choque de las mismas');
				 $('txtfecfin2').value=""; 
				 lb_correcto = false;
				}
			}
	  ...
	  if((!lb_valido)||(!lb_valido2)||(!lb_correcto))
	  {
	  	$('controlguard').value=0;
	  }
	  if(lb_correcto)
	  {
