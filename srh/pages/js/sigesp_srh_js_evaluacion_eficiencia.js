...
function ue_guardar()
{
 ...
 if (($F('txtcodper') == $F('txtcodeva'))  && ($F('txtcodper')!=""))
  ...
  if (lb_valido) {
	...
	total = filas.length-2;
	h=1;
	m=0;
	codaspant="";
	checkcodasp=true;
	while ((h <= filas.length)&&(lb_correcto))
	{
		codasp = eval ("document.form1.txtcodasp"+h+".value");
		if (codasp!=codaspant)
		{
				m=m+1;
			if (checkcodasp==false)
			{
				 lb_correcto=false;
			}
			else
...
if((checkcodasp==false)||(lb_correcto==false))
{
	 alert ('Debe Completar todos los Items de Evaluación.');
	 lb_correcto=false;
}
  if(lb_correcto)
