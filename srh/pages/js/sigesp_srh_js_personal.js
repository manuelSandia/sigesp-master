...
function ue_guardar_guarderia()
  {
	...
	if(lb_valido)
    {	
	  la_objetos =new Array ("txtcodguar", "txtguarbene", "txtnomguarben","txtguarmonto");
      la_mensajes=new Array ("el numero de guarderia","el codigo de beneficiario","el nombre del beneficiario","el monto de la guarderia");
	  lb_valido = valida_datos_llenos(la_objetos,la_mensajes);
	  ...
	  if (lb_valido)
	  {
		  ...
		  function onGuardar(respuesta)
		  {
			  ...
			  var guarderia =
			  {
				  ...
				  "nomper"      : $F('txtnomper'),
				  "apeper"      : $F('txtapeper'),
				  "numguar"		: $F('txtcodguar'),
				  "guarbene"	: $F('txtguarbene'),
				  "nomguarbene"	: $F('txtnomguarben'),
				  "guarmonto"	: $F('txtguarmonto')
						  
			  }
			  
			  var objeto = JSON.stringify(guarderia);
			  $aux = "insertar";	
			  params = "operacion=ue_guardar_guarderia&objeto="+objeto+"&insmod="+$F('hidguardar_guard')
			  new Ajax.Request(url,{method:'post',parameters:params,onComplete:onGuardar});
		  }
					...
					var movimiento =
					  {
						  ...
						  "obs"			:  $('txtobs').value,
						  "horact"		:  $('txthoract').value,
						  "horpro"		:  $('txthorpro').value,
						  "comact"		:  $('txtcomact').value,
						  "otringact"	:  $('txtotringact').value,
						  "otringpro"	:  $('txtotringpro').value,
						  "tipnom"		:  $('txttipnom').value,
						  "codgraact"	:  $('txtgraact').value,
						  "codpasact"	:  $('txtpasact').value
					  }
...
function ue_eliminar_guarderia()
  {
  	  lb_valido = false; 
	  if ($('hidguardar_guard').value !='modificar')
	  { alert ('Seleccione una guarderia del catalogo para eliminar.');
		lb_valido= false;
	  }
	 else
	 {  
    ...
    {	
	 ...
	 if (confirm("¿ Esta seguro de Eliminar este Registro ?"))
	 {
	  ...
	  params = "operacion=ue_eliminar_guarderia&numguar="+$F('txtcodguar')+"&codper="+$F('txtcodper')
	  new Ajax.Request(url,{method:'get',parameters:params,onComplete:onEliminar});	  
  }
  else
	{
...
function ue_limpiar_guarderia ()
{
	$('txtcodguar').value="";
	$('txtguarbene').value="";	
    $('txtnomguarben').value="";	
	$('txtguarmonto').value="";	
    $('hidguardar_guard').value="insertar";
	scrollTo(0,0);
...
function ue_nueva_guarderia ()
  {
	 ue_limpiar_guarderia();
	 function onNuevo(respuesta)
      {
	   $('txtcodguar').value  = trim(respuesta.responseText);
	   $('txtcodguar').focus();
      }
	 
	  var codper = $F('txtcodper');
	
     params = "operacion=ue_nueva_guarderia&codper="+codper;
     new Ajax.Request(url,{method:'get',parameters:params,onComplete:onNuevo});
...
function ue_buscarbeneficiario()
	{
	window.open("../../../../sno/sigesp_snorh_cat_benef_guarderia.php?tipo=srhguard","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
	}
...
function ue_calculardias()
{
	...
	else
	{
		...
		var feciniper = new fecha(feciniper);   
		var fecfinper = new fecha(fecfinper);
		//Obtiene objetos Date
		var feciniper = new Date( feciniper.anio, (feciniper.mes-1), feciniper.dia);
		var fecfinper = new Date( fecfinper.anio, (fecfinper.mes-1), fecfinper.dia);
		//Resta fechas y redondea
		var diferencia = fecfinper.getTime() - feciniper.getTime();
		var dias = Math.floor(diferencia / (1000 * 60 * 60 * 24));
		f.txtnumdiaper.value=dias+1;
	}
}
...
function ue_buscar_guarderia () 
{
	if ($('txtcodper').value=='') 
	{ alert ('Debe seleccionar un personal del catalogo');		}
	else 
	{ codper= $('txtcodper').value;
	  window.open("../catalogos/sigesp_srh_cat_guarderia.php?codper="+codper,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=518,height=400,left=50,top=50,location=no,resizable=yes");
	}
...
function sumar_sueldo()
{
  ...
  if ($('txtotringpro').value=="")
  { totingpro = 0; }
  else
  { 
    totingpro = $('txtotringpro').value;
	uf_convertir (totingpro);
    totingpro = totingpro.replace ('.','');
    totingpro = totingpro.replace (',','.');
   }

    suma = 	 parseFloat (suelpro) + parseFloat(compro) + parseFloat(totingpro);
    suma = uf_convertir (suma);
	$('txtsuetotpro').value= suma;
}

function sumar_sueldo_actual()
{
  if ($('txtsuelact').value=="")
  { suelact = 0; }
  else
  {  
  	suelact = $('txtsuelact').value 
	uf_convertir (suelact);
    suelact = suelact.replace ('.','');
    suelact = suelact.replace (',','.');
  }
  ...
  { 
    comact = $('txtcomact').value;
	uf_convertir (comact);
    comact = comact.replace ('.','');
    comact = comact.replace (',','.');
   }

  if ($('txtotringact').value=="")
  { totingact = 0; }
  else
  { 
    totingact = $('txtotringact').value;
	uf_convertir (totingact);
    totingact = totingact.replace ('.','');
    totingact = totingact.replace (',','.');
   }

    suma = parseFloat (suelact) + parseFloat(comact) + parseFloat(totingact);
    suma = uf_convertir (suma);
	$('txtsumsueact').value= suma;
}
