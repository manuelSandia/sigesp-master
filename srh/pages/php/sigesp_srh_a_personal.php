...
if (isset($_GET['valor']))
{
		...
		if($evento=="createXML")
		{
			...
			print $io_personal->uf_srh_buscar_personal($ls_codper,$ls_cedper,$ls_apeper,$ls_nomper,$ls_numexp,$ls_tipo);		
		}
		...
		elseif($evento=="buscar")
		{
			...
			$ls_numexp="%".utf8_encode($_REQUEST['txtnumexp'])."%";
			$ls_tipo=$_REQUEST['hidtipo'];
		...
		elseif($evento=="createXML_guarderia")
		{
			$ls_codper=$_GET['codper'];
			header('Content-type:text/xml');
			print $io_personal->uf_srh_buscar_guarderia($ls_codper);		
		}
}
...
{  
	...
	if ($valido_fam)
	{
	 ...
	 else if($validar_codigo){$ls_salida = 'El codigo ya existe para otro familiar'; $validar_codigo = 0;}
...
{  
	...
	list($valido,$existe)= $io_personal->uf_srh_eliminar_familiar($_GET["cedfam"],$_GET["codper"],$la_seguridad); 
	  if ($existe)
	  {
	  		...
	  		if($io_personal->existe){$ls_salida = "El familiar no puede ser eliminado porque tiene deducciones".'\n\n'.$io_personal->srt_sql;}
			else{$ls_salida = "El familiar no puede ser eliminado: ".'\n\n'.$io_personal->error_conex;}
	  }
	  else 
	  {
		  if ($valido)
...
//OPERACIONES PARA EL MANEJO DE LAS GUARDERIAS
if ($ls_operacion == "ue_nueva_guarderia")
{  
	$ls_salida = $io_personal->uf_srh_getProximoCodigo_Guarderia($_GET["codper"]);  
}

if ($ls_operacion == "ue_guardar_guarderia")
{  
    $objeto = str_replace('\"','"',$_POST["objeto"]);	
	$lo_guarderia = $io_json->decode(utf8_decode ($objeto));	
    $valido_ded= $io_personal->uf_srh_guardar_guarderia($lo_guarderia,$_POST["insmod"],$la_seguridad);
	if ($valido_ded)
	{
		 if ($_POST["insmod"]=='modificar')
		  {$ls_salida = 'La Guarderia fue Actualizada';	}
		 else { $ls_salida = 'La Guarderia fue Registrada';}
	 }
	 else {$ls_salida = 'Error al Guardar la Guarderia';} 
}  
if ($ls_operacion == "ue_eliminar_guarderia")
{  
    	
	 $valido= $io_personal->uf_srh_eliminar_guarderia ($_GET["numguar"],$_GET["codper"],$la_seguridad);  
	  
	  if ($valido)
	  {$ls_salida = 'La Guarderia fue Eliminada';}
	  else 
	  {$ls_salida = 'Error al Eliminar la Guarderia';}
