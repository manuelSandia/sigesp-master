...
// Para hacer los reportes por Gerencias
		   ...
		   {
		     $lb_valido=$io_report->uf_select_evaldes_gerencias($ld_fecini,$ld_fecfin,$ls_codgerdes,$ls_codgerhas,$rs_data3);
			 $unidades = array();
			 $totales = array();	 
			...
			if (($p==0) && ($h!=0))
				{
				  $io_pdf->ezNewPage();
				  //Para imprimir las gráfica con su tabla  según la unidad administrativa
		 		uf_print_grafica("<b>RESUMEN EVALUACIÓN DESEMPEÑO</b>","<b>".$gerenciaaux2."</b>" ,$la_renglones,$la_valores2, &$io_pdf,$io_grafica);  
					   $io_pdf->ezSetDy(-575);
					   
					  // Para calcular el porcentaje
					   uf_calcular_porcentaje (count($la_puntos), count ($totales[$gerenciaaux2]), $la_datos2);
			...
			$io_pdf->ezNewPage();
		   //Para imprimir las gráfica con su tabla  según la unidad administrativa
			uf_print_grafica("<b>RESUMEN EVALUACIÓN DESEMPEÑO</b>","<b>".$gerencia."</b>" ,$la_renglones,$la_valores2, &$io_pdf,$io_grafica);  
				   $io_pdf->ezSetDy(-575);
				   
				  // Para calcular el porcentaje
				     uf_calcular_porcentaje (count($la_puntos), count ($totales[$gerencia]), $la_datos2);
