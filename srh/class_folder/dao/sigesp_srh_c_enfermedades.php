...
function sigesp_srh_c_enfermedades($path)
	...
	function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo c�digo de un registro de una enfermedad del personal
		//    Description: Funcion que genera un c�digo de registro de una enfermedad del personal
		//	   Creado Por: Maria Beatriz Unda
		// Fecha Creaci�n:17/01/2008							Fecha �ltima Modificaci�n:17/01/2008
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function uf_srh_guardarEnfermedad ($ao_enfermedad,$as_operacion="insertar", $aa_seguridad)
  { 
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->enfermedad M�TODO->guardarEnfermedad ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
function uf_srh_eliminarEnfermedad($as_nroreg, $aa_seguridad)
  {
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->enfermedad M�TODO->uf_srh_eliminarEnfermedad ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
...
function uf_srh_buscar_enfermedades($as_nroreg,$as_codper,$as_apeper,$as_nomper)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->enfermedad M�TODO->uf_srh_buscar_enfermedades( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
