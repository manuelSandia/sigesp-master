...
function sigesp_srh_c_evaluacion_eficiencia($path)
	...
	function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo n�mero de una evaluaci�n de eficiencia
		//    Description: Funcion que genera un n�mero de una evaluaci�n de eficiencia
...
function uf_srh_guardarevaluacion_eficiencia ($ao_evaluacion,$as_operacion="insertar", $aa_seguridad)
  { 
		...
		//         access: public (sigesp_srh_p_evaluacion_eficiencia)
		//      Argumento: $ao_evaluacion    // arreglo con los datos de la evaluaci�n de eficiencia    
		//		            $as_operacion    //  variable que guarda la operacion a ejecutar (insertar o modificar)              
		//                 $aa_seguridad    //   arreglo de registro de seguridad                                               
		//	      Returns: Retorna un Booleano																					
		//    Description: Funcion que inserta o modifica una evaluaci�n de desempeno en la tabla srh_evaluacion_eficiencia
		//	   Creado Por: Maria Beatriz Unda																				    
		// Fecha Creaci�n: 30/12/2007							Fecha �ltima Modificaci�n: 30/12/2007							
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  	...
  	if ($as_operacion == "modificar")
	{
	 ...
	 $ao_evaluacion->fecfin=$this->io_funcion->uf_convertirdatetobd($ao_evaluacion->fecfin);
	  ...
	  $ls_sql = "UPDATE srh_evaluacion_eficiencia SET ".
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Modific� la evaluaci�n de eficiencia ".$as_nroeval;
	...
	{ $this->io_sql->begin_transaction();
	  ...
	  $ls_sql = "INSERT INTO srh_evaluacion_eficiencia (nroeval, fecha,fecini, fecfin, comen_sup, observacion, actuacion, total, tipo_eval, acciones, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� la evaluaci�n de eficiencia ".$ao_evaluacion->nroeval;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->uf_srh_guardarevaluacion_eficiencia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		...
		else
		{
				...
				if ($lb_valido)
				{
				//Guardamos el detalle de la evaluaci�n de eficiencia
					$lb_guardo = $this->uf_guardarDetalle_evaluacion_eficiencia($ao_evaluacion, $aa_seguridad);
					if ($lb_guardo)
					{
				//Guardamos las Personas involucradas en la Evaluaci�n de Eficiencia (Evaluado, Evaluador y Trabajador)
...
function uf_srh_eliminarevaluacion_eficiencia($as_nroeval, $aa_seguridad)
  {
		...
		//        access:  public (sigesp_srh_p_evaluacion_eficiencia)														
		//      Argumento: $as_nroeval        // c�digo de la evaluaci�n de eficiencia 
		//                 $aa_seguridad     //  arreglo de registro de seguridad                                                
		//	      Returns: Retorna un Booleano																					
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->uf_srh_eliminarevaluacion_eficiencia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� la evaluaci�n de eficiencia ".$as_nroeval;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_buscar_evaluacion_eficiencia($as_nroeval,$as_fecha1,$as_fecha2,$as_tipo,$as_tipo_caja)
{
		...
		//                 $as_fecha   //    fecha de la evaluaci�n de eficiencia	     
		//         Returns: Retorna un XML  																				
		//    Description: Funcion busca una evaluaci�n en la tabla srh_evaluacion_eficiencia y crea un XML para mostrar    
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->uf_srh_buscar_evaluacion_eficiencia( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
...
//FUNCIONES PARA EL MANEJO DEL DETALLE DE LA EVALUACI�N DE EFICIENCIA
...
function uf_srh_guardar_Detalle_evaluacion_eficiencia($ao_evaluacion, $aa_seguridad)
  { 
		...
		//      Argumento: $ao_evaluacion   // arreglo con los datos de los detalle de la evaluaci�n de eficiencia                
		//                 $aa_seguridad    //   arreglo de registro de seguridad                                               
		//	      Returns: Retorna un Booleano																					
		//    Description: Funcion que inserta una competencia de evaluaci�n en la tabla srh_Detalle_evaluacion_eficiencia
		//	   Creado Por: Maria Beatriz Unda																				    
		// Fecha Creaci�n: 30/12/2007							Fecha �ltima Modificaci�n: 30/12/2007							
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->uf_srh_guardar_Detalle_evaluacion_eficiencia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
...
function uf_srh_eliminar_Detalle_evaluacion_eficiencia($as_nroeval, $aa_seguridad)
  {
		...
		//	               $aa_seguridad    //  arreglo de registro de seguridad                                                
		//	      Returns: Retorna un Booleano																				    
		//    Description: Funcion que elimina competencias de evaluaci�n en la tabla srh_Detalle_evaluacion_eficiencia
		//	   Creado Por: Maria Beatriz Unda																				    
		// Fecha Creaci�n: 30/12/2007							Fecha �ltima Modificaci�n: 30/12/2007							
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->uf_srh_eliminar_Detalle_evaluacion_eficiencia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
...
function uf_srh_load_evaluacion_eficiencia($as_nroeval, &$ai_totrows,&$ao_object)
	{
		...
		//	    Arguments: as_nroeval  // c�digo de la evaluaci�n de eficiencia
		...
		//	  Description: Funcion que obtiene las competencias de una evaluaci�n de eficiencia
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->uf_srh_load_evaluacion_eficiencia_competencia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
...
//FUNCIONES PARA EL MANEJO DEL LAS PERSONAS INVOLUACRADAS EN LA EVALUACI�N DE EFICIENCIA
...
function uf_srh_eliminar_persona($as_nroeval)
  {
		...
		//                 $aa_seguridad     //  arreglo de registro de seguridad                                  
		//	      Returns: Retorna un Booleano																					
		//    Description: Elimina a las personas involucradas en una evaluaci�n de eficiencia
		//	   Creado Por: Maria Beatriz Unda																				    
		// Fecha Creaci�n: 30/12/2007							Fecha �ltima Modificaci�n: 30/12/2007						
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->uf_srh_eliminar_persona ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
  ...
  { 
	 ...
	 $this->io_sql->begin_transaction();
	  ...
	  $ls_sql = "INSERT INTO srh_persona_evaluacion_eficiencia (nroeval,codper, tipo, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� Persona de la evaluaci�n de eficiencia ".$ao_evaluacion->nroeval;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->uf_srh_guardar_trabajador ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
 ...
 function uf_srh_guardar_evaluador ($ao_evaluacion, $aa_seguridad)
  { 
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->guardar_perosonas_evaluacion_eficiencia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
...
function uf_srh_consultar_rango_actuacion ($as_codeval, $ai_total)
{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->uf_srh_consultar_rango_actuacion( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
function uf_srh_consultar_items ($as_codeval, &$ai_totrows,&$ao_object)
{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->evaluacion_eficiencia M�TODO->uf_srh_consultar_items( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
