...
function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo c�digo de una registro de metas de personal
		//    Description: Funcion que genera un c�digo nuevo de una registro de metas de personal
...
function uf_srh_guardarregistro_metas($ao_registro,$as_operacion="insertar", $aa_seguridad)
  { 
		...
		//	   Creado Por: Maria Beatriz Unda																				    
		// Fecha Creaci�n: 03/01/2008							Fecha �ltima Modificaci�n: 03/01/2008							
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  	...
  	if ($as_operacion == "modificar")
	{
	 ...
	 $ao_registro->fecfin=$this->io_funcion->uf_convertirdatetobd($ao_registro->fecfin);
	  ...
	  $ls_sql = "UPDATE srh_registro_metas SET ".
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Modific� el registro de metas de personal ".$as_nroreg;
	...
	{ $this->io_sql->begin_transaction();
	  ...
	  $ao_registro->fecini=$this->io_funcion->uf_convertirdatetobd($ao_registro->fecini);
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� el registro de metas de personal".$as_nroreg;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->registro_metas M�TODO->uf_srh_guardarregistro_metas ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
function uf_select_metas_revisiones ($as_nroreg)
	{
		...
		//		   Access: private
 		//	    Arguments: as_nroreg // c�digo del registro de revision de las metas
		...
		if($rs_data===false)
		{
        	$this->io_msg->message("CLASE->registro_metas  M�TODO->uf_select_metas_revisiones ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
...
function uf_srh_eliminarregistro_metas($as_nroreg,  $aa_seguridad)
  {
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->registro_metas M�TODO->uf_srh_eliminarregistro_metas ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� lel registro de metas de personal ".$as_nroreg;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_buscar_registro_metas($as_nroreg,$as_codper,$as_apeper,$as_nomper,$as_fecreg1,$as_fecreg2)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->registro_metas M�TODO->uf_srh_buscar_registro_metas( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
  ...
  { 
	 ...
	 $this->io_sql->begin_transaction();
	   ...
	   $ls_sql = "INSERT INTO srh_dt_registro_metas (nroreg, codmeta, meta,estado_meta, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� la meta del personal ".$ao_registro->nroreg;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->registro_metas M�TODO->uf_srh_guardar_meta ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
...
function uf_srh_eliminar_meta($as_nroreg,$aa_seguridad)
  {
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->registro_metas M�TODO->uf_srh_eliminar_meta ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� la meta del personal ".$as_nroreg;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_load_registro_metas_campos($as_nroreg,&$ai_totrows,&$ao_object)
	{
		...
		//	    Arguments: $as_nroreg  // n�mero del registro de metas de perosnal
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->registro_metas M�TODO->uf_srh_load_registro_metas_campos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
