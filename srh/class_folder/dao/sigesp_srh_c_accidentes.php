...
function sigesp_srh_c_accidentes($path)
  ...
  {  
		...
		//	   Creado Por: Maria Beatriz Unda																				    //
		// Fecha Creaci�n: 20/11/2007							Fecha �ltima Modificaci�n: 20/11/2007							//
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function uf_srh_guardarAccidente ($ao_accidente,$as_operacion="insertar", $aa_seguridad)
  { 
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->accidentes M�TODO->guardarAccidente ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
function uf_srh_eliminarAccidente($as_nroreg, $aa_seguridad)
  {
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->accidentes M�TODO->eliminarAccidente ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
...
function uf_srh_buscar_accidentes($as_nroreg,$as_codper,$as_apeper,$as_nomper)
	{
		...
		//         access: public (sigesp_srh_accidentes)													                    //
		//      Argumento: $as_nroreg   //  n�mero del registro del accidete							                        //
		//                 $as_codper   //  c�dula del trabajador                                                               //
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->accidentes M�TODO->uf_srh_buscar_accidentes( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
