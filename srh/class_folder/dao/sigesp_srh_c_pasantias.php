 ...
 function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo n�mero de un registro de pasant�a
		//    Description: Funcion que genera un registro de pasant�a
    ...
    $ls_nropas = str_pad ($ls_nropas,10,"0",STR_PAD_LEFT);
	return $ls_nropas;
  }
  ...
  {  
		...
		//	   Creado Por: Maria Beatriz Unda																				    //
		// Fecha Creaci�n: 12/11/2007							Fecha �ltima Modificaci�n: 12/11/2007							//
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function uf_srh_guardarPasantia($ao_pasantia,$as_operacion="insertar", $aa_seguridad)
  { 
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->pasantias M�TODO->uf_srh_guardarPasantia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
function uf_srh_eliminarPasantia($as_nropas, $aa_seguridad)
  ...
  {    $ls_sql = "DELETE FROM srh_pasantias ".
	  ...
	  if($lb_borro===false)
	   {
		  $this->io_msg->message("CLASE->pasantias M�TODO->uf_srh_eliminarPasantia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		  $lb_valido=false;
		  $this->io_sql->rollback();
	   }
	   else
...
function uf_srh_buscar_pasantias($as_nropas,$as_cedpas,$as_apepas,$as_nompas,$as_fecini1,$as_fecini2)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->pasantia M�TODO->uf_srh_buscar_pasantias( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
	...
	///--------------------------------------------------------------------------------------------------------------
	  function buscar_evaluacion_pasantia($ls_rnopas)
	  {
		...
		if($rs_data===false)
		{
        	$this->io_msg->message("CLASE->Evaluaci�n  M�TODO->buscar_evaluacion_pasantia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
			$ls_valido=false;
		}
		else
		{
