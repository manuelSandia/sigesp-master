...
function sigesp_srh_c_documentos($path)
	...
	function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo n�mero de un registro de un documento legal
		//    Description: Funcion que genera un n�mero un registro un documento legal
    ...
    $ls_nrodoc = str_pad ($ls_nrodoc,10,"0",STR_PAD_LEFT);
    return $ls_nrodoc;
  }
...
function uf_srh_guardarDocumentos ($ao_documento,$as_operacion="insertar", $aa_seguridad)
  { 
		...
		//	   Creado Por: Maria Beatriz Unda																				    //
		// Fecha Creaci�n: 09/02/2008							Fecha �ltima Modificaci�n: 09/02/2008							//
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->documentos M�TODO->uf_srh_guardarDocumento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
function uf_srh_eliminarDocumentos($as_nrodoc, $aa_seguridad)
  {
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->documentos M�TODO->uf_srh_eliminarDocumento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
...
function uf_srh_buscar_documentos($as_nrodoc,$as_dendoc,$as_codtipdoc)
	{
		...
		//         access: public (sigesp_srh_documentos)													                    //
		//      Argumento: $as_nrodoc   //  numero del registro del Documento							                        //
		//                 $as_dendoc   //  denominaci�n del documento
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->documentos M�TODO->uf_srh_buscar_documentos( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
