...
function sigesp_srh_c_contratos($path)
	...
	function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo n�mero de un registro de un contrato de personal
		//    Description: Funcion que genera un n�mero un registro un contrato de personal
...
function uf_srh_guardarContrato ($ao_contrato,$as_operacion="insertar", $aa_seguridad)
  { 
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->contratos M�TODO->uf_srh_guardarContrato ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
function uf_srh_eliminarContrato($as_nroreg, $aa_seguridad)
  {
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->contratos M�TODO->uf_srh_eliminarContrato ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
...
function uf_srh_buscar_contratos($as_nroreg,$as_codper,$as_apeper,$as_nomper,$as_codtipcon)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->contratos M�TODO->uf_srh_buscar_contratos( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
