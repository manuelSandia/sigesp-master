...
function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo n�mero de una evaluaci�n de desempe�o
		//    Description: Funcion que genera un n�mero de una evaluaci�n de desempe�o
    ...
    $ls_nroeval= str_pad ($ls_nroeval,10,"0",STR_PAD_LEFT);
     return $ls_nroeval;
  }
		
   
function uf_srh_guardarevaluacion_desempeno ($as_newnroeval,$ao_evaluacion,$ps_operacion="insertar",&$as_comp, $aa_seguridad)
  { 
  
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_srh_guardarevaluacion_desempeno																		
		//         access: public (sigesp_srh_p_evaluacion_desempeno)
		//      Argumento: $ao_evaluacion    // arreglo con los datos de la evaluaci�n de desempe�o    
		//		            $ps_operacion    //  variable que guarda la operacion a ejecutar (insertar o modificar)              
		//                 $aa_seguridad    //   arreglo de registro de seguridad                                               
		//	      Returns: Retorna un Booleano																					
		//    Description: Funcion que inserta o modifica una evaluaci�n de desempeno en la tabla srh_evaluacion_desempeno
		//	   Creado Por: Maria Beatriz Unda																				    
		// Fecha Creaci�n: 27/12/2007							Fecha �ltima Modificaci�n: 27/12/2007							
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  	$as_comp=0;
  	...
  	if ($ps_operacion == "modificar")
	{
	 ...
	 $ao_evaluacion->fecfin=$this->io_funcion->uf_convertirdatetobd($ao_evaluacion->fecfin);
	  ...
	  $ls_sql = "UPDATE srh_evaluacion_desempeno SET ".
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Modific� la evaluaci�n de desempe�o ".$as_nroeval;
	...
	{ 	  $this->io_sql->begin_transaction();
		  ...
		  $ao_evaluacion->fecini=$this->io_funcion->uf_convertirdatetobd($ao_evaluacion->fecini);
		  $ao_evaluacion->fecfin=$this->io_funcion->uf_convertirdatetobd($ao_evaluacion->fecfin);
		  $ld_fecini=$ao_evaluacion->fecini;
		  $ld_fecfin=$ao_evaluacion->fecfin;
		  $ls_personal=$ao_evaluacion->codper;
		  $lb_guardo=true;
		  $lb_existe = $this->uf_buscar_evaluacion_existente($ls_personal,$ld_fecini,$ld_fecfin);
		  if (!$lb_existe)
		  {
			   $ls_sql = "INSERT INTO srh_evaluacion_desempeno (nroeval, fecha,fecinie, fecfine,  totalodi, totalcompe, actuacion, obs_sup, obs_jefe, tipo_eval, codemp) ".	  
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
						$ls_evento="INSERT";
						$ls_descripcion ="Insert� la evaluaci�n de desempe�o ".$ao_evaluacion->nroeval;
						$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
														$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
														$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
		  ...
		  else
		  {
	       ...
	       if ($as_comp==0)
		   {
			   ...
			   $lb_valido=true;	   
			   if(($lb_guardo===false)&&($ps_operacion != "modificar"))
				{
					...
					if ($this->io_sql->errno=='23505' || $this->io_sql->errno=='1062' || $this->io_sql->errno=='-239' || $this->io_sql->errno=='-5'|| $this->io_sql->errno=='-1')
					{
						...
						$this->uf_srh_guardarevaluacion_desempeno ($ls_codigonew,$ao_evaluacion,$ps_operacion="insertar", $aa_seguridad);
						$lb_guardo=true;
					}
					else
					{
						$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_guardarevaluacion_desempeno ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
						$lb_valido=false;
					}
				}
				elseif($lb_valido)
				{
					...
					if ($lb_valido)
					{
					   //Guardamos el detalle de la evaluaci�n de desempe�o
...
function uf_buscar_evaluacion_existente ($ls_personal,$ld_fecini,$ld_fecfin)
{
	$lb_valido=true;
	$ls_sql= "SELECT srh_evaluacion_desempeno.nroeval ".
			"FROM srh_evaluacion_desempeno, srh_persona_evaluacion_desempeno ".
			"WHERE srh_evaluacion_desempeno.codemp='0001' ".
			"AND srh_persona_evaluacion_desempeno.codper='".$ls_personal."' ".
			"AND srh_persona_evaluacion_desempeno.tipo='P' ".
			"AND srh_evaluacion_desempeno.fecinie BETWEEN '".$ld_fecini."' AND '".$ld_fecfin."' ".
			"AND srh_evaluacion_desempeno.nroeval=srh_persona_evaluacion_desempeno.nroeval";
	$rs_data=$this->io_sql->select($ls_sql);
	if($rs_data===false)
	{
		$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_buscar_evaluacion_existente( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
...
function uf_srh_eliminarevaluacion_desempeno($as_nroeval, $aa_seguridad)
  {
		...
		//        access:  public (sigesp_srh_p_evaluacion_desempeno)														
		//      Argumento: $as_nroeval        // c�digo de la evaluaci�n de desempe�o 
		//                 $aa_seguridad     //  arreglo de registro de seguridad                                                
		//	      Returns: Retorna un Booleano																					
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_eliminarevaluacion_desempeno ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� la evaluaci�n de desempe�o ".$as_nroeval;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_buscar_evaluacion_desempeno($as_nroeval,$as_fecha1,$as_fecha2)
	{
		...
		//                 $as_fecha   //    fecha de la evaluaci�n de desempe�o	     
		//         Returns: Retorna un XML  																						
		//    Description: Funcion busca una evaluaci�n en la tabla srh_evaluacion_desempeno y crea un XML para mostrar    
		...
		/*$ls_sql="SELECT DISTINCT * FROM srh_evaluacion_desempeno,srh_persona_evaluacion_desempeno,sno_personal, ". 
				" srh_tipoevaluacion,sno_personalnomina,sno_asignacioncargo,sno_cargo,sno_nomina ".
				" WHERE srh_evaluacion_desempeno.nroeval=srh_persona_evaluacion_desempeno.nroeval ".
				" AND srh_persona_evaluacion_desempeno.codper=sno_personalnomina.codper ".
				" AND srh_persona_evaluacion_desempeno.codper=sno_personal.codper ".
				" AND srh_tipoevaluacion.codeval = srh_evaluacion_desempeno.tipo_eval ".
				...
				" AND srh_evaluacion_desempeno.fecha BETWEEN  '".$as_fecha1."' AND '".$as_fecha2."' ".
				" AND srh_evaluacion_desempeno.nroeval LIKE '$as_nroeval' ".
				" ORDER BY srh_persona_evaluacion_desempeno.nroeval";*/
				
		$ls_sql= "SELECT DISTINCT srh_evaluacion_desempeno.codemp,srh_evaluacion_desempeno.nroeval,srh_evaluacion_desempeno.fecha,srh_evaluacion_desempeno.fecinie, ".
				" srh_evaluacion_desempeno.fecfine,srh_evaluacion_desempeno.revision,srh_evaluacion_desempeno.totalodi,srh_evaluacion_desempeno.totalcompe, ".
				" srh_evaluacion_desempeno.actuacion,srh_evaluacion_desempeno.obs_sup,srh_evaluacion_desempeno.obs_jefe,srh_evaluacion_desempeno.tipo_eval, ".
				" srh_persona_evaluacion_desempeno.codper,srh_persona_evaluacion_desempeno.tipo,sno_personal.nomper,sno_personal.apeper, srh_tipoevaluacion.codeval, ".
				" srh_tipoevaluacion.deneval,srh_tipoevaluacion.codesc,MAX(sno_asignacioncargo.denasicar) as denasicar,MAX(sno_cargo.descar) as descar ".
				" FROM srh_evaluacion_desempeno,srh_persona_evaluacion_desempeno,sno_personal,  srh_tipoevaluacion, ".
				" sno_personalnomina,sno_asignacioncargo,sno_cargo,sno_nomina  ".
				" WHERE srh_evaluacion_desempeno.nroeval=srh_persona_evaluacion_desempeno.nroeval  ".
				" AND srh_persona_evaluacion_desempeno.codper=sno_personalnomina.codper  ".
				" AND srh_persona_evaluacion_desempeno.codper=sno_personal.codper  ".
				" AND srh_tipoevaluacion.codeval = srh_evaluacion_desempeno.tipo_eval ".  
				...
				" AND srh_evaluacion_desempeno.fecha BETWEEN  '".$as_fecha1."' AND '".$as_fecha2."'  ".
				" AND srh_evaluacion_desempeno.nroeval LIKE '".$as_nroeval."'  ".
				" GROUP BY srh_evaluacion_desempeno.codemp,srh_evaluacion_desempeno.nroeval,srh_evaluacion_desempeno.fecha,srh_evaluacion_desempeno.fecinie, ".
				" srh_evaluacion_desempeno.fecfine,srh_evaluacion_desempeno.revision,srh_evaluacion_desempeno.totalodi,srh_evaluacion_desempeno.totalcompe, ".
				" srh_evaluacion_desempeno.actuacion,srh_evaluacion_desempeno.obs_sup,srh_evaluacion_desempeno.obs_jefe,srh_evaluacion_desempeno.tipo_eval, ".
				" srh_persona_evaluacion_desempeno.codper,srh_persona_evaluacion_desempeno.tipo,sno_personal.nomper,sno_personal.apeper, srh_tipoevaluacion.codeval, ".
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_buscar_evaluacion_desempeno( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
...
//FUNCIONES PARA EL MANEJO DEL DETALLE DE LA EVALUACI�N DE DESEMPE�O
...
function uf_srh_guardar_competencias_evaluacion_desempeno($ao_evaluacion, $aa_seguridad)
  { 
		...
		//      Argumento: $ao_evaluacion   // arreglo con los datos de los detalle de la evaluaci�n de desempe�o                
		//                 $aa_seguridad    //   arreglo de registro de seguridad                                               
		//	      Returns: Retorna un Booleano																					
		//    Description: Funcion que inserta una competencia de evaluaci�n en la tabla srh_competencias_evaluacion_desempeno
		//	   Creado Por: Maria Beatriz Unda																				    
		// Fecha Creaci�n: 27/12/2007							Fecha �ltima Modificaci�n: 27/12/2007							
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (($ao_evaluacion->rango==0) || ($ao_evaluacion->rango=='0'))
	 ...
	 if (($ao_evaluacion->peso==0) || ($ao_evaluacion->peso=='0'))
	 {
	  ...
	  $ls_sql = "INSERT INTO srh_competencias_evaluacion_desempeno (nroeval, codite, peso, rango, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� la competencia de la evaluaci�n de desempeno ".$ao_evaluacion->nroeval;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_guardar_competencias_evaluacion_desempeno ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
function uf_srh_eliminar_competencias_evaluacion_desempeno($as_nroeval, $aa_seguridad)
  {
		...
		//      Argumento: $as_nroeval       // c�digo de la evaluaci�n 
		//	               $aa_seguridad    //  arreglo de registro de seguridad                                                
		//	      Returns: Retorna un Booleano																				    
		//    Description: Funcion que elimina competencias de evaluaci�n en la tabla srh_competencias_evaluacion_desempeno
		//	   Creado Por: Maria Beatriz Unda																				    
		// Fecha Creaci�n: 27/12/2007							Fecha �ltima Modificaci�n: 27/12/2007							
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_eliminar_competencias_evaluacion_desempeno ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� la competencia de la evaluacion de desempeno ".$as_nroeval;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
  ...
  { 
	 ...
	 if (($ao_evaluacion->pesran==0) || ($ao_evaluacion->pesran=='0'))
	 {
	  ...
	  $ls_sql = "INSERT INTO srh_evaluacion_odi (nroeval, cododi, rango, peso_rango, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� el odi de la evaluci�n ".$ao_evaluacion->nroeval;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_guardar_evaluacion_odi ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
function uf_srh_eliminar_evaluacion_odi($as_nroeval, $aa_seguridad)
  {
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_eliminar_evaluacion_odi ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� el ODI ".$as_nroeval;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
 ...
 function uf_load_evaluacion_desempeno_odi($as_nroeval, $as_codper, $as_fecini,$as_fecfin, &$ai_totrows,&$ao_object)
	{
		...
		//	    Arguments: $as_nroeval   // c�digo de la evlauaci�n de desempe�o
		...
		//	  Description: Funcion que obtiene todos los campos de un ODI de una evaluaci�n de desempe�o
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT *  ".
				"  FROM srh_evaluacion_odi ".
				" INNER JOIN (srh_dt_odi ".
				"       INNER JOIN srh_odi ".
				"          ON srh_odi.fecha  BETWEEN  '".$as_fecini."' AND '".$as_fecfin."'".
				"         AND srh_odi.codemp = srh_dt_odi.codemp ".
				"         AND srh_odi.nroreg = srh_dt_odi.nroreg ".
				"       INNER JOIN (srh_persona_odi ".
				"			  INNER JOIN srh_persona_evaluacion_desempeno ".
				"				 ON srh_persona_evaluacion_desempeno.nroeval='$as_nroeval' ".
				"				AND srh_persona_evaluacion_desempeno.codper='$as_codper' ".
				...
				"          ON srh_persona_odi.codper = '$as_codper' ".
				"		  AND srh_persona_odi.codemp = srh_dt_odi.codemp ".
				"         AND srh_persona_odi.nroreg = srh_dt_odi.nroreg)".
				"    ON srh_evaluacion_odi.codemp='".$this->ls_codemp."' ".
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_load_evaluacion_desempeno_odi ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
...
function uf_load_evaluacion_desempeno_competencia($as_nroeval, &$ai_totrows,&$ao_object)
	{
		...
		//	    Arguments: as_nroeval  // c�digo de la evaluaci�n de desempe�o
		//				   ai_totrows  // total de filas del detalle
		//				   ao_object  // objetos del detalle
		//	      Returns: lb_valido True si se ejecuto el buscar � False si hubo error en el buscar
		//	  Description: Funcion que obtiene las competencias de una evaluaci�n de desempe�o
		// Fecha Creaci�n: 27/11/2007							Fecha �ltima Modificaci�n : 
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_load_evaluacion_desempeno_competencia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
...
function uf_srh_consultar_evaluacion_desempeno ($as_codper, $as_fecini,$as_fecfin,&$ai_totrows,&$ao_object,&$as_nomper)
{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_consultar_evaluacion_desempeno( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
//FUNCIONES PARA EL MANEJO DEL LAS PERSONAS INVOLUACRADAS EN LA EVALUACI�N DE DESEMPE�O
...
function uf_srh_eliminar_persona($as_nroeval)
  {
		...
		//                 $aa_seguridad     //  arreglo de registro de seguridad                                  
		//	      Returns: Retorna un Booleano																					
		//    Description: Elimina a las personas involucradas en una evaluaci�n de desempe�o
		//	   Creado Por: Maria Beatriz Unda																				    
		// Fecha Creaci�n: 27/12/2007							Fecha �ltima Modificaci�n: 27/12/2007						
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_eliminar_persona ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
...
function uf_srh_guardar_trabajador ($ao_evaluacion, $aa_seguridad)
  { 
		...
		//                 $aa_seguridad    //  arreglo de registro de seguridad                                               
		//	      Returns: Retorna un Booleano																					
		//    Description: Funcion que inserta un trabajador en una evaluacion de desempe�o en la tabla 
	 ...
	 $this->io_sql->begin_transaction();
	  ...
	  $ls_sql = "INSERT INTO srh_persona_evaluacion_desempeno (nroeval,codper, tipo, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� Persona de la evaluaci�n de desempe�o ".$ao_evaluacion->nroeval;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_guardar_trabajador ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
 ...
 function uf_srh_guardar_evaluador ($ao_evaluacion, $aa_seguridad)
  { 
		...
		//         access: public 													      
		//      Argumento: $ao_evaluacion    // arreglo con los datos de los detalle de la evaluacion de desempe�o					
		//                 $aa_seguridad    //   arreglo de registro de seguridad                                               //
		//	      Returns: Retorna un Booleano																					//
	 ...
	 $this->io_sql->begin_transaction();
	  ...
	  $ls_sql = "INSERT INTO srh_persona_evaluacion_desempeno (nroeval,codper, tipo, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� Persona de la Revsion de ODI".$ao_evaluacion->nroeval;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->guardar_perosonas_evaluacion_desempeno ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
 ...
 function uf_srh_guardar_supervisor ($ao_evaluacion, $aa_seguridad)
  { 
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_guardar_supervisor ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
//FUNCION PARA CONSULTAR EL RANGO DE ACTUACI�N 
...
function uf_srh_consultar_rango_actuacion ($as_codeval, $ai_total)
{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->evaluacion_desempeno M�TODO->uf_srh_consultar_rango_actuacion( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		...
		else
		{
		 ...
		 if ($num!=0) {
		  while ($row=$this->io_sql->fetch_row($rs_data)) 
		  {   					
		    $as_ranact = htmlspecialchars($row["dendetesc"]);
		 }    
		}	
		 else
			{   		
		   $as_ranact='No pertenece a ning�n rango de actuaci�n';
		 }
	}
...
function uf_srh_consultar_items ($as_codeval, &$ai_totrows,&$ao_object)
{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->evaluacion_psicologica M�TODO->uf_srh_consultar_items( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		...
		{ 
		  ...
		  if ($num!=0) {
		       $ai_totrows=0;
				...
				{   			
					...
					$ls_codite=$row["codite"];
					$ls_denite=trim (htmlentities ($row["denite"]));
					$li_valormax=$row["valormax"];
				
										
					$ao_object[$ai_totrows][1]="<textarea name=txtcodite".$ai_totrows."  cols=15 rows=3 id=txtcodite".$ai_totrows." class=sin-borde readonly>".$ls_codite."</textarea>";
					$ao_object[$ai_totrows][2]="<textarea name=txtdenite".$ai_totrows."    cols=35 rows=3 id=txtdenite".$ai_totrows."  class=sin-borde readonly>".$ls_denite." </textarea>";
					$ao_object[$ai_totrows][3]="<textarea name=txtpeso".$ai_totrows."    cols=6 rows=3 id=txtpeso".$ai_totrows."  class=sin-borde onKeyUp='javascript: ue_validarnumero(this);'  onBlur='ue_multiplicar(this,txtrangoc".$ai_totrows.",txtpesranc".$ai_totrows."); sumar_competencias(txtrescom,txtresodi,txttotal);'>".$li_valormax."</textarea>";
		  ...
		  {
		     $this->io_msg->message("No se encontraron Items de Evaluaci�n.");
	 		$ai_totrows=1;	
