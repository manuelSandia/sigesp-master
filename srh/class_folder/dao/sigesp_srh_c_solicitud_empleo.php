...
function sigesp_srh_c_solicitud_empleo($path)
  ...
  function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo c�digo de una solicitud de empleo
		//    Description: Funcion que genera un c�digo nuevo de una solicutd de empleo
    ...
    $ls_nrosol = str_pad ($ls_nrosol,10,"0",STR_PAD_LEFT);
    return $ls_nrosol;
  } 
 ...
 function getCedPersonal($as_cedper,&$ao_datos="")
		...
		//      Argumento: $as_cedper   //  c�digo del personal										                        
		//                 $$ao_datos   //  arreglo con datos del personal                                         
		//	      Returns: Retorna un XML  																						
		//    Description: Funcion busca un personal en la tabla sno_personal  dado la c�dula del personal                    
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if($rs_data===false)
			{
					$this->io_msg->message("CLASE->solicitud_empleo M�TODO->getCedPersonal ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
  ...
  {  
		...
		//	   Creado Por: Maria Beatriz Unda																				    //
		// Fecha Creaci�n: 26/10/2007							Fecha �ltima Modificaci�n: 26/10/2007							//
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function uf_srh_guardarsolicitud_empleo($newsol,$po_solicitud,$ps_operacion="insertar", $aa_seguridad)
  { 
  	...
  	if ($ps_operacion == "modificar")
	{
	 ...
	 $po_solicitud->fecnacper=$this->io_funcion->uf_convertirdatetobd($po_solicitud->fecnacper);
	  ...
	  $ls_sql = "UPDATE srh_solicitud_empleo SET ".
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Modific� la solicitud de empleo ".$as_nrosol;
     ...
     if($lb_guardo===false)
		{
			...
			if ($this->io_sql->errno=='23505' || $this->io_sql->errno=='1062' || $this->io_sql->errno=='-239' || $this->io_sql->errno=='-5'|| $this->io_sql->errno=='-1')
			{
				...
				$this->uf_srh_guardarsolicitud_empleo($ls_codigonew,$po_solicitud,$ps_operacion="insertar", $aa_seguridad);
				$lb_guardo=true;
			}
			else
			{
				$this->io_msg->message("CLASE->solicitud_empleo M�TODO->uf_srh_guardarsolicitud_empleo ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
			}
...
function uf_srh_eliminarsolicitud_empleo($ps_nrosol, $aa_seguridad)
  {
   ...
   //  $this-> uf_srh_eliminar_dt_solicitud_empleo($ps_nrosol, $aa_seguridad);
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->solicitud_empleo M�TODO->uf_srh_eliminarsolicitud_empleo ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� el solicitud de empleo ".$ps_nrosol;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_buscar_solicitud_empleo($as_nrosol,$as_cedper,$as_apeper,$as_nomper,$as_fecsol1, $as_fecsol2,$as_tipo,$as_tipocaja)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->solicitud_empleo M�TODO->uf_srh_buscar_solicitud_empleo( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
					...
					switch ($as_tipo)
				    {
					  ...
					  case "EVALUACION":
		        			...
		        			$cell->appendChild($dom->createTextNode($row['nrosol']." ^javascript:aceptarevaluacion(\"$ls_codperdestino\",\"$ls_cedper\",\"$ls_nomperdestino\",\"$ls_nomper\");^_self"));		        			
					  break;
				    }
		//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------		    
...
function uf_srh_guardar_dt_solicitud_empleo($po_solicitud, $aa_seguridad)
  { 
		...
		//	   Creado Por: Maria Beatriz Unda																				    //
		// Fecha Creaci�n: 30/10/2007							Fecha �ltima Modificaci�n: 30/10/2007							//
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 $this->io_sql->begin_transaction();
	  $ls_sql = "INSERT INTO srh_dt_solicitudempleo (nrosol,codare, expare, observacion, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� el detalle de solicitud de empleo ".$as_nrosol;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->solicitud_empleo M�TODO->uf_srh_guardar_dt_solicitud_empleo ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
...
function uf_srh_eliminar_dt_solicitud_empleo($ps_nrosol, $aa_seguridad)
  {
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->solicitud_empleo M�TODO->uf_srh_eliminar_dt_solicitud_empleo ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� el detalle de solicitud de empleo ".$as_nrosol;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_load_solicitud_empleo_campos($as_nrosol,&$ai_totrows,&$ao_object)
	{
		...
		//	    Arguments: $as_nrosol // c�digo de la solicutd de empleo
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->solicitud_empleo M�TODO->uf_srh_load_soliciutd_empleo_campos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
