...
function sigesp_srh_c_solicitud_adiestramiento($path)
	 ...
	 function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo c�digo de una solicitud de adiestramiento
		//    Description: Funcion que genera un c�digo nuevo de una solicutd de adiestramiento
		//	   Creado Por: Maria Beatriz Unda
		// Fecha Creaci�n:16/01/2008							Fecha �ltima Modificaci�n:16/01/2008
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function uf_srh_guardarsolicitud_adiestramiento ($ao_solicitud,$as_operacion="insertar", $aa_seguridad)
  { 
		...
		//	   Creado Por: Maria Beatriz Unda																				    
		// Fecha Creaci�n: 20/12/2007							Fecha �ltima Modificaci�n: 20/12/2007							
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  	...
  	if ($as_operacion == "modificar")
	{
	 ...
	 $ao_solicitud->costo=str_replace(",",".",$ao_solicitud->costo);
	  ...
	  $ls_sql = "UPDATE srh_solicitud_adiestramiento SET ".
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Modific� la solicitud de adiestramiento ".$ao_solicitud->nroreg;
	...
	{ $this->io_sql->begin_transaction();
	 ...
	 $ao_solicitud->costo=str_replace(",",".",$ao_solicitud->costo);
	  ...
	  $ls_sql = "INSERT INTO srh_solicitud_adiestramiento (nroreg, codper, codunivipladin, fecha, codprov, descripcion, observacion, fecini, fecfin, costo, durhras, objetivo, area, estrategia, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� la solicitud de adiestramiento ".$ao_solicitud->nroreg;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->solicitud_adiestramiento M�TODO->uf_srh_guardarsolicitud_adiestramiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
...
function uf_select_solicitud_evaluacion($as_nroreg)
	{
		...
		//		   Access: private
 		//	    Arguments: as_nroreg // n�mero del registro de solicitud
		...
		if($rs_data===false)
		{
        	$this->io_msg->message("CLASE->solicitud_adiestramiento M�TODO->uf_select_solicitud_evaluacion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
...
function uf_srh_eliminarsolicitud_adiestramiento($as_nroreg,  $aa_seguridad)
  {
		    ...
		    
		{
		   $lb_existe=false;
			...
			if($lb_borro===false)
			 {
				$this->io_msg->message("CLASE->solicitud_adiestramiento M�TODO->uf_srh_eliminarsolicitud_adiestramiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
				$this->io_sql->rollback();
			 }
			else
			 {
						...
						/////////////////////////////////         SEGURIDAD               /////////////////////////////
						$ls_evento="DELETE";
						$ls_descripcion ="Elimin� la solicitud de adiestramiento ".$as_nroreg;
						$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
														$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
														$aa_seguridad["ventanas"],$ls_descripcion);
						/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_buscar_solicitud_adiestramiento($as_nroreg,$as_fecsol1, $as_fecsol2,$as_codprov,$as_des)
	{
		...
		//      Argumento: $as_nroreg    // n�mero de la soliciutd de adiestramiento
		//                 $as_fecsol    // fecha de la solicitud                                                           
		//                 $as_codprov   //  c�digo del proveedor de adiestramiento
		//                 $as_des       //   descripci�n de adiestramiento
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->solicitud_adiestramiento M�TODO->uf_srh_buscar_solicitud_adiestramiento( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
  ...
  { 
	 ...
	 $this->io_sql->begin_transaction();
	  ...
	  $ls_sql = "INSERT INTO srh_dt_solicitud_adiestramiento (nroreg,codper,carper,dep, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� el detalle de evaluaci�n de la solicitud de adiestramiento ".$ao_solicitud->nroreg;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->solicitud_adiestramiento M�TODO->uf_srh_guardar_dt_solicitud_adiestramiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
   ...
   function uf_select_solicitud_evaluacion_adiestramiento ($as_nroreg)
	{
		...
		//		   Access: private
 		//	    Arguments: as_nroreg // n�mero de registro de la solicitud
		//	      Returns: lb_existe True si existe � False si no existe
		//	  Description: Funcion que verifica si el registro de solicitud de adiestramiento esta asociada a una evaluaci�n
		...
		if($rs_data===false)
		{
        	$this->io_msg->message("CLASE->soliciutd_adiestramiento  M�TODO->uf_select_solicitud_evaluacion_adiestramiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
...
function uf_srh_eliminar_dt_solicitud_adiestramiento($as_nroreg, $aa_seguridad)
  {
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->solicitud_adiestramiento M�TODO->uf_srh_eliminar_dt_solicitud_adiestramiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� el detalle de evaluaci�n de solicitud de adiestramiento ".$as_nroreg;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_load_solicitud_adiestramiento_campos($as_nroreg,&$ai_totrows,&$ao_object)
	{
		...
		//	    Arguments: as_nroreg  // n�mero de la solicitud de adiestramiento
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->solicitud_adiestramiento M�TODO->uf_srh_load_solicitud_adiestramiento_campos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
...
//FUNCIONES PARA EL MANEJO DEL CAT�LOGO DE PROVEEDOR 
...
function uf_srh_buscar_proveedor ($as_codprov, $as_denprov)
	{
	     ...
	     /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_srh_buscar_solicitud_adiestramiento																											
		//      Argumento: $as_codprov   //  c�digo del proveedor de adiestramiento       
	    //                 $as_denprov   //  denominaci�n del proveedor de adiestramiento
		//	      Returns: Retorna un XML  																						
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->solicitud_adiestramiento M�TODO->uf_srh_buscar_proveedor( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
