...
function sigesp_srh_c_llamada_atencion($path)
   ...
   function uf_srh_getProximoCodigo()
	  {
			...
			//      Argumento: 
			//	      Returns: Retorna el nuevo n�mero de una llamada de atenci�n 
			//    Description: Funcion que genera un n�mero de una llamada de atenci�n
		...
		$ls_nrollam = str_pad ($ls_nrollam,10,"0",STR_PAD_LEFT);
		return $ls_nrollam;
	  }
  ...
  {  
		...
		//	   Creado Por: Maria Beatriz Unda																				    //
		// Fecha Creaci�n: 08/11/2007							Fecha �ltima Modificaci�n: 08/11/2007							//
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  { 
  	...
  	if ($as_operacion == "modificar")
	{
	 ...
	 $ao_llamada->fecllam=$this->io_funcion->uf_convertirdatetobd($ao_llamada->fecllam);
	  ...
	  $ls_sql = "UPDATE srh_llamada_atencion SET ".
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Modific� la Amontestacion / llamada de atencion ".$as_nrollam;
	...
	{ $this->io_sql->begin_transaction();
	  ...
	  $ls_sql = "INSERT INTO srh_llamada_atencion (nrollam, codtrab, fecllam, unidad, causa, tipo, descripcion, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� la Amontestacion / llamada de atencion ".$as_nrollam;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->llamada_atencion M�TODO->uf_srh_guardarllamada_atencion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		...
		else
		{
				...
				if ($lb_valido)
				{
				  //Guardamos las causas de la llamada de atenioci�n
...
function uf_srh_eliminarllamada_atencion($as_nrollam, $aa_seguridad)
  {
		...
		//                 $aa_seguridad    //  arreglo de registro de seguridad                                                //
		//	      Returns: Retorna un Booleano																					//
		//    Description: Funcion que elimina una llamada de atenci�n en la tabla srh_llamada_atencion                         //
		//	   Creado Por: Maria Beatriz Unda																				    //
		// Fecha Creaci�n: 08/11/2007							Fecha �ltima Modificaci�n: 08/11/2007							//
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->llamada_atencion M�TODO->uf_srh_eliminarllamada_atencion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� la llamada de atencion ".$as_nrollam;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_buscar_llamada_atencion($as_nrollam,$as_codtrab,$as_apetrab,$as_nomtrab)
	{
		...
		//    Description: Funcion busca una llamada de atenci�n en la tabla srh_llamada_atencion y crea un XML para mostrar    //
		...
		$ls_sql= "SELECT *  ".
				 "  FROM srh_llamada_atencion ".
				 " INNER JOIN sno_personal ".
				 "    ON sno_personal.codemp = srh_llamada_atencion.codemp ".
				 "   AND sno_personal.codper = srh_llamada_atencion.codtrab ".
				 ...
				 "    ON srh_llamada_atencion.codemp=sno_personalnomina.codemp   ".
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->llamada_atencion M�TODO->uf_srh_buscar_llamada_atencion( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
  ...
  { 
	 ...
	 $this->io_sql->begin_transaction();
	  $ls_sql = "INSERT INTO srh_dt_llamada_atencion (nrollam,codcaullam_aten, codemp) ".	  
		...
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� el detalle de llamada de atencion ".$ao_llamada->nrollam;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
		/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	$lb_guardo = $this->io_sql->execute($ls_sql);
     ...
     if($lb_guardo===false)
		{
			$this->io_msg->message("CLASE->llamada_atencion M�TODO->uf_srh_guardar_dt_llamada_atencion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
...
function uf_srh_eliminar_dt_llamada_atencion($as_nrollam, $aa_seguridad)
  {
	...
	if($lb_borro===false)
	 {
		$this->io_msg->message("CLASE->llamada_atencion M�TODO->uf_srh_eliminar_dt_llamada_atencion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
		$this->io_sql->rollback();
	 }
	else
	 {
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� el detalle de llamada de atencion ".$as_nrollam;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
...
function uf_srh_load_llamada_atencion_campos($as_nrollam,&$ai_totrows,&$ao_object)
	{
		...
		//	    Arguments: $as_nrollam  // c�digo de la escala
		...
		//	  Description: Funcion que obtiene todos los campos de una llamada de atenci�n
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->llamada_atencion M�TODO->uf_srh_uf_srh_load_llamada_atencion_campos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
