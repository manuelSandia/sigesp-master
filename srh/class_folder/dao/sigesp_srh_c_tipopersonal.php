...
class sigesp_srh_c_tipopersonal
{
	...
	function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo c�digo de un tipo de personal 
		//    Description: Funcion que genera un c�digo de un tipo de personal
    ...
    $ls_codtipper = str_pad ($ls_codtipper,3,"0",STR_PAD_LEFT);
	...
	function uf_srh_select_tipopersonal($as_codtipper)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_select_tipopersonal ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function  uf_srh_insert_tipopersonal($as_codtipper,$as_dentipper,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_insert_tipopersonal ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
	...
	function uf_srh_update_tipopersonal($as_codtipper,$as_dentipper,$aa_seguridad) 
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_update_tipopersonal ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
...
function uf_select_tipopersonal_bono ($as_codtipper)
	{
		...
		if($rs_data===false)
		{
        	$this->io_msg->message("CLASE->tipopersonal  M�TODO->uf_select_tipopersonal_bono ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
...
function uf_select_tipopersonal_tabla ($as_codtipper)
	{
		...
		if($rs_data===false)
		{
        	$this->io_msg->message("CLASE->tipopersonal  M�TODO->uf_select_tipopersonal_tabla ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
...
function uf_select_tipopersonal_puntuacion ($as_codtipper)
	{
		...
		//		   Access: private
 		//	    Arguments: as_coddep // c�digo del departamento 
		//	      Returns: lb_existe True si existe � False si no existe
		...
		if($rs_data===false)
		{
        	$this->io_msg->message("CLASE->tipopersonal  M�TODO->uf_select_tipopersonal_puntuacion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
...
function uf_srh_delete_tipopersonal($as_codtipper,$aa_seguridad)
	{
		...
		if (($this->uf_select_tipopersonal_puntuacion ($as_codtipper)===false)&&
			 ...
			 ($this->uf_select_tipopersonal_bono ($as_codtipper)===false))
		 {
			...
			if($li_row===false)
			{
				$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_delete_tipopersonal ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
				$this->io_sql->rollback();
			}
			else
 ...
 function uf_srh_buscar_tipopersonal($as_codtipper,$as_dentipper)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_buscar_tipopersonal( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
