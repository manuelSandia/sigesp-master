...
function uf_srh_getProximoCodigo()
  {
		...
		//      Argumento: 
		//	      Returns: Retorna el nuevo c�digo de una profesi�n 
		//    Description: Funcion que genera un c�digo de una profesi�n
    ...
    $ls_codpro = str_pad ($ls_codpro,3,"0",STR_PAD_LEFT);
	...
	function uf_srh_select_profesion($as_codpro)
	{
		...
		//	   Creado Por: Maria Beatriz Unda
		// Fecha Creaci�n: 06/11/2007							Fecha �ltima Modificaci�n: 06/11/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_select_profesion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function  uf_srh_insert_profesion($as_codpro,$as_despro,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_insert_profesion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
	...
	function uf_srh_update_profesion($as_codpro,$as_despro,$aa_seguridad) 
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_update_profesion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
	...
	function uf_srh_delete_profesion($as_codpro,$aa_seguridad)
	{
		...
		if (($this->uf_srh_select_profesion_solicitud ($as_codpro)===false)&&
		     ($this->uf_srh_select_profesion_personal ($as_codpro)===false))
		 {
			...
			if($li_row===false)
			{
				$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_delete_profesion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
				$this->io_sql->rollback();
			}
			else
	...
	function uf_srh_select_profesion_solicitud ($as_codpro)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_select_profesion_solicitud ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_srh_select_profesion_personal($as_codpro)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_select_profesion_personal ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_srh_buscar_profesion($as_codpro,$as_despro)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->profesion M�TODO->uf_srh_buscar_profesion( ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{	
