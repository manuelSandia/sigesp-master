...
echo "<td width=80 style=text-align:center>Reg. Solvencia Laboral</td>";
echo "</tr>";
		...
		while (!$rs_data->EOF)
			  {
					$lb_existe=true;
						...
						$ls_numpririf = substr($ls_rifpro,2,8);//N�mero Principal del RIF, 8 D�gitos (0-9).
						$ls_numterrif = substr($ls_rifpro,11,1);//N�mero Terminal  del RIF, 1 D�gitos (0-9).
                        ...
                        $ls_ageviapro     = $rs_data->fields["ageviapro"];
                        $ls_aerolipro     = $rs_data->fields["aerolipro"];
                        $li_comprobante   = number_format($rs_data->fields["comprobante"],0,"","");
						if ($ls_estatus==0)
						   {
						    $ls_estprov="A";
						   }
						...
						'$ls_emailrep','$ls_grado','$ls_codbansig','$ls_denbansig','$ls_tipconpro','$lb_registronacional','$ls_contablerecdoc','$ls_tipperrif','$ls_numpririf','$ls_numterrif','$ls_ctaant','$ls_denctaant','$ls_tipperpro','$ls_ageviapro','$ls_aerolipro','$li_comprobante');\">".$ls_codpro."</a></td>";
...
function aceptar(codpro,nompro,dirpro,tipoorg,telpro,faxpro,nacpro,especialidad,rifpro,nitpro,capital,monmax,banco,cuenta,moneda,provee,contra,contable,denocontable,observacion,cedula,nomrep,cargo,
				 ...
				 ageviapro,aerolipro,comprobante)
  {
	...
	opener.document.form1.chkageviapro.checked=false;
	opener.document.form1.chkaerolipro.checked=false;
	if(ageviapro=='1')
	{
		opener.document.form1.chkageviapro.checked=true;
	}
	if(aerolipro=='1')
	{
		opener.document.form1.chkaerolipro.checked=true;
	}
	opener.document.form1.comprobante.value=comprobante;
	if (provee=="1")
	   {
