...
public function LeerTodos2()
{
	...
	$sql=" select sigesp_sfp_plancuentas.monto_anreal,sigesp_sfp_plancuentas.estatus,sigesp_sfp_plancuentas.monto_anest,sigesp_sfp_plancuentas
			...
			left outer join sigesp_plan_unico_caif s4 on s4.codplacaif=v2.cuentahaber
			where  sigesp_sfp_plancuentas.codemp='{$this->codemp}' and sigesp_sfp_plancuentas.ano_presupuesto={$this->ano_presupuesto}
			and sigesp_sfp_plancuentas.estatus='C'
			order by sigesp_sfp_plancuentas.sig_cuenta asc";
...
public function LeerPorCadenaTodas($cr,$cad)
{
	global $db;
	$plancuentas = new planUnicoRe();
	$cantidad = $plancuentas->Cantdigitoscuentas();
	$sql="select $this->_table.monto_anreal,$this->_table.monto_anest,$this->_table.sig_cuenta as codigo,$this->_table.denominacion
			...
			left outer join sigesp_plan_unico_caif s4 on s4.codplacaif=v2.cuentahaber
			where sigesp_sfp_asociacion.estatus='C'
			and sigesp_plan_unico_re.{$cr} like '{$cad}%'
			and {$this->_table}.codemp='{$this->codemp}' and {$this->_table}.ano_presupuesto={$this->ano_presupuesto}
			order by sigesp_plan_unico_re.sig_cuenta asc";
	//ver($sql);
	$Rs = $db->Execute($sql);
...
{	
	global $db;	
	$sql="select sigesp_sfp_asociacion.sig_cuenta as spg_cuenta,
	             sigesp_plan_unico_re.denominacion as denominacion,
				 s1.denominacion as dendebe,
				 s2.denominacion as denhaber,
				 s1.sc_cuenta as codigodebe,
				 s2.sc_cuenta as codigohaber ,
				 v1.cuentadebe  as codvardebe,
				 s3.denominacion as denvardebe,
				 v2.cuentahaber as codvarhaber ,
				 s4.denominacion as denvarhaber 
		 from sigesp_sfp_asociacion 
		   inner join sigesp_plan_unico_re on sigesp_sfp_asociacion.sig_cuenta=sigesp_plan_unico_re.sig_cuenta 
		   ...
		   inner join $this->_table on $this->_table.sig_cuenta=sigesp_sfp_asociacion.sig_cuenta 
		 where sigesp_sfp_asociacion.estatus='C'
 	   	   and sigesp_plan_unico_re.sig_cuenta like '4%' 
		   and sigesp_plan_unico_re.sig_cuenta not in(select sig_cuenta 
		                                                from spe_plan_ingr 
													   where spe_plan_ingr.codemp = '{$this->codemp}' 
													     and spe_plan_ingr.ano_presupuesto='{$this->ano_presupuesto}') and 
 	   	 sigesp_plan_unico_re.sig_cuenta in(select spe_int_cuentas.sig_cuenta 
		                                    from spe_relacion_es 
											  inner join spe_int_cuentas on spe_relacion_es.codinte=spe_int_cuentas.codinte 
											where spe_relacion_es.fuentecuentas='1' and spe_relacion_es.codemp = '{$this->codemp}' and spe_relacion_es.ano_presupuesto = '{$this->ano_presupuesto}') and {$this->_table}.codemp='{$this->codemp}' and {$this->_table}.ano_presupuesto='{$this->ano_presupuesto}'";
	//$db->debug=1;
	$Rs = $db->Execute($sql);
	return $Rs;
}
