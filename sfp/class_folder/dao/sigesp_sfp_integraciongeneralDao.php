...
//		$sql="select * from sfp_conversiones,sigesp_plan_unico as tblCont1 where tblCont1.sc_cuenta=sfp_conversiones.codcod";
		$sql="select $this->_table.*,sigesp_plan_unico.denominacion as dencontable,sigesp_plan_unico_re.denominacion as dencuentadebe,sigesp_plan_unico_re2.denominacion as dencuentahaber from $this->_table inner join sigesp_plan_unico on $this->_table.cuentacontable=sigesp_plan_unico.sc_cuenta inner join sigesp_plan_unico_re on $this->_table.cuentadebe=sigesp_plan_unico_re.sig_cuenta inner join sigesp_plan_unico_re as  sigesp_plan_unico_re2 on $this->_table.cuentahaber=sigesp_plan_unico_re2.sig_cuenta";
		//echo $sql;
		//die();
		$Rs = $db->Execute($sql);
		return $Rs;
...
//		$sql="select * from sfp_conversiones,sigesp_plan_unico as tblCont1 where tblCont1.sc_cuenta=sfp_conversiones.codcod";
		$sql="select $this->_table.*,sigesp_plan_unico.denominacion as dencontable,sigesp_plan_unico_caif.desplacaif as dencuentadebe,sigesp_plan_unico_caif2.desplacaif as dencuentahaber 
			from $this->_table inner join sigesp_plan_unico on $this->_table.cuentacontable=sigesp_plan_unico.sc_cuenta inner join sigesp_plan_unico_caif on $this->_table.cuentadebe=sigesp_plan_unico_caif.codplaif 
			inner join sigesp_plan_unico_caif as  sigesp_plan_unico_caif2 on $this->_table.cuentadebe=sigesp_plan_unico_caif2.codplaif";
		//echo $sql;
		//die();
		$Rs = $db->Execute($sql);
		return $Rs;
...
public function LeerCuentaDebe()
{
	...
	global $db;
	//$db->debug=1;
	$sql="select codplacaif,desplacaif from 
