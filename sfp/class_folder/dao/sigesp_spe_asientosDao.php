...
class Asientos extends ADOdb_Active_Record
{
	...
	public function LeerResultadoEjercicio()
	{
		...
		$ls_ctaresact = "";
		$ls_ctaresant = "";
		if(!$this->obtenerCuentasResultadoEmpresa($ls_ctaresant,$ls_ctaresact))
		{
		 $ls_ctaresact = "325020000";
		 $ls_ctaresant = "325010000";
		}
		...
		$oSaldos->sc_cuenta=$ls_ctaresact;
		$rsSaldos = $oSaldos->LeerSaldoInicial();
		$oSaldos->sc_cuenta=$ls_ctaresant;
	...
	public function LeerAsientoGasto()
	{
		...
		$sql="select scg_dt_sfp_cmp.sc_cuenta as codigo,sigesp_sfp_plan_unico.denominacion,scg_dt_sfp_cmp.debhab as operacion,scg_dt_sfp_cmp.monto
			  ...
			  where sigesp_sfp_cmp.codemp='{$this->codemp}' and sigesp_sfp_cmp.ano_presupuesto={$this->ano_presupuesto}
			  and sigesp_sfp_cmp.sig_cuenta='{$this->sig_cuenta}' and  spe_relacion_es.codinte='{$this->codinte}' and spe_relacion_es.codemp='{$this->codemp}' and spe_relacion_es.ano_presupuesto='{$this->ano_presupuesto}'order by scg_dt_sfp_cmp.debhab asc";
		//ver($sql);
		$Rs = $db->Execute($sql);
		return $Rs;	  	
	}
...
//			}
	...
	public function obtenerCuentasResultadoEmpresa(&$as_codctaresant,&$as_codctaresact)
	{
	    global $db;
		$lb_valido = false;
		$ls_sql = "SELECT substr(c_resultad,1,9) as c_resultad, substr(c_resultan,1,9) as c_resultan FROM sigesp_empresa WHERE codemp = '".$_SESSION["la_empresa"]["codemp"]."'";
		$resulset = $db->Execute($ls_sql);
		if ($resulset->RecordCount()>0)
		{
			$as_codctaresant = $resulset->fields["c_resultan"];	
			$as_codctaresact = $resulset->fields["c_resultad"];		
			$lb_valido =  true;
		}
		return $lb_valido;	
	}
