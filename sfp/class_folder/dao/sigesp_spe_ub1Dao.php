...
public function FiltrarEst($Cond='')
{
	...
	$Rs = $db->Execute("select * from {$this->_table} and codemp= '{$this->codemp}'"); 
	return $Rs;
}
...
public function Copiardatosestado()
{
	...
	$sql="  delete from sigesp_ub4;
			...
			delete from sigesp_ub2;
			delete from sigesp_ub1;
			insert into sigesp_ub1 (codemp,codubgeo1,denominacion) select '{$_SESSION['codemp']}',codest,desest from sigesp_estados where codpai='{$this->codpai}';
			insert into sigesp_ub2 (codemp,codubgeo1,codubgeo2,denominacion) select '{$_SESSION['codemp']}',codest,codmun,denmun from sigesp_municipio where codpai='{$this->codpai}';
			insert into sigesp_ub3 (codemp,codubgeo1,codubgeo2,codubgeo3,denominacion) select '{$_SESSION['codemp']}',codest,codmun,codpar,denpar from sigesp_parroquia where codpai='{$this->codpai}'";	
	$rs= $db->execute($sql);
	if($rs)
	{
		return true;
