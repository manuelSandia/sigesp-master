...
ini_set('precision','15');
 ...
 class class_sigesp_int_scg extends class_sigesp_int
 {
    ...
    {		
		...
		//incluir movimientos.
        if($this->uf_scg_select_movimiento($as_cuenta,$as_procede_doc,$as_documento,$as_operacion,&$ldec_monto_movimiento,&$li_orden))
		{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_scg_procesar_insert_movimiento($as_codemp,$as_procede, $as_comprobante, $as_fecha,$as_tipo_destino,$as_cuenta,$as_procede_doc, $as_documento,$as_debhab,$as_descripcion,$adec_monto_anterior,$adec_monto_actual)
    {							
	...
	function uf_scg_insert_movimiento($as_cuenta,$as_procede_doc,$as_documento,$as_debhab,$as_descripcion,$adec_monto)
	{
		...
		$ls_sql="INSERT INTO scg_dt_sfp_cmp(codemp,procede,comprobante,fecha,sc_cuenta,documento,debhab,descripcion,".
				"monto,orden) " . 
				" VALUES ('".$this->is_codemp."','".$this->is_procedencia."','".$this->is_comprobante."','" .$this->id_fecha."',".
				"'".$as_cuenta."','".$as_documento."','".$as_debhab."','".$as_descripcion."',".
				"".$adec_monto.",".$li_orden.")";
		//echo $ls_sql;
		//die();
		$li_result=$db->Execute($ls_sql);
		if($li_result===false)
		{
			if($db->ErrorNo()==1452)
			{   
    ...
    {	
		...
		$ls_sql="UPDATE scg_sfp_saldos ".$ls_str.
				" WHERE codemp='".$this->is_codemp."' ".
				"   AND sc_cuenta= '".$as_sc_cuenta."' ".
				"   AND fecsal= '".$ls_fecha."'" ;
		$li_row = $db->Execute($ls_sql);
		if($li_row===false)
		{
			$this->is_msg_error = "CLASE->sigesp_int_scg MÉTODO->uf_scg_select_saldo ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$lb_valido = false;		
	...
	function uf_scg_insert_saldo($as_cuenta,$as_fecha,$adec_monto_actual,$as_debhab)
	{
		...
		if($as_debhab=="D")
		{	 
			$ls_sql="INSERT INTO scg_sfp_saldos (codemp,sc_cuenta,fecsal,debe_mes,haber_mes) " .
