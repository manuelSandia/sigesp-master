...
{	
	...
	public function LeerEstados()
	{
		global $db;
		//$db->debug=true;
		$sql="select * from {$this->_table} where codpai='{$this->codpai}' and {$this->_table}.codemp='{$this->codemp}'";
		$Rs = $db->Execute($sql);
		return $Rs;
	}
	...
	public function LeerMunicipios()
	{
		global $db;
		//$db->debug=true;
		$sql="select * from {$this->_table} where codpai='{$this->codpai}' and codest='{$this->codest}' and {$this->_table}.codemp='{$this->codemp}'";
		$Rs = $db->Execute($sql);
		return $Rs;
	}
