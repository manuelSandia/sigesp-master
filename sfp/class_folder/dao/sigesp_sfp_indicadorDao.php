...
class IndicadorDao extends ADOdb_Active_Record
{
	var $_table='sig_indicador';
	...
	{
		global $db;
		$Rs = $db->Execute("select max(cod_ind)  as cod from {$this->_table} where {$this->_table}.codemp='{$this->codemp}'"); 
		$dato = $Rs->fields['cod'];
		return $dato;
	}
