			...
			for($i=0;$i<count($oInte);$i++)
			{
				...
				if($Resp=="1")
				{
					if($ArJson->DatosAd)
					{
						for($j=0;$j<count($ArJson->DatosAd);$j++)
						{
							$oInte[$i]->Ads[$j] = new intUniAdDao();
							$oInte[$i]->Ads[$j]->codinte = $codigo;
							PasarDatos($oInte[$i]->Ads[$j],$ArJson->DatosAd[$j]);	
						}
					}
					...
					if($ArJson->DatosUb)
					{
						for($j=0;$j<count($ArJson->DatosUb);$j++)
						{
							$oInte[$i]->Ubs[$j] = ObtenerNivelUb($ArJson->NivelUb);
							$oInte[$i]->Ubs[$j]->codinte = $codigo;
							PasarDatos($oInte[$i]->Ubs[$j],$ArJson->DatosUb[$j]);	
						}
					}
				
				 $Resp=$oInte[$i]->ActualizarTodos();
				}
				$codigo++;
			}
