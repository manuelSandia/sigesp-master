...
require_once("../class_folder/dao/sigesp_sfp_intGastosArtsDao.php");
...
if ($_POST['ObjSon']) 		
{
	...
	if($ArJson->DatosGas || $ArJson->oper=='eliminarCuentas')
	{
		for($j=0;$j<count($ArJson->DatosGas);$j++)
		{
			...
			if($ArJson->arts)
			{	
				for($r=0;$r<count($ArJson->arts);$r++)
				{
					$oInte->Gastos[$j]->arts[$r] = new intArtsDao();
					PasarDatos($oInte->Gastos[$j]->arts[$r],$ArJson->arts[$r]);		
				}
			}
		}
	...
	switch ($Evento)
	{
		...
		case 'actualizarInt':
				...
				//			//	die();
								...
								$codinte=$oInte->codinte;
								$ano_presupuesto = $_SESSION["ano_presupuesto"];
				//				//luego de hacer la formulacion (incluir en la tabla).
				//				//- Formar el comprobante contable(llamar al correlativo de comprobantes).
				//				//-cargar cuentas(hacer la consulta en la tabla conversiones, luego traer la cuenta contable1, traer del formulario la cuenta contable2 y el monto).
				//				//-una vez traida la informacion de las cuentas crear  el arreglo de cuentas, montos y operacion.
		...
		case 'buscaruna':
			...
			if($Datos->RecordCount()>0)
			{
				...
				$ObjMetas = GenerarJsonMetas($ObjMetas);
		...
		case 'leerecursos':
			...
			else
			{
				...
				$Datos = $oIngresos->LeerDistribucionporCuenta($oInte->codinte);
			}
			$ObjSon = GenerarJson2($Datos);	
			echo $ObjSon;		
			break;	
			...
			while($Datos2=$Datos->FetchRow())
			{
				foreach($Datos2 as $Propiedad=>$valor)
				{
					if(!is_numeric($Propiedad))
					{
						...
						if (is_numeric($valor) && $Propiedad!="ano_presupuesto" && $Propiedad!="codcaif" && $Propiedad!="codinte" && $Propiedad!="sig_cuenta" && $Propiedad!="spg_cuenta" && $Propiedad!="codigo" && $Propiedad!="cod_var" && $ArJson->oper!="leermetas" && $Propiedad!="codvardebe" && $Propiedad!="codvarhaber" && $Propiedad!="codigodebe" && $Propiedad!="codigohaber" && $Propiedad!="cod_ind")
						{							
...
function GenerarJsonMetas($Datos)
{
			global $json;
			$i=0;
			while($Datos2=$Datos->FetchRow())
			...
			while($Datos2=$Datos->FetchRow())
			{
				foreach($Datos2 as $Propiedad=>$valor)
				{
					if(!is_numeric($Propiedad))
					{
						...
						if (is_numeric($valor) && $Propiedad=="totalmeta")
						{							
							$valor = number_format($valor,2,",",".");	
						}
						$Propiedad = strtolower($Propiedad);
