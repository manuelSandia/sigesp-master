...
require_once('../class_folder/dao/sigesp_spg_transferenciaDao.php');
...
if ($_POST['ObjSon']) 		
{
	...
	switch ($Evento)
	{
		...
		case 'leerempresas':
		   ...
		   {
		   	$p=0;
			for($i=0;$i<=count($empresa[$campo]);$i++)
			{
				if($empresa[$campo][$i]!=NULL)
				{
					$auxRegistros[$i][$campo]= $empresa[$campo][$i];
					$p++;
				}
					
			}
		   }		   
			$TextJson = $json->encode($auxRegistros);
			echo "{$TextJson}|{$p}";
			break;
		case 'grabarsesion':
			...
			break;
		case 'transferir':
			$oTrans = new Transferencia();
			$oTrans->TransferirDatos($ArJson);
			echo $ObjSon;
			break;
	}
}
