...
if ($_POST['ObjSon']) 		
{
	...
	if($ArJson->arts)
	{	
		for($j=0;$j<count($ArJson->arts);$j++)
		{
			$Arts[$j] = new intArtsdDao();
			PasarDatos($Arts[$j],$ArJson->arts[$j]);	
		}
		ver($Arts);
	}
	...
	switch ($Evento)
	{
		...
		$ArObjetos2->cuenta="000000000";
		$rs1 = $ArObjetos->LeerUna();
		$rs2 = $ArObjetos2->LeerUna();
		$ObjSon1 = GenerarJson2($rs1);
		$ObjSon2 = GenerarJson2($rs2);
		$arArticulo =new articulos();
		
		$arArticulo->codart = trim($ArJson->codart);
		$rsIva = $arArticulo->LeerCargos();
		
		$ObjSonIva = GenerarJson2($rsIva);
		echo "{$ObjSon1}|{$ObjSon2}|{$ObjSonIva}";	
		break;
		case 'incluirInt':
			...
			while($Datos2=$Datos->FetchRow())
			{
				foreach($Datos2 as $Propiedad=>$valor)
				{
					if(!is_numeric($Propiedad))
					{
						...
						if (is_numeric($valor) && $Propiedad!="ano_presupuesto" && $Propiedad!="codcaif" && $Propiedad!="codinte" && $Propiedad!="porcar" && $Propiedad!="mes" && $Propiedad!="monto")
						{							
