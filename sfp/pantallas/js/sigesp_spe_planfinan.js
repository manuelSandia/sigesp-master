	    		...
	    		if(winCatCuentas=='')
                 {
                   winCatCuentas = new Ext.Window(
                   {
                     ...
                     handler: function()
                     {
                     		...
                     		//alert(datosRegistroActual.length);
	                  		for(s=0;s<datosRegistroActual.length;s++)
	                  		{
	                  			rec = datosRegistroActual[s];
	                  			resp = validarExistencia2(rec,grid,'codigo','sig_cuenta');
...
montoFormateado=numFormat(Registro.get('montoglobal'),2,true);
...
if(Registro.get('NuevoRegistro')==true)
{
	...
	if(Cod!='')
	{
			...
			{
				grid.getSelectionModel().getSelected().set('NuevoRegistro',false);
				grid.getSelectionModel().getSelected().set('montoglobal',montoFormateado);
