...
if(validarMontosDist())
{
	if(Asientos==true)
	{
		...
		natgasto=Ext.getCmp('natgas').getValue();
		if(natgasto=='Corrientes')
		{
			natgasto='co';
		}
		else
...
function CatPlanCompras(cuentaactual)
{
			...
			gridcompras = new Ext.grid.GridPanel({
			width:770,
			autoScroll:true,
            border:true,
		...
		function vercuentas()
		{
			ActualizarDataCompras('sig_cuenta','');
		}
		var MostrarCuentas = new Ext.Action(
		{
			...
			handler: vercuentas,
			iconCls: 'bmenuagregar',
		    tooltip: 'Mostrar Todos los artículos'
		});				  			  
		...
		items: [{
				...
				changeCheck: function(){
							  var v = this.getValue();
							 ActualizarDataCompras('sig_cuenta',v,cuentaactual);
							if(String(v) !== String(this.startValue)){
								this.fireEvent('change', this, v, this.startValue);
							} 
							 },
				...
				changeCheck: function(){
							var v = this.getValue();
							ActualizarDataCompras('denominacion',v,cuentaactual);
							if(String(v) !== String(this.startValue))
							{
								this.fireEvent('change', this, v, this.startValue);
							} 
...
function ActualizarDataCuentas(criterio,valor,cuentaactual)
{

	var myJSONObject ={
		"oper": 'Catinventario', 
		"spg_cuenta":cuentaactual,
		"criterio":criterio, 
		"cadena": valor
	};	
