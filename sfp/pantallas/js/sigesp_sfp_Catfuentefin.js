...
{		
	...
	this.MostrarCatalogo =MostrarCatalogoFu;
	this.MostrarCatalogoMontos =MostrarCatalogoFu2;
...
function MostrarCatalogoFu2()
{
	...
	success: function ( resultado, request ) { 
		...
		{name: 'cod_fuenfin'},     // "mapping" property not needed if it's the same as "name"
		{name: 'denfuefin'},
		{name: 'monto'}	// This field will use "occupation" as the mapping.
		]);

		  
                  if (!gridOnOff)
                  {
            gridFuenteFin = new Ext.grid.GridPanel({
			width:770,
			autoScroll:true,
                        border:true,
                        ds: new Ext.data.Store({
			...
			reader: new Ext.data.JsonReader({
				...
				data: myObject
                        ...
                        cm: new Ext.grid.ColumnModel([
                            {header: "Denominación", width: 30, sortable: true,   dataIndex: 'denfuefin'},
			...
			stripeRows: true
                ...
                {
                gridFuenteFin.store.loadData(myObject);
