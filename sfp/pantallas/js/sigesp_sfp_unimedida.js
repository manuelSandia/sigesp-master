...
ruta ='../../procesos/sigesp_sfp_unimedidapr.php';
pantalla ='sigesp_sfp_unimedida.php';
ObtenerSesion(ruta,pantalla);
LimpiarCampos();
 Ext.get('BtnGrabar').on('click', function()
 {
 if(Ext.get('chkgen').dom.checked)
 {
 	var check = '1';
 }
 else
...
var myJSONObject ={
				"oper": evento, 
				"cod_uni": Ext.get('txtcod').dom.value, 
...
Ext.get('BtnSalir').on('click',function()
{
 ...
 {			
	...
	var myJSONObject ={
		...
		"cod_uni": Ext.get('txtcod').dom.value, 
		"denominacion": Ext.get('txtden').dom.value,
		"genero":Ext.get('chkgen').dom.value,
	};
...
function LimpiarCampos()
{
	...
	Ext.get('chkgen').dom.checked = false;
}
	...
	success: function ( resultado, request ){ 
		 ...
		 if(datos!='')
		 {
		 	 ...
		 	 if(DatosNuevo.raiz==null)
		 	{
				...
				var DatosNuevo={"raiz":[{"cod_uni":'',"denominacion":''}]};
...
function ValidarObjetos(Obj,TipoVal)
{
 ...
 Ext.get('BtnCat').on('click', function(){
	...
	success: function ( resultado, request ) { 
		...
		{name: 'cod_uni'},     // "mapping" property not needed if it's the same as "name"
		{name: 'denominacion'},
		{name: 'genero'}	// This field will use "occupation" as the mapping.
		]);
				  ...
				  ,
					data: myObject
                        ...
                        cm: new Ext.grid.ColumnModel([
                            ...
                            {header: "C&#243;digo", width: 30, sortable: true,   dataIndex: 'cod_uni'},
		...
		items: [{
				...
				changeCheck: function(){
                  ...
                  { 
                   ...
                   {
                    //layout:'fit',
                    title: 'Cat&aacute;logo de Unidad de Medida',
		    		autoScroll:true,
                    width:800,
                    height:400,
                    modal: true,
                    ...
                    buttons: [{
                     ...
                     handler: function()
                     {
	                     if (grid.getSelectionModel().getSelected().get('genero')=='1')
						 {
						 	var check = true;
						 }
						 else
					     ...
					     Ext.get('txtcod').dom.value = grid.getSelectionModel().getSelected().get('cod_uni');
                      	 Ext.get('txtden').dom.value = trim(grid.getSelectionModel().getSelected().get('denominacion'));
                      	 Ext.get('chkgen').dom.checked = check;
