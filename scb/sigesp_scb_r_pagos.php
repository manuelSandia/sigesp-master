...
$io_fun_banco->uf_load_seguridad("SCB","sigesp_scb_r_pagos.php",$ls_permisos,$la_seguridad,$la_permisos);
$ls_reporte=$io_fun_banco->uf_select_config("SCB","REPORTE","LISTADO_PAGOS","sigesp_scb_rpp_pagos_pdf.php","C");
$li_diasem = date('w');
switch ($li_diasem){
  ...
  <table width="535" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
      ...
      <td height="22" colspan="4" align="center"><table width="483" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
        <tr>
          <td width="224"><div align="right">
                <input name="rb" type="radio" class="sin-borde" value="G" <?php print $lb_chkG?>>
            General          </div></td>
          <td width="257"><div align="left">
                <input name="rb" type="radio" class="sin-borde" value="E" <?php print $lb_chkE?>>
            Especifico</div></td>
        </tr>
      </table></td>
    </tr>
    <tr>
      <td height="68" colspan="4" align="center"><table width="483" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
        ...
        <tr>
          <td height="28"><div align="right">Cuenta Bancaria </div></td>
          <td><div align="left">
    ...
    <tr>
      <td height="52" colspan="4" align="center"><table width="483" border="0" cellspacing="0" class="formato-blanco">
        ...
        <tr>
          <td height="22" align="right"><div align="center">
              <label>
              ...
              <input type="radio" name="rb_operaciones" value="ND" class="sin-borde">
                Nota D&eacute;bito (Pagos de Nómina)</label>
              <label>
        ...
        <tr>
          <td height="22" align="center">
		    <div align="left">Filtrar por
		      <select name="orden">
		            <option value="CH">Documento</option>
		            <option value="F">Fecha</option>
					<option value="S">Solicitud</option>
					<option value="C">Cuenta</option>
		            <option value="M">Monto</option>
					<option value="P">Proveedor/Beneficiario</option>
                </select>
	          <span class="Estilo1">
	          <input name="operacion"   type="hidden"   id="operacion"   value="<?php print $ls_operacion;?>">
	          <input name="operacion"   type="hidden"   id="reporte"   value="<?php print $ls_reporte;?>">
	          </span></div></td>
          </tr>
      </table></td>
    </tr>
...
function ue_imprimir()
{
  ...
  else if(f.rb_operaciones[1].checked)
  {
  	ls_codope='CH';
  }
  else if(f.rb_operaciones[2].checked)
  {
  	ls_codope='ND';
  }
  else if(f.rb_operaciones[3].checked)
  {
  	ls_codope='CO';
  }
  ...
  if (li_imprimir=='1')
     {
		  if(f.rb[0].checked==true)
		  {
			...
			{
				pagina="reportes/"+li_nreporte+"?fecdes="+ld_fecdesde+"&fechas="+ld_fechasta+"&tiprep="+ls_tiprep+"&orden="+ls_orden+"&operacion="+ls_codope+"&tiporeporte="+ls_tiporeporte;
		  ...
		  else
		  {
			...
			{
				pagina="reportes/"+li_nreporte+"?fecdes="+ld_fecdesde+"&fechas="+ld_fechasta+"&probendes="+ls_probendesde+"&probenhas="+ls_probenhasta+"&tiprep="+ls_tiprep+"&tipproben="+ls_tipproben+"&codban="+ls_codban+"&ctaban="+ls_ctaban+"&nomban="+ls_nomban+"&dencta="+ls_dencta+"&orden="+ls_orden+"&operacion="+ls_codope+"&tiporeporte="+ls_tiporeporte;
				window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,resizable=yes,location=no");
			}
			else
			{
  ...
  if (li_imprimir=='1')
     {
		  if(f.rb[0].checked==true)
		  {
			...
			if(ld_fecdesde!="" && ld_fechasta!="")
			{
				pagina="reportes/sigesp_scb_rpp_pagos_excel.php?fecdes="+ld_fecdesde+"&fechas="+ld_fechasta+"&tiprep="+ls_tiprep+"&orden="+ls_orden+"&operacion="+ls_codope+"&tiporeporte="+ls_tiporeporte;
				window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,resizable=yes,location=no");
			}
			else
			{
		  ...
		  else
		  {
			...
			if((ls_probendesde!="")&&(ls_probenhasta!="")&&(ls_tipproben!=""))
			{
				pagina="reportes/sigesp_scb_rpp_pagos_excel.php?fecdes="+ld_fecdesde+"&fechas="+ld_fechasta+"&probendes="+ls_probendesde+"&probenhas="+ls_probenhasta+"&tiprep="+ls_tiprep+"&tipproben="+ls_tipproben+"&codban="+ls_codban+"&ctaban="+ls_ctaban+"&nomban="+ls_nomban+"&dencta="+ls_dencta+"&orden="+ls_orden+"&operacion="+ls_codope+"&tiporeporte="+ls_tiporeporte;
				window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,resizable=yes,location=no");
			}
			else
			{
