...
/*	$la_security[1]=$ls_empresa;
	...
	{			
		$ldec_salsegban   = $_POST["txtsalsegbco"];
		...
		$ldec_salseglib   = str_replace(",",".",$ldec_salseglib);
		 ...
		 $aa_conciliacion["mesano"]		  = str_replace("/","",$ls_mesano);
		 $aa_conciliacion["salseglib"]	  = $ldec_salseglib;
		 $aa_conciliacion["salsegban"]	  = $ldec_salsegban;	
		 $aa_conciliacion["conciliacion"] = $ldec_montomov;
		 $aa_conciliacion["estcon"]		  = 0;
		...
		if ($ls_tip_mov=='L')
		{
			...
			$ls_codfuefin='--';
			$in_classmovbco->io_sql->begin_transaction();
			$lb_valido=$in_classmovbco->uf_guardar_automatico($ls_codban,$ls_cuenta_banco,$ls_docmov,$ls_mov_operacion,$ld_fecha,$ls_desmov,$ls_codconmov,$ls_codpro,$ls_cedbene,$ls_desproben,$ldec_montomov,$ldec_monobjret,$ldec_montoret,$ls_chevau,$ls_estmov,$li_estint,0,$ls_estbpd,'SCBMOV',$ls_estreglib,$ls_estdoc,$ls_tipo,$ls_codfuefin,$ls_numordpagmin,$ls_codtipfon);
			$ld_fecdb=$fun->uf_convertirdatetobd($ld_fecha);
			if($lb_valido)
			{
				  $lb_valido = $in_classconciliacion->uf_procesar_conciliacion($aa_conciliacion,$ls_empresa,$ls_codban,$ls_cuenta_banco);
				  if($lb_valido)
				  {
	                  $lb_valido = $in_classconciliacion->uf_update_movimientos($ls_empresa,$ls_codban,$ls_cuenta_banco,$ls_docmov,$ls_mov_operacion,1,$ld_fecdb);
				  }
			}
			if($lb_valido)
			{
  ...
  <table width="780" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
	...
	<?php
      ...
      ?>  <select name="tip_ajuste" id="tip_ajuste" style="width:200px">
		   ...
		   <input name="txtsalsegbco" type="hidden" id="txtsalsegbco">
          <input name="txtsalseglib" type="hidden" id="txtsalseglib">
	  <?php
	  }
	?>
...
function ue_guardar()
{
	...
	f.txtsalsegbco.value=fop.txtsalsegban.value;
	f.txtsalseglib.value=fop.txtsalseglib.value;
	ls_document=f.txtdocumento.value	
	if ((ls_document=="")||(ls_document=='000000000000000'))
	{
		alert("Por favor coloque un numero de documento valido..");
	}
	else
	{
