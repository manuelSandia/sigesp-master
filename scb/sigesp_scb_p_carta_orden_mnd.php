...
<html>
<head>
<title>Transferencia Bancaria M&uacute;ltiples Notas de D&eacute;bito</title>
  ...
  if ($ls_procede=='SCVSOV')
     {
       $ls_sql = "SELECT distinct a.codcueban,a.tipcuebanper as tipcuebanper,b.nomper as nomper,b.apeper as apeper,
	                     b.nacper as nacben, b.cedper, $adec_monto as monnetres
...
if ($ls_operacion == "NUEVO")
 ...
 if ($ls_operacion=="GUARDAR")
	{		
		...
		$lb_valido=$io_carord->uf_procesar_movbanco($ls_codban,$ls_cuenta_banco,$ls_numdoc,$ls_mov_operacion,$ld_fecha,$ls_desmov,$ls_codconmov,"----------","----------","-",$ldec_montomov,$ldec_monobjret,$ldec_montoret,$ls_chevau,$ls_estmov,0,0,'T','SCBCOR','',$ls_tipo,$ls_numcarord,"--",$ls_numordpagmin,$ls_codtipfon,$ls_numcontint);
		$lb_cheked=0;
		$ls_solicitudes="";
		for ($li_i=1;$li_i<=$li_totalRows;$li_i++)				
		    {
			  if (array_key_exists("chk".$li_i,$_POST))
			     { 
				   ...
				   {
				   		$li_aux2 = $li_i-1;
						$ls_codprobenaux = $_POST["txtcodproben".$li_aux2];
				   }
				   if($ls_codfuefin=="")
				   {
						$ls_codfuefin="--";
				   ...
				   if ($ls_tipo=='P')
				      {
					    ...
					    $ls_cedbene = "----------";
						/*print "Codigo del Porveedor---->  ".$ls_codpro."<br>";
						print "------>  ".$li_i."<br>"; 
						print "Codigo de Proveedor--->  ".$ls_codpro."<br>";
						print "Codigo de Proveedor Auxiliar--->  ".$ls_codprobenaux."<br>";*/
						if($li_cont==1) // Anteriormente estaba------> en el if $li_i==1
						{
							$lb_valido=$io_carord->uf_update_provbene_movimiento($ls_codemp,$ls_codban,$ls_cuenta_banco,$ls_numdoc,$ls_mov_operacion,$ls_codpro,$ls_cedbene);	
						}
						if (($li_cont>1)&&($ls_codpro!=$ls_codprobenaux)) //Anteriormente estaba------> en el if ($li_i > 1)&&($ls_codpro!=$ls_codprobenaux)&&($lb_cheked==0)
						{
							$lb_valido=$io_carord->uf_update_provbene_movimiento($ls_codemp,$ls_codban,$ls_cuenta_banco,$ls_numdoc,$ls_mov_operacion,"----------",$ls_cedbene);	
							$lb_cheked=1;
						}
				      }
				   else
				      {
						...
						if (($li_cont>1)&&($ls_cedbene!=$ls_codprobenaux)) //($li_i > 1)&&($ls_cedbene!=$ls_codprobenaux)&&($lb_cheked==0)
						{
							$lb_valido=$io_carord->uf_update_provbene_movimiento($ls_codemp,$ls_codban,$ls_cuenta_banco,$ls_numdoc,$ls_mov_operacion,$ls_codpro,"----------");
							$lb_cheked=1;	
						}
				      }
				   $lb_valido  = true;
				  ...
				  // print "Procedencia----->  ".$ls_procede."<br>";
				   if ($ls_procede=='SCVSOV')
					  {
...
///-----------------------------------------------------------------------------------------------------------------------------------					  
					  ...
					  //  $lb_valido     = $in_classmovbanco->uf_procesar_dt_contable($arr_movbco,$ls_cuenta_scg,'SCBCOR',$ls_desmov,$ls_numdoc,'H',$ldec_monto,$ldec_monobjret,false,'00000');
					    if ($lb_valido){
							...
							{
								$ls_ctaprovbene=$io_carord->uf_select_ctaprovbene($ls_tipo,$ls_codproben,&$as_codban,&$as_ctaban);
							      if($io_emiche->uf_verificar_sol_repchequenomina($ls_codemp,$ls_numsol))
									{
										$ls_ctaprovbene=$io_emiche->uf_cuenta_repchequenomina($ls_codemp);
									}
						    }
							     ...
							     if ($lb_valido){
									 ...
									 if ($ls_estsol=="C"){
									 	foreach($aa_dt_spgcxp as $dt_cxpspg){
												...
												$lb_valido      = $in_classmovbanco->uf_procesar_dt_gasto($ls_codban,$ls_cuenta_banco,$ls_numdoc,'ND',$ls_estmov,$ls_programa,$ls_cuentaspg,$ls_numsol,$ls_descripcion,'CXPSOP',$ld_monto_par,'PG',$ls_estcla);
											}
									 }else{
									 	if($ldec_monto<$ld_totpre){
								        	foreach($aa_dt_spgcxp as $dt_cxpspg){
												...
												$ls_programa    = $ls_codestpro1.$ls_codestpro2.$ls_codestpro3.$ls_codestpro4.$ls_codestpro5;
												$lb_valido      = $in_classmovbanco->uf_procesar_dt_gasto($ls_codban,$ls_cuenta_banco,$ls_numdoc,'ND',$ls_estmov,$ls_programa,$ls_cuentaspg,$ls_numsol,$ls_descripcion,'CXPSOP',$ld_monto_spg,'PG',$ls_estcla);
											}
										 }
	 ...
	 if ($lb_valido)
		    ...
		    if ($lb_valido)
			   {
					  ...
					  if ($li_numrows>0 && $li_tipvia=='1')
						 { 
						   ...
						   $lb_valido = $io_metodobanco->uf_metodo_banco($ls_ruta,$ls_nommetban,'','','',$ld_fecha,$ldec_montomov,$ls_cuenta_banco,$rs_datosbene,$ls_metban,'',0,'','',$la_seguridad);
		...
		else
		{
			...
			//$io_msg->message("No pudo registrarse el movimiento, ya se gener� una carta orden para esa solicitud!");
		}		
		uf_nuevo();			
	}
...
?>
  ...
  <table width="762" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
      ...
      <td height="22" style="text-align:right"><div align="center" style="display:none"><span style="text-align:left">No. Orden Pago Ministerio
  ...
  &nbsp;<a href="javascript:uf_catalogo_ordenes();"><img src="../shared/imagebank/tools15/buscar.gif" alt="Buscar Ordenes de Pago Ministerio..." width="15" height="15" border="0" title="Buscar Ordenes de Pago Ministerio..."></a></span></div></td>
    ...
    <tr>
      ...
      <?php
		...
		{
	  ?>
	  <td width="345" height="22" style="text-align:left">N� Control Interno
    ...
    <tr>
      ...
      <td height="22"><span style="text-align:left">Monto Retenido</span>
        <input name="txtretenido" type="text" id="txtretenido" value="<?php print number_format($ldec_montoret,2,",",".");?>" size="22" maxlength="22" style="text-align:right" readonly></td></tr>
    <tr>
      <td height="13">&nbsp;</td>
      <td height="13">&nbsp;</td>
      <td height="13">&nbsp;</td>
		   ...
		   if ((ls_numdoc!="")&&(ls_concepto!="")&&(ldec_monto>0) && (ls_fecha!="") && (ls_codban!="") && (ls_cuenta!="") && (li_totrows>0))
			  {
				if (li_tipvia=='1' && ls_metban=="")
				   {
					 alert("Debe seleccionar un M�todo a Banco !!!");
				   }
				else
				   {
			  ...
			  {
				alert("Complete todos los datos para poder registrar la Transferencia Bancaria !!!");
			  }	
		 }
	 }
  else
