...
class sigesp_scb_c_cmp_ret
{
	...
	function uf_crear_comprobante($ld_fecdesde,$ld_fechasta, $ls_tipo, $ls_codproben, &$ls_mensaje)
	{
		...
		elseif ($this->ls_retid=='0000000005')   
		{
			if (!$this->uf_rep_cur_pagos($ld_fecdesde, $ld_fechasta,$ls_tipo, $ls_codproben))// Busco pagos para Proveedor/Beneficiario
			    {
			      $ls_mensaje = "No hay pagos en el periodo para el Proveedor/Beneficiario $ls_codproben";
					    ...
					    if ($lb_valido)
					       {
							...
							{
								$this->msg->message("Ocurrio un error al generar el comprobante.");	
							} 
					   }				
                 }					
			}
	...
	{		
		$ls_tipret = " sigesp_deducciones.estretmun=1 ";
		$ls_codemp=$this->dat_emp["codemp"];				
		if($this->ls_retid=='0000000003')
		{
			$ls_sql=" SELECT scb_movbco_scg.codded as codded, sigesp_deducciones.porded as porded, sum(scb_movbco_scg.monto) as retenido, sum(scb_movbco_scg.monobjret) as moret 
			  		    FROM scb_movbco_scg ,sigesp_deducciones 
					   WHERE ".$ls_tipret." 
						 AND scb_movbco_scg.numdoc = '".$ls_numdoc."'
						 AND scb_movbco_scg.codemp = '".$ls_codemp."' 
						 AND sigesp_deducciones.codemp = '".$ls_codemp."'
					     AND scb_movbco_scg.codded = sigesp_deducciones.codded
					   GROUP BY scb_movbco_scg.codded, sigesp_deducciones.porded";
		}
		elseif($this->ls_retid=='0000000005')
		{
			$ls_tipret = " sigesp_deducciones.estretmil='1' ";
			$ls_sql=" SELECT scb_movbco_scg.codded as codded, sigesp_deducciones.porded as porded, sum(scb_movbco_scg.monto) as retenido, sum(scb_movbco_scg.monobjret) as moret 
			  		    FROM scb_movbco_scg ,sigesp_deducciones  
					   WHERE ".$ls_tipret." 
						 AND scb_movbco_scg.numdoc = '".$ls_numdoc."'
		...
		{
		   $ls_sql=" SELECT cxp_rd_deducciones.codded as codded, sigesp_deducciones.porded as porded, sum(cxp_rd_deducciones.monret) as retenido, sum(cxp_rd_deducciones.monobjret) as moret
			  		   FROM cxp_dt_solicitudes ,sigesp_deducciones , cxp_rd_deducciones  
			 		  WHERE cxp_dt_solicitudes.numsol = '".$ls_numdoc."'
					    AND cxp_dt_solicitudes.numrecdoc = cxp_rd_deducciones.numrecdoc
						AND cxp_rd_deducciones.codded = sigesp_deducciones.codded
						AND  ".$ls_tipret."
						AND cxp_dt_solicitudes.codemp='".$ls_codemp."' 
						AND sigesp_deducciones.codemp='".$ls_codemp."' 
						AND cxp_rd_deducciones.codemp='".$ls_codemp."'
					  GROUP BY cxp_rd_deducciones.codded, sigesp_deducciones.porded ";
		}
		$rs_result=$this->io_sql->select($ls_sql);
	...
	function uf_ccr_get_nro($ls_periodofiscal,&$ls_nrocomp)
	{
		...
		else
		{
		    ...
		    {
			   if ($this->ls_retid=='0000000005')
			   {
			   		$codigo  = $this->dat_emp["concommil"];
			   }
			   elseif($this->ls_retid=='0000000003')
			   {
					$codigo  = $this->dat_emp["concommun"];
			   }
			   $ls_nrocomp=$ls_agno.$ls_mes.$this->io_function->uf_cerosizquierda($codigo,8);
			   $this->io_sql->free_result($rs_result);
   			   return true;
		...
		else
		{
			...
			if($row=$this->io_sql->fetch_row($rs_result))
			{
				...
				{			
					...
					if( ($ldec_totimp==0)&&($lb_desagregar))
					{
	...
	function uf_ccr_guarda_inf_clc(&$lb_firsttime, $ls_periodofiscal, $ls_nrocomp, $arr_datos_probene, $ldec_ivaretenido, $ls_numsol, $ls_concepto, $ldec_porc, $ldec_mor,$ls_numdoc)
	{
		...
		if($lb_valido)
		{	
			if ($lb_firsttime==true){$ls_chance='VERDADERO';}
			else{$ls_chance='FALSO';}			
			$ls_num_ND = ' ';
			$ls_num_NC = ' ';
			$ls_tipotrans = '01-reg';
			$ls_num_fact_afec = ' ';
			...
			for($li_i=1;$li_i<=$li_total;$li_i++)
			{
				...
				$ls_sql="SELECT b.porded
				   ...
				   { 
			          $ldec_porcimpuesto=$this->ds_libcompra->getValue("porc_impuesto",$li_i);
					  $ldec_ivaretenido = ($ldec_baseimponible * $ldec_porcimpuesto)/100;
				   }
				if($ldec_ivaretenido > 0)
				{
