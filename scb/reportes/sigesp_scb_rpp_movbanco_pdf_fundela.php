<?php
	...
	function uf_print_encabezado_pagina($ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,&$io_pdf)
	{
		...
		$io_pdf->addText(483,760,9,"<b>Fecha: </b>"); // Agregar el título
		$io_pdf->addText(520,760,9,date("d/m/Y")); // Agregar la Fecha			
		 $io_pdf->Rectangle(15,575,570,117);
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
	}// end function uf_print_encabezadopagina.

	function uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$ld_fecmov,$ls_operacion,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_data=array(array('ordenes'=>'<b>Banco :</b>  '.$ls_nomban),
					   array('ordenes'=>'<b>Cuenta Nº :</b> '.$ls_ctaban),
	...
	function uf_print_autorizacion(&$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->setStrokeColor(0,0,0);		
		$io_pdf->Rectangle(11,90,580,58);
		$io_pdf->line(150,90,150,148);
		$io_pdf->line(300,90,300,148);
		$io_pdf->line(450,90,450,148);
				
		$io_pdf->addText(16,137.6,9,'<b>Elaborado por:</b>');
		$io_pdf->addText(190,137.6,9,'<b>Revisado por:</b>');
		$io_pdf->addText(343,137.6,9,'<b>Revisado por:</b>');
		$io_pdf->addText(493,137.6,9,'<b>Revisado por:</b>');
		$io_pdf->addText(343,92.6,9,'<b>Presupuesto</b>');		
		$io_pdf->addText(493,92.6,9,'<b>Administración</b>');		
		$io_pdf->addText(190,92.85,9,'<b>Contabilidad</b>');
...
for($li_i=1;$li_i<=$li_totrow;$li_i++)
{
	...
	$ls_numcarord   = $ds_voucher->data["numcarord"][$li_i];
	...
	if($ls_numcarord!=""){
		uf_print_cabecera($ls_numcarord,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$ld_fecmov,$ls_operacion,$io_pdf); // Imprimimos la cabecera del registro
	}
	else{
		uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$ld_fecmov,$ls_operacion,$io_pdf); // Imprimimos la cabecera del registro
	}
