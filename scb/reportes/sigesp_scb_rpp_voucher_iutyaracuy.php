<?php
	...
	header("Cache-Control: must-revalidate, post-check=0, pre-check=0");//IUETAEB
	...
	function uf_print_autorizacion(&$io_pdf)
	{
		...
		$io_pdf->line(140,90,140,148);
		$io_pdf->line(260,90,260,148);
		$io_pdf->line(410,90,410,148);
		
		$io_pdf->line(191,43,191,75);
		$io_pdf->line(310.5,43,310.5,75);
		$io_pdf->line(411,43,411,75);	
		
		$io_pdf->addText(16,137.6,9,'<b>Preparado por:</b>');
		$io_pdf->addText(160,137.6,9,'<b>Revisado por:</b>');
		$io_pdf->addText(300,137.6,9,'<b>Autorizado por:</b>');		
		$io_pdf->addText(450,137.6,9,'<b>Registrado por:</b>');		
	   ...
	   //Descompongo la cadena de operaciones en un arreglo tomando como separación el '-'
		...
		{	
			...
			for ($i=1;$i<=$li_total;$i++)
			{
				...
				if (trim($ls_codban)=="001")//Banco Corpbanca.
				   {
					 $ls_archivo="cheque_configurable/medidas_corpbanca.txt";
				...
				elseif(trim($ls_codban)=="004")//Banco Occidental de Descuento.
				   {
