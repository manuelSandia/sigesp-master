<?php
	...
	$li_medidas=16;
	//-------------------------------------------------------------------------------------------------
	function uf_inicializar_variables($as_archivo)
	{
		global $valores;
	...
	function uf_print_cabecera($ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,&$io_pdf)
	{
		...
		-----------------------------------------------------------------------------------------*/
		//banco
		$io_pdf->add_texto(20,86,10,"<b>$ls_nomban</b>");
		//Numero de cheque(solo mostrara los ultimos 8 digitos del numero)
		$io_pdf->add_texto(166,86,10,"<b>".substr($ls_numdoc,-8)."</b>");		
		//Numero Cuenta (solo mostrara los ultimos 20 digitos del numero)
		$io_pdf->add_texto(98,86,10,"<b>".substr($ls_ctaban,-23)."</b>");	
              //Orden de Pago (solo mostrara los ultimos 6 digitos del numero)
		$io_pdf->add_texto(140,99.5,10,"<b>".substr($ls_solicitudes,-6)."</b>");
		//Concepto
	...
	function uf_print_firmas(&$io_pdf) 
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//Presupuesto
		$io_pdf->add_texto(10,154,10,"<b>Elaborado por:</b>");
		//Contabilidad
		$io_pdf->add_texto(79,154,10,"<b>Revisado por:</b>");
	}
	   ...
	   //Descompongo la cadena de operaciones en un arreglo tomando como separación el '-'
		...
		{	
			...
			for ($i=1;$i<=$li_total;$i++)
			{
				...
				if ($ls_codban=='001')//Banco Industrial de Venezuela.
			   {
				 $ls_archivo="cheque_configurable/medidas_industrial.txt";
			   }
				elseif($ls_codban=='002')//Banco del Tesoro.
				   {
				...
				uf_inicializar_variables($ls_archivo);
