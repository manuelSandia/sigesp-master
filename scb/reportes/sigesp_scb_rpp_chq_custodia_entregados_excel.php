<?php
 header("Content-Type: application/vnd.ms-excel");
  ...
  header("content-disposition: attachment;filename=reporte_chq_Custodia_Entregados.xls");
   ...
   //--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_fecha,&$io_pdf)
	{
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $io_encabezado=$io_pdf->openObject();    //DARRELL
		$io_pdf->saveState();
		...
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');		  //FIN COD DARRELL
		  //$hora=date("h:i a"); $fecha_hoy=date("d/m/Y");
		  //echo "<table width='800' border='1'><tr><td>&nbsp;</td><td>&nbsp;</td></tr><tr align='center'><td>".$as_titulo."</td><td>".$fecha_hoy."</td></tr><tr align='center'><td>".$as_fecha."</td><td>".$hora."</td></tr></table>";
	...
	function uf_print_cabecera($la_data,&$io_pdf)
	{
		...
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);		  // FIN CODG DARRELL		
	...
	include("funciones_darrell_custodia.php");
//--------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------  Parámetros para Filtar el Reporte  --------------------------------------
    ...
    // Cargar el dts_cab con los datos de la cabecera del reporte( Selecciono todos comprobantes )		
	 ...
	 else // Imprimimos el reporte
	 {
		...
		dar_encabezado($ls_titulo,$ls_fecha);  //?????????????????????????????????????????????????????????????????????????? CODIGO DARRELL
		...
		for($z=1;$z<=$li_tot;$z++)
		{
			...
			$li_totdocumentos=$io_report->ds_documentos->getRowCount("codban"); /// esta variable es la bandera que indica que cuenta  corresponde al rango de fecha establecido			
			dar_cabecera($ls_nomban,$ls_ctaban,$ls_dencta,$li_totdocumentos);  //?????????????????????????????????????????????????????????????? CODIGO DARRELL
			if($li_altousado>230)			
			{
				 $io_pdf->ezNewPage();
			}
			if($li_totdocumentos>0)
			{
				...
				$ld_total_monto=0;
				unset($la_data);
				dar_detalle_sub(1); //?????????????????????????????????????????????????????????????? CODIGO DARRELL
				for($y=1;$y<=$li_totdocumentos;$y++)
				{
					...
					dar_detalle($ld_fecemi,$ls_numdoc,$ls_nomproben,$ldec_monto,$ls_estmov,$ld_fecvenc); //?????????????????????????????????????????????????????????? código darrell 
					if($li_estentrega==1)
					{
