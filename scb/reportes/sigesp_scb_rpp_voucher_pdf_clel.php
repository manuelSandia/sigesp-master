<?php
	...
	function uf_print_encabezado_pagina($ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,&$io_pdf)
	{
		...
		global $valores;
		//Imprimo el monto
	         $io_pdf->add_texto($valores[0],$valores[1],16,"<b>***".$ldec_monto."***</b>");
...
//						array('banco'=>$ls_nomban,'cheque'=>$ls_numdoc,'cuenta'=>$ls_ctaban,'voucher'=>$ls_chevau));
		$la_data=array(array('banco'=>'<b>Banco</b>  ','cuenta'=>'<b>Cuenta Nº:</b>  ','cheque'=>'<b>Cheque Nº</b>  ','voucher'=>'<b>Voucher Nº:</b>  '),
						array('banco'=>$ls_nomban,'cuenta'=>$ls_ctaban,'cheque'=>$ls_numdoc,'voucher'=>$ls_chevau));

		$la_columna=array('banco'=>'','cuenta'=>'','cheque'=>'','voucher'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>580,
						 'cols'=>array('banco'=>array('justification'=>'left','width'=>160),'cuenta'=>array('justification'=>'left','width'=>160),
						 'cheque'=>array('justification'=>'left','width'=>100),'voucher'=>array('justification'=>'left','width'=>160))); // Ancho Máximo de la tabla
...
//		$io_pdf->Rectangle(11,43,580,105);
		$io_pdf->Rectangle(11,43,580,120); //Del CLEL
		$io_pdf->line(11,90,590,90);
		$io_pdf->line(11,74.6,590,74.6);		
//		$io_pdf->line(127,90,127,148);
		$io_pdf->line(127,90,127,162); //Del CLEL
//		$io_pdf->line(243,90,243,148);
		$io_pdf->line(243,90,243,162); //Del CLEL
//		$io_pdf->line(359,90,359,148);
		$io_pdf->line(359,90,359,162); //Del CLEL	
//		$io_pdf->line(475,90,475,148);
		$io_pdf->line(475,90,475,162); //Del CLEL		
...
// ---------------------------  Del CLEL ------------------------------
		$io_pdf->addText(16,152,9,'<b>Tesorería:</b>');
		$io_pdf->addText(132,152,9,'<b>Contabilidad:</b>');
		$io_pdf->addText(248,152,9,'<b>Administración:</b>');		
		$io_pdf->addText(363,152,9,'<b>Dirección de Serv. Adm.:</b>');		
		$io_pdf->addText(480,152,9,'<b>Presidencia:</b>');
//------------------------------------------------------------------
