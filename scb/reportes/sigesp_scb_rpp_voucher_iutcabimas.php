<?php
	...
	function uf_print_encabezado_pagina($ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,&$io_pdf)
	{
		...
		$io_pdf->add_texto($valores[14],$valores[15],12,"<b>CADUCA A LOS ".$_SESSION["la_empresa"]["diacadche"]." DIAS</b>");		
	...
	function uf_print_autorizacion(&$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->setStrokeColor(0,0,0);		
		$io_pdf->Rectangle(11,59,580,105); //(11,43,580,105)

		$io_pdf->line(11,120,590,120);

		$io_pdf->line(11,90,590,90);
		$io_pdf->line(11,104.6,590,104.6); //(11,74.6,590,104.6)		
		$io_pdf->line(127,104.6,127,164); //(127,90,127,180)
		$io_pdf->line(243,104.6,243,164); //(243,90,243,148)
		$io_pdf->line(359,104.6,359,164); //(359,90,359,148)
		$io_pdf->line(475,104.6,475,164); //(475,90,475,148)		
		$io_pdf->line(191,59,191,90); //(191,43,191,75)
		$io_pdf->line(310.5,59,310.5,90); //(310.5,43,310.5,75)
		$io_pdf->line(411,59,411,90);	//(411,43,411,75)
		
		$io_pdf->addText(16,150.6,9,'<b>Elab. Rev.:</b>');
		$io_pdf->addText(132,150.6,9,'<b>Verificada por:</b>');
		$io_pdf->addText(273,150.6,9,'<b>Certificada</b>');		
		$io_pdf->addText(384,150.6,9,'<b>Conformada</b>');		
		$io_pdf->addText(508,150.6,9,'<b>Autorizada</b>');

		$io_pdf->addText(16,110.6,9,'<b>Administración</b>');
		$io_pdf->addText(132,110.6,9,'<b>Control Previo</b>');
		$io_pdf->addText(273,110.6,9,'<b>Presupuesto</b>');		
		$io_pdf->addText(384,110.6,9,'<b>Sub.Dire. Admint.</b>');		
		$io_pdf->addText(508,110.6,9,'<b>Direción</b>');




		$io_pdf->addText(258,95.85,10,'<b>Recibí Conforme</b>');
		
		$io_pdf->addText(16,80.27,10,'<b>Nombre:</b>');		
		$io_pdf->addText(196,80.27,10,'<b>Cédula Identidad:</b>');		
		$io_pdf->addText(316,80.27,10,'<b>Fecha:</b>');
		$io_pdf->addText(416,80.27,10,'<b>Firma:</b>');
		//$io_pdf->restoreState();
		//$io_pdf->closeObject();
	...
	if ($ls_tipimp=='lote')
	{
	   ...
	   //Descompongo la cadena de operaciones en un arreglo tomando como separación el '-'
		...
		{	
			...
			$io_pdf->selectFont('../../shared/ezpdf/fonts/Courier-Bold.afm'); // Seleccionamos el tipo de letra
			$io_pdf->set_margenes(0,40,0,0);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	else 
	{
		...
		$io_pdf->transaction('start'); // Iniciamos la transacción
		$thisPageNum=$io_pdf->ezPageCount;
//		$io_pdf->ezStartPageNumbers(570,47,10,'','',1); // Insertar el número de página
		uf_print_autorizacion($io_pdf);	
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			unset($la_data);
