<?php
	...
	function uf_print_encabezado_pagina_cheque($ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->ezText('                     ',100);//Inserto una linea en blanco
		//$io_pdf->ezNewPage();
	}// end function uf_print_encabezadopagina.

	function uf_print_encabezado_pagina($ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,$ls_nomban,$ls_ctaban,$ls_numdoc,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_encabezado_pagina
		//		    Acess: private 
		...
		//Imprimo el monto
		$linea=30;	
		$io_pdf->addText(106,750,14,'<b>COMPROBANTE DE PAGO</b>');
		$io_pdf->addText(20,730,9,'<b>Banco: </b>'.$ls_nomban);
		$io_pdf->add_texto($valores[0]-130,$valores[1]+$linea,9,"<b>CTA.CTE Nro.: ".$ls_ctaban."</b>");
		$io_pdf->add_texto($valores[0]-50,$valores[1]+$linea,9,"<b>Nro.: ".$ls_numdoc."</b>");
		$io_pdf->add_texto($valores[0],$valores[1]+$linea,9,"<b>Bs.***".$ldec_monto."***</b>");
		//Beneficiario del Cheque
		$io_pdf->add_texto($valores[2]-30,$valores[3]+$linea,9,"<b>PAGUESE A LA ORDEN DE : </b>");
		$io_pdf->add_texto($valores[2]+15,$valores[3]+$linea,9,"<b>".$ls_nomproben."</b>");
		//Monto en letras del Cheque
		//Cortando el monto en caso de que sea muy largo		
		$ls_monto_cortado=wordwrap($ls_monto,70,"?");
		$la_arreglo=array();
		$la_arreglo=explode("?",$ls_monto_cortado);
		$io_pdf->add_texto($valores[4]-30,$valores[5]+$linea,9,"<b>LA CANTIDAD DE :</b>");
		if(array_key_exists(0,$la_arreglo))
			$io_pdf->add_texto($valores[4],$valores[5]+$linea,9,"<b>".$la_arreglo[0]."</b>");
		if(array_key_exists(1,$la_arreglo))
			$io_pdf->add_texto($valores[6],$valores[7]+$linea,9,"<b>$la_arreglo[1]</b>");
		$ls_anio=substr($ls_fecha,-4);
		$ls_fecha_corta=substr($ls_fecha,0,(strlen($ls_fecha)-5));	
		//Fecha del Cheque
		$io_pdf->add_texto($valores[8],$valores[9]+$linea,9,"<b>$ls_fecha_corta</b>");
		$io_pdf->add_texto($valores[10],$valores[11]+$linea,9,"<b>$ls_anio</b>");	
		//$io_pdf->add_texto($valores[12],$valores[13]+$linea,9,"<b>NO ENDOSABLE</b>");
		//$io_pdf->add_texto($valores[14],$valores[15]+$linea,9,"<b>CADUCA A LOS ".$_SESSION["la_empresa"]["diacadche"]." DIAS</b>");		
	}// end function uf_print_encabezadopagina.
	...
	function uf_print_autorizacion(&$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->setStrokeColor(0,0,0);		
		$io_pdf->Rectangle(11,43,580,115);
		$io_pdf->line(11,90,590,90);
		$io_pdf->line(11,74.6,590,74.6);		
		$io_pdf->line(130,90,130,158); //Vertical
		$io_pdf->line(270,90,270,158); //Vertical
		$io_pdf->line(423,90,423,158); //Vertical
		$io_pdf->line(191,43,191,75);
		$io_pdf->line(11,127,590,127);
		$io_pdf->line(310.5,43,310.5,75);
		$io_pdf->line(411,43,411,75);	
		
		$io_pdf->addText(36,147.6,8,'<b>ELABORADO</b>');
		$io_pdf->addText(172,147.6,8,'<b>REVISADO</b>');
		$io_pdf->addText(276,147.6,8,'<b>COORDINACIÓN DE PLANIFICACIÓN</b>');
		$io_pdf->addText(276,138,8,'<b>PRESUPUESTO, ORGANIZACIÓN Y</b>');
		$io_pdf->addText(308,130,8,'<b> SISTEMAS</b>');
		$io_pdf->addText(449,147.6,8,'<b>DIRECCIÓN DE GESTIÓN</b>');
		$io_pdf->addText(464,137,8,'<b>ADMINISTRATIVA</b>');		
		...
		{	
			...
			for ($i=1;$i<=$li_total;$i++)
			{
				...
				for($li_i=1;$li_i<=$li_totrow;$li_i++)
				{
					...
					uf_print_encabezado_pagina_cheque(number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$_SESSION["la_empresa"]["ciuemp"].", ".$ld_fecmov,$io_pdf); // Imprimimos el encabezado de la página
					uf_print_encabezado_pagina(number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$_SESSION["la_empresa"]["ciuemp"].", ".$ld_fecmov,$ls_nomban,$ls_ctaban,$ls_numdoc,$io_pdf); // Imprimimos el encabezado de la página
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	else 
	{
		...
		//uf_print_autorizacion($io_pdf);	
			...
			$ls_repmovbco	 = "sigesp_scb_rpp_voucher_pdf_saime_cheque.php?codban=".$ls_codban."&ctaban=".$ls_ctaban."&numdoc=".$ls_numdoc."&chevau=".$ls_chevau."&codope=CH";
			$sc_name_path=''.$ls_repmovbco;
			$ls_repmovbco2	 = "sigesp_scb_rpp_voucher_pdf_saime_detalle.php?codban=".$ls_codban."&ctaban=".$ls_ctaban."&numdoc=".$ls_numdoc."&chevau=".$ls_chevau."&codope=CH";
			$sc_name_path2=''.$ls_repmovbco2;
			//---> muestra el pdf en un ventana adicional
			print('<script language=JavaScript>');
			print(" window.open('$sc_name_path','$ls_repmovbco','menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,left=0,top=0,location=no,resizable=yes');");
			print(" window.open('$sc_name_path2','$ls_repmovbco2','menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,left=0,top=0,location=no,resizable=yes');");
			print('close();');
			print('</script>');
