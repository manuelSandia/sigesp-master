...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	else 
	{
		...
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			$ls_solicitudes = $class_report->uf_select_solicitudes($ls_numdoc,$ls_codban,$ls_ctaban);
			$ls_codguard 	= $class_report->uf_select_codguarderias($ls_cedbene,$ls_solicitudes);
			$ls_codguard	= substr($ls_codguard,3,4);
			$ls_codguard=$io_funciones->uf_cerosizquierda($ls_codguard,10);
			if ($ls_codguard!='')
			{
				$ls_cedbene=trim($ls_cedbene);
				$ls_cedbeneaux=str_pad($ls_cedbene,10,"0",STR_PAD_LEFT);
				$ls_benefaux 	= $class_report->uf_select_guarderias($ls_cedbeneaux,$ls_codguard);
				if ($ls_benefaux!="")
				{
					$ls_nomproben=$ls_benefaux;
				}
			}
			$ls_conmov		= $ds_voucher->getValue("conmov",$li_i);
			$ldec_monret	= $ds_voucher->getValue("monret",$li_i);
