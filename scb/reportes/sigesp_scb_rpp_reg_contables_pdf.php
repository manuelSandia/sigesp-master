<?php
	...
	if($li_total>0)
	{
		...
		for($i=1;$i<=$li_total;$i++)
		{
			...
			if ($ls_codope=='CH')
			{
				$ldec_monto	  = $io_report->uf_buscavalorchk($ls_numdoc,$ls_codban,$ls_ctaban);
			}
			else
			{
			...
			$ds_dt_spg->data=$io_report->uf_cargar_dt_spg($ls_numdoc,$ls_codban,$ls_ctaban,$ls_codope,$ls_estmov);
