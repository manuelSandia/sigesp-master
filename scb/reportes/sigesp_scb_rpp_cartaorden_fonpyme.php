<?php
	...
	function uf_print_encabezado_pagina($as_numdoc,$as_fecha,&$io_pdf)
	{
		...
		//Fecha actual
		$io_pdf->add_texto(118,4.5,9,"<b>".$as_fecha."</b>");
		
		
		//pie de pagina
		$io_pdf->line(20,90,580,90);
		$ls_direccion1='Avenida Fransisco de Miranda, Torre Europa, Piso 7, El Rosal, Municipio Chacao';
	...
	if(!array_key_exists("opener",$_GET))
	{
		...
		$ls_tipproben =	$_GET["tipproben"];
		$ls_solicitudes=$_GET["solicitud"];
		$la_solicitudes=split('--',$ls_solicitudes);
		$li_cnsol=count($la_solicitudes);
		...
		$li_cont=count($la_cartaorden["monto"]);
		$li_totales=0;
		for ($i=1;$i<=$li_cont;$i++)
		{
		...
		if($la_cartaorden["tipo_destino"][1]=='P')
		{
			...
			$la_campo["rif"] ="";
			$class_report->uf_select_banco_cuenta("rpc_proveedor",$ls_criterio,&$la_campo["bancoemisor"],&$la_campo["cuentabancariaemisor"],&$la_campo["rif"]);
		}
		else
		{
			...
			$la_campo["nombre"] = $la_cartaorden["nomproben"][1];
			$class_report->uf_select_banco_cuenta("rpc_beneficiario",$ls_criterio,&$la_campo["bancoemisor"],&$la_campo["cuentabancariaemisor"],&$la_campo["rif"]);
		}
