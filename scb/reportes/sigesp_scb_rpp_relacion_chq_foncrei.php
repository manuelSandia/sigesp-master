<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$ls_periodo,$ls_denban,$ls_ctaban,$ls_dencta,&$io_pdf)
	{
		...
		$io_pdf->addText(50,738,10,'<b>Banco:</b>'.$ls_denban); // Agregar la Fecha
		$io_pdf->addText(50,725,10,'<b>Cuenta:</b>'.$ls_ctaban."   ".$ls_dencta); // Agregar la Fecha
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	$ls_titulo="<b>Relacion de Cheques</b>";
   ...
   //Descompongo la cadena de documentos en un arreglo tomando como separación el '-'
	...
	$class_report->uf_cargar_documentos_relacion($arr_documentos,$arr_fecmov,$arr_operaciones,$ls_codban,$ls_ctaban);
	$ld_totdeb  = 0;
	$ld_totcre  = 0;
	$ldec_saldo = 0;
	$lb_valido  = true;
	$li_total   = $class_report->ds_documentos->getRowCount("numdoc");//print "Total =>".$li_total.'<br>';
	if ($li_total>0)
	   {
		 ...
		 uf_print_encabezado_pagina($ls_titulo,'',$ls_denban,$ls_ctaban,$ls_dencta,$io_pdf); // Imprimimos el encabezado de la página
