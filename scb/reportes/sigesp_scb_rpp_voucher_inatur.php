...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	function uf_print_encabezado_pagina($ls_ctaban,$ls_nomban,$ls_codtipcta,$ls_codope,$ldec_monto,$ls_nomproben,$ls_monto,$ls_fecha,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		else
		{
			...
			$io_pdf->add_texto(177,19,8,"<b>Fecha:   </b>".$ld_fechaact);
			$io_pdf->add_texto(177,12,8,"<b>Pág.   </b>");
			$io_pdf->add_texto(14,12,10,"<b>INSTITUTO NACIONAL DE TURISMO</b>");
			$io_pdf->add_texto(14,20,9,"<b>Banco                :</b>  ".$ls_nomban);
			$io_pdf->add_texto(14,25,9,"<b>Tipo Cuenta      :</b>   ".$ls_tipo_cta);
			$io_pdf->add_texto(14,30,9,"<b>Cuenta Banco   :</b>  ".$ls_ctaban);
			$io_pdf->add_texto(88,42,10,"<b>LISTADO DE:  $ls_codope</b>");
		}
	}// end function uf_print_encabezadopagina.
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	function uf_print_cabecera($ldec_total,$ld_fecmov,$ls_codope,$ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		else
		{
			...
			$io_pdf->convertir_valor_mm_px($li_pos);
			$io_pdf->ezSetY($li_pos);
			$la_data=array(array('banco'=>'<b>Nro. Documento</b>  ','cheque'=>'<b>Fecha</b>  ','cuenta'=>'<b>Beneficiario</b>  ','voucher'=>'<b>Monto St</b>  '),
							array('banco'=>$ls_numdoc,'cheque'=>$ld_fecmov,'cuenta'=>$ls_nomproben,'voucher'=>$ldec_total."  C"));
			$la_columna=array('banco'=>'','cheque'=>'','cuenta'=>'','voucher'=>'');
			$la_config=array('showHeadings'=>0, // Mostrar encabezados
							 ...
							 'maxWidth'=>580,
							 'cols'=>array('banco'=>array('justification'=>'left','width'=>160),'cheque'=>array('justification'=>'left','width'=>100),
							 'cuenta'=>array('justification'=>'left','width'=>200),'voucher'=>array('justification'=>'right','width'=>120))); // Ancho Máximo de la tabla
			$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
			$io_pdf->ezSetY(510);
	...
	function uf_print_detalle($ldec_total,$ls_codope,$la_title,$la_data,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		else
		{
			...
			$la_data_title=array($la_title);
			$io_pdf->set_margenes(90,55,0,0);
			$io_pdf->line(180,380,280,380);
			$io_pdf->line(300,380,400,380);
			$ldec_total=number_format($ldec_total,2,",",".");
			$io_pdf->addText(205,360,9,"<b>$ldec_total</b>");
			$io_pdf->addText(325,360,9,"<b>$ldec_total</b>");
			//$io_pdf->add_texto(177,19,8,"<b>Fecha:   </b>".$ld_fechaact);
			$la_columna=array('title'=>'','title2'=>'');
			$la_config=array('showHeadings'=>0, // Mostrar encabezados
							 'showLines'=>0, // Mostrar Líneas
							 'shaded'=>0, // Sombra entre líneas
							 'shadeCol2'=>array(0.9,0.9,0.9), // Color de la sombra
							 'xPos'=>73, // Orientación de la tabla
							 'width'=>580, // Ancho de la tabla
							 'maxWidth'=>580,
							 'cols'=>array('title'=>array('justification'=>'center','width'=>20),'title2'=>array('justification'=>'center','width'=>230))); // Ancho Máximo de la tabla
			$io_pdf->ezTable($la_data_title,$la_columna,'',$la_config);	
			//Imprimo los detalles tanto `de presupuesto como contablwe del movimiento
			$io_pdf->ezSetDy(-10);
			$la_config=array('showHeadings'=>1, // Mostrar encabezados
							 ...
							 'maxWidth'=>580, // Ancho Máximo de la tabla
							 'xOrientation'=>'center', // Orientación de la tabla
							 'cols'=>array('scg_cuenta'=>array('justification'=>'left','width'=>80), // Justificación y ancho de la columna
										   'debe'=>array('justification'=>'right','width'=>115), // Justificación y ancho de la columna
										   'haber'=>array('justification'=>'right','width'=>115), // Justificación y ancho de la columna
										   'denominacion'=>array('justification'=>'right','width'=>175))); // Justificación y ancho de la columna
			$la_columnas=array('scg_cuenta'=>'<b>Cuenta</b>',
							   'debe'=>'<b>Debe</b>',
							   'haber'=>'<b>Haber</b>',
							   'denominacion'=>'');
			$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	else 
	{
		...
		{
			$io_pdf->ezStartPageNumbers(570,750,8,'','',1); // Insertar el número de página
		}
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			unset($la_data);
			...
			$ldec_monret	= $ds_voucher->getValue("monret",$li_i);
			$ldec_monto		= $ds_voucher->getValue("monto",$li_i);
			$ls_codtipcta	= $ds_voucher->data["codtipcta"][$li_i];
			$ldec_total		= $ldec_monto-$ldec_monret;
			//Asigno el monto a la clase numero-letras para la conversion.
			$numalet->setNumero($ldec_total);
			//Obtengo el texto del monto enviado.
			$ls_monto= $numalet->letra();
			uf_print_encabezado_pagina($ls_ctaban,$ls_nomban,$ls_codtipcta,$ls_codope,number_format($ldec_total,2,",","."),$ls_nomproben,$ls_monto,$_SESSION["la_empresa"]["ciuemp"].", ".$ld_fecmov,$io_pdf); // Imprimimos el encabezado de la página
			uf_print_cabecera($ldec_total,$ld_fecmov,$ls_codope,$ls_numdoc,$ls_nomban,$ls_ctaban,$ls_chevau,$ls_nomproben,$ls_solicitudes,$ls_conmov,$io_pdf); // Imprimimos la cabecera del registro
				 ...
				 for ($li_s=1;$li_s<=$li_totrow_det;$li_s++)
					 {
					   ...
					   $ls_debhab     = $ds_dt_scg->data["debhab"][$li_s];
					   $ldec_monto    = $ds_dt_scg->data["monto"][$li_s];
					   $ls_denominacion_scg    = $ds_dt_scg->data["denominacion"][$li_s];
					   if ($ls_debhab=='D')
						  {
							$ldec_mondeb = number_format($ldec_monto,2,",",".");
							$ldec_monhab = "";
					   ...
					   $la_data[$li_s]=array('spg_cuenta'=>$ls_cuentaspg,'estpro'=>$ls_estpro,'monto_spg'=>$ldec_monto_spg,'scg_cuenta'=>$ls_scg_cuenta,'debe'=>$ldec_mondeb,'haber'=>$ldec_monhab,'denominacion'=>$ls_denominacion_scg);
					 }
			   }
			if ($li_totrow_spg>$li_totrow_det)
			   {
			...
			if (empty($la_data))
			   {
				 ...
				 $ldec_mondeb	 = '';
				 $ldec_monhab	 = '';
				 $la_data[1]=array('spg_cuenta'=>$ls_cuentaspg,'estpro'=>$ls_estpro,'monto_spg'=>$ldec_monto_spg,'scg_cuenta'=>$ls_scg_cuenta,'debe'=>$ldec_mondeb,'haber'=>$ldec_monhab,'denominacion'=>$ls_denominacion_scg);
				 $la_data[2]=array('spg_cuenta'=>$ls_cuentaspg,'estpro'=>$ls_estpro,'monto_spg'=>$ldec_monto_spg,'scg_cuenta'=>$ls_scg_cuenta,'debe'=>$ldec_mondeb,'haber'=>$ldec_monhab,'denominacion'=>$ls_denominacion_scg);
				 $la_data[3]=array('spg_cuenta'=>$ls_cuentaspg,'estpro'=>$ls_estpro,'monto_spg'=>$ldec_monto_spg,'scg_cuenta'=>$ls_scg_cuenta,'debe'=>$ldec_mondeb,'haber'=>$ldec_monhab,'denominacion'=>$ls_denominacion_scg);
			   }
			if (($ls_codope!='ND')&&($ls_codope!='NC'))
			{
				uf_print_detalle($ldec_total,$ls_codope,array('title'=>'Detalle Presupuestario Pago','title2'=>'Detalle Contable Pago'),$la_data,$io_pdf);
			}
			else
			{
				uf_print_detalle($ldec_total,$ls_codope,array('title'=>'','title2'=>'<b>Contabilidad</b>'),$la_data,$io_pdf);
			}	
		}
		$io_pdf->ezStopPageNumbers(1,1);
		$io_pdf->ezStream();
