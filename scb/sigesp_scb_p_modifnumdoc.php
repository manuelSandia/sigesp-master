...
$io_fun_banco->uf_load_seguridad("SCB","sigesp_scb_p_modifnumdoc.php",$ls_permisos,$la_seguridad,$la_permisos);
$li_diasem = date('w');
switch ($li_diasem){
		...
		for($li_i=1;$li_i<=$li_total_record;$li_i++)
		{
			if(array_key_exists("chksel".$li_i,$_POST))
			{
				...
				if($ls_estcon==0)
				{
					...
					$lb_valido = $in_movbco->uf_actualiza_documento($ls_docum,$ls_codban,$ls_ctaban,'',$ls_estmov,$ld_fecha,$ls_numdocnew);
					if($lb_valido)		
					{
						$in_movbco->msg->message("El N° de Documento $ls_docum fue cambiado");
					}
					else
					{
						$in_movbco->msg->message("Error en el cambio del número de documento $ls_docum.");
					}
				}
				else
				{
					$in_movbco->msg->message("El $ls_docum no puede ser cambiado, ya fue Conciliado");
				}		
			}
		}
...
<p>&nbsp;</p>
  <table width="502" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td height="22" colspan="4">Modificaci&oacute;n de Movimientos de Bancos </td>
    </tr>
    <tr>
      <td colspan="4">&nbsp;</td>
    </tr>
    <tr>
      <td width="286"><div align="left">N&uacute;mero de Documento a Buscar</div></td>
      <td width="251"><div align="left">
        <input name="txtnumdoc" type="text" id="txtnumdoc" onBlur="rellenar_cad(this.value,15,'doc1')" style="text-align:center" value="<?php print $ls_numdoc;?>" size="20" maxlength="15">
      </div></td>
	...
	<td width="236" height="28"><div align="left">Nuevo N&uacute;mero de Documento</div></td>
      <td width="251" height="28"><div align="left">
        <input name="txtnumdocnew" type="text" id="txtnumdocnew" onBlur="rellenar_cad(this.value,15,'doc')" style="text-align:center" value="<?php print $ls_numdocnew;?>" size="20" maxlength="15">
      </div></td>
	<tr>
    ...
    <tr>
	  ...
	  <?php
		 ...
		 {
	     	$in_class_contabiliza->uf_select_banco_contabilizar_documento( $ls_operacion_bco, $arr_object ,$li_total_record,"N",$ls_numdoc,$ld_fecha);
...
{
	f=document.form1;
	ls_numdocnew=f.txtnumdocnew.value;
	li_ejecutar=f.ejecutar.value;
	if(li_ejecutar==1)
	{
		if (ls_numdocnew=="")
		{
			alert("Debe colocar el nuevo numero de documento!");
	    }
		else
		{
			if(confirm("Está seguro de cambiar este(os) registro(s)?\n  Esta operación no puede reversarse"))
			{
				...
				f.action="sigesp_scb_p_modifnumdoc.php";
				f.submit();
			}
		}
	}
...
function rellenar_cad(cadena,longitud,campo)
{
	if (cadena!="")
	{
		...
		if(campo=="doc")
		{
			document.form1.txtnumdocnew.value=cadena;
		}
		if(campo=="doc1")
		{
			document.form1.txtnumdoc.value=cadena;
		}
