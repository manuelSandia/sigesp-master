...
$ls_reporteanulado = $io_fun_banco->uf_select_config("SCB","REPORTE","CHEQUE_VOUCHER_ANULADO","sigesp_scb_rpp_voucher_anulado.php","C");
$ls_report         = $io_fun_banco->uf_select_config("SCB","REPORTE","CARTA_ORDEN","sigesp_scb_rpp_cartaorden_pdf.php","C");
$ls_reporte_mov    = $io_fun_banco->uf_select_config("SCB","REPORTE","MOV_BANCO","sigesp_scb_rpp_movbanco_pdf.php","C");//print $ls_reporte_mov;
$li_diasem = date('w');
 //revision
switch ($li_diasem){
...
<?php
	...
	if( array_key_exists("operacion",$_POST))
	{
		...
		if(array_key_exists("hidconnocontabi",$_POST))
		{			
			$ls_connocontabi=$_POST["hidconnocontabi"]; 	
		}
		if(array_key_exists("txtnumconint",$_POST))
		{			
		...
		if(array_key_exists("chkcobranza",$_POST))
		...
		{   $lb_nocontab="";   }
		if(array_key_exists("chktranoreglib",$_POST))
		{   $lb_transnoreglib="checked";  }
		else
		{   $lb_transnoreglib="";   }
	...
	function uf_nuevo()
	{
		...
		global $lb_nocontab;
		$lb_nocontab="";
		global $lb_transnoreglib;
		$lb_transnoreglib="";
		global $li_estint;
		$li_estint=0;
		global $li_estcob;
		$li_estcob=0;
		global $li_esttranoreglib;
		$li_esttranoreglib=0;
		$array_fecha=getdate();
		$ls_dia=$array_fecha["mday"];
		$ls_mes=$array_fecha["mon"];
	...
	$title[1]="Cuenta";        $title[2]="Program�tico";    $title[3]="Documento";      $title[4]="Descripci�n";   $title[5]="Procede";        $title[6]="Operaci�n";  $title[7]="Monto";       $title[8]="Edici�n";
    $titleSpi[1] = "Cuenta";
	if ($li_estpreing==1)
	   {
		 $titleSpi[2] = "Program�tico";
		 $titleSpi[3] = "Documento";
		 $titleSpi[4] = "Descripci�n";
		 $titleSpi[5] = "Procede";
		 $titleSpi[6] = "Operaci�n";
		 $titleSpi[7] = "Monto";
		 $titleSpi[8] = "Edici�n";	   
	   }
	else
	   {
		 $titleSpi[2] = "Descripci�n";
		 $titleSpi[3] = "Procede";
		 $titleSpi[4] = "Documento";
		 $titleSpi[5] = "Operaci�n";
		 $titleSpi[6] = "Monto";
		 $titleSpi[7] = "Edici�n";
	   }
	$gridSpi="grid_Spi";
		...
		if ($ls_codigo!="000")
		   {
			...
			$ls_pagina="reportes/sigesp_scb_rpp_voucher_pdf.php?codban=$ls_codban&ctaban=$ls_cuenta_banco&numdoc=$ls_docmov&chevau=&codope=ND";			
		?>
		<script language="javascript">						
  ...
  if ($ls_operacion == "ELIMINAR")
	 {
	   ...
	   if ($ls_mov_operacion=='DP' || $ls_mov_operacion=='NC')
	      {
			$ls_numordpagmin=trim($ls_numordpagmin);
...
if ($ls_operacion=="DELETESPG")
  ...
  if($ls_operacion=="DELETESPI")
	...
	if($ls_operacion == "CAMBIO_OPERA" || $ls_operacion == "CAMBIO_CONCEPTO")//MODIFICADO POR OFIMATICA DE VENEZUELA EL 01-06-2011 SE AGREGA OPERACION CAMBIO_CONCEPTO
	{
		uf_cargar_dt();	
	}
	...
	if($ls_mov_operacion=='CH')
	{
		...
		$lb_re="";
		$lb_ch="selected";
		if(($ls_operacion=="NUEVO")||($ls_operacion=="CAMBIO_OPERA")||($ls_operacion=="CAMBIO_CONCEPTO"))//MODIFICADO POR OFIMATICA DE VENEZUELA EL 01-06-2011 SE AGREGA OPERACION CAMBIO_CONCEPTO
		{
...
?>
  ...
  <table width="780" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
	  ...
	  <input name="hidconnocontabi" type="hidden" id="hidconnocontabi" value="<?php echo $ls_connocontabi; ?>"></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td colspan="3" style="text-align:left"><input name="txtcodban" type="text" id="txtcodban"  style="text-align:center" value="<?php print $ls_codban;?>" size="10" readonly>
        ...
        <input name="txtdenban" type="text" id="txtdenban" value="<?php print $ls_denban?>" size="51" class="sin-borde" readonly>      </td><td width="95" style="text-align:right">Fecha</td>
...
-->	<tr>
    ...
    <tr>
      ...
      <td colspan="3" style="text-align:left"><input name="txtdocumento" type="text" id="txtdocumento" style="text-align:center" onBlur="rellenar_cad(this.value,15,'doc')" value="<?php print $ls_docmov;?>" size="24" maxlength="15" <?php print $ls_readOnly_doc; ?>>
       <?php
		if((($ls_mov_operacion=="NC")||($ls_mov_operacion=="ND")||($ls_mov_operacion=="DP"))&&($ls_contintmovban==1))
		{
	   ?>
	  N&deg; de Control Interno 
    ...
    <tr>
      ...
      <td colspan="5" style="text-align:left"><input name="txtconcepto" type="text" id="txtconcepto" value="<?php print $ls_desmov;?>" size="122" maxlength="254" onKeyPress="return keyRestrict(event,'0123456789'+'abcdefghijklmnopqrstuvwxyz� .,*/-()$%&!�������[]{}<>')"></td>
    </tr>
	<tr>
      <td height="22" style="text-align:right">Concepto Anulaci&oacute;n</td>
	  <td colspan="5" style="text-align:left"><input name="txtconceptoanu" type="text" id="txtconceptoanu" value="<?php print $ls_desmovanu;?>" size="122" maxlength="254" readonly></td>
    </tr>
    <tr>
...
<input name="txtmonto" type="text" id="txtmonto" style="text-align:right" onBlur="javascript:uf_montoobjret(this);javascript:uf_verificar_monto(this);" onKeyPress="return(currencyFormat(this,'.',',',event));" value="<?php print number_format($ldec_montomov,2,",",".");?>" size="24" maxlength="22">	  </div></td>
    ...
    <tr>
      ...
      <td><div align="right">
          <?php
			if(($ls_mov_operacion=="NC")||($ls_mov_operacion=="ND")||($ls_mov_operacion=="DP"))
			{
			?>
		  <input name="chktranoreglib" type="checkbox" class="sin-borde" <?php print $lb_chktranoreglib;?> id="chktranoreglib" style="width:15px; height:15px" onClick="uf_selec_tranoreglib(this);" value="1">
      		 <?php	
			}	
			?>
		 <input name="esttranoreglib" type="hidden" id="esttranoreglib" value="<?php print $li_esttranoreglib;?>">
	  </div></td>
      <?php
			if(($ls_mov_operacion=="NC")||($ls_mov_operacion=="ND")||($ls_mov_operacion=="DP"))
			{
			?>
	  <td style="text-align:left"><div align="left">Transacciones no Registradas en libro </div></td>
    ...
    <tr>
      <td height="22" colspan="6"><div align="center">
        ...
        <?php $io_grid->makegrid($li_row,$title2,$objectScg,770,'Detalles Contable',$grid2);?>
		  ...
		  <input name="hidnumcont" type="hidden" id="hidnumcont" value="<?php print $ls_contintmovban; ?>">
		  <input name="repmovban" type="hidden" id="repmovban" value="<?php print $ls_reporte_mov; ?>">
      </div></td>
    </tr>
    <tr>
  ...
  <?php
	...
	if(($ls_operacion=="SELECT_CONCEPTO_CTA")&&($ls_casacon=="1"))
	{
		require_once("../shared/class_folder/class_sql.php");
		require_once("../shared/class_folder/sigesp_include.php");
		...
		$ls_codcon =	$_POST["opepre"];
		$ls_totalbancos=$in_classmovbco->uf_total_bancos($ls_codcon,'','');
		if ($ls_totalbancos >1)
		{
			$ls_pagina="sigesp_cat_bancos.php?codcon=$ls_codcon";
			?>
			<script language="javascript">						
			window.open('<?php print $ls_pagina; ?>',"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=583,height=400,left=50,top=50,location=no,resizable=yes");
			</script>
		...
		elseif ($ls_totalbancos==1)
		{
			$rs_data_ban=$in_classmovbco->uf_bancos_ctaconcepto($ls_codcon,'','');
			while (!$rs_data_ban->EOF)
			   {        
				 $li_row++;
				 $ls_codban	 = $rs_data_ban->fields["codban"];
				 $ls_nomban  = $rs_data_ban->fields["nomban"];
				 $rs_data_ban->MoveNext();
			     $ls_totalcuentas=$in_classmovbco->uf_total_cuentas($ls_codcon,$ls_codban,'','');
			   }
			   ?>
			   <script language="javascript">						
			   f = document.form1;
			   f.txtcodban.value="<?php print $ls_codban; ?>";
			   f.txtdenban.value="<?php print $ls_nomban; ?>";
			   f.txtcuenta.value="";
			   f.txtdenominacion.value="";
			   </script>
			   <?php
			if ($ls_totalcuentas > 1)
			{
				$ls_pagina="sigesp_cat_ctabanco.php?codigo=$ls_codban&hidnomban=$ls_nomban&codcon=$ls_codcon";
				?>
				<script language="javascript">						
				window.open('<?php print $ls_pagina; ?>',"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=583,height=400,left=50,top=50,location=no,resizable=yes");
				</script>
			...
			elseif($ls_totalcuentas==1)
			{
				$rs_data_cta=$in_classmovbco->uf_ctas_ctaconcepto($ls_codcon,$ls_codban,'','');
				while (!$rs_data_cta->EOF)
				   {        
					 require_once("sigesp_c_cuentas_banco.php");
					 $io_ctaban   = new sigesp_c_cuentas_banco();
					 $ls_ctaban      = trim($rs_data_cta->fields["ctaban"]);
					 $ls_dencta      = $rs_data_cta->fields["dencta"];
					 $ls_sc_cuenta		 = trim($rs_data_cta->fields["sc_cuenta"]);
					 $ls_codtipcta     = $rs_data_cta->fields["codtipcta"];
					 $ls_nomtipcta   = $rs_data_cta->fields["nomtipcta"];
					 $io_ctaban->uf_verificar_saldo($ls_codban,$ls_ctaban,&$adec_saldo);
				     $ldec_saldo = number_format($adec_saldo,2,',','.');
					 $rs_data_cta->MoveNext();
				   }
			?>
			   <script language="javascript">						
			   f = document.form1;
			   f.txtcuenta.value="<?php print $ls_ctaban; ?>";
			   f.txtdenominacion.value="<?php print $ls_dencta; ?>";
			   f.txttipocuenta.value="<?php print $ls_codtipcta; ?>";
			   f.txtdentipocuenta.value="<?php print $ls_nomtipcta; ?>";
			   f.txtcuenta_scg.value="<?php print $ls_sc_cuenta; ?>";
			   f.txtdisponible.value="<?php print $ldec_saldo; ?>";
			   </script>
			<?php
			}
		}
  ...
  ?>
  <input name="casacon" type="hidden" id="casacon" value="<?php print $ls_casacon;?>">	    
  </p>
  </form>  
</body>
<script language="javascript">
...
function ue_llenar_bancoconcepto(ls_codban,ls_nomban)
{
...
function ue_llenar_ctaconcepto(ls_codban,ls_ctaban,ls_dencta,ls_sc_cuenta,ls_codtipcta,ls_nomtipcta,ldec_saldo)
{
  ...
  if (lb_mesabi=='true')
     {
	   ...
	   if (ls_operacion=='DP' || ls_operacion=='NC' || ls_operacion=='ND')
	      {
		    if ((f.nocontabili.checked==true)||(f.chktranoreglib.checked==true))
	      	   {
		          ls_spgcta = '';
		          ls_spicta = '';
	           }
	   ...
	   else if(ls_operacion=='RE' || ls_operacion=='CH'){
		    ...
		    if (lb_valido)
			   {
				 ...
				 /*if (li_estcob==0)
				 {
				 	lb_valido = uf_validar_monto_movimiento(ls_objeto); NO QUITAR ESTE COMENTARIO!!
			   	 }*/
			   }
		  }
	   ...
	   if ((ls_status!='C')&&(ls_status!='O')&&(ls_status!='A'))
	      {
		    ...
		    //ls_numcontint=f.txtnumconint.value;
			if(ls_operacion=="CH" || ls_operacion=="RE")
		   	{
					ls_numconint =" ";
		   	}
			else if(ls_numcont==0)
		   	{
					ls_numconint =" ";
		   	}
			else
...
function ue_imprimir(ls_reporte)
{
	...
	ls_status=f.estmov.value;
	ls_numcarord=f.txtnumcarord.value;
	ls_reporte_mov=f.repmovban.value;
	if(ls_codope=='CH')
	{
		ls_chevau=f.txtchevau.value;
	}
	...
	//if(ls_numcarord=="")
	//{	
		if((ls_numdoc!="")&&(ls_codban!="")&&(ls_ctaban!="")&&(ls_codope=="CH"))
		{
			if(ls_status!="A")
			{
		...
		else if((ls_numdoc!="")&&(ls_codban!="")&&(ls_ctaban!="")&&(ls_codope!="CH")&&(ls_codope!=""))
		{
			if(ls_status!="A")
			{
				ls_pagina="reportes/"+ls_reporte_mov+"?codban="+ls_codban+"&ctaban="+ls_ctaban+"&numdoc="+ls_numdoc+"&chevau="+ls_chevau+"&codope="+ls_codope;
				window.open(ls_pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,left=0,top=0,location=no,resizable=yes");
			}
			else
			{
...
{
  ls_status = f.estmov.value;
  ls_casacon =f.casacon.value;
  if ((ls_status!="C")&&(ls_status!='O')&&(ls_status!='A'))
	 {
	   if (ls_casacon==1)
	   {
	   	f.operacion.value = "SELECT_CONCEPTO_CTA";
	   	f.opepre.value	 = f.ddlb_conceptos.value;
	   	f.submit();
	   }
	   else
	   {
	   	f.operacion.value = "CAMBIO_CONCEPTO";//MODIFICADO POR OFIMATICA DE VENEZUELA EL 01-06-2011 SE CAMBIA OPERACION CAMBIO_OPERA POR CAMBIO_CONCEPTO
		f.submit();
	   }	   
	 }
  else
  ...
  if (lb_mesabi=='true')
     {
					   ...
					   if ((ls_status!="C")&&(ls_status!='O')&&(ls_status!='A'))
						  {
							...
							ls_anticipo=0;
							if ((lb_valido)&&(ls_provbene!="")&&(ls_descripcion!="")&&(ls_codban!="")&(ls_ctaban!="")&&(ls_documento!="")&&(ls_operacion!="")&&(ls_codconmov!="")&&(ldec_monto>0))
							   {
								 ld_totmondis = f.txtdisponible.value;
								 ls_tipvaldis = "<?php echo $ls_tipvaldis; ?>";
								 lb_valido    = uf_validar_disponible(ls_operacion,ls_tipvaldis,ld_totmondis,f.txtmonto.value);
								 if (lb_valido)
									{
									  ls_codtipfon    = f.hidcodtipfon.value;
									  ls_reglibro	  = f.esttranoreglib.value;
									  if(ls_operacion=='CH' || ls_operacion=='RE' || ls_numcont==0)
									  {
											ls_numconint 	  = " ";
											ls_numordpagmin   = " ";
									  }
									  ...
									  f.hidconnocontabi.value="1";
									  ls_contabilizable=f.hidconnocontabi.value;
									  ls_pagina = "sigesp_w_regdt_contable.php?txtprocedencia=SCBMOV&mov_document="+ls_documento+"&fecha="+ld_fecha+"&procede="+ls_procede+"&tipo="+ls_tipo+
												  ...
												  "&anticipo="+ls_anticipo+"&codtipfon="+ls_codtipfon+"&numordpagmin="+ls_numordpagmin+"&estcob="+li_estcob+"&numconint="+ls_numconint+"&reglibro="+ls_reglibro+"&nocontabiliza="+ls_contabilizable;
		  ...
		  {
			alert("No se pueden agregar detalles contables al documento no contabilizable..!");
		  }	 
     }
  else
     {
				...
				if ((lb_valido)&&(ls_provbene!="")&&(ls_descripcion!="")&&(ls_codban!="")&(ls_ctaban!="")&&(ls_documento!="")&&(ls_operacion!="")&&(ls_codconmov!="")&&(ldec_monto>0))
				   {
					 if ((ls_operacion!="NC")&&(ls_operacion!="DP"))
						  ...
						  if (lb_valido)
							 {
							   ...
							   ls_pagina = "sigesp_w_regdt_presupuesto.php?txtprocedencia=SCBMOV&mov_document="+ls_documento+"&fecha="+ld_fecha+"&procede="+ls_procede+
										  ...
										  "&estmov="+ls_estmov+"&codconmov="+ls_codconmov+"&tip_mov= &opener=sigesp_scb_p_movbanco.php&estdoc="+ls_estdoc+"&codfuefin="+ls_codfuefin+"&numordpagmin="+ls_numordpagmin+"&codtipfon="+ls_codtipfon+"&estcob="+li_estcob+"&numconint="+ls_numconint+"&reglibro="+ls_reglibro;
					  ...
					  else if ((ls_operacion=="NC")&&(li_estcob=="1"))
						{
						  ld_totmondis = f.txtdisponible.value;
						  ls_tipvaldis = "<?php echo $ls_tipvaldis; ?>";
						  lb_valido    = uf_validar_disponible(ls_operacion,ls_tipvaldis,ld_totmondis,f.txtmonto.value);
...
function uf_selec_tranoreglib(obj)
{
	if(obj.checked==true)
	{
		f.esttranoreglib.value=1;
	}
	else
	{
		f.esttranoreglib.value=0;
	}
}
...
{   
  ...
  if ((f.hidconnocontabi.value==1))
  {
  		alert("El movimiento posee detalles contables, no puede tildar esta opci�n");
		f.nocontabili.checked=false;
  }
...
function uf_cat_tipo_fondo()
{
  ...
  if (ls_codope=='DP' || ls_codope=='NC')
     ...
     {
	   alert("Cat�logo solamente habilitado para operaciones DP=Dep�sito y NC=Nota de Cr�dito !!!");	 
	 }
} 
	   ...
	   if (ls_numordpagmin=='' && ls_codtipfon!='')
		  {
		    ...
		    alert("Fu� eliminado el enlace con el Tipo de Fondo !!!"); 
		  }
	 }
}
...
function uf_catalogo_ordenes()
{
  ...
  if (ls_codope=='CH' || ls_codope=='ND')
     ...
     {
	   alert("Cat�logo solamente habilitado para operaciones CH=Cheque y ND=Nota de D�bito !!!");	 
	 }
}
