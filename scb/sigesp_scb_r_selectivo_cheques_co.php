...
$io_fun_banco->uf_load_seguridad("SCB","sigesp_scb_r_selectivo_cheques_co.php",$ls_permisos,$la_seguridad,$la_permisos);
$ls_reporte   = $io_fun_banco->uf_select_config("SCB","REPORTE","LISTADO_CH_CO","sigesp_scb_rpp_cheques_co_pdf.php","C");
$li_diasem = date('w');
switch ($li_diasem){
...
function uf_load_documentos($as_codemp,$as_codban,$as_ctaban,$as_codope,$as_conmov,$as_fecdes,$as_fechas,$as_estmov,$as_orden,$ls_numdocmov,&$lb_valido)
{
  global $io_sql;
  global $io_funcion;
  ...
  //{
  //  $ls_straux = $ls_straux." AND (M.codope='CH' OR M.codope='ND') AND (M.codope<>'DP')    ";
  //}
  if (!empty($as_conmov))
  {
  ...
  {
  	$ls_straux = $ls_straux." AND M.numdoc LIKE '%".$ls_numdocmov."%'";
  }
  if (!empty($ls_numdocmov))
  {
  	$ls_straux = $ls_straux." AND M.numdoc LIKE '%".$ls_numdocmov."%'";
	...
	if (!empty($as_codban))
	{
		$ls_codbanco = " M.codban='".$as_codban."' AND ";
	}
	else
	{
		$ls_codbanco = "  ";
	...
	if (!empty($as_codban))
	{
        ...
        {
            $ls_codctabco = " M.ctaban='".$as_ctaban."' AND ";
        }
  ...
  $ls_sql  = "SELECT M.codemp,M.codban,M.ctaban,M.numdoc,M.codope,M.estmov,M.cod_pro,M.ced_bene,M.tipo_destino,               ".
             ...
             " WHERE M.codemp='".$as_codemp."' AND ". 
			 $ls_codbanco.
			 $ls_codctabco.
			 "		 M.codope<>'OP' ".
			 "       $ls_straux AND ".
             "		 (M.procede = 'SCBBCH' or M.procede = 'SCBCOR') AND ".
             "       ((M.codope='CH' OR M.codope='ND') AND (M.codope<>'DP')) AND ".
			 "		 M.codban=B.codban AND M.codemp=B.codemp AND M.codban=C.codban AND M.ctaban=C.ctaban       ".
			 " ORDER BY M.fecmov,M.numdoc ";  //ORDER BY $as_orden
...
?>
  <table width="535" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr class="titulo-ventana">
      <td height="22" colspan="4" align="center">Relación Selectiva de Cheques / Carta Orden </td>
    </tr>
    <tr>
...
if ($ls_operacion=="BUSCAR")
   {
	 $lb_valido = true;
	 $li_fila   = 0;
	 $rs_data   = uf_load_documentos($ls_codemp,$ls_codban,$ls_ctaban,$ls_codope,$ls_conmov,$ld_fecdes,$ld_fechas,$ls_estmov,$ls_orden,$ls_numdocmov,&$lb_valido);
	 if ($lb_valido)
	    {
	      $title[1] = ""; 
  ...
  if (li_leer==1)
	 {
	   ...
	   //   {
            f.operacion.value = "BUSCAR";
            f.action          = "sigesp_scb_r_selectivo_cheques_co.php";
			f.submit();
		//  }
	}
  else
  ...
  if (li_imprimir=='1')
     {
	  ...
	  /* if (ls_codban=="" || ls_ctaban=="" || ls_nomban=="")
	      ...
	      {*/
 	        //pagina="reportes/"+ls_reporte+"?fecdes="+ld_fecdesde+"&fechas="+ld_fechasta+"&codope="+ls_codope+"&codban="+ls_codban+"&ctaban="+ls_ctaban+"&codconcep="+ls_concepto+"&orden="+ls_orden+"&hidestmov="+ls_estmov+"&nomban="+ls_nomban+"&tiporeporte="+ls_tiporeporte+"&documento="+ls_numdoc+"&chked="+ls_chkmarc+"&chkados="+ls_docs;
  ...
  if (li_imprimir=='1')
     {
	  ...
	  /* if (ls_codban=="" || ls_ctaban=="" || ls_nomban=="")
	      ...
	      {*/
 	        pagina="reportes/sigesp_scb_rpp_cheques_co_excel.php?fecdes="+ld_fecdesde+"&fechas="+ld_fechasta+"&codban="+ls_codban+"&ctaban="+ls_ctaban+"&nomban="+ls_nomban+"&tiporeporte="+ls_tiporeporte+"&documento="+ls_numdoc+"&chked="+ls_chkmarc+"&chkados="+ls_docs;
	        window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,resizable=yes,location=no");
		  //}  
	 }
  else
