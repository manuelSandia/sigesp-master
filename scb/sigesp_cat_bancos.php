				       ...
				       if (empty($ls_procede))
				          {
								if ($ls_tipo==1)
								{
									echo "<td style=text-align:center width=100><a href=\"javascript: aceptarcoldes('$ls_codban','$ls_denban');\">".$ls_codban."</a></td>";
								}
								elseif ($ls_tipo==2)
								{
									echo "<td style=text-align:center width=100><a href=\"javascript: aceptarcolhas('$ls_codban','$ls_denban');\">".$ls_codban."</a></td>";
								}
								else
								{
...
function aceptarcoldes(as_codban,as_nomban)
{
	fop = opener.document.form1; 
    fop.txtcodbandes.value=as_codban;
    fop.txtdenbandes.value=as_nomban;
	close();
}

function aceptarcolhas(as_codban,as_nomban)
{
	fop = opener.document.form1; 
	ls_opener = opener.document.form1.id;
    fop.txtcodbanhas.value=as_codban;
    fop.txtdenbanhas.value=as_nomban;
	close();
}
...
function ue_search()
{
  ...
  f.action="sigesp_cat_bancos.php?tipo=<?php print $ls_tipo;?>";
  f.submit();
}
