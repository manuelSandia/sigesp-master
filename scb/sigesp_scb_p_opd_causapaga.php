	...
	if (array_key_exists("operacion",$_POST))
	   {
		...
		if (array_key_exists("chkserext",$_POST))
		   {
			 $ls_estserext = "checked";
		   }
		else
		   {
			 $ls_estserext = "";
...
?>
  ...
  <table width="780" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
      ...
      <input name="modordpag" type="hidden" id="modordpag" value="<?php print $ls_modalidad;?>">
	  <span class="sin-borde">
      <input name="chkserext" type="checkbox" class="sin-borde" id="chkserext" value="1" <?php print $ls_estserext; ?>>
      Servicio Exterior</span></td>
    </tr>
    <tr>
	  ...
	  if (uf_evaluate_cierre('SPG'))
		 {
			...
			if((ls_status!='C')&&(ls_status!='O')&&(ls_status!='A'))
			{
				...
				ls_nrocontrol=f.txtnrocontrol.value;
				if (eval("f.chkserext.checked==true"))
				   {
					 li_estserext = 1;   
				   }
				else
...
function uf_valida_cambio()
{
	...
	else
	{
	  ...
	  if (ls_modalidad=='PP' || ls_modalidad=='CM')
     {
	   f.chkserext.disabled = false;
	   f.chkserext.checked  = false;
	 }	
  	 else
     {
	   f.chkserext.checked  = false;
	   f.chkserext.disabled = true;
	 }
