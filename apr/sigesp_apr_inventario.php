...
<title>Movimiento Inicial de Existencias</title>
...
<?php
require_once("sigesp_apr_c_inventario.php");
$io_inventario=new sigesp_apr_c_inventario();
...
{
    $lb_valido=$io_inventario->uf_insert_movimientoinicial();
    if($lb_valido)
	{
	 	$io_inventario->io_mensajes->message(" El proceso se ejecuto satifactoriamente");
	}
	else
	{
		$io_inventario->io_mensajes->message("Ocurrio un Error al Crear el Movimiento Inicial");
	}
}
?>
</div> 
...
?>		  
  <table width="400" height="21" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr>
      <td width="604" colspan="2" class="titulo-ventana">MOVIMIENTO INICIAL DE EXISTENCIAS DE INVENTARIO </td>
    </tr>
  </table>
...
function uf_aceptar()
{
	...
	if((li_cambiar==1)||(li_incluir==1))
	{
		...
		f.action="sigesp_apr_inventario.php";
		f.submit();
	}	
}
