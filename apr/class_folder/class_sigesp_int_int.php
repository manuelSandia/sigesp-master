...
class class_sigesp_int_int extends class_sigesp_int
{
	...
	function class_sigesp_int_int()
	{
		...
		$ls_nombrearchivo="resultado/".trim($_SESSION["la_empresa"]["sigemp"])."_saldos_contables_result_".$ld_fecha.".txt";
						 ...
						 $ab_spg_enlace_contable,$as_codban,$as_ctaban,$ai_tipo_comp)
	{
		...
		if(!($this->uf_init_valida_parametros_comprobante($as_codemp,$as_procedencia,$as_comprobante,$as_fecha,$as_descripcion,
														  $as_tipo,$as_fuente,$as_codban,$as_ctaban)))
		{			
	   		return false;
		}
												   ...
												   $as_tipo,$as_fuente)
	{
		...
		if((is_null($as_comprobante)) or (empty($as_comprobante)))
		{
			...
			$this->is_msg_error="El N° de Comprobante no puede tener valor nulo o vacío.\r\n";
			$ls_cadena=$this->is_msg_error."\r\n";
			if ($this->lo_archivo)			
			{
				@fwrite($this->lo_archivo,$ls_cadena);
		...
		if((is_null($as_procedencia)) or (empty($as_procedencia)))
		{
			...
			$this->is_msg_error="La procedencia no puede tener valor nulo o vacio.\r\n";
			$ls_cadena=$this->is_msg_error."\r\n";
			if ($this->lo_archivo)			
			{
				@fwrite($this->lo_archivo,$ls_cadena);
		...
		if((is_null($as_descripcion)) or (empty($as_descripcion)))
		{
			...
			$this->is_msg_error="La descripción no puede tener valor nulo o vacío.\r\n";
			$ls_cadena=$this->is_msg_error."\r\n";
			if ($this->lo_archivo)			
			{
				@fwrite($this->lo_archivo,$ls_cadena);
		...
		{ 
			...
			$this->is_msg_error="El Tipo (Beneficiario o Proveedor) no puede tener valor nulo o vacío.\r\n";
			$ls_cadena=$this->is_msg_error."\r\n";
			if ($this->lo_archivo)			
			{
				@fwrite($this->lo_archivo,$ls_cadena);
		...
		if((is_null($as_fuente)) or ($as_fuente==""))
		{
			...
			$this->is_msg_error="El Beneficiario o Proveedor no puede tener valor nulo o vacío.\r\n";
			$ls_cadena=$this->is_msg_error."\r\n";
			if ($this->lo_archivo)			
			{
				@fwrite($this->lo_archivo,$ls_cadena);
		...
		if($this->uf_select_proveedor($as_codemp,$as_codpro)===false)
		{
			...
			$this->is_msg_error="El Proveedor ".$as_codpro." no Existe en la Ficha de Proveedores.\r\n";
			$ls_cadena=$this->is_msg_error."\r\n";
			if ($this->lo_archivo)			
			{
				@fwrite($this->lo_archivo,$ls_cadena);
		...
		if($this->uf_select_beneficiario($as_codemp,$as_cedben)===false)
		{
			...
			$this->is_msg_error="El Beneficiario ".$as_cedben." no Existe en la Ficha de Beneficiarios.\r\n";
			$ls_cadena=$this->is_msg_error."\r\n";
			if ($this->lo_archivo)			
			{
				@fwrite($this->lo_archivo,$ls_cadena);
		...
		{ 
			...
			$this->is_msg_error="Error en valida procedencia del Comprobante.\r\n";
			$ls_cadena=$this->is_msg_error."\r\n";
			if ($this->lo_archivo)			
			{
				@fwrite($this->lo_archivo,$ls_cadena);
		...
		if (!($this->int_fecha->uf_valida_fecha_mes( $as_codemp , $as_fecha )))
		{
			...
			$this->is_msg_error="Error en valida fecha del Comprobante.\r\n";
			$ls_cadena=$this->is_msg_error."\r\n";
			if ($this->lo_archivo)			
			{
				@fwrite($this->lo_archivo,$ls_cadena);
	...
	function uf_init_delete($as_codemp,$as_procedencia,$as_comprobante,$as_fecha,$as_tipo,$as_ced_bene,$as_cod_pro,
							$ab_check_close,$as_codban,$as_ctaban)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_init_load_datastore_integracion()
	{
		...
		// Datastore Gasto
		$ls_sql="SELECT dt.codestpro1 as codestpro1,dt.codestpro2 as codestpro2,dt.codestpro3 as codestpro3,dt.codestpro4 as codestpro4,dt.codestpro5 as codestpro5,".
				"       dt.spg_cuenta as spg_cuenta,dt.procede_doc as procede_doc,dt.documento as documento,dt.operacion as operacion,dt.descripcion as descripcion,".
				"       dt.monto as monto,dt.orden as orden,c.sc_cuenta as sc_cuenta ".
		...
		// Datastore Ingresos
		$ls_sql="SELECT dt.spi_cuenta as spi_cuenta,dt.procede_doc as procede_doc,dt.documento as documento,dt.operacion as operacion,".
	...
	function uf_int_valida_informacion_datastore()
	{
		...
		{
			$ld_diferencia=(doubleval(trim($this->idec_monto_debe))-doubleval(trim($this->idec_monto_haber)));
			if(abs($ld_diferencia)<1)
			{
				$this->is_msg_error="No Cuadra el Comprobante Contable Debe=[".$this->idec_monto_debe."] , Haber[".$this->idec_monto_haber."]\r\n".
									" Una vez que termine el proceso en la Base de Datos nueva Debe hacer un ajuste al Asiento de apertura ".
									" según el numeral 4to del pronunciamiento de la federación de Colegios de Contadores  Públicos de Venezuela";
				$ls_cadena=$this->is_msg_error."\r\n";
				if ($this->lo_archivo)			
				{
					@fwrite($this->lo_archivo,$ls_cadena);
			...
			{
				$this->is_msg_error="No Cuadra el Comprobante Contable Debe=[".$this->idec_monto_debe."] , Haber[".$this->idec_monto_haber."]\r\n".
									"No se Creo el Asiento de Apertura";
				$ls_cadena=$this->is_msg_error."\r\n";
				if ($this->lo_archivo)			
				{
					@fwrite($this->lo_archivo,$ls_cadena);
	...
	function uf_init_load_anular_datastore_integracion($as_codemp,$as_procedencia,$as_comprobante,$adt_fecha,$as_codban,$as_ctaban)
	{
		...
		$ls_sql="SELECT dt.codestpro1, dt.codestpro2, dt.codestpro3, dt.codestpro4, dt.codestpro5, dt.spg_cuenta, dt.operacion,".
				...
				"   AND dt.codestpro5=c.codestpro5 ".
				"   AND dt.spg_Cuenta=c.spg_cuenta ".
				" GROUP BY dt.codestpro1, dt.codestpro2, dt.codestpro3, dt.codestpro4, dt.codestpro5, dt.spg_cuenta, c.sc_cuenta, ".
		...
		// Datastore Ingresos
		$ls_sql="SELECT dt.spi_cuenta as spi_cuenta, dt.procede_doc as procede_doc, dt.comprobante as documento, dt.operacion as operacion, ".
