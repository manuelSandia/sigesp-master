   ...
   function uf_print($as_ruta)
   {
		...
		while (false!==$file = readdir($handle))
		{
			 if(($file != '.') && ($file != '..'))
			 {
					...
					print "<td width='70' height='22'><div align='center'><a href='sigesp_ins_cat_descarga.php?enlace=".$as_ruta."&file=".$file."'><img src='../shared/imagebank/tools20/download.gif' width='20' height='20' border='0'></a></div></td>";
...
<?php
	...
	$ls_ruta=$_GET["ruta"];
	$ls_file=$io_fun_ins->uf_obtenervalor_get("file","");
	if($ls_file!="")
	{
...
function ue_delete(ruta,file)
{
	...
	{
		f=document.form1;
		f.action="sigesp_ins_cat_directorio.php?ruta="+ruta+"&file="+file+"";
		f.submit();
	}
}
</script>
