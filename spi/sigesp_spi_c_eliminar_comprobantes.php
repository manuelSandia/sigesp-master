<?php
class sigesp_spi_c_eliminar_comprobantes
{
	var $is_msg_error;
	var $io_sql;
	var $io_include;	
	...
	var $is_descripcion;
	var $is_tipo;
	function sigesp_spi_c_eliminar_comprobantes()
	{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql=" SELECT sigesp_cmp.comprobante, sigesp_cmp.procede,                    ".
						...
						"   LIKE '%SPICMP%'                                                     ".
						"    AND  sigesp_cmp.comprobante NOT IN (SELECT  spi_dt_cmp.comprobante ".
						"                                          FROM spi_dt_cmp              ".
						"                                         WHERE  spi_dt_cmp.procede LIKE '%SPICMP%') ".
						"    AND sigesp_cmp.comprobante NOT IN (SELECT scg_dt_cmp.comprobante ".
						"                                        FROM scg_dt_cmp WHERE scg_dt_cmp.procede LIKE '%SPICMP%')".
						$ls_criterio1. 
						"   UNION                                                               ".
						...
						"  FROM sigesp_cmp_md                       ".
						" WHERE sigesp_cmp_md.procede LIKE '%SPI%'  ".
						"   AND sigesp_cmp_md.comprobante NOT IN (SELECT spi_dtmp_cmp.comprobante  ".
						"                                           FROM spi_dtmp_cmp WHERE spi_dtmp_cmp.procede LIKE '%SPI%')  ".
						"   AND sigesp_cmp_md.comprobante NOT IN (SELECT scg_dt_cmp.comprobante ".
						"                                        FROM scg_dt_cmp WHERE scg_dt_cmp.procede LIKE '%SPI%')".
						$ls_criterio2.
...
}// fin de la calse sigesp_spi_c_eliminar_comprobantes
?>