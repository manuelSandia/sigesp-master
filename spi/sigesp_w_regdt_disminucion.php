...
$io_seguridad= new sigesp_c_seguridad();
	...
	$ls_sistema="SPI";
	$ls_ventana="sigesp_spi_p_disminucion.php";
...
if($ls_operacion=="GUARDARPRE")
{
	...
	//-------------RELACIÓN DE ESTRUCTURA CON EL PRESUPUESTO DE INGRESOS------------------------------------------
		if ($li_estpreing==1)
		 {
				$ls_estcla     = trim($_POST["estcla"]);
	...
	//---------------------------------------------------------------------------------------------------------------------
	$lb_valido=$in_classcmp->uf_spi_saldo_select($dat["codemp"],$ls_cuenta,$ld_previsto,$ld_aumento,
												 $ld_disminucion,$ld_devengado,$ld_cobrado,$ld_cobrado_ant);
	$ld_previsto_disponible=$ld_previsto-$ld_devengado;
	if($ld_previsto_disponible>=$ldec_monto){ 
		$lb_pasar=true;
	}
	else { 
		$lb_pasar=false;	
	...
	if(($lb_pasar)&&($lb_valido))										 
	{
		...
		if($lb_valido)
		{
			...
			$lb_valido=$in_classcmp->uf_guardar_movimientos($arr_cmp,$ls_cuenta,$ls_procede,$ls_descripcion,$ls_documento,$ls_operacionpre,0,$ldec_monto,"C",$ls_codestpro1,$ls_codestpro2,$ls_codestpro3,$ls_codestpro4,$ls_codestpro5,$ls_estcla);
			if($lb_valido)
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="INSERT";
				...
				{
					$ls_desc_event=$in_classcmp->io_int_scg->is_log_transacciones.",Inserto el movimiento presupuestario ".$ls_documento." con operacion".$ls_operacionpre." por un monto de ".$ldec_monto." para la cuenta ".$ls_cuenta." ";
				}
				else
				{
				...
				////////////////////////////////         SEGURIDAD               //////////////////////////////
				$in_classcmp->io_int_spi->io_sql->commit();
				$io_msg->message("El Movimiento fue registrado.");
			}
			else	
			{	
			   $in_classcmp->io_int_scg->io_sql->rollback();
			   $io_msg->message("El Movimiento no fue registrado.");
			}
		}
	}
	else
	{
		$io_msg->message(" Intenta Disminuir mas de Previsto...");	
		$ls_cuenta      = $_POST["txtcuenta"];
		$ls_documento   = $_POST["txtdocumento"];
		$ls_descripcion = $_POST["txtdescripcion"];
		$ls_denominacion= $_POST["txtdenominacion"];
...
<table width="583" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
  ...
  <tr>
    ...
    <td height="22"><label>
      <input name="codestpro1" type="text" id="codestpro1" size="<?php echo $li_size1; ?>" maxlength="<?php echo $li_loncodestpro1 ?>" value="<?php print $ls_estpro1; ?>" style="text-align:center">
      ...
      <input name="hidtipestpro" type="hidden" id="hidtipestpro" value="<?php /*echo $ls_estcla;*/ ?>">
  ...
  <tr>
    ...
    <td height="22"><label>
      <input name="codestpro2" type="text" id="codestpro2" size="<?php echo $li_size2; ?>" maxlength="<?php echo $li_loncodestpro2 ?>" value="<?php print $ls_estpro2; ?>" style="text-align:center">
  ...
  <tr>
    ...
    <td height="22"><label>
      <input name="codestpro3" type="text" id="codestpro3" size="<?php echo $li_size3; ?>" maxlength="<?php echo $li_loncodestpro3 ?>" value="<?php print $ls_estpro3; ?>" style="text-align:center">
  ...
  <tr>
    ...
    <td height="22"><label>
      <input name="codestpro4" type="text" id="codestpro4" size="<?php echo $li_size4; ?>" maxlength="<?php echo $li_loncodestpro4 ?>" value="<?php print $ls_estpro4; ?>" style="text-align:center">
  ...
  <tr>
    ...
    <td height="22"><label>
      <input name="codestpro5" type="text" id="codestpro5" size="<?php echo $li_size5; ?>" maxlength="<?php echo $li_loncodestpro5 ?>" value="<?php print $ls_estpro5; ?>" style="text-align:center">
  ...
  <tr>
    ...
    <a href="javascript:catalogo_cuentasSPI();"><img src="../shared/imagebank/tools15/buscar.gif" width="15" height="15" border="0" alt="Catálogo de Cuentas de Gastos"></a>	 <input name="txtdenominacion" type="text" class="sin-borde" id="txtdenominacion3" style="text-align:left" value="<? print $ls_denominacion ?>" size="50" maxlength="254"></td>
  </tr>
  <tr>
    <td height="22"><div align="right">Operaci&oacute;n</div></td>
    <td><div align="left">
      <label>
      <select name="ddlb_operacion" id="ddlb_operacion">
...
<script language="JavaScript">
  function aceptar_presupuestario()
  {
	...
	if((ls_cuenta!="")&&(ls_descripcion!="")&&(ls_procedencia!="")&&(ls_documento!="")&&(ls_operacion!="")&&(ldec_monto!=""))
	{
		...
		f.action="sigesp_w_regdt_disminucion.php";
		f.submit();
	}
	else
	{
    ...
    if (li_estpreing==1)
     {
	   ...
	   ls_hidestcla  = f.hidtipestpro.value;
...
function catalogo_estpro2()
{
	...
	{
		pagina="sigesp_spi_cat_public_estpro2.php?codestpro1="+codestpro1+"&denestpro1="+denestpro1+"&hidestcla="+ls_hidestcla+"&estcla="+ls_estcla;
...
function catalogo_estpro3()
{
	...
	if(<?php print $li_estmodest?>==1)
	{
		...
		{
			pagina="sigesp_spi_cat_public_estpro3.php?codestpro1="+codestpro1+"&denestpro1="+denestpro1+"&codestpro2="+codestpro2+"&denestpro2="+denestpro2+"&hidestcla="+ls_hidestcla+"&estcla="+ls_estcla+"&tipo=comprobante";
...
function catalogo_estpro4()
{
	...
	{
		pagina="sigesp_spi_cat_public_estpro4.php?codestpro1="+codestpro1+"&denestpro1="+denestpro1+"&codestpro2="+codestpro2+"&denestpro2="+denestpro2+"&codestpro3="+codestpro3+"&denestpro3="+denestpro3+"&hidestcla="+ls_hidestcla+"&estcla="+ls_estcla;
		window.open(pagina,"_blank","menubar=no,toolbar=no,scrollbars=yes,width=568,height=400,resizable=yes,location=no");
	}
	else
	{
...
function catalogo_estpro5()
{
	...
	{
		pagina="sigesp_spi_cat_public_estpro5.php?codestpro1="+codestpro1+"&denestpro1="+denestpro1+"&codestpro2="+codestpro2+"&denestpro2="+denestpro2+"&codestpro3="+codestpro3+"&denestpro3="+denestpro3+"&codestpro4="+codestpro4+"&denestpro4="+denestpro4+"&hidestcla="+ls_hidestcla+"&estcla="+ls_estcla;
		window.open(pagina,"_blank","menubar=no,toolbar=no,scrollbars=yes,width=568,height=400,resizable=yes,location=no");
	}
	else
	{
