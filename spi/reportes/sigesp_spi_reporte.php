...
/********************************************************************************************************************************/	
    ...
    function uf_spi_reporte_acumulado_cuentas($adt_fecini,$adt_fecfin,$ai_nivel,
											  $ab_subniveles,&$ai_MenorNivel,$as_spi_cuentades,$as_spi_cuentahas)
    {//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/********************************************************************************************************************************/	
	...
	{//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 //	Description :	Método  que ejecuta todas funciones de acumulado para asi sacar el acumulado por cuentas
	  ...
	  { // error interno sql
		$this->io_msg->message("CLASE->sigesp_spi_reporte
		                        MÉTODO->uf_calcular_acumulado_operaciones_por_cuenta 
								ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
		$lb_valido = false;
 	  }
	  else
	...
	{//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 //	Description :	Método  que ejecuta todas funciones de acumulado para asi sacar el acumulado por cuentas
	  ...
	  $ls_sql=" SELECT * ".
			  ...
			  " AND substr(spi_dt_cmp.fecha,1,10) <='".$adt_fecfin."'";
...
/********************************************************************************************************************************/	
    ...
    function uf_spi_reporte_acumulado_cuentas2($as_codestpro1_ori,$as_codestpro2_ori,$as_codestpro3_ori,$as_codestpro4_ori,
											  ...
											  $adt_fecfin,$ai_nivel,
											  $ab_subniveles,&$ai_MenorNivel,$ls_modalidad,$as_spi_cuentades,$as_spi_cuentahas)
    {//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/********************************************************************************************************************************/	
	...
	{
		$lb_valido=false;
		$this->io_msg->message("CLASE->sigesp_spi_reporte MÉTODO->uf_spi_reporte_detalle_acumulado_cuentas ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
	}
	else
	{
		$ld_saumento=0;
		...
		if($li_totfila>0)
		{
			while($row=$this->io_sql->fetch_row($rs_data))
			{	
			   /* $this->dts_reporte->insertRow("previsto",$row["previsto"]);			
			    $this->dts_reporte->insertRow("aumento",$row["aumento"]);
			    $this->dts_reporte->insertRow("disminucion",$row["disminucion"]);
			    $this->dts_reporte->insertRow("devengado",$row["devengado"]);
			    $this->dts_reporte->insertRow("cobrado",$row["cobrado"]);*/
		...
		{
			/*$this->dts_reporte->insertRow("previsto",0);			
	...
	//$ls_columnas_adic = " 0.00 as aumento,0.00 as disminucion,0.00 as devengado,0.00 as cobrado ";
	/*$ls_sql=" SELECT DISTINCT (spi_cuentas.spi_cuenta) as spi_cuenta,MAX(spi_cuentas.denominacion) as denominacion, MAX(spi_cuentas.status) as status,MAX(spi_cuentas.nivel) as nivel,SUM(spi_cuentas_estructuras.previsto) as previsto  ".
              " FROM   spi_cuentas, spi_cuentas_estructuras, spg_ep5 ".
			  " WHERE  spi_cuentas.spi_cuenta BETWEEN '".$as_spi_cuentades."' AND '".$as_spi_cuentahas."'  ".
			  "	AND spi_cuentas_estructuras.codestpro1<>'-------------------------' ".
			  " AND spi_cuentas_estructuras.codestpro2<>'-------------------------' ".
	...
	{
		  $lb_valido=false;
		  $this->io_msg->message("CLASE->sigesp_spi_reporte MÉTODO->uf_spi_reporte_listado_cuentas2 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
	}
	else
	{
		while(!$rs_data->EOF)
		{
			...
			$ld_previsto = 0;
			$this->uf_spi_calcular_previsto_estructura($ls_spi_cuenta,$ls_estructura_origen,$ls_estructura_dstino,$ld_previsto);
			$this->dts_reporte->insertRow("spi_cuenta",$ls_spi_cuenta);			
			$this->dts_reporte->insertRow("denominacion",$ls_denominacion);
...
//------------------------------------------------------------------------------------------------------------------------------------- 
    ...
    //      Creado por :    Ing. Jennifer rivero.
    //  Fecha Creación :    07/09/2009          Fecha última Modificacion :      Hora :
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$ls_codemp=$_SESSION["la_empresa"]["codemp"];
	$lb_valido=true;
	$ls_sql= "SELECT 
...
//------------------------------------------------------------------------------------------------------------------------------------- 
   ...
   function uf_calcular_operaciones_por_cuenta($adt_fecini,$adt_fecfin,&$ad_previsto,&$ad_aumento,
	...
	{///////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 // Fecha Creación :    24/05/2010          Fecha última Modificacion :      Hora :
  	 ////////////////////////////////////////////////////////////////////////////////////////////////////////
	  $lb_valido = true;
	  $ld_previsto=0;
	  $ld_acumtrimestre=0;
	  ...
	  { // error interno sql
		$this->io_msg->message("CLASE->sigesp_spi_reporte
		                        MÉTODO->uf_spi_calcular_previsto_estructura 
								ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
		$lb_valido = false;
 	  }
	  else
