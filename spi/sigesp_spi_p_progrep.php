...
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
	...
	$ls_sistema="SPI";
	$ls_ventanas="sigesp_spi_p_progrep.php";
        ...
        <tr>
            ...
            <tr>
              ...
              <td colspan="4"><select name="cmbrep" id="select" onChange="uf_cargargrid()">			  
                ...
                <option value="0802" <?php print $ls_est_result08 ?>>Estado de Resultado Instructivo 8</option>
			    </select>
...
<?php	
 ...
 $title[3]="Previsto"; 
 if($_POST["cmbrep"]=='0802')
 {
     $title[4]="Pres.Aprobado"; 
     $title[5]="Pres.Modificado"; 
 }
 
 
 $ls_nombre="grid_progrep";
   ...
   if($lb_valido)
   {
	 ...
	 if($row=$io_sql->fetch_row($rs_load))
	 {
		...
		{    
            ...
            $ld_presupuesto_aprobado=number_format($data["saldo_apro"][$i],2,",",".");
            $ld_presupuesto_modificado=number_format($data["saldo_mod"][$i],2,",",".");
			...
			if($ls_status=="I")
			{
                ...
                if ($ls_codrep=='0802')
                {
                    $object[$i][4]="<input type=text name=txtPresupAprobado".$i." onBlur='uf_formato(this)'  onFocus= uf_fila(".$i.") value='$ld_presupuesto_aprobado' size=10 class=formato-azul >";
                    $object[$i][5]="<input type=text name=txtPresupModificado".$i." onBlur='uf_formato(this)'  onFocus= uf_fila(".$i.") value='$ld_presupuesto_modificado' size=10 class=formato-azul >"; 
                }
...
if ($ls_operacion=="DISTRIBUIR")
{
  ...
  if ($ls_opcion=="M")
  {
    ...
    {    
        ...
        $ld_presupuesto_aprobado= $_POST["saldo_apro".$i];   
        $ld_presupuesto_modificado= $_POST["saldo_mod".$i];  
		if($li_rows==$i)
		{
			$ls_distribuir=3;
			$ls_modrep=1;
...
if ($ls_operacion=="GUARDAR" )
{
   ...
   { 
		...
		$ld_noviembre=$_POST["noviembre".$i];
		$ld_diciembre=$_POST["diciembre".$i];
        $ld_presupuesto_aprobado = ((empty($_POST["txtPresupAprobado".$i])) ? "0.00" : $_POST["txtPresupAprobado".$i]);
        $ld_presupuesto_modificado = ((empty($_POST["txtPresupModificado".$i])) ? "0.00" : $_POST["txtPresupModificado".$i]);
		...
		$ld_noviembre=str_replace('.','',$ld_noviembre);                                $ld_noviembre=str_replace(',','.',$ld_noviembre);
		$ld_diciembre=str_replace('.','',$ld_diciembre);                                $ld_diciembre=str_replace(',','.',$ld_diciembre);
        $ld_presupuesto_aprobado=str_replace('.','',$ld_presupuesto_aprobado);          $ld_presupuesto_aprobado=str_replace(',','.',$ld_presupuesto_aprobado);
        $ld_presupuesto_modificado=str_replace('.','',$ld_presupuesto_modificado);      $ld_presupuesto_modificado=str_replace(',','.',$ld_presupuesto_modificado);
		...
		$lb_valido=$io_class_progrep->uf_spi_guardar_programacion_reportes($ls_status,$ld_previsto,$ls_distribuir,$ls_modrep,$ld_enero,
															 ...
															 $ls_codrep,$ld_presupuesto_aprobado,$ld_presupuesto_modificado);
	   if ($lb_valido)
	   {
...
function uf_cargargrid()
{
	...
	f.action="sigesp_spi_p_progrep.php";
	f.submit();
}
...
function ue_distribuir()
{
	...
	if(document.opcion=="M" ) 
	{
		 ...
		 pagina="sigesp_spi_p_progrep_distribucion.php?fila="+li+"&txtPrevisto="+ld_previsto+"&enero="+ld_enero
