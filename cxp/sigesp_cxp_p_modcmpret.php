...
<table width="799" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr>
    ...
    <td class="toolbar" width="29"><a href="javascript: ue_cerrar();"><img src="../shared/imagebank/tools20/salir.gif" alt="Salir" width="20" height="20" border="0" title="Salir"></a><a href="javascript: ue_imprimir();"></a></td>
    ...
    <td class="toolbar" width="29"><div align="center"></div></td>
              ...
              <tr>
                ...
                <td height="22" align="right"><div align="left">
				  ...
				  <?php if($ls_modageret=$_SESSION["la_empresa"]["estretiva"]!="B"){?>
                    <option value="0000000001" selected <?php if($ls_codret=="0000000001"){ print "selected";} ?>>IVA</option>
                    ...
                    <option value="0000000005" <?php if($ls_codret=="0000000005"){ print "selected";} ?>>1 X 1000</option>
                  </select>
                  </label>
                </div></td>
              </tr>
...
function ue_guardar()
{
	...
	if(((lb_existe=="TRUE")&&(li_cambiar==1))||(lb_existe=="FALSE")&&(li_incluir==1))
	{
		...
		else
		{
			...
			if(valido)
			{
				valido=ue_validarcampo(f.txtnumcom.value,"Debe seleccionar un Comprobante.",f.txtnumcom);
			}
			if(valido)
			{
				valido=ue_validarcampo(f.txtcodigo.value,"Debe seleccionar un Proveedor / Beneficiario.",f.txtcodigo);
			}
			if(valido)
			{
				...
				for(row=1;row<rowrecepciones;row++)
				{
					...
					//fecfac= eval("f.txtfecfac"+row+".value");
					//mesfac=fecfac.substr(3, 2);
					if(ls_mes!=fecsis)
					{
						alert("El Comprobante debe pertenecer al periodo fiscal actual");
						valido=false;
						break;
					}
				}		
...
function ue_eliminar()
{
	...
	if(li_eliminar==1)
	{
		...
		else
		{
			...
			if(valido)
			{
				valido=ue_validarcampo(numcom,"Debe seleccionar un Comprobante.",f.numcom);
			}
			if(valido)
			{
				rowrecepciones=f.totrowrecepciones.value;
  ...
  switch (ls_tipret)
  {
	...
	break;
  	case "0000000005":
	   ...
	   if ((ld_basimp!="0,00") && (ld_basimp!=""))
		  {
		    pagina="sigesp_cxp_pdt_deducciones.php?tipo=CMPRETMIL";
		    window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=700,height=600,resizable=yes,location=no");	  
		  }  
	   else
		  {
	...
	break;
  	case "0000000006":
	   ...
	   if ((ld_basimp!="0,00") && (ld_basimp!=""))
		  {
		    pagina="sigesp_cxp_pdt_deducciones.php?tipo=ISLR";
		    window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=700,height=600,resizable=yes,location=no");	  
		  }  
	   else
		  {
