<?php
	...
	{	
		...
		//	  Description: M�todo que impirme el grid de las recepciones a ser aprobadas o para reversar la aprovaci�n
		...
		if($rs_datasol!=false)
		{
			while($row=$io_aprobacion->io_sql->fetch_row($rs_datasol))
			{
				...
				if($lb_imprimir)
				{
					...
					$lo_object[$li_fila][7]="<textarea input name=txtjustapro".$li_fila." id=txtjustapro".$li_fila."  type=text class=sin-borde  size=80   style=text-align:left >".$ls_justapro."</textarea>";
...
*/		if($li_fila==0)
		{
			...
			$lo_object[$li_fila][7]="<input type=text name=txtjustapro".$li_fila."    class=sin-borde style=text-align:left   size=15 readonly>";
