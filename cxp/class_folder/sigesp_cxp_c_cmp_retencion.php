	      ...
	      if($as_tipo=='P')
		   {
			$ls_sql = " SELECT ".$ls_id." as id,RD.numrecdoc,RD.fecemidoc,SUM(RDC.monobjret) AS basimpiva,MAX(RD.codproalt) as codproalt,".
                      ...
                      " AND (SO.fecaprosol BETWEEN '".$ld_fecdesde."' AND '".$ld_fechasta."')AND (RD.estprodoc='C')".
					  $ls_joinalt."  AND".
...
/******************************************************************************************************************/
	...
	function uf_procesar_cmp_retencion($as_mes,$as_agno,$as_probendesde,$as_probenhasta,$as_tipo,$as_tiporet,&$aa_numcmp,$aa_seguridad)
	{
		...
		for($li_i=1;$li_i<=$li_totalfilaspro;$li_i++)
		{
			...
			$this->uf_get_documento($as_mes,$as_agno,$ls_codpro,$as_tipo,$as_tiporet,$ls_codproalt,&$la_RD);
				...
				if($lb_valido)
				{
					...
					if($lb_valido)
					{
						...
						//	$ls_codproalt=$this->io_datadocu->getValue("codproalt",$li_j);
							$ls_numope=$this->uf_get_nrooperacion($li_j);
	...
	function uf_update_datos_proveedor($as_codded,$as_nrocomp,$as_numsop,$as_codproalt)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Generar Comprobate MÉTODO->uf_update_datos_proveedor ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
			return false;
		}
		else
		{
	...
	function uf_update_cabecera($as_codded,$as_nrocomp,$as_codproalt,$as_nomproalt,$as_dirproalt,$as_rifproalt,$as_nitproalt)
	{
		...
		if($li_result===false)
		{	
			$this->io_msg->message("CLASE->Generar Comprobate MÉTODO->uf_update_cabecera ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		return $lb_valido;
    }	  
...
/*		$this->ds_numcmp= new class_datastore();
	...
	function uf_load_numeroinicial($as_codret)
	{
		...
		//////////////////////////////////////////////////////////////////////////////
		$ls_concomiva=1;
		$ls_sql="SELECT concomiva,concommun,concommil ".
				"  FROM sigesp_empresa ".
