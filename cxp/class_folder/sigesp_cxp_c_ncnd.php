...
class sigesp_cxp_c_ncnd
{
								 ...
								 $ld_fecha,$ls_tiponota,$ldec_monto,$ls_estsol,$ldec_moncar,$aa_seguridad)
	{
		...
		else
		{
			...
			if($this->ls_supervisor!=0)
			{
				...
				$ls_bodyenv="Se le envia la notificación de actualización en el modulo de CXP, se insertó la Nota de Credito - Debito  N°.. ";
	...
	function uf_actualizar_cabecera($ls_numncnd,$ls_numord,$ls_tipproben,$ls_codproben,$ls_numrecdoc,$ls_tipdoc,$ls_connota,$ld_fecha,$ls_tiponota,$ldec_monto,$ls_estsol,$aa_seguridad)
	{
		...
		else
		{
			...
			{
				$ls_fromname="Nota Debito - Credito";
				$ls_bodyenv="Se le envia la notificación de actualización en el modulo de CXP, se actualizó la Nota de Credito - Debito  N°.. ";
				$ls_nomper=$_SESSION["la_nomusu"];
				$lb_valido_3= $this->io_seguridad->uf_envio_correo_activo($ls_fromname,$ls_numncnd,$ls_bodyenv,$ls_nomper);
			}
			/////////////////////////////////         SEGURIDAD               /////////////////////////////	
		...
		for($li=1;$li<=$li_numrowspre;$li++)
		   {
			 ...
			 switch($ls_modalidad)
			 {
				...
				if(($ls_tiponota=="ND")||($ls_estafenc==0))
				{
				 ...
				 if ($lb_valido)
					{
					  $ls_sql = "INSERT INTO cxp_dc_spg(codemp, numsol, numrecdoc, codtipdoc, ced_bene, cod_pro, codope, numdc, codestpro, estcla, spg_cuenta, monto, procede_doc, numdoccom)
								 VALUES('".$ls_codemp."','".$ls_numord."','".$ls_numrecdoc."','".$ls_tipdoc."','".$ls_cedbene."','".$ls_codpro."','".$ls_tiponota."','".$ls_numncnd."','".$ls_codestproaux."','".$ls_estcla."','".$ls_cuentaspg."','".$ldec_monto."','".$ls_procede."','".$ls_numdoccom."')";
					  $li_rows=$this->io_sql->execute($ls_sql);
					  if ($li_rows===false)
						 {
						   $this->io_mensajes->message("CLASE->sigesp_cxp_c_ncnd METODO->uf_guardar_detalle ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
				 ...
				 if ($lb_valido)
					{
					  ...
					  $ls_sql = "INSERT INTO cxp_dc_spi(codemp, numsol, numrecdoc, codtipdoc, ced_bene, cod_pro, codope, numdc, codestpro, estcla, spi_cuenta, monto)
