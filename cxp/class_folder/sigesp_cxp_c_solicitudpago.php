...
//	    $lb_valido=$this->io_id_process->uf_check_id("cxp_solicitudes","numsol",&$as_numsol);   
		...
		if($lb_valido)
		{
			$ls_sql="INSERT INTO cxp_solicitudes (codemp, numsol, cod_pro, ced_bene, codfuefin, tipproben, fecemisol, consol,".
					"                             estprosol, monsol, obssol, estaprosol,procede,numordpagmin,codtipfon,repcajchi,nombenaltcre)".
			...
			else
			{
				...
				if($this->ls_supervisor!=0)
				{
					...
					$ls_bodyenv="Se le envia la notificación de actualización en el modulo de CXP, se insertó la solicitud de pago  N°.. ";
	...
	function uf_guardar($as_existe,&$as_numsol,$as_codpro,$as_cedbene,$as_codfuefin,$as_tipproben,$ad_fecemisol,$as_consol,
						...
						$as_numordpagmin,$as_codtipfon,$as_repcajchi,$as_nombenaltcre)
	{		
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_guardar
	...
	function uf_load_recepciones($as_numsol)
	{
		...
		$ls_sql="SELECT cxp_dt_solicitudes.numrecdoc, cxp_dt_solicitudes.monto, cxp_dt_solicitudes.codtipdoc,".
				"       cxp_documento.dentipdoc,cxp_dt_solicitudes.cod_pro,cxp_dt_solicitudes.ced_bene,cxp_rd.codproalt ".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_delete_solicitud($as_numsol,$as_codpro,$as_cedbene,$ad_fecemisol,$ai_totrow,$as_repcajchi,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_select_cheque($as_numsol)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Solicitud MÉTODO->uf_select_cheque ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
