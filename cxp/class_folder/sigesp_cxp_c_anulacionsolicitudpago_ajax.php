<?php
	...
	require_once("sigesp_cxp_c_anulacionsolicitudpago.php");
	$io_anulacion=new sigesp_cxp_c_anulacionsolicitudpago('../../');
	// proceso a ejecutar
	...
	{	
		...
		//	  Description: Método que impirme el grid de las solicitudes a ser anuladas 
		//	   Creado Por: Ing. Luis Anibal Lang
		// Fecha Creación: 24/12/2007								Fecha Última Modificación : 
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $io_grid, $io_funciones_cxp, $io_funciones, $io_anulacion, $io_mensajes;
		$ls_nivapro=$_SESSION["la_empresa"]["nivapro"];
		$ls_codasiniv="";
		$ls_codusu=$_SESSION["la_logusr"];
		$ls_codasiniv=$io_anulacion->uf_nivel_aprobacion_usu($ls_codusu,'3');
		$li_monnivhas=0;
		if($ls_codasiniv!="")
		{
		...
		$as_numsol="%".$as_numsol."%";
		$as_proben="%".$as_proben."%";
		$rs_datasol=$io_anulacion->uf_load_solicitudes($as_numsol,$ad_fecemides,$ad_fecemihas,$as_tipproben,$as_proben,$as_tipooperacion);
		$li_fila=0;
		if($rs_datasol!=false)
		{
			while($row=$io_anulacion->io_sql->fetch_row($rs_datasol))
			{
				...
				$lb_valido=$io_anulacion->uf_verificar_recepciones($ls_numsol,$lb_imprimir); 
