...
/*		$ls_sql="SELECT cxp_rd_spg.numdoccom, cxp_rd_spg.codestpro, cxp_rd_spg.spg_cuenta, cxp_rd_spg.monto, spg_cuentas.sc_cuenta, ".
	...
	function uf_load_cuentas_cargos($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Recepcion MÉTODO->uf_load_cuentas_cargos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$rs_data=false;
		}
		else
		{
								 ...
								 $as_estcla,$as_estact,$as_numordpagmin,$as_codtipfon,$as_repcajchi,$as_codproalt,$aa_seguridad)
	{
		...
		else
		{
			...
			if($this->ls_supervisor!=0)
			{
				...
				$ls_bodyenv="Se le envia la notificación de actualización en el modulo de CXP, se insertó la recepción de documentos  N°.. ";
...
//echo "<br>positivoooo";
	...
	function uf_load_monto_ajustes($as_comprobante,$as_procedencia,$as_tipodestino,$as_codpro,$as_cedbene,&$ai_monto)
	{
		...
		{
			$lb_valido=false; 
			$this->io_mensajes->message("CLASE->Recepción MÉTODO->uf_load_monto_ajustes ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			if(!$rs_data->EOF)
...
//echo "<br>br>".$ls_sql;
											...
											$as_codestpro4,$as_codestpro5,$as_estcla,$as_cuenta,&$ai_monto)
	{
		...
		{
			$lb_valido=false; 
			$this->io_mensajes->message("CLASE->Recepción MÉTODO->uf_load_baseimp_causado_anterior ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
...
//echo "<br>".$ls_sql;
		...
		{
			$lb_valido=false; 
			$this->io_mensajes->message("CLASE->Recepción MÉTODO->uf_validacion_compromiso ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			if(!$rs_data->EOF)
