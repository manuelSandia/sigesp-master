<?php
	...
	require_once("sigesp_cxp_c_anulacionrecepcion.php");
	$io_aprobacion=new sigesp_cxp_c_anulacionrecepcion('../../');
	// proceso a ejecutar
	...
	{	
		...
		if($rs_datasol!=false)
		{
			while($row=$io_aprobacion->io_sql->fetch_row($rs_datasol))
			{
				...
				$li_montotdoc_com=$row["montotdoc"];
				...
				if($li_rowspg>=1)
				{
					$lb_valido=$io_funciones_cxp->uf_verificar_cierre_spg("../../",$ls_estciespg);
					if($ls_estciespg=="1")
					{
						$lb_imprimir=false;
					}
				...
				if($li_rowscg>=1)
				{
					$lb_valido=$io_funciones_cxp->uf_verificar_cierre_scg("../../",$ls_estciescg);
					if($ls_estciescg=="1")
					{
						$lb_imprimir=false;
					}
				...
				if ($ls_nivapro==1)
				{
					if(($ls_codniv!="")&&($li_monnivhas!=0)&&($li_montotdoc_com <= $li_monnivhas))
				    {
						if($lb_imprimir)
						{
							$li_fila=$li_fila + 1;
