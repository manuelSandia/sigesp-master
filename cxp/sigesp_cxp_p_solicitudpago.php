<?php
   ...
   function uf_limpiarvariables()
   {
		...
		global $ls_totrowrecepciones,$io_fun_cxp,$ls_numsol,$ls_parametros,$ls_codfuefin,$ls_denfuefin,$ls_consol,$ls_obssol,$ls_consolanu;
		global $ls_numordpagmin,$ls_codtipfon,$ls_repcajchi,$ls_nombenaltcre,$ls_estbenalt;
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_estsol,$ld_fecemisol,$ls_numsol,$ls_tipodestino,$ls_codprovben,$ls_nomprovben,$ls_consol,$ls_obssol,$ls_consolanu;
		global $io_fun_cxp,$li_totrowrecepciones,$ls_codpro,$ls_cedbene,$ls_codfuefin,$li_monsol,$ls_rifproben;
		global $ls_numordpagmin,$ls_codtipfon,$ls_repcajchi,$ls_nombenaltcre;
		...
		$ls_nombenaltcre=$_POST["txtnombenaltcre"];
		if($ls_codfuefin=="")
		{
			$ls_codfuefin="--";
		}
   ...
   function uf_load_data(&$as_parametros)
   {
		...
		for($li_i=1;$li_i<$li_totrowrecepciones;$li_i++)
		{
			...
			$ls_auxpro=trim($io_fun_cxp->uf_obtenervalor("txtauxpro".$li_i,""));   
            $ls_auxben=trim($io_fun_cxp->uf_obtenervalor("txtauxben".$li_i,""));   
            $as_parametros=$as_parametros."&txtnumrecdoc".$li_i."=".$ls_numrecdoc."&txtcodtipdoc".$li_i."=".$ls_codtipdoc."".             
                                          ...
                                          "&txtauxpro".$li_i."=".$ls_auxpro."&txtauxben".$li_i."=".$ls_auxben.""; 
	...
	switch($ls_operacion)
	{
		...
		case "GUARDAR":
			...
			$lb_valido=$io_cxp->uf_guardar($ls_existe,&$ls_numsol,$ls_codpro,$ls_cedbene,$ls_codfuefin,$ls_tipodestino,
										   ...
										   $li_totrowrecepciones,$la_seguridad,$la_permisos["administrador"],$ls_numordpagmin,$ls_codtipfon,$ls_repcajchi,$ls_nombenaltcre);
		...
		case "ELIMINAR":
			uf_load_variables();
			$lb_valido=$io_cxp->uf_delete_solicitud($ls_numsol,$ls_codpro,$ls_cedbene,$ld_fecemisol,$li_totrowrecepciones,$ls_repcajchi,$la_seguridad);
			if(!$lb_valido)
			{
				uf_load_data(&$ls_parametros);
			}
        ...
        <td width="790"  height="136"><p>&nbsp;</p>
            <table width="721" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
              ...
              <tr>
                ...
                <td height="22" align="left"><input name="txtnumsol" type="text" id="txtnumsol" onBlur="javascript: ue_rellenarcampo(this,15);" value="<?php print $ls_numsol; ?>" maxlength="15" <?php if(($la_permisos["administrador"]!=1)||($ls_operacion!="NUEVO")){print "readonly";} ?> onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnopqrstuvwxyz'+'-/');" style="text-align:center"></td>
                <input name="hidrepcajchi" type="hidden" id="hidrepcajchi" value="<?php print $ls_repcajchi; ?>">
				<td height="22" colspan="2" style="text-align:left">Reposici&oacute;n
				  <input name="chkrepcajchi" type="checkbox" class="sin-borde" id="chkrepcajchi" value="1" <?php if($ls_repcajchi!="0"){print "checked";}?>></td>
              </tr>
              <tr>
              ...
              <tr>
                <td height="13"><div align="right">Justificaci&oacute;n</div></td>
			 ...
			 <tr>
                <td height="13"><div align="right">Concepto Anulaci&oacute;n</div></td>
                <td colspan="3" rowspan="2"><textarea name="txtconsolanu" cols="80" rows="2" id="txtconsolanu" onKeyUp="ue_validarcomillas(this);" disabled><?php print $ls_consolanu;?></textarea></td>
              </tr>
			 <tr>
                <td height="13">&nbsp;</td>
              </tr> 
              <tr>
                <td height="22" align="center"><div align="right" style="display:none">No. Orden de Pago Ministerio</div></td>
                <td height="13" align="center"><div align="left" style="display:none">
			  ...
			  ?>
              <tr>
                <td height="22" align="center"><div align="right">Beneficiario alterno para emisi&oacute;n de cheques </div></td>
                <td height="13" colspan="3" align="center"><div align="left">
                  <input name="txtnombenaltcre" type="text" id="txtnombenaltcre" value="<?php print $ls_nombenaltcre; ?>" size="80" maxlength="254">
                </div></td>
              </tr>
			  <?php 
			  	}
              ...
              <tr>
                ...
                <td height="13" colspan="3" align="center"><div align="left">
                  <input name="txtnombenaltcre" type="hidden" id="txtnombenaltcre" value="">
                </div></td>
              </tr>
			   <?php
			   }
...
function ue_guardar()
{
	...
	if(((lb_existe=="TRUE")&&(li_cambiar==1))||(lb_existe=="FALSE")&&(li_incluir==1))
	{
		...
		if(valido)
		{
			valido=ue_validarcampo(numsol,"El N�mero de Solicitud no puede estar vacio.",f.txtnumsol);
		}
		if(valido)
		{
		...
		if(valido)
		{
			...
			if(rowrecepciones<=1)
			{
				alert("Debe Tener al menos una Recepci�n de Documentos Seleccionada.");
				valido=false;
			}
		}
		if(valido)
...
function ue_cambiardestino()
{
	...
	else
	{
		f.txtcodigo.value="";
		f.txtnombre.value="";
		//Validacion de Boton Reposicion Caja Chica
		...
		if(tipdes!="-")
		{
			...
			if(tipdes=="P")
			{
				window.open("sigesp_cxp_cat_proveedor.php?tipo="+tipo+"&repcajchi="+repcajchi+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
			}
			else
			{
				window.open("sigesp_cxp_cat_beneficiario.php?tipo="+tipo+"&repcajchi="+repcajchi+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
			}	
		}
...
function ue_catalogorecepciones()
{
	...
	if(codproben!="")
	{
		window.open("sigesp_cxp_cat_recepcion.php?tipdes="+tipdes+"&codproben="+codproben+"&nombre="+nombre+"&numordpagmin="+numordpagmin+"&codtipfon="+codtipfon+"&repcajchi="+repcajchi+"&tipo=SOLICITUDPAGO","_blank","menubar=no,toolbar=no,scrollbars=yes,width=650,height=400,left=50,top=50,location=no,resizable=yes");
	}
	else
	{
		...
		if(confirm("�Desea eliminar el Registro actual?"))
		{
			...
			for(j=1;(j<rowrecepciones)&&(valido);j++)
			{
				if(j!=fila)
				{
					...
					auxpro	=eval("document.formulario.txtauxpro"+j+".value");   
                    auxben	=eval("document.formulario.txtauxben"+j+".value");
                    parametros=parametros+"&txtnumrecdoc"+li_i+"="+numrecdoc+"&txtdentipdoc"+li_i+"="+dentipdoc+"&txtmontotdoc"+li_i+"="+montotdoc+  
                               "&txtcodtipdoc"+li_i+"="+codtipdoc+"&txtauxpro"+li_i+"="+auxpro+"&txtauxben"+li_i+"="+auxben; 
