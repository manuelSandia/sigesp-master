...
function ue_search()
{
	...
	if((fecemides!="")&&(fecemihas!=""))
	{
		...
		ajax.send("catalogo=SOLICITUDPAGO&numsol="+numsol+"&fecemides="+fecemides+"&fecemihas="+fecemihas+
				  "&tipdes="+tipdes+"&codproben="+codproben+"&tipo="+tipo+"&orden="+orden+"&campoorden="+campoorden+"&repcajchi="+repcajchi);
	}
	else
	{
...
function ue_aceptar(ls_numsol,ls_codfuefin,ls_denfuefin,ls_codigo,ls_nombre,li_monsol,ls_estprosol,ls_estaprosol,ld_fecemisol,
					ls_estatus,ls_tipodestino,li_i,ls_rifproben,ls_numordpagmin,ls_codtipfon,repcajchi,conanusol,ls_nombenaltcre)
{
	...
	opener.document.formulario.txtconsolanu.value=conanusol;
	...
	opener.document.formulario.txtnombenaltcre.value=ls_nombenaltcre;
	if(repcajchi==1)
	{
