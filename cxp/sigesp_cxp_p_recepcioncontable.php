<?php
   ...
   function uf_load_variables()
   {
		...
		//	     Function: uf_load_variables
		//		   Access: private
		//	  Description: Funciï¿½n que carga todas las variables necesarias en la pï¿½gina
		//	   Creado Por: Ing. Yesenia Moreno/ Ing. Luis Lang
		// Fecha Creaciï¿½n: 29/04/2007								Fecha ï¿½ltima Modificaciï¿½n : 
		//////////////////////////////////////////////////////////////////////////////
              ...
              <tr>
				...
				<table width="969" border="0" cellpadding="0" cellspacing="1" class="celdas-blancas">
                  <tr>
                    ...
                    <td width="178" height="22"><div align="right">N&uacute;mero de Control </div></td>
                    <td width="312" height="22"><div align="left">
                  ...
                  <tr>
                    <td height="22"><div align="right">Justificaci&oacute;n</div></td>
                    <td colspan="3" rowspan="2"><textarea name="txtdencondoc" cols="100" rows="3" id="txtdencondoc"  onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnï¿½opqrstuvwxyzï¿½ï¿½ï¿½ï¿½ï¿½ ()@#!%/[]*-+_,.$ï¿½?ï¿½');"><?php print $ls_dencondoc;?></textarea></td>
                  </tr>
                  <tr>
                    <td height="22">&nbsp;</td>
                  </tr>
				  ...
				  <tr>
				    <td height="22"><div align="right" style="display:none">No. Orden de Pago Ministerio </div></td>
				    <td height="22"><div style="display:none"><input name="txtnumordpagmin" type="text" id="txtnumordpagmin" value="<?php print $ls_numordpagmin; ?>" size="19" maxlength="19" style="text-align:center" readonly>
				  ...
				  <tr>
                    ...
                    <td width="178" height="22"><div align="right">Impuesto
...
function ue_verificar_rd()
{
	...
	if(f.existe.value=="FALSE")
	{
		...
		if(parametros!="")
		{
			...
			// Pagina donde estï¿½n los mï¿½todos para buscar y pintar los resultados
...
function ue_guardar()
{
	...
	if(((lb_existe=="TRUE")&&(li_cambiar==1))||(lb_existe=="FALSE")&&(li_incluir==1))
	{
		...
		if(estaprord=="1")
		{
			alert("La Recepciï¿½n de Documentos, no se puede modificar ha sido Aprobada.");
		}
		else
		{
			valido=true;
			...
			if(valido)
			{
				valido=ue_validarcampo(codigo,"El Proveedor ï¿½ Beneficiario no puede estar vacio.",f.txtcodigo);
			}
			if(valido)
			{
				valido=ue_validarcampo(numrecdoc,"El Nï¿½mero de Recepciï¿½n no puede estar vacio.",f.txtnumrecdoc);
			}
			if(valido)
			{
			...
			if(valido)
			{
				valido=ue_validarcampo(fecregdoc,"La Fecha de Recepciï¿½n no puede estar vacia.",f.txtfecregdoc);
			}
			if(valido)
			{
				valido=ue_validarcampo(fecemidoc,"La Fecha dee Emisiï¿½n no puede estar vacia.",f.txtfecemidoc);
			}
			if(valido)
			{
			...
			if(valido)
			{
				if((presupuestario=="1")||(presupuestario=="2")) // Causa ï¿½ compromete y causa
				{
					if(totrowspg==0)
					{
						...
						if (confiva=="C")
						...
						else
						   {
							 if (parseFloat(cargos)!=parseFloat(li_cargos))
								{
								  alert("La suma de los detalles Presupuestarios del Cargo Difiere con los Otros Crï¿½ditos.");
								  valido=false;
								}
						   }
					}
			...
			if(valido)
			{
				...
				{
					alert("La Recepciï¿½n estï¿½ descuadrada Contablemente.");
					valido=false;
				}
			}
			if(valido)
			{
				...
				{
					alert("La Recepciï¿½n Debe ser Mayor que Cero.");
					valido=false;
				}
			}
			if(valido)
...
function ue_cambioctarepcajchi()
{
	...
	{
		alert("Debe seleccionar el destino de la recepciï¿½n(Proveedor / Beneficiario)");
	...
	{//FIN PRIMERA PARTE BLOQUE 1 AGREGADO POR OFIMATICA DE VENEZUELA EL 13-06-2011 
		...
		{
			alert("Se ha desconfigurado la opciï¿½n de caja chica, por favor vuelva a cargar los datos");
			f.operacion.value="NUEVO";
			f.existe.value="FALSE";		
			f.action="sigesp_cxp_p_recepcioncontable.php";
			f.submit();
...
function ue_eliminar()
{
	...
	if(li_eliminar==1)
	{
		if(lb_existe=="FALSE")
		{
			alert("Debe seleccionar una Recepciï¿½n de Documentos.");
		}
		else
		{
			estaprord=f.estaprord.value;
			if(estaprord=="1")
			{
				alert("La Recepciï¿½n de Documentos, no se puede eliminar ha sido Aprobada.");
			}
			else
			{
				...
				{
					alert("La Recepciï¿½n de Documentos, Fuï¿½ generada desde otro mï¿½dulo no se puede eliminar");
				}
			}
		}
	}
...
function ue_cambiartipodocumento()
{
	...
	if((presupuestario=="3")||(presupuestario=="4")) // Sin afectaciï¿½n presupuestaria
	{
		if(contable=="1")
		{
			ls_proceso="CONTABLE";
...
{
	f=document.formulario;
	// Se verifica si el destino es un proveedor ï¿½ beneficiario y se carga el catalogo
	// dependiendo de esa informaciï¿½n
	f.txtcodigo.value="";
	f.txtnombre.value="";
	f.recepcionexiste.value="false";
	tipdes=ue_validarvacio(f.cmbtipdes.value);
...
function ue_proveedoralterno()
{
	...
	{
		alert("Esta opciï¿½n esta permitida solo para agencias de viajes.");
	}
}
		...
		{		
			...
			if(valido)
			{
				if((f.recepcionexiste.value=="true")||(lb_existe))
				{
					...
					if((presupuesto=="3")||(presupuesto=="4")) // Ninguna ï¿½ Sin afectaciï¿½n
					{
						...
						else
						{
							if((codtipdoc!='-')&&(codproben!='')&&(numdoc!=''))
							{
								if(f.valclacon.value==1)
								{
									...
									{
										alert("Debe seleccionar una clasificaciï¿½n del concepto valida.");
										valido=false;
									}
								}
								if(valido)
							...
							{
								alert("Debe seleccionar el Tipo de Documento, Proveedor/Beneficiario y Escribir el Nï¿½mero de Documento");
							}
						}
					}
				}
				...
				{
					alert("El nro de Recepciï¿½n de Documentos no ha sido validada por favor tipeelo de nuevo.");
				}
			}
			else
			{
				alert("La fecha de emisiï¿½n debe ser menor a la fecha de vencimiento.");
			}
		}
		else
		{
			alert("La Recepciï¿½n de Documentos, Fuï¿½ generada desde otro mï¿½dulo no se puede modificar");
		}
	}
	else
	{
		alert("La Recepción esta Aprobada no puede realizar ningún cambio.");
	}
}
...
function ue_catalogo_cuentas_scg()
{
	f=document.formulario;
	estaprord = f.estaprord.value;
	//Se agrego la variable para verificar el estatus de la recepcion de documento
	estreg=f.txtestatus.value;
	valido=true;
	if(estaprord!="1")
	{
		//Se agregó el if que valida si la recepcion de documento esta anulada
		if (estreg=='Anulada'){
			alert("La Recepción de Documento esta Anulada, no la puede modificar.");
		}
		else
		{
			procede=f.procede.value;
			...
			{		
				...
				if((f.recepcionexiste.value=="true")||(lb_existe))
				{
					...
					if(contable=="2") // Ninguna ï¿½ Sin afectaciï¿½n
					{
			...
			{
				alert("La Recepción de Documentos, Fue generada desde otro módulo no se puede modificar");
			}
		}
	}
	else
...
function ue_catalogo_compromisos()
{
	...
	{
		alert("La Recepiï¿½n esta Aprobada no puede realizar ningï¿½n cambio.");
	}
}
...
function ue_catalogocreditos()
{
	...
	totrowspg=ue_calcular_total_fila_local("txtspgnrocomp");
	f.totrowspg.value=totrowspg;
	if (estaprord!='1') // No estï¿½ Aprobada
	{
		procede=f.procede.value;
		if(procede=="CXPRCD")
		{		
			...
			{
				if ((contable=='1' && presupuesto=='2')||(causadoparcial=='1')||((estmodiva=='1')&&(contable=='1' && presupuesto=='1'))) // Compromete y causa  ï¿½ si se puede modificar el IVA
				{  // Numero que me indica si el Documento es de tipo Compromete y Causa o
				   // ha reprocesado algun documento Tipo Causa para darle acceso al catalogo de otros Creditos.
					...
					{
						alert("Debe Seleccionar un proveedor ï¿½ Beneficiario, escribir el Nï¿½mero de Documento \n y el Monto del Subtotal Debe ser Mayor que Cero");
					}
					else
					{	
				...
				{
					alert("El Tipo de Documento, no admite Otros Crï¿½ditos. ");
				}
			}
			else
			{
				alert("El contribuyente es formal no se le aplican Otros Crï¿½ditos");
			}
		}
		else
		{
...
function ue_catalogodeducciones()
{
	...
	if (estaprord!='1') // No estï¿½ Aprobada
	{
		...
		{	
			...
			{
				alert("El monto de la Recepciï¿½n debe ser Mayor a Cero.");
			}
			else
			{
				if (procede!='SNOCNO')
				...
				{
					alert("La Nï¿½mina no puede ser objeto de Deducciones.");
				}
			}
		}
	}
							...
							else
							{
								if(ajax.readyState==4)
								{
									...
									else
									{
										if(ajax.status==404)
										{
											divgrid.innerHTML = "La pï¿½gina no existe";
										}
										else
	...
	else
	{
		...
		{		
			...
			if(validarcuenta!="")
			{
				...
				{
					alert("Debe Tener Detalles Preuspuestarios ï¿½ contables para poder Cerrar el asiento.");
				}
			}
			else
			{
