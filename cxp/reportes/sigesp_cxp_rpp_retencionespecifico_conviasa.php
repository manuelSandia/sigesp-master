<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_fecdes,$as_fechas,&$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/conviasa.jpg',116,525,540,40); // Agregar Logo
		...
		$tm=396-($li_tm/2);
		$io_pdf->addText($tm,505,11,$ls_periodo); // Agregar el título
	...
	function uf_print_cabecera($as_codded,$as_dended,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('cedula_rif'=>'<b>Cédula / RIF</b>','beneficiario'=>'<b>Beneficiario</b>','solicitud'=>'<b>Solicitud de Pago</b>','fecha'=>'<b>Fecha</b>','numcom'=>'<b>Comprobante','monto'=>'<b>Monto Objeto Retencion','porded'=>'<b>Alicuota de Retencion (%) </b>','retencion'=>'<b>Monto Retenido</b>'));
		$la_columna=array('cedula_rif'=>'','beneficiario'=>'','solicitud'=>'','fecha'=>'','numcom'=>'','monto'=>'','porded'=>'','retencion'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'titleFontSize' =>10,  // Tamaño de Letras de los títulos
						 ...
						 'colGap'=>1,
						 'width'=>530, // Ancho de la tabla
						 'maxWidth'=>530, // Ancho Máximo de la tabla  cedula_rif
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('cedula_rif'=>array('justification'=>'center','width'=>70),
						 			   'beneficiario'=>array('justification'=>'left','width'=>150),
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('cedula_rif'=>array('justification'=>'center','width'=>70),
						               ...
						               'solicitud'=>array('justification'=>'center','width'=>90), // Justificación y ancho de la columna
						 			   ...
						 			   'porded'=>array('justification'=>'right','width'=>80), // Justificación y ancho de la columna
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_select_retenciones($ls_codded,$ls_coddedhas);
	}
	if($lb_valido===false)
	{
		print("<script language=JavaScript>");
	...
	else
	{
		...
		for($li_j=1;$li_j<=$li_rowcargos;$li_j++)
		{
			...
			for ($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				$la_data[$li_i]=array('beneficiario'=>$ls_nombre,'solicitud'=>$ls_numsol,'fecha'=>$ld_fecemisol,'numcom'=>$ls_numcom,'cedula_rif'=>$ls_cedrif,'monto'=>$li_monsol,
