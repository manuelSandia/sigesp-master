...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estprosol,&$io_pdf)
	{
		...
		$io_pdf->addText(257,63,7,"SUBGERENCIA DE TESORERIA"); // Agregar el tï¿½tulo
		$io_pdf->addText(440,122,7,"GERENTE FINANZAS"); // Agregar el tï¿½tulo
		$io_pdf->addText(415,63,7,"CNEL(EJ) MARCO A. ROJAS TORRES"); // Agregar el tï¿½tulo
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_cabecera($as_numsol,$as_codigo,$as_nombre,$as_denfuefin,$ad_fecemisol,$as_consol,$as_obssol,
							   $ai_monsol,$as_monto,$as_nombenalt,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($as_nombenalt!="")
		{
			$la_data[1]=array('titulo'=>'<b> Beneficiario del Cheque: </b>'.$as_nombenalt);
			$la_columnas=array('titulo'=>'');
			$la_config=array('showHeadings'=>0, // Mostrar encabezados
							 'fontSize' => 9, // Tamaño de Letras
							 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				{
					$ls_codigo=$ls_codpro;
					$ls_nombre=$ls_rifpro." - ".$ls_nombre;
				}
				else
				{
					$ls_codigo=$ls_cedbene;
					$ls_nombre=$ls_cedbene." - ".$ls_nombre;
				}						
				/*if($ls_tiporeporte==0)
				{
				...
				$ls_nombenalt=$io_report->uf_load_beneficiario_alterno($ls_numsol);
				uf_print_encabezado_pagina($ls_titulo,$ls_numsol,$ld_fecemisol,$ls_estprosol,&$io_pdf);
				uf_print_cabecera($ls_numsol,$ls_codigo,$ls_nombre,$ls_denfuefin,$ld_fecemisol,$ls_consol,$ls_obssol,$li_monsol,$ls_monto,$ls_nombenalt,&$io_pdf);
