<?PHP
	...
	function uf_print_encabezadopagina($as_titulo,&$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/logo_caaez.jpg',190,545,570,50); // Agregar Logo
		...
		$tm=504-($li_tm/2);
		$io_pdf->addText($tm,530,12,$as_titulo); // Agregar el título	
		$ls_encabezado="Providencia Administrativa No. snat/2005/0056-A del 27/01/2005, Publicada en Gaceta Oficial No.38.136 del 28/02/2005";
        $li_tm=$io_pdf->getTextWidth(8,$ls_encabezado);
		$tm=504-($li_tm/2);
		$io_pdf->addText($tm,520,8,$ls_encabezado);
		$io_pdf->restoreState();
		$io_pdf->closeObject();
					           ...
					           $as_numcon,$ad_fecrep,$ai_estcmpret,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);								
		$io_pdf->Rectangle(400,345.5,370,30);	
		$io_pdf->addText(405,365,9,"<b>REGISTRO DE INFORMACION FISCAL DEL SUJETO RETENIDO (R.I.F)</b>"); // Agregar el titulo
		$io_pdf->addText(405,350,9,$as_rif); // Agregar el titulo									 
	...
	function uf_print_ley_ret_iva(&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'shaded'=>0, // Sombra entre Lineas
						 'width'=>900, // Ancho de la tabla
						 'maxWidth'=>900, // Ancho Minimo de la tabla
						 'xPos'=>500, // Orientacion de la tabla
						 'cols'=>array('name'=>array('justification'=>'center','width'=>875)));  
		$io_pdf->ezTable($la_data1,$la_columna,'',$la_config);	
		unset($la_data1);
		unset($la_columna);
		unset($la_config);
	...
	if($ls_tiporeporte==1)
	{
		$ls_titulo="<b>COMPROBANTE DE RETENCION DEL IMPUESTO AL VALOR AGREGADO EN Bs.F.</b>";	
	}
	else
	{
		$ls_titulo="<b>COMPROBANTE DE RETENCION DEL IMPUESTO AL VALOR AGREGADO EN Bs.</b>";	
	}
				...
				if($lb_valido)
				{
					...
					if($lb_valido)
					{
						...
						for($li_i=1;$li_i<=$li_total;$li_i++)
						{
							...
							$ls_numfac=trim($io_report->ds_detalle->data["numfac"][$li_i]);	
