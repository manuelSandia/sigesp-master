<?PHP
	...
	function uf_print_encabezadopagina($as_titulo,&$io_pdf)
	{
		...
		$io_pdf->addText(220,555,12,"AGENCIA BOLIVARIANA PARA ACTIVIDADES ESPACIALES");
		$io_pdf->addText(220,542,11,"COMPROBANTE DE RETENCION DEL IMPUESTO AL VALOR AGREGADO");
		$io_pdf->addText(220,530,9,"Ley IVA - Art. 11 Ser�n responsables del pago del Impuesto en calidad de agente de retenci�n, los");
		$io_pdf->addText(220,520,9,"compradores o adquirientes de determinados bienes muebles y los receptores de ");
		$io_pdf->addText(220,510,9,"ciertos servicios, a quienes la Administraci�n Tributaria designe como tal");

		//$io_pdf->line(23,75,357,75); //LINEA HORIZONTAL
		$io_pdf->addtext(60,185,10,"Agente de Retenci�n");
		$io_pdf->Rectangle(50,100,400,100);
		$io_pdf->addtext(460,185,10,"Recibido por");
		$io_pdf->Rectangle(450,100,400,100);
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_detalle($la_data,$ai_totconiva,$ai_totsiniva,$ai_totbasimp,$ai_totmonimp,$ai_totivaret,&$io_pdf)
	{
		...
		$la_columna=array('numope'=>'<b>Oper Nro.</b>',
						  ...
						  'numcomp'=>'<b>Numero de Comprobante</b>',
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('numope'=>array('justification'=>'center','width'=>60), // Justificacion y ancho de la columna
						 			   ...
						 			   'numcomp'=>array('justification'=>'center','width'=>80), // Justificacion y ancho de la columna
		...
		unset($la_columna);
		unset($la_config);
		$la_data[1]=array('name1'=>'<b>'.$ai_totconiva.'</b>','name2'=>'<b>'.$ai_totsiniva.'</b>','name3'=>'<b>'.$ai_totbasimp.'</b>',
		                  'name4'=>'','name5'=>'<b>'.$ai_totmonimp.'</b>','name6'=>'<b>'.$ai_totivaret.'</b>');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>200,
						 'cols'=>array('name1'=>array('justification'=>'center','width'=>90), // Justificacion y ancho de la columna
									   ...
									   'name5'=>array('justification'=>'center','width'=>70), // Justificacion y ancho de la columna
   						 			   'name6'=>array('justification'=>'center','width'=>71)));
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
				...
				if($lb_valido)
				{
					...
					if($lb_valido)
					{
						...
						for($li_i=1;$li_i<=$li_total;$li_i++)
						{
							...
							$la_data[$li_i]=array('numcomp'=>'','numope'=>$ls_numope,'fecfac'=>$ld_fecfac,'numfac'=>$ls_numfac,'numref'=>$ls_numref,
