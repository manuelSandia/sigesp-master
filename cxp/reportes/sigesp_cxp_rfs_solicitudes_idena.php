...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estprosol,&$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->line(480,700,480,780);
		$io_pdf->line(480,740,585,740);
        $io_pdf->Rectangle(15,700,570,80);
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],30,705,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$io_pdf->addJpegFromFile('../../shared/imagebank/comunas_idena.jpg',15,43,528,35); // Agregar banne inferior
		if ($as_estprosol=="A")
		{
			$io_pdf->addText(487,765,10,"<b>ANULADO</b>"); // Agregar la Fecha
		}
		$li_tm=$io_pdf->getTextWidth(11,$as_titulo);
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,730,11,$as_titulo); // Agregar el título
		$io_pdf->addText(485,750,9,"No. ".$as_numsol); // Agregar el título
		$io_pdf->addText(485,715,9,"Fecha ".$ad_fecregsol); // Agregar el título
		$io_pdf->addText(540,770,7,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(546,764,6,date("h:i a")); // Agregar la Hora
		// cuadro inferior
        $io_pdf->Rectangle(15,85,570,70);
		$io_pdf->line(15,145,585,145);		         // Horizontal Firmante Superior
		$io_pdf->addText(40,148,7,"ELABORADO POR");  // Agregar el título		
		$io_pdf->addText(42,90,7,$_SESSION["la_nomusu"], $_SESSION["la_apeusu"]);  // Agregar el título
		$io_pdf->line(145,85,145,155);		         // Vertical
		$io_pdf->addText(195,148,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(185,90,7,"FIRMA / SELLO / FECHA"); // Agregar el título
		$io_pdf->line(290,85,290,155);              // Vertical
		$io_pdf->addText(335,148,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(315,90,7,"ADMINISTRACIÓN Y FINANZAS"); // Agregar el título
		$io_pdf->line(435,85,435,155);		        // Vertical
		$io_pdf->addText(470,148,7,"CONTABILIZADO POR"); // Agregar el título
		$io_pdf->addText(465,90,7,"FIRMA / SELLO / FECHA"); // Agregar el título
		$io_pdf->line(15,100,585,100);		        // Horizontal Firmante Inferior
		$io_pdf->line(15,40,585,40); // Linea Horizontal pie de pagina
		$io_pdf->restoreState();
		$io_pdf->closeObject();
