<?php
	...
	else // Imprimimos el reporte
	{
		...
		$lo_datacenternum= &$lo_libro->addformat(array(num_format => '#,##0.00'));
		$lo_datacenternum->set_font("Verdana");
		$lo_datacenternum->set_align('center');
		$lo_datacenternum->set_size('9');
		$lo_dataleft= &$lo_libro->addformat();
		$lo_dataleft->set_text_wrap();
		$lo_dataleft->set_font("Verdana");
		$lo_dataleft->set_align('left');
		$lo_dataleft->set_size('9');
		$lo_dataleftnum= &$lo_libro->addformat(array(num_format => '#,##0.00'));
		$lo_dataleftnum->set_text_wrap();
		$lo_dataleftnum->set_font("Verdana");
		$lo_dataleftnum->set_align('left');
		$lo_dataleftnum->set_size('9');
		...
		{	
		    ...
		    $li_row=$li_row+1; 
			 $lo_hoja->write($li_row, 14,"TOTALES", $lo_dataright);
			 $lo_hoja->write($li_row, 15,$ldec_totcomconiva, $lo_dataright);
			 $lo_hoja->write($li_row, 16,$ldec_totcomsiniva, $lo_dataright);
			 $lo_hoja->write($li_row, 17,$ldec_basimpga, $lo_dataright);
			 $lo_hoja->write($li_row, 19,$ldec_totimpiva, $lo_dataright);
			 $lo_hoja->write($li_row, 20,$ldec_totivaret, $lo_dataright);
			$li_row1=$li_row+2;
			...
			$lo_hoja->write($li_row7, 2, "0,00", $lo_dataright);
			$lo_hoja->write($li_row7, 3,"342", $lo_datacenter);
			$lo_hoja->write($li_row7, 4,"0,00", $lo_dataright);
