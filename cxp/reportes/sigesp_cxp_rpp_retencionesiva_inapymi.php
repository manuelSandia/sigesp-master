<?PHP
	...
	function uf_print_encabezadopagina($as_titulo,&$io_pdf)
	{
        ...
        $line1="<b>Ley I.V.A.- Art 11: Serán responsables del pago de impuesto en calidad de agentes de retención, los compradores o</b>";
		$line1=utf8_decode($line1);
		$line2="<b>adquirientes de determinados bienes muebles y los receptores de ciertos servicios, a quienes la Administración tributaria</b>";
		$line2=utf8_decode($line2);
		$line3="<b>INSTITUTO NACIONAL DE DESARROLLO DE LA PEQUEÑA Y MEDIANA INDUSTRIA</b>";
		$line3=utf8_decode($line3);
		$io_pdf->addText(410,570,9,"<b>REPUBLICA BOLIVARIANA DE VENEZUELA</b>"); // Agregar el título
     	$io_pdf->addText(338,559,9,$line3); // Agregar el título
		$io_pdf->addText(380,548,9,"<b>DIRECCION GENERAL DE ADMINISTRACION Y FINANZAS</b>"); // Agregar el título
		$io_pdf->addText(120,488,8,$line1); // Agregar el título
		$io_pdf->addText(120,479,8,$line2); // Agregar el título
		$io_pdf->addText(120,470,8,"<b>designa como tal.</b>"); // Agregar el título
		$io_pdf->Rectangle(150,500,800,40);	
		$io_pdf->addText(240,515,13,"<b>".$as_titulo."</b>"); // Agregar el t�ulo				
		$io_pdf->restoreState();
		$io_pdf->closeObject();
					           ...
					           $as_numcon,$ad_fecrep,$ai_estcmpret,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);								 
		$io_pdf->Rectangle(845,453,100,28);	
		$io_pdf->addText(850,470,9,"<b>FECHA</b>"); // Agregar el titulo
		$io_pdf->addText(850,455,9,$ad_fecrep); // Agregar el titulo								 
		$io_pdf->Rectangle(845,407,100,28);	
		$io_pdf->addText(850,426,9,"<b>PERIODO FISCAL</b>"); // Agregar el titulo	
		$io_pdf->addText(850,411,9,$as_perfiscal); // Agregar el titulo									 
		$io_pdf->addText(700,96,9,"<b>FIRMA DEL AGENTE DE RETENCION</b>"); // Agregar el titulo
		$io_pdf->addText(685,107,9,"<b>_________________________________________</b>"); // Agregar el titulo
		$io_pdf->addText(140,107,9,"<b>_________________________________________</b>"); // Agregar el titulo
		$io_pdf->addText(150,96,9,"<b>FIRMA Y SELLO DEL SUJETO RETENIDO</b>"); // Agregar el titulo
		...
		unset($la_columna);
		unset($la_config);								 
		$io_pdf->Rectangle(400,407,370,30);	
		$io_pdf->addText(405,427,9,"<b>REGISTRO DE INFORMACION FISCAL DEL AGENTE DE RETENCION</b>"); // Agregar el titulo
		$io_pdf->addText(405,412,9,$as_rifagenteret); // Agregar el t�ulo									 
        //---------------------------------------------------------------------------------------------------
		$la_data[1]=array('titulo'=>'');
		$la_columna=array('titulo'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_columna);
		unset($la_config);								
		$io_pdf->Rectangle(400,305,370,30);	
		$io_pdf->addText(405,322,9,"<b>REGISTRO DE INFORMACION FISCAL DEL SUJETO RETENIDO (R.I.F)</b>"); // Agregar el titulo
		$io_pdf->addText(405,309,9,$as_rif); // Agregar el titulo									 
	}// end function uf_print_cabecera
	//--------------------------------------------------------------------------------------------------------------------------------			
			
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle($la_data,$ai_totconiva,$ai_totsiniva,$ai_totbasimp,$ai_totmonimp,$ai_totivaret,$as_ordenp,$ls_conceptosop,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_encabezadopagina
		//		   Access: private 
		...
		// Fecha Creación: 14/07/2007 
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$io_pdf->Rectangle(845,355,100,28);	
		$io_pdf->addText(850,373,9,"<b>ORDEN DE PAGO</b>"); // Agregar el titulo	
		$io_pdf->addText(850,360,9,$as_ordenp); // Agregar el titulo
				...
				if($lb_valido)
				{
					...
					if($lb_valido)
					{
						...
						for($li_i=1;$li_i<=$li_total;$li_i++)
						{
						  ...
						  $ls_conceptosop=$io_report->uf_select_concepto_sop($ls_ordenp);
						  ...
						  uf_print_detalle($la_data,$li_totalconiva,$li_totalsiniva,$li_totalbaseimp,$li_totalmontoimp,
						  				   $li_totalivaret,$ls_ordenp,$ls_conceptosop,&$io_pdf); 						 						  
						  unset($la_data);							 
					}
				}
				if($li_z<($li_totrow-1))
