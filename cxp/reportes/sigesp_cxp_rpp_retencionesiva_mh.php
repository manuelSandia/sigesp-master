<?PHP
	...
	function uf_print_encabezadopagina($as_titulo,&$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.'logo_mh.jpg',47,545,600,50); // Agregar Logo
		$io_pdf->addText(910,595,8,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(916,585,7,date("h:i a")); // Agregar la Hora
		$io_pdf->setStrokeColor(0,0,0);
        $io_pdf->Rectangle(50,500,800,40);
		$io_pdf->addText(230,515,13,"<b>".$as_titulo."</b>"); // Agregar el t�ulo
		$io_pdf->restoreState();
		$io_pdf->closeObject();
					           ...
					           $as_numcon,$ad_fecrep,$ai_estcmpret,$as_tlfagenteret,&$io_pdf)
	{
		...
		//       Function: uf_print_encabezadopagina
		//		   Access: private
		//	    Arguments: as_agenteret // Nombre del Agente de retenci�n
		//	    		   as_rifagenteret // Rif del Agente de retenci�n
		//	    		   as_perfiscal // Per�odo fiscal
		//	    		   as_codsujret // C�digo del Sujeto a retenci�n
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$io_pdf->setStrokeColor(0,0,0);
		$li_ajuste = 25;  //para manejar el desplazamiento hacia abajo por la inclusion del logo
		if($ai_estcmpret==2)
		{
		    $io_pdf->Rectangle(45+$li_ajuste,480,180,30);
			$io_pdf->addText(90+$li_ajuste,490,15,"<b> ANULADO </b>");
		}
		$io_pdf->ezSetDy(-25);
		...
		unset($la_columna);
		unset($la_config);
		$io_pdf->Rectangle(845,483-$li_ajuste,100,28);
		$io_pdf->addText(850,500-$li_ajuste,9,"<b>FECHA</b>"); // Agregar el titulo
		$io_pdf->addText(850,485-$li_ajuste,9,$ad_fecrep); // Agregar el titulo
		$io_pdf->Rectangle(845,439-$li_ajuste,100,28);
		$io_pdf->addText(850,456-$li_ajuste,9,"<b>PERIODO FISCAL</b>"); // Agregar el titulo
		$io_pdf->addText(850,443-$li_ajuste,9,$as_perfiscal); // Agregar el titulo
		...
		unset($la_columna);
		unset($la_config);
		$io_pdf->Rectangle(400,439-$li_ajuste,370,30);
		$io_pdf->addText(405,458-$li_ajuste,9,"<b>REGISTRO DE INFORMACION FISCAL DEL AGENTE DE RETENCION</b>"); // Agregar el titulo
		$io_pdf->addText(405,443-$li_ajuste,9,$as_rifagenteret); // Agregar el t�ulo
        //---------------------------------------------------------------------------------------------------
		$la_data[1]=array('titulo'=>'');
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_columna);
		unset($la_config);
		$io_pdf->Rectangle(845,400-$li_ajuste,100,28);
		$io_pdf->addText(850,418-$li_ajuste,9,"<b>TELEFONO</b>"); // Agregar el titulo
		$io_pdf->addText(850,405-$li_ajuste,9,$as_tlfagenteret); // Agregar el titulo
		//---------------------------------------------------------------------------------------------------
		$la_data[1]=array('titulo'=>'');
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_columna);
		unset($la_config);
		$io_pdf->Rectangle(400,355-$li_ajuste,370,30);
		$io_pdf->addText(405,375-$li_ajuste,9,"<b>REGISTRO DE INFORMACION FISCAL DEL SUJETO RETENIDO (R.I.F)</b>"); // Agregar el titulo
		$io_pdf->addText(405,360-$li_ajuste,9,$as_rif); // Agregar el titulo
	...
	function uf_print_firmas(&$io_pdf)
	{
		...
		// Fecha Creación: 131/10/2008
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$la_data1[1]=array('firma1'=>'',
		                   'firma2'=>'');

		$la_data1[2]=array('firma1'=>'',
		                   'firma2'=>'');

		$la_data1[3]=array('firma1'=>'',
		                   'firma2'=>'');

		$la_data1[4]=array('firma1'=>'',
		                   'firma2'=>'');

		$la_data1[5]=array('firma1'=>'_________________________________________________',
		                   'firma2'=>'_________________________________________________'   );

		$la_data1[6]=array('firma1'=>'FIRMA Y SELLO DEL AGENTE DE RETENCION',
						   'firma2'=>'FIRMA DEL SUJETO RETENIDO');
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('firma1'=>array('justification'=>'center','width'=>400),
						       	       'firma2'=>array('justification'=>'center','width'=>400)));
		$io_pdf->ezTable($la_data1,$la_columna,'',$la_config);
		unset($la_data1);
		unset($la_columna);
		unset($la_config);
	}// end function print_firmas
	...
	$ls_tlfagenteret=$_SESSION["la_empresa"]["telemp"];
	//--------------------------------------------------------------------------------------------------------------------------------
			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				if($lb_valido)
				{
					...
					uf_print_cabecera($ls_agenteret,$ls_rifagenteret,$ls_perfiscal,$ls_codsujret,$ls_nomsujret,$ls_rif,
					                  $ls_diragenteret,$ls_numcon,$ls_fecrep,$li_estcmpret,$ls_tlfagenteret,&$io_pdf);
