<?PHP
	...
	function uf_print_encabezadopagina($as_titulo,&$io_pdf)
	{
		...
		$io_pdf->line(45,47,940,47);
		$io_pdf->addJpegFromFile('../../shared/imagebank/banner_minfra.jpg',40,520,905,50); // Agregar Logo
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],43,460,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$io_pdf->addText(910,582,8,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(916,575,7,date("h:i a")); // Agregar la Hora
		$io_pdf->Rectangle(150,460,795,40);	
		$io_pdf->addText(240,475,14,"<b>".$as_titulo."</b>"); // Agregar el t�ulo				
		$io_pdf->restoreState();
		$io_pdf->closeObject();
					           ...
					           $as_numcon,$ad_fecrep,$ai_estcmpret,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);								 
		$io_pdf->Rectangle(845,410,100,28);	
		$io_pdf->addText(850,427,9,"<b>FECHA</b>"); // Agregar el titulo
		$io_pdf->addText(850,415,9,$ad_fecrep); // Agregar el titulo								 
		$io_pdf->Rectangle(845,370,100,28);	
		$io_pdf->addText(850,387,9,"<b>PERIODO FISCAL</b>"); // Agregar el titulo	
		$io_pdf->addText(850,375,9,$as_perfiscal); // Agregar el titulo									 
		...
		unset($la_columna);
		unset($la_config);								 
		$io_pdf->Rectangle(400,368,370,30);	
		$io_pdf->addText(405,387,9,"<b>REGISTRO DE INFORMACION FISCAL DEL AGENTE DE RETENCION</b>"); // Agregar el titulo
		$io_pdf->addText(405,374,9,$as_rifagenteret); // Agregar el t�ulo									 
        //---------------------------------------------------------------------------------------------------
		$la_data[1]=array('titulo'=>'');
		$la_columna=array('titulo'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_columna);
		unset($la_config);								
		$io_pdf->Rectangle(400,285,370,29);	
		$io_pdf->addText(405,303,9,"<b>REGISTRO DE INFORMACION FISCAL DEL SUJETO RETENIDO (R.I.F)</b>"); // Agregar el titulo
		$io_pdf->addText(405,290,9,$as_rif); // Agregar el titulo									 
	...
	if($lb_valido)
	{
		...
		else
		{
			...
			$io_pdf->ezSetCmMargins(6,2.5,3,3);
			$lb_valido=true;
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				$io_pdf->ezStartPageNumbers(910,35,10,'','',1); // Insertar el número de página
