<?PHP
	...
	function uf_print_encabezadopagina($as_titulo,&$io_pdf)
	{
		...
		$io_pdf->saveState();
//		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],47,535,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$io_pdf->addJpegFromFile('../../shared/imagebank/logo_sata2.jpg',47,535,550,50); // Agregar Logo
		$io_pdf->setStrokeColor(0,0,0);
       	$io_pdf->Rectangle(20,20,950,575);	
		$io_pdf->addText(240,528,12,"<b>".$as_titulo."</b>"); // Agregar el t�ulo				
		$io_pdf->restoreState();
		$io_pdf->closeObject();
					           ...
					           $as_numcon,$ad_fecrep,$ai_estcmpret,&$io_pdf)
	{
		...
		$la_data[1]=array('name'=>'<b>0.NRO COMPROBANTE </b>');
		...
		unset($la_columna);
		unset($la_config);								 
		$io_pdf->Rectangle(845,534,100,28);	
		$io_pdf->addText(850,552,9,"<b>1.FECHA</b>"); // Agregar el titulo
		$io_pdf->addText(850,540,9,$ad_fecrep); // Agregar el titulo								 
		$io_pdf->Rectangle(845,490,100,28);	
		$io_pdf->addText(850,507,9,"<b>4.PERIODO FISCAL</b>"); // Agregar el titulo	
		$io_pdf->addText(850,495,9,$as_perfiscal); // Agregar el titulo									 
		...
		unset($la_columna);
		unset($la_config);	
		$la_data[1]=array('name'=>'<b>2.NOMBRE O RAZON SOCIAL DEL AGENTE DE RETENCION </b>');
		...
		unset($la_columna);
		unset($la_config);								 
		$io_pdf->Rectangle(400,490,370,30);	
		$io_pdf->addText(405,507,9,"<b>3.REGISTRO DE INFORMACION FISCAL DEL AGENTE DE RETENCION</b>"); // Agregar el titulo
		$io_pdf->addText(405,495,9,$as_rifagenteret); // Agregar el titulo									 
        //---------------------------------------------------------------------------------------------------
		$la_data[1]=array('titulo'=>'');
		$la_columna=array('titulo'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_config);			
		//---------------------------------------------------------------------------------------------------
		$la_data[1]=array('name'=>'<b>5.DIRECCION FISCAL DEL AGENTE DE RETENCION</b>  ');
		...
		unset($la_config);			
		//---------------------------------------------------------------------------------------------------
		$la_data[1]=array('name'=>'<b>6.NOMBRE O RAZON SOCIAL DEL SUJETO RETENIDO</b>  ');
		...
		unset($la_columna);
		unset($la_config);								
		$io_pdf->Rectangle(400,395,370,30);	
		$io_pdf->addText(405,413,9,"<b>7.REGISTRO DE INFORMACION FISCAL DEL SUJETO RETENIDO (R.I.F)</b>"); // Agregar el titulo
		$io_pdf->addText(405,400,9,$as_rif); // Agregar el titulo									 
	...
	function uf_print_detalle($la_data,$ai_totconiva,$ai_totsiniva,$ai_totbasimp,$ai_totmonimp,$ai_totivaret,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data[1]=array('name'=>'TOTALES','name1'=>$ai_totconiva,'name2'=>$ai_totsiniva,'name3'=>$ai_totbasimp,
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>200,
						 'cols'=>array('name'=>array('justification'=>'right','width'=>50), // Justificacion y ancho de la columna
						               'name1'=>array('justification'=>'center','width'=>89), // Justificacion y ancho de la columna
						 			   'name2'=>array('justification'=>'center','width'=>71), // Justificacion y ancho de la columna
									   ...
									   'name5'=>array('justification'=>'center','width'=>69), // Justificacion y ancho de la columna
	...
	function uf_print_firmas(&$io_pdf)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$io_pdf->ezSetDy(-25);
		$la_data[1]=array('name1'=>'CONFORME','name2'=>'FIRMA Y SELLO');
		$la_data[2]=array('name1'=>'','name2'=>'AGENTE DE RETENCION');
		$la_data[3]=array('name1'=>'','name2'=>'');
		$la_data[4]=array('name1'=>'','name2'=>'');
		$la_data[5]=array('name1'=>'','name2'=>'');
		$la_data[5]=array('name1'=>'_____________________________________','name2'=>'_____________________________________');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>900, // Ancho Mínimo de la tabla
						 'xPos'=>500, // Orientación de la tabla
						 'cols'=>array('name1'=>array('justification'=>'center','width'=>500), // Justificacion y ancho de la columna
  						 			   'name2'=>array('justification'=>'center','width'=>500))); 
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
	...
	if($ls_tiporeporte==1)
	{
		$ls_titulo="COMPROBANTE DE RETENCION EN Bs.F.";	
	}
	else
	{
		$ls_titulo="COMPROBANTE DE RETENCION EN Bs.";	
	}
