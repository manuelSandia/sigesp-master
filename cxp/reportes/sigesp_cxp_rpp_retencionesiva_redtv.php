<?PHP
	...
	function uf_print_encabezadopagina($as_titulo,&$io_pdf)
	{
       ...
       //$io_pdf->Rectangle(150,540,800,40);	
        $io_pdf->Rectangle(30,60,940,530);	
		$io_pdf->addText(560,555,11,"<b>".$as_titulo."</b>"); // Agregar el t�ulo				
		$io_pdf->addText(610,540,11,"<b>I.V.A. Providencia Administrativa No. SNAT/2002/1,455</b>"); // Agregar el t�ulo				
		$io_pdf->line(570,80,845,80);
		$io_pdf->addText(610,70,10,"<b>FIRMA Y SELLO AGENTE DE RETENCION</b>"); // Agregar el t�ulo				
		$io_pdf->restoreState();
		$io_pdf->closeObject();
					           ...
					           $as_numcon,$ad_fecrep,$ai_estcmpret,&$io_pdf)
	{
		...
		$io_pdf->addText(850,443,9,"Año: ".substr($as_perfiscal,0,4)); // Agregar el titulo									 
		$io_pdf->addText(910,443,9,"Mes: ".substr($as_perfiscal,4,2)); // Agregar el titulo									 
	...
	function uf_print_detalle($la_data,$ai_totconiva,$ai_totsiniva,$ai_totbasimp,$ai_totmonimp,$ai_totivaret,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data[1]=array('name'=>'TOTAL RETENCIONES','name1'=>$ai_totconiva,'name2'=>$ai_totsiniva,'name3'=>$ai_totbasimp,
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>900, // Ancho Mínimo de la tabla
						 'xPos'=>500, // Orientación de la tabla
						 'cols'=>array('name'=>array('justification'=>'center','width'=>480), // Justificacion y ancho de la columna
