<?PHP
	...
	function uf_print_encabezado_pagina(&$io_pdf)
	{
		...
		$li_tm=$io_pdf->getTextWidth(13,"<b>COMPROBANTE DE RETENCION (AR-CV) </b>");
		$tm=396-($li_tm/2);
		$io_pdf->addText($tm,555,13,"<b>COMPROBANTE DE RETENCION (AR-CV) </b>"); // Agregar el título
							   ...
							   $as_apebene,$as_nacben,$as_cedbene,$as_numpasben,$as_dirbene,$as_telbene,&$io_cabecera,&$io_pdf)
	{
		...
		$io_pdf->rectangle(27,30,719,80);
//		$io_pdf->line(385,30,385,110);
		$io_pdf->addText(55,95,9,'AGENTE DE RETENCIÓN  (SELLO Y FIRMA)'); // Agregar el título
...
//		$io_pdf->addText(540,95,9,' PARA USO DE LA ADMINISTRACIÓN PÚBLICA'); // Agregar el título
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('fecha'=>'             Fecha de Pago             Día    Mes    Año','total'=>'Total Cantidad Pagada',
		                     'retencion'=>'Cantidad Objeto Retención','porcentaje'=>'% de Ret.',
							 'impuesto'=>'Impuesto Retenido','pagacumulado'=>'Total Cantidad Pagada Acumulada','totalacumulado'=>'Total Cantidad Obj Ret. Acumulada',
							 'retencionacumulada'=>'Impuesto Retención Acumulada'));
		$la_columna=array('fecha'=>'','total'=>'','retencion'=>'','porcentaje'=>'','impuesto'=>'','pagacumulado'=>'','totalacumulado'=>'','retencionacumulada'=>'' );
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 8, // Tamaño de Letras
						 'showLines'=>0, // Mostrar Líneas
						 'shaded'=>0, // Sombra entre líneas
						 ...
						 'cols'=>array('fecha'=>array('justification'=>'center','width'=>118),
						               'total'=>array('justification'=>'center','width'=>86),
									   'retencion'=>array('justification'=>'center','width'=>86),
									   'porcentaje'=>array('justification'=>'center','width'=>86),
									   'impuesto'=>array('justification'=>'center','width'=>86),
									   'pagacumulado'=>array('justification'=>'center','width'=>86),
									   'totalacumulado'=>array('justification'=>'center','width'=>86),
									   'retencionacumulada'=>array('justification'=>'center','width'=>86))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		unset($la_data);
		unset($la_columna);
		unset($la_config);
	...
	function uf_print_pie_cabecera($ai_total_pagado,$ai_total_retencion,$ai_total_impuesto,&$io_pdf)
	{
		...
		$la_data=array(array('total'=>'<b>Total</b>', 'pagado'=>$ai_total_pagado, 'retencion'=>$ai_total_retencion,
		                     'porcentaje'=>'','impuesto'=>$ai_total_impuesto,'pagacumulado'=>'','totalacumulado'=>'','retencionacumulada'=>''));
		$la_columna=array('total'=>'','pagado'=>'','retencion'=>'','porcentaje'=>'','impuesto'=>'','pagacumulado'=>'','totalacumulado'=>'','retencionacumulada'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 8, // Tamaño de Letras
						 'showLines'=>0, // Mostrar Líneas
						 'shaded'=>0, // Sombra entre líneas
						 ...
						 'cols'=>array('total'=>array('justification'=>'center','width'=>118),
						               'pagado'=>array('justification'=>'center','width'=>86),
									   'retencion'=>array('justification'=>'center','width'=>86),
									   'porcentaje'=>array('justification'=>'center','width'=>86),
									   'impuesto'=>array('justification'=>'center','width'=>86),
									   'pagacumulado'=>array('justification'=>'center','width'=>86),
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			if($lb_valido)
			{
				...
				for($li_s=1;$li_s<=$li_totrow_det;$li_s++) 
				{
					...
					$li_montotdocacumulado=$li_montotdocacumulado+$io_report->ds_detalle->data["montotdoc"][$li_s];
					$li_retencionacumulada=$li_retencionacumulada+$io_report->ds_detalle->data["monret"][$li_s];
					$li_montotdocacumuladoaux=number_format($li_montotdocacumulado,2,",",".");
					...
					$la_data[$li_s]=array('fecha'=>$ls_fecha,'total'=>$li_montotdoc,'retencion'=>$li_monobjret,
					                      'porcentaje'=>$li_porded,'impuesto'=>$li_monret,'pagacumulado'=>$li_montotdocacumuladoaux,'totalacumulado'=>$li_total,
