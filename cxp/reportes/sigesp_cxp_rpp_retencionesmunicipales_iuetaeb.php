<?php
	...
	function uf_print_encabezado_pagina($as_titulo,&$io_pdf)
	{
		...
		$tm=396-($li_tm/2);
		$io_pdf->addText($tm,530,9,"ARTICULO 114 ORDENANZA SOBRE ACTIVIDADES ECONOMICAS"); // Agregar el título
	...
	function uf_print_cabecera($as_numcon,$ad_fecrep,$as_agenteret,$as_rifagenteret,$as_perfiscal,$as_licagenteret,$as_diragenteret,
							   $as_nomsujret,$as_rif,$as_numlic,$ai_estcmpret,$ld_feccausa,$ls_concepto_solpago,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_encabezadopagina
		//		   Access: private
		...
		$la_data[2]=array('name'=>$ld_feccausa);
		$la_columna=array('name'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 8, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Lieas
		...
		unset($la_config);
		$io_pdf->ezSetY(455);
		$la_data[1]=array('name'=>'<b>CONCEPTO ORDEN DE PAGO</b>');
		$la_data[2]=array('name'=>$ls_concepto_solpago);
		$la_columna=array('name'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'shaded'=>0, // Sombra entre lineas
						 'shadeCol2'=>array(0.9,0.9,0.9), // Color de la sombra
						 'xPos'=>675, // Orientación de la tabla
						 'xOrientation'=>'left',
						 'width'=>640, // Ancho de la tabla
						 'maxWidth'=>640,
	...
	function uf_print_detalle($la_data,$ai_totbasimp,$ai_totmonimp,$as_rifagenteret,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data1[0]=array('firma1'=>'','firma2'=>'');
		$la_data1[1]=array('firma1'=>'','firma2'=>'');
		$la_data1[2]=array('firma1'=>'____________________________','firma2'=>'____________________________');
		$la_data1[3]=array('firma1'=>'AGENTE DE RETENCION','firma2'=>'LIDIA RAMOS');
		$la_data1[4]=array('firma1'=>'DPTO. RECURSOS FINANCIEROS','firma2'=>'LIQUIDADOR');
		$la_data1[5]=array('firma1'=>'','firma2'=>'');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
				 		 'cols'=>array('firma1'=>array('justification'=>'center','width'=>450), // Justificación y ancho de la columna
						 			   'firma2'=>array('justification'=>'center','width'=>450))); // Justificación y ancho de la columna
		$io_pdf->addText(350,120,10,"Fecha de Liquidación"); // Agregar la Fecha
		$io_pdf->addText(367,130,10,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->ezSetDy(-50);
		$io_pdf->ezTable($la_data1,$la_columna,'',$la_config);
		unset($la_data1);
		unset($la_columna);
	...
	if($lb_valido)
	{
		...
		else
		{
			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				$lb_valido=$io_report->uf_retencionesmunicipales_detalle_solpago($ls_numcom);
				$ld_feccausa = $io_report->ds_detalle_solpago->data["fecha"][1];
				$ls_concepto_solpago = $io_report->ds_detalle_solpago->data["descrip"][1];
				//$io_report->ds_detalle_solpago->reset_ds();
				$lb_valido=$io_report->uf_retencionesmunicipales_proveedor($ls_numcom,$ls_mes,$ls_anio);

				if($lb_valido)
				{
					$li_total=$io_report->DS->getRowCount("numcom");//print 'antes del for :'.$li_total;
					...
					uf_print_cabecera($ls_numcon,$ls_fecrep,$ls_agenteret,$ls_rifagenteret,$ls_perfiscal,$ls_licagenteret,
									  $ls_diragenteret,$ls_nomsujret,$ls_rif,$ls_numlic,$li_estcmpret,$ld_feccausa,$ls_concepto_solpago,&$io_pdf);
