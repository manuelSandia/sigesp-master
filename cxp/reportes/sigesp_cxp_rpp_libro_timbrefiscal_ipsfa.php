<?php
	...
	function uf_print_encabezado_pagina($as_titulo,&$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/logo_sat_dc.jpeg',665,530,100,$_SESSION["ls_height"]); // Agregar Logo
		$li_tm=$io_pdf->getTextWidth(12,$as_titulo);
		$tm=396-($li_tm/2);
		$io_pdf->addText($tm,540,12,$as_titulo); // Agregar el título
		$io_pdf->addText(722,603,8,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(728,596,7,date("h:i a")); // Agregar la Hora
		$io_pdf->restoreState();
		$io_pdf->closeObject();
...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_firmas(&$io_pdf)
	{
		...
		$la_data[6]=array('firma1'=>'<b>TESORERO / AGENTE DE RETENCION</b>','firma2'=>'<b>Cuentas por Pagar / Tributos</b>');
	...
	if($lb_valido)
	{
		...
		else
		{
			...
			while (!$rs_data->EOF)
			{
				...
				$ls_numsop=$rs_data->fields["numsop"];
				...
				$la_data[$li_i]=array('fecha'=>$ls_fecrep,'nombre'=>$ls_nomsujret, 'rif'=>$ls_rif,
									  ...
									  'monimp'=>number_format($li_totimp,2,",","."),'municipio'=>$ls_denmun,'comp'=>$ls_numcon,'obs'=>$ls_numsop);
				$rs_data->MoveNext();	
