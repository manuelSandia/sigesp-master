<?php
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado($as_agente,$as_nombre,$as_rif,$as_nit,$as_telefono,$as_direccion,$ls_consol,$ls_numsol,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$li_periodo = substr($_SESSION["la_empresa"]["periodo"],0,4);

		$la_data[1]=array('name'=>'<b><i>Periodo Fiscal:    </i></b>'.$li_periodo);
		$la_data[2]=array('name'=>'<b><i>Agente de Retención:</i></b>'."  ".$as_agente);
		$la_data[3]=array('name'=>'<b><i>Rif de la Institución:</i></b>'."  ".$_SESSION["la_empresa"]["rifemp"]);
		$la_data[4]=array('name'=>'<b><i>Nombre o Razón Social:</i></b>'."  ".$as_nombre);
		$la_data[5]=array('name'=>'<b><i>Concepto:</i></b>'."  ".$ls_consol);
                $la_data[6]=array('name'=>'<b><i>Nº Solicitud de Pago:</i></b>'."  ".$ls_numsol);//orden de pago******
                $la_data[7]=array('name'=>'<b><i>Rif de Proveedor o beneficiario:</i></b>'."  ".$as_rif);//rif pro o ben******
//		$la_data[5]=array('name'=>'<b><i>Concepto:</i></b>'.$ls_consol);                                                        <b><i>NIT:</i></b>  ".$as_nit);
				...
				switch ($ls_procede)
				{
					...
					default:
						$lb_valido= $io_report->uf_retencionesislr_cxp($ls_numsol);
                                                $iut=1;//verifik reporte
					break;
				}
				if($lb_valido)
				{
					$li_total=$io_report->DS->getRowCount("numdoc");
                                      // print_r($io_report->DS->data);
					for($li_i=1;($li_i<=$li_total);$li_i++)
					{
						...
						$ld_fecemidoc=$io_funciones->uf_convertirfecmostrar($io_report->DS->data["fecemidoc"][$li_i]);
                                      if($iut==1)//*****          
                                        $ld_fecemidoc=$io_funciones->uf_convertirfecmostrar($io_report->DS->data["fecemisol"][$li_i]);//fecha emision solicitud
						...
						if($ls_codigo!=$ls_codigoant)
						{
							...
							uf_print_encabezado($ls_agente,$ls_nombre,$ls_rif,$ls_nit,$ls_telefono,$ls_direccion,$ls_consol,$ls_numsol,$io_pdf);
