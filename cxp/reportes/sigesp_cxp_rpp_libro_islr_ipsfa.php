...
//--------------------------------------------------------------------------------------------------------------------------------
							...
							&$io_pdf)
	{
		...
		$la_data[0]=array('total1'=>'<b>TOTAL RETENCIÓN Bs.</b>','total2'=>$ai_totalret.'</b>');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>0, // Mostrar Líneas
						 'shaded'=>0, // Sombra entre líneas
						 ...
						 'xPos'=>450, // Orientación de la tabla
				 		 'cols'=>array('total1'=>array('justification'=>'right','width'=>200), // Justificación y ancho de la columna
						 			   'total2'=>array('justification'=>'right','width'=>90))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,'','',$la_config);
		unset($la_data);
		unset($la_config);
	...
	if($lb_valido)
	{
		...
		else
		{
				...
				while(!$rs_datadetalle->EOF)
				{
					...
					switch(trim($li_porcentaje))
					{
						case 0.02:
							...
							$la_data[]=array('islr1'=>$ls_nombre,'islr2'=>$ls_rif,
			...
			if(count($la_data)>0) // Si no ocurrio ningún error
			{
