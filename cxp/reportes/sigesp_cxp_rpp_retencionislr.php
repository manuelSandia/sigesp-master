<?php
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	//Se agregó el parámetro $ls_rifagente para ser impreso en el reporte
	function uf_print_encabezado($as_agente,$as_rifagente,$as_nombre,$as_rif,$as_nit,$as_telefono,$as_direccion,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//Se agrega al arreglo el Rif del Agente de Retención
		$la_data[2]=array('name'=>'<b><i>Rif Agente de Retención:</i></b>'."  ".$as_rifagente);
		$la_data[3]=array('name'=>'<b><i>Nombre o Razón Social:</i></b>'."  ".$as_nombre);
		$la_data[4]=array('name'=>'<b><i>RIF:</i></b>'."  ".$as_rif."                                                        <b><i>NIT:</i></b>  ".$as_nit);
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle($as_numsol,$as_concepto,$as_fechapago,$ad_monto,$ad_monret,$ad_porcentaje,$as_numcon,$as_comprobante,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_data[1]=array('comprobante'=>'<b><i>N° Comprobante:</i></b>'."  ".$as_comprobante);	
		$la_columna=array('comprobante'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
					     'fontSize' => 10,  // Tamaño de Letras
					     'showLines'=>0,    // Mostrar Líneas
					     'shaded'=>0,       // Sombra entre líneas
					     'width'=>530,     // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('comprobante'=>array('justification'=>'left','width'=>500))); // Ancho Máximo de la tabla
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);		       
		unset($la_data);
		unset($la_columna);
		unset($la_config);
	...
	//Se agregaron las variables para la impresion de los totales de Monto Objeto de Retención y Total impuesto retenido
	function uf_print_firmas(&$io_pdf,$li_totalmonobjret,$li_totalretenido)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// Fecha Creación: 05/07/2007 
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//Se agrego la impresion de los totales de Monto Objeto de Retención y Total impuesto retenido
		
		$la_data[1]=array('name'=>'<b><i>Totales</i></b>','totalobj'=>$li_totalmonobjret,'totalret'=>$li_totalretenido);	
		$la_columna=array('name'=>'','totalobj'=>'','totalret'=>'');
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
					     'fontSize' => 10,  // Tamaño de Letras
					     'showLines'=>0,    // Mostrar Líneas
					     'shaded'=>0,       // Sombra entre líneas
						 ...
						 'cols'=>array('name'=>array('justification'=>'left','width'=>100),
		                               'totalobj'=>array('justification'=>'right','width'=>150),
		                               'totalret'=>array('justification'=>'right','width'=>250))); // Ancho Máximo de la tabla
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);		       
		unset($la_data);
		unset($la_columna);
		unset($la_config);
		...
		$la_data[3]=array('firma1'=>'____________________________','firma2'=>'____________________________');
		$la_data[4]=array('firma1'=>'AGENTE DE RETENCION','firma2'=>'BENEFICIARIO');
    ...
    // Se asigna a una variable el rif del agente de retención
	...
	if($lb_valido)
	{
		...
		else
		{
			...
			$lb_valido=true;
			$ls_codigoant="";
			//Se agregan acumuladores para los totales
			$li_totalmonobjret = 0;
			$li_totalretenido = 0;
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				if($lb_valido)
				{
					...
					for($li_i=1;($li_i<=$li_total);$li_i++)
					{
						...
						//Se busca el numero de certificado de retencion
						...
						//Se obtienen los valores de los montos sin el formato
						...
						if($ls_codigo!=$ls_codigoant)
						{
							if($li_z>=1)
							{
								//Se le coloca formato a los totales
								$li_totalmonobjret = number_format($li_totalmonobjret,2,',','.');
							    $li_totalretenido = number_format($li_totalretenido,2,',','.');
							    //Se agregaron los parametros de totales a la impresión de la firma
								uf_print_firmas($io_pdf,$li_totalmonobjret,$li_totalretenido);
								$io_pdf->ezNewPage();
								//Se limpian los totales para que se acumulen en el nuevo proveedor  
								$li_totalmonobjret = 0;
								$li_totalretenido = 0;
							}
							//Se agregó el parámetro $ls_rifagente para ser impreso en el reporte
							uf_print_encabezado($ls_agente,$ls_rifagente,$ls_nombre,$ls_rif,$ls_nit,$ls_telefono,$ls_direccion,$io_pdf);
							$ls_codigoant=$ls_codigo;
						}
						
						//Se van acumulando los totales Si se está recorriendo al mismo proveedor o beneficiario
						$li_totalmonobjret = $li_totalmonobjret + $li_monobjret;
						$li_totalretenido = $li_totalretenido + $li_retenido;
						
						//Se le coloca el formato a los montos
						$li_monobjret=number_format($li_monobjret,2,',','.');    
						$li_retenido=number_format($li_retenido,2,',','.'); 
						uf_print_detalle($ls_numdoc,$ls_consol,$ld_fecemidoc,$li_monobjret,$li_retenido,$li_porcentaje,$ls_numref,$ls_comprobante,$io_pdf);
					}
				}	
			}
			//Se le coloca formato a los totales
