<?php
   ...
   session_start();   
	...
	function uf_print_encabezado_pagina($as_titulo,$ls_mes,$ls_ano,&$io_pdf)
	{
		...
		$tm=505-($li_tm/2);
		$io_pdf->addText($tm,550,16,$as_titulo); // Agregar el ttulo
		$li_tm=$io_pdf->getTextWidth(14,$ls_periodo);
		$tm=510-($li_tm/2);
		$io_pdf->addText(35,520,10,"<b>".$ls_orgnom."<b>"); // Agregar Texto		
	...
	function uf_print_titulo(&$io_pdf)
	{
		...
		$la_config =array('showHeadings'=>0,     // Mostrar encabezados
						  ...
						  'maxWidth'=>130,
						  'cols'=>array('name1'=>array('justification'=>'center','width'=>160))); // Justificacin y ancho de la columna
	...
	function uf_print_cabecera(&$io_pdf)
	{
		...
		$la_data=array(array('name20'=>'<b>Nro Cheque</b>','name1'=>'<b>Nro Oper</b>',
							 ...
							 'name4'=>'<b>Nombre o Razn Social</b>','name5'=>'<b>Nro Compr.</b>',
	...
	function uf_print_table_default($ld_monto,$ad_totbasimp,$ad_totimpuestos,$ld_sumcom,$ad_totcomsiniva,$ad_totimpred,$ad_totbasimp8,$ad_basimpga,$ad_totgenadi,&$io_pdf)
	{
		...
		$la_data[0] = array('name1'=>'','name2'=>'','name3'=>'<b>Base Imponible</b>','name4'=>'','name5'=>'<b>Crdito Fiscal</b>','name6'=>'<b>Iva Retenido a Terceros</b>','name7'=>'<b>Anticipo IVA</b>');
		$la_data[1] = array('name1'=>'Total: Compras Excentas y/o sin derecho a crdito fiscal','name2'=>'30','name3'=>$ad_totcomsiniva,'name4'=>'','name5'=>'','name6'=>'','name7'=>'');
	    $la_data[2] = array('name1'=>'E de las: Compras Importacin Afectadas Alicuota General','name2'=>'31','name3'=>'','name4'=>'32','name5'=>'','name6'=>'','name7'=>'');
	    $la_data[3] = array('name1'=>'E de las: Compras Importacin Afectadas en Alicuota General + Adicional','name2'=>'312','name3'=>'','name4'=>'322','name5'=>'','name6'=>'','name7'=>'');
	    $la_data[4] = array('name1'=>'E de las: Compras Importacin Afectadas en Alicuota Reducida','name2'=>'313','name3'=>'','name4'=>'323','name5'=>'','name6'=>'','name7'=>'');
	...
	function uf_print_table_totales($ldec_totcomsiniva,$ldec_totimp8,$ldec_totimp9,$ldec_totimp25,&$io_pdf)
	{
		...
		$la_data[0] = array('name1'=>'Compras Excentas:','name2'=>$ldec_totcomsiniva);
	...
	if($lb_valido)
	{
		...
		while($row=$io_report->io_sql->fetch_row($rs_resultado))	
		{
			...
			$la_cmpret=$io_report->uf_select_rowdata($io_sql,"SELECT DISTINCT max(a.numrecdoc) as numrecdoc,max(a.monobjret) as monobjret,SUM(a.monret) as monret,max(a.porded) as porded,max(b.codret) as codret,max(b.numcom) as numcom,max(b.iva_ret) as iva_ret,tiptrans".
			...
			//$ldec_sinderiva    = $ldec_montodoc-$ldec_monret;// Total de la Compra sin Derecho a Crédito Iva.				 
			...
			$la_data[$li]=array('name20'=>$ls_cheque,'name1'=>$li,
							 ...
							 'name12'=>$ls_tiptrans,'name13'=>$row["numrecdoc"], 'name14'=>number_format($ldec_montodoc,2,",","."),
