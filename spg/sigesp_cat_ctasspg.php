...
if($ls_operacion=="BUSCAR")
{
	...
	{	
		...
		else
	    {
		 	...
		 	if ($li_numrows>0)
		 	{
		      $ld_fecini=substr($arr["periodo"],0,4)."-01-01";
			  $ld_fecha = date("d/m/Y");
		      $ld_fecfin=$fun->uf_convertirdatetobd($ld_fecha);
			  while(!$rs_data->EOF)
			  { 
					 ...
					 $ls_aumento=0;
					 $ls_opera="aumento";
					 $ls_aumento=$io_buscar->uf_buscar_monto($ls_opera,$ld_fecini,$ld_fecfin,$rs_data->fields["codestpro1"],$rs_data->fields["codestpro2"],
														$rs_data->fields["codestpro3"],$rs_data->fields["codestpro4"],$rs_data->fields["codestpro5"],$ls_estcla,$ls_spgcta);
					 $ls_disminucion=0;
					 $ls_opera="disminucion";
					 $ls_disminucion=$io_buscar->uf_buscar_monto($ls_opera,$ld_fecini,$ld_fecfin, $rs_data->fields["codestpro1"],$rs_data->fields["codestpro2"],
														$rs_data->fields["codestpro3"],$rs_data->fields["codestpro4"],$rs_data->fields["codestpro5"],$ls_estcla,$ls_spgcta);
					 $ls_comprometer=0;
					 $ls_opera="comprometer";
					 $ls_comprometer=$io_buscar->uf_buscar_monto($ls_opera,$ld_fecini,$ld_fecfin, $rs_data->fields["codestpro1"],$rs_data->fields["codestpro2"],
														$rs_data->fields["codestpro3"],$rs_data->fields["codestpro4"],$rs_data->fields["codestpro5"],$ls_estcla,$ls_spgcta);
					 $ls_precomprometer=0;
					 $ls_opera="precomprometer";
					 $ls_precomprometer=$io_buscar->uf_buscar_monto($ls_opera,$ld_fecini,$ld_fecfin, $rs_data->fields["codestpro1"],$rs_data->fields["codestpro2"],
														$rs_data->fields["codestpro3"],$rs_data->fields["codestpro4"],$rs_data->fields["codestpro5"],$ls_estcla,$ls_spgcta);
															
					 $ld_mondis =($rs_data->fields["asignado"]+$ls_aumento)-($ls_disminucion+$ls_comprometer+$ls_precomprometer);	
				     if ($li_estmodest==2)
				        {
					      if ($ls_status=="S")
					         {
