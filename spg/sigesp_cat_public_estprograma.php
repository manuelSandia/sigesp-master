...
<form name="form1" method="post" action="">
  <p align="center">
	...
	if($ls_operacion=="BUSCAR")
	{
		...
		{
			$ls_sql_seguridad = " AND CONCAT('".$ls_codemp."','SPG','".$ls_logusr."',e.codestpro1,e.codestpro2,e.codestpro3,e.codestpro4,e.codestpro5,e.estcla) IN (SELECT distinct CONCAT(codemp,codsis,codusu,codintper) 
		                       FROM sss_permisos_internos WHERE codusu = '".$ls_logusr."' AND codsis = 'SPG' AND enabled=1)";
		}
		else
		{
			$ls_sql_seguridad = " AND '".$ls_codemp."'||'SPG'||'".$ls_logusr."'||e.codestpro1||e.codestpro2||e.codestpro3||e.codestpro4||e.codestpro5||e.estcla IN (SELECT distinct codemp||codsis||codusu||codintper
		                       FROM sss_permisos_internos WHERE codusu = '".$ls_logusr."' AND codsis = 'SPG' AND enabled=1)";
		}
		...
		$ls_sql=" SELECT a.codestpro1 as codestpro1, a.denestpro1 as denestpro1, b.codestpro2 as codestpro2, ".
				...
				"        a.estcla=b.estcla AND  b.estcla=c.estcla AND  c.estcla=d.estcla   ".$ls_sql_seguridad.
				" ORDER BY 1,3,5,7,9 ";
