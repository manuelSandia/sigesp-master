	...
	$lb_valido=$io_report->uf_spg_reporte_ejecucion_financiera_mensual($ls_codestpro1,$ls_codestpro2,$ls_codestpro3,$ls_codestpro4,
	 ...
	 else // Imprimimos el reporte
	 {
		...
		$io_tcpdf->SetFont("helvetica","B",8);
	 	$io_tcpdf->Cell(0,3,$ls_titulo,0,0,'C');
		$io_tcpdf->Ln(3);
		$io_tcpdf->Cell(0,3,$ls_titulo1,0,0,'C');
		$io_tcpdf->Ln();
		...
		while(!$rs_data->EOF)
		{
			  ...
			  $lb_valido3=$io_report->uf_spg_reporte_detalle_ejecucion_financiera_mensual_acumulado($ls_codestpro1,$ls_codestpro2,$ls_codestpro3,
			                                                                   $ls_codestpro4,$ls_codestpro5,
																			   $ls_codestpro1h,$ls_codestpro2h,$ls_codestpro3h,
	                                                                           $ls_codestpro4h,$ls_codestpro5h,
																			   $ls_estclades,$ls_estclahas,$ls_spg_cuenta,$ld_fecinirep,
			  ...
			  $la_data[$z]=array($ls_spg_cuenta,utf8_encode($ls_denominacion),number_format($ld_precompromiso,2,",","."),
			...
			if($z==($li_tot-1))
			{
				  ...
				  $la_data_tot[$z]=array('TOTAL Bs.',$ld_total_precompromiso,$ld_total_compromiso,$ld_total_causado,$ld_total_pagado);
			}//if
		...
		if($lb_valido2)
		{
			$io_tcpdf->uf_print_cabecera_acumulado_fonpyme();
			$io_tcpdf->uf_print_detalle_acumulado_fonpyme($la_data); // Imprimimos el detalle 
			$io_tcpdf->uf_print_total_acumulado_fonpyme($la_data_tot);//Bs		
			unset($la_data);
			unset($la_data_tot);			
