<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_titulo1,&$io_pdf)
	{
		...
		$tm=505-($li_tm/2);
		$io_pdf->addText($tm,515,16,$as_titulo1); // Agregar el título
	...
	//----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
		$ls_titulo="<b>RESUMEN DEL PRESUPUESTO DE GASTOS A NIVEL DE </b>";  
		$ls_titulo1="<b>PROYECTOS Y ACCIONES CENTRALIZADAS (FORMA 0705)</b>";     
	//--------------------------------------------------------------------------------------------------------------------------------
    // Cargar el dts_cab con los datos de la cabecera del reporte( Selecciono todos comprobantes )	
    $lb_valido_proyectos=$io_report->uf_spg_reportes_comparados_forma0705_proyectos($ls_codestpro1,$ls_codestpro2,
	                                                                                $ls_codestpro3,$ls_codestpro4,
																				    $ls_codestpro5,$ls_codestpro1h,
																	  			    $ls_codestpro2h,$ls_codestpro3h,
																	                $ls_codestpro4h,$ls_codestpro5h,
																				    $ls_combo,$ls_cant_mes,"P",
																					$ls_codfuefindes,$ls_codfuefinhas);
     $lb_valido_acciones=$io_report->uf_spg_reportes_comparados_forma0705_acciones($ls_codestpro1,$ls_codestpro2,
	                                                                               $ls_codestpro3,$ls_codestpro4,
																				   $ls_codestpro5,$ls_codestpro1h,
																	  			   $ls_codestpro2h,$ls_codestpro3h,
																	               $ls_codestpro4h,$ls_codestpro5h,
																				   $ls_combo,$ls_cant_mes,"A",
																				   $ls_codfuefinhas,$ls_codfuefinhas);
	 ...
	 else // Imprimimos el reporte
	 {
        ...
        uf_print_titulo_proyecto("PROYECTO",$io_pdf);
		...
		if(!$lb_valido_acciones)
		{
			 ...
			 if($ls_tipoformato==1)
		 	{
				 ...
				 $la_data_totales[$z]=array('total'=>'<b>TOTAL </b>','pres_anual'=>$ld_total_c1,'prog_acum'=>$ld_total_c2,
	 ...
	 if($lb_valido_acciones)
	 {
		...
		for($z=1;$z<=$li_total;$z++)
		{
		    ...
		    $ls_codestpro2=substr($ls_programatica,25,50);
		    if($lb_valido)
		    {
			  $ls_denestpro2="";
		    ...
		    $ls_codestpro3=substr($ls_programatica,50,75);
		    if($lb_valido)
		    {
			  $ls_denestpro3="";
			...
			$la_data_a[$z]=array('codigo'=>$ls_programatica,'denominacion'=>$ls_denestpro,'pres_anual'=>$ld_pres_anual,
							   'prog_acum'=>$ld_programada_acumulado,'monto_eject'=>$ld_monto_ejecutado,
							   'acum_eject'=>$ld_ejecutado_acumulado,'varia_abs'=>$ld_variacion_absoluta,
							   'varia_porc'=>$ld_porcentaje_variacion,'varia_abs_acum'=>$ld_variacion_abs_acum,
							   'varia_porc_acum'=>$ld_porcentaje_variacion_acum,'reprog_prox_mes'=>$ld_reprog_prox_mes);
        ...
        uf_print_titulo_proyecto("ACCIONES CENTRALIZADAS",$io_pdf);
