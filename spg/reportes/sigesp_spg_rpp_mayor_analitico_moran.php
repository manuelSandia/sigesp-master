			...
			{  
				...
				// var_dump($ld_totspg_por_pagar);
			      ...
			      if(($ls_spg_cuenta!=$ls_spg_cuenta_next)&& ($li_tmp!=($li_totrow_det-1)))
			      {
					  ...
					  $la_data[$li_tmp]=array($ls_fecha,$ls_procede,$ls_comprobante,$ls_documento,
					 ...
					 $io_tcpdf->uf_print_titulos_procede ($ls_mostrar);
					 $io_tcpdf->uf_print_detalle_campo_procede($la_auxdata,$y,0,$ls_mostrar); // Imprimimos los saldos anteriores							 
					 $io_tcpdf->uf_print_detalle_campo_procede($la_data,$y,1,$ls_mostrar); // Imprimimos el detalle
				...
				if($li_tmp==($li_totrow_det-1))
				{
					...
					$la_data_subtot2[1]=array('SUB TOTAL CUENTA '.$ls_spg_cuenta,
					 ...
					 $io_tcpdf->uf_print_titulos_procede ($ls_mostrar);
					 $io_tcpdf->uf_print_detalle_campo_procede($la_auxdata,$y,0,$ls_mostrar); // Imprimimos los saldos anteriores	
					 $io_tcpdf->uf_print_detalle_campo_procede($la_data,$y,1,$ls_mostrar); // Impr,imimos el detalle 
					 $io_tcpdf->uf_print_total($la_data_subtot2,'0',$y,$ls_mostrar);
					 $la_data_subtot[1]=array("TOTAL CUENTA ".$ls_spg_cuenta,number_format($ld_totspg_asignado,2,",","."),
					  						  number_format($ld_totspg_aumento ,2,",","."),
											  number_format($ld_totspg_disminucion,2,",","."),
