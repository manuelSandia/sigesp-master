...
//------------------------------------------------------------------------------------------------------------------------------
	...
	$lb_valido=$io_report->uf_spg_reporte_acumulado_cuentas2($ls_codestpro1,$ls_codestpro2,$ls_codestpro3,$ls_codestpro4,
	 ...
	 else // Imprimimos el reporte
	 {
		...
		$ls_mensaje = str_repeat(' ',2).$ls_titulo;
		$ls_mensaje2 = str_repeat(' ',5).$ls_titulo1.str_repeat(' ',5).date("d/m/Y").'-'.$_SESSION["ls_database"];
		...
		while(!$rs_data->EOF)
		{
			  ...
			  $lb_valido2=$io_report->uf_spg_reporte_detalle_acumulado_cuentas($ls_codestpro1,$ls_codestpro2,$ls_codestpro3,$ls_codestpro4,$ls_codestpro5,$ls_codestpro1h,$ls_codestpro2h,$ls_codestpro3h,$ls_codestpro4h,$ls_codestpro5h,'','',$rs_data->fields["spg_cuenta"],$ldt_fecfin,$rs_data2);
		...
		if($lb_valido2)
		{
			...
			unset($la_data);
			unset($la_data_tot);
			$io_tcpdf->Output("sigesp_spg_rpp_acum_x_cuenta.pdf", "I");
			unset($io_tcpdf);
		}
		else
	    {
