			...
			{  
				...
				while(!$rs_data2->EOF)
				{
				  ...
				  if (($ls_spg_cuenta!=$ls_spg_cuenta_aux)||($li_tmp==0))
				  {
					 ...
					 $lo_hoja->write($li_row, 3, "Proveedor/Beneficiario",$lo_titulo);
						 ...
						 {  
							...
							while (!$rs_data3->EOF)
							{
							    ...
							    $ld_totspg_por_pagar         = $ld_totspg_por_pagar + $ld_por_paga2;
								 ...
								 $lo_hoja->write($li_row, 4, 'SALDOS ANTERIORES',$lo_dataleft);
								 $lo_hoja->write($li_row, 5, $ld_asignado2,$lo_dataright);
								 $lo_hoja->write($li_row, 6, $ld_aumento2,$lo_dataright);
								 $lo_hoja->write($li_row, 7, $ld_disminucion2,$lo_dataright);
								 $lo_hoja->write($li_row, 8, $ld_monto_actualizado2,$lo_dataright);
								 $lo_hoja->write($li_row, 9, $ld_precompromiso2,$lo_dataright);
								 $lo_hoja->write($li_row, 10, $ld_compromiso2,$lo_dataright);
								 $lo_hoja->write($li_row, 11, $ld_causado2,$lo_dataright);
								 $lo_hoja->write($li_row, 12, $ld_pagado2,$lo_dataright);
								 $lo_hoja->write($li_row, 13, $ld_por_paga2,$lo_dataright);
				  ...
				  $lo_hoja->write($li_row, 3, $ls_nomproben,$lo_dataleft);              //nombre proveedor-beneficiario
				...
				/*	 $ls_sub_asignado=abs($ld_totspg_asignado-$ls_antasig);
					 ...
					 $ls_sub_asignado=$ld_totspg_asignado-$ls_antasig;
					 $ls_subaumento=$ld_totspg_aumento-$ls_antaum; 
					 $ls_subdisminucion=$ld_totspg_disminucion-$ls_antdis;
					 $ls_submonto_actual=$ld_totspg_monto_actualizado-$ls_antmon_act;
					 $ls_subprecompromiso=$ld_totspg_precompromiso-$ls_antpre;
					 $ls_subcompromiso=$ld_totspg_compromiso-$ls_antcom;
					 $ls_subcausado=$ld_totspg_causado-$ls_antcau;
					 $ls_subpagar=$ld_totspg_por_pagar-$ls_antpagar;	
					 $ls_subpagado=$ld_totspg_pagado-$ls_antpagado;

					 
					
					 $li_row=$li_row+1;
					$lo_hoja->write($li_row, 4, "SUB TOTAL CUENTA ".$ls_spg_cuenta,$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'right','size'=>'10')));
					$lo_hoja->write($li_row, 5, $ls_sub_asignado,$lo_dataright);
					$lo_hoja->write($li_row, 6, $ls_subaumento,$lo_dataright);
					$lo_hoja->write($li_row, 7, $ls_subdisminucion,$lo_dataright);
					$lo_hoja->write($li_row, 8, $ls_submonto_actual,$lo_dataright);
					$lo_hoja->write($li_row, 9, $ls_subprecompromiso,$lo_dataright);
					$lo_hoja->write($li_row, 10, $ls_subcompromiso,$lo_dataright);
					$lo_hoja->write($li_row, 11, $ls_subcausado,$lo_dataright);
					$lo_hoja->write($li_row, 12, $ls_subpagado,$lo_dataright);
					$lo_hoja->write($li_row, 13, $ls_subpagar,$lo_dataright);
					...
					$li_row=$li_row+1;
					$lo_hoja->write($li_row, 4, "TOTAL CUENTA ".$ls_spg_cuenta,$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'right','size'=>'10')));
					$lo_hoja->write($li_row, 5, $ld_totspg_asignado,$lo_dataright);
					$lo_hoja->write($li_row, 6, $ld_totspg_aumento,$lo_dataright);
					$lo_hoja->write($li_row, 7, $ld_totspg_disminucion,$lo_dataright);
					$lo_hoja->write($li_row, 8, $ld_totspg_monto_actualizado,$lo_dataright);
					$lo_hoja->write($li_row, 9, $ld_totspg_precompromiso,$lo_dataright);
					$lo_hoja->write($li_row, 10, $ld_totspg_compromiso,$lo_dataright);
					$lo_hoja->write($li_row, 11, $ld_totspg_causado,$lo_dataright);
					$lo_hoja->write($li_row, 12, $ld_totspg_pagado,$lo_dataright);
					$lo_hoja->write($li_row, 13, $ld_totspg_por_pagar,$lo_dataright);	
				...
				if($li_tmp==($li_totrow_det-1))
				{
					...
					$lo_hoja->write($li_row, 13, $ld_totspg_por_pagar,$lo_dataright);	
					 ...
					 $li_row=$li_row+1;
				 	$lo_hoja->write($li_row, 3, "Saldo Por Comprometer",$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'right','size'=>'10')));
