<?php
	...
	function uf_print_titulo_reporte($io_encabezado,$as_programatica,$ai_ano,$as_mes,$as_denestpro,$as_tirmestres,
	                                 $as_nombre,$ab_desde,$ab_hasta,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_encabezadopagina
		//		    Acess: private 
		...
		$ls_trimestre = "";
		switch($as_tirmestres)
		{
		 case 'I'   : $ls_trimestre = 'ENERO - MARZO';
		 break;
		 
		 case 'II'  : $ls_trimestre = 'ABRIL - JUNIO';
		 break;
		 
		 case 'III' : $ls_trimestre = 'JULIO - SEPTIEMBRE';
		 break;
		 
		 case 'IV'  : $ls_trimestre = 'OCTUBRE - DICIEMBRE';
		 break;
		
		}
		
		if($ab_desde && $ab_hasta){
			$la_data[]=array('desde'=>'<b>CODIGO PRESUPUESTARIO DEL ENTE:     </b>'.$ls_codasiona,'hasta'=>'');
			$la_data[]=array('desde'=>'<b>DENOMINACION DEL ENTE:    </b>'.$ls_nombre,'hasta'=>'');
			$la_data[]=array('desde'=>'<b>ORGANO DE ADSCRIPCION:    </b>'.$ls_nomorgads,'hasta'=>'');
			$la_data[]=array('desde'=>'<b>PERIODO PRESUPUESTARIO:   </b>'.$ls_trimestre." ".$ai_ano,'hasta'=>'');
			$la_data[]=array('desde'=>'<b>PROYECTO Y/O ACCION CENTRALIZADA:    </b>'.'CONSOLIDADO GENERAL','hasta'=>'');	
		}
		else {
			$la_data[]=array('desde'=>'<b>CODIGO PRESUPUESTARIO DEL ENTE:     </b>'.$ls_codasiona,'hasta'=>'');
			$la_data[]=array('desde'=>'<b>DENOMINACION DEL ENTE:    </b>'.$ls_nombre,'hasta'=>'');
			$la_data[]=array('desde'=>'<b>ORGANO DE ADSCRIPCION:    </b>'.$ls_nomorgads,'hasta'=>'');
			$la_data[]=array('desde'=>'<b>PERIODO PRESUPUESTARIO:   </b>'.$ls_trimestre." ".$ai_ano,'hasta'=>'');
			$la_data[]=array('desde'=>'<b>'.strtoupper($_SESSION["la_empresa"]["nomestpro1"]).' Desde:    </b>'.$as_nombre["D"]["1"],'hasta'=>'<b> - Hasta:</b> '.$as_nombre["H"]["1"]);
			$la_data[]=array('desde'=>'<b>'.strtoupper($_SESSION["la_empresa"]["nomestpro2"]).' Desde:    </b>'.$as_nombre["D"]["2"],'hasta'=>'<b> - Hasta:</b> '.$as_nombre["H"]["2"]);
			$la_data[]=array('desde'=>'<b>'.strtoupper($_SESSION["la_empresa"]["nomestpro3"]).' Desde:    </b>'.$as_nombre["D"]["3"],'hasta'=>'<b> - Hasta:</b> '.$as_nombre["H"]["3"]);
			if ($_SESSION["la_empresa"]["estmodest"]==2) {
				$la_data[]=array('desde'=>'<b>'.strtoupper($_SESSION["la_empresa"]["nomestpro4"]).' Desde:    </b>'.$as_nombre["D"]["4"],'hasta'=>'<b> - Hasta:</b> '.$as_nombre["H"]["4"]);
				$la_data[]=array('desde'=>'<b>'.strtoupper($_SESSION["la_empresa"]["nomestpro5"]).' Desde:    </b>'.$as_nombre["D"]["5"],'hasta'=>'<b> - Hasta:</b> '.$as_nombre["H"]["5"]);
			}
		}
		
		$la_columna=array('desde'=>'','hasta'=>'');
...
//-----------------------------------------------------------------------------------------------------------------------------
	...
	//--------------------------------------------------  Parámetros para Filtar el Reporte  -----------------------------------------
		...
		$ls_tipoformato=1;
		$lb_est_desde_vacia = false;
		$lb_est_hasta_vacia = false;
		if($li_estmodest==1)
		{
		...
		switch($ls_cmbmes)
		{
		 case '0103': $ls_trimestre = "I";
		 break;
		 
		 case '0406': $ls_trimestre = "II";
		 break;
		 
		 case '0709': $ls_trimestre = "III";
		 break;
		 
		 case '1012': $ls_trimestre = "IV";
		 break;
...
//--------------------------------------------------------------------------------------------------------------------------------
	 ...
	 else // Imprimimos el reporte
	 {
		...
		uf_print_titulo_reporte($io_encabezado,"",$li_ano,$ls_mesdes,"",$ls_trimestre,$as_nombre,$lb_est_desde_vacia,$lb_est_hasta_vacia,$io_pdf);
		...
		{		
			...
			$ls_status="";
				  ...
				  else
				  {
					...
					/*if(floatval($ld_asignado_partida) == floatval($ld_modificado_partida))
					{
				  ...
				  $ld_disp_fecha             = number_format($ld_disp_fecha,2,",",".");
				   ...
				   /*if(floatval($ld_modificado) == floatval($ld_asignado))
					{
					  $ld_modificado = '';
					} */
		...
		/*if(floatval($ld_total_asignado) == floatval($ld_total_modificado))
		{
		  $ld_total_modificado = '';
		}*/
