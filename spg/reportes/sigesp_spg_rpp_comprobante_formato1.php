<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_periodo_comp,$as_fecha_comp,&$io_pdf)
	{
		...
		$io_pdf->addText(900,730,10,$_SESSION["ls_database"]);// Agrerar el nombre de la base de datos actual
	 ...
	 else // Imprimimos el reporte
	 {
		...
		for($li_i=1;$li_i<=$li_tot;$li_i++)
		{
            ...
            if($lb_valido)
			{
				...
				for($li_s=1;$li_s<=$li_totrow_det;$li_s++)
				{
                    ...
                    if ($ls_procede=='SPGTRA')
                    {
						...
						{
							$ld_monto_disminucion = $ld_monto_disminucion+$ld_monto;
						}
						if (trim($ls_operacion)=='AU')
						{
							$ld_monto_aumento = $ld_monto_aumento+$ld_monto;
						}
						
                    }
                    //print "aum: $ld_monto_aumento    dis: $ld_monto_disminucion  $ld_monto <br>";
				...
				if ($ls_procede=='SPGTRA')
				{
					if ($ld_monto_aumento==$ld_monto_disminucion)
					{
						$ld_monto_aumento = str_replace('.','',$ld_monto_aumento);
						$ld_monto_aumento = str_replace(',','.',$ld_monto_aumento);
						 
						$ld_monto_aumento = number_format($ld_monto_aumento,2,",",".");
						$ld_totalcomprobante = $ld_monto_aumento;
						//$ld_totalcomprob=$ld_totalcomprobante;
					}
				}
			    $ld_totalcomprob=$ld_totalcomprobante;
			    $ld_total_comp = $ld_total_comp + $ld_totalcomprobante;
			...
			if($li_i==$li_tot)
			{
				  if($ls_tipoformato==1)
				  {
				  	  	$ld_total_comp = $ld_total_comp + $ld_total;
				  ...
				  {     
			  			...
			  			//uf_print_pie_cabecera($ld_total_comp,'Bs.',$io_pdf); // Imprimimos pie de la cabecera
