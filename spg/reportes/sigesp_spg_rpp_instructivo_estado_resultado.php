<?php
	...
	function uf_print_cabecera(&$io_pdf)
	{
		...
		$io_encabezado=$io_pdf->openObject();
		$io_pdf->saveState();
		$la_data=array(array('cuenta'=>'<b>Código</b>','denominacion'=>'<b>Denominación</b>','presupuesto'=>'<b>Presupuesto Aprobado  </b>',
	...
	//----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
		...
		$ls_bs="<b>(En Bolivares Fuertes)</b>"  ; 
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	else // Imprimimos el reporte
	{
		...
		for($z=1;$z<=$li_total;$z++)
		{
			...
			//var_dump($io_report->dts_reporte->data);
			...
			//print "cuenta: $ls_spg_cuenta  denominacion: $ls_denominacion     asignado: $ld_asignado  <br>";
			if($ld_asignado == $ld_asignado_modificado)
			{
				$ld_asignado_modificado = '';
			}
			
			$la_data[$z]=array('cuenta'=>$io_function_report-> uf_formato_cuenta_instructivo(trim($ls_spg_cuenta)),'denominacion'=>$ls_denominacion,
