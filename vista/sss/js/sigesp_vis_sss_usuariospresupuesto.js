...
* al proceso de asignar usuarios a una estructura presupuestaria. 
...
var cambiar = false;
var panel      = '';
var pantalla   = 'usuariospresupuesto';
...
(
	function()
	{
		...
		var agregar = new Ext.Action(
		{
        	...
        	tooltip: 'Agregar usuario a un presupuesto'
		});
		...
		var quitar = new Ext.Action(
		{
			...
			handler: irQuitar,
			iconCls: 'bmenuquitar',
        	tooltip: 'Quitar usuario de presupuesto'
		});
        ...
        labelWidth: 75,
     	title: 'Proceso de Asignar Usuarios a Presupuesto',
		...
		items:[{
			    xtype:'fieldset',
				title:'Datos de la Estructura Presupuestaria',
				id:'fsformpresupuesto',
    			autoHeight:true,
				autoWidth:true,
				cls :'fondo',	
			    items:[{			   
						xtype:'textfield',
						fieldLabel:'Código',
						name:'Código de la Estructura Presupuestaria',
						id:'txtcodest',
						disabled: true,
						width:200
					},{
						...
						name:'Denominación de la Estructura Presupuestaria',
						id:'txtdenestpro5',
						disabled: true,
						width:400
					}]
					...
					xtype:'panel',
					width:500,
					title:'Usuarios para el Presupuesto',
					tbar: [agregar,quitar],
					contentEl:'grid-usuariospresupuesto'
			}]
		});
...
***********************************************************************************/
	...
	{	
		...
		gridUsu.render('grid-usuariospresupuesto');
	}
...
***********************************************************************************/		
	...
	{		 
		Ext.getCmp('txtcodest').setValue('');
		Ext.getCmp('txtdenestpro5').setValue('');		
	}
...
***********************************************************************************/
	function irGuardar()
	{
		...
		if (Ext.getCmp('txtcodest').getValue()=='')
		{
			Ext.Msg.alert('Mensaje','Debe seleccionar una Estructura');
		}
		else
		{
			obtenerMensaje('procesar','','Guardando Datos');
			
			var cadenaJson = "{'oper': 'actualizar','codsis':'SPG','seleccionado':'presupuesto','sistema': sistema,'vista': vista,'codtippersss': '"+Ext.getCmp('txtcodcompleto').getValue()+"','dentippersss': '"+Ext.getCmp('txtdenestpro5').getValue()+"'";				
...
/***********************************************************************************
*  @Función que llama al catalogo para mostrar los datos de la estructura presupuestaria.
* @parametros: 
* @retorno: 
* @fecha de creación: 29/10/2008. 
* @autor: Ing. Gusmary Balza. 
...
***********************************************************************************/
	function irBuscar()
	{
		var arreglotxt     = new Array('txtcodest','txtcodcompleto','txtdenestpro5');		
		var arreglovalores = new Array('codest','codcompleto','nombre');			
		objCatEstPre = new catalogoEstPre();
		objCatEstPre.mostrarCatalogoEstPre(arreglotxt, arreglovalores);
	}
	
	
/***********************************************************************************
* @Función que elimina un usuario de una estructura seleccionada.
* @parametros: 
* @retorno: 
* @fecha de creación: 29/10/2008. 
* @autor: Ing. Gusmary Balza. 
...
***********************************************************************************/
	function irEliminar()
	{
		...
		function Result(btn)
		{
			if(btn=='yes')
			{ 
				if (validarObjetos('txtcodest','60','novacio')=='0')
				{					
