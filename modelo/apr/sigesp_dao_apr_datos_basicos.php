	...
	function  verificarExistenciaTabla($conexionorigen)
	{
		...
		if (!is_numeric($clave))
		{
			...
			$validos = array(0 => 'sigesp_proc_cons', 1 => 'sigesp_dt_proc_cons', 2 => 'rpc_niveles', 
							 ...
							 62=>'saf_entrega', 63=>'saf_catsudeban',64=>'srh_gerencia');
...
***********************************************************************************/
	function  cargarCampos($result,$conexionbd)
	{
		...
		$tablasspg = array(0 => 'spg_ep1', 1 => 'spg_ep2', 2 => 'spg_ep3', 
						   ...
						   29 => 'sep_dt_articulos',30=>'sep_cuentagasto',31=>'cxp_rd_cargos',32 => 'cxp_dc_spg',33 => 'spg_plantillareporte');
			...
			switch($this->_table)
			{
				...
				case 'sigesp_empresa':
					$validos = array(0 => 'saliniproaux', 1 => 'saliniejeaux', 2 => 'candeccon', 
									 3 => 'tipconmon', 4 => 'redconmon', 5 => 'sueintivss', 6 => 'suenetivss', 7 => 'estintban',
									 8 => 'ctaresact', 9 => 'ctaresant', 10 => 'estvaldisfin', 11 => 'dedconproben', 12 => 'estaprsep',
									 13 => 'sujpasesp', 14 => 'bloanu', 15 => 'estretmil', 16 => 'concommil', 17 => 'contintmovban',
									 18 => 'valinimovban', 19 => 'cueproacu', 20 => 'cuedepamo', 21 => 'valclacon', 22 => 'valcomrd',
									 23 => 'ctaejeprecie', 24 => 'estaprsoc', 25 => 'estaprcxp', 26 >= 'scforden_h');
					$clave = array_search($campo, $validos);
					if (!is_numeric($clave))
					{	
						$this->valido=false;
				...
				case 'sno_tipopersonal':
					$validos = array(0 => 'tipperobr');
					$clave = array_search($campo, $validos);
					if (!is_numeric($clave))
					{	
						$this->valido=false;
				...
				case 'sno_ubicacionfisica':
					$validos = array(0 => 'tietras');
					$clave = array_search($campo, $validos);
					if (!is_numeric($clave))
					{	
						$this->valido=false;
				...
				case 'sno_unidadadmin':
					$validos = array(0 => 'codestpro1', 1 => 'codestpro2', 2 => 'codestpro3',3 => 'codestpro4', 4 => 'codestpro5', 5=>'codprouniadm', 6=>'codproviauniadm');
					$clave = array_search($campo, $validos);
					if (!is_numeric($clave))
					{	
						$this->valido=false;
				...
				case 'sno_proyecto':
					$validos = array(0 => 'codestpro1', 1 => 'codestpro2', 2 => 'codestpro3',3 => 'codestpro4', 4 => 'codestpro5');
					$clave = array_search($campo, $validos);
					if (!is_numeric($clave))
					{	
						$this->valido=false;
				...
				case 'sno_unidadadmin':
					$validos = array(0 => 'loncodestpro1', 1=>'codprouniadm', 2=>'codproviauniadm');
					$clave = array_search($campo, $validos);
					if (!is_numeric($clave))
					{	
						$this->valido=false;
...
***********************************************************************************/
	function  criterioConversion($columna,&$valor)
	{
		...
		if (($this->_table == 'sigesp_empresa') && ($this->campos[$columna] == 'ctaresact'))
		{
		...
		if ($this->campos[$columna] == 'cod_pro')
		{
			if (strlen(trim($valor)) < 12 )
			{
				...
				{
					$valorant=substr(trim($valorant),1,8);
					$valor=str_pad($valorant,8,'0',0).'99';
		...
		if (($this->campos[$columna] == 'codart')&& ($this->sistema == 'SIV'))
		{
			$valor = trim($valor);
		}
...
***********************************************************************************/		
	/*function actualizarNomina()
	{
		...
		$aniomen=substr($this->fecinimen,0,4);
		$mesmen=substr($this->fecinimen,8,2);
		$diamen=substr($this->fecinimen,5,2);
		$nommensual=$aniomen."-".$mesmen."-".$diamen;
		
		$aniosem=substr($this->fecinisem,0,4);
		$messem=substr($this->fecinisem,8,2);
		$diasem=substr($this->fecinisem,5,2);
		$nomsemana=$aniosem."-".$messem."-".$diasem;
		...
		$objNomina->fecininom = $nommensual;
		if ($this->tippernom == 0)
		{
			$objNomina->fecininom = $nomsemana;
		}
	...
	function actualizarNomina() 
	{
		...
		$consulta="UPDATE sno_nomina ".
				  ...
				  "  AND codnom = '".$this->valornuevo."'";  
		$result = $conexionbd->Execute($consulta);
		escribirArchivo($this->script,$consulta.";");
		$consulta="UPDATE sno_nomina ".
				  "  SET anocurnom = '".substr($this->fecinimen,0,4)."', ".
				  "      fecininom = '".$nommensual."',".
				  "      peractnom = '001'  ".
...
***********************************************************************************/		
	function generarHistoricosAdicionales($conexionbdorigen)
	{
		...
		while (!$result->EOF)
		{
			...
			/*$nombrearchivo='../../vista/apr/resultados/SCRIPT_NOMINAS_sno_periodo.sql';;
			...
			//escribirArchivo($script,$consulta.";");
