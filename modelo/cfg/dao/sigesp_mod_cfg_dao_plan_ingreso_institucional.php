...
function uf_valida_cuenta($as_cuenta ,$as_cuenta_scg)
{
	...
	if($li_len_cta!=$li_len_formato)
	{
		...
		echo '<br>La cuenta '.$ls_spi_cuenta.' no posee el formato del plan unico|';
		return false;
	}
	...
	if(substr($ls_spi_cuenta,0,1)!=trim($this->dat["ingreso_p"]))
	{
		...
		echo '<br>Las cuenta '.$ls_spi_cuenta.' debe comenzar con '.$this->dat["ingreso_p"].'|';
		return false;
	}
	...
	if($li_nivel<= 2)
	{
		...
		echo '<br>La cuenta '.$ls_spi_cuenta.' de Nivel Generica no es valida|';
		return false;
	}	
		...
		do 
		{
			if($this->int_spi->uf_spi_select_cuenta($this->dat["codemp"], $ls_NextCuenta,&$as_status,&$as_denominacion, $as_cuenta_scg))
			{
				if($as_status=="C")
				{
					...
					echo '<br>Existen cuentas de nivel superior con movimiento para la cuenta '.$ls_spi_cuenta.'|';
					return false;
				}
			}
							...
							if($this->int_spi->uf_spi_select_cuenta($ls_codemp,$ls_NextCuenta, &$ls_status, &$ls_denominacion, $as_cuenta_scg ))
							{
								 ...
								 $this->int_scg->uf_select_plan_unico_cuenta($ls_PadNextCuenta,&$as_denominacion_plan);
									...
									if($li_total_rows>1)
									{
										...
										/*echo '<br>Existen cuentas de nivel inferior para la cuenta '.$as_cuenta_spi.' no se puede eliminar|';
										return false;*/				
									}
									else
									{		  
