...
class planCuentaDaoSpi
{
	...
	function planCuentaDaoSpi()
	{
		require_once("../../shared/class_folder/sigesp_c_check_relaciones.php");
		require_once("../../shared/class_folder/class_mensajes.php");
		require_once("../../shared/class_folder/sigesp_c_seguridad.php");
		require_once("../../shared/class_folder/class_funciones.php");
		require_once("../../base/librerias/php/general/class_sigesp_int_scg.php");
		require_once("../../base/librerias/php/general/class_sigesp_int_spi.php");
...
function uf_valida_cuenta($as_cuenta ,$as_cuenta_scg)
{
	...
	if($li_len_cta!=$li_len_formato)
	{
		...
		echo '<br>La cuentas '.$ls_spi_cuenta.' no posee el formato del plan unico|';
		return false;
	}
	...
	if($li_len_ctaspi!=$li_len_formato_spi)
	{
		...
		echo "<br>Formato de presupuesto ".$ls_formato_spi." no corresponde al de la cuenta introducida ".$ls_spi_cuenta."|";
		return false;
	}
	...
	if(substr($ls_spi_cuenta,0,1)!=trim($this->dat["ingreso_p"]))
	{
		...
		echo "<br>Las Cuentas de Ingreso deben comenzar con ".$this->dat["ingreso_p"]." |";
		return false;
	}
	...
	if($li_nivel <= 1)
	{
		...
		echo '<br>La Cuenta '.$ls_spi_cuenta.' de Nivel Partida no es valida |';
		return false;
	}
	...
	if($li_nivel<= 2)
	{
		...
		echo '<br>La Cuenta '.$ls_spi_cuenta.' de Nivel Genericas no es valida |';
		return false;
	}	
		...
		do 
		{
			if($this->int_spi->uf_spi_select_cuenta($this->dat["codemp"], $ls_NextCuenta,&$as_status,&$as_denominacion, $as_cuenta_scg))
			{
				if($as_status=="C")
				{
					...
					echo '<br>Para la cuenta '.$ls_spi_cuenta.', existen cuentas de nivel superior con movimiento |';
					return false;
				}
			}
							 ...
							 $ls_codestpro5,$ls_estcla,$aa_security)
{
		...
		{
				$lb_valido=$this->uf_spi_update_cuenta($ls_cuentaspi,$ls_codestpro1,$ls_codestpro2,
								  ...
								  $as_codestpro5,$as_estcla,$aa_security)
{
	...
	if($lb_existe)
	{
		...
		{	
			...
			echo '<br>"Existen movimientos asociados a la cuenta ".$as_cuenta_spi." cuya cuenta Contable es ".$as_cuenta_scg |';
			return false;
		}
		else
		{			
				...
				{	
				   $lb_valido=false;
				   echo '<br>"La cuenta ".$as_cuenta_spi." no pudo ser eliminada |';
				}
				else 
				{ 
