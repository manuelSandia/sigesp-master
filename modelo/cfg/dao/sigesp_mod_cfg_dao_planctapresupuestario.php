...
class planCuentaDao
{
	...
	function planCuentaDao()
	{
		require_once("../../shared/class_folder/class_mensajes.php");
		require_once("../../base/librerias/php/general/class_sigesp_int_scg.php");
		require_once("../../base/librerias/php/general/class_sigesp_int_spg.php");
	...
	function uf_valida_cuenta($as_cuenta ,$aa_estpro,$as_cuenta_scg)
	{
		...
		if($li_len_cta!=$li_len_formato)
		{
			...
			echo '<br>La cuenta '.$ls_spg_cuenta.' no poseen el formato del plan unico|';
			return false;
		}
		...
		if($li_len_ctaspg!=$li_len_formato_spg)
		{
			...
			echo '<br>Formato de presupuesto '.$ls_formato_spg.' no corresponde al de la cuenta introducida '.$ls_spg_cuenta.'|';
			return false;
		}
		...
		if(substr($ls_spg_cuenta,0,1)!=trim($this->dat["gasto_p"]))
		{
			...
			echo '<br>Las cuenta '.$ls_spg_cuenta.' debe comenzar con '.$this->dat["gasto_p"].'|';
			return false;
		}
		...
		if($li_nivel <= 1)
		{
			...
			echo '<br>La cuenta '.$ls_spg_cuenta.' de Nivel Partida no es valida|';
			return false;
		}
		...
		if($li_nivel<= 2)
		{
			...
			echo '<br>La cuenta '.$ls_spg_cuenta.' de Nivel Generica no es valida|';
			return false;
		}	
			...
			do 
			{
				if($this->int_spg->uf_spg_select_cuenta($this->dat["codemp"], $aa_estpro, $ls_NextCuenta,&$as_status,&$as_denominacion, $as_cuenta_scg))
				{
					if($as_status=="C")
					{
						...
						echo '<br>Existen cuentas de nivel superior con movimiento para la cuenta '.$ls_spg_cuenta.'|';
						return false;
					}
				}
