...
<script language="JavaScript">
	...
	function  ue_validarcargo(li_row)
	{
		f=document.form1;
		totrow=f.totalfilas.value;
		tipoiva=eval("f.txttipoiva"+li_row+".value");
		if(eval("f.chkagregar"+li_row+".checked")==true)
		{
			for(i=1;i<=totrow;i++)
			{
				if((eval("f.chkagregar"+i+".checked")==true)&&(i!=li_row))
				{
					tipoivagrid=eval("f.txttipoiva"+i+".value");
					if(tipoiva==tipoivagrid)
					{
						alert("No se debe seleccionar mas de un cargo del mismo tipo");
						eval("f.chkagregar"+li_row+".checked=false");
					}
				}
			}
		}
