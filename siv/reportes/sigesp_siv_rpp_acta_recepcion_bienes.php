<?php
	...
	function uf_print_encabezado_pagina($as_titulo,&$io_pdf)
	{
		...
		$io_pdf->saveState();
		//$io_pdf->line(10,40,578,40); // linea horizontal al final del documento
		...
		$io_pdf->addText(500,740,12,"Pág."); // Agregar texto -- Enumeracion del reporte
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_cabezera($as_nomresalm,$as_nompro,$as_nomreppro,$as_nombre,$as_nomrep,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_encabezado=$io_pdf->openObject();
		$io_pdf->saveState();
	    $la_data=array(array('name'=>'Entre el <b>'.trim($as_nombre).'</b> representado en este acto por el ciudadano <b>'.trim($as_nomrep).'</b> por una parte, y por la otra <b>"LA EMPRESA" '.$as_nompro.'</b> representada en este acto por el ciudadano  <b>'.$as_nomreppro.'</b>, se ha procedido a efectuar la recepcion de los siguientes bienes:'));
		
		//$la_data=array(array('name'=>'Entre el <b>'.trim($as_nombre).'</b> representado en este acto por el ciudadano '.trim($as_nomrep)),
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		                 ...
		                 'cols'=>array('name'=>array('justification'=>'full'))); // Justificacion del texto 
	...
	function uf_print_pie_detalle($as_estpro,$as_numordcom,$as_nombre,$as_montotart,$as_ciuemp,$adt_fecrec,&$io_pdf)
	{
		...
		$la_data=array(array('name'=>'los cuales fueron recibidos el '.$adt_fecrec.' segun <b>'.$ls_tipo_orden.' No. '.$as_numordcom.'</b> acta levantada al efecto, en la forma siguiente:  El represenante del  <b>"'.trim($as_nombre).'"</b> luego de haber constatado que <b>"LA EMPRESA"</b> ejecuto el suministro señalado en la referida '.$ls_tipo_orden.', declarada <b>ACEPTADA</b> la compra. El monto correspondiente a este suministro es de ('.$ls_titulo.'  '.number_format($as_montotart,2,",",".").')<b> '.$ls_monto_letras.'</b>.'),
					   array('name'=> 'En prueba de conformidad se firma, en la ciudad de '.trim($as_ciuemp).', a los '.$li_dia.' dias del mes de '.$ls_mes.' de '.$li_agno.'.'));
    ...
    // Cargar el dts_cab con los datos de la cabecera del reporte( Selecciono todos comprobantes )	
	 ...
	 else // Imprimimos el reporte
	 {
		...
		uf_cabezera($ls_nomresalm,$ls_nompro,$ls_nomreppro,$ls_nombre,$ls_nomrep,$io_pdf); // primer parrafo
		uf_cabezera_detalle($io_pdf); // cabezera de los renglones
		uf_print_pie_detalle($ls_estpro,$ls_numordcom,$ls_nombre,$ls_montotart,$ls_ciuemp,$ldt_fecrec,$io_pdf); // segundo parrafo
		uf_print_detalle($la_data,$io_pdf); // Imprimimos el detalle de los bienes
		uf_print_pie_cabecera($ls_nombre,$io_pdf);  // seccion de firmas
		unset($la_data);
