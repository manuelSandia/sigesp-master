<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$ad_fecha,&$io_pdf)
	{
		...
		$io_pdf->addText(425,715,10,"Fecha:"); // Agregar texto
		$io_pdf->addText(465,715,10,date("d/m/Y H:i")); // Agregar la Fecha
		// cuadro inferior
        $io_pdf->Rectangle(50,40,500,70);
		$io_pdf->line(50,56,550,56);//horizontal
		$io_pdf->line(50,48,550,48);//horizontal			
		$io_pdf->line(50,100,550,100);//horizontal		
		$io_pdf->line(210,40,210,110);//vertical		
		$io_pdf->line(430,40,430,110);//vertical		
		$io_pdf->addText(100,102,7,"Elaborado por:"); // Agregar el título
		$io_pdf->addText(100,50,7,"Victor Torres"); // Agregar el título
		$io_pdf->addText(70,41,7,"Coordinador de Servicios Generales"); // Agregar el título
		$io_pdf->addText(290,102,7,"Verificado por"); // Agregar el título
		$io_pdf->addText(290,50,7,"Maryoris Lurua"); // Agregar el título
		$io_pdf->addText(250,41,7,"Gerente de Administración y Finanzas"); // Agregar el título
		$io_pdf->addText(455,102,7,"Fecha de Validación"); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera($as_nomemp,$as_codalm,$as_nomfisalm,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'maxWidth'=>500, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('codigo'=>array('justification'=>'left','width'=>140), // Justificación y ancho de la columna
						 			   'articulo'=>array('justification'=>'left','width'=>180), // Justificación y ancho de la columna
						 			   'unidad'=>array('justification'=>'center','width'=>100), // Justificación y ancho de la columna
						 			   'cantidad'=>array('justification'=>'right','width'=>60))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
	...
	$li_ordenart=1;
	//--------------------------------------------------------------------------------------------------------------------------------
	$lb_valido=true;//$io_report->uf_prenomina_personal($ls_codperdes,$ls_codperhas,$ls_orden); // Cargar el DS con los datos de la cabecera del reporte
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			uf_print_cabecera($ls_nomemp,$ls_codalm,$ls_nomfisalm,$io_pdf); // Imprimimos la cabecera del registro
			$lb_valido=$io_report->uf_select_articuloxalmacen($ls_codemp,$ls_codalm,$ls_codart,'',$li_ordenalm,$li_ordenart); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
				...
				for($li_s=1;$li_s<=$li_totrow_det;$li_s++)
				{
					...
					$ls_unidad= "Mayor___ Detal___";
					$ls_cantidad="______";
					$la_data[$li_s]=array('codigo'=>$ls_codart,'articulo'=>$ls_denart,'unidad'=>$ls_unidad,'cantidad'=>$ls_cantidad);
				}
