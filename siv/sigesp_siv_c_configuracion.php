...
class sigesp_siv_c_configuracion
{
   ...
   //---------------------------------------------------------------------------------------------------------------------------
	function uf_siv_procesar_articulos_primarios($as_codemp,$as_value,$aa_seguridad)
	{
		...
		$lb_valido=true;
		$as_valueaux=$as_value;
		$lb_existe=$this->uf_siv_load_articulos_primarios($as_codemp,$as_valueaux);
		if($lb_existe)
		{
   ...
   //---------------------------------------------------------------------------------------------------------------------------
	function uf_siv_insert_articulos_primarios($as_codemp,$as_value,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->configuracion MÉTODO->uf_siv_insert_articulos_primarios ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
   ...
   //---------------------------------------------------------------------------------------------------------------------------
	function uf_siv_update_articulos_primarios($as_codemp,$as_value,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->configuracion MÉTODO->uf_siv_update_articulos_primarios ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
   ...
   //---------------------------------------------------------------------------------------------------------------------------
	function uf_siv_procesar_centro_costos($as_codemp,$as_value,$aa_seguridad)
	{
		...
		$lb_valido=true;
		$as_valueaux=$as_value;
		$lb_existe=$this->uf_siv_load_centro_costos($as_codemp,$as_valueaux);
		if($lb_existe)
		{
   ...
   //---------------------------------------------------------------------------------------------------------------------------
	function uf_siv_load_centro_costos($as_codemp,&$as_value)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->configuracion MÉTODO->uf_siv_load_centro_costos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
   ...
   //---------------------------------------------------------------------------------------------------------------------------
	function uf_siv_insert_centro_costos($as_codemp,$as_value,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->configuracion MÉTODO->uf_siv_insert_centro_costos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="INSERT";
			$ls_descripcion ="Insertó el estatus de Centro de Costos a ".$as_value." para la empresa ".$as_codemp;
			$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
   ...
   //---------------------------------------------------------------------------------------------------------------------------
	function uf_siv_update_centro_costos($as_codemp,$as_value,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->configuracion MÉTODO->uf_siv_update_centro_costos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Actualizó el estatus de Centro de Costos a ".$as_value." para la empresa ".$as_codemp;
			$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
