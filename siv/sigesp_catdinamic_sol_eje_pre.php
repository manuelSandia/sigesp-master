...
<table width="489" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
  ...
  <!--  //Se comentó este codigo porque no deben seleccionarse los tipos de SEP para los despachos, ya que solamente deben aparecer aquellos que son sin afectación presupuestaria  
  <tr>
    <td height="22"><div align="right">Tipo de SEP </div></td>
    <td height="22"><select name="cmbtipsep" id="cmbtipsep">
      <option value="T" selected>--Todas--</option>
      <option value="C">Compromiso</option>
      <option value="P">Pre-Compromiso</option>
    </select>
    </td>
    <td height="22">&nbsp;</td>
    <td height="22">&nbsp;</td>
...
<?php
	...
	print "<td style=text-align:center width=125>N�mero de Solicitud</td>";
	...
	if($ls_operacion=="BUSCAR")
	{
		if (array_key_exists("txtdesde",$_POST))
		{
			...
			if (array_key_exists("txthasta",$_POST))
			{
				...
				/*  No se va a filtar por tipo de SEP, por eso se comentó el código
				 switch($ls_tipsep)
				{
					case"T":
						$ls_filtro="AND (sep_tiposolicitud.estope='O' OR sep_tiposolicitud.estope='S' OR sep_tiposolicitud.estope='R')";
					break;
					case"P":
						$ls_filtro="AND sep_tiposolicitud.estope='R'";
					break;
					case"C":
						$ls_filtro="AND sep_tiposolicitud.estope='O'";
					break;
					default:
						$ls_filtro="AND (sep_tiposolicitud.estope='O' OR sep_tiposolicitud.estope='S' OR sep_tiposolicitud.estope='R')";
					break;
				}*/
				//Solo deben aparecer aquellas SEP que son sin afectación presupuestaria para su despacho
				$ls_filtro="AND sep_tiposolicitud.estope='S' ";
				if($lb_fechavalida)
				{
					...
					$ls_sql=" SELECT sep_solicitud.numsol,sep_solicitud.coduniadm,sep_solicitud.fecregsol,sep_solicitud.estsol,
							 ...
							 WHERE sep_solicitud.codemp = '".$ls_codemp."'
							   ...
							   AND sep_solicitud.codtipsol=sep_tiposolicitud.codtipsol ".$ls_filtro."
			  ...
			  while ($row=$io_sql->fetch_row($rs_data))
			        {
					  ...
					  $li_estint    = $row["estint"];//Entero que Indica si la Estructura de Nivel 1 es Inter Compa�ia.
					  $ls_ctascgint = $row["ctascgint"];//Cuenta Contable Inter Compa�ias.
