...
class sigesp_siv_c_toma
{
	...
	function uf_siv_load_articuloalmacen($as_codemp,$as_codalm,&$ao_object,&$ai_totrows)
	{
		...
		$ls_sql = "SELECT siv_articuloalmacen.codart,siv_articulo.denart,".
				  ...
				  "   AND siv_articuloalmacen.codalm='".$as_codalm."' ORDER BY 1" ;
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tomainventario M�TODO->uf_siv_load_articuloalmacen ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][3]="<input name=txtdenunimed".$ai_totrows." type=text id=txtdenunimed".$ai_totrows." class=sin-borde size=20 maxlength=50 value='".$ls_denunimed."' readonly>";
				//$ao_object[$ai_totrows][4]="<div align='center'><select name=cmbunidad".$ai_totrows." style='width:110px '><option value=->-Seleccione uno-</option><option value=D>Detal</option><option value=M>Mayor</option></select></div>";
				$ao_object[$ai_totrows][4]="<div align='center'><select name=cmbunidad".$ai_totrows." style='width:110px '><option value=D>Detal</option></select></div>";
				$ao_object[$ai_totrows][5]="<input name=txtcanfis".$ai_totrows." type=text id=txtcanfis".$ai_totrows." class=sin-borde size=12 maxlength=12 onKeyPress=return(ue_formatonumero(this,'.',',',event)); value=0,00>";
				$ao_object[$ai_totrows][6]="<img src='../shared/imagebank/tools/espacio.gif' width=20 height=20>";
	...
	function uf_siv_load_toma($as_codemp,$as_numtom,&$ao_object,&$ai_totrows)
	{
		...
		//	   Creado Por: Ing. Luis Anibal Lang
		// Fecha Creaci�n: 31/03/2006 								Fecha �ltima Modificaci�n : 31/03/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tomainventario M�TODO->uf_siv_load_toma ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][2]="<input name=txtdenart".$ai_totrows." type=text id=txtdenart".$ai_totrows." class=sin-borde size=40 maxlength=50 value='".$ls_denart."' readonly>";
				$ao_object[$ai_totrows][3]="<input name=txtdenunimed".$ai_totrows." type=text id=txtdenunimed".$ai_totrows." class=sin-borde size=20 maxlength=50 value='".$ls_denunimed."' readonly>";
				$ao_object[$ai_totrows][4]="<div align='center'><select name=cmbunidad".$ai_totrows." style='width:110px '><option value=->-Seleccione uno-</option><option value=D ".$la_unidad[0].">Detal</option><option value=M ".$la_unidad[1].">Mayor</option></select></div>";
				$ao_object[$ai_totrows][5]="<input name=txtcanfis".$ai_totrows." type=text id=txtcanfis".$ai_totrows." class=sin-borde size=12 maxlength=12 value='".$li_canexifis."'onKeyPress=return(ue_formatonumero(this,'.',',',event)); value=0,00>";
				if($li_canexifisaux==$li_canexisis)
				{
					$ao_object[$ai_totrows][6]="<img src='../shared/imagebank/ok.png' width=10 height=10>";
				}
				else
				{
					$ao_object[$ai_totrows][6]="<img src='../shared/imagebank/failed.png' width=10 height=10>";
				}
	...
	function uf_siv_select_comparararticulos($as_codemp,$as_codalm,$as_codart,$ai_canexifis,$as_unidad,&$ai_canexisis,&$ab_ok,&$ai_unidad)
	{
		...
		//	   Creado Por: Ing. Luis Anibal Lang
		// Fecha Creaci�n: 24/03/2006 								Fecha �ltima Modificaci�n : 24/03/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tomainventario M�TODO->uf_siv_select_comparararticulos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_siv_insert_tomainventario($as_codemp,$as_codalm,&$as_numtom,$ad_fectom,$as_obstom,$as_codusu,$aa_seguridad)
	{
		...
		$ls_sql="INSERT INTO siv_toma (codemp, codalm, numtom, fectom, obstom, codusu)".
			...
			//	echo "<br>".$ls_sql."  ".$li_row;
		
		$li_row=$this->io_sql->execute($ls_sql);
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->tomainventario M�TODO->uf_siv_insert_tomainventario ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="INSERT";
			$ls_descripcion ="Insert� la toma ".$as_numtom."  de Almac�n ".$as_codalm." Asociado a la empresa ".$as_codemp;
											 ...
											 $ai_canexifisant,$as_unidad,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->tomainventario M�TODO->uf_siv_insert_dt_tomainventario ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_siv_load_ultimocosto($as_codemp,$as_codart,&$ai_preuniart)
	{
		...
		//				   siv_dt_recepcion
		//	   Creado Por: Ing. Luis Anibal Lang
		// Fecha Creaci�n: 27/03/2006 								Fecha �ltima Modificaci�n : 27/03/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tomainventario M�TODO->uf_siv_load_ultimocosto ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_siv_update_tomainventario($as_codemp,$as_codalm,&$as_numtom,$ad_fectom,$as_obstom,$as_codusu,$aa_seguridad) 
	{
		...
		//	   Creado Por: Ing. Luis Anibal Lang
		// Fecha Creaci�n: 01/04/2006 								Fecha �ltima Modificaci�n : 01/04/2006 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 	$lb_valido=true;
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->almacen M�TODO->uf_siv_update_tomainventario ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Actualiz� la Toma ".$as_numtom." del Almac�n ". $as_codalm ." Asociado a la Empresa ".$as_codemp;
											 ...
											 $ai_canexifisant,$as_unidad,$aa_seguridad) 
	{
		...
		if ($lb_valido)
		{
			...
			if($li_row===false)
			{
				$this->io_msg->message("CLASE->almacen M�TODO->uf_siv_update_dt_tomainventario ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
			}
			else
			{
...
//												$aa_seguridad["ventanas"],$ls_descripcion);
	...
	function uf_siv_select_dt_tomainventario($as_codemp,$as_codalm,$as_numtom,$as_codart,&$ai_canexifisant)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->toma M�TODO->uf_siv_select_dt_tomainventario ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_siv_update_estatustoma($as_codemp,$as_codalm,$as_numtom,$aa_seguridad) 
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->almacen M�TODO->uf_siv_update_estatustoma ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Ajust� el Almac�n ".$as_codalm." Asociado a la Empresa ".$as_codemp.
							 " por la toma de Inventario n�mero ".$as_numtom;
			$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
