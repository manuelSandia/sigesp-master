<?php
	...
	$ls_filtro=$io_fun_scf->uf_obtenervalor_get("filtro","");
	$ls_restriccion=$io_fun_scf->uf_obtenervalor_get("restriccion","");
	unset($io_fun_scf);
...
<script type="text/javascript"  language="JavaScript1.2" src="../shared/js/js_intra/datepickercontrol.js"></script>
</head>
<body>
...
<input name="restriccion" type="hidden" id="restriccion" value="<?php print $ls_restriccion; ?>">
<table width="600" border="0" align="center" cellpadding="1" cellspacing="1">
  ...
  <br>
    <table width="600" border="0" align="center" cellpadding="1" cellspacing="0" class="formato-blanco">
      <tr>
        ...
        <td width="194" rowspan="4"><table width="159" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
          ...
          <tr>
            ...
            <td width="99"><input name="txtfecdes" type="text" id="txtfecdes" size="15" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);"  value="<?php print $ld_fecdes;?>"  maxlength="10" datepicker="true"></td>
          </tr>
          <tr>
            <td height="22"><div align="right">Hasta</div></td>
            <td><input name="txtfechas" type="text" id="txtfechas" size="15" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);"  value="<?php print $ld_fechas;?>" maxlength="10" datepicker="true" ></td>
          </tr>
        </table></td>
      </tr>
      <tr>
      ...
      <tr>
        <td height="22"><div align="right">Ordenado por: </div></td>
        <td height="22"><select name="cmbtipord" id="cmbtipord">
...
function ue_aceptar(ls_comprobante,ls_procede,ld_fecha,ls_tipdes,ls_codigo,ls_nombre,ls_codban,ls_ctaban,li_i,ls_codpro,ls_cedbene)
{
	...
	if(opener.document.formulario.cmbtipdes != null)
	{	
	 opener.document.formulario.cmbtipdes.value=ls_tipdes;
	}
	if(opener.document.formulario.txtcodigo != null)
	{	
	 opener.document.formulario.txtcodigo.value=ls_codigo;
	}
	if(opener.document.formulario.txtnombre != null)
	{	
...
function ue_search()
{
	...
	tipo=f.tipo.value;
	orden=f.orden.value;
	campoorden=f.cmbtipord.value;
	filtro=f.filtro.value;
	restriccion=f.restriccion.value;
	if((fecdes!="")&&(fechas!=""))
	{
		// Div donde se van a cargar los resultados
		...
		ajax.send("catalogo=COMPROBANTE&comprobante="+comprobante+"&procede="+procede+"&tipdes="+tipdes+"&codigo="+codigo+
				  "&fecdes="+fecdes+"&fechas="+fechas+"&tipo="+tipo+"&orden="+orden+"&campoorden="+campoorden+"&filtro="+filtro+"&restriccion="+restriccion);
	}
	else
	{
