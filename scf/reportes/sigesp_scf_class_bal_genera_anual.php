...
class sigesp_scf_class_bal_general
{
	...
	function sigesp_scf_class_bal_general()
	{
		...
		$this->ds_reporte=new class_datastore();
		$this->ds_scforden_d=new class_datastore();
		$this->ds_scforden_h=new class_datastore();
	...
	function uf_balance_general($ad_fecfin,$ai_nivel)
	{
		...
		$this->ls_resultado_t=trim($this->la_empresa["resultado_t"]);
		$this->ls_scforden_d=trim($this->la_empresa["scforden_d"]);
	    $this->ls_scforden_h=trim($this->la_empresa["scforden_h"]);		
		//---------------------------------------------------//////////////
...
////----------------activos del tesoro----------------------------------------------------------------------------------------
	 ...
	 else
	 {
       //while($row=$this->io_sql->fetch_row($rs_data))
	    while(!$rs_data->EOF)
		{
		  ...
		  $ls_sc_cuenta_p=$rs_data->fields["cuenta_p"];
		  $ls_denom_p=$rs_data->fields["denom_p"];
		  $ls_status_p=$rs_data->fields["status_p"];
		  $ls_rnivel_p=$rs_data->fields["rnivel_p"];
		  $ld_total_debe_p=$rs_data->fields["total_debe_p"];
		  $ld_total_haber_p=$rs_data->fields["total_haber_p"];
	 ...
	 else
	 {
       //while($row=$this->io_sql->fetch_row($rs_data))
	    while(!$rs_data->EOF)
		{
		  ...
		  $ls_sc_cuenta_h=$rs_data->fields["cuenta_h"];
		  $ls_denom_h=$rs_data->fields["denom_h"];
		  $ls_status_h=$rs_data->fields["status_h"];
		  $ls_rnivel_h=$rs_data->fields["rnivel_h"];
		  $ld_total_debe_h=$rs_data->fields["total_debe_h"];
		  $ld_total_haber_h=$rs_data->fields["total_haber_h"];		  
  ...
  ////----------------PASIVO HACIENDA----------------------------------------------------------------------------------------
	 ...
	 else
	 {
		...
		while(!$rs_data->EOF)
		{
		  ...
		  $ls_sc_cuenta_p_h=$rs_data->fields["cuenta_p_h"];
		  $ls_denom_p_h=$rs_data->fields["denom_p_h"];
		  $ls_status_p_h=$rs_data->fields["status_p_h"];
		  $ls_rnivel_p_h=$rs_data->fields["rnivel_p_h"];
		  $ld_total_debe_p_h=$rs_data->fields["total_debe_p_h"];
		  $ld_total_haber_p_h=$rs_data->fields["total_haber_p_h"];
  ...
  ////----------------ingreso----------------------------------------------------------------------------------------
	 ...
	 else
	 {
	   ...
	   while(!$rs_data->EOF)
		{
		  $ls_sc_cuenta_i=$rs_data->fields["cuenta_i"];
		  $ls_denom_i=$rs_data->fields["denom_i"];
		  $ls_status_i=$rs_data->fields["status_i"];
		  $ls_rnivel_i=$rs_data->fields["rnivel_i"];
		  $ld_total_debe_i=$rs_data->fields["total_debe_i"];
		  $ld_total_haber_i=$rs_data->fields["total_haber_i"];
  ...
  ////----------------gastos----------------------------------------------------------------------------------------
	 ...
	 else
	 {
       //while($row=$this->io_sql->fetch_row($rs_data))
	    while(!$rs_data->EOF)
		{
		  $ls_sc_cuenta_g=$rs_data->fields["cuenta_g"];
		  $ls_denom_g=$rs_data->fields["denom_g"];
		  $ls_status_g=$rs_data->fields["status_g"];
		  $ls_rnivel_g=$rs_data->fields["rnivel_g"];
		  $ld_total_debe_g=$rs_data->fields["total_debe_g"];
		  $ld_total_haber_g=$rs_data->fields["total_haber_g"];
  ...
  ////----------------resultado del tesoro----------------------------------------------------------------------------------------
	 ...
	 else
	 {
       //while($row=$this->io_sql->fetch_row($rs_data))
	     while(!$rs_data->EOF)
		{
		  $ls_sc_cuenta_t=$rs_data->fields["cuenta_t"];
		  $ls_denom_t=$rs_data->fields["denom_t"];
		  $ls_status_t=$rs_data->fields["status_t"];
		  $ls_rnivel_t=$rs_data->fields["rnivel_t"];
		  $ld_total_debe_t=$rs_data->fields["total_debe_t"];
		  $ld_total_haber_t=$rs_data->fields["total_haber_t"];
  ...
  ////----------------CUENTAS DE ORDEN - DEUDORAS----------------------------------------------------------------------------------------
	 ...
	 $ls_sql="SELECT SC.sc_cuenta as cuenta_orden_d,SC.denominacion as denom_orden_d ,SC.status as status_orden_d,SC.nivel as rnivel_orden_d , 
				 (SELECT case sum(debe_mes) when null then 0 else sum(debe_mes) end FROM scg_saldos WHERE codemp='".$ls_empresa."' AND fecsal<='".$ad_fecfin."' "." and sc_cuenta=SC.sc_cuenta GROUP BY codemp,sc_cuenta) as total_debe_orden_d,
			     (SELECT case sum(haber_mes) when null then 0 else sum(haber_mes) end  FROM scg_saldos WHERE codemp='".$ls_empresa."' AND fecsal<='".$ad_fecfin."' "." and  sc_cuenta=SC.sc_cuenta GROUP BY codemp,sc_cuenta) as total_haber_orden_d,
				 0 as nivel_p FROM scg_cuentas SC 
			     WHERE  (SC.sc_cuenta like '".$this->ls_scforden_d."%' ) and SC.nivel='".$ai_nivel."'".
				"ORDER BY SC.sc_cuenta "; 
	...
	$rs_data=$this->io_sql->select($ls_sql);
	 ...
	 else
	 {
       //while($row=$this->io_sql->fetch_row($rs_data))
	     while(!$rs_data->EOF)
		{
		  $ls_sc_cuenta_orden_d=$rs_data->fields["cuenta_orden_d"];
		  $ls_denom_orden_d=$rs_data->fields["denom_orden_d"];
		  $ls_status_orden_d=$rs_data->fields["status_orden_d"];
		  $ls_rnivel_orden_d=$rs_data->fields["rnivel_orden_d"];
		  $ld_total_debe_orden_d=$rs_data->fields["total_debe_orden_d"];
		  $ld_total_haber_orden_d=$rs_data->fields["total_haber_orden_d"];
		  $ls_saldo_orden_d = 0;
		  $this->ds_scforden_d->insertRow("cuenta_orden_d",$ls_sc_cuenta_orden_d);
		  $this->ds_scforden_d->insertRow("denom_orden_d",$ls_denom_orden_d);
		  $this->ds_scforden_d->insertRow("status_orden_d",$ls_status_orden_d);
		  $this->ds_scforden_d->insertRow("nivel_orden_d",$ls_nivel);
		  $this->ds_scforden_d->insertRow("rnivel_orden_d",$ls_rnivel_orden_d);
		  $this->ds_scforden_d->insertRow("total_debe_orden_d",$ld_total_debe_orden_d);
		  $this->ds_scforden_d->insertRow("total_haber_orden_d",$ld_total_haber_orden_d);
		  $ls_saldo_orden_d=$ld_total_debe_orden_d-$ld_total_haber_orden_d;
		  $this->ds_scforden_d->insertRow("saldo_orden_d",$ls_saldo_orden_d);			  
		  $lb_valido = true;
		  $rs_data->MoveNext();	 
		}//while
  ...
  ////----------------CUENTAS DE ORDEN - ACREEDORAS----------------------------------------------------------------------------------------
	 ...
	 $ls_sql="SELECT SC.sc_cuenta as cuenta_orden_h,SC.denominacion as denom_orden_h ,SC.status as status_orden_h,SC.nivel as rnivel_orden_h , 
				 (SELECT case sum(debe_mes) when null then 0 else sum(debe_mes) end FROM scg_saldos WHERE codemp='".$ls_empresa."' AND fecsal<='".$ad_fecfin."' "." and sc_cuenta=SC.sc_cuenta GROUP BY codemp,sc_cuenta) as total_debe_orden_h,
			     (SELECT case sum(haber_mes) when null then 0 else sum(haber_mes) end  FROM scg_saldos WHERE codemp='".$ls_empresa."' AND fecsal<='".$ad_fecfin."' "." and  sc_cuenta=SC.sc_cuenta GROUP BY codemp,sc_cuenta) as total_haber_orden_h,
				 0 as nivel_p FROM scg_cuentas SC 
			     WHERE  (SC.sc_cuenta like '".$this->ls_scforden_h."%' ) and SC.nivel='".$ai_nivel."'".
				"ORDER BY SC.sc_cuenta "; 
	...
	$rs_data=$this->io_sql->select($ls_sql);
	 ...
	 else
	 {
       //while($row=$this->io_sql->fetch_row($rs_data))
	     while(!$rs_data->EOF)
		{
		  $ls_sc_cuenta_orden_h=$rs_data->fields["cuenta_orden_h"];
		  $ls_denom_orden_h=$rs_data->fields["denom_orden_h"];
		  $ls_status_orden_h=$rs_data->fields["status_orden_h"];
		  $ls_rnivel_orden_h=$rs_data->fields["rnivel_orden_h"];
		  $ld_total_debe_orden_h=$rs_data->fields["total_debe_orden_h"];
		  $ld_total_haber_orden_h=$rs_data->fields["total_haber_orden_h"];
		  $ls_saldo_orden_h = 0;
		  $this->ds_scforden_h->insertRow("cuenta_orden_h",$ls_sc_cuenta_orden_h);
		  $this->ds_scforden_h->insertRow("denom_orden_h",$ls_denom_orden_h);
		  $this->ds_scforden_h->insertRow("status_orden_h",$ls_status_orden_h);
		  $this->ds_scforden_h->insertRow("nivel_orden_h",$ls_nivel);
		  $this->ds_scforden_h->insertRow("rnivel_orden_h",$ls_rnivel_orden_h);
		  $this->ds_scforden_h->insertRow("total_debe_orden_h",$ld_total_debe_orden_h);
		  $this->ds_scforden_h->insertRow("total_haber_orden_h",$ld_total_haber_orden_h);
		  $ls_saldo_orden_h=$ld_total_haber_orden_h - $ld_total_debe_orden_h;
		  $this->ds_scforden_h->insertRow("saldo_orden_h",$ls_saldo_orden_h);			  
		  $lb_valido = true;
		  $rs_data->MoveNext();	 
		}//while
