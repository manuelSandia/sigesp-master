<?php
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_scf->uf_load_seguridad_reporte("SCF","sigesp_scf_r_hoja_trabajo.php",$ls_descripcion);
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		//---------------------------------------------------------------------------------------------------------------------------
	        $la_data_t[1]=array('denominacion'=>'',						   
						        ...
						        'egreso'=>'   <b> Ejecución Presupuestaria </b>',						        
								   ...
								   $as_suma_Balance_1,$as_suma_Balance_2,&$io_pdf)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $ls_bolivares;
		if($as_total_egreso<0)
		{
		 $ld_total_egreso_aux = abs($as_total_egreso);
		 $as_total_egreso = "(".number_format($ld_total_egreso_aux,2,",",".").")";
		}
		else
		{
		...
		if($as_total_ingreso<0)
		{
		 $ld_total_ingreso_aux = abs($as_total_ingreso);
		 $as_total_ingreso = "(".number_format($ld_total_ingreso_aux,2,",",".").")";
		}
		else
		{
		...
		if($as_debe_g<0)
		{
		 $ld_total_debe_g_aux = abs($as_debe_g);
		 $as_debe_g = "(".number_format($ld_total_debe_g_aux,2,",",".").")";
		}
		else
		{
		...
		if($as_haber_g<0)
		{
		 $ld_total_haber_g_aux = abs($as_haber_g);
		 $as_haber_g = "(".number_format($ld_total_haber_g_aux,2,",",".").")";
		}
		else
		{
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_hojatrabajo($ld_fecdes,$ld_fechas,$ls_cuentadesde,$ls_cuentahasta,$li_nivel);
	}
	/*	if($lb_valido)
	{
	 ...
	 else // Imprimimos el reporte
	 {
		...
		$ls_tot_debe_g=0;
		$ls_tot_haber_g=0;
		$ld_total_saldo_activo    = 0;
		$ld_total_saldo_pasivo    = 0;
		$ld_total_saldo_gasto     = 0;
		$ld_total_saldo_ingreso   = 0;
		$ld_total_saldo_activo_bg = 0;
		$ld_total_saldo_pasivo_bg = 0;
		...
		for($i=1;$i<=$li_tot;$i++)
		{
			...
			$ls_cuenta=rtrim($io_report->DS->getValue("sc_cuenta",$i));
					...
					$li_totfil=0;
					$as_cuenta="";
					/////////para el formato de las cuentas/////////////////////////////////////////////////
			...
			if(((substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["activo_t"])))||(substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["activo_h"]))))&&($ldec_nivel==1))
			{
			 $ld_total_saldo_activo += $ldec_saldo;
			 $ld_total_saldo_activo_bg += $ldec_saldo;
			}
			elseif(((substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["pasivo_t"])))
				 ||(substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["pasivo_h"])))
				 ||(substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["resultado_t"])))
				 ||(substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["resultado_h"]))))&&($ldec_nivel==1))
			{
			 $ldec_saldo = abs($ldec_saldo_ant)+$ldec_haber-$ldec_debe;
			 $ld_total_saldo_pasivo += $ldec_saldo;
			 $ld_total_saldo_pasivo_bg += $ldec_saldo;
			}
			elseif((substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["gasto_f"])))&&($ldec_nivel==1))
			{
			 $ld_total_saldo_gasto  += $ldec_saldo;
			 $ld_total_saldo_activo += $ldec_saldo;
			}
			elseif((substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["ingreso_f"])))&&($ldec_nivel==1))
			{
			 $ldec_saldo = abs($ldec_saldo_ant)+$ldec_haber-$ldec_debe;
			 $ld_total_saldo_ingreso += $ldec_saldo;
			 $ld_total_saldo_pasivo  += $ldec_saldo;
			}
			     ...
			     if($ls_tipo_cta=="31")
			      {
				    ...
				    if ($ldec_status=='S')
				    {
				       if ($ldec_nivel==1)
				       {
				    	$ls_total_egreso=$ls_total_egreso+$ls_egreso-$ls_ingreso;
				       }
				    }
			     ...
			     if($ls_tipo_cta=="32")
			      {
				     ...
				     if ($ldec_status=='S')
				     {
				     	if ($ldec_nivel==1)
				     	{
				     	 $ls_total_ingreso += ($ls_ingreso-$ls_egreso);
				     	}				     
				     }
			...
			if((substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["activo_t"])))||(substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["activo_h"]))))
			{
			 $la_data[$i]=array('cuenta'=>$as_cuenta,'denominacion'=>$ls_denominacion,'debe'=>$ldec_saldo,'haber'=>"",
			                   'egreso'=>"",'ingreso'=>"",'debe_g'=>$ldec_saldo,'haber_g'=>"");
			}
			elseif((substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["pasivo_t"])))
			     ||(substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["pasivo_h"])))
				 ||(substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["resultado_t"])))
				 ||(substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["resultado_h"]))))
			{
			 $la_data[$i]=array('cuenta'=>$as_cuenta,'denominacion'=>$ls_denominacion,'debe'=>"",'haber'=>$ldec_saldo,
			                    'egreso'=>"",'ingreso'=>"",'debe_g'=>"",'haber_g'=>$ldec_saldo);
			}
			elseif(substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["gasto_f"])))
			{
			 $la_data[$i]=array('cuenta'=>$as_cuenta,'denominacion'=>$ls_denominacion,'debe'=>$ldec_saldo,'haber'=>"",
			                   'egreso'=>$ldec_saldo,'ingreso'=>"",'debe_g'=>"",'haber_g'=>"");
			}
			elseif(substr(trim($ls_cuenta),0,2) == (trim($_SESSION["la_empresa"]["ingreso_f"])))
			{
			  $la_data[$i]=array('cuenta'=>$as_cuenta,'denominacion'=>$ls_denominacion,'debe'=>"",'haber'=>$ldec_saldo,
			                   'egreso'=>"",'ingreso'=>$ldec_saldo,'debe_g'=>"",'haber_g'=>"");
			}
		...
		$ls_resul_1=$ld_total_saldo_ingreso-$ld_total_saldo_gasto;
		$ls_resul_2="";
		if($ls_resul_1<0)
		  {
			...
			$ls_suma_Balance_1=$ld_total_saldo_pasivo_bg;
			$ls_suma_Balance_2=$ld_total_saldo_pasivo_bg;
			//$ls_suma2= $ls_resul_2+$ls_total_ingreso;
			//$ls_suma1= $ls_resul_1+$ls_total_egreso;
			$ls_suma2= $ls_resul_2+$ld_total_saldo_ingreso;
			$ls_suma1= $ls_resul_1+$ld_total_saldo_gasto;
		...
		elseif($ls_resul_1>0)
		{
		   ...
		   $ls_suma_Balance_2=$ld_total_saldo_pasivo_bg+$ls_resul_Balnace_2;
		   $ls_resul_Balnace_1="";
		   $ls_suma_Balance_1=$ld_total_saldo_activo_bg;
		...
		if($ld_total_saldo_activo<0)
		{
		 $ld_total_saldo_activo_aux = abs($ld_total_saldo_activo) ;
		 $ld_total_saldo_activo = "(".number_format($ld_total_saldo_activo_aux,2,",",".").")";
		}
		else
		{
		 $ld_total_saldo_activo = number_format($ld_total_saldo_activo,2,",",".");
		}
		
		if($ld_total_saldo_pasivo<0)
		{
		 $ld_total_saldo_pasivo_aux = abs($ld_total_saldo_pasivo);
		 $ld_total_saldo_pasivo = "(".number_format($ld_total_saldo_pasivo_aux,2,",",".").")";
		}
		else
		{
		 $ld_total_saldo_pasivo = number_format($ld_total_saldo_pasivo,2,",",".");
		}
		
		
		uf_print_pie_cabecera($ld_total_saldo_activo,$ld_total_saldo_pasivo,$ldec_total_saldo,$ld_total_saldo_gasto,$ld_total_saldo_ingreso,
		                      $ld_total_saldo_activo_bg,$ld_total_saldo_pasivo_bg,$ls_resul_1,$ls_resul_2,$ls_denomina,
