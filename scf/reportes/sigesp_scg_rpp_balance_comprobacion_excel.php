<?php
	...
	else // Imprimimos el reporte
	{
		...
		if ($li_saldomes==1)
		{
			$lo_hoja->write(3, 5, "Saldo del Mes",$lo_titulo);
		}
		$lo_hoja->write(3, 5+$ncol, "Saldo Actual",$lo_titulo);
		...
		for($i=1;$i<=$li_tot;$i++)
		{
			...
			if ($li_saldomes==1)
			{
				$lo_hoja->write($li_row, 5, $ldec_debe-$ldec_haber,$lo_dataright);
			}
		...
		if ($li_saldomes==1)
		{
			$lo_hoja->write($li_row, 5, $ldec_totaldebe-$ldec_totalhaber,$lo_dataright);
		}
