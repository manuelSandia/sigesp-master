...
if($ls_operacion=="BUSCAR")
{
		...
		{
			$ls_sql="SELECT cmp.comprobante,cmp.descripcion,cmp.procede,cmp.fecha,cmp.tipo_comp,cmp.cod_pro,cmp.ced_bene,
             				...
             				(CASE WHEN cmp.procede = 'SCGCMP'
             			          THEN
             			             (SELECT SUM(monto) 
             			               FROM scg_dt_cmp
             			               WHERE cmp.codemp=scg_dt_cmp.codemp AND cmp.procede=scg_dt_cmp.procede AND cmp.comprobante=scg_dt_cmp.comprobante AND cmp.fecha=scg_dt_cmp.fecha AND cmp.codban=scg_dt_cmp.codban AND cmp.ctaban=scg_dt_cmp.ctaban
             			                     AND debhab='D' 
             			               GROUP BY scg_dt_cmp.codemp,scg_dt_cmp.procede,scg_dt_cmp.comprobante,scg_dt_cmp.fecha,scg_dt_cmp.codban,scg_dt_cmp.ctaban)
             			          ELSE
		...
		while(!$rs_data->EOF)
		{
			    ...
			    if($ls_tipocat=="")
			    {
					...
					print "<td><a href=\"javascript: uf_aceptar('$ls_comprobante','$ls_descripcion','$ls_procedencia','$ls_fecha','$ls_tip','$ls_prov','$ls_bene','$ls_nomproben','$ls_codban','$ls_ctaban','$ls_tipo_comp');\">".$ls_comprobante."</a></td>";
...
var patron2 = new Array(1,3,3,3,3);
  ...
  function uf_aceptar(comprobante,descripcion,procede,fecha,tipo,prov,bene,ls_nomproben,codban,ctaban,tipo_comp)
  {
