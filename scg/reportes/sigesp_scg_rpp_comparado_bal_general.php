<?php
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_scg->uf_load_seguridad_reporte("SCG","sigesp_scg_r_comparados_balance_general.php",$ls_descripcion);
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	if($lb_valido)
	{
		...
		else// Imprimimos el reporte
		{
		    ...
		    $ld_saldo2="";
			$ld_total=0;
			//$la_data[1]=array('cuentas'=>'CUENTAS','saldo_ant'=>' ','programado'=>'BALANCE PROGRAMADO','real'=>'BALANCE REAL','variacion'=>'VARIACION','reprog'=>'REPROGRAMACION');			
			...
			for($li_i=1;$li_i<=$li_tot;$li_i++)
			{
				...
				$ls_cuenta_sal   		= $io_report->ds_cuentas->getValue("sc_cuenta_salida",$li_i).'   ';
                ...
                //print "cuenta: $ls_cuenta  nivel: $ls_nivel <br>";
				if($ls_tipo=='999')
				{
				...
				$la_data[$li_i]=array('cuentas'=>$ls_cuenta,'denominacion'=>$ls_denominacion ,
										'periodo1'=>$ldec_periodo,
							  			...
							  			'periodo2'=>$ldec_periodo2,'acumulado2'=>$ldec_acumulado2,'absoluta1'=>$ldec_absoluta1,'porc1'=>number_format($ldec_porc1,2,",","."),'absoluta2'=>$ldec_absoluta2,'porc2'=>number_format($ldec_porc2,2,",","."),'reprox'=>number_format($ldec_reprog,2,",","."),
										'cuentas_sal'=>$ls_cuenta_sal,
										'saldo_real_ant'=>$ldec_saldo_real_ant,
