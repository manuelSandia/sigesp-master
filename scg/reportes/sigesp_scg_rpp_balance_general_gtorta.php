	...
	if($lb_valido)
	{
		...
		else// Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_tot;$li_i++)
			{
				...
				if($ls_activo."000"==substr($ls_sc_cuenta,0,4))
				{
					$ld_total=$ld_total+$ld_saldo;
					$li_totalactivo=$li_totalactivo+$ld_saldo;

				}
				if($ls_pasivo."000"==substr($ls_sc_cuenta,0,4))
				{
					$ld_total=$ld_total+$ld_saldo;
					$li_totalpasivo=$li_totalpasivo+$ld_saldo;
				...
				{
					 $li_totalresultado=$li_totalresultado+$ld_saldo;
				...
				$cont=$cont+1;
				//print $as_cuenta." ".$ls_denominacion."  ".$ld_saldo;
				switch($ls_nivel)
				{
				 case 7:
						if($ls_estatus == 'C')
			...
			$li_totalactivo=abs($li_totalactivo);
			$li_totalresultado=abs($li_totalresultado);
			$li_totalpasivo=abs($li_totalpasivo);
			 // Dataset definition 
			 $DataSet = new pData;
			 $DataSet->AddPoint(array($li_totalresultado,$li_totalpasivo,$li_totalactivo),"Serie1");
