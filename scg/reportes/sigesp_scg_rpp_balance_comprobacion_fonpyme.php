<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_fecha,&$io_pdf)
	{
		...
		//pie de pagina
		//$io_pdf->line(20,80,580,90);
		$ls_direccion1='Avenida Fransisco de Miranda, Torre Europa, Piso 7, El Rosal, Municipio ChacaoEstado Miranda, Caracas, Venezuela';
		$li_tm_d1=$io_pdf->getTextWidth(11,$ls_direccion1);
		$tm_dir1=376-($li_tm_d1/2);
		...
		else
		{
			$la_config=array('showHeadings'=>1, // Mostrar encabezados
							 ...
							 'cols'=>array('cuenta'=>array('justification'=>'center','width'=>80), // Justificación y ancho de la columna
									...
									'saldoanterior'=>array('justification'=>'right','width'=>70), // Justificación y ancho de la columna
									...
									'saldomes'=>array('justification'=>'right','width'=>70), // Justificación y ancho de la columna
	...
	function uf_print_pie_cabecera($li_saldomes,$ldec_total_saldomes,$adec_totaldebe,$adec_totalhaber,$adec_total_saldo,&$io_pdf)
	{
		...
		else
		{
			...
			$la_config=array('showHeadings'=>0, // Mostrar encabezados
							 ...
							 'xOrientation'=>'center', // Orientación de la tabla
							 'xPos'=>299, // Orientación de la tabla
					 		 'cols'=>array('total'=>array('justification'=>'right','width'=>270), // Justificación y ancho de la columna
							 			   'debe'=>array('justification'=>'right','width'=>70), // Justificación y ancho de la columna
							 			   'haber'=>array('justification'=>'right','width'=>70), // Justificación y ancho de la columna
							 			   'saldomes'=>array('justification'=>'right','width'=>70), // Justificación y ancho de la columna
										   'saldo'=>array('justification'=>'right','width'=>70))); // Justificación y ancho de la columna
	...
	function uf_obtener_longitud_niveles($formato) {
		$arrformato = array();
		$arrlonnive = array();
	...
	function uf_obtener_escalera($cuenta){
		$li_lon_formato = strlen(str_replace('-','',trim($_SESSION["la_empresa"]["formcont"])));
		$arrnivlon      = uf_obtener_longitud_niveles($_SESSION["la_empresa"]["formcont"]);
		$numniv         = count($arrnivlon);
		$cancarniv      = $li_lon_formato;
		$arrescalera    = array();
		
		for ($j = 0; $j < $numniv; $j++) {
			$cancarniv        = $cancarniv - $arrnivlon[$j];
			$cantcero         = str_pad('',$cancarniv,0,0);
			$arrescalera[$j]  = substr($cuenta, 0,$li_lon_formato-$cancarniv).$cantcero;
		}
	...
	{
		if ($li_precierre!=1) {
			$lb_valido=$io_report->uf_scg_reporte_balance_comprobante($ls_cuentadesde,$ls_cuentahasta,$ld_fecdesde,$ld_fechasta,$li_nivel,$li_saldocero);
		}
		else{
			$ld_fecdesde="01/01/".substr($_SESSION["la_empresa"]["periodo"],0,4);
			$ld_fechasta="30/06/".substr($_SESSION["la_empresa"]["periodo"],0,4);
			
			$lb_valido=$io_report->uf_scg_reporte_balance_comprobante($ls_cuentadesde,$ls_cuentahasta,$ld_fecdesde,$ld_fechasta,$li_nivel,$li_saldocero);
			if ($lb_valido) {
				$ld_totgasing=$io_report->uf_obtener_total_gastoingreso($ld_fecdesde,$ld_fechasta);
			}
		}
	...
	else // Imprimimos el reporte
	{
		...
		for($i=1;$i<=$li_tot;$i++)
		{
			...
			if($li_precierre!=1){
				...
				if (substr($as_cuenta, 0,1)=="3") {
					if ($li_nivel==1) {
						if($ld_totgasing<0){
							$ldec_debe=$io_report->dts_reporte->getValue("debe_mes",$i) + abs($ld_totgasing);
							$ldec_haber=$io_report->dts_reporte->getValue("haber_mes",$i);	
						}
						else{
							$ldec_debe=$io_report->dts_reporte->getValue("debe_mes",$i);
							$ldec_haber=$io_report->dts_reporte->getValue("haber_mes",$i) + abs($ld_totgasing);
						}
					}
					else if(substr($ls_cuenta, 0,1)==substr($_SESSION["la_empresa"]["c_resultad"], 0,1)){
						$arrescalera = uf_obtener_escalera($_SESSION["la_empresa"]["c_resultad"]);
						foreach ($arrescalera as $nivcuenta) {
							if ($nivcuenta==$ls_cuenta) {
								if($ld_totgasing<0){
									$ldec_debe=$io_report->dts_reporte->getValue("debe_mes",$i) + abs($ld_totgasing);
									$ldec_haber=$io_report->dts_reporte->getValue("haber_mes",$i);
								}
				...
				elseif (substr($as_cuenta, 0,1)=="4" || substr($as_cuenta, 0,1)=="5"){
					$ldec_debe=$io_report->dts_reporte->getValue("debe_mes",$i);
					$ldec_haber=$io_report->dts_reporte->getValue("haber_mes",$i);
					$ld_sal_aux=$ldec_debe-$ldec_haber;
					if($ld_sal_aux<0){
						$ldec_debe = $ldec_debe + abs($ld_sal_aux);
					}
					else{
						$ldec_haber = $ldec_haber + $ld_sal_aux;
					}
				}
				else{
					$ldec_debe=$io_report->dts_reporte->getValue("debe_mes",$i);
