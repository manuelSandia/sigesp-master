<?php
	...
	elseif(!$data->EOF){
				...
				case '2':
					...
					if($cambioniv2 && !$cambioultimo){
						...
						if($cambioultimo){
							...
							$lo_hoja->write($li_row, 1, '    ', $lo_dataleft);
							$lo_hoja->write($li_row, 3, uf_is_negative($ld_totantpasivo), $lo_dataright);
							$lo_hoja->write($li_row, 4, uf_is_negative($ld_totpasivo), $lo_dataright);
		...
		/*foreach ($la_data as $fila) {
			$ls_denominacion = $fila['denominacion'];
			$ld_saldoant     = $fila['periodo_an'];
			$ld_saldo        = $fila['periodo_ac'];
