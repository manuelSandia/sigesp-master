...
echo "<td style=text-align:center width=300>Denominación</td>";
echo "<td style=text-align:center width=200>Cuenta Contable de Provisiones Acumuladas y Reservas Técnicas o de  Depreciación y Amortización Acumulada</td>";
echo "</tr>";

if ($ls_operacion=="BUSCAR")
   {
	 $ls_sql = "SELECT trim(sc_cuenta) as sc_cuenta, denominacion, status,cueproacu 
	              FROM scg_cuentas
			    ...
			    while(!$rs_data->EOF)
				     {
					   ...
					   switch ($ls_destino){
					     case "":
						   echo "<td style=text-align:center width=100><a href=\"javascript: aceptar('$ls_scgcta','$ls_dencta','$ls_estcta','$ls_cueproacu');\">".$ls_scgcta."</a></td>";
						 break;
					     case "destino":
				       ...
				       echo "<td style=text-align:left title='".$ls_dencta."' width=200>".$ls_dencta."</td>";
				       echo "<td style=text-align:left title='".$ls_cueproacu."' width=200>".$ls_cueproacu."</td>";
					   echo "</tr>";
                       $rs_data->MoveNext();
					 }
			  }
...
<script language="JavaScript">
  ...
  function aceptar(cuenta,d,status,cueproacu)
  {
	...
	opener.document.form1.txtcueproacu.value=cueproacu;
    close();
  }
