			...
			else
			{
				...
				if(uf_valida_cuenta($ls_cuenta_valid,$ls_denominacion_cta))
				{
					...
					if($int_scg->uf_scg_select_cuenta($is_codemp,$ls_cuenta,$ls_status,$ls_denominacion))
					{
							if($ls_status_ope=='C')
							{
								$lb_valido=$int_scg->uf_scg_update_cuenta($is_codemp,$ls_cuenta,$ls_denominacion_cta,$ls_cueproacu);
								$ls_evento="UPDATE";
						...
						do 
						{
						  ...
						  if(!$int_scg->uf_scg_select_cuenta($is_codemp,$ls_NextCuenta,&$as_status,&$as_denominacion))
						  {
							  ...
							  if(!$lb_existe)
							  {
								  ...
								  $lds_cuenta_temp->insertRow("cueproacu",$ls_cueproacu);				  			  
								  $li_fila =  $li_fila + 1;
						...
						$li_total = $lds_cuenta_temp->getRowCount("SC_cuenta");
						 ...
						 if($li_total>0)
						 {
							for($li_fila=1;$li_fila<=$li_total;$li_fila++)
							{
								 ...
								 $ls_cueproacu   = $lds_cuenta_temp->getValue("cueproacu",$li_fila); 	    		 
								 $ls_mensaje_error="Error en Guardar";
								 $ls_status = "S";
								 $int_scg->io_sql->begin_transaction();
								 $lb_valido = $int_scg->uf_scg_insert_cuenta($is_codemp,$ls_cuenta,$ls_denominacion,$ls_status,$li_Nivel,$ls_cuenta_ref,$ls_mensaje_error,$ls_cueproacu);
						 ...
						 {							
								...
								$lb_valido     = $int_scg->uf_scg_insert_cuenta($is_codemp,$ls_cuenta,$ls_denominacion_cta,"C",$li_Nivel,$ls_NextCuenta,&$ls_mensaje_error,$ls_cueproacu);		
						 }
...
<input type="hidden" id="txt_activo" value="<?php print $activo; ?>" name="txt_activo"   >
<input type="hidden" id="txt_cueproacu" value="<?php print $ls_cpart; ?>" name="txt_cueproacu">   
<input type="hidden" id="txt_cuedepamo" value="<?php print $ls_cdaa; ?>" name="txt_cuedepamo">
          ...
          <table width="614" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
              ...
              <tr class="formato-blanco">
                ...
                <td width="513" height="22"><input name="txtcuenta" <?php print $disabled ?> type="text" id="txtcuenta" value="<?php print $ls_cuenta ?>" maxlength="<?php print $li_size_form;?>" onKeyPress="return keyRestrict(event,'1234567890');"  onBlur="rellenar_cad(this.value,<?php print $li_size_form;?>)" style="text-align:center">                    <a href="javascript: cat();"><img src="../../shared/imagebank/tools15/buscar.gif" alt="Catalogo de Cuentas del Plan Unico" width="15" height="15" border="0"></a>
                    ...
                    <input name="status" type="hidden" id="status" value="<?php print $ls_status_ope;?>">              </td>
              </tr>
              <tr class="formato-blanco">
                ...
                <td height="22"><input name="txtdenominacion" type="text" id="txtdenominacion" value="<?php print $ls_denominacion?>" size="95" maxlength="254" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnopqrstuvwxyzñ '+'.,-()*/%$!');">
                  <input name="txtstatus" type="hidden" id="txtstatus" value="<?php print $ls_status ?>">                </td>
              </tr>
            <tr class="formato-blanco">
              <td height="22" colspan="2"><table width="603" height="20" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="270"><div align="left">Cuenta Contable de Provisiones  Acumuladas &nbsp;&nbsp;&nbsp;&nbsp; <br> y Reservas T&eacute;cnicas o de &nbsp;Depreciaci&oacute;n y Amortizaci&oacute;n Acumulada</div></td>
                  <td width="237"><input name="txtcueproacu" type="text" id="txtcueproacu" value="<?php print $ls_cueproacu; ?>" size="28" maxlength="25" style="text-align:center" >
                    <a href="javascript: cat_provisiones();"><img src="../../shared/imagebank/tools15/buscar.gif" alt="Cuentas Contables de Provisiones Acumuladas y Reservas T&eacute;cnicas o de Depreciaci&oacute;n y Amortizaci&oacute;n Acumulada" width="15" height="15" border="0"></a></td>
                </tr>
              </table></td>
            </tr>
          </table>
...
function cat_provisiones()
{
	f=document.form1;
	f.txtcuenta.readonly=false;
	f.operacion.value="CAT";
    if (document.form1.txtcueproacu.disabled == false)
    {
        window.open("sigesp_scg_cat_provisionesacumuladas.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=518,height=400,left=50,top=50,location=no,resizable=yes");
    }
}
...
function ue_buscar()
{
	...
	if (li_leer==1)
	   {
	     window.open("sigesp_scg_cat_ctas.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=618,height=400,left=50,top=50,location=no,resizable=yes");
	   }
	else
 	   {
	     alert("No tiene permiso para realizar esta operación");
...
function rellenar_cad(cadena,longitud)
{
    ...
    if (primero==document.form1.txt_activo.value)           
    {
        document.form1.txtcueproacu.disabled = false;
        document.form1.txtcueproacu.readonly = false;
    }
    else
    {
        document.form1.txtcueproacu.disabled = true;
        document.form1.txtcueproacu.readonly = true;
    }
}
</script>
</html>