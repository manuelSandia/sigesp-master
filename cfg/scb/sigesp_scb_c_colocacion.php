...
function uf_guardar_colocacion($as_colocacion,$as_dencol,$as_codban,$as_cuenta_banco,$as_tasa,$ad_desde,$ad_hasta,$adec_monto,
                               $adec_interes,$as_codtipcol,$as_cuentascg,$as_cuentacob,$as_cuentaspi,$ai_estrei,$as_plazo,$as_codigo,
							   $as_codpro,$as_codproben,$ls_codestpro1,$ls_codestpro2,$ls_codestpro3,$ls_codestpro4,$ls_codestpro5,$ls_estcla)
{
	...
	$ls_sql= " INSERT INTO scb_colocacion(codemp, codban, ctaban, numcol, dencol, codtipcol, feccol, diacol,    ".
				 ...
				 "                            codconmov,cod_pro,ced_bene,sc_cuentacob,codestpro1,codestpro2,codestpro3,codestpro4,codestpro5,estcla)    ".
				 ...
				 "                             '".$as_codigo."','".$as_codpro."','".$as_codproben."','".$as_cuentacob."','".$ls_codestpro1."','".$ls_codestpro2."','".$ls_codestpro3."','".$ls_codestpro4."','".$ls_codestpro5."','".$ls_estcla."') ";
...
function uf_update_colocacion($as_colocacion,$as_dencol,$as_codban,$as_cuenta_banco,$as_tasa,
							$ad_desde,$ad_hasta,$adec_monto,$adec_interes,$as_codtipcol,
							$as_cuentascg,$as_cuentacob,$as_cuentaspi,$ai_estreicol,$as_plazo,$as_codigo,
							$as_codpro,$as_codproben,$ls_codestpro1,$ls_codestpro2,$ls_codestpro3,$ls_codestpro4,$ls_codestpro5,$ls_estcla)
{
	$li_desde=0;$li_hasta=0;
	$ls_codemp=$this->dat["codemp"];
	...
	$ls_sql= " UPDATE scb_colocacion ".
				 ...
				 "        codconmov='---',cod_pro='----------',ced_bene='----------',spi_cuenta='".$as_cuentaspi."',sc_cuentacob='".$as_cuentascg."',".
				 "        codestpro1='".$ls_codestpro1."',codestpro2='".$ls_codestpro2."',codestpro3='".$ls_codestpro3."',codestpro4='".$ls_codestpro4."',codestpro5='".$ls_codestpro5."',estcla='".$ls_estcla."'".
...
//print $ls_sql;
	...
	{
		$lb_valido=false;
		$this->is_msg_error="Error en metodo uf_update_colocacion".$this->fun->uf_convertirmsg($this->io_sql->message);
		$this->io_sql->rollback();
	}
	else
	{
...
/*function sumardias($fecha,$ndias) 
	...
	function sumarDias ($fecha, $ndias)
	{
		if ($ndias > 0)
		{
			...
			{         
				...
				$fecha_final=$this->uf_last_day($mes,$anio);
				//$ultimo_dia=substr($$ultimo_dia,0,2);
				//$fecha_final=$anio.'/'.str_pad($mes,2,"0",0).'/'.str_pad($ultimo_dia,2,"0",0); 
			}
			else   
			{
	...
	function sumarDias2 ($fecha,$ad_hasta,$ndias)
	{
		if ($ndias > 0)
		{
			$dia=substr($ad_hasta,0,2);     
			$mes=substr($ad_hasta,3,2);      
			$anio=substr($ad_hasta,6,4); 
			$ultimo_dia=date("d",mktime(0, 0, 0,$mes+1,0,$anio)); 
			$dias_adelanto=$ndias;
			$siguiente=$dia+$dias_adelanto; 
			if ($ultimo_dia < $siguiente)
			{         
				...
				$VL_UltDia = date ('d', mktime (0, 0, 0, $mes + 1, 0, $anio));//NUEVO     
				$fecha_final=$anio.'/'.str_pad($mes,2,"0",0).'/'.str_pad($VL_UltDia,2,"0",0); 
			}
			else   
			{
			     $fecha_final=$anio.'/'.$mes.'/'.$siguiente;
	...
	function sumarDias3 ($fecha, $ndias)
	{
		if ($ndias > 0)
		{
			$dia=substr($fecha,0,2);     
			$mes=substr($fecha,3,2);      
			$anio=substr($fecha,6,4); 
			$ultimo_dia=$this->uf_last_day($mes,$anio);
			$ultimo_dia=substr($ultimo_dia,0,2); 
			$dias_adelanto=$ndias;
			$siguiente=$dia+$dias_adelanto; 
			if ($ultimo_dia < $siguiente)
			{         
				$dia_final=$siguiente-$ultimo_dia;
				$mes++;         
				$ultimodia_2=$this->uf_last_day($mes,$anio);
				$ultimodia_2=substr($ultimodia_2,0,2);
				$VL_UltDia = date ('d', mktime (0, 0, 0, $mes + 1, 0, $anio));//NUEVO  
				if($ndias=='365')
				{
					$dia_final=$dia;
