...
$title[1]="Check <input name=chkall type=checkbox id=chkall value=T style=height:15px;width:15px onClick=javascript:uf_select_all(); >";	$title[2]="Cuenta Presupuestaria";   $title[3]="Denominación"; $title[4]="Cuenta Contable"; $title[5]="Cta. Contable Articulos"; 
$grid1="grid";	
if ($ls_operacion=="BUSCAR")
   {
	...
	$ls_sql="SELECT spg_cuenta,denominacion,sc_cuenta,status,sc_cuenta_art".
	 		"  FROM spg_cuentas".
		...
		if($row=$io_sql->fetch_row($rs_data))
		{
			...
			if($totrow>0)
			{
				for($z=1;$z<=$totrow;$z++)
				{
					...
					$cuentascgart = $data["sc_cuenta_art"][$z];
					if($status=='C')
					{
						...
						$object[$z][5]="<input type=text name=txtcuentascgart".$z." value='".$cuentascgart."' id=txtcuentascgart".$z." class=sin-borde readonly style=text-align:center size=16 maxlength=25>";
					}
					else
					{
			...
			else
			{
					...
					$object[1][5]="<input type=text name=txtcuentascgart1 value='' id=txtcuentascgart1 class=sin-borde readonly style=text-align:center size=16 maxlength=25>";
					$totrow=0;
			}
  ...
  for(i=1;(i<=total)&&(li_sel<50);i++)	
  {
	...
	if(status=='C')
	{
		if(eval("f.chkcta"+i+".checked==true"))
		{
			...
			ls_cuentaart_scg=eval("f.txtcuentascgart"+i+".value");
			...
			eval("fop.txtcuentascgart"+li_row+".value='"+ls_cuentaart_scg+"'");
		}
