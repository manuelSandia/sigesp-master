...
function uf_procesar_cuentas($as_cuenta_spi,$as_denominacion_cta,$as_cuenta_scg,$aa_security)
{
		...
		if($this->int_spi->uf_spi_select_cuenta($this->dat["codemp"],$ls_spi_cuenta, &$ls_status, &$ls_denominacion, $ls_scg_cuenta ))
		{
				...
				if($lb_valido)
				{
					...
					////////////////////////////////         SEGURIDAD               //////////////////////////////
					 if(($lb_valido)&&($_SESSION["la_empresa"]["estpreing"]==0))
					 {
						 $lb_valido = 	$this->uf_spi_insert_estructura($this->dat["codemp"],$ls_spi_cuenta);
					 }			
				}
				else
				{
			  ...
			  if(!$this->int_spi->uf_spi_select_cuenta($this->dat["codemp"],$ls_NextCuenta, &$ls_status, &$ls_denominacion, $ls_scg_cuenta ))
			  {
				  $ls_PadNextCuenta =$this->int_scg->uf_pad_cuenta_plan($this->dat["formplan"],$ls_NextCuenta);
 				  $this->int_spi->uf_select_plan_unico_cuenta($ls_PadNextCuenta,&$as_denominacion_plan);
					  ...
					  $lds_cuenta_temp->insertRow("spi_cuenta",trim($ls_NextCuenta));
			...
			$li_total = $lds_cuenta_temp->getRowCount("sc_cuenta");
			 ...
			 if($li_total>0)
			 {
				...
				for($li_fila=1;$li_fila<=$li_total;$li_fila++)
				{
					 ...
					 $lb_valido = $this->int_spi->uf_spi_insert_cuenta(trim($ls_spi_cuenta),$ls_denominacion,trim($ls_scg_cuenta),$ls_status,$li_Nivel,$ls_cuenta_ref);
					 if (!$lb_valido)
					 {
						break; 
					 }
...
function uf_procesar_delete_cuenta($as_cuenta_spi,$as_dencuentaspi,$as_cuenta_scg,$lb_existe,$aa_security)
{
  ...
  //-----------------------------------------------------------------------------------------------------------------------------------
	function uf_spi_insert_estructura($as_codemp,$as_spi_cuenta)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_spi_insert_estructura
		//		   Access: public 
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		 ...
		 $ls_sql="INSERT INTO spi_cuentas_estructuras (codemp, spi_cuenta, codestpro1, codestpro2, codestpro3, codestpro4, codestpro5, estcla) ".
		 		 " SELECT '".$as_codemp."', '".$as_spi_cuenta."', '-------------------------', '-------------------------', '-------------------------', '-------------------------', '-------------------------', '-' ".
				 "   FROM spi_cuentas ".
				 "  WHERE spi_cuenta = '".$as_spi_cuenta."' ".
				 "    AND spi_cuenta  NOT IN (SELECT spi_cuenta FROM spi_cuentas_estructuras WHERE codestpro1='-------------------------' AND codestpro2='-------------------------' AND codestpro3='-------------------------' AND codestpro4='-------------------------' AND codestpro5='-------------------------' AND estcla='-')";
		$li_rows=$this->int_spi->io_sql->execute($ls_sql);
		if($li_rows===false)
		{
			$lb_valido=false;	
