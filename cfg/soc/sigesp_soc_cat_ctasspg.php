...
if($ls_operacion=="BUSCAR")
{
   ...
   else
   {
		...
		if(!$rs->EOF)	  
		{						  
			 $ls_cadena = trim($rs->fields["soc_servic"]);                  
		}
   		$ls_cadena_filtro = "";
		...
		{
			if(!empty($la_spg_cuenta[$li_i]))
			{
			 if($li_i==0)
			 {
				$ls_cadena_filtro .= "   AND (spg_cuenta like '".$la_spg_cuenta[$li_i]."%'";
			 }
			 else
			 {
				$ls_cadena_filtro .="    OR spg_cuenta like '".$la_spg_cuenta[$li_i]."%'";
			 }
			}
		}
		if(!empty($ls_cadena_filtro))
		{
		 $ls_sql .= $ls_cadena_filtro.") ";						
		}
		$ls_sql .=" GROUP BY spg_cuenta ORDER BY spg_cuenta ASC"; 
		...
		else
		{
			...
			if(!$rs_cta->EOF)
			{
				while(!$rs_cta->EOF)
				{
					$cuenta=trim($rs_cta->fields["spg_cuenta"]);
					$denominacion=$rs_cta->fields["denominacion"];
					$scgcuenta=$rs_cta->fields["sc_cuenta"];
					$status=$rs_cta->fields["status"];
