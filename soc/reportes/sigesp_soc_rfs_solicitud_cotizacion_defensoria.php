<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsolcot,$as_fecsolcot,$as_dentipsolcot,$as_obssolcot,&$io_pdf)
	{
		...
		$io_pdf->addText(20,100,7,"COMPRADOR:"); // Agregar el título
		$io_pdf->line(290,60,290,117);	//VERTICAL		
		$io_pdf->addText(300,100,7,"JEFE DE COMPRAS:"); // Agregar el título
		$io_pdf->addText(245,119,8,"FIRMAS AUTORIZADAS"); // Agregar el título
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_columna=array('codigo'=>'<b>Código</b>',
						  'denominacion'=>'<b>Denominación</b>',
  						  'unimedida'=>'<b>Unidad de Medida</b>',
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('codigo'=>array('justification'=>'center','width'=>115),      // Justificación y ancho de la columna
									   ...
									   'unimedida'=>array('justification'=>'left','width'=>110), // Justificación y ancho de la columna
								     ...
								     while($row=$io_sql->fetch_row($rs_datos))
								          {
											...
											if ($ls_tipsolcot=='B')
											{
												$ls_denunimed=$io_report->uf_select_denominacion("siv_unidadmedida,siv_articulo","denunimed","WHERE siv_articulo.codart='".$ls_codigo."' AND siv_unidadmedida.codunimed=siv_articulo.codunimed");
											}
											else
											{
												$ls_denunimed=$io_report->uf_select_denominacion("siv_unidadmedida,soc_servicios","denunimed","WHERE soc_servicios.codser='".$ls_codigo."' AND siv_unidadmedida.codunimed=soc_servicios.codunimed");
											}
										    $ls_denite       = $row["denite"];
											//$ls_denunimed    = $row["denunimed"];
										    $ld_canite       = number_format($row["canite"],2,',','.');
									        $la_datos[$li_i] = array('codigo'=>$ls_codigo,'denominacion'=>$ls_denite,'unimedida'=>$ls_denunimed,'cantidad'=>$ld_canite);
									      }
				    		       }
						        else
							       {
