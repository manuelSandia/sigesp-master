...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
									   ...
									   $ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$as_estcom,&$io_pdf)
	{
		...
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,705,14,$ls_titulo); // Agregar el título
		...
		// cuadro inferior
        $io_pdf->Rectangle(15,60,570,45);
		$io_pdf->Rectangle(15,60,570,78); 
		$io_pdf->addText(20,130,7,"OBSERVACIONES:"); // Agregar el título
		$io_pdf->line(15,116,585,116);	//HORIZONTAL	
		$io_pdf->addText(40,108,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText(25,87,7,$_SESSION["la_nomusu"]." ".$_SESSION["la_apeusu"]);
		$io_pdf->addText(30,63,7,"ANALISTA DE COMPRA"); // Agregar el título
		$io_pdf->line(130,60,130,116);	//VERTICAL	
		$io_pdf->addText(167,108,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(160,63,7,"JEFE DE COMPRAS"); // Agregar el título
		$io_pdf->line(260,60,260,116);	//VERTICAL		
		$io_pdf->addText(295,108,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(290,63,7,"COORD. DE ADMINISTRACION"); // Agregar el título
		$io_pdf->line(410,60,410,116);	//VERTICAL		
		$io_pdf->addText(470,108,7,"PROVEEDOR"); // Agregar el título
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			$io_pdf->ezSetCmMargins(5.8,5,3,3); // Configuración de los margenes en centímetros
			...
			if ($row=$io_sql->fetch_row($rs_data))
			{
			   ...
			   if ($lb_valido)
			   {
				 ...
				 if ($li_totrows>0)
				 {
					...
					/*for ($i=$li_i+1;$i<=80;$i++)
					    {
						  $la_data[$i]=array('codigo'=>$ls_codartser,'denominacion'=>$ls_denartser,'cantidad'=>$li_cantartser,
											  'unidad'=>$ls_unidad,'cosuni'=>$ld_preartser,'baseimp'=>$ld_subtotartser,
											  'cargo'=>$ld_carartser,'montot'=>$ld_totartser);
					    }*/
					}
