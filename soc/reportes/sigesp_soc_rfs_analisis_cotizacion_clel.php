<?PHP
	...
	function uf_print_encabezado_pagina($as_numanacot,$ad_fecha,$ds_contcol,&$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],38,545,80,40); // Agregar Logo
        $li_tm=$io_pdf->getTextWidth(14,"<b>ANALISIS DE COTIZACIONES, INFORME DE RECOMENDACIÓN Y ADJUDICACION</b>");
        $tm=396-($li_tm/2);
        $io_pdf->addText($tm,562,14,"<b>"."<b>ANALISIS DE COTIZACIONES, INFORME DE RECOMENDACIÓN Y ADJUDICACION</b>"."</b>");
        $io_pdf->addText(550,546,9,"<b>"."<b>NUMERO</b>"."</b>");
        $io_pdf->addText(550,535,9,"<b>"."<b>FECHA</b>"."</b>");
        
        $io_pdf->addText(650,551,9,"<b>"."<b>$as_numanacot</b>"."</b>");
        $io_pdf->addText(650,540,9,"<b>"."<b>$ad_fecha</b>"."</b>");
...
//		$io_pdf->Rectangle(544,529,240,42);
	...
	function uf_print_subtotales($ds_contcol,$la_cotizaciones,$la_countcot,$io_ds_detalle,$as_numanacot,$as_tipsolcot,$aa_ganadores,$ds_linea_tot,&$io_pdf)
	{
		...
		$ls_diavalofe1=$ds_contcol[0]["diavalofe1"];
		$ls_diavalofe2=$ds_contcol[1]["diavalofe2"];
		$ls_diavalofe3=$ds_contcol[2]["diavalofe3"];				
		...
		$la_anchos=array();
		$la_justificaciones=array();
		$la_data[0]["0"]="TIEMPO DE ENTREGA";
		$la_data[0]["1"]=$ls_diaentcom1." Días";
		$la_data[0]["2"]=$ls_diaentcom2." Días";
		$la_data[0]["3"]=$ls_diaentcom3." Días";
		...
		$la_anchos=array();
		$la_justificaciones=array();
		$la_data[0]["0"]="VIGENCIA DE COTIZACION";
		$la_data[0]["1"]=$ls_diavalofe1." Días";
		$la_data[0]["2"]=$ls_diavalofe2." Días";
		$la_data[0]["3"]=$ls_diavalofe3." Días";
		$la_anchos_col = array(69,65,65,65);
		$la_justificaciones = array("left","center","center","center","left","center");
		$la_opciones = array("color_texto"     => array(0,0,0),
							 "anchos_col"    => $la_anchos_col,
	...
	//------------------------------------------------------------------------------------------------------------------------------------
    //------------------------------------------------------------------------------------------------------------------------------------
    function uf_print_recomendaciones($observacion,&$io_pdf)
    {
        $la_data=array();
        $la_anchos=array();
        $la_justificaciones=array();
        $la_data[0]["1"]="<b>RECOMENDACIONES:</b>";
        $la_data[0]["2"]=$observacion;        
        $la_anchos_col = array(69,195);
        $la_justificaciones = array("left","left");
        $la_opciones = array("color_texto"     => array(0,0,0),
	...
	function uf_print_pie_pagina(&$io_pdf)
	{
		...
		$io_pdf->setStrokeColor(0,0,0);
		$io_pdf->Rectangle(30,30,750,70);
		$io_pdf->line(30,42,780,42);	//VERTICAL
		$io_pdf->line(30,88,780,88);	//VERTICAL
		$io_pdf->addText(85,90,8,"<b>ELABORADO  POR:</b>");
		$io_pdf->addText(75,32,8,"<b>ANALISTA DE COMPRAS</b>");
		$io_pdf->line(218,30,218,100);	//VERTICAL
		$io_pdf->addText(244,90,8,"<b>REVISADO Y RECOMENDADO POR:</b>");
		$io_pdf->addText(243,32,8,"<b>JEFE DE LA UNIDAD DE COMPRAS</b>");
		$io_pdf->line(404,30,405,100);	//VERTICAL
		$io_pdf->addText(425,90,8,"<b>CONFORMADO  Y RECOMENDADO POR:</b>");
		$io_pdf->addText(405,32,8,"<b>DIRECTOR (A) DE SERVICIOS ADMINISTRATIVO</b>");
		$io_pdf->line(594,30,593,100);	//VERTICAL
		$io_pdf->addText(618,90,8,"<b>APROBADO Y ADJUDICADO POR:</b>");
		$io_pdf->addText(660,32,8,"<b>PRESIDENCIA</b>");
	...
	//CLEL Se comentaron estas dos lineas para no tomar la observacion y recomendacion desde el metodo _GET sino de BD
	...
	if($lb_valido)
	{
		...
		if($lb_valido)
		{
			...
			//CLEL para sacar estos campos desde la BD y no desde el metodo _GET
            ...
            if ($li_totcot==1)
            {
                ...
                $io_ds_detallepro->insertRow("diavalofe_1",$la_proveedor[1]["diavalofe"]);
                $io_ds_detallepro->insertRow("diavalofe_2",'');
                $io_ds_detallepro->insertRow("diavalofe_3",''); 
            ...
            if ($li_totcot==2)
            {
                ...
                $io_ds_detallepro->insertRow("diavalofe_1",$la_proveedor[1]["diavalofe"]);
                $io_ds_detallepro->insertRow("diavalofe_2",$la_proveedor[2]["diavalofe"]);
                $io_ds_detallepro->insertRow("garanacot_3",'');
            ...
            if ($li_totcot==3)
            {
                ...
                $io_ds_detallepro->insertRow("diavalofe_1",$la_proveedor[1]["diavalofe"]);
                $io_ds_detallepro->insertRow("diavalofe_2",$la_proveedor[2]["diavalofe"]);
                $io_ds_detallepro->insertRow("diavalofe_3",$la_proveedor[3]["diavalofe"]);
            }
			...
			$cCol=1;
                ...
                $diavalofe1     = $io_ds_detallepro->getValue('diavalofe_1',1 );
                $diavalofe2     = $io_ds_detallepro->getValue('diavalofe_2',1 ); 
                $diavalofe3     = $io_ds_detallepro->getValue('diavalofe_3',1 );
				...
				$ds_col1 = array("nombre1"=>$nompro1,"rif1"=>$rifpro1,"dirpro1"=>$dirpro1,"feccot1"=>$feccot1,"codpro1"=>$codpro1,"diaentcom1"=>$diaentcom1,"forpagcom1"=>$forpagcom1,"estasitec1"=>$estasitec1,"estesp1"=>$estesp1,"garanacot1"=>$garanacot1,"diavalofe1"=>$diavalofe1); //
				$ds_col2 = array("nombre2"=>$nompro2,"rif2"=>$rifpro2,"dirpro2"=>$dirpro2,"feccot2"=>$feccot2,"codpro2"=>$codpro2,"diaentcom2"=>$diaentcom2,"forpagcom2"=>$forpagcom2,"estasitec2"=>$estasitec2,"estesp2"=>$estesp2,"garanacot2"=>$garanacot2,"diavalofe2"=>$diavalofe2); //
				$ds_col3 = array("nombre3"=>$nompro3,"rif3"=>$rifpro3,"dirpro3"=>$dirpro3,"feccot3"=>$feccot3,"codpro3"=>$codpro3,"diaentcom3"=>$diaentcom3,"forpagcom3"=>$forpagcom3,"estasitec3"=>$estasitec3,"estesp3"=>$estesp3,"garanacot3"=>$garanacot3,"diavalofe3"=>$diavalofe3); //
			...
			if($lb_valido)
			{
                ...
                uf_print_recomendaciones($ls_recomendacion,&$io_pdf);
