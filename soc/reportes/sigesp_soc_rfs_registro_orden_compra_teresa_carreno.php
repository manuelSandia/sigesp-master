...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
										...
										$ad_monantpag,$as_estcom,&$io_pdf)
	{
		...
		$io_pdf->rectangle(106,726,480,36);
		$io_pdf->line(380,726,380,761);
		$io_pdf->line(380,744,586,744);
        $io_pdf->addJpegFromFile('../../shared/imagebank/sigesp.jpg',15,726,65,45); // Agregar Logo
		if ($as_estcom=='3')
		{
		...
		$tm=240-($li_tm/2);
		$io_pdf->addText($tm,740,12,"<b>".$ls_titulo."</b>"); // Agregar el título
		$io_pdf->addText(450,750,10,"<b>Fecha: </b> ".$ad_fecordcom); // Agregar el título
		$io_pdf->addText(450,730,10,"<b>No.: </b>".$as_numordcom); // Agregar el título
		$io_pdf->addText(554,770,7,date("d/m/Y")); // Agregar la Fecha	
        $io_pdf->ezSetY(715);
		...
		$la_data[1]  = array('columna1'=>'<b>Condición de Pago</b>','columna2'=>'<b>Forma de Pago</b>','columna3'=>'<b>Anticipo de Pago</b>');
		...
		$io_pdf->Rectangle(16,70,573,115); //primer campo mueve a la derecha,segundo campo baja el recuadro
		$io_pdf->line(17,175,589,175);		
		$io_pdf->line(17,138,589,138);		
		$io_pdf->line(130,139,130,184);		
		$io_pdf->line(270,139,270,184);		
		$io_pdf->line(448,139,448,184);		
		
        if( ($as_estcondat=="B")  || ($as_estcondat=="-") || ($as_estcondat=="") )
        {
			$io_pdf->addText(27,178,7,"SECCION DE COMPRAS"); // Agregar el título
		}
		else
		{
			$io_pdf->addText(27,178,7,"SECCION DE SERVICIOS"); // Agregar el título
		}

		$io_pdf->addText(80,83,6,""); // Agregar el título
		$io_pdf->addText(139,178,7,"UNIDAD DE BIENES Y SERVICIOS"); // Agregar el título
		$io_pdf->addText(200,83,6,""); // Agregar el título
		$io_pdf->addText(280,178,7,"GERENCIA CENTRAL DE ADM. Y FINANZAS"); // Agregar el título
		$io_pdf->addText(310,83,6,""); // Agregar el título
		$io_pdf->addText(454,178,7,"UNIDAD DE VERIFICACION Y CONTROL"); // Agregar el título
		$io_pdf->addText(450,83,6,""); // Agregar el título
			  
		$io_pdf->line(17,125,589,125);		
		$io_pdf->addText(225,128,7,"RECEPCION DE LA ORDEN POR EL PROVEEDOR"); // Agregar el título				
		$io_pdf->line(130,70,130,124);		
		$io_pdf->line(270,70,270,124);	
		$io_pdf->addText(22,117,7,"APELLIDOS Y NOMBRES"); // Agregar el título	
		$io_pdf->addText(145,117,7,"CEDULA DE IDENTIDAD"); // Agregar el título
		$io_pdf->addText(300,117,7,"FIRMA RECIBIDO CONFORME"); // Agregar el título
		$io_pdf->line(448,70,448,124);	
		$io_pdf->addText(508,117,7,"CARGO"); // Agregar el título
		$io_pdf->line(17,115,589,115);
		
		$io_pdf->Rectangle(16,28,573,40);
		$io_pdf->line(205,28,205,68);		
		$io_pdf->line(405,28,405,68);		
		
		$io_pdf->addText(20,60,6,"FIANZA DE FIEL CUMPLIMIENTO: AL APROBARSE ESTA "); // Agregar el título	
		$io_pdf->addText(20,53,6,"ORDEN SE EXIGIRA AL BENEFICIARIO, FIANZA DE FIEL "); // Agregar el título	
		$io_pdf->addText(20,46,6,"CUMPLIENTO EQUIVALENTE AL ______ % DEL MONTO "); // Agregar el título	
		$io_pdf->addText(20,39,6,"DE ESTA ORDEN, OTORGADAPOR UN BANCO O "); // Agregar el título	
		$io_pdf->addText(20,32,6,"COMPAÑIA DE SEGUROS, VIGENTE HASTA LA TOTAL"); // Agregar el título
		
		$io_pdf->addText(230,60,6,"CLAUSULA PENAL: QUEDA ESTABLECIDA LA CLAUSULA "); // Agregar el título	
		$io_pdf->addText(230,53,6,"PENAL,  SEGUN LA CUAL EL PROVEEDOR PAGARA "); // Agregar el título	
		$io_pdf->addText(230,46,6,"AL FISCO EL _______% SOBRE EL MONTO DE LA "); // Agregar el título	
		$io_pdf->addText(230,39,6,"MERCANCIA RESPECTIVA, POR CADA DIA HABIL DE "); // Agregar el título	
		$io_pdf->addText(230,32,6,"RETARDO EN LA ENTREGA."); // Agregar el título
		
		$io_pdf->addText(420,60,6,"CLAUSULA ESPECIAL: EL ORGANISMO SE RESERVA EL "); // Agregar el título	
		$io_pdf->addText(420,53,6,"DERECHO DE ANULAR UNILATERALMENTE LA "); // Agregar el título	
		$io_pdf->addText(420,46,6,"PRESENTE ORDEN DE COMPRA SIN INDEMNIZACION, "); // Agregar el título	
		$io_pdf->addText(420,39,6,"DE CONFORMIDAD CON LAS PREVISIONES LEGALES "); // Agregar el título	
		$io_pdf->addText(420,32,6,"QUE RIGEN LA MATERIA."); // Agregar el título
	...
	function uf_print_detalle_cuentas($la_data,&$io_pdf)
	{
		...
		$la_data2[1]=array('codestpro'=>'<b>Estructura Presupuestaria</b>','cuenta'=>'<b>Cuenta Presupuestaria</b>','denominacion'=>'<b>Denominación</b>','monto'=>'<b>Monto ('.$ls_bolivares.')</b>');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('codestpro'=>array('justification'=>'center','width'=>145), // Justificación y ancho de la columna
						 			   ...
						 			   'denominacion'=>array('justification'=>'center','width'=>225), // Justificación y ancho de la columna
	...
	function uf_print_piecabecera($as_estcondat,$as_lugcom,$as_codmoneda,$ad_tasa,$ad_mondiv,$as_pais,$as_estado,$as_municipio,$as_parroquia,$ld_montot,$ls_monlet,&$io_pdf)
	{
		...
		//------------------------------------------------  Datos   ----------------------------------------------------------
        /*/if( ($as_estcondat=="B")  || ($as_estcondat=="S")|| ($as_estcondat=="-") )
        {
...
function uf_print_detalle_sep($la_datasep,&$io_pdf)
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
		    ...
		    $io_pdf->ezSetCmMargins(7,7,1,1); // Configuración de los margenes en centímetros
			$io_pdf->ezStartPageNumbers(588,780,7,'','',1);
			if ($row=$io_sql->fetch_row($rs_data))
			{
				...
				uf_print_encabezado_pagina($ls_estcondat,$ls_numordcom,$ld_fecordcom,$ls_codpro,$ls_nompro,$ls_rifpro,$ls_dirpro,
										   ...
										   $ld_monseg,$ld_monantpag,$ls_estcom,&$io_pdf);
			   ...
			   if ($lb_valido)
			   {
				 ...
				 if ($li_totrows>0)
				 {
					...
					for ($i=$li_i+1;$i<=50;$i++)
					{
						$la_data[$i]  = array('codigo'=>$ls_codartser,
						                         'denominacion'=>$ls_denartser,
											     'cantidad'=>number_format($li_cantartser,2,',','.'),
											     'precio'=>$ld_preartser,
											     'unidad'=>$ls_unidad,
