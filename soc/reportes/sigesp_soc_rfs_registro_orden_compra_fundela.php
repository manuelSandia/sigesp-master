...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
									   ...
									   $ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$as_estcom,&$io_pdf)
	{
		...
		$io_pdf->addText(35,122,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText(35,63,7,"FIRMA / FECHA"); // Agregar el título

		$io_pdf->line(120,60,120,130);	//VERTICAL	
		$io_pdf->addText(143,122,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(140,63,7,"JEFE DE COMPRAS"); // Agregar el título

		$io_pdf->line(230,60,230,130);	//VERTICAL
		$io_pdf->addText(254,122,7,"REVISADO POR"); // Agregar el título
		$io_pdf->addText(239,63,7,"UNIDAD DE PRESUPUESTO"); // Agregar el título

		$io_pdf->line(340,60,340,130);	//VERTICAL		
		$io_pdf->addText(375,122,7,"VERIFICADO POR"); // Agregar el título
		$io_pdf->addText(379,63,7,"PRESIDENCIA"); // Agregar el título

		$io_pdf->line(470,60,470,130);	//VERTICAL		
		$io_pdf->addText(495,122,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(495,63,7,"ADMINISTRACION"); // Agregar el título
		$io_pdf->line(15,73,585,73); //HORIZONTAL		
		
/*        $io_pdf->Rectangle(15,60,570,70); 
		$io_pdf->line(15,117,585,117);	//HORIZONTAL	
		$io_pdf->addText(40,122,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText(46,63,7,"FIRMA / FECHA"); // Agregar el título
		$io_pdf->line(130,60,130,130);	//VERTICAL	
		$io_pdf->addText(167,122,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(160,63,7,"JEFE DE COMPRAS"); // Agregar el título
		$io_pdf->line(260,60,260,130);	//VERTICAL		
		$io_pdf->addText(300,122,7,"VERIFICADO POR"); // Agregar el título
		$io_pdf->addText(305,63,7,"PRESIDENCIA"); // Agregar el título
		$io_pdf->line(410,60,410,130);	//VERTICAL		
		$io_pdf->addText(465,122,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(465,63,7,"ADMINISTRACION"); // Agregar el título
		$io_pdf->line(15,73,585,73); //HORIZONTAL		
*/		
		$io_pdf->ezSetY(695);
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		unset($la_config);
		$io_pdf->ezSetDy(-2);
		$la_columnas=array('item'=>'<b>Item</b>',
						   'codigo'=>'<b>Código</b>',
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'maxWidth'=>570, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('item'=>array('justification'=>'center','width'=>25), // Justificación y ancho de la columna
						 			   ...
						 			   'unidad'=>array('justification'=>'center','width'=>40), // Justificación y ancho de la columna
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function obtenerItems($cod1,$cod2,$cod3,$cod4,$cod5,$arritems,$cuenta) {
		$descitems  = "";
		$primeritem = true;
		foreach ($arritems as $item) {
			if ($cod1==trim($item["codestpro1"]) && $cod2==trim($item["codestpro2"]) && $cod3==trim($item["codestpro3"]) && $cod4==trim($item["codestpro4"]) && $cod5==trim($item["codestpro5"]) && $cuenta==trim($item["cuenta"])) {
				if ($primeritem) {
					$descitems = $descitems.". Items asociados : {$item["item"]}";
				}
				else {
					$descitems = $descitems.", {$item["item"]} ";
				}
				$primeritem = false;
			}
		}
			   ...
			   if ($lb_valido)
			   {
				 ...
				 if ($li_totrows>0)
				 {
				    ...
				    while($row=$io_sql->fetch_row($rs_datos))
					{
						...
						$la_data[$li_i]=array('item'=>$li_i,'codigo'=>$ls_codartser,'denominacion'=>$ls_denartser,'cantidad'=>$li_cantartser,
											  'unidad'=>$ls_unidad,'cosuni'=>$ld_preartser,'baseimp'=>$ld_subtotartser,
											  'cargo'=>$ld_carartser,'montot'=>$ld_totartser);
						$la_estart[] = array('item'=>$li_i,'codestpro1'=>$row["codestpro1"],'codestpro2'=>$row["codestpro2"],'codestpro3'=>$row["codestpro3"],'codestpro4'=>$row["codestpro4"],'codestpro5'=>$row["codestpro5"],'cuenta'=>$row["spg_cuenta"]);
					}
					...
					if($lb_valido)
					{
						 ...
						 if ($li_totrows>0)
						 {
							...
							while($row=$io_sql->fetch_row($rs_datos_cuenta))
							{
								...
								$ls_estitems = obtenerItems($ls_codestpro1, $ls_codestpro2, $ls_codestpro3, $ls_codestpro4, $ls_codestpro5, $la_estart,$ls_spg_cuenta);
								
								$la_data[$li_s]=array('codestpro'=>$ls_codestpro,'denominacion'=>$ls_dencuenta.$ls_estitems,
