<?php
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_numsolcot,$as_fecsolcot,$as_dentipsolcot,$as_obssolcot,$as_nomsolic,$as_soltel,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->saveState();
		$io_pdf->rectangle(41,678,548,45);
		$io_pdf->line(450,678,450,722);
		$io_pdf->line(450,700,590,700);
		$io_pdf->addJpegFromFile('../../shared/imagebank/logo_inn.jpg',41,726,550,40); // Agregar Logo
		$li_tm=$io_pdf->getTextWidth(10,$as_titulo);
		$io_pdf->addText(200,695,14,"<b>".$as_titulo."</b>"); // Agregar el título
		$io_pdf->addText(460,705,10,"<b>   No.:</b>");      // Agregar texto
		$io_pdf->addText(495,705,10,$as_numsolcot); // Agregar Numero de la solicitud
		$io_pdf->addText(450,685,10,"<b>  Fecha:</b>"); // Agregar texto
		$io_pdf->addText(495,685,10,$as_fecsolcot); // Agregar la Fecha
		$io_pdf->addText(555,775,7,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(560,768,7,date("h:i a")); // Agregar la hora
		//print_r($as_soltel).'<br>';
		$io_pdf->ezSetY(677);
		$la_data=array(array('name'=>'<b>                  TIPO:  </b>'.$as_dentipsolcot),
					   array('name'=>'<b>OBSERVACIÓN: </b> '.$as_obssolcot),
					   array('name'=>'<b>SOLICITANTE: </b> '.$as_nomsolic),
					   array('name'=>'<b>TELEFONO: </b> '.$as_soltel),);
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('codigo'=>array('justification'=>'center','width'=>125),     // Justificación y ancho de la columna
									   ...
									   'unidad'=>array('justification'=>'left','width'=>100), 		// Justificación y ancho de la columna
		      ...
		      if ($li_numrows>0)
		         {
				   ...
				   while (($row=$io_sql->fetch_row($rs_data)) && $lb_valido)

						 {
								     ...
								     while($row=$io_sql->fetch_row($rs_datos))
								          {
									        ...
									        $la_datos[$li_i] = array('codigo'=>$ls_codigo,'denominacion'=>$ls_denite,'unidad'=>$ls_uniite,'cantidad'=>$ld_canite);
									      }
				    		       }
						        else
							       {
					       ...
					       uf_print_encabezado_pagina($ls_titulo,$ls_numsolcot,$ls_fecsolcot,$ls_tipo,$ls_obssolcot,$ls_solic,$ls_soltel,$io_pdf);
