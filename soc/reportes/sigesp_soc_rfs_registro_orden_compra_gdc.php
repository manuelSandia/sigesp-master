...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_encabezado_pagina($as_estcondat,$as_numordcom,$ad_fecordcom,$as_coduniadm,$as_denuniadm, $as_codfuefin,
	                                   $as_denfuefin,$as_codigo,$as_nombre,$as_conordcom,$as_rifpro,$as_telpro,$as_ocei_no_reg,$as_diaplacom,
					   $as_dirpro,$ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$as_lugentnomdep,$as_lugentdir,$as_concom,
						$as_uniaso,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->addText(185,720,6,"<b>DIRECCION FISCAL: AV. URDANETA FRENTE A LA PLAZA BOLIVAR, CASA DE</b>"); // Agregar el título
		$io_pdf->addText(245,712,6,"<b>GOBIERNO, URB. CATEDRAL</b>"); // Agregar el título
		$io_pdf->addText(285,702,6,"<b>G-20008816-8</b>"); // Agregar el título
		...
		$io_pdf->addText(25,63,7,"COMPRAS Y SUMINISTROS"); // Agregar el título
		$io_pdf->line(130,60,130,130);	//VERTICAL	
		$io_pdf->addText(167,122,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(160,63,7,"ADMINISTRACION"); // Agregar el título
		$io_pdf->line(260,60,260,130);	//VERTICAL		
		$io_pdf->addText(295,122,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(300,63,7,"DESPACHO"); // Agregar el título
		$io_pdf->line(410,60,410,130);	//VERTICAL		
		$io_pdf->addText(470,122,7,"PROVEEDOR"); // Agregar el título
		$io_pdf->addText(455,63,7,"FIRMA / SELLO / FECHA"); // Agregar el título
		$io_pdf->line(15,73,585,73); //HORIZONTAL		
		
		$io_pdf->ezSetY(695);
		$la_data[1]=array('columna1'=>'<b>Proveedor:</b>  '.$as_nombre.'<b>             Rif:</b> '.$as_rifpro.'  <b>Telf.:</b> '.$as_telpro.' <b>R.N.C.:</b> '.$as_ocei_no_reg,
		...
		$la_data[1]=array('columna1'=>'<b>Unidad Ejecutora:</b>    '.$ls_uniadm.' <b>U. Solicitante:</b> '.$as_lugentnomdep,'columna2'=>'<b>Forma de Pago:</b>    '.$ls_forpagcom);
		...
		$la_data[1]=array('columna1'=>'<b>Lugar de Entrega:</b> '.$as_lugentdir);
		...
		$la_data[1]=array('columna1'=>'<b>Condicion:</b>               '.$as_concom.'									<b>Unidades Asociadas:</b>'.$as_uniaso);
		$la_columnas=array('columna1'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_piecabecera($li_subtot,$li_totcar,$li_montot,$ls_monlet,$ls_obsordcom,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>540, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('titulo'=>array('justification'=>'right','width'=>450),
						 'contenido'=>array('justification'=>'right','width'=>120))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
		unset($la_data);
		unset($la_columnas);
		...
		$la_data[1]=array('columna1'=>'<b>Observaciones:</b>         '.$ls_obsordcom);
		$la_columnas=array('columna1'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
		...
		$la_data[1]=array('columna1'=>'<b>25. FIANZA DE FIEL CUMPLIMIENTO</b> AL APROBARSE ESTA ORDEN SE EXIGIRÁ AL BENEFICIARIO FIANZA DE FIEL CUMPLIMIENTO EQUIVALENTE AL ___% DEL MONTO DE LA ORDEN OTORGADA POR UN BANCO O UNA COMPAÑÍA DE SEGUROS, NOTARIADA, VIGENTE HASTA LA TOTAL RECEPCIÓN DE LA MERCANCÍA.',
				  'columna2'=>'<b>26. FIANZA DE ANTICIPO</b> AL APROBARSE ESTA ORDEN SE EXIGIRÁ AL BENEFICIARIO FIANZA DE ANTICIPO EQUIVALENTE AL ___% DEL MONTO DE LA ORDEN.',
				  'columna3'=>'<b>27. CLÁUSULA ESPECIAL</b> EL ORGANISMO SE RESERVA EL DERECHO DE ANULAR UNILATERALMENTE LA PRESENTE ORDEN DE INDEMNIZACIÓN DE CONFORMIDAD CON LO DISPUESTO EN REGLAMENTO QUE RIGE LA MATERIA.',
				  'columna4'=>'<b>28. CLÁUSULA PENAL</b> QUEDA ESTABLECIDA LA CLÁUSULA PENAL SOBRE LA CUAL EL PROVEEDOR PAGARÁ EL ___% SOBRE EL MONTO DE LA MERCANCÍA RESPECTIVA POR CADA DÍA HÁBIL DE RETARDO EN LA ENTREGA.');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>570, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('columna1'=>array('justification'=>'left','width'=>142.5),
							       'columna2'=>array('justification'=>'left','width'=>142.5),
							       'columna3'=>array('justification'=>'left','width'=>142.5),
								'columna3'=>array('justification'=>'left','width'=>142.5))); 
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			if ($row=$io_sql->fetch_row($rs_data))
			{
				...
				$ls_ocei_no_reg=$row["ocei_no_reg"];
				$ls_concom=$row["concom"];
				$ls_uniaso=$row["concom"];
				...
				uf_print_encabezado_pagina($ls_estcondat,$ls_numordcom,$ld_fecordcom,$ls_coduniadm,$ls_denuniadm,
				                $ls_codfuefin,$ls_denfuefin,$ls_codpro,$ls_nompro,$ls_obscom,$ls_rifpro,$ls_telpro,
						$ls_ocei_no_reg, $ls_diaplacom,$ls_dirpro,$ls_forpagcom,$ld_perentdesde,$ld_perenthasta,
						$ls_lugentnomdep,$ls_lugentdir,$ls_concom,$ls_uniaso,&$io_pdf);
		...
		uf_print_piecabecera($ld_monsubtot,$ld_monimp,$ld_montot,$ls_monto,$ls_obsordcom,&$io_pdf);
	} 	  	 
