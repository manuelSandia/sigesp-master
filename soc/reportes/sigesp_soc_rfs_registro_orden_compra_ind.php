...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
									   ...
									   $ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$as_estcom,&$io_pdf)
	{
		...
		$io_pdf->line(195,145,195,60);	
		$io_pdf->line(390,145,390,60);	
		$io_pdf->addText(65,138,6,"T.S.U. JOSEFINA RODRÍGUEZ"); // Agregar el título
		$io_pdf->addText(250,138,6,"LIC. FRANCISCO MORONTA"); // Agregar el título
		$io_pdf->addText(450,138,6,"LIC. HECTOR GOLINDANO "); // Agregar el título
		$io_pdf->addText(45,63,5,"COORDINACIÓN Y ADQUISICIÓN DE SUMINISTROS"); // Agregar el título
		$io_pdf->addText(225,63,5,"DIRECTOR DE BIENES Y SERVICIOS ADMINISTRATIVOS"); // Agregar el título
		$io_pdf->addText(435,63,5,"OFICINA DE ADMINISTRACIÓN Y FINANZAS"); // Agregar el título
		$io_pdf->ezSetY(695);
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_piecabecera($li_subtot,$li_totcar,$li_montot,$ls_monlet,$li_totexe,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		unset($la_columnas);
		unset($la_config);
		$la_data[1]=array('titulo'=>'<b>Total Exento </b>','contenido'=>$li_totexe,);
		$la_columnas=array('titulo'=>'',
						   'contenido'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
	...
	else  // Imprimimos el reporte
	{
			   ...
			   if ($lb_valido)
			   {
				 ...
				 if ($li_totrows>0)
				 {
				    ...
				    while($row=$io_sql->fetch_row($rs_datos))
					{
						...
						if($ld_carartser==0)
						{
							$li_totexe=$li_totexe+$ld_subtotartser;
						}
					...
					if($lb_valido)
					{
						 ...
						 if ($li_totrows>0)
						 {
							...
							while($row=$io_sql->fetch_row($rs_datos_cuenta))
							{
								...
								$io_fun_soc->uf_formatoprogramatica($ls_codpro,&$ls_programatica);
								/*if($ls_estmodest==1)
								{
								...
								$la_data[$li_s]=array('codestpro'=>$ls_programatica,'denominacion'=>$ls_dencuenta,
		...
		$li_totexe=number_format($li_totexe,2,",",".");
		uf_print_piecabecera($ld_monsubtot,$ld_monimp,$ld_montot,$ls_monto,$li_totexe,&$io_pdf);
	} 	  	 
