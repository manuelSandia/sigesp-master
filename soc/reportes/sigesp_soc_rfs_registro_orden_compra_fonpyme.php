...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_encabezado_pagina($as_estcondat,$as_numordcom,$ad_fecordcom,$as_coduniadm,$as_denuniadm, $as_codfuefin,
									   ...
									   $ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$ls_opcion,$as_estcom,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$li_tm=$io_pdf->getTextWidth(7,"Avenida Fransisco de Miranda Torre Europa, Piso 7, El Rosal, Municipio Chacao,Estado Miranda, Caracas, Venezuela");
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,68,7,"Avenida Fransisco de Miranda Torre Europa, Piso 7, El Rosal, Municipio Chacao,Estado Miranda, Caracas, Venezuela"); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(7,"Telfs:(0212)954.0011-0710-0968 correo:compras@fonpyme.gob.ve web:www.fonpyme.gob.ve");
		$tm=296-($li_tm/2);
		$io_pdf->addText($tm,60,7,"Telfs:(0212)954.0011-0710-0968 correo:compras@fonpyme.gob.ve web:www.fonpyme.gob.ve"); // Agregar el título
		...
		// cuadro inferior
        $io_pdf->Rectangle(15,75,570,110); 
		$io_pdf->line(15,172,585,172);	//HORIZONTAL	
		$io_pdf->addText(278,175,10,"FIRMAS"); // Agregar el título
		$io_pdf->line(15,160,585,160);	//HORIZONTAL	
		$io_pdf->addText(80,163,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->line(205,75,205,172);	//VERTICAL	
		$io_pdf->addText(245,163,7,"IMPUTACIÓN PRESUPUESTARIA"); // Agregar el título
		$io_pdf->line(395,75,395,172);	//VERTICAL	
		$io_pdf->addText(460,163,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(73,100,7,"JOSÉ MANUEL BALZA"); // Agregar el título
		$io_pdf->addText(100,93,6,"Firma"); // Agregar el título
		$io_pdf->addText(60,80,7,"ESPECIALISTA EN COMPRAS"); // Agregar el título
		$io_pdf->addText(255,100,7,"MUSSOULINE CEVALLOS"); // Agregar el título
		$io_pdf->addText(290,93,6,"Firma"); // Agregar el título
		$io_pdf->addText(228,80,7,"PLANIFICACION Y CONTROL DE GESTION"); // Agregar el título
		$io_pdf->line(15,90,585,90);	//HORIZONTAL	
		switch($ls_opcion)
		{
			case "1":
				$io_pdf->addText(458,100,7,"MARYORIS LURÚA"); // Agregar el título
				$io_pdf->addText(480,93,6,"Firma"); // Agregar el título
				$io_pdf->addText(415,80,7,"GERENTE DE ADMINISTRACION Y FINANZAS"); // Agregar el título
			break;
			
			case "2":
				$io_pdf->addText(458,100,7,"ANGEL SCARANO"); // Agregar el título
				$io_pdf->addText(480,93,6,"Firma"); // Agregar el título
				$io_pdf->addText(453,80,7,"GERENTE GENERAL"); // Agregar el título
			break;
			
			case "3":
				$io_pdf->addText(458,100,7,"ALFREDO RIERA"); // Agregar el título
				$io_pdf->addText(480,93,6,"Firma"); // Agregar el título
				$io_pdf->addText(464,80,7,"PRESIDENTE"); // Agregar el título
			break;

			case "4":
				$io_pdf->addText(458,100,7,"JUNTA DIRECTIVA"); // Agregar el título
				$io_pdf->addText(480,93,6,"Firma"); // Agregar el título
				$io_pdf->addText(458,80,7,"JUNTA DIRECTIVA"); // Agregar el título
			break;
		}		
		$io_pdf->ezSetY(700);
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			$io_pdf->ezSetCmMargins(6.7,6.5,3,3); // Configuración de los margenes en centímetros
			...
			if ($row=$io_sql->fetch_row($rs_data))
			{
				...
				$ld_valunitri=$row["valunitri"];
				$ls_opcion="";
				if(number_format(($ld_valunitri*450),2,".","")>=number_format($ld_montot,2,".",""))
				{
					$ls_opcion="1";
				}
				elseif(number_format(($ld_valunitri*1099),2,".","")>=number_format($ld_montot,2,".",""))
				{
					$ls_opcion="2";
				}
				elseif(number_format(($ld_valunitri*2500),2,".","")>=number_format($ld_montot,2,".",""))
				{
					$ls_opcion="3";
				}
				else
				...
				uf_print_encabezado_pagina($ls_estcondat,$ls_numordcom,$ld_fecordcom,$ls_coduniadm,$ls_denuniadm,
										   ...
										   $ls_diaplacom,$ls_dirpro,$ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$ls_opcion,$ls_estcom,&$io_pdf);
