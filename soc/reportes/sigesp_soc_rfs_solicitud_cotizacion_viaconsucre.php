<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsolcot,$as_fecsolcot,$as_dentipsolcot,$as_obssolcot,&$io_pdf)
	{
		...
		$io_pdf->addText(200,670,14,"<b>".$as_titulo."</b>"); // Agregar el título
		$io_pdf->addText(470,680,9,"<b>   No.:</b>");      // Agregar texto
		$io_pdf->addText(505,680,9,$as_numsolcot); // Agregar Numero de la solicitud
		$io_pdf->addText(460,665,9,"<b>  Fecha:</b>"); // Agregar texto
		$io_pdf->addText(505,665,9,$as_fecsolcot); // Agregar la Fecha
		$io_pdf->addText(543,708,7,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(548,700,7,date("h:i a")); // Agregar la hora
		$io_pdf->addText(40,645,7,"<b>".$ls_nomemp."</b>"); // Agregar el título
		$io_pdf->addText(40,635,7,"<b>".$ls_rifemp."</b>"); // Agregar el título
		$io_pdf->addText(40,625,7,"<b>".$ls_diremp."</b>"); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$_SESSION["la_nomusu"]." ".$_SESSION["la_apeusu"]);
		$tm=450-($li_tm/2);
		//$io_pdf->addText($tm,730,14,$ls_titulo); // Agregar el título
		// cuadro inferior
		$io_pdf->Rectangle(40,60,550,90); 
		$io_pdf->addText(45,138,9,"NOTA: Toda Cotización debe llevar:"); // Agregar el título
		$io_pdf->addText(45,126,9,".Condiciones de Pago"); // Agregar el título
		$io_pdf->addText(45,114,9,".Tiempo de Entrega"); // Agregar el título
		$io_pdf->addText(45,102,9,".Garantia"); // Agregar el título
		$io_pdf->addText(45,90,9,".Si incluye IVA"); // Agregar el título
		$io_pdf->addText(45,78,9,".Validez de la Oferta"); // Agregar el título
		$io_pdf->addText($tm,135,9,$_SESSION["la_nomusu"]." ".$_SESSION["la_apeusu"]);
		$io_pdf->addText(220,63,9,"Enviar Cotización al FAX No. (0212) 576-84-19"); // Agregar el título
		$io_pdf->line(315,73,315,150);	//VERTICAL		
	...
	function uf_print_datos_proveedor($as_codpro,$as_nompro,$as_dirpro,$as_telpro,$as_email,$as_rifpro,&$io_pdf)
	{
		...
		$la_data=array(array('name'=>'<b>Nombre o Razón Social: </b>'.$as_codpro.'  -  '.$as_nompro),
 		               array('name'=>'<b>RIF: </b>'.$as_rifpro),
 		               array('name'=>'<b>Dirección: </b>'.$as_dirpro),
					   array('name'=>'<b>Teléfono: </b> '.$as_telpro.'  -                                                 <b>E-Mail</b>: '.$as_email));				
