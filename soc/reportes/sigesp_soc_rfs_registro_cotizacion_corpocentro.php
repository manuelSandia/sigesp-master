<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numcot,$as_feccot,$as_dentipcot,$as_obscot,$as_numsolcot,&$io_pdf)
	{
		...
		$io_pdf->addText(500,740,10,$as_numcot); // Agregar Numero de la solicitud
		$io_pdf->addText(465,710,10,"<b>  Fecha:</b>"); // Agregar texto
		$io_pdf->addText(510,710,10,$as_feccot); // Agregar la Fecha
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_datos_proveedor($as_codpro,$as_nompro,$as_dirpro,$as_telpro,$as_email,$as_rifpro,$ls_formpag,$ls_diaent,$ls_diavalofe,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_data=array(array('name'=>'<b>Forma de Pago: </b>'.$ls_formpag.'          '),
 		               array('name'=>'<b>Tiempo de Entrega (Dias):  </b>'.$ls_diaent),
                       array('name'=>'<b>Validéz de la Oferta (Dias):  </b>'.$ls_diavalofe));	
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('codite'=>array('justification'=>'center','width'=>95),      // Justificación y ancho de la columna
						 			   'denite'=>array('justification'=>'left','width'=>153), // Justificación y ancho de la columna
						 			   'denunimed'=>array('justification'=>'center','width'=>50), // Justificación y ancho de la columna
						 			   'canite'=>array('justification'=>'right','width'=>40),
	          ...
	          if ($row=$io_sql->fetch_row($rs_data))
		         {
				   ...
				   $ls_formpag   = $row["forpagcom"];
				   $ls_diaent    = $row["diaentcom"];
						     ...
						     while ($row=$io_sql->fetch_row($rs_datos))
								   {
									 ...
									 $ld_totite 	= number_format($row["totite"],2,',','.');
								 	 ...
								 	 $la_datos[$li_i] = array('codite'=>$ls_codite,'denite'=>$ls_denite,'denunimed'=>$ls_denunimed,'canite'=>$ld_canite,'preite'=>$ld_preite,'subite'=>$ld_subite,'totite'=>$ld_totite);
								   }
							 uf_print_encabezado_pagina($ls_titulo,$ls_numcot,$ls_feccot,$ls_tipo,$ls_obscot,$ls_numsolcot,$io_pdf);
							 uf_print_datos_proveedor($ls_codpro,$ls_nompro,$ls_dirpro,$ls_telpro,$ls_mailpro,$ls_rifpro,$ls_formpag,$ls_diaent,$ls_diavalofe,$io_pdf);
