...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_encabezado_pagina($as_estcondat,$as_numordcom,$ad_fecordcom,$as_coduniadm,$as_denuniadm, $as_codfuefin,
									   ...
									   $ls_forpagcom,$as_nomusuario,$as_fechentdesde,$as_fechenthasta,$as_estcom,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->addText(32.5,121,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText(20,74,6,$as_nomusuario); // Agregar el título
		$io_pdf->addText(42.5,64,7,"COMPRAS"); // Agregar el título

		$io_pdf->line(110,60,110,130);	//VERTICAL
		$io_pdf->addText(135,121,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(115,74,6,"MT/3 (Ej) ORLANDO A. JURADO M."); // Agregar el título
		$io_pdf->addText(130,64,7,"DPTO. ADQUISICION"); // Agregar el título

		$io_pdf->line(460,60,460,130);	//VERTICAL
        $io_pdf->addText(260,121,7,"CONFORME"); // Agregar el título
		$io_pdf->addText(230,64,7,"EJECUCION DE PRESUPUESTO"); // Agregar el título

		$io_pdf->line(215,60,215,130);	//VERTICAL
		$io_pdf->addText(375,121,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(365,64,7,"GERENTE DE FINANZAS"); // Agregar el título
		$io_pdf->addText(365,74,6,"CNEL. MARCO A. ROJAS T."); // Agregar el título

		$io_pdf->line(350,60,350,130);	//VERTICAL
		$io_pdf->addText(500,121,7,"PROVEEDOR"); // Agregar el título
		$io_pdf->addText(482.5,64,7,"FIRMA / SELLO / FECHA"); // Agregar el título
		...
		if($as_estcondat=="S")
        {
			$la_data[1]=array('columna1'=>'<b>Periodo de Entrega  DESDE</b>   '.$as_fechentdesde,'columna2'=>'<b> HASTA</b>    '.$as_fechenthasta);
			$la_columnas=array('columna1'=>'','columna2'=>'');
			$la_config=array('showHeadings'=>0, // Mostrar encabezados
							 'fontSize' => 9, // Tamaño de Letras
							 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
		...
		$la_data[1]=array('columna1'=>'<b>Concepto</b>         '.str_pad(substr($as_conordcom,0,1000),1000,' '));
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle($li_z,$la_data,$as_conordcom,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// Fecha Creación: 21/06/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $ls_estcondat, $ls_bolivares;
		if($ls_estcondat=='B')
		{
			$ls_titulo_grid="Bienes";
		}
		...
		$io_pdf->ezSetY($io_pdf->y-30);
		$la_datatitulo[1]=array('titulo'=>'<b> Detalle de '.$ls_titulo_grid.'</b>');
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle_cuentas($li_z,$la_data,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_clausulas($la_data,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->ezSetDy(-5,'makeSpace');
		$la_datatitulo[1]=array('titulo'=>'<b> CLAUSULAS:</b>');
		$la_columnas=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
		...
		unset($la_columnas);
		unset($la_config);
		//$io_pdf->ezSetDy(-20,'makeSpace');
		$la_columnas=array('clausula'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>570, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('clausula'=>array('justification'=>'left','width'=>570))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
	}// end function uf_print_clausulas
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)
		{
			...
			//$io_pdf->ezSetCmMargins(7.5,4,3,3); // Configuración de los margenes en centímetros
			$io_pdf->ezStartPageNumbers(570,47,8,'','','');
			for ($li_z=1;$li_z<=4;$li_z++)	//for para las copias de la orden
			{
				...
				if ($row=$io_sql->fetch_row($rs_data))
				{
					...
					$ls_fechentdesde=$row["fechentdesde"];
					$ls_fechenthasta=$row["fechenthasta"];
					$ls_codtipmod=$row["codtipmod"];
					$ls_estcom=$row["estcom"];
					$lb_valido=$io_report->uf_load_modalidades_clausulas($ls_codtipmod,&$la_clausulas);
					if($ls_tiporeporte==0)
					{
					...
					$ls_fechentdesde=$io_funciones->uf_convertirfecmostrar($ls_fechentdesde);
					$ls_fechenthasta=$io_funciones->uf_convertirfecmostrar($ls_fechenthasta);
					uf_print_encabezado_pagina($ls_estcondat,$ls_numordcom,$ld_fecordcom,$ls_coduniadm,$ls_denuniadm,
											   ...
											   $ls_diaplacom,$ls_dirpro,$ls_forpagcom,$ls_nomusuario,$ls_fechentdesde,$ls_fechenthasta,$ls_estcom,
											   &$io_pdf);
				   ...
				   if ($lb_valido)
				   {
					 ...
					 if ($li_totrows>0)
					 {
					    ...
					    while($row=$io_sql->fetch_row($rs_datos))
						{
							...
							if(($ld_carartser<0)&&($ld_carartser>-0.01))
							{
								$ld_carartser=abs($ld_carartser);
							}
							...
							$la_data2[$li_i]=array('codigo'=>$ls_codartser,'denominacion'=>$ls_denartser,'cantidad'=>$li_cantartser,
												  'unidad'=>$ls_unidad,'cosuni'=>$ld_preartser,'baseimp'=>$ld_subtotartser,
												  'cargo'=>$ld_carartser,'montot'=>$ld_totartser);
						}

						uf_print_detalle($li_z,$la_data2,$ls_obscom,&$io_pdf);
						...
						if($lb_valido)
						{
							 ...
							 if ($li_totrows>0)
							 {
								...
								uf_print_detalle_cuentas($li_z,$la_data,&$io_pdf);
							}
					     }
				      }
			       }
				  ...
				  if(!empty($la_clausulas))
				  {
				  	$li_rowcla=count($la_clausulas);
					for($li_f=1;$li_f<=$li_rowcla;$li_f++)
					{
						$ls_clausula=$la_clausulas[$li_f];
						$la_data1[$li_f]=array('clausula'=>$ls_clausula);
					}
					uf_print_clausulas($la_data1,&$io_pdf);
				  }
		     	}
				if ($li_z<=4)
				{
					...
					{
						$io_pdf->setColor(1,0,0,0);
						$io_pdf->addText(250,45,12,"CONTABILIDAD");
						$io_pdf->setColor(0,0,0,0);
						$io_pdf->ezNewPage();
					}
					else
