...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
									   ...
									   $ls_forpagcom,$ld_perentdesde,$ld_perenthasta,$as_estcom,&$io_pdf)
	{
		...
		if($as_estcondat=="B")
        {
			...
			$io_pdf->addText(35,63,7,"UNIDAD DE CONTROL Y"); // Agregar el título
			$io_pdf->line(130,60,130,130);	//VERTICAL
			$io_pdf->addText(160,63,7,"UNIDAD DE COMPRAS"); // Agregar el título
			$io_pdf->line(260,60,260,130);	//VERTICAL
			$io_pdf->addText(295,122,7,"COMPRA AUTORIZADA"); // Agregar el título
			$io_pdf->addText(275,63,7,"SUB-DIRECTOR ADMINISTRATIVO"); // Agregar el título
			$io_pdf->line(410,60,410,130);	//VERTICAL
			$io_pdf->addText(450,122,7,"CONFORMIDAD RECEPCION"); // Agregar el título
			$io_pdf->addText(455,63,7,"CORDINADOR ALMACEN"); // Agregar el título
        ...
        else
        {
			 ...
			 $io_pdf->Rectangle(15,60,570,70);
			 $io_pdf->line(15,117,585,117);	//HORIZONTAL
			 $io_pdf->addText(22,122,7,"ELABORADO POR"); // Agregar el título
			 $io_pdf->line(90,60,90,130);	//VERTICAL
			 $io_pdf->addText(93,72,7,"UNIDAD DE COMPRAS"); // Agregar el título
			 $io_pdf->addText(93,63,7,"FECHA:"); // Agregar el título
			 $io_pdf->line(190,60,190,130);	//VERTICAL
			 $io_pdf->addText(295,122,7,"COMPRA AUTORIZADA"); // Agregar el título
			 $io_pdf->addText(195,72,7,"RECURSOS FINANCIEROS"); // Agregar el título
			 $io_pdf->addText(195,63,7,"FECHA:"); // Agregar el título
			 $io_pdf->addText(325,72,7,"COORD. ADMINISTRATIVA"); // Agregar el título
			 $io_pdf->addText(325,63,7,"FECHA:"); // Agregar el título
			 $io_pdf->line(320,60,320,117);	//VERTICAL
			 $io_pdf->line(450,60,450,130);	//VERTICAL
			 $io_pdf->addText(470,122,7,"CONFORMIDAD SERVICIOS"); // Agregar el título
			 $io_pdf->addText(460,72,7,"UNIDAD SOLICITANTE"); // Agregar el título
			 $io_pdf->addText(460,63,7,"FECHA:"); // Agregar el título
			 $io_pdf->line(15,79,585,79); //HORIZONTAL
        }
		...
		$io_pdf->ezSetY(695);
		$la_data[1]=array('columna1'=>'<b>Proveedor</b>  '.$as_codigo.'  '.$as_nombre.'<b>             Rif</b> '.$as_rifpro,
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_clausulas($as_estcondat,&$io_pdf)//carlos zambrano
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_piecabecera
		//		    Acess: private
        ...
        {
		$io_pdf->ezSetDy(-190);
		$la_data[1]=array('titulo'=>'* La facturación por venta de Bienes por venta de bienes y/o prestacion de servicios debe cumplir con la PROVIDENCIA 0591 del Sistema Nacional de Administración Aduanera y Tributaria (SENIAT) de fecha de agosto de 2007, la cual establece las normas generales de emisión de facturas y otros documentos.');
		$la_data[2]=array('titulo'=>'* Factura (original), indicando en ella:');
		$la_data[3]=array('titulo'=>'- Descripción del bien:');
		$la_data[4]=array('titulo'=>'  Marca, Numero de Serial, Tiempo de Garantia (cuando aplique). En caso de que no tenga garantia, la factura debe señalar "Sin Garantia."');
		$la_data[5]=array('titulo'=>'- La Fecha de la factura debe ser posterior o igual a la fecha indicada en la Orden de Compra y/o Servicio.');
    	$la_data[6]=array('titulo'=>'- Factura a nombre del Instituto Universitario Experimental de Tecnologia "Andrés Eloy Blanco". (IUETAEB), RIF: G-20000114-3.');
   		$la_data[7]=array('titulo'=>'* Las entregas parciales de mercancias requieren Notas de Entrega original, las cuales deben estar firmadas al reverso con letra legible por la persona que recibe, para evitar confusiones se sugiere consignar mercancias y Notas de Entrega original (cuando aplique) y factura original por parte del proveedor ante la Unidad de Almacén.');
		$la_data[8]=array('titulo'=>'* En caso de renunciar a la totalidad o parte de los renglones indicados en la Orden de Compra y/o Servicio, debe consignar ante la Unidad de Almacén Carta de Renuncia original.');
		$la_data[9]=array('titulo'=>'* El proceso para el pago de la Orden de Compra y/o Servicio emitida por la Institución, se inicia una vez efectuada la entrega total de los articulos, Carta de Renuncia original (cuando aplique) y factura original por parte del proveedor ante la Unidad de Almacén');
		$la_data[10]=array('titulo'=>'* La Orden de Compra y/o Servicio original debe ser retirada por la Unidad de Compras y presentada para retirar su pago ante la Unidad de Caja');
		$la_data[11]=array('titulo'=>'* Una vez recibida la Orden de Compra y/o Servicio via fax dispone de 5 dias habiles, para la entrega total y/o parcial de lo descrito en la Orden.');
		$la_data[12]=array('titulo'=>'* Transcurrido los 5 dias hàbiles la Institución procederá a cancelar la Orden de Compra y/o Servicio por el monto de los artículos suministrados debiendo elaborar factura ajustada a la entrega.');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('titulo'=>array('justification'=>'full','width'=>560))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
		unset($la_data);
		unset($la_columnas);
		unset($la_config);
		...
		{
			$io_pdf->ezSetDy(-210);
			$la_data[1]=array('titulo'=>'* Factura (original), indicando en ella:');
			$la_data[2]=array('titulo'=>'- Descripción del bien:');
			$la_data[3]=array('titulo'=>' Marca, Numero de Serial, Tiempo de Garantia (cuando aplique). En caso de que no tenga garantia, la factura debe señalar "Sin Garantia"');
			$la_data[4]=array('titulo'=>'- La fecha de la factura debe ser posterior o igual a la fecha indicada en la Orden de Compra y/o Servicio');
			$la_data[5]=array('titulo'=>'- Factura a nombre del Instituto Universitario Experimental de Tecnologia "Andrés Eloy Blanco". (IUETAEB), RIF: G-20000114-3.');
			$la_data[6]=array('titulo'=>'* Las entregas parciales de mercancias requieren Notas de Entrega original, las cuales deben estar firmadas al reverso con letra legible por
			la persona que recibe, para evitar confusiones se sugiere consignar mercancias y Notas de Entrega original (cuando aplique) y factura original
			por parte del proveedor ante la Unidad de Almacén.');
			$la_data[8]=array('titulo'=>'* En caso de renunciar a la totalidad o parte de los renglones indicados en la Orden de Compra y/o Servicio, debe consignar ante la Unidad de
			Almacén Carta de Renuncia original.');
			$la_data[9]=array('titulo'=>'* El proceso para el pago de la Orden de Compra y/o Servicio emitida por la Institución, se inicia una vez efectuada la entrega total de los articulos,
			Carta de Renuncia original (cuando aplique) y factura original por parte del proveedor ante la Unidad de Almacén');
			...
			$la_config=array('showHeadings'=>0, // Mostrar encabezados
							 ...
							 'cols'=>array('titulo'=>array('justification'=>'full','width'=>590))); // Justificación y ancho de la columna
			$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
			unset($la_data);
			unset($la_columnas);
			unset($la_config);
	...
	else  // Imprimimos el reporte
	{
		...
		uf_print_clausulas($ls_estcondat,&$io_pdf);
	}
