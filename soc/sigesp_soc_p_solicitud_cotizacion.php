...
$ls_reporte = $io_fun_compra->uf_select_config("SOC","REPORTE","FORMATO_SOLCOT","sigesp_soc_rfs_solicitud_cotizacion_cnti.php","C");
...
switch ($li_diasem){
  ...
  break;
   ...
   function uf_limpiarvariables()
   {
		...
		//	     Function: uf_limpiarvariables
		//		   Access: private
		//	  Description: Funci_n que limpia todas las variables necesarias en la p_gina
		//	   Creado Por: Ing. N_stor Falc_n.
		// Fecha Creaci_n: 11/04/2007			Fecha _ltima Modificaci_n : 22/08/08.
		//////////////////////////////////////////////////////////////////////////////
		...
		global $ls_codestpro3,$ls_codestpro4,$ls_codestpro5,$ls_existe,$ls_estsolcot,$ls_estatus,$li_estciespg,$li_estciespi,$ld_fecha,$ls_correocomprador,$ls_telcomprador;
		...
		$ld_fecha =  $ld_fecregsolcot;
		$ls_correocomprador   = $_SESSION["la_nota"];
		$ls_telcomprador = $_SESSION["la_telusu"];
		$ls_tipbiesol = "-";//Tipo de Bienes Incluidos en la Solicitud de Cotización Materiales y/o Suministros _ Activos.
   ...
   function uf_load_variables()
   {
		...
		//	     Function: uf_load_variables
		//		   Access: private
		//	  Description: Funci_n que carga todas las variables necesarias en la p_gina
		//	   Creado Por: Ing. N_stor Falc_n.
		// Fecha Creaci_n: 17/03/2007			Fecha _ltima Modificaci_n : 29/04/2007
		//////////////////////////////////////////////////////////////////////////////
		...
		global $ls_codcarper,$ls_fecregsolcot,$ls_chkact,$ls_distipmat,$ls_distipact,$li_estciespg,$li_estciespi,$ld_fecha,$ls_correocomprador,$ls_telcomprador;
   ...
   function uf_load_data(&$as_parametros)
   {
		...
		//	  Description: Funci_n que carga todas las variables necesarias en la p_gina
		//	   Creado Por: Ing. N_stor Falc_n.
		// Fecha Creaci_n: 06/05/2007								Fecha _ltima Modificaci_n : 06/05/2007
		//////////////////////////////////////////////////////////////////////////////
...
?>
    <table width="800" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
      ...
      <tr>
        <td height="13" style="text-align:right">Justificaci&oacute;n</td>
        <td height="13" colspan="3"><label>
          <textarea name="txtconsolcot" cols="122" id="txtconsolcot" <?php echo $ls_discmb; ?> onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmn_opqrstuvwxyz_____ '+'_!:;__#@/?_%&$*-,.+(){}[]='); "><?php echo $ls_consolcot ?></textarea>
        </label></td>
      </tr>
      <tr>
        <td height="13" style="text-align:right">Unidades Asociadas </td>
        <td height="13" colspan="3"><textarea readonly name="txtuniejeaso" cols="122" id="txtuniejeaso" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmn_opqrstuvwxyz_____ '+'_!:;__#@/?_%&$*-,.+(){}[]='); "><?php echo $ls_uniejeaso ?></textarea>        </td>
      </tr>
      <tr>
        <td height="22" style="text-align:right">Observaci&oacute;n</td>
        <td height="22" colspan="3"><label>
          <input name="txtobssolcot" type="text" id="txtobssolcot" style="text-align:left" tabindex="2" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmn_opqrstuvwxyz_____ '+'_!:;__#@/?_%&$*-,.+(){}[]='); " value="<?php echo $ls_obssolcot ?>" size="125" <?php echo $ls_discmb; ?>/>
        </label></td>
      </tr>
      <tr>
        <td height="22" style="text-align:right">N° Consulta de Precio</td>
        <td height="22" colspan="3"><label>
          <input name="txtnumconsulta" type="text" id="txtnumconsulta" style="text-align:left" tabindex="2" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmn_opqrstuvwxyz_____ '+'_!:;__#@/?_%&$*-,.+(){}[]='); " value="" size="30" />
        </label></td>
      </tr>
      <tr>
        <td height="22" style="text-align:right">Correo Comprador</td>
        <td height="22"><label>
          <input name="txtcorreosolcot" type="text" id="txtcorreosolcot" style="text-align:left" tabindex="2" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmn_opqrstuvwxyz_____ '+'_!:;__#@/?_%&$*-,.+(){}[]='); " value="<?php echo $ls_correocomprador ?>" size="30" />
        </label></td>
        <td height="22" style="text-align:right">Teléfono Comprador</td>
        <td height="22"><label>
          <input name="txttlfcomprador" type="text" id="txttlfcomprador" style="text-align:left" tabindex="2" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmn_opqrstuvwxyz_____ '+'_!:;__#@/?_%&$*-,.+(){}[]='); " value="<?php echo $ls_telcomprador ?>" size="30" />
        </label></td>
      </tr>
      <tr>
        <td height="22" style="text-align:right">Fecha final para cotizar</td>
        <td height="22" colspan="3"><label>
      <input name="txtfecha" type="text" id="txtfecha"  style="text-align:center" value="<?php echo $ld_fecregsolcot; ?>" size="15" maxlength="10" datepicker="true" onKeyDown="javascript:ue_formato_fecha(this,'/',patron,true,event);" onBlur="javascript: ue_validar_formatofecha(this);" >
      	</label></td>
      </tr>
	...
	if (ls_estsolcot=="P")
	...
	else
	   {
		 if (f.hidestciespg.value==1 || f.hidestciespi.value==1)
		    {
			  alert("Ya fu_ procesado el Cierre Presupuestario, No pueden efectuarse movimientos, Contacte al Administrador del Sistema !!!");
			} 
		 else
		    {
...
function ue_clear_grid()
{
	...
	ajax=objetoAjax();
	// Pagina donde est_n los m_todos para buscar y pintar los resultados
  ...
  if (uf_evaluate_cierre())
     {
	   ...
	   if (ls_estsolcot=="P")
	      ...
	      {
		    if (confirm("_Desea eliminar el Registro actual?"))
		       {
				 valido=true;
				 parametros="";
				 codigo="";
 	     ...
 	     if (ls_tipsolcot!='-')
	        {
	          if (ls_codunieje=="")
			     {
			       pagina="sigesp_soc_cat_sep.php?hidtipsolcot="+ls_tipsolcot+"&tipo=SC&fecsolcot="+ls_fecsolcot;
	          ...
	          else
			     {
				   if (li_totrowsep==0)
				      {
				        alert("Si desea procesar Solicitudes de Ejecuci_n Presupuestaria, No indique Unidad Ejecutora ya que estas vienen definidas desde las mismas !!!");					  
					  }
                   else
				      {
			            pagina="sigesp_soc_cat_sep.php?hidtipsolcot="+ls_tipsolcot+"&tipo=SC&fecsolcot="+ls_fecsolcot;
  ...
  if (uf_evaluate_cierre())
     {
	   ...
	   if (((lb_existe=="TRUE")&&(li_cambiar==1))||(lb_existe=="FALSE")&&(li_incluir==1))
			     ...
			     if (ls_tipsolcot!='-')
			        {
				      ...
				      if (lb_valido)
					     {
						   lb_valido = ue_validarcampo(ls_numsolcot,"El N_mero de Solicitud no puede estar vac_o !!!.",f.txtnumsolcot);
					     }
					  if (lb_valido)
						 {
						  lb_valido = ue_validarcampo(ls_consolcot,"El Concepto de la Solicitud no puede estar vac_o !!!.",f.txtconsolcot);
						 }
					  if (lb_valido)
						 {
						   lb_valido = ue_validarcampo(ls_fecregsolcot,"La Fecha no puede estar vac_a !!!.",f.txtfecregsolcot);
						 }
				      if (lb_valido)
					     {
						   lb_valido = ue_validarcampo(ls_cedpersol,"La C_dula del Personal solicitante no puede estar vac_a !!!.",f.txtcedper);
					     }
				      if (lb_valido)
					     {
						   lb_valido = ue_validarcampo(ls_codunieje,"La Unidad Ejecutora no puede estar vac_a !!!.",f.txtcodunieje);
					     }
				      if (lb_valido)
				         {
					       if (li_totrowsep>1)
						   ...
						   if (ls_tipsolcot=='B' && lb_valido)//Solicitud de Cotización de Tipo Bienes.
						      {
							    if (li_rowbienes>1)
							       ...
							       {
								     alert("La Solicitud de Cotización debe tener al menos un Material/Suministro _ Activo incorporado !!!");
								     lb_valido = false;
							       }
						      }
					       else
	      ...
	      {
 	        alert("No tiene permiso para realizar esta operaci_n !!!");
	      }	 
	 }
}
...
function ue_imprimir()
{
	...
	if(li_imprimir==1)
	{
		if(lb_existe=="TRUE")
		{
			...
			ls_fecsolcot = f.txtfecregsolcot.value;
			ls_tipsolcot = f.cmbtipsolcot.value;
			ls_correosolcot = f.txtcorreosolcot.value;
			ls_numconsulta = f.txtnumconsulta.value;
			ls_telefono = f.txttlfcomprador.value;
			ld_fecha = f.txtfecha.value;
			if (ls_tipsolcot!='-')
			   {
			     ls_formato   = f.formato.value;
			     window.open("reportes/"+ls_formato+"?numsolcot="+ls_numsolcot+"&tipsolcot="+ls_tipsolcot+"&fecsolcot="+ls_fecsolcot+"&correosolcot="+ls_correosolcot+"&fecha="+ld_fecha+"&numconsulta="+ls_numconsulta+"&telefono="+ls_telefono,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,left=0,top=0,location=no,resizable=yes");
			   }
		    else
			   {
			     alert("Debe seleccionar un Tipo de Solicitud v_lida (Bienes/Servicios) !!!");
			   }
		}
		else
		{
