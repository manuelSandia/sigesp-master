...
switch ($li_diasem){
  ...
  break;
   ...
   function uf_limpiarvariables()
   {
		...
		global $ld_poriva,$ls_operacion,$li_estciespg,$li_estciespi,$ls_disabled,$li_diavalofe,$ls_garanacot,$ls_si1,$ls_no1,$ls_si2,$ls_no2;
        ...
        $ls_estesp="";
        $ls_garanacot="";
       if (!array_key_exists("rdasitec",$_POST))
           {
                    $ls_si1 = '';
                    $ls_no1 = '';
           }
         if (!array_key_exists("rdespecificaciones",$_POST))
           {
                    $ls_si2 = '';
                    $ls_no2 = '';
           }    
   ...
   function uf_load_variables()
   {
		...
		global $ld_poriva,$li_estinciva,$li_estciespg,$li_estciespi,$li_diavalofe,$ls_estasitec,$ls_estesp,$ls_garanacot,$ls_si1,$ls_no1,$ls_si2,$ls_no2;
        ...
        $li_diavalofe       = $_POST["txtvalidezoferta"];
        ...
        if (array_key_exists("rdasitec",$_POST))
           {
               print $ls_estasitec;
                $ls_estasitec=$_POST["rdasitec"];                 
                if ($ls_estasitec=='1')
                {
                    $ls_si1 = 'checked';              
                }
                elseif($ls_estasitec=='0')
                {
                    $ls_no1 = 'checked';           
                }
           }
        ...
        if (array_key_exists("rdespecificaciones",$_POST))
           {
                $ls_estesp=$_POST["rdespecificaciones"];
        ...
        $ls_garanacot=$_POST["txtgarantias"];
...
switch($ls_operacion){
  case 'GUARDAR':
			...
			$lb_valido=$io_soc->uf_guardar($ls_existe,$ls_fecregcot,$ls_numcot,$ls_numsolcot,$ls_tipcot,$ls_obscot,$li_totrowbienes,$li_totrowservicios,&$ls_estcot,
					                       $ls_forpag,$ls_codpro,$li_diaent,$ld_poriva,$ld_subtotal,$ld_creditos,$ld_total,$li_estinciva,$la_seguridad,$li_diavalofe,$ls_estasitec,$ls_estesp,$ls_garanacot);
...
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
?>
    <input name="tipo" type="hidden" id="tipo" value="RC" />
    <table width="800" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
      ...
      <tr>                                                
        <td height="22" style="text-align:right">Asistencia T&eacute;cnica </td>
         <td>
          <input name="rdasitec" type="radio" class="sin-borde" value="1" <?php print $ls_si1; ?> >    
          S&iacute;
          <input name="rdasitec" type="radio" class="sin-borde" value="0" <?php print $ls_no1; ?> >
          No</td>
      </tr>

      <tr>
        <td height="22" style="text-align:right">Cumple con Especificaciones</td>
        <td>
          <input name="rdespecificaciones" id="rdespecificaciones" type="radio" class="sin-borde" value="1" <?php print $ls_si2; ?> >
          S&iacute;
          <input name="rdespecificaciones" id="rdespecificaciones" type="radio" class="sin-borde" value="0" <?php print $ls_no2; ?> >
          No</td>
      ...
      <tr>
        ...
        <td height="22" style="text-align:right">Garant&iacute;as </td>
        <td>
          <input name="txtgarantias" type="text" id="txtgarantias" value="<?php print $ls_garanacot; ?>" size="50">
        </td>  
      </tr>
      ...
      <tr>
        ...
        <td height="22"><label>
              <select name="cmbformapago"  style="width:110px" tabindex="4" id="cmbformapago" <?php echo $ls_disabled;?>>
                <option value="-">---seleccione---</option>
                ...
                <option value="A50E50"  <?php print $ls_a50e50 ?>>50% AL APROBAR 50% A LA ENTREGA</option>
                <option value="OTROS"   <?php print $ls_otros ?>>OTROS</option>
			  </select>            
        </label></td>
        <td height="22">Plazo de Entrega</td> 
        <td>  <input name="txtdiasentrega" type="text" id="txtdiasentrega" value="<?php print $li_diaent ?>" size="5" maxlength="3" style="text-align:right" tabindex="5" onKeyPress="return keyRestrict(event,'1234567890'); " <?php echo $ls_disabled; ?>/> 
        <strong>d&iacute;as. </strong></td>

        <td height="22" colspan="3" style="text-align:rigth">Valid&eacute;z de la oferta 
          <input name="txtvalidezoferta" type="text" id="txtvalidezoferta" value="<?php print $li_diavalofe ?>" size="4" maxlength="3" style="text-align:right" tabindex="6" onKeyPress="return keyRestrict(event,'1234567890'); " <?php echo $ls_disabled; ?>/> 
        <strong>d&iacute;as. </strong></td>  
								  ...
								  for (j=1;(j<li_rowbienes)&&(lb_valido);j++)
								      {
										...
										if (ld_totart<0)
									       {
										     alert("El Total del Bien/Material "+ls_codart+" - "+ls_denart+" no debe ser menor que Cero.")
										     lb_valido = false;
									       }
								      }
							    }
								  ...
								  for (j=1;(j<li_rowservicios)&&(lb_valido);j++)
								      {
										...
										if (ld_totser<0)
									       {
										     alert("El Total del Servicio "+ls_codser+" - "+ls_denser+" debe ser menor que Cero.")
										     lb_valido = false;
								 	       }
								      }
							    }
		 ...
		 if (ls_tipcot=="B")
		    {
			  ...
			  formulacargo = eval("f.formula"+fila+".value");
				   ...
				   if ((ld_canart>0)||(ld_preart>0))
				      {
					    ...
					    //if ((formulacargo!="")) Comentado por Ofimatica de Venezuela el 30-05-2011
						if ((formulacargo!="") && (ls_tipconpro=='O' || ls_tipconpro=='E')) 
						{
						    // ld_creart = (ld_subart*(ld_porcre/100));
							  formula	   = formulacargo.replace("$LD_MONTO",ld_subart);
							  cargo		   = eval(formula);
							  ld_creart	   = redondear(cargo,2);
						}
						else
						{
						     ld_creart = 0;
				   ...
				   if ((ld_canser>0)||(ld_preser>0))//Si la cantidad de artículos ó el precio es mayor que cero calculamos
				   {
					    ...
					    { 
						     ...
						     if ((formulacargo!="") && (ls_tipconpro=='O' || ls_tipconpro=='E')) 
							 {
								  ...
								  formula	   = formulacargo.replace("$LD_MONTO",ld_preser*ld_canser);
								  cargo		   = eval(formula);
								  ld_creser		   = redondear(cargo,2);
						     }
					         else
						     {
							      ld_creser = "0";
