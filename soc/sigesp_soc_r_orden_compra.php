...
$io_fun_compra->uf_load_seguridad("SOC","sigesp_soc_r_orden_compra.php",$ls_permisos,&$la_seguridad,$la_permisos);
    ...
    <table width="543" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
      <tr>
        <td height="22" colspan="6" class="titulo-ventana"><input name="operacion" type="hidden" id="operacion" value="<?php print $ls_operacion ?>" />
          ...
          <input name="selectivo" type="hidden" id="selectivo" /></td>
      </tr>
      <tr>
        <td height="13">&nbsp;</td>
        <td height="13">&nbsp;</td>
  ...
  <input name="esttip" type="radio" class="sin-borde" value="S" onClick="uf_deshabilitar()"/>
      ...
      <tr>
        <td height="13" colspan="6"><table width="490" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
          ...
          <tr>
            <td><input name="rdemi" type="checkbox" class="sin-borde" id="rdemi" value="1" />
              Emitida</td>
            <td><input name="rdpre" type="checkbox" class="sin-borde" id="rdpre" value="1" />
              Emitida (Aprobada) </td>
            <td><input name="rdcon" type="checkbox" class="sin-borde" id="rdcon" value="1" />
          ...
          <tr>
            ...
            <td><input name="rdfin" type="checkbox" class="sin-borde" id="rdfin" value="1" />
              <span class="Estilo2">Finalizada-Anulada Parcial</span></td>
          </tr>
          <tr>
          	<td colspan = "2"><input name="rdsdp" type="checkbox" class="sin-borde" id="rdsdp" value="1" />
              <span class="Estilo2">Sin Disponibilidad Presupuestaria</span>
          	</td>
          	<td>
          		&nbsp;
          	</td></tr>
      ...
      <tr>
        <td height="19" colspan="6" style="text-align:center">&nbsp;</td>
      </tr>
      <tr>
        <td height="24">&nbsp;</td>
        <td height="24">&nbsp;</td>
        ...
        <td height="24">&nbsp;</td>
        <td height="24">&nbsp;</td>
        <td height="24"><a href="javascript: ue_buscar();"><img src="../shared/imagebank/tools15/buscar.gif" width="15" height="15" class="sin-borde" />Buscar</a></td>
      </tr>
    </table>
  <p>&nbsp;</p> <div id="ordenes" align="center"></div>
  </form>
...
f = document.formulario;
	...
	function ue_buscar()
	{
		...
		if (f.rdpre.checked) //EMITIDA (APROBADA)
		{ 
			rdpre = 1;
		}
		else
		...
		if (f.rdsdp.checked) //  SIN DISPONIBILIDAD PRESUPUESTARIA
		{ 
			rdsdp = 1;
		}
		else
		...
		{
			if((ls_fecordcomdes!="")&&(ls_fecordcomhas!=""))
			{
				f.selectivo.value=1;
				...
				ajax.open("POST","class_folder/sigesp_soc_c_reportes_ajax.php",true);
				...
				ajax.send("txtnumordcomdes="+ls_numordcomdes+"&rdtipo="+ls_tipord
						 ...
						 +"&txtcodartdes="+ls_codartdes+"&txtcodarthas="+ls_codarthas
						 +"&rdemi="+rdemi+"&rdpre="+rdpre+"&rdcon="+rdcon+"&rdanu="+rdanu+"&rdinv="+rdinv+"&rdfin="+rdfin+"&rdsdp="+rdsdp
						 +"&txtcodserdes="+ls_codserdes+"&txtcodserhas="+ls_codserhas+"&proceso=ORDEN");
...
//--------------------------------------------------------
//	Funci_n que valida que un intervalo de tiempo sea valido
//--------------------------------------------------------
...
function ue_imprimir()
{
	...
	if(selectivo==1)
	{
		...
		for (i=1;i<=total;i++)
		{
			...
			{
				ls_numordcom=eval("f.txtnumsol"+i+".value");
				ls_tipo=eval("f.txttipo"+i+".value");
				if(ls_tipo=="Bienes")
				{
					tipo="B";
				}
				...
				if (ls_ordenes.length>0)
				{
					ls_ordenes=ls_ordenes+">>"+ls_numordcom+">"+tipo;
				}
				else
				{
					ls_ordenes=ls_numordcom+">"+tipo;
				}
			}
		}
		if(ls_ordenes=="")
		...
		{
			tiporeporte = "SELECTIVO";
			ls_formato  = f.formato.value;
			pantalla    = "reportes/"+ls_formato+"?ordenes="+ls_ordenes+"&tiporeporte="+tiporeporte+"";
...
function ue_openexcel()
{
	...
	if(selectivo==1)
	{
		...
		{
			tiporeporte = "SELECTIVO";
			ls_formato  = "sigesp_soc_rpp_listado_orden_compra_excel.php";
			pantalla    = "reportes/"+ls_formato+"?ordenes="+ls_ordenes+"&tiporeporte="+tiporeporte+"";
			window.open(pantalla,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,left=0,top=0,location=no,resizable=yes");
		}
	}
	else
	{
		...
		{
			ls_reporte  = "sigesp_soc_rpp_listado_orden_compra_excel.php";
			tiporeporte = "NORMAL";
			pagina="reportes/"+ls_reporte+"?txtnumordcomdes="+ls_numordcomdes+"&rdtipo="+ls_tipord
							 +"&txtnumordcomhas="+ls_numordcomhas+"&txtcodprodes="+ls_codprodes+"&txtcodprohas="+ls_codprohas
							 +"&txtfecordcomdes="+ls_fecordcomdes+"&txtfecordcomhas="+ls_fecordcomhas+"&esttip="+ls_tipo
