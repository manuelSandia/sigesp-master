...
function uf_insert_update($aa_seguridad)
{
		...
		// 	       Fecha: 09/06/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($ls_catalogo=="T")// En caso de que el analisis venga de un catalogo, se hace update 
		{
			if($this->uf_estado_analisis() != 1)//Si no ha sido aprobada
			{
				...
				{
					$this->io_sql->commit();
					$this->io_mensajes->message("El An�lisis de Cotizaciones fue actualizado");
				}
				else
				{
					$this->io_sql->rollback();
					$this->io_mensajes->message("El An�lisis de Cotizaciones no pudo ser actualizado");
				}
			}
			else
			{
				$this->io_mensajes->message("El An�lisis de Cotizaci�n no puede ser modificado ya que fue Aprobado");
			}
		}
		else //En caso de que sea un nuevo analisis, se inserta
		{
			...
			if($lb_valido)
			{
				...
				{
					$this->io_mensajes->message("Se Asigno el Numero al An�lisis de Cotizaci�n: ".$ls_numanacot);
				}
				$this->io_sql->commit();
				$this->io_mensajes->message("El An�lisis de Cotizaciones fue registrado");
			}
			else
			{
				$this->io_sql->rollback();
				$this->io_mensajes->message("El An�lisis de Cotizaciones no pudo ser registrado");
			}
		}
		//return true;
		return $lb_valido;
...
function uf_update_estatus_solicitud_cotizacion($ai_estatus,$aa_seguridad) 
{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->uf_update_estatus_solicitud_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
		  /////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
function uf_update_estatus_cotizacion($ai_estatus,$aa_seguridad)
{
		...
		for($li_i=1;(($li_i<=$li_totalcotizaciones) &&($lb_valido));$li_i++)
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->f_update_estatus_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			}
			else
			{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
function uf_update_analisis($aa_seguridad)
{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 	        Fecha: 13/06/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_numanacot=$_POST["txtnumero"];
		...
		$ls_recanacot=$_POST["txtrecanacot"];
		...
		$ls_sql="UPDATE soc_analisicotizacion 
				 SET fecanacot='$ls_fecanacot',codusu='$ls_codusu',estana=$ls_estanacot,obsana='$ls_obsanacot',tipsolcot='$ls_tipsolcot',numsolcot='$ls_numsolcot',
				     estasitec='',estesp='',garanacot='',recanacot='$ls_recanacot'
				 WHERE 
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->uf_update_analisis ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$this->io_sql->rollback();
		}
		else
		{						
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="UPDATE";
				$ls_descripcion ="Actualiz� el An�lisis de Cotizacion ".$ls_numanacot.", asociado a la empresa ".$this->ls_codemp;
				$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
...
function uf_update_items_analisis_cotizacion($aa_seguridad)
{
		...
		//	  Creado Por: Ing. Laura Cabr�
		// 	       Fecha: 11/06/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_tipsolcot=$_POST["txttipsolcot1"];
		$ls_numanacot=$_POST["txtnumero"];
		...
		for($li_i=1;($li_i<=$li_totalitems)&&($lb_valido);$li_i++)
		{
								...
								// algunos items dentro del analisis de cotizacion (actualmente esta deshabilitada esta opcion  en una funcion javascript en el guardar)
			{
				...
				{
					$lb_valido=false;
					$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->uf_update_items_analisis_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				}
				else
				{
						/////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
function uf_insert_analisis($aa_seguridad,&$as_numanacot)
{
		...
		//	  Creado Por: Ing. Laura Cabr�
		// 	       Fecha: 03/06/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="INSERT INTO soc_analisicotizacion 
				 (codemp,numanacot,fecanacot,codusu,estana,obsana,tipsolcot,numsolcot,estasitec,estesp,garanacot,recanacot)
				 VALUES
				 ('$this->ls_codemp','$as_numanacot','$ls_fecanacot','$ls_codusu',$ls_estanacot,'$ls_obsanacot','$ls_tipsolcot','$ls_numsolcot','','','','$ls_recanacot')";				
		$li_row=$this->io_sql->execute($ls_sql);
		if($li_row===false)
		{
			$this->io_sql->rollback();
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->uf_insert_analisis ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			}
		}
		else
		{						
		  /////////////////////////////////         SEGURIDAD               /////////////////////////////		
		  $ls_evento="INSERT";
		  $ls_descripcion ="Insert� el An�lisis de Cotizacion $as_numanacot, asociado a la empresa ".$this->ls_codemp;
		  $lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
										$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
										$aa_seguridad["ventanas"],$ls_descripcion);
		  /////////////////////////////////         SEGURIDAD               /////////////////////////////
		  $this->ls_supervisor=$_SESSION["la_empresa"]["envcorsup"];
			...
			{
				$ls_fromname="Analisis de Cotizaci�n";
				$ls_bodyenv="Se le envia la notificaci�n de actualizaci�n en el modulo de SOC, se realiz� el analisis de cotizaci�n N�.. ";
...
function uf_insert_cotizacion_analisis($aa_seguridad,$as_numanacot)
{
		...
		for($li_i=1;(($li_i<$li_totcotizaciones)&& ($lb_valido));$li_i++)
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->uf_insert_cotizacion_analisis ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			...
			{						
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
					$ls_evento="INSERT";
					$ls_descripcion ="Se asocio la Cotizacion $ls_numcot, al analisis de cotizaci�n $as_numanacot, para la empresa ".$this->ls_codemp;
					$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               /////////////////////////////
...
function uf_insert_items_analisis_cotizacion($aa_seguridad,$as_numanacot)
{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 	        Fecha: 15/05/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido     = true; 
		$ls_tipsolcot  = $_POST["txttipsolcot1"];
		$li_totalitems = $_POST["totalitems"];
		for($li_i=1;($li_i<=$li_totalitems)&&($lb_valido);$li_i++)
		{
								...
								// algunos items dentro del analisis de cotizacion (actualmente esta deshabilitada esta opcion  en una funcion javascript en el guardar)
			{
				...
				{
					$lb_valido=false;
				 	$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->uf_insert_items_analisis_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				}
				else
				{
						/////////////////////////////////         SEGURIDAD               /////////////////////////////		
						$ls_evento="INSERT";
						$ls_descripcion ="Insert� el $ls_item $ls_coditem al Analisis de Cotizacion ".$as_numanacot.
										 ", asociado a la empresa ".$this->ls_codemp;
						$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
														$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
														$aa_seguridad["ventanas"],$ls_descripcion);
...
function uf_delete($aa_seguridad)
{
		...
		if($this->uf_estado_analisis() != 1)//Si no ha sido aprobada
		{
			...
			{
				$this->io_sql->commit();
				$this->io_mensajes->message("El An�lisis de Cotizaciones fue eliminado");
			}
			else
			{
				$this->io_sql->rollback();
				$this->io_mensajes->message("El An�lisis de Cotizaciones no pudo ser eliminado");
			}
		}
		else
		{
			$this->io_mensajes->message("El An�lisis de Cotizaci�n no puede ser eliminado ya que fue Aprobado");
			$lb_valido=false;
		}		
		return $lb_valido;
	}// fin de uf_delete
...
function uf_delete_items_analisis_cotizacion($aa_seguridad)
{
		...
		//	  Creado Por: Ing. Laura Cabr�
		// 	       Fecha: 015/05/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_tipsolcot=$_POST["txttipsolcot1"];
		$ls_numanacot=$_POST["txtnumero"];
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->uf_delete_items_analisis_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			if($lb_valido)
...
function uf_delete_cotizacion_analisis($aa_seguridad)
{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->uf_delete_cotizacion_analisis ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$this->io_sql->rollback();
		}
		else
		{						
...
function uf_delete_analisis($aa_seguridad)
{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->uf_delete_analisis ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$this->io_sql->rollback();
		}
		else
		{						
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="DELETE";
				$ls_descripcion ="Elimino el An�lisis de Cotizacion ".$ls_numanacot.", asociado a la empresa ".$this->ls_codemp;
				$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
...
function uf_select_cotizacion($as_numcot,$as_codpro,$as_numsolcot,&$la_cotizacion,&$la_dt_cotizacion)
{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 			  Fecha: 28/04/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$la_cotizacion=array();
		$la_dt_cotizacion=array();
		$lb_valido=false;				
		$ls_sql= "SELECT c.feccot, c.obscot, c.monsubtot, c.monimpcot, c.montotcot, c.diaentcom, c.forpagcom, c.poriva, s.tipsolcot, c.estasitec, c.estesp 
...
function uf_select_items($as_numcot,$as_codpro,$as_tipsolcot,&$aa_items)
{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 			  Fecha: 29/04/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$aa_items=array();
		$lb_valido=false;				
		...
		elseif($as_tipsolcot=='S') //Si la solicitud es de Servicios
		{
			...
			if($rs_data===false)
			{
				$this->io_mensajes->message("CLASE->Analisis Cotizacion M�TODO->uf_select_items  ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				$lb_valido=false;	
			}
			else
			{
...
function uf_proveedores_item($as_tipsolcot, $as_numsolcot,$as_coditem,&$aa_proveedores)
{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 			Fecha: 28/05/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function uf_select_solicitud_cotizacion()
{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 			  Fecha: 22/06/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_numanacot=$_POST["txtnumero"];				
...
function uf_estado_analisis()
{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 	        Fecha: 12/08/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$la_cargos=array();
		$lb_valido=false;
		$ls_numanacot=$_POST["txtnumero"];
