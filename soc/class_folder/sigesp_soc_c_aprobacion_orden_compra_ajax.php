<?php
	...
	{
		case "BUSCAR":
		  uf_load_ordenes_compra($ls_numordcom,$ls_codpro,$ld_fecdes,$ld_fechas,$ls_tipordcom,$ls_tipope,$ls_justapro);
		break;
		
	}	
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_ordenes_compra($as_numordcom,$as_codpro,$ad_fecdes,$ad_fechas,$as_tipordcom,$as_tipope,$ls_justapro)
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    ...
	    $lo_title[6] = "Fecha";
		$lo_title[7] = "Monto";
		$lo_title[8] = "Justificaci&oacute;n ";
		if ($as_tipope=='R')
		   {
		     $lo_title[9] = "Aprobaci&oacute;n";
		   } 
		$lo_object[0]="";
		require_once("sigesp_soc_c_aprobacion_orden_compra.php");
		$io_aprobacion = new sigesp_soc_c_aprobacion_orden_compra("../../");
		$rs_data       = $io_aprobacion->uf_load_ordenes_compra($as_numordcom,$as_codpro,$ad_fecdes,$ad_fechas,$as_tipordcom,$as_tipope,$ls_justapro);
		$li_fila=0;
		...
		while ($row=$io_aprobacion->io_sql->fetch_row($rs_data))	  
		      {
				...
				$lo_object[$li_fila][8]="<input name=txtjustapro".$li_fila." id=txtjustapro".$li_fila."  type=text class=sin-borde  size=50   style=text-align:left    value='".$ls_justapro."' >";
				//$lo_object[$li_fila][8]="<textarea input name=txtjusaproordencmp".$li_fila." id=txtjusaproordencmp".$li_fila."  type=text class=sin-borde  size=80   style=text-align:left    style=text-align:left   >$ls_jusaproordencmp</textarea><input name=txtnumcotsele".$li_fila."   id=txtnumcotsele".$li_fila."   type=hidden value='$ls_numcot'>";
				
				if ($as_tipope=='R')
				   {
		 	         $lo_object[$li_fila][9]="<input name=txtfecaprord".$li_fila." id=txtfecaprord".$li_fila."  type=text class=sin-borde  size=12   style=text-align:center    value='".$ld_fecaprcom."' readonly>";
				   } 
		      }
		if ($li_fila>=1)
		   {
