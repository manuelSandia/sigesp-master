<?php
	...
	//N�mero del An�lisis de Cotizacion.
	...
	//Tipo de Analisis de Cotizaci�n.
	$ls_tipanacot = $io_funciones_soc->uf_obtenervalor("tipanacot","");
	$ls_justapro = $io_funciones_soc->uf_obtenervalor("jusanacot","");
    // proceso a ejecutar
	...
	{
		case "BUSCAR":
		  uf_load_analisis_cotizacion($ls_numanacot,$ld_fecdes,$ld_fechas,$ls_tipanacot,$ls_tipope,$ls_justapro);
		break;
		
	}	
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_analisis_cotizacion($as_numanacot,$ad_fecdes,$ad_fechas,$as_tipanacot,$as_tipope,$ls_justapro)
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_load_analisis_cotizacion
		//		   Access: private
		//	  Description: M�todo que busca los Analisis de cotizacion
        ...
        $lo_title[4] = "Fecha"; 
        $lo_title[5] =" Tipo";
        $lo_title[6] =" Justificaci&oacute;n ";
		if ($as_tipope=='R')
		   {
		     $lo_title[7] = "Aprobaci&oacute;n";
		   } 
		$lo_object[0]="";
		...
		$rs_data       = $io_aprobacion->uf_load_analisis_cotizacion($as_numanacot,$ad_fecdes,$ad_fechas,$as_tipanacot,$as_tipope,$ls_justapro);
		$li_fila=0;
		...
		while ($row=$io_aprobacion->io_sql->fetch_row($rs_data))	  
		      {
				...
				//$lo_object[$li_fila][6]="<textarea input name=txtjusanacot".$li_fila." id=txtjusanacot".$li_fila."  type=text class=sin-borde  size=80   style=text-align:left    style=text-align:left   >$ls_jusanacot</textarea><input name=txtnumcotsele".$li_fila."   id=txtnumcotsele".$li_fila."   type=hidden value='$ls_numcot'>";
				$lo_object[$li_fila][6]="<textarea input name=txtjustapro".$li_fila." id=ttxtjustapro".$li_fila."  type=text class=sin-borde  size=80   style=text-align:left  >$ls_justapro</textarea>";
				
				if ($as_tipope=='R')
				   {
		 	         $lo_object[$li_fila][7]="<input name=txtfecapro".$li_fila." id=txtfecapro".$li_fila."  type=text class=sin-borde  size=12   style=text-align:center    value='".$ld_fecapro."' readonly>";
				   } 
		      }
		if ($li_fila>=1)
		   {
