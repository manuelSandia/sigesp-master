			...
			if (array_key_exists("chk".$i,$_POST))
			   {
					...
					for($li_i=0;$li_i<$li_totalganadores;$li_i++)
					{
						...
						$this->uf_calculardetalles_montos($li_totrow_cotizacion,&$la_items_cotizacion,$ls_tipsolcot,$ls_tipo_proveedor,$ls_cotizacion,$lb_viene_sep);
										 ...
										 { 
										   ...
										   if ($lb_valido)
											  { 
												...
												if ($ls_tipsolcot=="B")
												   {
													 $lb_valido=$this->uf_insert_bienes($ls_numordcom,$aa_seguridad,$ls_proveedor,$ls_cotizacion,$la_items_cotizacion,$li_totrow_cotizacion,$ls_tipo_proveedor,$lb_viene_sep);
												   }
												elseif($ls_tipsolcot=="S")
												   {
											   ...
											   if($lb_valido)//Si la afectacion del Iva es Presupuestaria.
												...
												if ($lb_valido && ($ls_tipo_proveedor != "F")) // si el proveedor es de tipo formal no se le calculan los cargos
												   { 
													 $lb_valido=$this->uf_insert_cuentas_cargos($ls_numordcom,$ls_tipsolcot,$la_items,$li_totrow,$aa_seguridad,$lb_viene_sep,$ls_unidad);
												   }
												if ($lb_valido)
												   {
													 $ls_estcom=0;
													 $lb_valido=$this->uf_validar_cuentas($ls_numordcom,$ls_estcom,$ls_tipsolcot, $aa_seguridad );
												   }
												if (!$lb_valido)
												   {
													 break;
	   ...
	   if ($lb_valido)
		  {
			...
			$this->io_sql->commit();
			//Se cambió el mensaje de registro de orden de compras
			$this->io_mensajes->message("La orden fue registrada");
		    $this->io_sql->close();
		  }
	   else 
		  {
	...
	function  uf_select_items($as_numcot,$as_codpro,$as_numanacot,$as_tipsolcot,&$aa_items,&$li_i)
	{
		...
		{				
			$ls_sql="SELECT d.codart as codigo, MAX(a.denart) as denominacion, MAX(p.nompro) AS nompro, MAX(dt.canart) as cantidad,
							...
							MAX(d.numcot) AS numcot, MAX(d.cod_pro) AS cod_pro, MAX(dts.numsep) AS numsep
		...
		else
		{
				...
				$ls_sql="SELECT d.codser as codigo, MAX(a.denser) as denominacion, MAX(p.nompro) AS nompro, MAX(dt.canser) as cantidad,
					            ...
					            MAX(d.obsanacot) AS obsanacot, MAX(d.numcot) AS numcot, MAX(d.cod_pro) AS cod_pro, MAX(dts.numsep) AS numsep
...
//				$li_totaliva=number_format($li_totaliva+$aa_items[$li_j]["moniva"],2,'.','');
	...
	//--------------------------------------------------------------------------------------------------------------------
	function uf_calculardetalles_montos($ai_totrow,&$aa_items,$as_tipsolcot,$as_tipo_proveedor,$as_cotizacion,$ab_viene_sep)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		for($li_j=1;$li_j<=$ai_totrow;$li_j++)
		{
			...
			if($ab_viene_sep)
			{
				...
				$li_moncargo=$this->io_evaluate->uf_evaluar($ls_formula,$li_subtotart,&$lb_valido);
			}
			else
			{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_insert_bienes($as_numordcom,$aa_seguridad,$as_codpro,$ls_numcot,$aa_items_cotizacion,$ai_totrow_cotizacion,$as_tipo_proveedor,$ab_viene_sep)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
								...
								
	{
		...
		for($li_i=1;($li_i<=$ai_totrow_cotizacion)&&($lb_valido);$li_i++)
		{
			...
			if(trim($ls_numsep)!="")
			{
				...
				if($ls_formula=="")
				{
					$la_cargos=$this->uf_select_cargos($ls_codser,"S");
					$ls_formula=$la_cargos['formula'];
					unset($la_cargos);
				}
				///////////////////////////////////////////////////////////////////////////////////
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------	
    //---------------------------------------------------------------------------------------------------------------------------------------
    function uf_insert_cuentas_cargos($as_numordcom,$as_estcondat,$aa_items,$li_totrow,$aa_seguridad,$ab_vienesep,$as_unidad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($ls_estceniva=="1")
		{
			$lb_valido= $this->uf_load_estructura_central($ls_coduniadm,$ls_codestprocen1,$ls_codestprocen2,$ls_codestprocen3,$ls_codestprocen4,$ls_codestprocen5,$ls_esclacen);
	...
	function uf_validar_cuentas($as_numordcom,&$as_estcom,$as_estcondat, $aa_seguridad)
	{
		...
		else
		{
			...
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_existe))
			{
				...
				if($li_existe>0)
				{
					$ls_estprog[0]=$ls_codestpro1;
					$ls_estprog[1]=$ls_codestpro2;
					$ls_estprog[2]=$ls_codestpro3;
					$ls_estprog[3]=$ls_codestpro4;
					$ls_estprog[4]=$ls_codestpro5;
					$ls_estprog[5]=$ls_estcla;
						
					require_once("sigesp_soc_c_aprobacion_orden_compra.php");
					$io_soc=new sigesp_soc_c_aprobacion_orden_compra("../");
						
					$lb_valido=$io_soc->uf_verificar_precompromiso($as_numordcom,$as_estcondat,$ls_estprog,$ls_spg_cuenta,&$as_montocuentasep);
					if($lb_valido)
					{
					...
					if($li_monto>$li_disponibilidad)
					{
						...
						$this->io_mensajes->message("No hay Disponibilidad en la cuenta ".$ls_spg_cuenta.", para la estructura ".$ls_codestpro1.'-'.$ls_codestpro2.'-'.$ls_codestpro3.". Disponible=[".$li_disponibilidad."]. Cuenta=[".$li_monto."]");
			...
			if(!$lb_disp)
			{
				...
				$lb_valido = $this->uf_liberar_sep($as_numordcom,$as_estcondat,$aa_seguridad);
...
//---------------------------------------------------------------------------------------------------------------------------------------
	function  uf_select_items_cotizacion($as_numcot,$as_codpro,$as_numanacot,$as_tipsolcot,&$aa_items,&$li_i)
	{
		...
		{				
			$ls_sql="SELECT d.codart as codigo, MAX(a.denart) as denominacion, MAX(p.nompro) as nompro, MAX(dt.canart) as cantidad, 
						    ...
						    MAX(soc_dtsc_bienes.codestpro3) as codestpro3,MAX(soc_dtsc_bienes.codestpro4) as cosestpro4,MAX(soc_dtsc_bienes.codestpro5) as codestpro5,
	...
	function uf_liberar_sep($as_numordcom,$as_estcondat,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//    Description: Funcion que a partir de la Ordenes de Compra libera la SEP y permite que se pueda retomar.
		//	   Creado Por: Ing. David Briceño.
		// Fecha Creación: 01/03/2013							Fecha Última Modificación : 01/03/2013
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		
		require_once("sigesp_soc_c_anulacion_solicitud_cotizacion.php");
		$io_soc=new sigesp_soc_c_anulacion_solicitud_cotizacion("../");
		...
		//Buscar el numero de SEP a partir del numero de Orden de Compra
		$lb_valido=true;
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->sigesp_soc_c_generar_orden_analisis.php;MÉTODO->uf_liberar_sep ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			//Con el numero de SEP buscar el numero de Solicitud de Cotizacion
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				{
					//Con el nro de SEP y el nro de SolCot, procedo a actualizar la SEP
					while ($row=$this->io_sql->fetch_row($rs_datos))
					{
						$as_numsolcot = $row["numsolcot"];
						$lb_valido = $io_soc->uf_update_estatus_solicitud($ls_numsol,$as_estcondat,$as_numsolcot,$aa_seguridad);
					}
		...
		/*
		 ...
		 {
		$la_enlace_sep=array();
		$lb_valido=$io_soc->uf_delete_enlace_sep($as_numordcom,$as_estcondat,$la_enlace_sep,$aa_seguridad);
		}*/
