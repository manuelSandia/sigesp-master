<?php
	...
	{	
		...
		$lo_title[8]="Criterio de Selecci&oacute;n";
		$li_totrows = $io_funciones_soc->uf_obtenervalor("hidtotrows","0");
		for ($li_fila=1;$li_fila<=$li_totrows;$li_fila++)
			{
			  $ls_rows['ls_coditem']     = $io_funciones_soc->uf_obtenervalor("txtcoditem".$li_fila,"");
			  $ls_rows['ls_nomitem']     = $io_funciones_soc->uf_obtenervalor("txtnomitem".$li_fila,"");
			  $ls_rows['ls_nompro']      = $io_funciones_soc->uf_obtenervalor("txtnomproitem".$li_fila,"");
			  $ls_rows['ls_codpro']      = $io_funciones_soc->uf_obtenervalor("txtcodproselec".$li_fila,""); 
			  $ls_rows['ld_cantidad']    = $io_funciones_soc->uf_obtenervalor("txtcanselec".$li_fila,"");
			  $ls_rows['ld_precio']      = $io_funciones_soc->uf_obtenervalor("txtpreuniselec".$li_fila,"");
			  $ls_rows['ld_moniva']      = $io_funciones_soc->uf_obtenervalor("txtivaselec".$li_fila,"");
			  $ls_rows['ld_monto']       = $io_funciones_soc->uf_obtenervalor("txtmonselec".$li_fila,"");
			  $ls_rows['ls_observacion'] = $io_funciones_soc->uf_obtenervalor("txtobservacion".$li_fila,"");
			  $ls_rows['ls_numcot']      = $io_funciones_soc->uf_obtenervalor("txtnumcotsele".$li_fila,"");
			  			  
			  $lo_object[$li_fila] = uf_print_items_cotizaciones($li_fila, $ls_rows);	
			}
		...
		for ($li_i=1;$li_i<=$li_total;$li_i++)
		    {
			  $ls_rows['ls_coditem']  = trim($la_items[$li_i]["codigo"]);
			  $ls_rows['ls_nomitem']  = trim($la_items[$li_i]["denominacion"]);
			  if ($ab_catalogo)
			     {
				   $ls_rows['ls_nompro']      = $la_items[$li_i]["nompro"];
				   $ls_rows['ld_cantidad']    = number_format($la_items[$li_i]["cantidad"],2,",",".");
				   $ls_rows['ld_precio']      = number_format($la_items[$li_i]["precio"],2,",",".");
				   $ls_rows['ld_moniva']      = number_format($la_items[$li_i]["moniva"],2,",",".");
				   $ls_rows['ld_monto']       = number_format($la_items[$li_i]["monto"],2,",",".");
				   $ls_rows['ls_observacion'] = $la_items[$li_i]["obsanacot"];
				   $ls_rows['ls_numcot']      = $la_items[$li_i]["numcot"];
				   $ls_rows['ls_codpro']      = $la_items[$li_i]["cod_pro"];
			     }
			   else
			     {				   
				   $ls_rows['ls_nompro']      = "";
	...
	{	
		...
		{
			$ls_rows['ls_coditem']  = trim($io_funciones_soc->uf_obtenervalor("txtcoditem".$li_fila,""));
			$ls_rows['ls_nomitem']  = $io_funciones_soc->uf_obtenervalor("txtnomitem".$li_fila,"");
			$ls_rows['ls_nompro']  	= $io_funciones_soc->uf_obtenervalor("txtnomproitem".$li_fila,"");
			$ls_rows['ld_cantidad'] = $io_funciones_soc->uf_obtenervalor("txtcanselec".$li_fila,"");
			$ls_rows['ld_precio'] 	= $io_funciones_soc->uf_obtenervalor("txtpreuniselec".$li_fila,"");
	...
	function uf_select_items_cotizacion($as_numcot,$as_codpro,$as_tipcot,&$aa_items)
	{
		...
		elseif(!empty($ls_strsql) && $as_tipcot=='S')
		   {
		     $ls_sqlaux = "AND soc_dtcot_servicio.codser NOT IN (SELECT soc_dtcot_servicio.codser 
	...
	function uf_print_items_cotizaciones($li_fila, $ls_rows)
	{
		$lo_object[1]="<input name=txtcoditem".$li_fila."          id=txtcoditem".$li_fila."        type=text  class=sin-borde   size=10  value='".$ls_rows['ls_coditem']."'  style=text-align:center	readonly>";
		$lo_object[2]="<input name=txtnomitem".$li_fila."          id=txtnomitem".$li_fila."        type=text  class=sin-borde   size=30  value='".$ls_rows['ls_nomitem']."' style=text-align:left   readonly title='".$ls_rows['$ls_nomitem']."'>";
		$lo_object[3]="<input name=txtnomproitem".$li_fila."       id=txtnomproitem".$li_fila."     type=text  class=sin-borde   size=38  value='".$ls_rows['ls_nompro']."'   style=text-align:left   readonly ><a href='javascript:ue_catalogo_proveedores(".$li_fila.")'><img src='../shared/imagebank/tools20/buscar.gif' title='Seleccionar Proveedor' width=20 height=20 border=0></a></div><input name=txtcodproselec".$li_fila."   id=txtcodproselec".$li_fila."   type=hidden value='".$ls_rows['ls_codpro']."'>";
		$lo_object[4]="<input name=txtcanselec".$li_fila."         id=txtcanselec".$li_fila."       type=text  class=sin-borde   size=4   value='".$ls_rows['ld_cantidad']."' style=text-align:right  readonly>";
		$lo_object[5]="<input name=txtpreuniselec".$li_fila."      id=txtpreuniselec".$li_fila."    type=text  class=sin-borde   size=12  value='".$ls_rows['ld_precio']."'   style=text-align:right  readonly>";
		$lo_object[6]="<input name=txtivaselec".$li_fila."         id=txtivaselec".$li_fila."       type=text  class=sin-borde   size=12  value='".$ls_rows['ld_moniva']."'   style=text-align:right  readonly>";
		$lo_object[7]="<input name=txtmonselec".$li_fila."         id=txtmonselec".$li_fila."       type=text  class=sin-borde   size=12  value='".$ls_rows['ld_monto']."'    style=text-align:right 	readonly>";
		//$lo_object[$li_fila][8]="<input name=txtobservacion".$li_fila."      id=txtobservacion".$li_fila."    type=text  class=sin-borde   size=30  value='".$ls_rows['ls_observacion']."'               style=text-align:left   ><input name=txtnumcotsele".$li_fila."   id=txtnumcotsele".$li_fila."   type=hidden value='$ls_numcot'>";
		...
		$ls_otrossel = '';
		$old_observaciones = 0;
		switch ($ls_rows['ls_observacion']){
			case 'P':
				$ls_preciosel = 'selected';
				break;
			case 'A':
				$ls_acuerdosel = 'selected';
				break;
			case 'C':
				$ls_cantidadsel = 'selected';
				break;
			case 'F':
				$ls_fechasel = 'selected';
				break;
			case 'M':
				$ls_marcasel = 'selected';
				break;
			case 'O':
				$ls_otrossel = 'selected';
				break;
			default:
				$old_observaciones = $ls_rows['ls_observacion'] != ""? 1 : 0;
				break;
		}
		
		if ($old_observaciones == 1)
			$lo_object[8]="<input name=txtobservacion".$li_fila."      id=txtobservacion".$li_fila."    type=text  class=sin-borde   size=30  value='".$ls_rows['ls_observacion']."'               style=text-align:left   ><input name=txtnumcotsele".$li_fila."   id=txtnumcotsele".$li_fila."   type=hidden value='".$ls_rows['$ls_numcot']."'>";
		else
		{
			 
			$lo_object[8] =
			"<select name=txtobservacion".$li_fila." style='width:150px '>
					  		<option value=P ".$ls_preciosel.">Precio</option>
					  		<option value=A ".$ls_acuerdosel.">Acuerdo de Pago</option>
					  		<option value=C ".$ls_cantidadsel.">Cantidad</option>
					  		<option value=F ".$ls_fechasel.">Fecha Entrega</option>
					  		<option value=M ".$ls_marcasel.">Marca Reconocida</option>
					  		<option value=O ".$ls_otrossel.">Otros</option>
					  	</select>
					  	<input name=txtnumcotsele".$li_fila."   id=txtnumcotsele".$li_fila."   type=hidden value='".$ls_rows['ls_numcot']."'>";
		}
