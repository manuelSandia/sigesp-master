...
if (array_key_exists("hidoperacion",$_POST))
   {
	 ...
	 $ls_tipo      = $_POST["tipo"];
	 $ls_fecsolcot = $_POST["txtfecregsolcot"];
   }
else
   {
     $ls_numsep    = ""; 
...
<input name="txtfecregsolcot" type="hidden" id="txtfecregsolcot" value="<?php print $ls_fecsolcot; ?>" />
<br />
...
function ue_aceptar(ls_numsep,ls_consep,ld_monsep,ls_unieje,ls_denuni,ls_codigo,ls_nombre,ls_direccion,ls_telefono,as_codestpro1,as_codestpro2,as_codestpro3,as_codestpro4,as_codestpro5,as_estcla,ls_fecsol)
{
    lb_existe_sol = false;
	valido        = true;
	parametros    = "";
	fecsolcot   = f.txtfecregsolcot.value;
    validarfecha = ue_comparar_fechas(ls_fecsol,fecsolcot);
	if(validarfecha)
	{
	...
	{
		alert("La fecha de la SEP no puede ser mayor a la fecha de la solicitud de cotizacion");
	}
}
...
<script type="text/javascript" language="JavaScript1.2" src="js/funcion_soc.js"></script></html>