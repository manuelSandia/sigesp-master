...
$ls_reporte=$io_fun_compra->uf_select_config("SOC","REPORTE","ANALISIS_COTIZACION","sigesp_soc_rfs_analisis_cotizacion_cnti.php","C");
...
<p>
  <?php 
   function uf_limpiarvariables()
   {
		...
		// Fecha Creaci_n: 03/06/2007			Fecha _ltima Modificaci_n : 
		//////////////////////////////////////////////////////////////////////////////
		...
		global $ls_estasitec,$ls_estesp,$ls_garanacot,$ls_recanacot;
	...
	function uf_load_parametros()
	{
		...
		//	     Function: uf_load_parametros
		//		   Access: private
		//	  Description: Funci_n que almacena los valores del analisis de cotizacion registrado para luego repintarlo
		//	   Creado Por: Ing. Laura Cabr_
		// Fecha Creaci_n: 19/11/2007			Fecha _ltima Modificaci_n : 
		//////////////////////////////////////////////////////////////////////////////
		global $ls_parametros,$ls_numero,$ls_estatus,$ls_fecha,$ls_observacion,$li_estciespg,$li_estciespi,$ls_disabled,$li_items;
		global $ls_estasitec,$ls_estesp,$ls_garanacot,$ls_recanacot;
		$ls_parametros ="&numanacot=".$_POST["txtnumero"]."&fecanacot=".$_POST["txtfecha"]."&obsana=".$_POST["txtobservacion"]."&numsolcot=".$_POST["txtnumsol1"]."&txttipsolcot1=".$_POST["txttipsolcot1"]."&txtrecanacot=".$_POST["txtrecanacot"];
...
?>
  <table width="800" border="0" cellpadding="0" cellspacing="0" class="formato-blanco"  >
	  ...
	  <tr>
        <td height="22" colspan="2" style="text-align:right">Recomendaci&oacute;n</td>
	    <td height="22" colspan="6" style="text-align:left "><input name="txtrecanacot" type="text" id="txtrecanacot" value="<?php print $ls_recanacot; ?>" size="127" maxlength="254" <?php echo $ls_disabled; ?>></td>
      </tr>
	  <tr>
	    <td height="22" colspan="2" style="text-align:right">&nbsp;</td>
	    <td height="22" colspan="6" style="text-align:left ">&nbsp;</td>
      </tr>
	  ...
	  <tr>
        ...
        <td width="320">&nbsp;</td>
        <td width="382">&nbsp;</td>
        <td><input name="Submit2"  type="button" class="boton" value="An&aacute;lisis de Precios" width="99" style="text-align:center; cursor:auto "  title="Haga click para visualizar el an_lisis de precios ofrecidos por los proveedores" onClick="javascript=ue_analisis('analisis_precios');" <?php echo $ls_disabled; ?>></td>
        <!-- 
        <td>><input name="Submit3"  type="button" class="boton" value="An&aacute;lisis Cualitativo Proveedores" width="99" style="text-align:center; cursor:auto " title="Haga click para visualizar el an_lisis cualitativo de los proveedores" onClick="javascript=ue_analisis('analisis_cualitativo');" <?php echo $ls_disabled; ?>></td>
...
if (uf_evaluate_cierre())
   {
	if(confirm("_Est_ seguro de eliminar la cotizaci_n?"))
	{
		parametros="";
...
function ue_analisis(catalogo)//abre el catalogo del analisis de precios
{
	...
	{
		alert("Seleccione al menos una cotizaci_n para el an_lisis");
	}	
}
	   ...
	   if (((lb_catalogo=="T")&&(li_cambiar==1))||(lb_catalogo=="")&&(li_incluir==1))
	      {
			...
			{
				alert("El numero de analisis no puede estar vac_o");
				lb_valido=false;
			}
			else if(f.txtfecha.value=="")
			{
				alert("La fecha no puede esta vacia");
				lb_valido=false;
			}
			...
			{
				alert("Seleccione al menos una cotizacion");
				lb_valido=false;
			}
			else 
			{
				for(li_i=1;li_i<=totalitem;li_i++)
				{
					...
					{
						alert("El campo observacion del _tem "+txtitem+" no puede estar vac_o");
						lb_valido=false;
						break;
					}
				}
  ...
  if (uf_evaluate_cierre())
     {
	   ...
	   if (li_eliminar==1)
 	      {
		    if (confirm("Esta Seguro de Eliminar Este  An_lisis de Cotizaci_n.?"))
		       {
...
{	
	...
	if(li_imprimir==1)
	{
		...
		ls_recomendacion=f.txtrecanacot.value;
		tiporeporte= f.cmbbsf.value;
		window.open("reportes/"+formato+"?numanacot="+ls_numanacot+"&tipsolcot="+ls_tipsolcot+"&fecha="+ls_fecha+"&tiporeporte="+tiporeporte+"&observacion="+ls_observacion+"&recomendacion="+ls_recomendacion,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=1000,height=800,left=50,top=50,location=no,resizable=yes");
	}
	else
	{
		alert("No tiene permiso para realizar esta operaci_n !!!");
...
{	
	...
	if(li_imprimir==1)
	{
		...
		ls_observacion=f.txtobservacion.value;
		tiporeporte= f.cmbbsf.value;
		window.open("reportes/sigesp_soc_rfs_analisis_cotizacion_excel.php?numanacot="+ls_numanacot+"&tipsolcot="+ls_tipsolcot+"&fecha="+ls_fecha+"&tiporeporte="+tiporeporte+"&observacion="+ls_observacion,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=1000,height=800,left=50,top=50,location=no,resizable=yes");
	}
	else
	{
		alert("No tiene permiso para realizar esta operaci_n !!!");
			...
			else
			{
				if(ajax1.readyState==4)
				{
					...
					else
					{
						if(ajax1.status==404)
						{
							divgrid1.innerHTML = "La p_gina no existe";
						}
						else
						{//mostramos el posible error     
			...
			else
			{
				if(ajax2.readyState==4)
				{
					...
					else
					{
						if(ajax2.status==404)
						{
							divgrid2.innerHTML = "La p_gina no existe";
						}
						else
						{//mostramos el posible error     
