<?php
   ...
   function uf_print($as_codper,$as_stavac)
   {
		...
		if(!empty($as_stavac))
		{
			$ls_criterio="   AND sno_vacacpersonal.stavac=".$as_stavac." ";
		}
		$ls_sql="SELECT sno_vacacpersonal.codper, sno_vacacpersonal.codvac, sno_vacacpersonal.fecvenvac, sno_vacacpersonal.fecdisvac, ".
				...
				"		sno_vacacpersonal.diapervac, sno_vacacpersonal.pagpersal, sno_vacacpersonal.profueper, sno_vacacpersonal.diavaccal, ".
				"		sno_personal.cedper, sno_personal.nomper, sno_personal.apeper, sno_vacacpersonal.candiaadi ".				
				"  FROM sno_vacacpersonal ".
				" INNER JOIN sno_personal ".
				"    ON sno_vacacpersonal.codemp='".$ls_codemp."' ".
				...
				"	AND sno_vacacpersonal.codemp=sno_personalnomina.codemp ".
		...
		else
		{
			while(!$rs_data->EOF)
			{
				...
				$li_codvac=$rs_data->fields["codvac"];
				$ld_fecvenvac=$io_funciones->uf_formatovalidofecha($rs_data->fields["fecvenvac"]);
				$ld_fecdisvac=$io_funciones->uf_formatovalidofecha($rs_data->fields["fecdisvac"]);
				$ld_fecreivac=$io_funciones->uf_formatovalidofecha($rs_data->fields["fecreivac"]);
				...
				$li_stavac=$rs_data->fields["stavac"];
				$li_sueintbonvac=$io_fun_nomina->uf_formatonumerico($rs_data->fields["sueintbonvac"]);
				...
				$li_quisalvac=$rs_data->fields["quisalvac"];
				$li_quireivac=$rs_data->fields["quireivac"];
				...
				$li_diapag=$rs_data->fields["diapag"];
				$li_pagcan=$rs_data->fields["pagcan"];
				$li_diapervac=$rs_data->fields["diapervac"];
				$ls_pagpersal=$rs_data->fields["pagpersal"];
				$ls_profueper=$rs_data->fields["profueper"];
				$li_diavaccal=$rs_data->fields["diavaccal"];
				$li_candiaadi=$rs_data->fields["candiaadi"];
				switch($li_stavac)
				{
				...
				print "'$li_sabdom','$li_diapag','$li_pagcan','$ls_codper','$ls_nomper','$li_diapervac','$ls_pagpersal','$ls_profueper',";
				print "'$li_diavaccal','$li_candiaadi');\">".$li_codvac."</a></td>";
...
function aceptar(codvac,fecvenvac,fecdisvac,fecreivac,diavac,stavac,sueintbonvac,sueintvac,diabonvac,obsvac,diapenvac,persalvac,
				...
				pagpersal,profueper,diavaccal,candiaadi)
{
	...
	if (profueper=="1")
	{
		opener.document.form1.chkprofueper.checked=true;
	}
	else
	{
		opener.document.form1.chkprofueper.checked=false;
	}
	if (diavaccal=="1")
	{
	...
	if (candiaadi=="1")
	{
		opener.document.form1.chkcandiaadi.checked=true;
	}
	else
	{
		opener.document.form1.chkcandiaadi.checked=false;
	}
	opener.document.form1.hiddiavaccal.value=diavaccal;
...
{
	window.open("sigesp_sno_cat_personalnomina.php?tipo=catvacacion&codnom=<?PHP print $ls_codnom;?>","_blank","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
}
