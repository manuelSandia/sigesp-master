<?php
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print($as_codper, $as_cedper, $as_nomper, $as_apeper, $as_numexp, $as_codnom, $as_tipo)
   	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_print
		//		   Access: public
		...
		$ls_sql="SELECT sno_personal.codper, sno_personal.cedper, sno_personal.nomper, sno_personal.apeper, sno_personal.dirper, ".
				...
				"       sno_personal.porcajahoper, sno_personal.codger, sno_personal.anoperobr,sno_personal.carantper,sno_personal.rifper,sno_personal.messervpreper, ".
				...
				" AND sss_permisos_internos.enabled=1 AND sno_personal.codtippersss=sss_permisos_internos.codintper  )";
		...
		if(($as_tipo=="repconttrabdes")||($as_tipo=="repconttrabhas")||($as_tipo=="replispermintrades")||($as_tipo=="replispermintrahas"))
		{
		...
		else
		{
			while(!$rs_data->EOF)
			{
				...
				$ld_fecnacper=$io_funciones->uf_formatovalidofecha($rs_data->fields["fecnacper"]);
				...
				$ls_obsper=preg_replace("/\s+/"," ",$rs_data->fields["obsper"]);	
				...
				$ld_fecingadmpubper=$io_funciones->uf_formatovalidofecha($rs_data->fields["fecingadmpubper"]);
				...
				$ld_fecegrper=$io_funciones->uf_formatovalidofecha($rs_data->fields["fecegrper"]);
				$ld_fecegrper=$io_funciones->uf_convertirfecmostrar($ld_fecegrper);				
				$ls_cauegrper=$rs_data->fields["cauegrper"];			
				$ls_obsegrper=preg_replace("/\s+/"," ",$rs_data->fields["obsegrper"]);	// para reemplazar los \n o ENTER 		
				...
				$ls_rifper=$rs_data->fields["rifper"];
				$messerprev	= $rs_data->fields["messervpreper"];
				...
				$ls_descom=$rs_data->fields["descom"];
				$ls_desran=$rs_data->fields["desran"];
				$ls_numexpper=$rs_data->fields["numexpper"];
				$ls_despainac=$rs_data->fields["despainac"];		
				$ls_desestnac=$rs_data->fields["desestnac"];
				$ls_codtippersss=$rs_data->fields["codtippersss"];
				$ls_dentippersss=$rs_data->fields["dentippersss"];			
				$ld_fecreingper=$io_funciones->uf_formatovalidofecha($rs_data->fields["fecreingper"]);
				$ld_fecreingper=$io_funciones->uf_convertirfecmostrar($ld_fecreingper);				
				$ld_fecjubper=$io_funciones->uf_formatovalidofecha($rs_data->fields["fecjubper"]);
				$ld_fecjubper=$io_funciones->uf_convertirfecmostrar($ld_fecjubper);				
				$ls_codunivipladin=$rs_data->fields["codunivipladin"];
				$ls_denunivipladin=$rs_data->fields["denunivipladin"];	
				$ls_enviorec=$rs_data->fields["enviorec"];	
				$ls_fecleypen=$io_funciones->uf_formatovalidofecha($rs_data->fields["fecleypen"]);					
				$ls_fecleypen=$io_funciones->uf_convertirfecmostrar($ls_fecleypen);	
				$ls_codcausa=$rs_data->fields["codcausa"];
				$ls_fecsitu=$io_funciones->uf_formatovalidofecha($rs_data->fields["fecsitu"]);
				$ls_fecsitu=$io_funciones->uf_convertirfecmostrar($ls_fecsitu);
				$ls_talcamper=$rs_data->fields["talcamper"];
				$ls_talpanper=$rs_data->fields["talpanper"];
				$ls_talzapper=$rs_data->fields["talzapper"];
				$ls_anoprevcont=$rs_data->fields["anoservprecont"];	
				$ls_anoprevfijo=$rs_data->fields["anoservprefijo"];		
				$ls_codorg=$rs_data->fields["codorg"];			
				$ls_desorg=$rs_data->fields["desorg"];
				$ls_codger=$rs_data->fields["codger"];			
				$ls_denger=$rs_data->fields["denger"];								
				$li_porcajahoper=$rs_data->fields["porcajahoper"];				
				$li_porcajahoper=$io_fun_nomina->uf_formatonumerico($li_porcajahoper);	
				$li_anoperobr=$rs_data->fields["anoperobr"];
				$ls_carantper=$rs_data->fields["carantper"];
				switch ($as_tipo)
				{
					case "":
					...
					case "replispermintrades":
						print "<tr class=celdas-blancas>";
						print "<td><a href=\"javascript: aceptarreplispermintrades('$ls_codper');\">".$ls_codper."</a></td>";
						print "<td>".$ls_cedper."</td>";
						print "<td>".$ls_nomper."  ".$ls_apeper."</td>";
						print "<td>".$ls_tipperrif."-".$ls_numpririf."-".$ls_numterrif."</td>";
						print "<td>".$ls_estper."</td>";
					...
					case "replispermintrahas":
						print "<tr class=celdas-blancas>";
						print "<td><a href=\"javascript: aceptarreplispermintrahas('$ls_codper');\">".$ls_codper."</a></td>";
						print "<td>".$ls_cedper."</td>";
						print "<td>".$ls_nomper."  ".$ls_apeper."</td>";
						print "<td>".$ls_tipperrif."-".$ls_numpririf."-".$ls_numterrif."</td>";
						print "<td>".$ls_estper."</td>";
...
<br>
    <table width="500" border="0" cellpadding="1" cellspacing="0" class="formato-blanco" align="center">
	  ...
	  <tr>
        <td height="22"><div align="right">Nro. Expediente</div></td>
        <td><div align="left">
          <input name="txtnumexp" type="text" id="txtnumexp" size="30" maxlength="60" onKeyPress="javascript: ue_mostrar(this,event);">
        </div></td>
      </tr>
      <tr>
        <td height="22">&nbsp;</td>
...
<?php
	...
	if($ls_operacion=="BUSCAR")
	{
		...
		$ls_numexp="%".$_POST["txtnumexp"]."%";
		
		uf_print($ls_codper, $ls_cedper, $ls_nomper, $ls_apeper, $ls_numexp ,$ls_codnom, $ls_tipo);
	}
	unset($io_fun_nomina);
?>
</div>
...
function aceptar(codper,cedper,nomper,apeper,dirper,fecnacper,edocivper,telhabper,telmovper,sexper,estaper,pesper,codpro,
				 ...
				 anoperobr, carantper,tipperrif,numpririf,numterrif,jmesserprev)
{
	...
	opener.document.form1.txtmesserprev.value= jmesserprev;
	...
	opener.document.form1.btnsueldoshistoricos.disabled=false;
	opener.document.form1.btndeudaanterior.disabled=false;
	opener.document.form1.btnantiguedadanterior.disabled=false;
	opener.document.form1.btnanticipos.disabled=false;
	opener.document.form1.btnguarderia.disabled=false;
...
function aceptarreplispermintrades(codper)
{
	opener.document.form1.txtcodperdes.value=codper;
	opener.document.form1.txtcodperdes.readOnly=true;
	opener.document.form1.txtcodperhas.value="";
...
function aceptarreplispermintrahas(codper)
{
	if(opener.document.form1.txtcodperdes.value<=codper)
	{
		opener.document.form1.txtcodperhas.value=codper;
...
function aceptarreplisperracrecdes(codper)
{
	opener.document.form1.txtcodperdes.value=codper;
	opener.document.form1.txtcodperdes.readOnly=true;
	opener.document.form1.txtcodperhas.value="";
...
function aceptarreplisperracrechas(codper)
{
	if(opener.document.form1.txtcodperdes.value<=codper)
	{
		opener.document.form1.txtcodperhas.value=codper;
