<?php
   ...
   function uf_print($as_codconc, $as_nomcon, $as_tipo)
   {
		...
		$ls_sql="SELECT anocur, codperi, codconc, nomcon, titcon, sigcon, forcon, glocon, acumaxcon, valmincon, valmaxcon, concon, ".
				"		cueprecon, cueconcon, aplisrcon, sueintcon, intprocon, codestpro1, codestpro2, codestpro3, codestpro4, codestpro5, ".
				"		forpatcon, cueprepatcon, cueconpatcon, titretempcon, titretpatcon, valminpatcon, valmaxpatcon, codprov, cedben, ".
				"		conprenom, sueintvaccon, aplarccon, conprocon,estcla, intingcon, poringcon, spi_cuenta, repacucon, repconsunicon, ".
				...
				"		    AND spg_ep1.codestpro1=sno_thconcepto.codestpro1 ".
				"           AND spg_ep1.estcla = sno_thconcepto.estcla) as denestpro1, ".
				"		(SELECT denestpro2 ".
				"		   FROM spg_ep2 ".
				"		  WHERE spg_ep2.codemp=sno_thconcepto.codemp".
				"		    AND spg_ep2.codestpro1=sno_thconcepto.codestpro1 ".
				"		    AND spg_ep2.codestpro2=sno_thconcepto.codestpro2 ".
				"           AND spg_ep2.estcla = sno_thconcepto.estcla) as denestpro2, ".
				"		(SELECT denestpro3 ".
				"		   FROM spg_ep3 ".
				"		  WHERE spg_ep3.codemp=sno_thconcepto.codemp".
				"		    AND spg_ep3.codestpro1=sno_thconcepto.codestpro1 ".
				"		    AND spg_ep3.codestpro2=sno_thconcepto.codestpro2 ".
				"		    AND spg_ep3.codestpro3=sno_thconcepto.codestpro3 ".
				"           AND spg_ep3.estcla = sno_thconcepto.estcla) as denestpro3, ".
				"		(SELECT denestpro4 ".
				"		   FROM spg_ep4 ".
				"		  WHERE spg_ep4.codemp=sno_thconcepto.codemp".
				"		    AND spg_ep4.codestpro1=sno_thconcepto.codestpro1 ".
				"		    AND spg_ep4.codestpro2=sno_thconcepto.codestpro2 ".
				"		    AND spg_ep4.codestpro3=sno_thconcepto.codestpro3 ".
				"		    AND spg_ep4.codestpro4=sno_thconcepto.codestpro4 ".
				"           AND spg_ep4.estcla = sno_thconcepto.estcla) as denestpro4, ".
				"		(SELECT denestpro5 ".
				"		   FROM spg_ep5 ".
				"		  WHERE spg_ep5.codemp=sno_thconcepto.codemp".
				"		    AND spg_ep5.codestpro1=sno_thconcepto.codestpro1 ".
				"		    AND spg_ep5.codestpro2=sno_thconcepto.codestpro2 ".
				"		    AND spg_ep5.codestpro3=sno_thconcepto.codestpro3 ".
				"		    AND spg_ep5.codestpro4=sno_thconcepto.codestpro4 ".
				"		    AND spg_ep5.codestpro5=sno_thconcepto.codestpro5 ".
		...
		{ 
			while(!$rs_data->EOF)
			{
				...
				$li_acumaxcon=$io_fun_nomina->uf_formatonumerico($rs_data->fields["acumaxcon"]);
				$li_valmincon=$io_fun_nomina->uf_formatonumerico($rs_data->fields["valmincon"]);
				$li_valmaxcon=$io_fun_nomina->uf_formatonumerico($rs_data->fields["valmaxcon"]);
				$ls_concon=$rs_data->fields["concon"];
				$ls_cueprecon=trim($rs_data->fields["cueprecon"]);
				$ls_cueconcon=trim($rs_data->fields["cueconcon"]);
				$li_aplisrcon=$rs_data->fields["aplisrcon"];
				$li_aplarccon=$rs_data->fields["aplarccon"];
				$li_sueintcon=$rs_data->fields["sueintcon"];
				$li_intprocon=$rs_data->fields["intprocon"];
				$li_conprocon=$rs_data->fields["conprocon"];
				...
				$ls_cueprepatcon=trim($rs_data->fields["cueprepatcon"]);
				$ls_cueconpatcon=trim($rs_data->fields["cueconpatcon"]);
				$ls_titretempcon=$rs_data->fields["titretempcon"];
				$ls_titretpatcon=$rs_data->fields["titretpatcon"];
				$li_valminpatcon=$io_fun_nomina->uf_formatonumerico($rs_data->fields["valminpatcon"]);
				$li_valmaxpatcon=$io_fun_nomina->uf_formatonumerico($rs_data->fields["valmaxpatcon"]);
				$li_sueintvaccon=$rs_data->fields["sueintvaccon"];
				$li_conprenom=$rs_data->fields["conprenom"];
				$ls_denprecon=$rs_data->fields["denprecon"];
				$ls_dencuecon=$rs_data->fields["dencuecon"];
				$ls_dencueprepat="";
				$ls_dencueconpat="";
				...
				$ls_repacucon=$rs_data->fields["repacucon"];
				$ls_repconsunicon=$rs_data->fields["repconsunicon"];
				$ls_consunicon=$rs_data->fields["consunicon"];
				$ls_descon="";
				$ls_codcon="";
				$ls_nombre="";
				$li_intingcon=$rs_data->fields["intingcon"];
				$li_poringcon=number_format($rs_data->fields["poringcon"],2,",",".");
				$ls_spicuenta=trim($rs_data->fields["spi_cuenta"]);
				$ls_deningcon=$rs_data->fields["deningcon"];
				$ls_quirepcon=$rs_data->fields["quirepcon"];
				$ls_asifidper=$rs_data->fields["asifidper"];
				$ls_asifidpat=$rs_data->fields["asifidpat"];
				$ls_frevarcon=$rs_data->fields["frevarcon"];
				$ls_persalnor=$rs_data->fields["persalnor"];
				$ls_perenc=$rs_data->fields["conperenc"];
				$ls_aplresenc=$rs_data->fields["aplresenc"];
				$ls_codente=$rs_data->fields["codente"];
				$ls_txtente=$rs_data->fields["nombre_ente"];
				if($ls_codpro=="----------")
				{
					$ls_descon="B";
					$ls_codcon=$ls_cedben;
				...
				switch ($ls_sigcon)
				{
					...
					case "X":
						$ls_sigcon="Prestación Antiguedad";
						break;

					case "I":
						$ls_sigcon="Intereses Prestación";
						break;
				}			
				switch ($as_tipo)
				{
					case "":  // el llamado se hace desde sigesp_sno_d_hconcepto.php
						...
						print "'$ls_estcla','$li_intingcon','$li_poringcon','$ls_spicuenta','$ls_deningcon','$ls_repacucon','$ls_repconsunicon',";
						print "'$ls_consunicon','$ls_quirepcon','$ls_asifidper','$ls_asifidpat','$ls_frevarcon','$ls_persalnor',";
						print "'$ls_perenc','$ls_aplresenc','$ls_codente','$ls_txtente');\">".$ls_codconc."</a></td>";
					...
					case "MODIFICAR":  // el llamado se hace desde sigesp_sno_p_hmodificarconcepto.php
						...
						print "'$ls_estcla','$li_intingcon','$li_poringcon','$ls_spicuenta','$ls_deningcon','$ls_asifidper','$ls_asifidpat');\">".$ls_codconc."</a></td>";
						print "<td>".$ls_nomcon."</td>";
						print "<td>".$ls_signo."</td>";
						print "<td><input name='txtforcon' type='text' class='sin-borde' id='txtforcon' size='30' maxlength='30' title='".$ls_forcon."' value='".$ls_forcon."'></td>";
						print "</tr>";			
...
function aceptar(codconc,nomcon,titcon,sigcon,forcon,glocon,acumaxcon,valmincon,valmaxcon,concon,cueprecon,cueconcon,aplisrcon,
				 ...
				 estcla,intingcon,poringcon,spicuenta,deningcon,repacucon,repconsunicon,consunicon,quirepcon,
...
function aceptarmodificar(codconc,nomcon,titcon,aplisrcon,sueintcon,intprocon,codestpro1,codestpro2,codestpro3,denestpro1,
						  ...
						  sueintvaccon,aplarccon,conprocon,estcla,intingcon,poringcon,spicuenta,
