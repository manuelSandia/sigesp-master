...
class sigesp_sno_c_reversarnomina
{
	...
	function uf_load_resumenpago($as_peractnom,&$ai_totasi,&$ai_totded,&$ai_totapoemp,&$ai_totapopat,&$ai_totnom,&$ai_nropro)
	{
		...
		//				   ai_totnom  // Total de la N�mina
		//				   ai_nropro  // N�mero de personas a procesar
		//	      Returns: lb_valido True si se encontro � False si no se encontr�
		//	  Description: Funci�n que obtiene el resumen de pago de la n�mina 
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 01/01/2006 								Fecha �ltima Modificaci�n : 
		//////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Reversar N�mina M�TODO->uf_load_resumenpago ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
	...
	function uf_reversarnomina($aa_seguridad)
	{
		...
		//	  Description: Funcion que reversa la informaci�n de la n�mina de un personal en espec�fico
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 01/01/2006 								Fecha �ltima Modificaci�n : 
		//////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			
		...
		//	if(($li_desincorpora=='1')&&($li_desincorporaprestamo=='1'))
			//{
				// Reverso de cancelaci�n de cuotas
				...
				"   AND feciniper BETWEEN '".$ld_fecdesper."' AND '".$ld_fechasper."' ";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			
		...
		{
			$this->io_sql->commit();
			$this->io_mensajes->message("La N�mina fue reversada.");
		}
		else
		{
			$this->io_sql->rollback();
			$this->io_mensajes->message("Ocurrio un error al reversar la n�mina.");
		}
		return $lb_valido;
	...
	function uf_delete_salidas()
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Reversar N�mina M�TODO->uf_delete_salidas ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		return $lb_valido;
	...
	function uf_delete_resumen()
	{
		...
		//		   Access: private
		//	      Returns: lb_valido True si se ejecuto el delete � False si hubo error en el update
		//	  Description: Funcion que elimina de la tabla resumen toda la informac��n
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 01/01/2006 								Fecha �ltima Modificaci�n : 
		//////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Reversar N�mina M�TODO->uf_delete_resumen ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		return $lb_valido;
	...
	function uf_update_periodos($aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Reversar N�mina M�TODO->uf_update_periodos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion="Revers� la n�mina ".$this->ls_codnom." para el per�odo ".$this->ls_peractnom." ";
	...
	function uf_select_fecha_finvacper_cierre($ad_desde_r,$ad_hasta_r)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Cierre Periodo M�TODO->uf_select_fecha_finvacper_cierre ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_select_personal_prestamo_cierre($ad_desde_r,$ad_hasta_r)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Cierre Periodo M�TODO->uf_select_personal_prestamo_cierre ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
