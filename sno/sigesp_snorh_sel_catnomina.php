<?php
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print($as_codconc, $as_nomcon, $txtcodnom, $txtdesnom, &$ai_totrow)
   	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_print
		//		   Access: public
...
$ls_criterio2="";
		if (($txtdesnom!=""))
		{
			$ls_criterio=" AND desnom like  '%".$txtdesnom."%'";
		}
		if (($txtcodnom!=""))
		{
			$ls_criterio2=" AND codnom like  '%".$txtcodnom."%'";
		}
		...
		else
		{
			...
			while(!$rs_data->EOF)
			{
				...
				$object[$ai_totrow][2]="<input type=text name=txtcodnom".$ai_totrow." value='".$ls_codnom."' id=txtcodnom".$ai_totrow." class=sin-borde readonly style=text-align:center size=18 maxlength=18 >";		
				$object[$ai_totrow][3]="<input type=text name=txtdesnom".$ai_totrow." value='".$ls_desnom."' id=txtdesnom".$ai_totrow." class=sin-borde readonly style=text-align:left size=25 maxlength=25>";	
				$ai_totrow++;
				$rs_data->MoveNext();
			}
			//$object[$ai_totrow][1]="<input name=chkcta1 type=checkbox id=chkcta1 value=1 onClick=javascript:uf_selected('".$z."');>";
			//$object[$ai_totrow][2]="<input type=text name=txtcodnom value='' id=txtcodnom class=sin-borde readonly style=text-align:center size=20 maxlength=20>";		
			//$object[$ai_totrow][3]="<input type=text name=txtdesnom value='' id=txtdesnom class=sin-borde readonly style=text-align:center size=50 maxlength=254>";
			$grid->makegrid($ai_totrow,$title,$object,600,'Catalogo de Nominas','gridconceptos');			
		}
		unset($io_include);
		unset($io_conexion);
		unset($io_sql);
...
<form name="form1" method="post" action="">
  ...
  <table width="500" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr>
      <td width="496" height="20" colspan="2" class="titulo-ventana">Cat&aacute;logo de Nominas </td>
    </tr>
  </table>
...
<?php
	...
	$txtcodnom=$io_fun_nomina->uf_obtenervalor_get("txtcodnom","");
$txtdesnom=$io_fun_nomina->uf_obtenervalor_get("txtdesnom","");
	...
	{
		$ls_codconc="%".$txtcodnom."%";
		$ls_nomcon="%".$txtdesnom."%";
		$totrow=0;
		uf_print($ls_codconc, $ls_nomcon, $ls_codconc, $ls_nomcon, $li_totrow);		
	}
	unset($io_fun_nomina);	
?>
</div>
	  ...
	  for(i=1;(i<=parseInt(li_total,10));i++)	
	  {
   		if(li_sel<parseInt(li_selected,10))
		{
			if(eval("f.chknomina"+i+".checked==true"))
			{
				...
				if(parametro=='')
				{
					parametro=ls_txtcodnom;				
				}
				else
				{
					parametro=parametro+"-"+ls_txtcodnom;
				}
...
fop.txtcodnomdes.value=parametro;
...
function ue_search()
{
	f=document.form1;
     ...
     txtdesnom=f.txtdesnom.value;
	txtcodnom=f.txtcodnom.value;
  	f.action="sigesp_snorh_sel_catnomina.php?txtcodnom="+txtcodnom+"&txtdesnom="+txtdesnom;
  	f.submit();
}
