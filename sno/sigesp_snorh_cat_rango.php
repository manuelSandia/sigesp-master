<?php
   ...
   function uf_print($as_codcom, $as_codran, $as_desran, $as_tipo,$as_codcomdes,$as_codcomhas)
   {
		...
		$ls_sql=" SELECT sno_rango.codcom, sno_rango.codran, sno_rango.desran, sno_rango.codcat, sno_componente.descom, sno_rango.nomabr, ".
		...
		else
		{
			while(!$rs_data->EOF)
			{
				...
				$ls_codcat=$rs_data->fields["codcat"];
				$ls_descat=$rs_data->fields["descat"];
				$ls_nomabr=$rs_data->fields["nomabr"];
				switch($as_tipo)
				{
					case "":
						print "<tr class=celdas-blancas>";
						print "<td><a href=\"javascript: ue_aceptar('$ls_codran','$ls_desran','$ls_codcat','$ls_descat','$ls_nomabr');\">".$ls_codran."</a></td>";
...
function ue_aceptar(codran,desran,codcat,descat,nomabr)
{
	...
	opener.document.form1.txtnomabr.value=nomabr;
