<?php
	...
	$ls_codclie="20774";
	$ls_fecactual=$io_fun_nomina->uf_obtenervalor_get("fechaact","");
	$ls_fecactual=str_replace("/","",$ls_fecactual);
	$ls_nombreexcel="t_20774_21_".$ls_fecactual."_.";
	//---------------------------------------------------------------------------------------------------------------------------
	//Busqueda de la data 
	...
	$ls_titcedula="Cédula";
	$ls_titnombre="Nombre";
	$ls_titapellido="Apellido";
	$ls_titfechanac="Fecha de Nac.";
	$ls_titdia="Dia";
	$ls_titmes="Mes";
	$ls_titano="Año";
	$ls_titcodptoent="Codigo Pto. Entrega";
	$ls_titmontcarga="Monto Carga";
	if($lb_valido)
	{
	...
	else // Imprimimos el reporte
	{
		...
		////////////////////////////////////////////////////////////////////////////////////////////////
		$lo_hoja->write(3,1,$ls_titcedula,$lo_encabezado2);
		$lo_hoja->write(3,2,$ls_titnombre,$lo_encabezado2);
		$lo_hoja->write(3,3,$ls_titapellido,$lo_encabezado2);
		$lo_hoja->write(2,5,$ls_titfechanac,$lo_encabezado2);
		$lo_hoja->write(3,4,$ls_titdia,$lo_encabezado2);
		$lo_hoja->write(3,5,$ls_titmes,$lo_encabezado2);
		$lo_hoja->write(3,6,$ls_titano,$lo_encabezado2);
		$lo_hoja->write(3,7,$ls_titcodptoent,$lo_encabezado2);
		$lo_hoja->write(3,8,$ls_titmontcarga,$lo_encabezado2);
		////////////////////////////////////////////////////////////////////////////////////////////////
		$ls_coduniadmact="";
		$ls_desuniadmact="";
		$ls_desuniadm="";
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$ls_fecnacper=$io_report->DS->data["fecnacper"][$li_i];
			$ls_dianac=substr($ls_fecnacper,8,2);
			$ls_mesnac=substr($ls_fecnacper,5,2);
			$ls_anonac=substr($ls_fecnacper,0,4);
			$ls_codptoent='162371';
			...
			$ls_nomper=rtrim($io_report->DS->data["nomper"][$li_i]); //nombres
			...
			$ls_apeper=rtrim($io_report->DS->data["apeper"][$li_i]); //nombres
			$posape1 =strpos(rtrim($ls_apeper)," ",0);
			$ls_apeper1=substr(rtrim($ls_apeper),0,$posape1);  //Nombre de Persona 1
			$ls_apeper2=substr(ltrim(rtrim($ls_apeper)),$posape1+1,60);  //Nombre de Persona 2
			if($posape1==0)
			{
			...
			{   
				...
				for($li_k=1;$li_k<=$li_totrow3;$li_k++)
				{
					...
					$li_col3=$columna+2; 						
					$lo_hoja->write($li_row, 8, $li_valsal, $lo_dataright);						
				}
			...
			if ($ls_cedper!=$ls_cedperaux)
			{
				$lo_hoja->write($li_row, 0, $ls_nacper, $lo_dataright2);
				$lo_hoja->write($li_row, 1, $ls_cedper, $lo_dataright2);
				$lo_hoja->write($li_row, 2, $ls_nomper1, $lo_datacenter);
				$lo_hoja->write($li_row, 3, $ls_apeper1, $lo_datacenter);
				$lo_hoja->write($li_row, 4, $ls_dianac, $lo_datacenter);
				$lo_hoja->write($li_row, 5, $ls_mesnac, $lo_datacenter);
				$lo_hoja->write($li_row, 6, $ls_anonac, $lo_datacenter);
				$lo_hoja->write($li_row, 7, $ls_codptoent, $lo_datacenter);
		...
		$lo_libro->close();
		header("Content-Type: application/x-msexcel; name=\"".$ls_nombreexcel."xls\"");
		header("Content-Disposition: inline; filename=\"".$ls_nombreexcel."xls\"");
