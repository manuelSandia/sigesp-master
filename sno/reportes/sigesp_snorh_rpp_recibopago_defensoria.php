<?php
	...
	function uf_print_cabecera1($as_cedper,$as_nomper,$as_descar,$as_ubifis,&$io_cabecera,&$io_pdf)
	{
		...
		$la_data=array(array('ubicacion'=>'Unidad Administrativa: '.$as_ubifis));
	...
	else // Imprimimos el reporte
	{
		...
		while((!$io_report->rs_data->EOF)&&($lb_valido))
		{
			...
			$ls_uniadm=$ls_minorguniadm.$ls_ofiuniadm.$ls_uniuniadm.$ls_depuniadm.$ls_prouniadm.' - '.$ls_desuniadm;
			if($li_racnom==1)
			{
			...
			if($li_reg==1)
			{
				uf_print_cabecera1($ls_cedper,$ls_nomper,$ls_descar,$ls_uniadm,&$io_cabecera,&$io_pdf); // Imprimimos la cabecera del registro
			}
			else
			{
				uf_print_cabecera2($ls_cedper,$ls_nomper,$ls_descar,$ls_uniadm,&$io_cabecera,&$io_pdf); // Imprimimos la cabecera del registro
			}
			$ls_codperi="";
			if($ls_consolidar=="0")
			{
