<?php 
  ...
  session_start();   
	...
	$ls_titulo="<b>REPORTE POR UNIDADES ADMINISTRATIVAS</b>";
	...
	function encabezado($opciones=array()){		
						...
						//$pdf->Cell($ancho[7], 3,'UBICACIÓN', 1,0,'C',1);
	...
	function fila($datos=array()){		
						...
						$pdf->Cell($ancho[5], 3,calcular_anos(strtotime($datos['fecnacper']),strtotime(date('Y/m/d'))), 1,0,'C',1);
						$pdf->Cell($ancho[6], 3,$datos['dentippersss'], 1,0,'L',1);
						//$pdf->Cell($ancho[7], 3,substr($datos['desubifis'],0,10), 1,0,'L',1);
						$pdf->Cell($ancho[8], 3,$cargo, 1,0,'L',1);
