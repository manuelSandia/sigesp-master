<?php
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
function uf_print_carta_banco($as_nomban,$as_desnom,$ad_fecdesper,$ad_fechasper,$as_codcue,$as_peractnom,$ls_gerente,$ls_tipocuenta,$ls_monto_conv,$li_total,&$io_pdf)
{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],105,690,60,45); // Agregar Logo
		$as_titulo1=$as_desnom;
		$li_tm=$io_pdf->getTextWidth(10,$as_titulo1);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,670,10,$as_titulo1); // Agregar el título
		$as_titulo2=$as_peractnom;
		$li_tm=$io_pdf->getTextWidth(10,$as_titulo2);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,645,10,$as_titulo2); // Agregar el título		
		$io_pdf->addText(260,658,9,"<b>DEPOSITOS AL BANCO</b>"); // Agregar el título		
		$io_pdf->addText(70,616,9,$ls_gerente); // Agregar el título
		$io_pdf->addText(70,604,9,"Gerente de Negocios"); // Agregar el título
		$io_pdf->addText(70,592,9,$as_nomban); // Agregar el título
		$io_pdf->addText(70,572,9,$ls_ciudad); // Agregar el ciudad
		$io_pdf->addText(70,560,9,"Ciudad."); // Agregar el título
		$io_pdf->addText(70,540,9,"Estimados Señores:"); // Agregar el título
		
		$io_pdf->ezSetDy(-160);
		$ls_texto="           Por medio de la presente le autorizamos a cargar nuestra cuenta  ".$ls_tipocuenta." No.".
		          "".$as_codcue.", las siguientes cantidades de acuerdo al siguiente detalle, esto ".
				  "corresponden a  ".$as_desnom." del periodo que va desde ".$ad_fecdesper." hasta el ".
				  "".$ad_fechasper.", Son ".$ls_monto_conv."   ".$li_total."                                                                                                                                                          NOTA: ANEXO ENVIAMOS DISKETTE QUE CONTIENE LA INFORMACION DE ESTE LISTADO PARA SU PROCESAMIENTO.  ";			 
		$io_pdf->ezText($ls_texto,11,array('justification'=>'full', 'spacing' =>1.5));
	    $io_pdf->addText(40,758,10,"<b>INSTITUTO UNIVERSITARIO DE TECNOLOGIA</b>"); // Agregar el ciudad
		$io_pdf->addText(60,748,10,"<b>''JACINTO NAVARRO VALLENILLA''</b>"); // Agregar el ciudad
		$io_pdf->addText(75,738,10,"<b>CARUPANO EDO.SUCRE</b>"); // Agregar el ciudad
		
		$io_pdf->addText(40,158,10,"______________________________"); // Agregar el ciudad
		$io_pdf->addText(70,146,10,"<b>Lcdo. Luis Pláceres</b>"); // Agregar el ciudad
		$io_pdf->addText(40,136,10,"<b>Coord. Comisión de Transformación</b>"); // Agregar el ciudad
		$io_pdf->addText(80,126,10,"<b>y Modernización</b>"); // Agregar el ciudad
		
		$io_pdf->addText(400,158,10,"______________________________"); // Agregar el ciudad
		$io_pdf->addText(430,146,10,"<b>Lcdo. Arturo Hurtado</b>"); // Agregar el ciudad
		$io_pdf->addText(430,136,10,"<b>Coord. Sub Dirección</b>"); // Agregar el ciudad
		$io_pdf->addText(445,126,10,"<b>Administrativa</b>"); // Agregar el ciudad
	...
	$ls_gerente=$io_report->uf_buscagerente_banco($ls_codban);
	$ls_tipocuentaesp=$io_report->uf_buscatipocuenta_banco($ls_codban,$ls_codcue);
		...
		{			
			...
			if($lb_valido)
			{
				...
				while(!$io_report->rs_data_detalle->EOF)
				{
					...
					$li_monnetres=$io_fun_nomina->uf_formatonumerico($io_report->rs_data_detalle->fields["monnetres"]);					
					$ls_codcueban=$io_report->rs_data_detalle->fields["codcueban"];
					$la_data[$li_s]=array('nro'=>$li_nr,'cedula'=>$ls_cedper,'nombre'=>$ls_nomper,'monto'=>$li_monnetres, 'cuenta'=>$ls_codcueban);
					$li_s++;
					$io_report->rs_data_detalle->MoveNext();
				}
				if(!empty($la_data))
			...
			unset($io_encabezado);
			$io_pdf->ezNewPage();
			uf_print_carta_banco($ls_nomban,$ls_desnom,$ld_fecdesper,$ld_fechasper,$ls_codcue,$ls_periodo,$ls_gerente,$ls_tipocuentaesp,$ls_monto_conv,$li_total,$io_pdf); 
