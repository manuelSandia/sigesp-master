<?php
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_carta_banco($as_nomban,$as_desnom,$ad_fecdesper,$as_codcue,$ai_total,$ai_nro,$ad_fecpro,$as_peractnom,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if(substr($ad_fecdesper,0,2)=="01")
		{
			$ls_quincena="Primera Quincena ";
		}
		else
		{
			$ls_quincena="Segunda Quincena ";
		}
		$ls_quincena=$ls_quincena." del mes de ".$io_fecha->uf_load_nombre_mes(substr($ad_fecdesper,3,2))." de ".substr($ad_fecdesper,6,4);
		$io_pdf->addJpegFromFile('../../shared/imagebank/logo_tm_gobierno.jpg',50,700,220,70); // Agregar Logo
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],300,700,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$io_pdf->addJpegFromFile('../../shared/imagebank/logo_tm_venetur.jpg',440,700,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo

		$io_pdf->ezText("Mérida, ".date('d')." de ".$io_fecha->uf_load_nombre_mes(substr($ad_fecdesper,3,2))." de ".date('Y'),12,array('justification'=>'right'));
		$io_pdf->ezText("STM-GG-TES-OF ".$_SESSION["la_nomina"]["codnom"]." - ".$as_peractnom,12,array('justification'=>'right'));
		$io_pdf->ezSetDy(-15);
		$io_pdf->ezText("Señores:",12,array('justification' => 'left'));
		$io_pdf->ezText(strtoupper($as_nomban),12,array('justification' => 'left'));
		$io_pdf->ezText("Ciudad.-",12,array('justification' => 'left'));
		$io_pdf->ezSetDy(-35);
		$ls_texto="   Por medio de la presente solicitamos la ejecución de nuestra Nómina para el personal ".strtoupper($as_desnom)." del ".
				  strtoupper($_SESSION["la_empresa"]["nombre"]).", correspondiente a la ".$ls_quincena.". ";
		$io_pdf->ezText($ls_texto,12,array('justification'=>'full', 'spacing' =>1.5));
		$io_pdf->ezSetDy(-25);
		$ls_texto="   Asi mismo autorizamos a debitar el monto de la misma, según se instruye a continuación:";
		$io_pdf->ezText($ls_texto,12,array('justification'=>'full', 'spacing' =>1.5));
		$io_pdf->ezSetDy(-25);
		$la_data[0]=array('texto'=>'<b>DEBITAR A LA CUENTA</b>','valor'=>$as_codcue);
		$la_data[1]=array('texto'=>'<b>MONTO '.$ls_bolivares.'</b>','valor'=>$ai_total);
		$la_data[2]=array('texto'=>'<b>No DE REGISTROS </b>','valor'=>$ai_nro);
		$la_data[3]=array('texto'=>'<b>FECHA DEL ABONO</b>','valor'=>$ad_fecpro);
		$la_columna=array('texto'=>'','valor'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('texto'=>array('justification'=>'left','width'=>175), // Justificación y ancho de la columna
						 			   'valor'=>array('justification'=>'center','width'=>175))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		$io_pdf->ezSetDy(-25);
		$ls_texto="			Agradeciendo de antemano su atención, nos reiteramos de ustedes.";
		$io_pdf->ezText($ls_texto,12,array('justification' => 'full','spacing' =>1.5));
		$io_pdf->ezSetDy(-90);
		unset($la_data);
		$la_data[0]=array('texto1'=>'<b>Tcnel. Juan de Jesus Jaimes Díaz</b>','texto2'=>'<b>Lic. Lesbia Quintero</b>');
		$la_data[1]=array('texto1'=>'<b>Gerente General</b>','texto2'=>'<b>Administradora</b>');
		$la_columna=array('texto1'=>'','texto2'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>500, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('texto1'=>array('justification'=>'center','width'=>250), // Justificación y ancho de la columna
						 			   'texto2'=>array('justification'=>'center','width'=>250))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		unset($la_data);
		$io_pdf->ezSetY(120);
		$io_pdf->ezText("ANEXO: Diskette",12,array('justification' => 'left','spacing' =>1.5));
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	else // Imprimimos el reporte
	{
		...
		while(!$io_report->rs_data->EOF)
		{
			...
			unset($io_encabezado);
			$io_pdf->ezNewPage();
			uf_print_carta_banco($ls_nomban,$ls_desnom,$ld_fecdesper,$ls_codcue,$li_total,$li_nro,$ld_fecpro,$ls_peractnom,$io_pdf);
