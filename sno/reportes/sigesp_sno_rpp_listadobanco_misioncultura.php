<?php
	...
	function uf_print_piecabecera($ai_total,&$io_pdf)
	{
		...
		$la_data[2]=array('firma1'=>'____________________________','firma2'=>'','firma3'=>'____________________________');
		$la_data[3]=array('firma1'=>'Elaborado Por','firma2'=>'','firma3'=>'Aprobado Por');
	...
	function uf_print_carta_banco($as_nomban,$as_desnom,$ad_fecdesper,$as_codcue,$ai_total,$ai_nro,$ad_fecpro,$as_peractnom,&$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],90,700,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$io_pdf->ezText("Caracas, ".date('d')." de ".$io_fecha->uf_load_nombre_mes(substr($ad_fecdesper,3,2))." de ".date('Y'),12,array('justification'=>'right'));
		$io_pdf->ezSetDy(-15);
		...
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		$io_pdf->ezSetDy(-55);
		$io_pdf->ezText("Atentamente.",12,array('justification' => 'center','spacing' =>1.5));
		$io_pdf->ezSetDy(-70);
		unset($la_data);
		$la_data[0]=array('texto1'=>'<b>A.DE STEFANO      Y      B.ARREDONDO</b>');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>500, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('texto1'=>array('justification'=>'center','width'=>500))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		unset($la_data);
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
