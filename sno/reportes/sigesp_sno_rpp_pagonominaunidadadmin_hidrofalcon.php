<?php
							   ...
							   &$io_cabecera,&$io_pdf)
	{
		...
		$io_pdf->addText(55,695,9,'<b>Unidad Administrativa</b> '.$as_minorguniadm.'-'.$as_ofiuniadm.'-'.$as_uniuniadm.'-'.$as_depuniadm.'-'.$as_prouniadm.'  '.$as_desuniadm.''); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	$ls_desincorpora=$io_fun_nomina->uf_obtenervalor_get("desincorpora","");
	$ls_desincorporaprestamo=$io_fun_nomina->uf_obtenervalor_get("desincorporaprestamo","");
	//--------------------------------------------------------------------------------------------------------------------------------
			...
			while((!$io_report->rs_data_detalle->EOF)&&($lb_valido))
			{
				...
				if($lb_valido)
				{
					...
					{						
						...
						$ld_fecdesperaux=$_SESSION["la_nomina"]["fecdesper"];
						$ld_fechasperaux=$_SESSION["la_nomina"]["fechasper"];
						$lb_desinprestamo=$io_report->uf_select_persona_prestamo_desincorpora($ls_codper,$ld_fecdesperaux,$ld_fechasperaux);
						if (($ls_desincorpora==1)&&($ls_desincorporaprestamo==1)&&($lb_desinprestamo))
						{
							//$ls_codconc=$io_report->rs_data_detalle->fields["codconc"];
							$ls_pering_perso=$io_report->uf_select_periodo_ingvacac($ls_codper,$ld_fecdesperaux,$ld_fechasperaux);
							$ls_conc_busq=$io_report->uf_select_conc_ingvacac($ls_codper,$ls_pering_perso);
							//print "CONC. BUSQ    ".$ls_conc_busq."  ====  ".$ls_codconc."   ------>".$ls_tipsal."<br>";
							if (($ls_conc_busq==$ls_codconc)&&($ls_tipsal=='D'))
							{
								//print "ENTRE!!";
