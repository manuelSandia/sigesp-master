<?php
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_desnom,$as_periodo,$as_quincena,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if ($as_quincena==1)
		{
			$li_tm=$io_pdf->getTextWidth(11,'Primera Quincena');
			$tm=306-($li_tm/2);
			$io_pdf->addText($tm,720,11,'<b>Primera Quincena</b>'); // Agregar el título
			$li_tm=$io_pdf->getTextWidth(10,$as_desnom);
			$tm=306-($li_tm/2);
			$io_pdf->addText($tm,710,10,$as_desnom); // Agregar el título
		}
		elseif ($as_quincena==2)
		{
			$li_tm=$io_pdf->getTextWidth(11,'Segunda Quincena');
			$tm=306-($li_tm/2);
			$io_pdf->addText($tm,720,11,'<b>Segunda Quincena</b>'); // Agregar el título
			$li_tm=$io_pdf->getTextWidth(10,$as_desnom);
			$tm=306-($li_tm/2);
			$io_pdf->addText($tm,710,10,$as_desnom); // Agregar el título
		}
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nro'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
						 			   ...
						 			   'banco'=>array('justification'=>'center','width'=>75),
									   'rango'=>array('justification'=>'center','width'=>75), // Justificación y ancho de la columna
	...
	$ls_otros_bancos=$io_fun_nomina->uf_obtenervalor_get("otros_bancos","");
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_listadobanco_banco($ls_codban,$ls_suspendido,$ls_sc_cuenta,$ls_ctaban,$ls_subnomdes,$ls_subnomhas,$ls_codperdes,$ls_codperhas,$ls_otros_bancos); // Cargar el DS con los datos de la cabecera del reporte
	}
	...
	else // Imprimimos el reporte
	{
		...
		uf_print_encabezado_pagina($ls_titulo,$ls_desnom,$ls_periodo,$ls_quincena,$io_pdf); // Imprimimos el encabezado de la página
			...
			if (($ls_tipocuenta == '') ||  ($ls_tipocuenta == $ls_tipcueban))
			{
				...
				if($lb_valido)
				{
					...
					while(!$io_report->rs_data_detalle->EOF)
					{
						...
						$ls_desran=$io_report->rs_data_detalle->fields["denran"];
						$la_data[$li_s]=array('nro'=>$li_nro,'cedula'=>$ls_cedper,'nombre'=>$ls_nomper,'banco'=>$ls_nomban,'monto'=>$li_monnetres, 'rango'=>$ls_desran);
						$li_s++;
