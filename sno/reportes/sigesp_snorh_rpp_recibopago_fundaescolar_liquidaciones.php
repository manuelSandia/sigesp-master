<?php
	...
	function uf_print_cabecera1($as_cedper,$as_nomper,$as_descar,$as_desubifis,$as_denmun,$as_denpar,$ad_fecingnom,$ad_fecculcontr,&$io_cabecera,&$io_pdf)
	{
		...
		$io_pdf->ezSetY(667);
		$la_data=array(array('ingreso'=>'Fecha de Ingreso: '.$ad_fecingnom,'egreso'=>'Fecha de Egreso: '.$ad_fecculcontr,'causa'=>'Causa de Liquidación: CULMINACIÓN DE CONTRATO'));
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('ingreso'=>array('justification'=>'left','width'=>140),
									   ...
									   'causa'=>array('justification'=>'left','width'=>250))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
	...
	function uf_print_pie_cabecera1($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('conforme'=>array('justification'=>'left','width'=>270),
						 		 	   'espacio'=>array('justification'=>'left','width'=>230))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
		$la_data[1]=array('conforme'=>'YO arriba debidamente identificado por medio de la presente hago constar que recibo de Fundación Escolar del Estado Lara FUNDAESCOLAR la cantidad de (<b>'.$ai_totnet.'</b>), por concepto de pago de Prestaciones sociales, sueldos y salarios y demás remuneraciones como consecuencia de la relación laboral que hoy finaliza, con la cual nada queda a debérseme ni yo a reclamar de conformidad con lo previsto con la Ley Orgánica del trabajo y su reglamento.');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>500, // Ancho Máximo de la tabla
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('conforme'=>array('justification'=>'full','width'=>450))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('elaborado'=>array('justification'=>'center','width'=>118),
						 		 	   ...
						 		 	   'autorizado'=>array('justification'=>'center','width'=>105))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	$ls_titulo="<b>COMPROBANTE DE LIQUIDACIÓN</b>";
	$ls_periodo="";
	if($ls_consolidar=="1")
	{
