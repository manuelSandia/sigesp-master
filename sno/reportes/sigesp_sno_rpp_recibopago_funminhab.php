<?php
	...
	function uf_print_encabezado_pagina1($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->addJpegFromFile('../../shared/imagebank/logo_mh.jpg',50,730,470,55); // Agregar Logo
	...
	function uf_print_encabezado_pagina2($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$io_encabezado=$io_pdf->openObject();
		$io_pdf->saveState();
		$io_pdf->addJpegFromFile('../../shared/imagebank/logo_mh.jpg',50,321,470,55); // Agregar Logo
		$li_tm=$io_pdf->getTextWidth(11,$as_titulo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,320,11,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_periodo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,310,9,$as_periodo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,295,9,$as_desnom); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
	...
	function uf_print_cabecera2($as_cedper,$as_nomper,$as_descar,&$io_cabecera,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nombre'=>array('justification'=>'left','width'=>220), // Justificación y ancho de la columna
						 			   ...
						 			   'cargo'=>array('justification'=>'left','width'=>260))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		$io_pdf->addText(40,274,'10','______________________________________________________________________________________________');
		$la_data=array(array('nombre'=>$as_nomper, 'cedula'=>$as_cedper, 'cargo'=>substr($as_descar,0,100)));
		$la_columna=array('nombre'=>'','cedula'=>'','cargo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'xPos'=>315,
	...
	function uf_print_pie_cabecera1($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,&$io_pdf)
	{
		...
		$io_pdf->addText(40,448,'10','____________________________');
		$io_pdf->addText(70,434,'9','Recibí Conforme');
		$io_pdf->addText(300,448,'10','____________________________');
		$io_pdf->addText(330,434,'9','Cedula de Identidad');
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_pie_cabecera2($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,&$io_pdf)
	{
		...
		$io_pdf->addText(40,38,'10','____________________________');
		$io_pdf->addText(70,24,'9','Recibí Conforme');
		$io_pdf->addText(300,38,'10','____________________________');
		$io_pdf->addText(330,24,'9','Cedula de Identidad');
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_piepagina,'all');
		$io_pdf->stopObject($io_piepagina); // Detener el objeto pie de página
