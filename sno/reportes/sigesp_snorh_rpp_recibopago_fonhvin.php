<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creación: 23/04/2007 
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$io_encabezado=$io_pdf->openObject();
		$io_pdf->saveState();
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],50,720,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$li_tm=$io_pdf->getTextWidth(9,"FONDO PARA EL DESARROLLO INTEGRAL DE LA VIVIENDA");
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,730,9,"FONDO PARA EL DESARROLLO INTEGRAL DE LA VIVIENDA"); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,"Y HABITAD DEL ESTADO MÉRIDA");
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,720,9,"Y HABITAD DEL ESTADO MÉRIDA"); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_titulo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,710,9,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_periodo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,700,9,$as_periodo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,685,9,$as_desnom); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_cabecera($as_cedper,$as_nomper,$as_descar,$as_unidad,$as_desunidad,&$io_cabecera,&$io_pdf)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$io_pdf->saveState();
		$io_pdf->addText(40,675,'10','______________________________________________________________________________________________');
		$io_pdf->ezSetDy(-31);
		...
		$io_pdf->addText(40,664,'10','______________________________________________________________________________________________');
		unset($la_data);
		unset($la_columna);
		unset($la_config);
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nombre'=>array('justification'=>'left','width'=>220), // Justificación y ancho de la columna
						 			   ...
						 			   'cargo'=>array('justification'=>'left','width'=>260))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		$io_pdf->addText(40,654,'10','______________________________________________________________________________________________');
		$io_pdf->ezSetY(654);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
	...
	function uf_print_pie_cabecera($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,&$io_pdf)
	{
		...
		/*if($ls_tiporeporte==0)
		{
			...
			unset($la_columna);
			unset($la_config);
			$la_data=array(array('linea'=>'<b>Neto a Cobrar Bs.F. :</b>  '.$li_montobsf));
			$la_columna=array('linea'=>'');
			$la_config=array('showHeadings'=>0, // Mostrar encabezados
							 'fontSize' => 9, // Tamaño de Letras
							 'titleFontSize' => 7,  // Tamaño de Letras de los títulos
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_firmas($as_cedper,$as_nomper,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$la_data[0]=array('firma1'=>'','firma2'=>'','firma3'=>'Recibi Conforme');
		$la_data[1]=array('firma1'=>'','firma2'=>'','firma3'=>'');
		$la_data[2]=array('firma1'=>'Lic. Maribel Plaza Vezga','firma2'=>'','firma3'=>'');
		$la_data[3]=array('firma1'=>'Gerente de Administración','firma2'=>'','firma3'=>$as_nomper);
		$la_data[4]=array('firma1'=>'y Finanzas del FONHVIM','firma2'=>'','firma3'=>$as_cedper);
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
				 		 'cols'=>array('firma1'=>array('justification'=>'center','width'=>200), // Justificación y ancho de la columna
						 			   ...
						 			   'firma3'=>array('justification'=>'left','width'=>200))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
	...
	else // Imprimimos el reporte
	{
		...
		while((!$io_report->rs_data->EOF)&&($lb_valido))
		{
			...
			if($ls_consolidar=="0")
			{
				...
				$tm=306-($li_tm/2);
				$io_pdf->addText($tm,700,9,$ls_periodo); // Agregar el título
			}
