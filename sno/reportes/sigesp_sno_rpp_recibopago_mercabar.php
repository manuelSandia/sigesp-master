<?php
	...
	function uf_print_encabezado_pagina1($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,730,9,$as_periodo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,715,9,$as_desnom); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_cabecera1($as_cedper,$as_nomper,$as_descar,&$io_cabecera,$as_gerencia,$ad_fechaingreso,&$io_pdf)
	{
		...
		$io_pdf->addText(40,694,'10','______________________________________________________________________________________________');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nombre'=>array('justification'=>'left','width'=>200), // Justificación y ancho de la columna
									   ...
									   'fecha'=>array('justification'=>'left','width'=>90))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);		
		$io_pdf->addText(40,676,'10','______________________________________________________________________________________________');
		$io_pdf->ezSetY(678);
		...
		$io_pdf->addText(40,430,'8','_______________________');
		$io_pdf->addText(50,420,'8','RECIBI CONFORME');
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_cabecera2($as_cedper,$as_nomper,$as_descar,&$io_cabecera,$as_gerencia,$ad_fechaingreso,&$io_pdf)
	{
		...
		$io_pdf->addText(40,30,'8','_______________________');
		$io_pdf->addText(50,20,'8','RECIBI CONFORME');
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_cabecera,'all');
	}// end function uf_print_cabecera
						...
						case "2": // segunda quincena;
							while(!$io_report->rs_data_detalle->EOF)
							{
								...
								if(($ls_tipsal=="A") || ($ls_tipsal=="V1") || ($ls_tipsal=="V2") || ($ls_tipsal=="R")) // Buscamos las asignaciones
								{
									...
									$li_cantidad=$io_report->rs_data_detalle->fields["unidad"];
									if ($li_cantidad == 0)
									{
										$li_cantidad="";
									}
									...
									$la_data_a[$li_asig]=array('denominacion'=>$ls_nomcon."  ".$li_cantidad,'valor'=>$li_valsal);
								}
								else // Buscamos las deducciones y aportes
								{
									...
									$li_valsal=$io_fun_nomina->uf_formatonumerico(abs($io_report->rs_data_detalle->fields["valsal"]));
									$la_data_d[$li_dedu]=array('denominacion'=>$ls_nomcon."  ".$li_cantidad,'valor'=>$li_valsal);
								}
