<?php
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_carta_banco($as_nomban,$as_desnom,$ad_fecdesper,$as_codcue,$ai_total,$ai_nro,$ad_fecpro,$as_peractnom,$aa_general,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->ezSetDy(-15);
		$io_pdf->ezText("Respetables Señores:",12,array('justification' => 'left'));
		$io_pdf->ezSetDy(-20);
		$ls_texto="   Por medio de la presente me dirijo a uds en la oportunidad de solicitar la afiliación del servicio de nómina ".
				  "automatizada correspondiente al pago de la nómina de los empleados adscritos al ". strtoupper($_SESSION["la_empresa"]["nombre"]).", ".
				  "los cuales serán debitados a través de la cuenta corriente plus identificada con el Nro. ".$as_codcue;
		$io_pdf->ezText($ls_texto,12,array('justification'=>'full', 'spacing' =>1.5));
		$io_pdf->ezSetDy(-25);
		$la_columna=array('nombre'=>'Nombre del Empleado','cedula'=>'C.I.','cuenta'=>'Nro. Cuenta');
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('nombre'=>array('justification'=>'left','width'=>250), // Justificación y ancho de la columna
						 			   'cedula'=>array('justification'=>'center','width'=>80), // Justificación y ancho de la columna
						 			   'cuenta'=>array('justification'=>'center','width'=>150))); // Justificación y ancho de la columna
		$io_pdf->ezTable($aa_general,$la_columna,'',$la_config);	
		$io_pdf->ezSetDy(-25);
		$ls_texto="			Atentamente,";
		$io_pdf->ezText($ls_texto,12,array('justification' => 'full','spacing' =>1.5));
		$io_pdf->ezSetDy(-35);
		$ls_texto="			______________________";
		$io_pdf->ezText($ls_texto,12,array('justification' => 'full','spacing' =>1.5));
		$io_pdf->ezSetDy(-5);
		$ls_texto="			C.I.";
		$io_pdf->ezText($ls_texto,12,array('justification' => 'full','spacing' =>1.5));
	...
	else // Imprimimos el reporte
	{
		...
		while(!$io_report->rs_data->EOF)
		{
			...
			if (($ls_tipocuenta == '') ||  ($ls_tipocuenta == $ls_tipcueban))
			{
				...
				if($lb_valido)
				{
					...
					while(!$io_report->rs_data_detalle->EOF)	
					{
						...
						$li_general=$li_general+1;
						$la_general[$li_general]=array('nombre'=>$ls_nomper, 'cedula'=>$ls_cedper, 'cuenta'=>$ls_codcueban);					
						$li_s++;
			...
			unset($io_encabezado);
			$io_pdf->ezNewPage();
			uf_print_carta_banco($ls_nomban,$ls_desnom,$ld_fecdesper,$ls_codcue,$li_total,$li_nro,$ld_fecpro,$ls_peractnom,$la_general,$io_pdf);
