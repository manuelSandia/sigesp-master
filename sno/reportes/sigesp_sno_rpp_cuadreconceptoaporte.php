<?php
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo,$ai_tipo)
	{
		...
		$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","sigesp_sno_r_cuadreconceptoaporte.php",$ls_descripcion,$ls_codnom);
		return $lb_valido;
	}
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	$ls_titulo="<b>Cuadre de Conceptos y Aportes</b>";
	...
	if($lb_valido) // Buscamos la información que afecta el presupuesto de los aportes
	{
		...
		{
			case "1":
				$lb_valido=$io_report->uf_cuadreconceptoaporte_aportes_proyecto();
				break;

			default:
				$lb_valido=$io_report->uf_cuadreconceptoaporte_aportes();
				break;
		}
	}
	if($lb_valido) // Buscamos la información que afecta el presupuesto de los conceptos
	{
		switch($ls_conpronom)
		{
			case "1":
				$lb_valido=$io_report->uf_cuadreconceptoaporte_conceptos_proyecto();
				break;

			default:
				$lb_valido=$io_report->uf_cuadreconceptoaporte_conceptos();
				break;
		}
	}
	...
	else // Imprimimos el reporte
	{
		...
		$lb_print=false;
		$ls_modalidad=$_SESSION["la_empresa"]["estmodest"];
		//--------------------------------------------- Imprimir el detalle Presupuestario concepto------------------------------------------------	
		$li_totrow=$io_report->DS_detalle->getRowCount("cueprecon");
		$li_totalpresupuesto=0;
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			$ls_programatica=$io_report->DS_detalle->data["codestpro1"][$li_i].$io_report->DS_detalle->data["codestpro2"][$li_i].
							 $io_report->DS_detalle->data["codestpro3"][$li_i].$io_report->DS_detalle->data["codestpro4"][$li_i].
							 $io_report->DS_detalle->data["codestpro5"][$li_i];
		...
		//--------------------------------------------- Imprimir el detalle Presupuestario Aporte------------------------------------------------	
