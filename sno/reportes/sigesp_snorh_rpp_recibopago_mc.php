<?php
	...
	function uf_print_encabezado_pagina2($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$io_encabezado=$io_pdf->openObject();
		$io_pdf->saveState();
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],40,300,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$li_tm=$io_pdf->getTextWidth(11,$as_titulo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,310,11,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,300,9,$as_desnom); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_periodo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,285,9,$as_periodo); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
		...
		while((!$io_report->rs_data->EOF)&&($lb_valido))
		{
			...
			if($ls_consolidar=="0")
			{
				...
				if($li_reg==1)
				{
					...
					$tm=306-($li_tm/2);
					$io_pdf->addText($tm,705,9,$ls_periodo); // Agregar el título
				}
				else
				{
					$li_tm=$io_pdf->getTextWidth(9,$ls_periodo);
					$tm=306-($li_tm/2);
					$io_pdf->addText($tm,285,9,$ls_periodo); // Agregar el título
				}
			}
			...
			if($lb_valido)
			{
				...
				$li_toting=number_format($li_toting,2,",",".");
