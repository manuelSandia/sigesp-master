<?php
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera($as_cedper,$as_apenomper,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_data1[1]=array('codigo'=>'___________________________________________________________________________________________________________________');
		$la_data1[2]=array('codigo'=>'___________________________________________________________________________________________________________________');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'rowGap' => 0.1,
						 'cols'=>array('codigo'=>array('justification'=>'center','width'=>560))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data1,$la_columna,'',$la_config);
		
		$io_pdf->ezSetDy(-5);
		$la_data[1]=array('codigo'=>'<b><i>Código</i></b>','nombre'=>'<b><i>Apellidos y Nombres</i></b>');
		$la_data[2]=array('codigo'=>$as_cedper,'nombre'=>$as_apenomper);
		$la_columna=array('codigo'=>'',
						  'nombre'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'width'=>500, // Ancho de la tabla
						 'maxWidth'=>500, // Ancho Máximo de la tabla
						 'xPos'=>225, // Orientación de la tabla
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera2($as_codconc,$as_nomcon,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_cabecera
		//		   Access: private 
		...
		$la_data[1]=array('codigo'=>'<b>Concepto:  </b>'.'<b>'.$as_codconc.'</b>'."     ",'nombre'=>'<b><i>'.$as_nomcon.'</i></b>');
		$la_columna=array('codigo'=>'',
						  'nombre'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 ...
						 'width'=>500, // Ancho de la tabla
						 'maxWidth'=>500, // Ancho Máximo de la tabla
						 'xPos'=>245, // Orientación de la tabla
						 'cols'=>array('codigo'=>array('justification'=>'center','width'=>120), // Justificación y ancho de la columna
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle($la_data,$li_montot,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$io_pdf->ezSetDy(-5);
		$la_data1=array(array('codperi'=>'<b>PERIODO</b>',
						   'fecdes'=>'<b>DESDE</b>',
						   'fechas'=>'<b>HASTA</b>',
						   'monto'=>'<b>MONTO</b>'));
		$la_columnas=array('codperi'=>'',
						   'fecdes'=>'',
						   'fechas'=>'',
						   'monto'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('codperi'=>array('justification'=>'center','width'=>100), // Justificación y ancho de la columna
						 			   'fecdes'=>array('justification'=>'center','width'=>100), // Justificación y ancho de la columna
						 			   'fechas'=>array('justification'=>'center','width'=>100), // Justificación y ancho de la columna
		...
		$la_columnas=array('codperi'=>'<b>PERIODO</b>',
						   'fecdes'=>'<b>DESDE</b>',
						   'fechas'=>'<b>HASTA</b>',
						   'monto'=>'<b>MONTO</b>');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		$la_data2=array(array('total'=>'<b>Total            </b>'.$li_montot));
		$la_columnas=array('total'=>'<b>Total</b>');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'titleFontSize' => 12,  // Tamaño de Letras de los títulos
						 'showLines'=>1, // Mostrar Líneas
	...
	$ls_detperiodo=$io_fun_nomina->uf_obtenervalor_get("detperiodo","");
	...
	$ls_titulo="<b>Consolidado de Conceptos Detallados</b>";
	...
	else  // Imprimimos el reporte
	{
		...
		$lb_valido=$io_report->uf_personas_conceptos($ls_codnomdes,$ls_codnomhas,$ls_codperdes,
							     					 $ls_codperhas,$ls_conceptocero,$ls_personaldes,$ls_personalhas,
					                     			 $ls_subnomdes,$ls_subnomhas,$ls_orden,$ls_anocurper); // Cargar el DS con los datos de la cabecera del reporte
		
		$li_totper=$io_report->DS_detper->getRowCount("codper");
		for($li_i=1;$li_i<=$li_totper;$li_i++)
		{
			$ls_codper=$io_report->DS_detper->data["codper"][$li_i];
			$ls_cedper=$io_report->DS_detper->data["cedper"][$li_i];
			$ls_apenomper=$io_report->DS_detper->data["apeper"][$li_i].", ". $io_report->DS_detper->data["nomper"][$li_i];
			uf_print_cabecera($ls_cedper,$ls_apenomper,$io_pdf);
			for($li_s=1;$li_s<=$li_totrow;$li_s++)
			{
				$ls_codconc=$io_report->DS->data["codconc"][$li_s];
				$ls_nomcon=$io_report->DS->data["nomcon"][$li_s];
				$ls_sigcon=$io_report->DS->data["sigcon"][$li_s];
				uf_print_cabecera2($ls_codconc,$ls_nomcon,$io_pdf);
				$lb_valido=$io_report->uf_consolidadoconcepto_personal_detallado($ls_codnomdes,$ls_codnomhas,$ls_codconc,$ls_codperdes,
																	   			 $ls_codperhas,$ls_conceptocero,$ls_codper,$ls_codper,
				...
				if ($lb_valido)
				{
					...
					if ($li_totrow_det > 0)
					{
						for($li_p=1;$li_p<=$li_totrow_det;$li_p++)
						{
							$ls_cedper=$io_report->DS_detalle->data["cedper"][$li_p];
							$ls_apenomper=$io_report->DS_detalle->data["apeper"][$li_p].", ". $io_report->DS_detalle->data["nomper"][$li_p];
							$li_montot=$li_montot + abs($io_report->DS_detalle->data["valsal"][$li_p]);
							$li_valsal=$io_fun_nomina->uf_formatonumerico(abs($io_report->DS_detalle->data["valsal"][$li_p]));
							$ld_fecdesper=$io_funciones->uf_convertirfecmostrar($io_report->DS_detalle->data["fecdesper"][$li_p]);
							$ld_fechasper=$io_funciones->uf_convertirfecmostrar($io_report->DS_detalle->data["fechasper"][$li_p]);
							$ls_codperi=$io_report->DS_detalle->data["codperi"][$li_p];
							$la_data[$li_p]=array('nro'=>$li_p,'cedula'=>$ls_cedper,'nombre'=>$ls_apenomper,'monto'=>$li_valsal,'fecdes'=>$ld_fecdesper,'fechas'=>$ld_fechasper,'codperi'=>$ls_codperi);
						}
						$io_report->DS_detalle->resetds("cedper");
						$li_montot=$io_fun_nomina->uf_formatonumerico(abs($li_montot));
						uf_print_detalle($la_data,$li_montot,$io_pdf); // Imprimimos el detalle 
						unset($la_data);
					}
				}
			}
		}
		$io_report->DS_detper->resetds("codper");
		unset($la_data);
