<?php
	...
	function uf_print_cabecera_2(&$io_pdf)
	{
		...
		$io_pdf->addText(50,485,9,'<b>DATOS DE LA ASIGNACIÓN DE CARGOS</b>'); // Agregar el título
		$io_pdf->ezSety(480);
        $io_pdf->setColor(0.9,0.9,0.9);
        $io_pdf->filledRectangle(45,465,709,$io_pdf->getFontHeight(12));
	...
	function uf_print_cabecera(&$io_pdf)
	{
		...
		$io_encabezado=$io_pdf->openObject();
		$io_pdf->saveState();
		$io_pdf->addText(50,485,9,'<b>DATOS DE LA ASIGNACIÓN DE CARGO</b>'); // Agregar el título
		$io_pdf->ezSety(480);
        $io_pdf->setColor(0.9,0.9,0.9);
        $io_pdf->filledRectangle(37,465,725,$io_pdf->getFontHeight(12));
        $io_pdf->setColor(0,0,0);
		$la_data[1]=array('codigo'=>'<b>Código</b>',
						  'unidad'=>'<b>Unidad Adm.</b>',
						  'cargo'=>'<b>Denominación del Cargo</b>',
...
function uf_print_detalle_2($la_data,&$io_pdf)
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_codigos_unicos_rac($aa_data,&$io_pdf)
	{
	  	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_codigos_unicos_rac
		//		   Access: private 
		//	    Arguments: io_pdf // Instancia de objeto pdf
		//    Description: función que imprime el detalle por concepto
		...
		$la_data[1]=array('codigo'=>'<b>DATOS DE LOS CÓDIGOS</b>');
		$la_columna=array('codigo'=>'');
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('codigo'=>array('justification'=>'left','width'=>200))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset ($la_data);
		unset ($la_columna);
		unset ($la_config);
		...
		$la_data[1]=array('codigo'=>'<b>Código Único</b>',
						  'estado'=>'<b>Estado</b>');
		$la_columna=array('codigo'=>'',
						  'estado'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('codigo'=>array('justification'=>'center','width'=>200), // Justificación y ancho de la columna
						 			   'estado'=>array('justification'=>'center','width'=>120))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset ($la_data);
		unset ($la_columna);
		unset ($la_config);
	...
	}// end function uf_print_codigos_unicos_rac
	...
	else  // Imprimimos el reporte
	{
		...
		$io_pdf->ezSetCmMargins(5.1,2.5,3,3); // Configuración de los margenes en centímetros
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$lb_valido=$io_report->uf_buscar_codigos_unico_rac($ls_codasi,$rs_data);
			if ($lb_valido)
			{	
				if ($io_report->io_sql->num_rows($rs_data)>0)
				{
					$i=0;
					while (!$rs_data->EOF)
					{
						...
						$i++;
						$la_det[$i]=array('codigo'=>$ls_codunirac,'estado'=>$ls_estcodunirac);
				...
				{
					$la_det[1]=array('codigo'=>'No tiene Códigos Asociados','estado'=>'');
				}
				uf_print_codigos_unicos_rac($la_det,&$io_pdf);
			}
