<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_titulo2,$as_titulo3,$as_titulo4,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,690,11,$as_periodo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(10,$as_desnom);
		$tm=306-($li_tm/2);
		$ls_codnom=$_SESSION["la_nomina"]["codnom"];
		$io_pdf->addText($tm,680,10,$as_desnom."   ".$ls_codnom); // Agregar el título
	...
	function uf_print_pie_cabecera_presupuesto($ai_total,$ai_totalpersonal,&$io_pdf)
	{
		...
		$la_data=array(array('name'=>'<b>Totales </b>','totalpersonal'=>$ai_totalpersonal,'total'=>$ai_total));
		$la_columna=array('name'=>'','totalpersonal'=>'','total'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
				 		 'cols'=>array('name'=>array('justification'=>'right','width'=>420), // Justificación y ancho de la columna
						 			   'totalpersonal'=>array('justification'=>'center','width'=>50), // Justificación y ancho de la columna
						 			   'total'=>array('justification'=>'right','width'=>80))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
	...
	if($lb_valido) // Buscamos la información que afecta el presupuesto
	{
		switch($ls_conpronom)
		{
			...
			default:
				$lb_valido=$io_report->uf_contableaportes_especifico_presupuesto($ls_codestpro1,$ls_codestpro2,$ls_codestpro3,
				                                                                 $ls_codestpro4,$ls_codestpro5,$ls_estcla,
																				 $ls_subnomdes,$ls_subnomhas);
				break;
		}
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$la_data[$li_i]=array('partida'=>$ls_cueprepatcon,'denominacion'=>$ls_denominacion,
