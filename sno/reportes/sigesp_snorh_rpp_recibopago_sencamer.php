<?php
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina1($as_titulo,$as_desnom,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$li_tm=$io_pdf->getTextWidth(9,"REPORTE GENERAL DE PAGO");
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,710,9,"REPORTE GENERAL DE PAGO"); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina2($as_titulo,$as_desnom,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$li_tm=$io_pdf->getTextWidth(9,$as_titulo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,340,9,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,330,9,$as_desnom); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,"REPORTE GENERAL DE PAGO");
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,320,9,"REPORTE GENERAL DE PAGO"); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera1($as_codper,$as_cedper,$as_nomper,$as_descar,$as_codcueban,$as_desuniadm,&$io_cabecera,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->saveState();
		$io_pdf->ezSetDy(-5);
		$la_data[1]=array('codigo'=>'Código', 'cedula'=>'Cédula', 'nombre'=>'                                                   Apellidos y Nombres', 'cuenta'=>'Cuenta Bancaria' );
		$la_data[2]=array('codigo'=>$as_codper, 'cedula'=>$as_cedper, 'nombre'=>$as_nomper, 'cuenta'=>$as_codcueban );
		$la_columna=array('codigo'=>'','cedula'=>'','nombre'=>'','cuenta'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('codigo'=>array('justification'=>'center','width'=>80), // Justificación y ancho de la columna
						 			   ...
						 			   'cuenta'=>array('justification'=>'center','width'=>120))); // Justificación y ancho de la columna
		...
		unset($la_columna);
		unset($la_config);
		$la_data[1]=array('asignacion'=>'ASIGNACIONES', 'deduccion'=>'DEDUCCIONES');
		$la_columna=array('asignacion'=>'','deduccion'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'xPos'=>315,
						 'fontSize' => 7, // Tamaño de Letras
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
						 'rowGap' => 0.5 ,
						 'cols'=>array('asignacion'=>array('justification'=>'center','width'=>275), // Justificación y ancho de la columna
						 			   'deduccion'=>array('justification'=>'center','width'=>275))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera2($as_codper,$as_cedper,$as_nomper,$as_descar,$as_codcueban,$as_desuniadm,&$io_cabecera,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		unset($la_config);
		$la_data[1]=array('cargo'=>'                                                             Cargo', 'departamento'=>'                                                          Departamento');
		$la_data[2]=array('cargo'=>$as_descar, 'departamento'=>'');
		$la_columna=array('cargo'=>'','departamento'=>$as_desuniadm);
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'xPos'=>315,
						 'fontSize' => 7, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
	...
	function uf_print_pie_cabecera1($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,&$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->ezSety(480);
		$la_data=array(array('denomasig'=>'<b>Total ASIGNACIONES '.$ls_bolivares.'</b>', 'valorasig'=>$ai_toting, 'denomdedu'=>'<b>Total DEDUCCIONES '.$ls_bolivares.'</b>','valordedu'=>$ai_totded));
	...
	else // Imprimimos el reporte
	{
		...
		uf_print_encabezado_pagina1($ls_titulo,$ls_desnom,$io_pdf); // Imprimimos el encabezado de la página
		uf_print_encabezado_pagina2($ls_titulo,$ls_desnom,$io_pdf); // Imprimimos el encabezado de la página
		...
		while((!$io_report->rs_data->EOF)&&($lb_valido))
		{
			...
			if($li_reg==1)
			{
				uf_print_cabecera1($ls_codper,$ls_cedper,$ls_nomper,$ls_descar,$ls_codcueban,$ls_desuniadm,$io_cabecera,$io_pdf); // Imprimimos la cabecera del registro
			}
			else
			{
				uf_print_cabecera2($ls_codper,$ls_cedper,$ls_nomper,$ls_descar,$ls_codcueban,$ls_desuniadm,$io_cabecera,$io_pdf); // Imprimimos la cabecera del registro
			}
			$ls_codperi="";
			if($ls_consolidar=="0")
			{
