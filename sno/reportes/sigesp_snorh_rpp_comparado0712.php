<?php
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$ls_descripcion="Generó el Reporte ".$as_titulo." Forma 0712";
		$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte("SNR","sigesp_snorh_r_comparado0712.php",$ls_descripcion);
		return $lb_valido;
	}
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_encabezado_pagina($as_titulo,$as_periodo,$as_rango,&$io_pdf)
	{
		...
		$li_tm=$io_pdf->getTextWidth(8,"PERSONAL JUBILADO, PENSIONADO Y ASIGNACIÓN A SOBREVIVIENTE");		
		$tm=504-($li_tm/2);
		$io_pdf->addText($tm,540,8,"<b>PERSONAL JUBILADO, PENSIONADO Y ASIGNACIÓN A SOBREVIVIENTE</b>"); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(8,"(En Bolívares".$ls_prefijo1.")");		
		$tm=504-($li_tm/2);
		$io_pdf->addText($tm,530,8,"<b>(En Bolívares".$ls_prefijo1.")</b>"); 
		...
		$io_pdf->addText(885,438,8,"MONTO EN ".$ls_prefijo2.""); // 100 pts
		$li_tm=$io_pdf->getTextWidth(8,$as_periodo);		
		$tm=98-($li_tm/2);
		$tm=775+$tm;
		$io_pdf->addText($tm,465,8,$as_periodo); 
		$io_pdf->addText(30,50,7,"FORMA 0712");
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	$ls_titulo="PERSONAL JUBILADO, PENSIONADO Y ASIGNACIÓN A SOBREVIVIENTE";
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_comparado0712_programado($ls_rango); // Obtenemos el detalle del reporte
	}
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$li_cargoproximo=0;
			$li_montoproximo=0;
			$lb_valido=$io_report->uf_comparado0712_real($ls_rango,$ls_codded,$ls_codtipper,$li_cargoreal,$li_montoreal); // Obtenemos los valores reales
			if($lb_valido)
			{
