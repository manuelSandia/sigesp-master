<?php
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte("SNR","sigesp_snorh_r_unidadvipladin.php",$ls_descripcion);
		return $lb_valido;
	}
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_cabecera(&$io_pdf)
	{
        ...
        $io_pdf->filledRectangle(50,505,700,$io_pdf->getFontHeight(12));
        $io_pdf->setColor(0,0,0);
		$la_data[1]=array('codigo'=>'<b>Código</b>',
						  'nombre'=>'<b>Apellidos y Nombres</b>',
						  'unidadadm'=>'<b>Unidad VIPLADIN</b>',
	...
	$ls_codunivides=$io_fun_nomina->uf_obtenervalor_get("codunivides","");
	$ls_codunivihas=$io_fun_nomina->uf_obtenervalor_get("codunivihas","");
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_listadopersonalunidad_vipladin_personal($ls_codnomdes,$ls_codnomhas,$ls_codperdes,$ls_codperhas,
																		$ls_activo,$ls_egresado,$ls_causaegreso,$ls_activono,
																		$ls_vacacionesno,$ls_suspendidono,$ls_egresadono,
																		$ls_masculino,$ls_femenino,$ls_codunivides,
																		$ls_codunivihas,$ls_orden); // Obtenemos el detalle del reporte
	}
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$ls_desunivi=$io_report->DS->data["denunivipladin"][$li_i];
			...
			$la_data[$li_i]=array('codigo'=>$ls_codper,'nombre'=>$ls_nomper,'unidadadm'=>$ls_desunivi,'desubifis'=>$ls_desubifis,
