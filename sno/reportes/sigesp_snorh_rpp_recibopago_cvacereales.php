<?php
	...
	function uf_print_encabezado_pagina1($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$li_tm=$io_pdf->getTextWidth(9,"Recibo ".$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,705,9,"Recibo ".$as_desnom); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_encabezado_pagina2($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$li_tm=$io_pdf->getTextWidth(9,"Recibo ".$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,315,9,"Recibo ".$as_desnom); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
							    ...
							    &$io_cabecera,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('denomasig'=>'<b>CONCEPTOS</b>', 'valorasig'=>'<b>ASIGNACIÓN</b>', 'denomdedu'=>'<b>CONCEPTOS</b>','valordedu'=>'<b>DEDUCCIÓN</b>'));
	...
	function uf_print_pie_cabecera1($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,$as_pago,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('conforme'=>'','espacio'=>''),array('conforme'=>'<b>Recibi Conforme:</b>_________________________________','espacio'=>''));
	...
	function uf_print_pie_cabecera2($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,$as_pago,&$io_pdf)
	{
		...
		/*if($ls_tiporeporte==0)
		{
			...
			$li_montobsf=$io_monedabsf->uf_convertir_monedabsf($ai_totnet,$_SESSION["la_empresa"]["candeccon"],$_SESSION["la_empresa"]["tipconmon"],1000,$_SESSION["la_empresa"]["redconmon"]);
			$li_montobsf=number_format($li_montobsf,2,",",".");
			$io_pdf->addText(385,50,'9','<b>Neto a Cobrar Bs.F. :</b>  '.$li_montobsf);
		}*/
		unset($la_data);
		unset($la_columna);
		unset($la_config);
		...
		while((!$io_report->rs_data->EOF)&&($lb_valido))
		{
			...
			if($ls_consolidar=="0")
			{
				...
				else
				{
					$li_tm=$io_pdf->getTextWidth(9,$ls_periodo);
					$tm=306-($li_tm/2);
					$io_pdf->addText($tm,330,9,$ls_periodo); // Agregar el título
				}
			}
