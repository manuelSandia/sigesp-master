<?php
	...
	function uf_print_encabezado_pagina1($as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$as_titulo1="INSTITUCION DE PREVISION SOCIAL";
		...
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,695,9,$as_desnom); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_cabecera1($as_cedper,$as_nomper,&$io_cabecera,$as_cuenta,$as_banco,$as_ano,$as_categoria,$as_forma,&$io_pdf)
	{
		...
		$io_pdf->saveState();		
		$io_pdf->ezSetDy(-35);
		$la_data[1]=array('nombre'=>'<b>PENSIONADO: </b>'.$as_nomper,'cedula'=>'<b>CI. </b>'.$as_cedper);			
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nombre'=>array('justification'=>'left','width'=>280),
						 			   'cedula'=>array('justification'=>'left','width'=>280))); // Justificación y ancho de la columna
		...
		$io_pdf->addText(40,522,'10','______________________________________________________________________________________________');
		$io_pdf->ezSetY(520);
		$la_data=array(array('codcon'=>'<b>CODIGO</b>','denomasig'=>'<b>DENOMINACIÓN</b>','cuota'=>'<b>CUOTA / PLAZO</b>', 'valorasig'=>'<b>ASIGNACIÓN</b>','valordedu'=>'<b>DEDUCCIÓN</b>'));
		...
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$io_pdf->addText(40,505,'10','______________________________________________________________________________________________');
		$io_pdf->restoreState();
		$io_pdf->closeObject();
 ...
 //--------------------------------------------------------------------------------------------------------------------------
	...
	else // Imprimimos el reporte
	{
		...
		{  
			...
			if($li_reg==1)
			{
				uf_print_cabecera1($ls_cedper,$ls_nomper,$io_cabecera,$ls_codcueban,$ls_banco,$ls_ano,
				                   $ls_categoria,$ls_forma, $io_pdf); // Imprimimos la cabecera del registro
			}
			$ls_codperi="";
			if($ls_consolidar=="0")
			{
				...
				$tm=306-($li_tm/2);
				$io_pdf->addText($tm,710,9,$ls_periodo); // Agregar el título
			}
			$lb_valido=$io_report->uf_recibopago_conceptopersonal($ls_codnom,$ld_fecdesper,$ld_fechasper,$ls_codper,
																  ...
																  $ls_tituloconcepto,$ls_codperi); // Obtenemos el detalle del reportedel reporte
			if($lb_valido)
			{   
				...
				$lb_valido1=$io_report->uf_recibo_nomina_oficiales($ls_codper,$ls_codnom,$ls_codperides,$ls_codperihas);	
				if ($lb_valido1)
				{
				  ...
				  for($li_j=1;(($li_j<=$li_nom)&&($lb_valido1));$li_j++)
				  {
				    ...
				    uf_detalle_nomina_oficial($io_fun_nomina->uf_formatonumerico($ls_sueldob),
				                            $io_fun_nomina->uf_formatonumerico($ls_prichof),
											$io_fun_nomina->uf_formatonumerico($ls_prianoserv),
				                            $io_fun_nomina->uf_formatonumerico($ls_prides),
											$io_fun_nomina->uf_formatonumerico($ls_noasc),
											$io_fun_nomina->uf_formatonumerico($ls_priesp),
											$io_fun_nomina->uf_formatonumerico($ls_priprof),
											$io_fun_nomina->uf_formatonumerico($ls_porcentaje),
											$io_fun_nomina->uf_formatonumerico($ls_subtotal),
											$ls_fecleypen,
											$io_pdf);
				}
				else
				{
				  uf_detalle_nomina_oficial($io_fun_nomina->uf_formatonumerico(0),
				                            $io_fun_nomina->uf_formatonumerico(0),
											$io_fun_nomina->uf_formatonumerico(0),
				                            $io_fun_nomina->uf_formatonumerico(0),
											$io_fun_nomina->uf_formatonumerico(0),
											$io_fun_nomina->uf_formatonumerico(0),
