<?php
	...
	else  // Imprimimos el reporte
	{
		...
		$li_totpat=0;
		$li_totaling=0;
		$ls_concepto_lph=trim($io_report->uf_select_config("SNO","NOMINA","COD CONCEPTO LPH","LPHLPHLPHL","C"));
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			if ($ls_concepto_lph==$ls_codconc)
			{
				$li_ingresos=$io_report->DS->data["sueintbanavih"][$li_i];
			}
			else
			{
