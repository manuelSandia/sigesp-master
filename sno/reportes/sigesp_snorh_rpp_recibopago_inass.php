<?php
	...
	function uf_print_encabezado_pagina1($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$io_pdf->saveState();
		$as_titulo1=$_SESSION["la_empresa"]["titulo"];
		$tm=40;
		$io_pdf->addText($tm,755,9,'<b><i>'.$as_titulo1.'</i></b>'); // Agregar el título
		$as_titulo1=$_SESSION["la_empresa"]["nombre"];
		$io_pdf->addText($tm,745,9,'<b><i>'.$as_titulo1.'</i></b>'); // Agregar el título
		...
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,720,9,'<b><i>'.$as_desnom.'</i></b>'); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_periodo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,710,9,'<b><i>'.$as_periodo.'</i></b>'); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_encabezado_pagina2($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$as_titulo1=$_SESSION["la_empresa"]["titulo"];
		$tm=40;
		$io_pdf->addText($tm,335,9,'<b><i>'.$as_titulo1.'</i></b>'); // Agregar el título
		$as_titulo1=$_SESSION["la_empresa"]["nombre"];
		$io_pdf->addText($tm,325,9,'<b><i>'.$as_titulo1.'</i></b>'); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(10,$as_titulo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,310,10,$as_titulo); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,300,9,'<b><i>'.$as_desnom.'</i></b>'); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_periodo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,290,9,'<b><i>'.$as_periodo.'</i></b>'); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera1($as_codper,$as_cedper,$as_nomper,$as_descar,$as_codcueban,$as_desuniadm,$ad_fecingper,$as_codcueban,
							    &$io_cabecera,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('codigo'=>array('justification'=>'center','width'=>65), // Justificación y ancho de la columna
						 			   'denominacion'=>array('justification'=>'left','width'=>245), // Justificación y ancho de la columna
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera2($as_codper,$as_cedper,$as_nomper,$as_descar,$as_codcueban,$as_desuniadm,$ad_fecingper,$as_codcueban,
							    &$io_cabecera,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_pie_cabecera1($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,&$io_pdf)
	{
		...
		$io_pdf->ezSety(480);
		$la_data=array(array('codigo'=>'', 'denominacion'=>'', 'asignacion'=>$ai_toting,'deduccion'=>$ai_totded));
		$la_columna=array('codigo'=>'<b>CÓDIGO</b>', 
						  'denominacion'=>'<b>DENOMINACIÓN</b>', 
						  'asignacion'=>'<b>ASIGNACION</b>',
						  'deduccion'=>'<b>DEDUCCION</b>');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('codigo'=>array('justification'=>'center','width'=>65), // Justificación y ancho de la columna
						 			   ...
						 			   'deduccion'=>array('justification'=>'right','width'=>70))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$io_pdf->addText(350,467,'10','________________________________________');
		$io_pdf->addText(40,467,'10','_________________________');
		$la_data=array(array('cuenta'=>'           RECIBE CONFORME', 'neto'=>'<b>TOTAL A COBRAR '.$ls_bolivares.':</b>                    '.$ai_totnet));
	...
	function uf_print_pie_cabecera2($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,&$io_pdf)
	{
		...
		$io_piepagina=$io_pdf->openObject(); // Creamos el objeto pie de página
		$io_pdf->saveState();
		$io_pdf->addText(350,60,'10','________________________________________');
		$io_pdf->ezSety(60);
		$la_data=array(array('codigo'=>'', 'denominacion'=>'', 'asignacion'=>$ai_toting,'deduccion'=>$ai_totded));
		$la_columna=array('codigo'=>'<b>CÓDIGO</b>', 
						  'denominacion'=>'<b>DENOMINACIÓN</b>', 
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('codigo'=>array('justification'=>'center','width'=>65), // Justificación y ancho de la columna
						 			   ...
						 			   'deduccion'=>array('justification'=>'right','width'=>70))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$io_pdf->addText(350,47,'10','________________________________________');
		$io_pdf->addText(40,47,'10','_________________________');
		$la_data=array(array('cuenta'=>'           RECIBE CONFORME', 'neto'=>'<b>TOTAL A COBRAR '.$ls_bolivares.':</b>                    '.$ai_totnet));
		$la_columna=array('cuenta'=>'',
						  'neto'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
	...
	if($ls_consolidar=="1")
	{
		$ls_periodo="Periodos Nro ".$ls_codperides." - ".$ls_codperihas.", del ".$ld_fecdesper." al ".$ld_fechasper."";
	}
	$ls_quincena=3;
	//--------------------------------------------------------------------------------------------------------------------------------
		...
		while((!$io_report->rs_data->EOF)&&($lb_valido))
		{
			...
			if($li_reg==1)
			{
				uf_print_cabecera1($ls_codper,$ls_cedper,$ls_nomper,$ls_descar,$ls_codcueban,$ls_desuniadm,$ld_fecingper,
								   $ls_codcueban,$io_cabecera,$io_pdf); // Imprimimos la cabecera del registro
			}
			else
			{
			...
			if($ls_consolidar=="0")
			{
				...
				if($li_reg==1)
				{
					...
					$tm=306-($li_tm/2);
					$io_pdf->addText($tm,710,9,'<b><i>'.$ls_periodo.'</i></b>'); // Agregar el título
				}
				else
				{
					$li_tm=$io_pdf->getTextWidth(9,$ls_periodo);
					$tm=306-($li_tm/2);
					$io_pdf->addText($tm,290,9,'<b><i>'.$ls_periodo.'</i></b>'); // Agregar el título
				}
			}
