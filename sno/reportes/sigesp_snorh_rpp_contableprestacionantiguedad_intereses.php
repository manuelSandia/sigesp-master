<?php
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo)
	{
		...
		// Fecha Creación: 25/10/2010 
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $io_fun_nomina;
		
		$ls_descripcion="Generó el Reporte ".$as_titulo." Para la Nómina ".$as_desnom." Periodo".$as_periodo;
		$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte("SNR","sigesp_snorh_r_afectacionprestacionantiguedad_intereses.php",$ls_descripcion);
		return $lb_valido;
	}
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	$ls_titulo="<b>Resumen Contable Presupuestario de Intereses Prestacion Antiguedad</b>";
	...
	{
		$lb_valido=$io_report->uf_prestacionantiguedad_afectacionpresupuestaria_intereses($ls_codnom,$ls_anocurper,$ls_mescurper);
	}
	if($lb_valido) // Buscamos la información que afecta contabilidad por el debe
	{
		$lb_valido=$io_report->uf_prestacionantiguedad_afectacioncontable_intereses($ls_codnom,$ls_anocurper,$ls_mescurper);
	}
	...
	else // Imprimimos el reporte
	{
		...
		{
			$ls_codest1=$io_report->rs_data->fields["codestpro1"];
			$ls_codest2=$io_report->rs_data->fields["codestpro2"];
			$ls_codest3=$io_report->rs_data->fields["codestpro3"];
			$ls_codest4=$io_report->rs_data->fields["codestpro4"];
			$ls_codest5=$io_report->rs_data->fields["codestpro5"];
			...
			$ls_cueprecon=$io_report->rs_data->fields["spg_cuenta"];
			$ls_denominacion=$io_report->rs_data->fields["denominacion"];
			$li_total=$io_report->rs_data->fields["monto"];
		...
		{
			$ls_cueconpatcon=trim($io_report->rs_detalle->fields["sc_cuenta"]);
			$ls_denominacion=$io_report->rs_detalle->fields["denominacion"];
			$ls_operacion=$io_report->rs_detalle->fields["debhab"];
			if($ls_operacion=="D")
			{
				$li_debe=abs($io_report->rs_detalle->fields["monto"]);
				$li_haber=0;
			...
			{
				$li_debe=0;
				$li_haber=abs($io_report->rs_detalle->fields["monto"]);
