				...
				{					
					while(!$io_report->rs_data_detalle->EOF)
					{
					   $ls_tipsal=rtrim($io_report->rs_data_detalle->fields["tipsal"]);
						...
						{
							$li_deduccion=abs($io_report->rs_data_detalle->fields["valsal"]);
							$li_totalduduccion=round($li_deduccion/2,2);
						}
					...
					switch($ls_quincena)
					{
						case "1": // primera quincena;
							...
							$li_total=$io_report->rs_data_detalle->fields["valsal"];//Agregado por Carlos Zambrano
							$li_valsal=round($li_total/2,2);
							$li_toting=$li_toting+$li_valsal-$li_totalduduccion;
						...
						case "2": // segunda quincena;
							while(!$io_report->rs_data_detalle->EOF)
							{
								...
								if(($ls_tipsal=="A") || ($ls_tipsal=="V1") || ($ls_tipsal=="V2") || ($ls_tipsal=="R")) // Buscamos las asignaciones
								{
									...
									$li_valsasi=$io_report->rs_data_detalle->fields["valsal"];//Agregado por Carlos Zambrano
									$la_data_a[$li_asig]=array('denominacion'=>$ls_nomcon,'valor'=>$li_valsal);
								}
							...
							$ls_codconc="----------";
							$ls_nomcon="ADELANTO 1ra QUINCENA";
							$li_valsal=round($li_valsasi/2,2)-$li_totalduduccion;
