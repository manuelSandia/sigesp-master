   ...
   function uf_print($as_codescdoc,$as_codcladoc,$as_descladoc,$as_tipo)
   {
		...
		else
		{
			while(!$rs_data->EOF)
			{
				...
				$ls_tiesercladoc=$rs_data->fields["tiesercladoc"];
				$li_suesupcladoc=number_format($rs_data->fields["suesupcladoc"],2,",",".");
				$li_suedircladoc=number_format($rs_data->fields["suedircladoc"],2,",",".");
				$li_suedoccladoc=number_format($rs_data->fields["suedoccladoc"],2,",",".");
				switch ($as_tipo)
				{
					case "":
