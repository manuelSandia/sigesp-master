...
//-----------------------------------------------------------------------------------------------------------------------------------
		...
		else
		{
			...
			while ((!$rs_data->EOF)&&($lb_valido))
			{
				...
				if(($lb_valido)&&($lb_aplica)&&($lb_filtro))// Si se aplica el concepto y el filtro es válido
				{
					...
					if($lb_valido)
					{
						if(($ls_sigcon=="A")||($ls_sigcon=="B")||($ls_sigcon=='X')||($ls_sigcon=='I'))// Si es una Asignación 
						{
	...
	function uf_verificar_montos($as_codper,$as_codconc,$as_sigcon,$ai_valcon,$ai_valconhis)
	{
		...
		//se cambia la condicion a fin de visualizar todo en la prenomina
		if(doubleval($ai_valcon)!=doubleval($ai_valconhis)||$ai_valcon!=0)
		{
	...
	function uf_evaluar_vacacionespersonal($as_codper,$as_perhis,$as_anocurhis)
	{
		...
		if($lb_valido)
		{
			if($_SESSION["la_vacacion"]["envacacion"]==1)
			{
				$li_sueintvac=0;
				$li_suebonvac=0;
				$lb_valido=$this->io_vacacion->uf_load_sueldointegral_vac($as_codper,$li_sueintvac,$_SESSION["la_vacacion"]["profueper"],$_SESSION["la_vacacion"]["fecdisvac"]);
				if($lb_valido)
				{
