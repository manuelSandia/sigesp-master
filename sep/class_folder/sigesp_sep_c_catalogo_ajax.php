<?php
	...
	switch($ls_catalogo)
	{
		case "BIENES":
			uf_print_bienes();
			break;
		case "BIENESSINAFECTACION":
				uf_print_bienes_sinafectacion();
				break;
	...
	function uf_print_bienes()
   	{
		...
		{
			$la_spgctas =  split(",",$rs_data->fields["soc_gastos"]);
			if (!empty($la_spgctas))
			{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_bienes_sinafectacion()
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_print_bienes_sinafectacion
		//		   Access: private
		//	    Arguments:
		//	  Description: M�todo que obtiene e imprime el resultado de la busqueda de los bienes
		//	   Creado Por: Ing. David Brice�o
		// Fecha Creaci�n: 13/05/2013								Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $io_fun_sep;
		require_once("../../shared/class_folder/sigesp_include.php");
		$io_include=new sigesp_include();
		...
		$ls_sql = "SELECT siv_articulo.codart,siv_articulo.denart,siv_articulo.ultcosart,siv_articulo.codunimed,TRIM(siv_articulo.spg_cuenta) AS spg_cuenta, ".
				"		  siv_unidadmedida.denunimed, siv_unidadmedida.unidad, siv_articulo.estartgen, siv_articulo.exiart, ".
				"		  (SELECT COUNT(spg_cuentas.spg_cuenta)  ".
				" 		 FROM spg_cuentas ".
				" 		WHERE spg_cuentas.codestpro1 = '".$ls_codestp1."' ".
				"			  AND spg_cuentas.codestpro2 = '".$ls_codestp2."' ".
		...
		else
		{
			...
			echo "<td style='cursor:pointer' title='Ordenar por Denominacion' align='center' onClick=ue_orden('siv_articulo.denart')>Denominacion</td>";
			echo "<td style='cursor:pointer' title='Ordenar por Unidad'       align='center' onClick=ue_orden('siv_unidadmedida.denunimed')>Unidad</td>";
			echo "<td style='cursor:pointer' title='Ordenar por Cantidad'     align='center' onClick=ue_orden('siv_articulo.exiart')>Cantidad</td>";
			echo "<td></td>";
			echo "</tr>";
			while(!$rs_data->EOF)
			{
				...
				$li_totalcargos  = $rs_data->fields["totalcargos"];
				$ls_estartgen    = $rs_data->fields["estartgen"];
				$ls_exiart	     = $rs_data->fields["exiart"];
				$ls_spg_cuenta   = $rs_data->fields["spg_cuenta"];
				if ($ls_exiart==0)
				{
					$ls_estilo = "celdas-blancas";
				...
				echo "<td align='left'>".$ls_denart."</td>";
				echo "<td align='left'>".$ls_denunimed."</td>";
				echo "<td align='right'>".$ls_exiart."</td>";
				echo "<td style='cursor:pointer'>";
				...
				{
					echo "<a href=\"javascript: alert('No hay disponibilidad de este producto en Almacen');\"><img src='../shared/imagebank/tools20/aprobado.gif' title='Agregar' width='15' height='15' border='0'></a></td>";
				}
				echo  "</tr>";
				$rs_data->MoveNext();
			}
	...
	}// end function uf_print_bienes_sinafectacion
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_proveedor()
   	{
		...
		$io_mensajes=new class_mensajes();		
		require_once("../../shared/class_folder/class_funciones.php");
		//Se agregó el require para operar con la fecha de vencimiento de rnc
		...
		//Se agregó la columna que indica si está o no vigente el proveedor con el RNC y se agregó al query el campo fecvenrnc
		...
		print "</tr>";
        $ls_sql="SELECT cod_pro,nompro,sc_cuenta,rifpro,tipconpro,fecvenince".
				"  FROM rpc_proveedor  ".
                " WHERE codemp = '".$ls_codemp."' ".
				"   AND cod_pro <> '----------' ".
				"   AND estprov = 0 ".
		...
		else
		{
			...
			//Se agregó en el query el campo "fecvenrnc" y se operó con él para mostrar la vigencia de RNC de los proveedores
				...
				if($io_fecha->uf_comparar_fecha($ld_hoy,$ls_fechavenRNC))
				{
					...
					//Ser construyen los links para seleccionar al proveedor
					$lb_linkCaseVacio="<a href=\"javascript:aceptar('$ls_codpro','$ls_nompro','$ls_rifpro','$ls_tipconpro');\">".$ls_codpro."</a>";
					$lb_linkCaseREPDES="<a href=\"javascript:aceptar_reportedesde('$ls_codpro');\">".$ls_codpro."</a>";
					$lb_linkCaseREPHAS="<a href=\"javascript:aceptar_reportehasta('$ls_codpro');\">".$ls_codpro."</a>";
					
				}
				else
				{
					$lb_registronacional="VENCIDO";
					// No se generan los links ya que solo se imprimirá el código del proveedor
					$lb_linkCaseVacio=".$ls_codpro.";
					$lb_linkCaseREPDES=".$ls_codpro.";
					$lb_linkCaseREPHAS=".$ls_codpro.";
				}
			switch ($ls_tipo)
				{
					case "":
	...
	function uf_print_solicitud()
	{
		...
		else
		{
			...
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				$ls_consol= utf8_encode(eregi_replace("[\n|\r|\n\r]",' ',$ls_consol));
				...
				switch ($ls_estsol)
				{
					...
					case "A":
						...
						break;
                                        case "F":
						$ls_estatus="FINALIZADA-ANULADA PARCIAL";
						break;
					...
					case "L":
						...
						break;
					//Se agrega estatus para la falta de disponibilidad presupuestaria
					case "S":
						$ls_estatus="SIN DISPONIBILIDAD PRESUPUESTARIA";
						break;
				}
				$li_i++;
				switch ($ls_tipo)
				{
					case "":
						...
						print "<td align='center'><a href=\"javascript: ue_aceptar('$ls_numsol','$ls_codtipsol','$ls_coduniadm','$ls_codfuefin',".
											...
											"'$ls_estayueco','$ls_tipsepbie','$ls_rif','$li_i','$ls_consolanu','$ls_numdocori');\">".$ls_numsol."</a>";
	...
	function uf_print_unidad_ejecutora()
	{
		...
		if (empty($ls_tipo))
		   {
		     print "<td width=400 style='cursor:pointer' title='Ordenar por Denominaci&oacute;n' align='center' onClick=ue_orden('denuniadm')>Denominaci&oacute;n</td>";
			 print "<td width=40  style='cursor:pointer' title='Seleccionar Estructura Presupuestaria'>Detalle</td>";   
		   }
		else
		   {
		     print "<td width=440 style='cursor:pointer' title='Ordenar por Denominaci&oacute;n' align='center' onClick=ue_orden('denuniadm')>Denominaci&oacute;n</td>";
		   }
		print "</tr>";
		if ($rs_data===false)
		   {
