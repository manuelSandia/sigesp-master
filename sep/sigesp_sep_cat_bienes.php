...
///--------------------------------------------------------------------------------------------------------------------------
function ue_aceptar(as_codart,as_denart,as_unidad,as_spg_cuenta,ai_precio,ai_totalcargos,ai_existecuenta,as_medida)
{
  //---------------------------------------------------------------------------------
  ...
  for (j=1;(j<rowbienes)&&(valido);j++)
	  {
		...
		medida		 = eval("opener.document.formulario.txtmedida"+j+".value");
	...
	parametros  = parametros+"&txtcodart"+rowbienes+"="+as_codart+"&txtdenart"+rowbienes+"="+as_denart+""+			               
						   "&txtcanart"+rowbienes+"=0,00&cmbunidad"+rowbienes+"=D"+"&txtmedida"+rowbienes+"="+as_medida+""+			              
...
/*	total=ue_calcular_total_fila_opener("txtcuentagas");
	...
	parametros=parametros+"&codprounidad="+codestpro1+codestpro2+codestpro3+codestpro4+codestpro5+"&estcla="+ls_estcla+"&coduniadm="+ls_coduniadm+"&tipconpro="+ls_tipconpro; 
...
function ue_search()
{
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	// Enviar todos los campos a la pagina para que haga el procesamiento
	//tiposolicitud = tiposolicitud.replace(/^\s+|\s+$/g,'');
	tiposolicitud = allTrim(tiposolicitud);
	if (tiposolicitud != '03-B-S-')
	{
		catalogo = 'BIENES';
	}
	else
	{
		catalogo = 'BIENESSINAFECTACION';
	} 
	ajax.send("catalogo="+catalogo+"&codart="+codart+"&denart="+denart+"&codtipart="+codtipart+"&codestpro1="+codestpro1+
			  "&codestpro2="+codestpro2+"&codestpro3="+codestpro3+"&codestpro4="+codestpro4+"&codestpro5="+codestpro5+"&estcla="+estcla+
			  "&orden="+orden+"&campoorden="+campoorden+"&tipsepbie="+ls_tipsepbie+"&estartgen="+estartgen+"&codartpri="+codartpri);
