<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estsol,&$io_pdf)
	{
        ...
        $io_pdf->Rectangle(85,700,500,60);
		...
		// cuadro inferior
        $io_pdf->Rectangle(15,60,570,70); // RECTANGULO
		$io_pdf->line(15,117,585,117);	 // HORIZONTAL
        $io_pdf->addText(45,122,7,"UNIDAD SOLICITANTE"); // Agregar el título
		$io_pdf->line(150,60,150,130); //VERTICAL	
        $io_pdf->addText(185,122,7,"UNIDAD DE COMPRA"); // Agregar el título
		$io_pdf->line(290,60,290,130); //VERTICAL	
        $io_pdf->addText(310,122,7,"GERENCIA DE ADMON Y RRHH"); // Agregar el título
		$io_pdf->line(440,60,440,130);	//VERTICAL
        $io_pdf->addText(480,122,7,"PRESIDENCIA"); // Agregar el título
		//$io_pdf->addText(420,63,7,"FIRMA AUTOGRAFA, SELLO, FECHA"); // Agregar el título*/
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		// Fecha Creación: 27/04/2006            Fecha de Modificacion: 07/05/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$io_pdf->ezSetDy(-2);
				...
				if($lb_valido)
				{
					...
					for($li_s=1;$li_s<=$li_totrowdet;$li_s++)
					{
						...
						$la_data[$li_s]=array('codigo'=>$ls_codigo,'denominacion'=>$ls_denominacion,'cantidad'=>$li_cantidad,
											  'cosuni'=>$li_cosuni,'unidad'=>$ls_unidad,'baseimp'=>$li_basimp,'cargo'=>$li_cargos,'montot'=>$li_monart);
					}
					...
					if($lb_valido)
					{
						...
						unset($la_data);
						$lb_valido=$io_report->uf_select_dt_spgcuentasnivel5($ls_numsol); // Cargar el DS con los datos del reporte
						if($lb_valido)
						{
