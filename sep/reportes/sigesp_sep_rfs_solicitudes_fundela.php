<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estsol,&$io_pdf)
	{
		...
		$io_pdf->addText(45,63,7,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(155,122,7,"VERIFICADO POR"); // Agregar el título
		$io_pdf->addText(145,63,7,"JEFE O COORDINADOR"); // Agregar el título
		$io_pdf->addText(284,122,7,"VISTO BUENO"); // Agregar el título
		$io_pdf->addText(287,63,7,"DIRECCION"); // Agregar el título
		$io_pdf->addText(447,122,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(452,63,7,"PRESIDENCIA"); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('item'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
						               'codigo'=>array('justification'=>'center','width'=>85), // Justificación y ancho de la columna
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle_cuentas($la_data,$ls_estmodest,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('codestpro'=>array('justification'=>'center','width'=>270), // Justificación y ancho de la columna
									   ...
									   'item'=>array('justification'=>'center','width'=>100), // Justificación y ancho de la columna	
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				if($lb_valido)
				{
					...
					for($li_s=1;$li_s<=$li_totrowdet;$li_s++)
					{
						...
						$ls_codest1 = $io_report->ds_detalle->data["codestpro1"][$li_s];
						$ls_codest2 = $io_report->ds_detalle->data["codestpro2"][$li_s];
						$ls_codest3 = $io_report->ds_detalle->data["codestpro3"][$li_s];
						$ls_codest4 = $io_report->ds_detalle->data["codestpro4"][$li_s];
						$ls_codest5 = $io_report->ds_detalle->data["codestpro5"][$li_s];
						$ls_estcla  = $io_report->ds_detalle->data["estcla"][$li_s];
						$ls_spg_cuenta = $io_report->ds_detalle->data["spg_cuenta"][$li_s];
						...
						$la_data[$li_s]=array('item'=>$li_s,'codigo'=>$ls_codigo,'denominacion'=>$ls_denominacion,'cantidad'=>$li_cantidad,
											  'unidad'=>$ls_unidad,'cosuni'=>$li_cosuni,'baseimp'=>$li_basimp,'cargo'=>$li_cargos,'montot'=>$li_monart);
						$la_estart[] = array('item'=>$li_s,'codestpro1'=>$ls_codest1,'codestpro2'=>$ls_codest2,'codestpro3'=>$ls_codest3,'codestpro4'=>$ls_codest4,'codestpro5'=>$ls_codest5,'cuenta'=>$ls_spg_cuenta);
					}
					...
					if($lb_valido)
					{
						...
						if($lb_valido)
						{
							...
							for($li_s=1;$li_s<=$li_totrowcuentas;$li_s++)
							{
								...
								$ls_estitems = obtenerItems($ls_codestpro1, $ls_codestpro2, $ls_codestpro3, $ls_codestpro4, $ls_codestpro5, $la_estart,$ls_spgcuenta);
								$la_data[$li_s]=array('codestpro'=>$ls_codestpro,'cuenta'=>$ls_spgcuenta,'item'=>$ls_estitems,'monto'=>$li_montocta);
							}	
