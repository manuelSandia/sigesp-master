<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estsol,&$io_pdf)
	{
		...
		// cuadro inferior
		$io_pdf->line(15,145,585,145);		
		$io_pdf->line(15,145,15,130);		
		$io_pdf->line(585,145,585,130);		
        $io_pdf->Rectangle(15,60,570,70);
		$io_pdf->line(195,130,195,60);	
		$io_pdf->line(390,130,390,60);	
		$io_pdf->line(50,85,165,85);	
		$io_pdf->line(233,85,343,85);	
		$io_pdf->line(432,85,550,85);	
		$io_pdf->addText(20,135,7,"CONFORMADO POR:"); // Agregar el título
		$io_pdf->addText(58,75,8,"DR. NICOLAS HURTADO B."); // Agregar el título
		$io_pdf->addText(241,75,8,"ECON. ELIEZER BLANCO"); // Agregar el título
		$io_pdf->addText(440,75,8,"LIC. HECTOR GOLINDANO"); // Agregar el título
		$io_pdf->addText(70,69,5,"DIRECTOR GENERAL DEL IND"); // Agregar el título
		$io_pdf->addText(243,69,5,"D.G. PLANIFICACIÓN Y PRESUPUESTO"); // Agregar el título
		$io_pdf->addText(433,69,5,"D.G. OFICINA DE ADMINISTRACIÓN Y FINANZAS"); // Agregar el título
		$io_pdf->addText(85,63,5,"(FIRMA Y SELLO)"); // Agregar el título
		$io_pdf->addText(270,63,5,"(FIRMA Y SELLO)"); // Agregar el título
		$io_pdf->addText(470,63,5,"(FIRMA Y SELLO)"); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_firmas(&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
				 		 'cols'=>array('firma1'=>array('justification'=>'left','width'=>600))); // Justificación y ancho de la columna
				...
				if($lb_valido)
				{
					...
					if($lb_valido)
					{
						...
						for($li_s=1;$li_s<=$li_totrowcargos;$li_s++)
						{
							...
							$li_porcar=number_format($io_report->ds_cargos->data["porcar"][$li_s],2,",",".")." %";
							...
							$la_data[$li_s]=array('codigo'=>$ls_codigo,'dencar'=>$li_porcar,'monbasimp'=>$li_monbasimp,
