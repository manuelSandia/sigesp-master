<?php
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$ad_unidadm,$as_estsol,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->line(15,95,585,95);//horizonta2		
		$io_pdf->line(205,60,205,155);//vertical 1		
		$io_pdf->line(325,60,325,155);//vertical 2				
		$io_pdf->line(405,60,405,155);//vertical 3
		$io_pdf->line(495,60,495,155);//vertical 4								
		//CUADRO 1
		
		if(strlen($ad_unidadm)>=53){
			$uniajustada=substr($ad_unidadm,0,strlen($ad_unidadm)-16);
		}
		else if(strlen($ad_unidadm)<53 && strlen($ad_unidadm)>45){
			$uniajustada=substr($ad_unidadm,0,44);
		}
		else if(strlen($ad_unidadm)<=45){
			$uniajustada=$ad_unidadm;
		}
		$io_pdf->addText(65,145,7,"REQUERIDO POR"); // Agregar el título
		$io_pdf->addText(55,85,7,"UNIDAD SOLICITANTE"); // Agregar el título
		$io_pdf->addText(20,75,7, $uniajustada); // Agregar el título
		$io_pdf->addText(20,65,7,"Fecha  ".$ad_fecregsol); // Agregar el título
		//CUADRO 2
		$io_pdf->addText(230,145,7,"REVISADO POR"); // Agregar el título
		$io_pdf->addText(215,85,7,"COORDINADOR DE PROYECTO"); // Agregar el título
		$io_pdf->addText(210,75,7,"Fecha  ".$ad_fecregsol); // Agregar el título
		//CUADRO 3
		$io_pdf->addText(330,145,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(335,85,7,"ADMINISTRACION"); // Agregar el título
		$io_pdf->addText(330,75,7,"Fecha:"); // Agregar el título
		//CUADRO 4
		$io_pdf->addText(420,145,7,"RECIBIDO POR"); // Agregar el título
		$io_pdf->addText(415,85,7,"UNIDAD DE COMPRAS"); // Agregar el título
		$io_pdf->addText(410,75,7,"Fecha:"); // Agregar el título
		//CUADRO 5
		$io_pdf->addText(510,145,7,"DESPACHADO POR"); // Agregar el título
		$io_pdf->addText(505,85,7,"UNIDAD DE ALMACEN"); // Agregar el título
		$io_pdf->addText(500,75,7,"Fecha:"); // Agregar el título
	...
	//----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
	 $ls_titulo='<b>REQUISICION DE MATERIALES, SUMINISTROS Y/0 EQUIPOS</b>';
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				uf_print_encabezado_pagina($ls_titulo,$ls_numsol,$ld_fecregsol,$ls_denuniadm,$ls_estsol,&$io_pdf);
