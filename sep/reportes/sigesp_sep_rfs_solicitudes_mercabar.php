<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estsol,&$io_pdf)
	{
		...
		$ls_titulo2="<b>MERCABAR, RIF:J-08512511-6</b>";
		$ls_titulo3="<b>ZONA INDUSTRIAL III. AV. CARLOS GIFFONI. BARQUSIMETO - LARA - VENEZUELA</b>";
		$io_pdf->saveState();
		...
		$tm2=300-($li_tm2/2);
		$tm3=350-($li_tm3/2);
		$io_pdf->addText($tm,750,8,$as_titulo); // Agregar el título
		$io_pdf->addText($tm2,740,8,$ls_titulo2); // Agregar el título
		$io_pdf->addText($tm3,730,8,$ls_titulo3); // Agregar el título
		...
		$io_pdf->addText(40,122,7,"<b>ELABORADO POR </b>"); // Agregar el título
		$io_pdf->addText(42,63,7,"<b>FIRMA / SELLO / FECHA</b>"); // Agregar el título
		$io_pdf->addText(157,122,7,"<b>PLANIFICACION Y PRESUPUESTO</b>"); // Agregar el título
		$io_pdf->addText(157,63,7,"<b>FIRMA / SELLO / FECHA</b>"); // Agregar el título
		$io_pdf->addText(330,122,7,"<b>ALMACEN</b>"); // Agregar el título
		$io_pdf->addText(310,63,7,"<b>FIRMA / SELLO / FECHA</b>"); // Agregar el título
		$io_pdf->addText(440,122,7,"<b>COORDINACION DE ADQUISICIONES</b>"); // Agregar el título
		$io_pdf->addText(445,63,7,"<b>FIRMA / SELLO / FECHA</b>"); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_detalle_cuentas($la_data,$ls_estmodest,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('codestpro'=>array('justification'=>'center','width'=>160), // Justificación y ancho de la columna
	...
	//----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
	 $ls_titulo='<b>MERCADO MAYORISTA DE ALIMENTOS DE BARQUISIMETO C.A.</b>';
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			$io_pdf->ezSetCmMargins(4.5,6,3,3); // Configuración de los margenes en centímetros
					...
					if($lb_valido)
					{
						...
						if($lb_valido)
						{
							...
							for($li_s=1;$li_s<=$li_totrowcuentas;$li_s++)
							{
								...
								$la_data[$li_s]=array('codestpro'=>$ls_codestpro,'cuenta'=>$ls_spgcuenta,'monto'=>$li_montocta,'denominacion'=>$ls_denominacion);
							}	
