<?php
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_dentipsol,$as_tiposoli,&$io_pdf)
//	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_dentipsol,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// cuadro inferior
		if(strtoupper($as_dentipsol)=="BIENES")
		{
			$io_pdf->addText(90,70,10,"_______________________"); // Agregar el título
			$io_pdf->addText(100,60,10,"UNIDAD SOLICITANTE"); // Agregar el título
			
			$io_pdf->addText(395,70,10,"__________________________"); // Agregar el título
			$io_pdf->addText(400,60,10,"COMPRAS Y SUMINISTROS"); // Agregar el título
		}
		elseif(strtoupper($as_dentipsol)=="SERVICIOS")
		{
			$io_pdf->addText(90,70,10,"_______________________"); // Agregar el título
			$io_pdf->addText(100,60,10,"UNIDAD SOLICITANTE"); // Agregar el título
			
			$io_pdf->addText(395,70,10,"________________________"); // Agregar el título
			$io_pdf->addText(400,60,10,"SERVICIOS GENERALES"); // Agregar el título
		}
		else
		{		
			//$io_pdf->Rectangle(15,60,570,70);
			//$io_pdf->line(15,83,585,83);		
			//$io_pdf->line(15,117,585,117);	//horizontal arriba	
			//$io_pdf->line(290,60,290,130);		
			//$io_pdf->addText(120,122,7,"ELABORADO POR"); // Agregar el título
			//$io_pdf->addText(135,63,7,"FIRMA"); // Agregar el título
			//$io_pdf->addText(420,122,7,"REVISADO"); // Agregar el título
			//$io_pdf->addText(400,73,7,"GERENCIA AREA PROCURA"); // Agregar el título
			//$io_pdf->addText(420,63,7,"FIRMA/SELLO"); // Agregar el título
			
				  // cuadro inferior
            $io_pdf->Rectangle(15,60,570,80); 
		    $io_pdf->line(15,128,585,128);	//HORIZONTAL	
		    //$io_pdf->addText(95,122,7,"ELABORADO"); // Agregar el título
			//$io_pdf->addText(70,71,7,"GERENCIA AREA PROCURA"); // Agregar el título
			$io_pdf->addText(538,35,6,"PC-SGC-FT-028"); // Agregar el título
		   // $io_pdf->addText(70,71,7,"FIRMA"); // Agregar el título
	   		
			
			if(strtoupper($as_tiposoli)=="O")
			{
			//	$io_pdf->addText(20,71,7,"compromiso"); // Agregar el título
				$io_pdf->addText(65,132,7,"ELABORADO"); // Agregar el título
			    $io_pdf->addText(40,65,7,"ANALISTA PRESUP. Y COSTOS"); // Agregar el título
				$io_pdf->addText(200,65,7,"SUPTTE. PRESUP. Y COSTOS"); // Agregar el título	
			}
			else
			{
			//	$io_pdf->addText(20,71,7,"precompromiso"); // Agregar el título
				$io_pdf->addText(45,132,7,"ELABORADO / CONFORMADO"); // Agregar el título
			    $io_pdf->addText(55,71,7,"ANALISTA DE COMPRA /"); // Agregar el		 título
				$io_pdf->addText(51,62,7,"GERENTE AREA PROCURA"); // Agregar el		 título
				$io_pdf->addText(200,71,7,"SUPTTE. PRESUP. Y COSTOS /"); // Agregar el título
				$io_pdf->addText(200,62,7,"ANALISTA PRESUP. Y COSTOS"); // Agregar el título
			}
			
		    $io_pdf->line(170,60,170,140);	//VERTICAL		
		    $io_pdf->addText(205,132,7,"REVISADO Y CONFORMADO"); // Agregar el título    		    
		    $io_pdf->line(335,60,335,140);	//VERTICAL
		    $io_pdf->addText(435,132,7,"APROBADO"); // Agregar el título
		    $io_pdf->addText(400,65,7,"PRESIDENCIA / JUNTA DIRECTIVA "); // Agregar el título
		    
		    $io_pdf->line(15,78,585,78); //HORIZONTAL	
		}
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				$ls_tiposoli=$io_report->DS->data["estope"][$li_i];
				...
				//$ls_titulo='<b>REQUISICIÒN DE '.$ls_dentipsol.'</b>';	
				$ls_titulo='<b>'.$ls_dentipsol.'</b>';				
				//uf_print_encabezado_pagina($ls_titulo,$ls_numsol,$ld_fecregsol,$ls_dentipsol,&$io_pdf);
				uf_print_encabezado_pagina($ls_titulo,$ls_numsol,$ld_fecregsol,$ls_dentipsol,$ls_tiposoli,&$io_pdf);
					...
					if($lb_valido)
					{
						...
						if($lb_valido)
						{
							...
							for($li_s=1;$li_s<=$li_totrowcuentas;$li_s++)
							{
								...
								if($ls_estmodest==1)
								{
									$aux=substr($ls_codestpro1,0,4);
									if ($aux!='PROY')
									{
										$ls_codestpro1=substr($ls_codestpro1,2,5);
									}
									/*else
									{
										$ls_codestpro1=substr($ls_codestpro1,0,5);
									}*/
