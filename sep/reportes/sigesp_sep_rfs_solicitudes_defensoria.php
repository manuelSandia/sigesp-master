<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estsol,&$io_pdf)
	{
		...
		$io_pdf->addText(485,740,9,"<b>No.</b> ".$as_numsol); // Agregar el título
		$io_pdf->addText(485,710,9,"<b>Fecha</b> ".$ad_fecregsol); // Agregar el título
		$io_pdf->addText(540,770,7,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(546,764,6,date("h:i a")); // Agregar la Hora
		// cuadro inferior
        $io_pdf->Rectangle(15,60,570,50);
		$io_pdf->line(300,60,300,110);		
		$io_pdf->addText(40,100,7,"<b>ELABORADO POR: </b>"); // Agregar el título
		$io_pdf->addText(320,100,7,"<b>APROBADO POR: </b>"); // Agregar el título
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	//----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
	 $ls_titulo='<b>SOLICITUD DE COMPRAS</b>';
	//--------------------------------------------------------------------------------------------------------------------------------
					...
					if($lb_valido)
					{
						...
						if($lb_valido)
						{
							...
							for($li_s=1;$li_s<=$li_totrowcuentas;$li_s++)
							{
								...
								$ls_spgcuenta = trim($io_report->ds_cuentas->data["spg_cuenta"][$li_s]);
								if($ls_estmodest==1)
								{
