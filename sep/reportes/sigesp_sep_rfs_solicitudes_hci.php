<?php
	...
	//  ORGANISMO: HOSPITAL CARDIOLOGICO INFANTIL
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estsol,&$io_pdf)
	{
		...
		$io_pdf->line(40,105,590,105);	//Horizontal
		$io_pdf->addText(100,110,7,"ELABORADO POR: ");      // Agregar texto
		$io_pdf->addText(110,65,7,"COMPRAS");      // Agregar texto
		$io_pdf->line(220,60,220,125);	//vertical
		$io_pdf->line(40,75,590,75);	//Horizontal
		$io_pdf->addText(285,110,7,"CONTRALORÍA INTERNA");      // Agregar texto
		$io_pdf->line(320,60,320,105);	//vertical
		$io_pdf->addText(235,65,7," PRESUPUESTO                               CONTRALORÍA ");      // Agregar texto
		$io_pdf->line(420,60,420,125);	//vertical
		$io_pdf->addText(460,110,7,"COMPRA AUTORIZADA");      // Agregar texto
		$io_pdf->addText(425,65,7,"DIRECTOR DE ADMINISTRACION Y FINANZAS");      // Agregar texto		
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			$io_pdf->ezSetCmMargins(3.6,4.7,3,3); // Configuración de los margenes en centímetros
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				if($lb_valido)
				{
					...
					for($li_s=1;$li_s<=$li_totrowdet;$li_s++)
					{
						...
						$la_data[$li_s]=array('codigo'=>$ls_codigo,'denominacion'=>$ls_denominacion,
						                      'cantidad'=>number_format($li_cantidad,2,',','.'),
