<?php
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_estsol,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//		 cuadro inferior
		...
		$io_pdf->line(15,130,585,130);
		$io_pdf->line(15,68,585,68);			
		$io_pdf->line(105,60,105,140);
		$io_pdf->line(198,60,198,140);	
		$io_pdf->line(317,60,317,140);			
		$io_pdf->line(405,60,405,140);
		$io_pdf->line(495,60,495,140);
		//------
		$io_pdf->addText(22,132,7,"UNIDAD SOLICITANTE"); // Agregar el título
		$io_pdf->addText(35,62,7,"FIRMA / SELLO"); // Agregar el título
		//------
		$io_pdf->addText(117,132,7,"JEFE DE COMPRAS"); // Agregar el título
		$io_pdf->addText(124,62,7,"FIRMA / SELLO"); // Agregar el título
		//------
		$io_pdf->addText(203,132,7,"DIR DE SERV ADMINISTRATIVOS"); // Agregar el título
		$io_pdf->addText(230,62,7,"FIRMA / SELLO"); // Agregar el título
		//------
		$io_pdf->addText(335,132,7,"PRESUPUESTO"); // Agregar el título
		$io_pdf->addText(335,62,7,"FIRMA / SELLO"); // Agregar el título
		//------
		$io_pdf->addText(415,132,7,"ALMACEN O BIENES"); // Agregar el título
		$io_pdf->addText(425,62,7,"FIRMA / SELLO"); // Agregar el título
		//------
		$io_pdf->addText(515,132,7,"PRESIDENCIA"); // Agregar el título
		$io_pdf->addText(515,62,7,"FIRMA / SELLO"); // Agregar el título
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle($la_data,&$io_pdf,$autcan=false)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if ($autcan){
			...
			$la_config=array('showHeadings'=>1, // Mostrar encabezados
					         ...
					         'outerLineThickness'=>0.5,
							 'innerLineThickness' =>0.5,
							 'cols'=>array('codigo'=>array('justification'=>'center','width'=>110), // Justificación y ancho de la columna
										   ...
										   'cantidadorg'=>array('justification'=>'left','width'=>45), // Justificación y ancho de la columna	
							 			   'unidad'=>array('justification'=>'center','width'=>45), // Justificación y ancho de la columna
							 			   'cosuni'=>array('justification'=>'right','width'=>50), // Justificación y ancho de la columna
										   'baseimp'=>array('justification'=>'right','width'=>60), // Justificación y ancho de la columna
							 			   'cargo'=>array('justification'=>'right','width'=>60), // Justificación y ancho de la columna
							 			   'montot'=>array('justification'=>'right','width'=>45))); // Justificación y ancho de la columna
			...
			$la_config=array('showHeadings'=>1, // Mostrar encabezados
							 ...
							 'cols'=>array('codigo'=>array('justification'=>'center','width'=>125), // Justificación y ancho de la columna
							 			   ...
							 			   'cantidad'=>array('justification'=>'left','width'=>45), // Justificación y ancho de la columna
							 			   'unidad'=>array('justification'=>'center','width'=>45), // Justificación y ancho de la columna
	...
	//--------------------------------------------------  Parámetros para Filtar el Reporte  -----------------------------------------
	 ...
	 $li_autcan=$io_fun_sep->uf_obtenervalor_get("autcan",0);
	//--------------------------------------------------------------------------------------------------------------------------------
	 global $ls_tipoformato;
	 if($ls_tipoformato==1)
	 {
	...
	$ls_titulo='<b>SOLICITUD DE EJECUCION PRESUPUESTARIA / REQUISICION</b>';
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				uf_print_encabezado_pagina($ls_titulo,$ls_numsol,$ld_fecregsol,$ls_estsol,&$io_pdf);
				...
				if($li_autcan==0){
					...
					$lb_valido=$io_report->uf_select_dt_solicitud($ls_numsol,true); // Cargar el DS con los datos del reporte
				}
				if($lb_valido)
				{
					...
					for($li_s=1;$li_s<=$li_totrowdet;$li_s++)
					{
						...
						if($li_autcan==1){
							$li_cantori=$io_report->ds_detalle->data["cantori"][$li_s];
						}
						...
						if($li_autcan==0){
							$la_data[$li_s]=array('codigo'=>$ls_codigo,'denominacion'=>$ls_denominacion,'cantidad'=>$li_cantidad,
											  'unidad'=>$ls_unidad,'aprob'=>'','cosuni'=>$li_cosuni,'baseimp'=>$li_basimp,'cargo'=>$li_cargos,'montot'=>$li_monart);
						}
						else{
							$la_data[$li_s]=array('codigo'=>$ls_codigo,'denominacion'=>$ls_denominacion,'cantidad'=>$li_cantidad,'cantidadorg'=>$li_cantori,
											  'unidad'=>$ls_unidad,'aprob'=>'','cosuni'=>$li_cosuni,'baseimp'=>$li_basimp,'cargo'=>$li_cargos,'montot'=>$li_monart);
						}
					}
					if($li_autcan==0){
						...
						uf_print_detalle($la_data,&$io_pdf,true);
					}
					unset($la_data);
