  ...
  function aceptar(cuenta,denominacion,status,ls_coddestino,ls_dendestino)
  {
	...
	if (ls_coddestino!="txtctacon")
	...
	else
	   {
		 ...
		 if (ls_opener=="sigesp_saf_d_depreciacion.php")
		    {
			  fop = opener.document.form1;
			  ls_txtorigen = fop.txtorigen.value;			  
			  if (ls_txtorigen=='GD')
			     {
				   fop.txtctaspg.value = cuenta;
				   fop.txtdenctaspg.value = denominacion;
				 }
			  else if (ls_txtorigen=='DA' || ls_txtorigen=='')
			     {
				   fop.txtctacon.value=cuenta;
			       fop.txtdenctacon.value=denominacion;
				 }
			}	
	     else
		    {
