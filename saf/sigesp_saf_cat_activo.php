		...
		if (array_key_exists("txthasta",$_POST))
		{
			...
			if($lb_fechavalida)
			{
				...
				$ls_sql="SELECT saf_activo.*, ".
					    ...
					    "        (SELECT max(dencat) ".
						"          FROM saf_catsudeban ".
						"		  WHERE saf_catsudeban.codemp = saf_activo.codemp".
						"           AND saf_catsudeban.codcat = saf_activo.codcat) as dencat ".   
						"  FROM saf_activo".	
	...
	if($li_numrows>0)
	{
		while($row=$io_sql->fetch_row($rs_cta))
		{
			...
			print " <td><a href=\"javascript: aceptar('$ls_codact','$ls_maract','$ls_modact','$ld_fecregact','$ld_feccmpact',".
				   ...
				   "'$ls_dendestino','$ls_codgru','$ls_codsubgru','$ls_codsec','$ls_codite','$ls_dengru','$ls_densubgru','$ls_densec',".
				   "'$ls_denite','$ls_denspg','$ls_clasif','$ls_denact','$ls_obsact','$ls_codcat','$ls_dencat');\">".$ls_codact."</a></td>";
...
<script language="JavaScript">   
  function aceptar(ls_codact,ls_maract,ls_modact,ld_fecregact,ld_feccmpact,li_cosact,ls_codconbie,ls_denconbie,ls_codfuefin,
				   ...
				   ls_dengru,ls_densubgru,ls_densec,ls_denite,ls_denspg, ls_clasif,ls_denact,ls_obsact,ls_codcat,ls_dencat)
  {
	f=document.form1;
	...
	else
	{
		...
		opener.document.form1.txtcodsudeban.value=ls_codcat;		
		opener.document.form1.txtdensudeban.value=ls_dencat;		
		if (ls_clasif!="")
		{
