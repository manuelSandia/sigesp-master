	...
	function uf_print($as_codper, $as_cedper, $as_nomper, $as_apeper, $as_codnom, $as_tipo,$as_respced,
	                  $as_codresced,$as_buscartest,$as_codreserec,$as_buscar)
   	{
		...
		$ls_sql=$ls_sql."  AND sno_personal.estper = '1'
		                   AND (sno_personal.codper IN (SELECT codper
		  ...
		  while(!$rs_data->EOF)
			   {
				...
				switch ($as_tipo)
				{
                   ...
                   case "asignacion3":
					...
					case "retornoactivos":
						print "<tr class=celdas-blancas>";
						print "<td><a href=\"javascript: uf_aceptar_personal('$ls_codper','$ls_nomper','$ls_apeper');\">".$ls_codper."</a></td>";
						print "<td>".$ls_cedper."</td>";
						print "<td>".$ls_nomper."  ".$ls_apeper."</td>";
						print "<td>".$ls_estper."</td>";
						print "</tr>";			
...
function uf_aceptar_personal(as_codper,as_nomper,as_apeper)
{
  fop = opener.document.form1;
  fop.txtcodbenpre.value = as_codper;
  fop.txtnombenpre.value = as_nomper+', '+as_apeper;
  close();
}
</script>
</html>
