<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_siv_transferencias.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[5]="";
		$li_widthtable=700;
		$ls_titletable="Comprobantes Transferencias";
		$ls_nametable="grid";
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_comprobante,$ld_fecemi;
		
		$ld_fecemi=$_POST["txtfecemi"];
		$ls_comprobante=$_POST["txtnumtra"];
   }
   //--------------------------------------------------------------
...
<html>
<head>
<title>Reverso del Despacho</title>
...
<?php
	...
	uf_limpiarvariables();
	$lb_estcencos=$_SESSION["la_empresa"]["estcencos"];
	if($lb_estcencos==='0')
	{
		print("<script language=JavaScript>");
		print(" alert('Esta definición esta desactiva cuando las empresas no están configuradas para trabajar con centro de costos.');");
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ld_fecemision=$_POST["txtfecemi".$li_i];
					$ld_fechaconta=$_POST["txtfechaconta".$li_i];
					$lb_valido = $in_class->uf_reversar_contabilizacion_transferencia($ls_comprobante,$ld_fecemision,$ld_fechaconta,$la_seguridad);
					if($lb_valido)
					{
		...
		case "BUSCAR":
			uf_load_variables();
		    $in_class_contabiliza->uf_select_comprobantes_transferencias($ls_comprobante,$ld_fecemi,1,$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reversar Contabilizaci&oacute;n de Despacho </td>
    </tr>
    <tr>
      <td width="182"  height="23"><div align="right">Transferencia </div>
          <div align="right"></div></td>
      <td width="566" ><input name="txtnumtra" type="text" id="txtnumtra" onKeyUp="javascript: ue_validarcomillas(this);" maxlength="15"></td>
    </tr>
    <tr>
      <td  height="23"><div align="right">Fecha de Transferencia </div>
          <div align="right"></div></td>
      <td ><input name="txtfecemi" type="text" id="txtfecemi" size="14" maxlength="10"  datepicker="true" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);"></td>
    </tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_siv_transferencias.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numtra,fecemi)
{
	Xpos=((screen.width/2)-(500/2)); 
	Ypos=((screen.height/2)-(400/2));
	window.open("sigesp_mis_pdt_siv_transferencias.php?numtra="+numtra+"&fecemi="+fecemi+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
