<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_scbop.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		global $io_ddlb,$ls_numdoc,$ls_fecdoc,$ls_fecha;
...
<html>
<head>
<title>Contabilizar Orden de Pago Directa</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_contabilizacion_orden_pago($ls_codban,$ls_ctaban,$ls_numdoc,$ls_codope,
																				 $ls_estmov,$ls_fecmov,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Documento Orden de Pago ".$ls_numdoc." fue contabilizado.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar el documento Orden de Pago ".$ls_numdoc);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_banco_contabilizar_op($ls_numdoc,$ls_fecdoc,"OP",$lo_object,$li_totrows,"N");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>	
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Orden de Pago Directa </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_scbop.php";
			f.submit();
		}
		else
		{
