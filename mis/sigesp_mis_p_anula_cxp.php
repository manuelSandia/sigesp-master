<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_anula_cxp.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_fecha,$ls_numsol,$ls_fecemi,$ls_fecapr,$ls_codigo,$ls_tipo;
...
<html>
<head>
<title>Anulaci&oacute;n de Contabilizaci&oacute;n de Cuentas por Pagar</title>
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_anular_solicitud($ls_numsol,$ls_fecha,$ld_fechaconta,$ls_conanu,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Solicitud de Pago  ".$ls_numsol." fue Anulada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo Anular la Solicitud de Pago ".$ls_numsol);
					}		
				}
			}
			$li_totrows=1;
...
?>	
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Anulaci&oacute;n de Solicitudes de Cuentas por Pagar </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_anula_cxp.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numsol)
{
	...
	window.open("sigesp_mis_pdt_cxp.php?numsol="+numsol+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
function ue_cargarconcepto(row)
{
	...
	if(eval("f.chksel"+row+".checked")==true)
	{
		Xpos=((screen.width/2)-(500/2)); 
		Ypos=((screen.height/2)-(400/2));
		window.open("sigesp_mis_pdt_conceptoanula_sol.php?row="+row+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=400,height=180,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
	}
}
