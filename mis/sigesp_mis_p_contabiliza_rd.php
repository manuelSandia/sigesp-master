<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_rd.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_contabilizacion_recepcion($ls_numrecdoc,$ls_codtipdoc,$ls_codpro,$ls_cedbene,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Recepcion de Documentos  ".$ls_numrecdoc." fue contabilizada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar la Recepcion de Documentos ".$ls_numrecdoc);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
	    	...
	    	$in_class_contabiliza->uf_select_recepciones_contabilizar($ls_numrecdoc,$ls_fecemi,$ls_fecapr,$ls_codigo,$ls_tipo,
																	  $lo_object,$li_totrows,"R","CONTABILIZAR");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>	
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Recepciones de Documentos </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_rd.php";
			f.submit();
		}
		else
		{
