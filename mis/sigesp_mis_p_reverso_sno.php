<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_sno.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $li_total_record,$ldt_fecha,$ls_operacion,$lo_title,$li_totrows,$oi_fun_integrador;
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_codnom,$ls_desnom,$ls_codperi,$ls_fecper,$ls_codcom, $ls_tipcom;
...
<html>
<head>
<title>Reverso de la N&oacute;mina</title>
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido = $in_class->uf_reversar_contabilizacion_nomina($ls_comprobante,$ld_fechaconta,$la_seguridad);
					if($lb_valido)
					{
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reversar Contabilizaci&oacute;n  Nómina </td>
    </tr>
    <tr>
      <td width="116"  height="23"><div align="right">N&oacute;mina</div></td>
      <td width="632" ><input name="txtcodnom" type="text" id="txtcodnom" size="8" maxlength="4" readonly>
    ...
    <tr>
      ...
      <td ><label>
        <select name="cmbtipcom" id="cmbtipcom">
          ...
          <option value="P">Prestación Antiguedad</option>
          <option value="K">Intereses Prestación Antiguedad</option>
          <option value="L">Liquidación</option>
          <option value="X">Anticipo de Prestaciones</option>
        </select>
      </label></td>
    </tr>
    <tr>
      <td  height="23"><div align="right">Comprobante</div></td>
      <td ><input name="txtcodcom" type="text" id="txtcodcom" onKeyUp="javascript: ue_validarcomillas(this);" value="<?PHP print $ls_codcom; ?>" maxlength="15"></td>
    </tr><tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_sno.php";
			f.submit();
		}
		else
		{
...
{
	window.open("sigesp_mis_cat_nomina.php?tipo=reversar","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
}
...
function ue_buscarperiodo()
{
	...
	window.open("sigesp_mis_cat_periodo.php?tipo=reversar&codnom="+codnom+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
}
