<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_revanula_asignacion_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[5]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Asignaciones Anuladas";
		$ls_nametable="grid";
...
<html>
<head>
<title>Reverso de la Anulaci&oacute;n de la asignaci&oacute;n</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_reverso_anulacion_asignacion($ls_codasi,$ld_fechaanula,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Asignación  ".$ls_codasi." fue Reversada la Anulación.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo Reversar la Anulación ".$ls_codasi);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_sob_asignaciones_contabilizar($ls_codasi,$ls_codobr,$ls_fecasi,$ls_codigo,2,
...
<p>&nbsp;</p>
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de la Anulaci&oacute;n  de Asignaci&oacute;n</td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_revanula_asignacion_sob.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codasi)
{
	...
	window.open("sigesp_mis_pdt_sobasi.php?codasi="+codasi+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
