<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_depreciacion_saf.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[5]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Depreciaciones por Contabilizar";
		$ls_nametable="grid";
...
<html>
<head>
<title>Contabilizar Depreciaci&oacute;n de Activos Fijos</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ls_mescomp=$_POST["txtmes".$li_i];
					$ls_anio=$_POST["txtano".$li_i];
					$lb_valido=$in_class->uf_procesar_contabilizacion_depreciacion($ls_comprobante,$ls_descripcion,$ls_mescomp,$ls_anio,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Depreciación del Año ".$ls_anio." Mes ".$ls_mescomp." fue contabilizada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar la Depreciación del Año ".$ls_anio." Mes ".$ls_mescomp);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_activos_contabilizar($ls_mes,$ls_ano,"0",$lo_object,$li_totrows);
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Depreciaci&oacute;n de Activos Fijos </td>
    </tr>
    <tr>
    ...
    <tr>
      <td  height="23"><div align="right">A&ntilde;o</div></td>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		if((f.cmbmes.value!="00")&&(f.txtano.value!=""))
		{
			...
			if(lb_valido)
			{
				...
				f.action="sigesp_mis_p_contabiliza_depreciacion_saf.php";
				f.submit();
			}
			else
			{
...
function uf_verdetalle(mes,ano)
{
	...
	window.open("sigesp_mis_pdt_saf.php?mes="+mes+"&ano="+ano+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
