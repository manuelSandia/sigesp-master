<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_anula_rd.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		global $ls_tipo_operacion,$io_ddlb,$ls_numdoc,$ls_fecdoc,$ls_fecha;
		
		$ls_fecha=date("d/m/Y");	
        $lo_title[1]="";
		$lo_title[2]="Nº Recepción";
		$lo_title[3]="Proveedor/Beneficiario";
		$lo_title[4]="Fecha Registro";
		$lo_title[5]="Concepto";
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Recepciones por Anular";
		$ls_nametable="grid";
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_fecha,$ls_numrecdoc,$ls_fecemi,$ls_fecapr,$ls_codigo,$ls_tipo;
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][2]="<input type=text name=txtnumrecdoc".$ai_totrows." class=sin-borde readonly style=text-align:center size=15 maxlength=15>";
		$aa_object[$ai_totrows][3]="<input type=text name=txtnomproben".$ai_totrows." class=sin-borde readonly style=text-align:center size=15 maxlength=15>";
		$aa_object[$ai_totrows][4]="<input type=text name=txtfecemidoc".$ai_totrows." class=sin-borde readonly style=text-align:left size=30 maxlength=254>";
		$aa_object[$ai_totrows][5]="<input type=text name=txtconsol".$ai_totrows." class=sin-borde readonly style=text-align:center size=18 maxlength=22>";
...
<html>
<head>
<title>Anulación de Contabilización de Cuentas por Pagar</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1"><link href="../shared/css/cabecera.css" rel="stylesheet" type="text/css">
...
<?php
	...
	{
		print("<script language=JavaScript>");
		print(" alert('Esta definición esta desactiva cuando las recepciones no se contabilizan.');");
	...
	$in_class = new class_sigesp_cxp_integracion();  
	uf_limpiarvariables();
	switch ($ls_operacion) 
	{
		case "NUEVO":
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_anula_recepcion($ls_numrecdoc,$ls_codtipdoc,$ls_codpro,$ls_cedbene,$ls_fecha,$ld_fechaconta,$ls_conanu,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Recepcion de Documentos  ".$ls_numrecdoc." fue Anulada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo Anular la Recepcion de Documentos ".$ls_numrecdoc);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_recepciones_contabilizar($ls_numrecdoc,$ls_fecemi,$ls_fecapr,$ls_codigo,$ls_tipo,
																	  $lo_object,$li_totrows,"C","ANULAR");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>	
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Anulaci&oacute;n de Recepciones de Documentos </td>
    </tr>
    <tr>
      <td width="120"  height="23"><div align="right">N&uacute;mero de Recepci&oacute;n </div></td>
    ...
    <tr>
      <td  height="23"><div align="right">Fecha Emisi&oacute;n </div></td>
      <td ><div align="left">
          <input name="txtfecemi" type="text" id="txtfecemi" size="14" maxlength="10" datepicker="true" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);" >
      </div></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td ><div align="left">
        <input name="txtfecha" type="text" id="txtfecha" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);" value="<?php print $ls_fecha; ?>" size="14" maxlength="10" datepicker="true" >
      </div></td>
    </tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_anula_rd.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codrecdoc)
{
	Xpos=((screen.width/2)-(500/2)); 
	Ypos=((screen.height/2)-(400/2));
	window.open("sigesp_mis_pdt_rd.php?codrecdoc="+codrecdoc+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
...
function ue_cargarconcepto(row)
{
	...
	if(eval("f.chksel"+row+".checked")==true)
	{
		Xpos=((screen.width/2)-(500/2)); 
		Ypos=((screen.height/2)-(400/2));
		window.open("sigesp_mis_pdt_conceptoanula_rd.php?row="+row+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=400,height=180,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
	}
}
