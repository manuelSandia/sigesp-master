<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_scbcol.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$la_value=array("ND","NC","DP");
		$la_text=array("Nota Débito","Nota Crédito","Depósito");
		$ls_numdoc="";
		$ls_fecdoc="";
		$ls_tipo_operacion="";
        $lo_title[1]="";
		...
		$lo_title[5]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Movimientos de Colocaciones por Contabilizar";
		$ls_nametable="grid";
...
<html>
<head>
<title>Contabilizaci&oacute;n de Colocaciones</title>
...
<?php
	...
	require_once("class_folder/class_sigesp_scbcol_integracion.php");  	
	$in_class = new class_sigesp_scbcol_integracion();  
	uf_limpiarvariables();
	switch ($ls_operacion) 
	{
		case "NUEVO":
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ls_numcol=$_POST["txtnumcol".$li_i];
					$ls_fecmov=$_POST["txtfecmov".$li_i];		
					$ls_conmov=$_POST["txtconmov".$li_i];
					$ls_codban=$_POST["txtcodban".$li_i];
					$ls_ctaban=$_POST["txtctaban".$li_i];
					$ls_estcol=$_POST["txtestcol".$li_i];
					$ls_codope=$_POST["txtcodope".$li_i];
					$lb_valido=$in_class->uf_procesar_contabilizacion_colocacion($ls_codban,$ls_ctaban,$ls_numcol,$ls_numdoc,
																				 $ls_codope,$ls_estcol,$ls_fecmov,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("El movimiento de colocación ".$ls_numdoc." fue contabilizado.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar el movimiento de colocacion ".$ls_numdoc);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_colocacion_contabilizar($ls_numdoc,$ls_fecdoc,$ls_tipo_operacion,$lo_object,$li_totrows,"N");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>	
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Colocaciones </td>
    </tr>
    <tr>
      <td width="159"  height="23"><div align="right">N&uacute;mero de Documento </div></td>
      <td width="589" >
        <div align="left">
          <input name="txtnumdoc" type="text" id="txtnumdoc" onKeyUp="javascript: ue_validarcomillas(this);" maxlength="15">
        </div></td>
    </tr>
    <tr>
      <td  height="23"><div align="right">Fecha del Documento </div></td>
      <td ><div align="left">
        <input name="txtfecdoc" type="text" id="txtfecdoc" size="14" maxlength="10" datepicker="true" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);" >
      </div></td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_scbcol.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numdoc,codban,ctaban,codope,numcol)
{
	Xpos=((screen.width/2)-(500/2)); 
	Ypos=((screen.height/2)-(400/2));
	window.open("sigesp_mis_pdt_scbcol.php?numdoc="+numdoc+"&codban="+codban+"&ctaban="+ctaban+"&codope="+codope+"&numcol="+numcol+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
