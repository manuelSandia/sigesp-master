<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_sfc_ded.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
<html>
<head>
<title>Reverso de Contabilizaci&oacute;n de Deducciones</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_reversar_contabilizacion_comprobantes_cxc($ls_comprobante,$ld_fecha,$ls_procede,$ls_descripcion,$ld_fechaconta,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Deduccion  ".$ls_comprobante." fue Reversada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo Reversar la Deduccion ".$ls_comprobante);
					}		
				}
			}
			uf_limpiarvariables();
		...
		case "BUSCAR":
			uf_load_variables();
			$in_class_contabiliza->uf_select_comprobantes_sfc_cxc($ls_comprobante,"SFCDED",$ld_fecha,"1",$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
    ...
    <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
      <tr class="titulo-ventana">
        <td colspan="2">Reverso de Contabilizaci&oacute;n de Deducciones </td>
      </tr>
      <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_sfc_ded.php";
			f.submit();
		}
		else
		{
