<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_sfc_nc.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
<html>
<head>
<title>Reverso de Contabilizaci&oacute;n de Notas de Cr&eacute;dito</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_reversar_contabilizacion_comprobantes_nc($ls_comprobante,$ld_fecha,$ls_procede,$ls_descripcion,$ld_fechaconta,$la_seguridad);
					if($lb_valido)
					{
		...
		case "BUSCAR":
			uf_load_variables();
			$in_class_contabiliza->uf_select_comprobantes_sfc_nc($ls_comprobante,"SFCNCR",$ld_fecha,"1",$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
    ...
    <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
      <tr class="titulo-ventana">
        <td colspan="2">Reverso de Contabilizaci&oacute;n de Notas de Cr&eacute;dito </td>
      </tr>
      <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_sfc_nc.php";
			f.submit();
		}
		else
		{
