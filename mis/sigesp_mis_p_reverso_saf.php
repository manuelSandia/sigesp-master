<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_saf.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[5]="Detalle";
		$li_widthtable=700;
		$ls_titletable= "Desincorporaciones por Reversar";
		$ls_nametable = "grid";
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][3]="<input type=text name=txtfecmov".$ai_totrows." class=sin-borde readonly style=text-align:left size=15 ><input type=hidden name=txtfeccon".$ai_totrows." readonly>";
...
<html>
<head>
<title>Reverso de Desincorporaci&oacute;n de Activos</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ls_feccon=$_POST["txtfeccon".$li_i];
					$ls_fecmov=$_POST["txtfecmov".$li_i];
					$lb_valido=$in_class->uf_reversar_contabilizacion_desincorporacion($ls_numdoc, $ls_feccon, $ls_fecmov, $la_seguridad);
					if($lb_valido)
					{
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_desincorporacion_reverso($ls_numdoc,$ls_fecdoc,$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de Activos</td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_saf.php";
			f.submit();
		}
		else
		{
