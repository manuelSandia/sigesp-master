<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_variacion_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[3]="Nº Variación";
		$lo_title[4]="Fecha ";
		$lo_title[5]="Observación"; 
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Variaciones por Contabilizar";
		$ls_nametable="grid";
   ...
   function uf_load_variables()
   {
		...
		// Fecha Creación: 30/04/2007 									Fecha Última Modificación : 
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_fecha,$ls_codvar,$ls_codcon,$ls_feccon,$ls_fecha;
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][3]="<input type=text name=txtcodvar".$ai_totrows." class=sin-borde readonly style=text-align:left size=30 maxlength=30>";
...
<html>
<head>
<title>Contabilizar Variaciones</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ls_codvar=$_POST["txtcodvar".$li_i];
					$ls_fecha=$_POST["txtfecha".$li_i];
					$ld_fechacontacontrato=$_POST["txtfechacontacontrato".$li_i];
					$lb_valido=$in_class->uf_procesar_contabilizacion_variacion($ls_codcon,$ls_codvar,$ls_fecha,$ld_fechacontacontrato,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Variacion ".$ls_codvar." del Contrato ".$ls_codcon." fue contabilizada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar La Variacion ".$ls_codvar." del Contrato ".$ls_codcon);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_sob_variaciones_contabilizar($ls_codvar,$ls_codcon,$ls_feccon,$ls_fecha,0,$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<p>&nbsp;</p>
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Variaciones </td>
    </tr>
    <tr>
    ...
    <tr>
      <td  height="23"><div align="right">N&uacute;mero de Variaci&oacute;n </div></td>
      <td ><div align="left">
          <input name="txtcodvar" type="text" id="txtcodvar" size="10" maxlength="6">
      </div></td>
    </tr>
    ...
    <tr>
      <td width="120"  height="23"><div align="right">Fecha de Variaci&oacute;n</div></td>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_variacion_sob.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codvar,codcon)
{
	Xpos=((screen.width/2)-(500/2)); 
	Ypos=((screen.height/2)-(400/2));
	window.open("sigesp_mis_pdt_sobvar.php?codvar="+codvar+"&codcon="+codcon+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
</script>
