...
class class_sigesp_soc_integracion
{
	...
	function class_sigesp_soc_integracion()
	{
		...
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 20/12/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function uf_contabilizar_ordencompra($as_numordcom,$as_estcondat,$adt_fecha,$aa_seguridad)
	{
		...
		//	  Description: Este metodo es el m�todo principal que genera la contabilizaci�n de la orden de compra  
		//	   Creado Por: Ing. Wilmer Brice�o
        ...
        if($lb_valido) 	
		{  // se genera la contabilizaci�n de la orden de compras
		...
		if($lb_valido)
		{
			...
			$ls_descripcion="Contabiliz� la Orden de Compra <b>".$as_numordcom."</b>, Estatus <b>".$ls_estatus."</b>, ".
							"Fecha <b>".$adt_fecha."</b>";
			...
			while($row=$this->io_sql->fetch_row($rs_data)and($lb_valido))
			{
				...
				if(!$lb_existe)
				{
					if($this->io_fecha->uf_comparar_fecha($ld_fechaconta,$ldt_fecha))
					{
						...
						if(!$lb_valido)
						{ 
							$this->io_msg->message("ERROR -> Al reversar precompromiso SEP N�=".$ls_numsol." ".$this->io_sigesp_int->is_msg_error);
							$lb_valido=false;
							return false;		   
						}
						else
						{
							...
							else
							{
								...
								if($lb_valido)
								{
									/////////////////////////////////         SEGURIDAD               /////////////////////////////		
									$ls_evento="PROCESS";
									$ls_descripcion="Reverso el Precompromiso de la Solicitud de Ejecuci�n Presupuestaria <b>".$ls_numsol."</b> ";
									$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
																	$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
																	$aa_seguridad["ventanas"],$ls_descripcion);
									/////////////////////////////////         SEGURIDAD               /////////////////////////////
					...
					{
						$this->io_msg->message("ERROR -> La orden de compra tiene una sep asociada nro ".$ls_numsol.", con fecha de contabilizaci�n mayor a la fecha de la contabilizaci�n de la orden de compra ");
						$lb_valido=false;
					}
	...
	function uf_obtener_data_orden_compra($as_numordcom,$as_estcondat)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_obtener_data_orden_compra ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
    ...
    function uf_reversar_precomprometido_solicitud_sep($as_numsol,$adt_fecha)	
	{
		...
		if($rs_data===false)
		{ 
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_reversar_precomprometido_solicitud_sep ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
    ...
    function uf_procesar_contabilizacion_compras($as_numordcom,$as_estcondat,$adt_fecha,$aa_seguridad)
	{
		...
		if ($ls_estcom!=1) 
		{
			$this->io_msg->message("ERROR-> La Orden de Compra ".$as_numordcom." debe estar en estatus EMITIDA para su contabilizaci�n.");
			return false;
		}
		...
		{
			$this->io_msg->message("ERROR-> La Fecha de Contabilizacion es menor que la fecha de Emision de la Orden de Compra N� ".$as_numordcom);
			return false;
		}
	...
	function uf_sumar_total_cuentas_gasto_ordencompra($as_numordcom,$as_estcondat)
	{
		...
		//	  Description: Este m�todo realiza una consulta sql para obtener la sumatoria de todos los movimiento de gasto
		//                 asociado a la compras de ejeciuci�n presupuestaria en la tabla SOC_CuentaGasto
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 21/12/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $ldec_monto=0;
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_sumar_total_cuentas_gasto_ordencompra ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
    ...
    function uf_generar_comprobante_ordencompra($adt_fecha)	
	{
		...
		if($rs_data===false)
		{ 
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_generar_comprobante_ordencompra ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;		   
		}
		else
		{                 
	...
	function uf_update_estatus_ordencompra($as_codemp,$as_numordcom,$as_estcondat,$as_estcom)
	{
		...
		//	      Returns: Retorna un booleano
		//	  Description: M�todo que actualiza el estatus de la orden de compra
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 21/12/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $lb_valido=true;
	...
	function uf_update_estatus_detalle_ordencompra($as_codemp,$as_numordcom,$as_estcondat,&$ls_numsol)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_update_estatus_detalle_ordencompra_SELECT ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{
		...
		if($ls_numsol!="")
		{
			...
			if($li_row===false)
			{
				$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_update_estatus_detalle_ordencompra_UPDATE ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
				return false;		   
			}
		}		
	...
	function uf_select_estatus_detalle_soc($as_codemp,$as_estcondat,$as_numsol,&$ai_ok)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_select_estatus_detalle_soc ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{
    ...
    function uf_reversar_ordencompra($as_numordcom,$as_estcondat,$ad_fechaconta,$aa_seguridad)	
	{
		...
		//				   aa_seguridad // Arreglo de las variables de seguridad
		//	      Returns: Retorna un booleano
		//	  Description: M�todo que reversa la conmtabilizaci�n de la orden de compra y sep si lo tiene 
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 21/12/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido = true;
		...
		// inicia transacci�n SQL
        $this->io_sigesp_int->uf_int_init_transaction_begin();
		// ser revers la contabilizaci�n de la orden de compra
		...
		if($lb_valido)
		{
			...
			$ls_descripcion="Revers� la Orden de Compra <b>".$as_numordcom."</b>, Estatus <b>".$ls_estatus."</b> ";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
    ...
    function uf_reversar_contabilizacion_ordencompra($as_numordcom,$as_estcondat,$adt_fecha,$aa_seguridad)
	{
		...
		//				   aa_seguridad // Arreglo de las variables de seguridad
		//	      Returns: Retorna un booleano
		//	  Description: M�todo que reversa la conmtabilizaci�n de la orden de compra
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 21/12/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		...
		if($this->uf_busca_ordencompra_en_recepcion($ls_codemp,$as_numordcom,$as_estcondat,$ls_procede,&$ls_status)) 
		{
			if($ls_status!='A')
			{
				$this->io_msg->message("Ya existe la compra en recepci�n de documento.");
				return false; 
			}
		}
		$ls_codban="---";
	...
	function uf_busca_ordencompra_en_recepcion($as_codemp,$as_numordcom,$as_estcondat,$as_procede,&$as_status)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_busca_ordencompra_en_recepcion ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{
	...
	function uf_verificar_ordencompra_sep($as_numordcom,$as_estcondat)
	{
		...
		//         Access: private
		//     Argumentos: as_numordcom      // n�mero de la orden de compra
		//                 as_estcondat // tipo de orden de compra (B=Bienes,S=Servicio,-=Ambos)		
		//	      Returns: retorna un boolean
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_verificar_ordencompra_sep ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{
	...
	function uf_reversar_sep_ordendecompra($as_codemp,$as_numordcom,$as_estcondat,$aa_seguridad)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_reversar_sep_ordendecompra ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{
			while(($row=$this->io_sql->fetch_row($rs_data))and($lb_valido))
			{
				...
				if (!$ab_existe)
				{
					...
					if($lb_valido)
					{
						/////////////////////////////////         SEGURIDAD               /////////////////////////////		
						$ls_evento="PROCESS";
						$ls_descripcion="Genero el Precompromiso de la Solicitud de Ejecuci�n Presupuestaria <b>".$ls_numsol."</b> ";
						$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
														$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
														$aa_seguridad["ventanas"],$ls_descripcion);
						/////////////////////////////////         SEGURIDAD               /////////////////////////////
    ...
    function uf_delete_contabilizacion_reverso_sep($as_codemp,$as_numsol,$adt_fecha,$aa_seguridad)
	{
		...
		//				   aa_seguridad // Arreglo de las variables de seguridad
		//	      Returns: Retorna un booleano
		//	  Description: M�todo que reversa la contabilizaci�n de la orden de compra
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 21/12/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
    function uf_procesar_anular_contabilizacion($as_numordcom,$as_estcondat,$ad_fechaconta,$adt_fecha_anula,$as_conanu,$aa_seguridad)
	{ 
	    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_ced_bene="";
		$ls_cod_pro="";
		$ls_descripcion="Anulaci�n de la Orden de Compra N� ".$as_numordcom;
		...
		if ($this->uf_busca_ordencompra_en_recepcion($ls_codemp,$as_numordcom,$as_estcondat,$ls_procede,&$ls_status)) 
		{
			if ($ls_status!='A')
			{
				$this->io_msg->message("ERROR-> Ya existe la Orden de Compra en recepci�n de documento N�:".$this->dts_recepcion->getValue("numrecdoc",1));
				return false; 
			}
		}
		$ls_codban="---";
		...
		$lb_valido = $this->io_sigesp_int->uf_int_anular($ls_codemp,$ls_procede,$ls_comprobante,$ad_fechaconta,
														 ...
														 $ls_ctaban,$li_tipo_comp,&$estatusorden);
		if(!$lb_valido )	
		{ 
 		   $this->io_msg->message("ERROR-> ".$this->io_sigesp_int->is_msg_error);
		   return false; 
		...
		// inicia transacci�n SQL
        $this->io_sigesp_int->uf_int_init_transaction_begin();
	    $ls_estcom=$estatusorden; // estatus en anulacion
		...
		if($lb_valido)
		{
			...
			$lb_valido=$this->uf_update_fecha_contabilizado_soc($ls_codemp,$as_numordcom,$as_estcondat,$ad_fechaconta,$ldt_fecha_anula,$as_conanu);
		}
		if($lb_valido)
		{
...
*/		if($lb_valido)
		{
			...
			$ls_descripcion="Anulo la Orden de Compra <b>".$as_numordcom."</b>, Estatus <b>".$ls_estatus."</b>, ".
							"Fecha Anulaci�n <b>".$adt_fecha_anula."</b>";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
	...
	function uf_update_estatus_sep_ordendecompra($as_codemp,$as_numordcom,$as_estcondat,$as_estatus)
	{
		...
		//         Access: private
		//     Argumentos: as_codemp->codigo de la empresa ; $as_numordcom->n�mero de la orden de compra
		//	      Returns: retorna un boolean
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_update_estatus_sep_ordendecompra ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{
    ...
    function uf_procesar_reverso_anulacion($as_numordcom,$as_estcondat,$ad_fechaconta,$ad_fechaanula,$aa_seguridad)
	{
	   //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//				   aa_seguridad // Arreglo de las variables de seguridad
		//	      Returns: Retorna un boleano 
		//	  Description: M�todo que reversa una anulacion. 
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 20/12/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $ls_codemp=$this->dts_empresa["codemp"];
		...
		if($ls_estcom!=3) 
		{
			$this->io_msg->message("ERROR-> La Orden de Compra ".$as_numordcom." no est� ANULADA.");
			return false;
		}
		$ls_codban="---";
		$ls_ctaban="-------------------------";
		...
		if($lb_valido)
		{
			...
			$ls_descripcion="Reverso la Anulaci�n la Orden de Compra <b>".$as_numordcom."</b>, Estatus <b>".$ls_estatus."</b> ";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
	...
	function uf_verificar_sep_ordencompra($as_codemp,$as_numordcom,$as_estcondat,$ad_fechaconta,$aa_seguridad)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_verificar_sep_ordencompra ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{
			...
			while(($row=$this->io_sql->fetch_row($rs_data)) and ($lb_valido) )
			{
				...
				{
					case "R": // Registro
						$this->io_msg->message("ERROR-> La Solicitud ".$ls_numsol." esta Registrada. No se puede Reversar la Anulaci�n.");			
						$lb_valido=false;
						return false;
						break;
						
					case "E": // Emitida
						$this->io_msg->message("ERROR-> La Solicitud ".$ls_numsol." esta Emitida. No se puede Reversar la Anulaci�n.");			
						$lb_valido=false;
						return false;
						break;
						
					case "A": // Anulada
						$this->io_msg->message("ERROR-> La Solicitud ".$ls_numsol." esta anulada. No se puede Reversar la Anulaci�n.");			
						$lb_valido=false;
						return false;
						break;
						
					case "P": // Procesada
						//$this->io_msg->message("ERROR-> La Solicitud ".$ls_numsol." esta procesada. No se puede Reversar la Anulaci�n.");			
						//$lb_valido=false;
						//return false;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_update_fecha_contabilizado_soc($as_codemp,$as_numordcom,$as_estcondat,$ad_fechaconta,$ad_fechaanula,$as_conanu)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//	      Returns: ad_fechaanula True si se ejecuto la contabilizaci�n correctamente
		...
		if($ad_fechaanula!="")
		{
			...
			$ls_campo2=$ls_campo2.", conanusoc='".$as_conanu."' ";
		}
		if($ls_campo1!="")
		{
			if($ls_campo2!="")
		...
		if($li_row===false)
		{
            $this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_update_fecha_contabilizado_soc ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			$lb_valido=false;
		}
		return $lb_valido;
	...
	function uf_verificarreverso_sep($as_codemp,$as_numordcom,$as_estcondat,$as_numsol,$as_estcom,&$ab_existe)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Integraci�n SOC M�TODO->uf_verificarreverso_sep ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{
