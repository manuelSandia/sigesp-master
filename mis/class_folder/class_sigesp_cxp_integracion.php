...
class class_sigesp_cxp_integracion
{
    ...
    function uf_reversar_contabilizacion_solicitud_cxp($as_numsol,$ad_fechaconta,$aa_seguridad)	
	{
		...
		if($this->uf_verificar_cierre_oc($as_numsol))
		{
			$this->io_msg->message(" Debe reversar el cierre de O/C de uno de los compromisos causados la  Orden de Págo N° ".$as_numsol);
			return false;
		}		
        $ls_tipo_destino=$this->dts_solicitud->getValue("tipproben",1);
		$ls_ced_bene=$this->dts_solicitud->getValue("ced_bene",1); 
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
    function uf_procesar_anular_solicitud($as_numsol,$adt_fecha,$ad_fechaconta,$as_conanu,$aa_seguridad)
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($lb_valido)
		{
			$lb_valido=$this->uf_update_fecha_contabilizado_cxp($ls_codemp,$as_numsol,$ldt_fecemisol,$ldt_fecha_anula,$as_conanu);
		}
		if($lb_valido)
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
    function uf_procesar_anula_recepcion($as_numrecdoc,$as_codtipdoc,$as_codpro,$as_cedbene,$adt_fecha,$ad_fechaconta,$as_conanu,$aa_seguridad)
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$this->io_sigesp_int->uf_int_init_transaction_begin(); 
		$as_numrecdocaux="";
		$lb_valido=$this->uf_anular_recepcion($as_numrecdoc,$as_codpro,$as_cedbene,$as_codtipdoc,&$as_numrecdocaux,$aa_seguridad);
		if ($lb_valido)
		{
			$lb_valido=$this->io_sigesp_int->uf_init_end_transaccion_integracion($aa_seguridad); 
			if($lb_valido===false)
		...
		if($lb_valido)
		{
			$lb_valido=$this->uf_update_fecha_contabilizado_rd($ls_codemp,$as_numrecdocaux,$as_codtipdoc,$as_cedbene,$as_codpro,$ad_fechaconta,$ldt_fecha_anula,$as_conanu);
		}
		if($lb_valido)
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion="Anuló la Recepcion de Documentos <b>".$as_numrecdoc."</b> - <b>".$as_codtipdoc."</b> - <b>".$as_cedbene."</b> - <b>".$as_codpro."</b>, ".
    ...
    function uf_procesar_reversar_anular_recepcion($as_numrecdoc,$as_codtipdoc,$as_codpro,$as_cedbene,$ad_fechaanula,$aa_seguridad)
	{
		...
		// Inicio de la Transacción
        $this->io_sigesp_int->uf_int_init_transaction_begin();
		$lb_valido=$this->uf_anular_recepcion($as_numrecdoc,$as_codpro,$as_cedbene,$as_codtipdoc,'',$aa_seguridad);
//	    $lb_valido=$this->uf_update_estatus_contabilizado_recepcion($ls_codemp,$as_numrecdoc,$as_codtipdoc,$as_codpro,$as_cedbene,"C");
	    if($lb_valido)
		{
														  ...
														  $as_codope,$as_numdc,$adt_fecha,$aa_seguridad)
	{
	    ...
	    if($lb_valido)
		{
			...
			{
				$lb_valido=$this->uf_procesar_detalles_spi_notas($ls_codemp,$as_numsol,$as_numrecdoc,$as_codtipdoc,$as_ced_bene,
																 $as_cod_pro,$as_codope,$as_numdc,$ls_descripcion);
				if(!$lb_valido)
				{
					$this->io_msg->message("No se pudo procesar los detalles de ingreso!");
				}
			}
		}
	    if($lb_valido)
											...
											$as_numdc,$as_descripcion)
	{
		...
		$ls_sql="SELECT cxp_dc_spg.numdc, cxp_dc_spg.codope, cxp_dc_spg.spg_cuenta, cxp_dc_spg.codestpro, cxp_dc_spg.estcla, cxp_dc_spg.monto, ".
				"		cxp_documento.estpre, cxp_dc_spg.procede_doc, cxp_dc_spg.numdoccom ".
		...
		{           
			while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
			{
				...
				$ls_procedeaux=$row["procede_doc"];				
				$ls_numdoccomaux=$row["numdoccom"];				
				switch($row["codope"])
				{
					case "ND":
						$ls_procede="CXPNOD";
					...
					case "NC":
						$ls_procede="CXPNOC";
						$ls_mensaje=" No existe la Nota de Crédito N° ".$as_numdc;
						$ldec_monto=$ldec_monto*-1; //CAMBIO PARA QUE LAS NC RESTEN EN SPG
						break;
					default:
						$this->io_msg->message(" Tipo de Nota Inválido");
						return false;
				...
				//SE AGREGARON ESTAS LINEAS DEBIDO A QUE ESTAS PERMITEN
				//ESTABLERCER LOS DOCUMENTOS ASOCIADOS
				$ls_procede=$ls_procedeaux;
				$ls_documento=$ls_numdoccomaux;
				
				/*
				ESTO SE COMENTO POR QUE LA NC SOLO ESTABA COMPROMENTIENDO 
				SE DESCONOCE EL MOTIVO, SE COMENTO PARA QUE COMPROMETA Y CAUSE
				EL CUAL SE CREE ES EL COMPORTAMIENTO CORRECTO
				if(($ls_codope=="NC")&&($ls_estafenc==0))
				{
					$ls_mensaje="O";
					$ls_procede=$ls_procedeaux;
					$ls_documento=$ls_numdoccomaux;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_procesar_detalles_spi_notas($as_codemp,$as_numsol,$as_numrecdoc,$as_codtipdoc,$as_ced_bene,$as_cod_pro,$as_codope,
											$as_numdc,$as_descripcion)
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_procesar_detalles_spi_notas
		//		   Access: private
		//     Argumentos: as_numsol // numero de la solicitud 
		//				   as_numrecdoc // numero de la recepción
		//                 as_codtipdoc // codigo del documento
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_valido=true;		
		$ls_sql="SELECT cxp_dc_spi.numdc, cxp_dc_spi.codope, cxp_dc_spi.spi_cuenta, cxp_dc_spi.codestpro, cxp_dc_spi.estcla, cxp_dc_spi.monto, ".
				"		cxp_documento.estpre ".
                "  FROM cxp_dc_spi, cxp_documento ".
                " WHERE cxp_dc_spi.codemp='".$as_codemp."' ".
				"   AND cxp_dc_spi.numsol='".$as_numsol."' ".
				"   AND cxp_dc_spi.numrecdoc='".$as_numrecdoc."' ".
                "   AND cxp_dc_spi.codtipdoc='".$as_codtipdoc."' ".
				"   AND cxp_dc_spi.ced_bene='".$as_ced_bene."' ".
                "   AND cod_pro='".$as_cod_pro."' ".
				"   AND cxp_dc_spi.codope='".$as_codope."' ".
				"   AND cxp_dc_spi.numdc='".$as_numdc."'".
				"   AND cxp_dc_spi.codtipdoc=cxp_documento.codtipdoc";
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{   
            $this->io_msg->message("CLASE->Integración CXP MÉTODO->uf_procesar_detalles_spg_notas ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
...
*/						break;
				...
				$ls_mensaje="";
				// Mensaje de operacion de gasto a ser aplicada
				$ls_mensaje="DC";print $ldec_monto."<br>";
				$ls_documento=$this->io_sigesp_int->uf_fill_comprobante(trim($as_numdc));
				$lb_valido=$this->io_sigesp_int->uf_spi_insert_datastore($as_codemp,$ls_spi_cuenta,$ls_mensaje,$ldec_monto,$ls_documento,$ls_procede,$as_descripcion,$ls_codestpro1,$ls_codestpro2,$ls_codestpro3,
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_update_fecha_contabilizado_cxp($as_codemp,$as_numsol,$ad_fechaconta,$ad_fechaanula,$as_conanu)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($ad_fechaanula!="")
		{
			...
			$ls_campo2=$ls_campo2.", conanusol='".$as_conanu."' ";
		}
		if($ls_campo1!="")
		{
			if($ls_campo2!="")
											  ...
											  $ad_fechaconta,$ad_fechaanula,$as_conanu)
	{
		...
		{
			$ls_campo2=" fechaanula='".$ad_fechaanula."' ";
			$ls_campo2=$ls_campo2.", conanurd='".$as_conanu."' ";
		}
		if($ls_campo1!="")
		{
			if($ls_campo2!="")
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_anular_recepcion($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc,&$ls_numrecdocnew,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$lb_valido=true;
		$ls_numrecdocnew="";
		$ls_numrecdocnew=$this->uf_load_numero_anulado($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc);
		if($ls_numrecdocnew!="")
		{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------	
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_numero_anulado($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		while($lb_existe)
		{
			...
			if($rs_data===false)
			{
				$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_load_numero_anulado ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			}
			else
			{
				if(!$row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_insert_recepcion_anulada($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Aprobacion MÉTODO->uf_validar_recepciones ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ls_fecemidoc= $this->io_function->uf_formatovalidofecha($row["fecemidoc"]);    
				$ls_fecregdoc= $this->io_function->uf_formatovalidofecha($row["fecregdoc"]);    
				$ls_fecvendoc= $this->io_function->uf_formatovalidofecha($row["fecvendoc"]);    
				...
				$ls_sql=" INSERT INTO cxp_rd (codemp, numrecdoc, codtipdoc, ced_bene, cod_pro, codcla,dencondoc, fecemidoc,  ".
					  	...
					  	" VALUES ('".$this->io_codemp."','".$as_numrecdocnew."','".$ls_codtipdoc."','".$ls_cedbene."','".$ls_codpro."', ".
				...
				{
					$lb_valido=false;
					$this->io_msg->message("CLASE->Aprobacion MÉTODO->uf_insert_recepcion_anulada ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
				...
				else
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
					$ls_evento="UPDATE";
					$ls_descripcion ="Insertó la Recepcion de Documentos Anulada <b>".$as_numrecdoc."</b> Asociado a la Empresa <b>".$this->io_codemp."<b>";
					$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_select_dt_cargos($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_select_dt_cargos ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_insert_dt_cargos_anulado($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_insert_dt_cargos_anulado ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$ls_sql=" INSERT INTO cxp_rd_cargos (codemp, numrecdoc, codtipdoc, cod_pro, ced_bene, codcar, procede_doc,".
						" 					  		 numdoccom, monobjret, monret, codestpro1, codestpro2, codestpro3, codestpro4,".
						"					 		 codestpro5, estcla, spg_cuenta, porcar, formula) ".
					  	" VALUES ('".$this->io_codemp."','".$as_numrecdocnew."','".$ls_codtipdoc."','".$ls_codpro."','".$ls_cedbene."', ".
	...
	function uf_select_dt_deducciones($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_select_dt_deducciones ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_insert_dt_deducciones_anulada($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		...
		else
		{
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				{
					$lb_valido=false;
					$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_insert_dt_deducciones_anulada ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
				}
				else
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_select_dt_spg($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_select_dt_spg ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_insert_dt_spg_anulada($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_insert_dt_spg_anulada ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_select_dt_scg($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_select_dt_scg ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_insert_dt_scg_anulada($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_insert_dt_scg_anulada ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_select_dt_solicitud($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_select_dt_solicitud ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_select_dt_amortizacion($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_select_dt_amortizacion ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_select_dt_anticipo($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_select_dt_anticipo ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_insert_dt_solicitud_anulada($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_insert_dt_solicitud_anulada ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ls_sql=" INSERT INTO cxp_dt_solicitudes (codemp, numsol, numrecdoc, codtipdoc, ced_bene, cod_pro, monto) ".
					  	" VALUES ('".$this->io_codemp."','".$ls_numsol."','".$as_numrecdocnew."','".$ls_codtipdoc."','".$ls_cedbene."', ".
					  	" 		  '".$ls_codpro."',".$li_monto.")";	
				$li_row=$this->io_sql->execute($ls_sql);
				if($li_row===false)
				{
	...
	function uf_insert_dt_amortizacion_anulada($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_insert_dt_amortizacion_anulada ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ls_sql=" INSERT INTO cxp_rd_amortizacion (codemp,  numrecdoc, codtipdoc, ced_bene, cod_pro, codamo, cuenta, ".
						"                                  monamo, monsal, montotamo) ".
					  	" VALUES ('".$this->io_codemp."','".$as_numrecdocnew."','".$ls_codtipdoc."','".$ls_cedbene."', ".
				...
				else
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
					$ls_evento="UPDATE";
					$ls_descripcion ="Insertó la Amortizacion Anulada <b>".$ls_codamo." - ".$as_numrecdocnew."</b> Asociado a la Empresa <b>".$this->io_codemp."<b>";
					$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_insert_dt_anticipo_anulado($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		...
		else
		{
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				{
					$lb_valido=false;
					$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_insert_dt_anticipo_anulado ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
				}
				else
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
					$ls_evento="UPDATE";
					$ls_descripcion ="Insertó el Anticipo Anulado de la Amortización <b>".$ls_codamo." - ".$as_numrecdocnew."</b> Asociado a la Empresa <b>".$this->io_codemp."<b>";
					$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_select_recepcion($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		// Fecha Creación: 07/05/2007								Fecha Última Modificación : 
		//////////////////////////////////////////////////////////////////////////////
		$ls_sql="SELECT codemp, numrecdoc, codtipdoc, ced_bene, cod_pro, codcla, dencondoc, fecemidoc, fecregdoc,".
				"       fecvendoc, montotdoc, mondeddoc, moncardoc, tipproben, numref, estprodoc, procede, estlibcom,".
				"       estaprord, fecaprord, usuaprord, numpolcon, estimpmun, montot, codfuefin, codrecdoc, fechaconta,".
				"       fechaanula, coduniadm, codestpro1, codestpro2, codestpro3, codestpro4, codestpro5, estcla, estact,".
				"       numordpagmin, codtipfon, repcajchi".
				"  FROM cxp_rd".
				" WHERE codemp = '".$this->io_codemp."'".
				"   AND numrecdoc = '".$as_numrecdoc."'".
				"   AND cod_pro = '".$as_codpro."'".
	...
	function uf_load_estatus($as_numrecdoc,$as_cedbene,$as_codpro,$as_codtipdoc,&$as_estprodoc)
	{
		...
		if ($rs_data===false)
		{
			$this->io_msg->message("CLASE->Recepcion MÉTODO->uf_load_estatus ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
	...
	function uf_delete_detallesrecepcion($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->Recepcion MÉTODO->uf_delete_detallesrecepcion ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
		}
		if($lb_valido)
		{
		...
		if($lb_valido)
		{
			...
			$ls_descripcion ="Eliminó todos los detalles de Recepción de Documentos ".$as_numrecdoc." Tipo ".$as_codtipdoc." Beneficiario ".$as_cedbene.
							 "Proveedor ".$as_codpro." Asociado a la empresa ".$this->io_codemp;
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////	
	...
	{		
		...
		if($lb_encontrado)
		{
			...
			if($ls_estprodoc!="C")
			{
				$this->io_msg->message("La Recepción de Documentos debe estar en contabilizada para ejecutar este proceso.");           
				$lb_valido=false;
			}
			if($lb_valido)
			{	
			...
			{	
				...
				{print $this->io_sql->message;
					$lb_valido=false;
					$this->io_msg->message("CLASE->Recepción MÉTODO->uf_eliminar ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
				}
				else
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
		...
		{
			$this->io_msg->message("No se encontro la Recepcion de Documentos");
			$lb_valido=false;	
		}
		return $lb_valido;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_select_historico($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		//////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_select_historico
		//		   Access: public
		//		 Argument: as_numrecdoc  // Numero de la recepcion de documentos
		//                 as_codpro     //  Codigo de Proveedor
		//                 as_cedben     //  Codigo de Beneficiario
		...
		// Fecha Creación: 07/05/2007								Fecha Última Modificación : 
		//////////////////////////////////////////////////////////////////////////////
		$ls_sql="SELECT codemp, numrecdoc, codtipdoc, ced_bene, cod_pro, fecha, estprodoc ".
				"  FROM cxp_historico_rd".
				" WHERE codemp = '".$this->io_codemp."'".
				"   AND numrecdoc = '".$as_numrecdoc."'".
				"   AND cod_pro = '".$as_codpro."'".
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_select_historico ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	}// end function uf_select_historico
	//-----------------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_insert_historico_anulada($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_insert_historico_anulada
		//		   Access: private
		//	    Arguments: as_numrecdoc    //  Número de Recepcion de Documentos Actual
		//                 as_numrecdocnew //  Número de Recepcion de Documentos Anulada
		//                 as_codpro       //  Codigo de Proveedor
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$rs_data=$this->uf_select_historico($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc);
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_insert_historico_anulada ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$ls_sql=" INSERT INTO cxp_historico_rd (codemp, numrecdoc, codtipdoc, ced_bene, cod_pro, fecha, estprodoc) ".
					  	" VALUES ('".$this->io_codemp."','".$as_numrecdocnew."','".$ls_codtipdoc."','".$ls_cedbene."','".$ls_codpro."', ".
					  	" 		  '".$ls_fecha."','".$ls_estprodoc."')";	
				$li_row=$this->io_sql->execute($ls_sql);
				if($li_row===false)
				{
					$lb_valido=false;
					$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_insert_historico_anulada2 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
				}
				else
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
					$ls_evento="UPDATE";
					$ls_descripcion ="Insertó la Recepcion de Documentos Anulada en Historico<b>".$as_numrecdoc."</b> Asociado a la Empresa <b>".$this->io_codemp."<b>";
					$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
		...
		return $lb_valido;
	}// end function uf_insert_historico_anulada
	//-----------------------------------------------------------------------------------------------------------------------------------	

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_select_solicitudes_scg($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		//////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_select_solicitudes_scg
		//		   Access: public
		//		 Argument: as_numrecdoc  // Numero de la recepcion de documentos
		//                 as_codpro     //  Codigo de Proveedor
		//                 as_cedben     //  Codigo de Beneficiario
		...
		// Fecha Creación: 07/05/2007								Fecha Última Modificación : 
		//////////////////////////////////////////////////////////////////////////////
		$ls_sql="SELECT codemp, numsol, numrecdoc, codtipdoc, ced_bene, cod_pro, procede_doc, numdoccom,".
				"       debhab, sc_cuenta, estasicon, monto ".
				"  FROM cxp_solicitudes_scg".
				" WHERE codemp = '".$this->io_codemp."'".
				"   AND numrecdoc = '".$as_numrecdoc."'".
				"   AND cod_pro = '".$as_codpro."'".
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_select_solicitudes_scg ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	}// end function uf_select_solicitudes_scg
	//-----------------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_insert_solicitudes_scg_anulada($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_insert_solicitudes_scg_anulada
		//		   Access: private
		//	    Arguments: as_numrecdoc    //  Número de Recepcion de Documentos Actual
		//                 as_numrecdocnew //  Número de Recepcion de Documentos Anulada
		//                 as_codpro       //  Codigo de Proveedor
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$rs_data=$this->uf_select_solicitudes_scg($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc);
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Anulacion MÉTODO->uf_insert_solicitudes_scg_anulada ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$ls_estasicon= $row["estasicon"];    
				$li_monto= $row["monto"];    
				$ls_sql=" INSERT INTO cxp_historico_rd (codemp, numsol, numrecdoc, codtipdoc, ced_bene, cod_pro, procede_doc, numdoccom, debhab, sc_cuenta, estasicon, monto) ".
					  	" VALUES ('".$this->io_codemp."','".$ls_numsol."','".$as_numrecdocnew."','".$ls_codtipdoc."','".$ls_cedbene."','".$ls_codpro."', ".
					  	" 		  '".$ls_numdoccom."','".$ls_debhab."','".$ls_sccuenta."','".$ls_estasicon."',".$li_monto.")";	
				$li_row=$this->io_sql->execute($ls_sql);
				if($li_row===false)
				{
					$lb_valido=false;
				...
				else
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
					$ls_evento="UPDATE";
					$ls_descripcion ="Insertó la Recepcion de Documentos Anulada en Solicitud SCG<b>".$as_numrecdoc."</b> Asociado a la Empresa <b>".$this->io_codemp."<b>";
					$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_reverso_anular_recepcion($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_reverso_anular_recepcion
		//		   Access: private
		//	    Arguments: as_numrecdoc  //  Número de Recepcion de Documentos
		//				   as_estsol     //  Estatus de la Solicitud
		//				   as_codpro     //  Codigo de Proveedor
		...
		$lb_valido=true;
		$ls_numrecdocnew="";
		$lb_valido==$this->uf_load_numero_original($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc,&$ls_numrecdocnew);
		if($lb_valido)
		{
			$lb_valido=$this->uf_insert_recepcion_anulada($as_numrecdoc,$ls_numrecdocnew,$as_codpro,$as_cedben,
														  $as_codtipdoc,$aa_seguridad);
	...
	//-----------------------------------------------------------------------------------------------------------------------------------	
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_numero_original($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc,&$as_numrecdocnew)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_load_numero_original
		//		   Access: private
		//	    Arguments: as_numrecdoc  //  Número de Recepcion de Documentos
		//				   as_codpro     //  Codigo de Proveedor
		//				   as_cedben     //  Codigo de Beneficiario
		...
		if ($ls_prenumrec=='SCV-' && $li_lonnumrecdoc>=14)//Para el Caso de las Solicitudes de Viaticos SCV-00000000000.
		   {
			 $as_numrecdocnew=substr(trim($as_numrecdoc),0,1);
			 $ls_num=substr(trim($as_numrecdocnew),5,10);
			 $as_numrecdocnew = substr(trim($as_numrecdocnew),0,3).str_pad($ls_num,12,'0');
		   }
		else
		   {
			 $as_numrecdocnew=substr(trim($as_numrecdoc),0,1);
		...
		$li_lonnewnumrecdoc = strlen(trim($as_numrecdocnew));
		if($li_lonnewnumrecdoc>15)
		{
			$as_numrecdocnew=substr($as_numrecdocnew,0,15);			
		}
		$ls_sql="SELECT numrecdoc ".
				"  FROM cxp_rd ".
				" WHERE codemp='".$this->io_codemp."' ".
				"   AND numrecdoc='".$as_numrecdocnew."' ".
				"   AND cod_pro='".$as_codpro."' ".
				"   AND ced_bene='".$as_cedben."' ".
				"   AND codtipdoc='".$as_codtipdoc."' ";
		$rs_data=$this->io_sql->select($ls_sql);
		...
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				$this->io_msg->message("Ya existe el numero original de la Recepcion de Documentos");
				return false;
			}
		}print $as_numrecdocnew;
				return false;
		return $lb_valido;
	}// end function uf_load_numero_anulado
	//-----------------------------------------------------------------------------------------------------------------------------------	
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_verificar_cierre_oc($as_numsol)
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_verificar_inter
		//		   Access: public 
		...
		{                 
				while((!$rs_data->EOF)&&($lb_valido))
				{
					...
					$ls_compromiso=$this->uf_load_compromiso_relacionado($ls_numrecdoc,$ls_codtipdoc,$ls_codpro,$ls_ced_bene);
					if($ls_compromiso!="")
					{
						$lb_existe=$this->uf_verificar_asiento_cierre($ls_compromiso);
					}
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_compromiso_relacionado($as_numrecdoc,$as_codtipdoc,$as_codpro,$as_ced_bene)
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_verificar_inter
		//		   Access: public 
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_verificar_asiento_cierre($as_documento)
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_verificar_asiento_cierre
		//		   Access: public 
		//	    Arguments: 
		//	      Returns: Retorna un bolean valido
		//	  Description: Método que verifica si existen asientos intercompañia y los elimina
