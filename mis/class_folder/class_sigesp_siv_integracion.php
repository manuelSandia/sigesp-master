...
class class_sigesp_siv_integracion
{
    ...
    function uf_procesar_contabilizacion_despachos($as_comprobante,$adt_fecha,$aa_seguridad)
	{
        ...
        {	// Se procesan los detalles de Contabilidad
			...
			if($lb_valido)
			{
				$lb_valido = $this->uf_procesar_detalles_presupuestario($as_comprobante,"");
			}
			if ($lb_valido)
			...
			if($lb_valido)
			{	// Se Actualiza el estatus de la nómina que está contabilizada
				$lb_valido=$this->uf_update_estatus_despachopre($as_comprobante,1);
			}
		}
		if($lb_valido)
		{
		...
		if($lb_valido)
		{
			$lb_valido=$this->uf_update_fecha_contabilizado_pre($this->is_codemp,$ls_comprobante,
															    $adt_fecha,'1900-01-01');
		}
		if($lb_valido)
		{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_update_estatus_despachopre($as_comprobante,$ai_estatus)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_update_estatus_despachopre
		//		   Access: private
		//	    Arguments: as_comprobante  // Código del comprobante 
		//				   ai_estatus  // estatus si es 0 ó 1
		//	      Returns: lb_valido True si se actualizó correctamente
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $lb_valido=true;	
		$ls_sql="UPDATE siv_dt_spg".
				"   SET estatus=".$ai_estatus.
				" WHERE codemp='".$this->is_codemp."' ".
				"   AND numorddes='".$as_comprobante."'";
		$li_row=$this->io_sql->execute($ls_sql);
		if($li_row===false)
		{
           	$this->io_msg->message("CLASE->Integración SNO MÉTODO->uf_update_estatus_despachopre ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			$lb_valido=false;
		}
		return $lb_valido;
	}// end function uf_update_estatus_despacho
    ...
    function uf_reversar_contabilizacion($as_comprobante,$ad_fechaconta,$aa_seguridad)	
	{
	    ...
	    if($lb_valido) 
		{// Si se hizo nota de Débito se Reversa 
		   $lb_valido=$this->uf_update_estatus_despachopre($as_comprobante,0); 
	    } 
		if($lb_valido)
		...
		if($lb_valido)
		{						
			$lb_valido=$this->uf_update_fecha_contabilizado_pre($this->is_codemp,$ls_comprobante,'1900-01-01','1900-01-01');
		}
		if($lb_valido)
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_update_fecha_contabilizado_pre($as_codemp,$as_comprobante,$ad_fechaconta,$ad_fechaanula)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_update_fecha_contabilizado_pre
		//		   Access: private
		//	    Arguments: as_codemp  // Código de empresa
		//                 as_comprobante  // Comprobante
		//                 ad_fechaconta  // Fecha de contabilización
		...
		if($li_row===false)
		{
            $this->io_msg->message("CLASE->Integración SEP MÉTODO->uf_update_fecha_contabilizado_pre ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			$lb_valido=false;
		}
		return $lb_valido;
	}// end function uf_update_fecha_contabilizado_pre
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_procesar_detalles_presupuestario($as_comprobante,$as_codcomapo)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_procesar_detalles_presupuestario
		//		   Access: private
		//	    Arguments: as_comprobante  // Código del comprobante de Nómina
		//				   as_codcomapo  // Código del comprobante de Aportes
		//	      Returns: lb_valido True si se insertaron correctamente los detalles en el datastored
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_valido=true;		
		$ls_sql="SELECT numorddes,feccmp,codestpro1,codestpro2,codestpro3,codestpro4,codestpro5,estcla,spg_cuenta,monto".
				"  FROM siv_dt_spg ".
				" WHERE codemp = '".$this->is_codemp."' ".
				"   AND numorddes = '".$as_comprobante."' ";
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{   
           	$this->io_msg->message("CLASE->Integración SNO MÉTODO->uf_procesar_detalles_presupuestario ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{           
   	       while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
		   {
				...
				$ldec_monto=$row["monto"];			
				$ls_documento = $row["numorddes"];								
                $ls_documento = $this->io_sigesp_int->uf_fill_comprobante(trim($ls_documento));
				// Incluimos el detalle de presupuesto en el datastored
				$ls_descripcion="";
				$ls_mensaje="OCP";
				$lb_valido = $this->io_sigesp_int->uf_spg_insert_datastore($this->is_codemp,$ls_codestpro1,$ls_codestpro2,$ls_codestpro3,
																	  	   ...
																	  	   $ldec_monto,$as_comprobante,$this->is_procede,$ls_descripcion);
				if (!$lb_valido)
				{  
				   $this->io_msg->message("ERROR->".$this->io_sigesp_int->is_msg_error);
				   break;
		...
		$this->io_sql->free_result($rs_data);	 
		return $lb_valido;
    } // end function uf_procesar_detalles_presupuestario
	//-----------------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_obtener_data_comprobante_transferencia($as_comprobante,$adt_fecemision)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_obtener_data_comprobante_transferencia
		//		   Access: private
		//	    Arguments: as_comprobante  // Código de Comprobante
		//				   adt_fecemision  // Fecha de Emisión
		//	      Returns: lb_existe True si existe el comprobante
		//	  Description: Este metodo que obtiene la información agrupada en un registro 
		//	   Creado Por: Ing. Yesenia Moreno	
		// Modificado Por: Ing. Yesenia Moreno								Fecha Última Modificación : 25/10/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_existe = false;		
		$ls_sql="SELECT siv_transferencia.codemp, siv_transferencia.numtra, MAX(siv_transferencia.obstra) as obstra, ".
				"		MAX(siv_dt_transferencia.fecemi) as fecemi, MAX(siv_dt_transferencia_scg.estint) as estint ". 
				"  FROM siv_transferencia ".
				" INNER JOIN (siv_dt_transferencia ".
				"       INNER JOIN siv_dt_transferencia_scg ".
				"          ON siv_dt_transferencia.codemp = '".$this->is_codemp."' ".
				"		  AND siv_dt_transferencia.numtra = '".$as_comprobante."'".
				"		  AND siv_dt_transferencia.fecemi ='".$adt_fecemision."'".
		...
		if($rs_data===false)
		{ 
			$this->io_msg->message("CLASE->Integración SNO MÉTODO->uf_obtener_data_comprobante_transferencia ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
		...
		$this->io_sql->free_result($rs_data);
		return $lb_existe;
	} // end function uf_obtener_data_comprobante_transferencia
	//-----------------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------------
    function uf_procesar_contabilizacion_transferencia($as_comprobante,$adt_fecemision,$adt_fecha,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_procesar_contabilizacion_transferencia
		//		   Access: private
		//	    Arguments: as_comprobante  // Código de Comprobante
		//				   adt_fecemision  // Fecha de Emisión
		//				   adt_fecha  // Fecha de contabilización
		//				   aa_seguridad  // Arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se ejecuto la contabilización correctamente
		//	  Description: Funcion que procesa la contabilización de una Transferencia
		//	   Creado Por: Ing. Yesenia Moreno
		// Modificado Por: Ing. Yesenia Moreno								Fecha Última Modificación : 25/10/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    if(!$this->uf_obtener_data_comprobante_transferencia($as_comprobante,$adt_fecemision))
		{
			return false;
		}
        $ls_procede="SIVCTR";
		$this->is_procede="SIVCTR";
		$ls_comprobante=$this->io_sigesp_int->uf_fill_comprobante(trim($as_comprobante));
		$ls_codigo_destino="----------";
		$ls_descripcion=$this->dts_inventario->getValue("obstra",1);	
        $ls_tipo_destino="-";			
		$li_estatus=$this->dts_inventario->getValue("estint",1);
		if($li_estatus==1) 
		{
		   $this->io_msg->message("La Transferencia ya esta contabilizada.");
		   return false;
		}
		$ls_fecemi=$this->dts_inventario->getValue("fecemi",1);
		$ls_fecemi=$this->io_function->uf_convertirdatetobd($ls_fecemi);
		$adt_fecha=$this->io_function->uf_convertirdatetobd($adt_fecha);
		if(!$this->io_fecha->uf_comparar_fecha($ls_fecemi,$adt_fecha))
		{
			$this->io_msg->message("ERROR-> La Fecha de Contabilizacion es menor que la fecha de la Transferencia ".$as_comprobante);
			return false;
		}
		// Creo la cabecera del Comprobante
		$ls_codban="---";
		...
		if ($lb_valido)
        {	// Se procesan los detalles de Contabilidad
			$lb_valido = $this->uf_procesar_detalles_contables_transferencia($as_comprobante,$adt_fecemision); 
			if ($lb_valido)
			{	// Se inserta el comprobante con sus detalles contables y presupuestarios
				$lb_valido = $this->io_sigesp_int->uf_init_end_transaccion_integracion($aa_seguridad); 
				if (!$lb_valido) 
			...
			if($lb_valido)
			{	// Se Actualiza el estatus de la transferencia que está contabilizada
				$lb_valido=$this->uf_update_estatus_transferencia($as_comprobante,$adt_fecemision,1,$adt_fecha,'1900-01-01');
			}
		}
		if($lb_valido)
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion="Contabilizó la Transferencia <b>".$ls_comprobante."</b> de fecha <b>".$adt_fecemision."</b>";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
		...
		$this->io_sigesp_int->uf_sql_transaction($lb_valido);		
		return  $lb_valido;
	} // end function uf_procesar_contabilizacion_transferencia
	//-----------------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_procesar_detalles_contables_transferencia($as_comprobante,$adt_fecemision)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_procesar_detalles_contables_transferencia
		//		   Access: private
		//	    Arguments: as_comprobante  // Código del comprobante 
		//				   adt_fecemision  // Fecha de Emisión
		//	      Returns: lb_valido True si se insertaron correctamente los detalles en el datastored
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_valido=true;		
		$ls_sql="SELECT codcmp, sc_cuenta, debhab, monto ".
				"  FROM siv_dt_transferencia,siv_dt_transferencia_scg ".
				" WHERE siv_dt_transferencia.codemp = '".$this->is_codemp."' ".
				"   AND siv_dt_transferencia.numtra='".$as_comprobante."'".
				"   AND siv_dt_transferencia.fecemi='".$adt_fecemision."'".
				"   AND siv_dt_transferencia.codemp=siv_dt_transferencia_scg.codemp ".
				"   AND siv_dt_transferencia.numtra=siv_dt_transferencia_scg.codcmp ".
		...
		if($rs_data===false)
		{   
           	$this->io_msg->message("CLASE->Integración SNO MÉTODO->uf_procesar_detalles_contables_transferencia ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{           
		   ...
		   {
				$ls_scg_cuenta = trim($rs_data->fields["sc_cuenta"]);
				$ls_mensaje = $rs_data->fields["debhab"];
				$ldec_monto = $rs_data->fields["monto"];				
				$ls_descripcion = $this->dts_inventario->getValue("obstra",1);		
				$ls_documento = $rs_data->fields["codcmp"];								
                $ls_documento = $this->io_sigesp_int->uf_fill_comprobante(trim($ls_documento));
				// Incluimos el detalle de contabilidad en el datastored
				$lb_valido = $this->io_sigesp_int->uf_scg_insert_datastore($this->is_codemp,$ls_scg_cuenta,$ls_mensaje,$ldec_monto,$ls_documento,$this->is_procede,$ls_descripcion);				
				if (!$lb_valido)
		...
		$this->io_sql->free_result($rs_data);	 
		return $lb_valido;
    } // end function uf_procesar_detalles_contables_transferencia
	//-----------------------------------------------------------------------------------------------------------------------------------
	
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_update_estatus_transferencia($as_comprobante,$adt_fecemision,$ai_estatus,$adt_fechaconta,$adt_fechaanula)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_update_estatus_transferencia
		//		   Access: private
		//	    Arguments: as_comprobante  // Código del comprobante 
		//				   adt_fecemision  // Fecha de Emisión
		//				   ai_estatus  // estatus si es 0 ó 1
		//				   adt_fechaconta  // Fecha de Contabilizacion
		//				   adt_fechaanula  // Fecha de Anulación
		//	      Returns: lb_valido True si se actualizó correctamente
		//	  Description: Método que actualiza el estatus de la transferencia en contabilizado o no 
		//	   Creado Por: Ing. Yesenia Moreno
		// Modificado Por: Ing. Yesenia Moreno								Fecha Última Modificación : 25/10/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $lb_valido=true;	
		$ls_sql="UPDATE siv_dt_transferencia_scg".
				"   SET estint=".$ai_estatus.", ".
				"       fechaconta='".$adt_fechaconta."', ".
				"       fechaanula='".$adt_fechaanula."' ".
				" WHERE codemp='".$this->is_codemp."' ".
				"   AND codcmp='".$as_comprobante."'".
				"   AND feccmp='".$adt_fecemision."'";
		$li_row=$this->io_sql->execute($ls_sql);
		if($li_row===false)
		{
           	$this->io_msg->message("CLASE->Integración SNO MÉTODO->uf_update_estatus_transferencia ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			$lb_valido=false;
		}
		return $lb_valido;
	}// end function uf_update_estatus_transferencia
	//-----------------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------------
    function uf_reversar_contabilizacion_transferencia($as_comprobante,$adt_fecemision,$ad_fechaconta,$aa_seguridad)	
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_reversar_contabilizacion_transferencia
		//		   Access: private
		//	    Arguments: as_comprobante  // Código de Comprobante
		//				   ad_fechaconta  // Fecha en que fue contabilizado el Documento
		//				   aa_seguridad  // Arreglo de las variables de seguridad
        ...
        $this->io_sigesp_int->uf_int_init_transaction_begin();
	    if($lb_valido) 
		{	// Se Actualiza el estatus de la transferencia que no está contabilizada
			$lb_valido=$this->uf_update_estatus_transferencia($as_comprobante,$adt_fecemision,0,'1900-01-01','1900-01-01');
		}
		if($lb_valido)
		{	// Reversamos los detalles y el comprobante
			$lb_valido = $this->io_sigesp_int->uf_init_end_transaccion_integracion($aa_seguridad); 
		...
		if($lb_valido)
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion="Reverso Contabilización de la Transferencia <b>".$ls_comprobante."</b> de fecha <b>".$adt_fecemision."</b>";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
		...
		$this->io_sigesp_int->uf_sql_transaction($lb_valido);
		return  $lb_valido;
	} // end function uf_reversar_contabilizacion_transferencia
	//-----------------------------------------------------------------------------------------------------------------------------------
