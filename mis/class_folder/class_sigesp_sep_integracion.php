...
class class_sigesp_sep_integracion
{
	...
	function uf_procesar_contabilizacion_solicitud_sep($as_numsol,$as_fecha,$aa_seguridad)
	{
		...
		$lb_valido=$this->uf_procesar_comprobante_spg_solicitud($as_numsol,$ls_fecsol,$ls_consol,$ls_tipo_destino,$ls_ced_bene,
																$ls_cod_pro,$as_fecha,$ls_codtipsol,$ls_consol,$ls_mensaje,"C",&$status);
		if(!$lb_valido)
		{
    ...
    function uf_procesar_comprobante_spg_solicitud($as_numsol,$adt_fecsol,$as_consol,$as_tipo_destino,$as_ced_bene,$as_cod_pro,
												   $adt_fecha,$as_codtipsol,$as_descripcion,$as_mensaje,$as_tipo,&$status)
    {
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
$ls_sql2="SELECT sum(monto)as monto  FROM cxp_rd_spg ".
	            " WHERE codemp='".$ls_codemp."' ".
				"   AND numdoccom='".$as_numsol."' and spg_cuenta='".$ls_spg_cuenta."'";
...
//echo "<br>". $ls_sql2  ."monto".$ldec_monto2."   dddd   ".$row["monto"];
...
//echo "aquiiii".$mm2."  a ".$ldec_monto2."   b".$ldec_monto."   ".$mm;
if($ldec_monto2>0){
//echo "<br>aaa";
$ldec_monto=(-1*$mm2);
$as_tipo="F";
$status="F";
$ls_procede="SEPSPF"; // procedencia u origen del documento a ser enviado al gasto anulado parcial. (INTEGRACION)
}else{
//echo "<br>bbb";
$ldec_monto=(-1*$row["monto"]);
//$as_tipo="A";
//$status="A";
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_procesar_anulacion_solicitud_sep($as_numsol,$adt_fecha,$as_fecha,$as_conanu,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $this->io_sigesp_int->uf_int_init_transaction_begin();
		$lb_valido=$this->uf_procesar_comprobante_spg_solicitud($as_numsol,$ls_fecsol,$ls_consol,$ls_tipo_destino,$ls_ced_bene,
																$ls_cod_pro,$as_fecha,$ls_codtipsol,$ls_consol,$ls_mensaje,"A",&$status);
		...
		{
			$lb_valido=$this->uf_update_estatus_contabilizado_sep($ls_codemp,$as_numsol,$status);
		}		
		...
		if($lb_valido)
		{
			$lb_valido=$this->uf_update_fecha_contabilizado_sep($ls_codemp,$as_numsol,$adt_fecha,$as_fecha,$as_conanu);
		}
		if($lb_valido)
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_update_fecha_contabilizado_sep($as_codemp,$as_numsol,$ad_fechaconta,$ad_fechaanula,$as_conanu)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($ad_fechaanula!="")
		{
			...
			$ls_campo2=$ls_campo2.", conanusep='".$as_conanu."' ";
		}
		if($ls_campo1!="")
		{
			if($ls_campo2!="")
