...
class class_sigesp_scbmov_integracion
{
											   ...
											   $aa_seguridad)
	{
		...
		$ls_numdoc=$this->dts_banco->getValue("numdoc",1);
		$this->is_numdocaux=$ls_numdoc;
		...
		$lb_valido=$this->uf_create_movimiento_banco($this->io_codemp,$as_codban,$as_ctaban,$this->is_numdocaux,$as_codope,$as_estmov,"C",$this->is_procede,$this->is_comprobante,$this->idt_date,$ldt_fecha);
		if (!$lb_valido)
		{   
		...
		$lb_valido = $this->uf_delete_movimiento_banco($this->io_codemp,$as_codban,$as_ctaban,$this->is_numdocaux,$as_codope,$as_estmov);
		if (!$lb_valido)
		{   
	    ...
	    if($lb_valido)
	    {
			$lb_valido=$this->uf_update_fecha_contabilizado_scbmov($this->io_codemp,$as_codban,$as_ctaban,$this->is_numdocaux,$as_codope,
	...
	function uf_obtener_data_movimento_banco()
	{
		...
		$ls_campo="numdoc";
		if(($ls_contintmovban==1)&&(($this->is_codope=="DP")||($this->is_codope=="ND")||($this->is_codope=="NC")))
		{
			$ls_campo="numconint";
		}
		$ls_sql="SELECT * ".
			...
			{                 
				while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
				{
					...
					{    
						...
						if(number_format($ldec_total,2,".","")==number_format($ldec_monto_sol,2,".","")) 
						{ 
										...
										$as_procede,$as_comprobante,$adt_fecmov,$adt_fecha)
	{
		...
		$ls_sql = "INSERT INTO scb_movbco (codemp,codban,ctaban,numdoc,codope,estmov,cod_pro,ced_bene,".
				  ...
				  "						   fechaconta, fechaanula,nrocontrolop, estant,docant,monamo,numconint,tranoreglib,estcondoc) ".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
    function uf_delete_movimiento_banco($as_codemp,$as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov,$ab_revanu=false)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//---------------------------------------------------------------------------------------------------------------------
		if($as_codope=='CH' && $ab_revanu){
			$ls_sql="UPDATE scb_cheques ".
					"   SET estche=0 ".
	                " WHERE codemp='".$as_codemp."' ".
					"   AND codban='".$as_codban."' ".
					"   AND ctaban='".$as_ctaban."' ".
					"   AND numche='".$as_numdoc."' ";
			$li_row=$this->io_sql->execute($ls_sql);
			if($li_row===false)
			{   
	           	$this->io_msg->message("CLASE->Integración SCB MÉTODO->uf_delete_movimiento_banco 8 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
	...
	function uf_reversar_contabilizacion_banco($as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov,$ad_fechaconta,$aa_seguridad)
	{
		...
		$this->is_numdocaux=$this->dts_banco->getValue("numdoc",1);
		...
		if ($ls_valor==0)
		{
			...
			if($lb_valido)
			{
				...
				$lb_valido=$this->uf_create_movimiento_banco($this->is_codemp,$as_codban,$as_ctaban,$this->is_numdocaux,$as_codope,$as_estmov,
				...
				$lb_valido = $this->uf_delete_movimiento_banco($this->is_codemp,$as_codban,$as_ctaban,$this->is_numdocaux,$as_codope,$as_estmov);
				if (!$lb_valido)
				{   
										 ...
										 $ad_fechaconta,$as_conanu,$aa_seguridad)
	{
		...
		if ($ls_valor==0)
		{
			...
			if($lb_valido)
			{   
				$lb_valido=$this->uf_eliminar_amortizacion($this->io_codemp,$as_codban,$as_ctaban,$this->is_numdocaux);			
			}		
			//---------------------------------------------------------------------------------------------------------		
			if($lb_valido)
			{
				if(($this->ii_datasource==1)||($this->ii_datasource==4))
				{
					...
					$lb_valido=$this->uf_create_movimiento_banco($this->is_codemp,$this->is_codban,$this->is_ctaban,$this->is_numdocaux,$this->is_codope,$this->is_estmov,"O",$this->is_procede,$this->is_comprobante,$ldt_fechamov,$this->idt_fecha);
					if($lb_valido) 
					{// Crea de Nuevo el movimiento de banco	Anulado 
						$lb_valido=$this->uf_create_movimiento_banco($this->is_codemp,$this->is_codban,$this->is_ctaban,$this->is_numdocaux,$this->is_codope,$this->is_estmov,"A",$ls_procede_anula,$this->is_comprobante,$adt_fecha_anula,$adt_fecha_anula); 
					}
				}   
				if ($lb_valido) 
				{	// Elimino el movimiento de banco
					$lb_valido=$this->uf_delete_movimiento_banco($this->is_codemp,$this->is_codban,$this->is_ctaban,$this->is_numdocaux,$this->is_codope,$this->is_estmov);
					if (!$lb_valido)
					{   
										   ...
										   $ad_fechaanula,$aa_seguridad)
	{
		...
		if($lb_valido)
		{
			if(($this->ii_datasource==1)||($this->ii_datasource==4))
			{	
				$lb_valido = $this->uf_delete_movimiento_banco($this->is_codemp,$as_codban,$as_ctaban,$this->is_numdocaux,$as_codope,
															   $as_estmov, true);
				if (!$lb_valido)
				{   
