<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_anula_sep.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
<html>
<head>
<title>Anulaci&oacute;n de contabilizaci&oacute;n de solicitud de Ejecuci&oacute;n Presupuestaria</title>
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_anulacion_solicitud_sep($ls_numsol,$ls_fechaconta,$ls_fecha,$ls_conanu,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Solicitud  ".$ls_numsol." fue Anulada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo Anular la solicitud ".$ls_numsol);
					}		
				}
			}
			$li_totrows=1;
...
?>
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Anulaci&oacute;n de Solicitudes de Ejecuci&oacute;n Presupuestarias</td>
    </tr>
    <tr>
      <td  height="23"><div align="right">N&uacute;mero de Solicitud </div></td>
      <td ><div align="left">
    ...
    <tr>
      <td width="120"  height="23"><div align="right">Fecha Anulaci&oacute;n </div></td>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_anula_sep.php";
			f.submit();
		}
		else
		{
...
function ue_cargarconcepto(row)
{
	...
	if(eval("f.chksel"+row+".checked")==true)
	{
		Xpos=((screen.width/2)-(500/2)); 
		Ypos=((screen.height/2)-(400/2));
		window.open("sigesp_mis_pdt_conceptoanula_sep.php?row="+row+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=400,height=180,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
	}
}
