<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_anula_soc.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		global $li_widthtable,$ls_titletable,$ls_nametable,$ls_operacion_soc;
		global $ls_fecha,$io_ddlb,$ls_numordcom,$ls_codprov,$ls_fecapr;
		...
		$lo_title[2]="Tipo";
		$lo_title[3]="Nº Orden";
		$lo_title[4]="Fecha Orden de Compra";
		$lo_title[5]="Concepto";
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Ordenes de Compras Contabilizadas";
		$ls_nametable="grid";
		$ls_operacion =$oi_fun_integrador->uf_obteneroperacion();
		$li_totrows=$oi_fun_integrador->uf_obtenervalor("totalfilas",0);
		$la_value=array("-","B","S");
		$la_text=array("Bienes/Servicios","Bienes","Servicios");
		$io_ddlb=new ddlb_generic($la_value,$la_text,$ls_operacion_soc,"cmboperacion","","200");
   }
   //--------------------------------------------------------------
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_fecha,$ls_operacion_soc,$ls_numordcom,$ls_codprov,$ls_fecapr;
		
		$ls_fecha=$_POST["txtfecha"];
		$ls_operacion_soc=$_POST["cmboperacion"];
		$ls_numordcom=$_POST["txtnumordcom"];
		$ls_codprov=$_POST["txtcodigo"];
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][2]="<input type=text name=txtestcondat".$ai_totrows." class=sin-borde readonly style=text-align:center size=1 maxlength=1>";													
		$aa_object[$ai_totrows][3]="<input type=text name=txtnumordcom".$ai_totrows." class=sin-borde readonly style=text-align:center size=15 maxlength=15>";
		$aa_object[$ai_totrows][4]="<input type=text name=txtfecordcom".$ai_totrows." class=sin-borde readonly style=text-align:left size=30 maxlength=254>";
		$aa_object[$ai_totrows][5]="<input type=text name=txtobscom".$ai_totrows." class=sin-borde readonly style=text-align:center size=18 maxlength=22>";
...
<html>
<head>
<title>Anulaci&oacute;n de contabilizaci&oacute;n de  Compras</title>
...
<?php
	...
	require_once("class_folder/class_sigesp_soc_integracion.php");  	
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_anular_contabilizacion($ls_numordcom,$ls_estcondat,$ls_fechaconta,$ls_fecha,$ls_conanu,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Orden de Compra ".$ls_numordcom." fue Anulada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo Anular La Orden de Compra ".$ls_numordcom);
					}
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
		    $in_class_contabiliza->uf_select_ordencompra_contabilizar($ls_operacion_soc,$ls_numordcom,$ls_codprov,$ls_fecapr,2,$lo_object,$li_totrows,"ANULAR");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<table width="762" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr>
    <td width="432" height="20" colspan="11" bgcolor="#E7E7E7"><table width="762" border="0" align="center" cellpadding="0" cellspacing="0">
      <td width="432" height="20" bgcolor="#E7E7E7" class="descripcion_sistema">M&oacute;dulo Integrador - <i>Compras</i></td>
...
?>
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Anulaci&oacute;n de Ordenes de Compra </td>
    </tr>
    <tr>
      <td  height="23"><div align="right">N&uacute;mero de Orden de compra </div></td>
      <td ><div align="left">
          <input name="txtnumordcom" type="text" id="txtnumordcom" onKeyUp="javascript: ue_validarcomillas(this);" maxlength="15">
      </div></td>
    </tr>
    <tr>
      ...
      <td ><div align="left">
          ...
          <a href="javascript: ue_buscarproveedor();"><img  src="../shared/imagebank/tools20/buscar.gif" alt="Buscar" width="15" height="15" border="0"></a>
          <input name="txtnombre" type="text" class="sin-borde" id="txtnombre" size="50" maxlength="30" readonly>
      </div></td>
    </tr>
    <tr>
      <td  height="23"><div align="right">Tipo Compra</div></td>
      <td ><?php $io_ddlb->uf_cargar_combo();?>
    ...
    <tr>
      <td width="120"  height="23"><div align="right">Fecha de Anulaci&oacute;n </div></td>
...
function ue_procesar()
{
	...
	{	
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_anula_soc.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numordcom,operacion)
{
	Xpos=((screen.width/2)-(500/2)); 
	Ypos=((screen.height/2)-(400/2));
	window.open("sigesp_mis_pdt_soc.php?numordcom="+numordcom+"&operacion="+operacion+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
...
function ue_cargarconcepto(row)
{
	...
	if(eval("f.chksel"+row+".checked")==true)
	{
		Xpos=((screen.width/2)-(500/2)); 
		Ypos=((screen.height/2)-(400/2));
		window.open("sigesp_mis_pdt_conceptoanula_soc.php?row="+row+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=400,height=180,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
	}
}
