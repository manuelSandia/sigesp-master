<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_contrato_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[3]="Nº Asignación";
		$lo_title[4]="Fecha ";
		$lo_title[5]="Observación"; 
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Contratos por Contabilizar";
		$ls_nametable="grid";
		$ls_operacion =$oi_fun_integrador->uf_obteneroperacion();
		$li_totrows=$oi_fun_integrador->uf_obtenervalor("totalfilas",0);
		$li_feccontcon=trim($oi_fun_integrador->uf_select_config("MIS","OBRAS","FECHA_CONT_CONTRATOS","0","C"));
   }
   //--------------------------------------------------------------
   ...
   function uf_load_variables()
   {
		...
		// Fecha Creación: 30/04/2007 									Fecha Última Modificación : 
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_fecha,$ls_codasi,$ls_codcon,$ls_feccon,$ls_fecinicon;
...
<html>
<head>
<title>Contabilizar Contratos</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ld_fechacontaasig=$_POST["txtfechacontaasig".$li_i];
					if($li_feccontcon=="0")
					{
						$ls_fecha=$_POST["txtfeccon".$li_i];
					}
					$lb_valido=$in_class->uf_procesar_contabilizacion_contrato($ls_codcon,$ls_codasi,$ls_fecha,$ld_fechacontaasig,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Contrato  ".$ls_codcon." fue contabilizado.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar el Contrato ".$ls_codcon);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_sob_contratos_contabilizar($ls_codasi,$ls_codcon,$ls_feccon,$ls_fecinicon,0,
...
<p>&nbsp;</p>
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Contratos </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_contrato_sob.php";
			f.submit();
		}
		else
		{
