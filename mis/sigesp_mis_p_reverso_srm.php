<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_srm.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		//	   Creado Por: Ing. Yesenia Moreno del Lang
		...
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Movimientos de Pagos por Reversar";
		$ls_nametable="grid";
   ...
   function uf_load_variables()
   {
		...
		//	   Creado Por: Ing. Yesenia Moreno del Lang
		// Fecha Creación: 27/04/2009 								Fecha Última Modificación : 
		//////////////////////////////////////////////////////////////////////////////
...
<html>
<head>
<title>Reverso de Resumen de Cobranza</title>
...
<?php
	...
	require_once("class_folder/class_sigesp_srm_integracion.php");  	
	$in_class = new class_sigesp_srm_integracion();  
	uf_limpiarvariables();
	switch ($ls_operacion) 
	{
		case "NUEVO":
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_reversar_contabilizacion_cobranzas($ls_comprobante,$ls_fecha,$ls_procede,$ls_codban,$ls_ctaban,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Documento  ".$ls_comprobante." fue reversado.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar el documento ".$ls_comprobante);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_comprobantes_srm_cobranzas($ls_numdoc,'SRMCOB',$ls_fecdoc,1,$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<table width="762" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr>
    <td width="432" height="20" colspan="11" bgcolor="#E7E7E7"><table width="762" border="0" align="center" cellpadding="0" cellspacing="0">
          <td width="432" height="20" bgcolor="#E7E7E7" class="descripcion_sistema">M&oacute;dulo Integrador - <i>Rentas municipales </i></td>
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de Contabilizaci&oacute;n de Resumen de Cobranzas </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_srm.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(comprobante,procede,fecha,codban,ctaban)
{
	...
	window.open("sigesp_mis_pdt_srm.php?comprobante="+comprobante+"&procede="+procede+"&fecha="+fecha+"&codban="+codban+"&ctaban="+ctaban+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
