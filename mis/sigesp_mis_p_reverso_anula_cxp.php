<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_anula_cxp.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[5]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Solicitudes Anuladas";
		$ls_nametable="grid";
...
<html>
<head>
<title>Reverso de Anulaci&oacute;n de Cuentas por Pagar</title>
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_reversar_anular_solicitud($ls_numsol,$ld_fechaanula,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Solicitud de Pago  ".$ls_numsol." fue Reversada la Anulación.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar la Anulación de la Solicitud de Pago ".$ls_numsol);
					}		
				}
			}
			$li_totrows=1;
...
?>	
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de Anulaci&oacute;n  de Solicitudes de Cuentas por Pagar </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_anula_cxp.php";
			f.submit();
		}
		else
		{
