<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_ncd.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$la_text=array("Nota Débito","Nota Crédito");
		$ls_operacion="";
        $lo_title[1]="";
		$lo_title[2]="Nº Solicitud";
		$lo_title[3]="Nº Recepción";
		$lo_title[4]="Nº Nota";
		$lo_title[5]="Fecha Documento";
		$lo_title[6]="Descripción";
		$lo_title[7]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Notas Débito/Crédito Contabilizadas";
		$ls_nametable="grid";
...
<html>
<head>
<title>Reversar Notas de D&eacute;bito/Cr&eacute;dito</title>
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_reversar_contabilizacion_creditos_debitos($ls_numsol,$ls_numrecdoc,$ls_codtipdoc,
																					   $ls_ced_bene,$ls_cod_pro,$ls_codope,
																					   $ls_numdc,$ld_fechaconta,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Nota de Crédito/Débito  ".$ls_numsol."-".$ls_numdc." fue reversada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar la  Nota de Crédito/Débito  ".$ls_numsol."-".$ls_numdc);
					}		
				}
			}
			$li_totrows=1;
...
?>	
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de Notas Cr&eacute;dito/D&eacute;bito de Cuentas por Pagar </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td width="593"><?php $io_ddlb->uf_cargar_combo();?>
      </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_ncd.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numsol,numdc)
{
	Xpos=((screen.width/2)-(500/2)); 
	Ypos=((screen.height/2)-(400/2));
	window.open("sigesp_mis_pdt_ncd.php?numsol="+numsol+"&disponibilidad=1&numdc="+numdc,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
