<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_valuacion_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[3]="Nº Valuación";
		$lo_title[4]="Fecha ";
		$lo_title[5]="Observación"; 
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Valuaciones por Contabilizar";
		$ls_nametable="grid";
   ...
   function uf_load_variables()
   {
		...
		// Fecha Creación: 30/04/2007 									Fecha Última Modificación : 
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_fecha,$ls_codval,$ls_codcon,$ls_feccon,$ls_fecha;
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][3]="<input type=text name=txtcodval".$ai_totrows." class=sin-borde readonly style=text-align:left size=30 maxlength=30>";
...
<html>
<head>
<title>Contabilizar Valuaciones</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ls_codval=$_POST["txtcodval".$li_i];
					$ls_fecha=date("Y-m-d");
					$lb_valido=$in_class->uf_procesar_contabilizacion_valuacion($ls_codcon,$ls_codval,$ls_fecha,$ls_codtipdoc,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Contrato  ".$ls_codcon.". Valuacion ".$ls_codval." fue contabilizado.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar el Contrato ".$ls_codcon.". Valuacion ".$ls_codval);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_sob_valuaciones_contabilizar($ls_codval,$ls_codcon,$ls_feccon,$ls_fecha,0,$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<p>&nbsp;</p>
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Valuaciones </td>
    </tr>
    <tr>
    ...
    <tr>
      <td  height="23"><div align="right">N&uacute;mero de Valuacion </div></td>
      <td ><div align="left">
          <input name="txtcodval" type="text" id="txtcodval" size="10" maxlength="6">
      </div></td>
    </tr>
    ...
    <tr>
      <td width="120"  height="23"><div align="right">Fecha de Valuacion </div></td>
      <td width="380" ><div align="left">
          <input name="txtfecha" type="text" id="txtfecha" size="14" maxlength="10" datepicker="true" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);" >
      </div></td>
    </tr>
    <tr>
      <td  height="23"><div align="right">Tipo de Documento </div></td>
      <td ><div align="left">
        <input name="txtcodtipdoc" type="text" id="txtcodtipdoc" size="6" readonly>
        <a href="javascript: ue_tipodocumento();"><img src="../shared/imagebank/tools15/buscar.gif" alt="Buscar" width="15" height="15" border="0"></a>
        <input name="txtdentipdoc" type="text" class="sin-borde" id="txtdentipdoc" size="50" readonly>
      </div></td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_valuacion_sob.php";
			f.submit();
		}
		else
		{
...
function ue_tipodocumento()
{
	window.open("sigesp_sob_cat_tipodocumentos.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=630,height=400,left=50,top=50,location=no,resizable=no");
}
...
function uf_verdetalle(codval,codcon)
{
	Xpos=((screen.width/2)-(500/2)); 
	Ypos=((screen.height/2)-(400/2));
	window.open("sigesp_mis_pdt_sobval.php?codval="+codval+"&codcon="+codcon+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
</script>
