<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_anticipo_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
   ...
   function uf_limpiarvariables()
   {
		...
		global $ls_fecha,$oi_fun_integrador,$li_feccontcon;
		
		$ls_fecha=date("d/m/Y");	 
	    $lo_title[1]="";
		$lo_title[2]="Nº Contrato";
		$lo_title[3]="Nº Anticipo";
		$lo_title[4]="Fecha ";
		$lo_title[5]="Monto"; 
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Anticipos por Contabilizar";
		$ls_nametable="grid";
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][3]="<input type=text name=txtcodant".$ai_totrows." class=sin-borde readonly style=text-align:left size=30 maxlength=30>";
...
<html>
<head>
<title>Contabilizar Anticipos</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ld_fechacontacontrato=$_POST["txtfechacontacontrato".$li_i];
					$lb_valido=$in_class->uf_procesar_contabilizacion_anticipo($ls_codcon,$ls_codant,$ls_fecha,$ld_fechacontacontrato,$ls_tipdoc,$ls_codpro,
																			   $li_monto,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("El Anticipo ".$ls_codant." del Contrato ".$ls_codcon." fue contabilizado.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar el Anticipo ".$ls_codant." del Contrato ".$ls_codcon);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_sob_anticipos_contabilizar($ls_codant,$ls_codcon,$ls_feccon,$ls_fecant,0,$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<form name="form1" method="post" action=""><table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Anticipos </td>
    </tr>
    <tr>
      <td  height="23"><div align="right">C&oacute;digo de Contrato </div></td>
      <td ><label>
        <input name="txtcodcon" type="text" id="txtcodcon" size="15" maxlength="12">
      </label></td>
    </tr>
    <tr>
      <td  height="23"><div align="right">C&oacute;digo de Anticipo </div></td>
      <td ><div align="left">
          <input name="txtcodant" type="text" id="txtcodant" size="10" maxlength="6">
      </div></td>
    </tr>
    ...
    <tr>
      <td width="120"  height="23"><div align="right">Fecha de Anticipo </div></td>
      <td width="380" ><div align="left">
          <input name="txtfecant" type="text" id="txtfecant" size="14" maxlength="10" datepicker="true" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);" >
      </div></td>
    </tr>
    <tr>
    ...
    <tr>
      <td  height="23"><div align="right">Tipo de Recepci&oacute;n de Documentos </div></td>
      <td ><div align="left">
        <input name="txttipdoc" type="text" id="txttipdoc" readonly>
        <a href="javascript: ue_buscartipodocumento();"><img src="../shared/imagebank/tools20/buscar.gif" alt="Buscar" width="15" height="15" border="0"></a> </div></td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		tipodocumento =f.txttipdoc.value;
		if (tipodocumento=='')
		{
			lb_valido=false;
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_anticipo_sob.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codcon,codant)
{
	Xpos=((screen.width/2)-(500/2)); 
	Ypos=((screen.height/2)-(400/2));
	window.open("sigesp_mis_pdt_sobant.php?codcon="+codcon+"&codant="+codant,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}

function ue_buscartipodocumento()
{
	window.open("sigesp_mis_cat_tipodocumento.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
}
