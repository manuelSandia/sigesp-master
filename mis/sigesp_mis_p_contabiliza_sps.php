<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_sps.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_fecha, $ls_codper, $ls_tipdoc;
...
<html>
<head>
<title>Contabilizaci&oacute;n de Prestaciones Sociales</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido = $in_class->uf_procesar_contabilizacion_prestaciones($ls_comprobante,$ls_tipdoc,$ls_fecha,$la_seguridad);
					if($lb_valido)
					{
		...
		case "BUSCAR":
			uf_load_variables();
		    $in_class_contabiliza->uf_select_prestaciones_contabilizar($ls_codper,$lo_object,$li_totrows,0);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Prestaciones Sociales </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td ><input name="txttipdoc" type="text" id="txttipdoc" readonly>
        <a href="javascript: ue_buscartipodocumento();"><img src="../shared/imagebank/tools20/buscar.gif" alt="Buscar" width="15" height="15" border="0"></a> </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	{	
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_sps.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codcom)
{
	...
	window.open("sigesp_mis_pdt_sps.php?codcom="+codcom+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
...
{
	window.open("sigesp_mis_cat_tipodocumento.php?tipo=SPS&docanti=0","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
}
