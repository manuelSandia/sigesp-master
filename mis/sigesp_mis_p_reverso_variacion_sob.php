<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_variacion_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Variaciones Contabilizadas";
		$ls_nametable="grid";
...
<html>
<head>
<title>Reversar Contabilizaci&oacute;n de Obras</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_reverso_variacion($ls_codcon,$ls_codvar,$ld_fechaconta,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Variación ".$ls_codvar." del Contrato  ".$ls_codcon." fue reversada la contabilizacion.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar la Contabilización de la Variación ".$ls_codvar." del Contrato ".$ls_codcon);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_sob_variaciones_contabilizar($ls_codvar,$ls_codcon,$ls_feccon,$ls_fecha,1,$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<p>&nbsp;</p>
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de Contabilizaci&oacute;n de Contratos </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_variacion_sob.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codvar,codcon)
{
	...
	window.open("sigesp_mis_pdt_sobvar.php?codvar="+codvar+"&codcon="+codcon+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
</script>
