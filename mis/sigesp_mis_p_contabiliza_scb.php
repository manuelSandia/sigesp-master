<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_scb.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[3]="Nº Carta Orden";
		$lo_title[4]="Fecha Movimiento";
		$lo_title[5]="Concepto";
		$lo_title[6]="Detalle";
		$li_widthtable=750;
		$ls_titletable="Movimientos de Banco por Contabilizar";
		$ls_nametable="grid";
		...
		$li_feccondep=trim($oi_fun_integrador->uf_select_config("MIS","BANCO","FECHA_CONT_DEPOSITOS","0","C"));
   }
   //--------------------------------------------------------------
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_fecha,$ls_numdoc,$ls_fecdoc,$ls_tipo_operacion,$ls_numcarord;
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][3]="<input type=text name=txtnumcarord".$ai_totrows." class=sin-borde readonly style=text-align:center size=15 maxlength=15>";
		$aa_object[$ai_totrows][4]="<input type=text name=txtfecmov".$ai_totrows." class=sin-borde readonly style=text-align:left size=30 maxlength=254>";
		$aa_object[$ai_totrows][5]="<input type=text name=txtconmov".$ai_totrows." class=sin-borde readonly style=text-align:center size=18 maxlength=22>";
...
<html>
<head>
<title>Contabilizaci&oacute;n de Banco</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_contabilizacion_banco($ls_codban,$ls_ctaban,$ls_numdoc,$ls_codope,
																	        $ls_estmov,$ls_fecha,$li_feccondep,$la_seguridad);
					if($lb_valido)
					{
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_banco_contabilizar($ls_numdoc,$ls_fecdoc,$ls_tipo_operacion,$ls_numcarord,$lo_object,$li_totrows,"N");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
  ...
  <p>&nbsp;</p>
  <table width="950" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Banco</td>
    </tr>
    <tr>
    ...
    <tr>
      <td width="155" height="23"><div align="right">N&uacute;mero de Carta Orden </div></td>
      <td width="593"><input name="txtnumcarord" type="text" id="txtnumcarord" onKeyUp="javascript: ue_validarcomillas(this);" maxlength="15"></td>
    </tr>
    <tr>
...
<?php
	if(($ls_tipo_operacion=="DP")&&($li_feccondep==1))
	{
?>
    <tr>
      <td width="155" height="23"><div align="right">Fecha Contabilizaci&oacute;n </div></td>
...
<?php
	...
	{
		print "<input name='txtfecha' type='hidden' id='txtfecha' value='".$ls_fecha."'>";
	}
?>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_scb.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numdoc,codban,ctaban,codope)
{
	...
	window.open("sigesp_mis_pdt_scb.php?numdoc="+numdoc+"&codban="+codban+"&ctaban="+ctaban+"&codope="+codope+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
