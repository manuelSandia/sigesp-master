<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_ncd.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[7]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Notas Débito/Crédito por Contabilizar";
		$ls_nametable="grid";
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					if($lb_valido)
					{
						$in_class->io_msg->message("La Nota de Crédito/Débito  ".$ls_numsol."-".$ls_numdc." fue contabilizada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar la  Nota de Crédito/Débito  ".$ls_numsol."-".$ls_numdc);
					}		
				}
			}
			$li_totrows=1;
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_ncd.php";
			f.submit();
		}
		else
		{
