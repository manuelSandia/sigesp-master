<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_sep.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
<html>
<head>
<title>Contabilizar Solicitud de Ejecuci&oacute;n Presupuestaria</title>
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ls_fecregsol=$_POST["txtfecregsol".$li_i];		
					$ls_consol=$_POST["txtconsol".$li_i];
					$lb_valido=$in_class->uf_procesar_contabilizacion_solicitud_sep($ls_numsol,$ls_fecha,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Solicitud  ".$ls_numsol." fue contabilizada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar la solicitud ".$ls_numsol);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
	    	...
	    	$in_class_contabiliza->uf_select_solicitudes_contabilizar($ls_numsol,$ls_fecreg,$ls_fecapr,$ls_codigo,$ls_tipo,
																	  $lo_object,$li_totrows,"E","CONTABILIZAR");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Solicitudes Presupuestarias</td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_sep.php";
			f.submit();
		}
		else
		{
