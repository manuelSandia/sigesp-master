<?php
   ...
   function uf_imprimirresultados($as_codcom,$as_procede)
   {
		...
		$ls_sql="SELECT comprobante, fecha, procede ". 
				"  FROM mis_sigesp_cxc ".
		...
		else
		{
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				print "  </tr>";
				print "  <tr>";
				print "		<td><div align='right' class='texto-azul'>Descripción</div></td>";
				...
				print "  </tr>";
				print "</table>";
				if($ls_procede=="SFCFAC")
				{
					$ls_sql="SELECT spi_cuenta, monto, '-------------------------' as codestpro1,  '-------------------------' as codestpro2,".
							"       '-------------------------' as codestpro3, '-------------------------' as codestpro4, '-------------------------' as codestpro5,'-' as estcla ".
							"  FROM mis_sigesp_cxc ".
							" WHERE codemp = '".$ls_codemp."' ".
							"   AND procede = '".$as_procede."' ".
							"   AND comprobante = '".$as_codcom."'".
					...
					else
					{
						...
						while($row=$io_sql2->fetch_row($rs_data2))
						{
							...
							else
							{
								...
								print "<td align=center width='150'>".$ls_estcla."</td>";
								print "<td align=center width='150'>".$ls_cuenta."</td>";
								print "<td align=right width='150'>".$li_monto."  </td>";
								print "</tr>";
							}				
