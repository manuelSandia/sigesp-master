<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_sfc_cxc.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[5]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Comprobantes de Cuentas por Cobrar";
		$ls_nametable="grid";
...
<html>
<head>
<title>Contabilizaci&oacute;n de Cuentas por Cobrar</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_contabilizacion_comprobantes_cxc($ls_comprobante,$ld_fecha,$ls_procede,$ls_descripcion,$ld_fecha,$la_seguridad);
		...
		case "BUSCAR":
			uf_load_variables();
			$in_class_contabiliza->uf_select_comprobantes_sfc_cxc($ls_comprobante,"SFCFAC",$ld_fecha,"0",$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Cuentas por Cobrar </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_sfc_cxc.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codcom,procede)
{
	...
	window.open("sigesp_mis_pdt_sfc_cxc.php?codcom="+codcom+"&procede="+procede+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
