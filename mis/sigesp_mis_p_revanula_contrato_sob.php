		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					if($lb_valido)
					{
						$in_class->io_msg->message("Contrato  ".$ls_codcon." fue reversada la Anulación de contabilizacion.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar la Anulación de Contabilización del Contrato ".$ls_codcon);
					}		
				}
			}
			$li_totrows=1;
