<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_anula_rd.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Recepciones por Contabilizar";
		$ls_nametable="grid";
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_numrecdoc,$ls_fecemi,$ls_fecapr,$ls_codigo,$ls_tipo;
...
<html>
<head>
<title>Contabilizar Cuentas por Pagar</title>
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_reversar_anular_recepcion($ls_numrecdoc,$ls_codtipdoc,$ls_codpro,$ls_cedbene,$ld_fechaanula,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message(" Se Reverso la Anuulación de La Recepcion de Documentos  ".$ls_numrecdoc."");
					}
					else
					{
						$in_class->io_msg->message("No se pudo Reversar la anulación de la Recepcion de Documentos ".$ls_numrecdoc);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
	    	...
	    	$in_class_contabiliza->uf_select_recepciones_contabilizar($ls_numrecdoc,$ls_fecemi,$ls_fecapr,$ls_codigo,$ls_tipo,
																	  $lo_object,$li_totrows,"A","REVERSO_ANULA");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>	
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de Anulaci&oacute;n  de Recepciones de Documentos </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_anula_rd.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codrecdoc)
{
	...
	window.open("sigesp_mis_pdt_rd.php?codrecdoc="+codrecdoc+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
