<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_saf.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		global $ls_fecha,$ls_numdoc,$ls_fecdoc;
		...
		$lo_title[5]="Detalle";
		$li_widthtable=700;
		$ls_titletable= "Desincorporaciones por Contabilizar";
		$ls_nametable = "grid";
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][2]="<input type=text name=txtnumdoc".$ai_totrows." class=sin-borde readonly style=text-align:center size=15 maxlength=15><input type=hidden name=txtnumcmp".$ai_totrows.">";
		$aa_object[$ai_totrows][3]="<input type=text name=txtfecmov".$ai_totrows." class=sin-borde readonly style=text-align:left size=15 >";
		$aa_object[$ai_totrows][4]="<input type=text name=txtconmov".$ai_totrows." class=sin-borde readonly style=text-align:center size=40 >";
...
<html>
<head>
<title>Contabilizaci&oacute;n de Desincorporaci&oacute;n de Activos</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_contabilizar_desincorporacion($ls_numcmp, $ls_conmov, $ls_fecha, $ls_fecmov, $la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Se proceso el Documento  ".$ls_numdoc.", generando el comprobante ".$ls_numcmp);
					}
					else
					{
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_desincorporacion_contabilizar($ls_numdoc,$ls_fecdoc,$lo_object,$li_totrows,0);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<table width="762" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr>
    <td width="432" height="20" colspan="11" bgcolor="#E7E7E7"><table width="762" border="0" align="center" cellpadding="0" cellspacing="0">
        <td width="432" height="20" bgcolor="#E7E7E7" class="descripcion_sistema">M&oacute;dulo Integrador - <i>Activos</i></td>
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Activos</td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_saf.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numdoc,fecdoc)
{
	Xpos=((screen.width/2)-(500/2)); 
	Ypos=((screen.height/2)-(400/2));
	window.open("sigesp_mis_pdt_safd.php?numdoc="+numdoc+"&fecdoc="+fecdoc,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
