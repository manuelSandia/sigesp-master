		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					if ($lb_valido)
					   {
						 $in_class->io_msg->message("La Depreciación del Comprobante ".$ls_comprobante." fué Reversada.");
					   }
					else
					   {
						 $in_class->io_msg->message("No se pudo Reversar la Depreciación del Comprobante ".$ls_comprobante);
					   }		
				}
			}
			$li_totrows=1;
