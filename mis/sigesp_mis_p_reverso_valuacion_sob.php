<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_valuacion_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
...
<html>
<head>
<title>Reverso de Contabilizar Valuaciones</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ls_fecha=date("Y-m-d");
					$lb_valido=$in_class->uf_procesar_reverso_valuacion($ls_codcon,$ls_codval,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Contrato  ".$ls_codcon.". Valuacion ".$ls_codval." fue reversado.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar el Contrato ".$ls_codcon.". Valuacion ".$ls_codval);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_sob_valuaciones_reversar($ls_codval,$ls_codcon,$ls_feccon,$ls_fecha,0,$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<p>&nbsp;</p>
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de Contabilizaci&oacute;n de Valuaciones </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		f.action="sigesp_mis_p_reverso_valuacion_sob.php";
		f.submit();
	}
	else
	{
