<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_anula_sep.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		global $ls_fecha,$ls_numordcom,$ls_codprov,$ls_fecapr;
		...
		$lo_title[5]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Solicitudes de Ejecucion Presupuestaria Anuladas";
		$ls_nametable="grid";
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_fecha,$ls_numsol,$ls_fecreg,$ls_fecapr,$ls_codigo,$ls_tipo;
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_reverso_anulacion($ls_numsol,$ld_fechaconta,$ld_fechaanula,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Solicitud de Ejecución Presupuestaria ".$ls_numsol." le fue Reversada la Anulación.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo Reversar la anulación de la Solicitud de Ejecución Presupuestaria ".$ls_numsol);
					}
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
		    $in_class_contabiliza->uf_select_solicitudes_anuladas($ls_numsol,$ls_fecreg,$ls_fecapr,$ls_codigo,$ls_tipo,
...
?>
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de Anulaci&oacute;n de de Solicitudes de Ejecuci&oacute;n Presupuestarias</td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	{	
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_anula_sep.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numsol)
{
	...
	window.open("sigesp_mis_pdt_sep.php?numsol="+numsol+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
function ue_limpiar()
{
	f=document.form1;
