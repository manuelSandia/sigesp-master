<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_reverso_anticipo_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Anticipos por Contabilizados";
		$ls_nametable="grid";
   ...
   function uf_load_variables()
   {
		...
		// Fecha Creación: 30/04/2007 									Fecha Última Modificación : 
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_codant,$ls_codcon,$ls_feccon,$ls_fecant;
...
<html>
<head>
<title>Reversar Contabilizaci&oacute;n de Anticipos</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_reverso_anticipo($ls_codcon,$ls_codant,$ls_codpro,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("El Anticipo ".$ls_codant." del Contrato ".$ls_codcon." fue Reversado.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo Reversar el Anticipo ".$ls_codant." del Contrato ".$ls_codcon);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_sob_anticipos_contabilizar($ls_codant,$ls_codcon,$ls_feccon,$ls_fecant,1,$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
  ...
  <tr>
    <td width="432" height="20" colspan="11" bgcolor="#E7E7E7"><table width="762" border="0" align="center" cellpadding="0" cellspacing="0">
      <td width="432" height="20" bgcolor="#E7E7E7" class="descripcion_sistema">M&oacute;dulo Integrador - <i>Obras</i></td>
            <td width="346" bgcolor="#E7E7E7"><div align="right" class="letras-pequenas"><b><?PHP print date("j/n/Y")." - ".date("h:i a");?></b></div></td>
      <tr>
...
<table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
  <tr class="titulo-ventana">
    <td colspan="2">Reverso de Contabilizaci&oacute;n de Anticipos </td>
  </tr>
  <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_reverso_anticipo_sob.php";
			f.submit();
		}
		else
		{
