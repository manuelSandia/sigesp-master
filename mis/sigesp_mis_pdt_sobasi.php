<?php
   ...
   function uf_imprimirresultados($as_codcom,$as_disponibilidad)
   {
		...
		$ls_group="";
		$ls_criterio="";
		$ls_sql="SELECT sob_asignacion.obsasi, sob_asignacion.cod_pro, sob_asignacion.codobr, sob_obra.desobr, sob_asignacion.fecasi, ".
		...
		else
		{
			while(!$rs_data->EOF)
			{
				$ls_descripcion=$rs_data->fields["obsasi"];
				$ls_desobr=$rs_data->fields["codobr"]." - ".$rs_data->fields["desobr"];
				$ls_nombre_destino=$rs_data->fields["cod_pro"]." - ".$rs_data->fields["nompro"];
				$ld_fecasi=$io_funciones->uf_convertirfecmostrar($rs_data->fields["fecasi"]);
				$_SESSION["fechacomprobante"]=$ld_fecasi;
				...
				print "		<td width='350'><div align='left'>".$ld_fecasi."</div></td>";
				print "  </tr>";
				print "  <tr>";
...
<?php
	...
	uf_imprimirresultados($ls_codasi,$ls_disponibilidad);
?>
</div>
</form>
</body>
