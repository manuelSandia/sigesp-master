<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_mp.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		global $ld_fecha,$ls_comprobante,$ls_procede,$ld_fecaprob;

		$ld_fecha="";	 
		$ls_comprobante="";
		$ls_procede="";
		$ld_fecaprob=date("d/m/Y");
        $lo_title[1]="";
		...
		$lo_title[5]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Comprobantes de Presupuesto de Gasto";
		$ls_nametable="grid";
...
<?php
	...
	require_once("class_folder/class_sigesp_spg_integracion.php");  	
	$in_class = new class_sigesp_spg_integracion();  
	uf_limpiarvariables();
	switch ($ls_operacion) 
	{
		case "NUEVO":
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_contabilizacion_comprobantes($ls_comprobante,$ld_fecha,$ls_procede,
																				   $ls_descripcion,$ld_fecaprob,$la_seguridad);

					if($lb_valido)
					{
						$in_class->io_msg->message("Modificación Presupuestaria ".$ls_comprobante." fue Aprobada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo aprobar la Modificación Presupuestaria ".$ls_comprobante);
					}		
				}
			}
			uf_limpiarvariables();
		...
		case "BUSCAR":
			uf_load_variables();
			$in_class_contabiliza->uf_select_comprobantes_spg($ls_comprobante,$ls_procede,$ld_fecha,"0",$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<table width="762" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr>
    <td width="432" height="20" colspan="11" bgcolor="#E7E7E7"><table width="762" border="0" align="center" cellpadding="0" cellspacing="0">
        <td width="432" height="20" bgcolor="#E7E7E7" class="descripcion_sistema">M&oacute;dulo Integrador - <i>Contabilidad Presupuestaria de Gasto </i></td>
...
?>		  
    ...
    <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
      <tr class="titulo-ventana">
        <td colspan="2">Aprobaci&oacute;n de Modificaciones Presupuestarias de Gasto </td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td ><label>
          <input name="txtfechadoc" type="text" id="txtfechadoc" style="text-align:center" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);" value="<?php print $ld_fecha;?>" maxlength="10" datepicker="true">
        </label></td>
      </tr>
      <tr>
        <td width="150"  height="23"><div align="right">Fecha de Aprobaci&oacute;n</div></td>
        <td width="598" ><div align="left">
            <input name="txtfecaprob" type="text" id="txtfecaprob" maxlength="10"  onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);" value="<?php print $ld_fecaprob; ?>" style="text-align:center" datepicker="true">
        </div></td>
      </tr>
      <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_mp.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codcom,procede)
{
	...
	window.open("sigesp_mis_pdt_spg.php?codcom="+codcom+"&procede="+procede+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
</script>
