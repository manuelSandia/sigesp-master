<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_cxp.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_numsol,$ls_fecemi,$ls_fecapr,$ls_codigo,$ls_tipo;
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][3]="<input type=text name=txtfecemisol".$ai_totrows." class=sin-borde readonly style=text-align:left size=30 maxlength=254>";
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$ls_fecemisol=$_POST["txtfecemisol".$li_i];		
					if($_SESSION["la_empresa"]["conrecdoc"]=="1")
					{
						$lb_valido=$in_class->uf_procesar_contabilizacion_solicitud_rd($ls_numsol,$ls_fecemisol,$la_seguridad);
					}
					else
					{
						$lb_valido=$in_class->uf_procesar_contabilizacion_solicitud_cxp($ls_numsol,$ls_fecemisol,$la_seguridad);
					}
					if($lb_valido)
					{
						$in_class->io_msg->message("La Solicitud de Pago  ".$ls_numsol." fue contabilizada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar la Solicitud de Pago ".$ls_numsol);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_orden_pago_contabilizar($ls_numsol,$ls_fecemi,$ls_fecapr,$ls_codigo,$ls_tipo,
...
?>	
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Solicitudes de Cuentas por Pagar </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_cxp.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numsol)
{
	...
	window.open("sigesp_mis_pdt_cxp.php?numsol="+numsol+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
