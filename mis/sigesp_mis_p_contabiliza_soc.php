<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_contabiliza_soc.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Ordenes de Compras Emitidas";
		$ls_nametable="grid";
...
<html>
<head>
<title>Contabilizaci&oacute;n de Compras</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_contabilizar_ordencompra($ls_numordcom,$ls_estcondat,$ls_fecha,$la_seguridad);
					
					if($lb_valido)
					{
						$in_class->io_msg->message("La Orden de Compra ".$ls_numordcom." fue contabilizada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar La Orden de Compra ".$ls_numordcom);
					}
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
		    $in_class_contabiliza->uf_select_ordencompra_contabilizar($ls_operacion_soc,$ls_numordcom,$ls_codprov,$ls_fecapr,1,$lo_object,$li_totrows,"CONTABILIZAR");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Ordenes de Compra </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	{	
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_contabiliza_soc.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numordcom,operacion)
{
	...
	window.open("sigesp_mis_pdt_soc.php?numordcom="+numordcom+"&operacion="+operacion+"&disponibilidad=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
</script>
</html>