...
function ue_tiempo_servicio()
{
	...
	if ( (ue_valida_null($('txtfecingper')))&& (ue_valida_null($('txtfecegrper'))) )
	{
		...
		//años
		ls_anoini = (ls_fecdes.substr(6,4));
		ls_anofin = (ls_fechas.substr(6,4));
		ls_ano = eval(ls_anofin)-eval(ls_anoini);
		//ls_ano = restarFechasLiq("txtfecingper","txtfecegrper","a");
		//meses	
		ls_mesini = (ls_fecdes.substr(3,2));
		ls_mesfin = (ls_fechas.substr(3,2));
		if (eval(ls_mesini)<=eval(ls_mesfin))
		{
			ls_mes    = (eval(ls_mesfin)-eval(ls_mesini));
		}
		else
		{
			ls_ano = eval(ls_ano)-1;
			ls_mesres = eval(12-ls_mesini);
			ls_mes    = (eval(ls_mesfin)+eval(ls_mesres));			
		}			    
		//dias
		ls_diaini = (ls_fecdes.substr(0,2));
		ls_diafin = (ls_fechas.substr(0,2));
	    if (ls_diafin<ls_diaini)
		{  
			ls_mes=(ls_mes-1); 
			ls_dia    = ((30 + eval(ls_diafin)) - eval(ls_diaini));
		}
		else
		{
			ls_dia    = (eval(ls_diafin) - eval(ls_diaini));
		}
	    //ENVIANDO LOS DATOS
	   	f.txtano.value = ls_ano;
	   	f.txtmes.value = ls_mes;
...
function ue_buscarpersonal()
{
  ...
  catalogo = popupWin(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=380,resizable=yes,location=no,top=0,left=0");
}
/*----------------------------------------------------------------------------------------------------------------------------------------------*/          
...
function copiarDatosDetalle(Fila, deducion)
{
  ...
  if (deducion)
  {
	  ...
	  var hidden2 = '<input name="id_art'+Fila.id+'" type="hidden" value="----">';
  }
  else
  { 
	  var hidden = '<input name="hid'+Fila.id+'" type="hidden" value="">';
	  var hidden2 =  '<input name="id_art'+Fila.id+'" type="hidden" value="'+$F("cmbarticulo")+'">';
  }
  var ultima_columna = boton+hidden+hidden2; 
	...
	{ 	
		...
		{	
			...
			{  
				...
				li_anio = ld_fecvenvac.substr(0,4);
				li_mes = ld_fecvenvac.substr(5,2);
				li_dia = ld_fecvenvac.substr(8,2);
				var fecvac = (li_dia+"/"+li_mes+"/"+li_anio);
	...
	{ 
		...
		{	
			...
			{  
				...
				li_mes = ld_fecvenvac.substr(5,2);
				li_dia = ld_fecvenvac.substr(8,2);
				var fecbonvac = (li_dia+"/"+li_mes+"/"+li_anio);
...
function ue_aplicar_articulo(literal)
{
	...
	ls_estpro   =trim(literal.estpro);
	ls_valmaxpro=trim(literal.valmaxpro);
    ...
    lb_valido   =false; 
		...
		{   
			...
			if (ls_operador==3)  // >=
			{  
				if ((eval(li_meses)>eval(ls_canmes))||(eval(li_meses)==eval(ls_canmes)))
				{   
					$('hidliteral').value = ls_numlitart;
					$('hiddiasal').value  = ls_diasal;
					$('hiddiasal').value  = ls_diasal;
	...
	{   
		li_resta = eval(15.00);
		li_dias = ue_calcular_dias_acum( li_meses,ls_canmes,ls_estacu,ls_estpro,ls_valmaxpro);
		...
		if ( (ls_numart=='108')&&(ld_diaabo!=0)&&($('hiddiaabo').value==0)&&(ls_id_art=='0001') )
		{   
			if (li_dias<ld_diaabo) 
			{	alert("Los días abonados no deben ser mayor a los días correspondientes a Antiguedad Art. 108."); }
			else
			{  
				if ((li_fecing==0)&&(li_fecing!=""))
				{ 
				     li_dias = li_dias-li_resta;
				}
				li_dias = (li_dias-ld_diaabo);
			    $('txtdiasal').value = uf_convertir(eval(li_dias),2);
		...
		if ( (ls_numart=='108')&&((ls_numlitart=='B')||(ls_numlitart=='b'))&&(ls_id_art=='0001') )
		{   
			$('txtdiasal').value = uf_convertir(eval(li_dias),2);
			ld_monto = (uf_convertir_monto($F('txtsalintdia'))*eval(li_dias));
			$('txtdescripcion').value = "Antiguedad complementaria";
			$('txtmonto').value = uf_convertir(eval(ld_monto),2);
			$(hidok).value=0;
  	        $(hidand).value=0;
			ue_agregar_detalle();
		}
		else if ( (ls_numart=='108')&&(ld_diaabo==0)&&($('hiddiaabo').value==0)&&(ls_id_art=='0001') )
		{   
			if ((li_fecing==0)&&(li_fecing!=""))
			{ 
				li_dias = li_dias-li_resta;
		...
		if ( (ls_numart=='108')&&(ls_id_art=='0005') ) //PARAGRAFO 1   
		{   
			if (li_dias<ld_diaabo) 
			{	alert("Los días abonados no deben ser mayor a los días correspondientes a Antiguedad Art. 108."); }
			else
		...
		if ((ls_id_art!='0001')&&(ls_numart!='108'))
		{  
			$('txtdiasal').value = uf_convertir(eval(li_dias),2);
			ld_monto = (uf_convertir_monto($F('txtsalintdia'))*eval(li_dias));
			$('txtmonto').value = uf_convertir(eval(ld_monto),2);
...
//--------------------------------------------------------------------------------------------------------------------------------------------//
function ue_calcular_dias_acum( ai_meses,ai_canmes,as_estacu,as_estpro,as_valmaxpro)
{  	
	if ($('hidtiempo').value=='A') //si es cada año
	{    
		  li_fecing =restarFechasLiq("txtfecdes","hidfvigente","m");
		  if (ai_canmes==24)
		  { 
			  numero = (ai_meses/12);
		  ...
		  //SI LA FECHA DESDE ES IGUAL A LA FECHA VIGENTE DE LA LEY
		  if ((li_fecing==0)&&(li_fecing!=""))
		  { 
		  	 numero = eval(numero+1);
		  }
	...
	if ($('hidtiempo').value=='V'){
		 li_dias=$('hiddiasal').value; }	
...
function ue_calcular()
{
	...
	{   
		...
		{  
			...
			for (li=0; li<articulo.numlitart.length; li++) 
			{      
				  var literal  = 
				  {
				  	   "id_art":articulo.id_art[li], 
					   "numart":articulo.numart[li],
					   "conart":articulo.conart[li],
					"numlitart":articulo.numlitart[li],
					 "operador":articulo.operador[li],
					   "canmes":articulo.canmes[li],
					   "tiempo":articulo.tiempo[li],
					   "diasal":articulo.diasal[li],
					"condicion":articulo.condicion[li],
					   "estacu":articulo.estacu[li],
					   "diaacu":articulo.diaacu[li],
					   "numcon":articulo.numcon[li],
					   "estpro":articulo.estpro[li],
					   "valmaxpro":articulo.valmaxpro[li]
				  };
...
{
	lb_valido=true;
	var la_objetos =new Array("txtfecliq","txtnumliq","txtcodper","txtcodnom","cmbcauret");
	var la_mensajes=new Array("la fecha de liquidación", "el numero de liquidación","el código de personal", "el código de  nómina", "La causa de Retiro" );
	lb_valido = valida_datos_llenos(la_objetos, la_mensajes);
	if(lb_valido)
	{
		...
		else
		{
			...
			else
			{
				...
				for (f=1; f<filas.length; f++)
				{
					...
					id_art=eval("document.form1.id_art"+f+".value");
