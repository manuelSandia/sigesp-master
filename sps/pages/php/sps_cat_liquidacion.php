...
{
  $lb_hay = $lo_dao->get_liquidacion($_GET["numliq"],$_GET["nomper"],$_GET["apeper"],"ORDER BY numliq ASC",$la_datos);
  ...
  else
  {
	...
	{   
	  ...
	  $ls_codcauret = $la_datos["codcauret"][$i];
	  $ls_fecliq  = $la_datos["fecliq"][$i];
	  $ls_fecing  = $la_datos["fecing"][$i];
	  $ls_fecegr  = $la_datos["fecegr"][$i];
	  $ls_salint  = $la_datos["salint"][$i];
	  $ls_descargo= $la_datos["descargo"][$i];
	  $ls_anoser  = $la_datos["anoser"][$i];
	  $ls_messer  = $la_datos["messer"][$i];
	  $ls_diaser  = $la_datos["diaser"][$i];
	  $ls_totasiliq = $la_datos["totasiliq"][$i];
	  $ls_totdedliq = $la_datos["totdedliq"][$i];
	  $ls_totpagliq = $la_datos["totpagliq"][$i];
	  $ls_estliq  = $la_datos["estliq"][$i];
	  $ls_dedicacion = $la_datos["dedicacion"][$i];
	  $ls_tipopersonal = $la_datos["tipopersonal"][$i];
	  $ls_diaabofid = $la_datos["diaabofid"][$i];
...
else
{
  ...
  $lb_hay = $lo_dao->get_liquidacion($ls_numliq, $ls_nomper, $ls_apeper, "ORDER BY $sort_col $sort_dir LIMIT $offset,$pagesize",$la_datos); 
  if ($lb_hay)
  {
    $li_registros = (count($la_datos,COUNT_RECURSIVE)/count($la_datos))-1;  
    ...
    for($i=0; $i<$li_registros; $i++)
    {
	  ...
	  $la_arr[$li_i++] = $ls_codcauret = $la_datos["codcauret"][$i];
	  $la_arr[$li_i++] = $ls_fecliq    = $la_datos["fecliq"][$i];
	  $la_arr[$li_i++] = $ls_fecing  = $la_datos["fecing"][$i];
	  $la_arr[$li_i++] = $ls_fecegr  = $la_datos["fecegr"][$i];
	  $la_arr[$li_i++] = $ls_salint  = $la_datos["salint"][$i];
	  $la_arr[$li_i++] = $ls_descargo= $la_datos["descargo"][$i];
	  $la_arr[$li_i++] = $ls_anoser  = $la_datos["anoser"][$i];
	  $la_arr[$li_i++] = $ls_messer  = $la_datos["messer"][$i];
	  $la_arr[$li_i++] = $ls_diaser  = $la_datos["diaser"][$i];
	  $la_arr[$li_i++] = $ls_totasiliq = $la_datos["totasiliq"][$i];
	  $la_arr[$li_i++] = $ls_totdedliq = $la_datos["totdedliq"][$i];
	  $la_arr[$li_i++] = $ls_totpagliq = $la_datos["totpagliq"][$i];
	  $la_arr[$li_i++] = $ls_estliq  = $la_datos["estliq"][$i];
	  $la_arr[$li_i++] = $ls_dedicacion = $la_datos["dedicacion"][$i];
	  $la_arr[$li_i++] = $ls_tipopersonal = $la_datos["tipopersonal"][$i];
	  $la_arr[$li_i++] = $ls_diaabofid = $la_datos["diaabofid"][$i];
      $la_newarr= "new Array(";
