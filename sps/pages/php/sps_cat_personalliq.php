...
{
  $lb_hay = $lo_dao->getPersonalliq("ORDER BY p.codper ASC",$la_datos,$_GET["cedper"],$_GET["nomper"],$_GET["apeper"],$_GET["codnom"]);
  if (!$lb_hay)
  {
  ...
  else
  {
	...
	{   
	  ...
	  //$ls_fecingper = $la_datos["fecingper"][$i];
//	  $ls_sueintper = $la_datos["sueintper"][$i];
//	  $ls_denasicar = $la_datos["denasicar"][$i];
//	  $ls_desded = $la_datos["desded"][$i];
//	  $ls_destipper = $la_datos["destipper"][$i];
...
else
{
  ...
  $lb_hay = $lo_dao->getPersonalliq("ORDER BY $sort_col $sort_dir LIMIT $offset,$pagesize",$la_datos,$ls_cedper,$ls_nomper,$ls_apeper,$ls_codnom);
  if ($lb_hay)
  {
    $li_registros = (count($la_datos,COUNT_RECURSIVE)/count($la_datos))-1;  
    ...
    for($i=0; $i<$li_registros; $i++)
    {
      $li_i = 1;
	  $la_arr = array();
	  $ls_cargo=$la_datos["denasicar"][$i];
	  if ($la_datos["codasicar"][$i]=='0000000')
	  {
		 	if ($la_datos["codcar"][$i]!='0000000000')
			{
				$ls_cargo=$la_datos["descar"][$i];
			}
			else
			{
				$ls_cargo="";
	  ...
	  $la_arr[$li_i++] = $ls_fecingper = $la_datos["fecingper"][$i];
	  $la_arr[$li_i++] = $ls_sueintper = $la_datos["sueintper"][$i];
	  $la_arr[$li_i++] = $ls_denasicar = $ls_cargo;
	  $la_arr[$li_i++] = $ls_desded    = $la_datos["desded"][$i];
	  $la_arr[$li_i++] = $ls_destipper = $la_datos["destipper"][$i];
