...
if ($ls_operacion=="ue_ver_sc_cuentas")
{
  $lb_hay = $lo_dao->get_sc_cuenta($_GET["sc_cuenta"],$_GET["denominacion"],"ORDER BY sc_cuenta ASC",$la_datos);
  ...
  else
  {
	...
	{
	  $ls_sc_cuenta   = $la_datos["sc_cuenta"][$i];
	  $ls_denominacion= $la_datos["denominacion"][$i];
      $li_i = 1;
	  $la_arr = array();
	  $la_arr[$li_i++] = $ls_sc_cuenta;
      $la_arr[$li_i++] = $ls_denominacion;
      $la_newarr= "new Array(";
	  ...
	  $ls_salida .='<td align="center" width="150"><a href="javascript:ue_seleccionar('.$la_newarr.');">'.$ls_sc_cuenta.'</a></td>';
...
else
{
  ...
  $pagesize = 100; // Registros a traerse de la bd
  ...
  $ls_sc_cuenta = $_GET["sc_cuenta"];
  $ls_denominacion = $_GET["denominacion"];
  
  $lb_hay = $lo_dao->get_sc_cuenta($ls_sc_cuenta, $ls_denominacion, "ORDER BY $sort_col $sort_dir LIMIT $offset,$pagesize",$la_datos); //$_GET["sc_cuenta"],$_GET["denominacion"],
  if ($lb_hay)
  {
    $li_registros = (count($la_datos,COUNT_RECURSIVE)/count($la_datos))-1;  
    ...
    for ($i=1; $i<=$li_registros; $i++)
    {
      $li_i = 1;
	  $la_arr = array();
	  $la_arr[$li_i++] = $ls_sc_cuenta    = $la_datos["sc_cuenta"][$i];
      ...
      $ls_salida .='<tr class="celdas-blancas">';
	  $ls_salida .='<td align="center" width="200"><a href="javascript:ue_seleccionar('.$la_newarr.');">'.$ls_sc_cuenta.'</a></td>';
	  $ls_salida .='<td align="center">'.$la_datos["denominacion"][$i].'</td>';
	  $ls_salida .='</tr>';
    }
