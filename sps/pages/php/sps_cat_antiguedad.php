...
elseif ($ls_operacion=="ue_ver_antiguedad")
{
  $lb_hay = $lo_dao->getPersonal("ORDER BY a.codper,p.nomper,p.apeper ASC",$la_datos,$_GET["codper"],$_GET["nomper"],$_GET["apeper"]);
  if (!$lb_hay)
  {
  ...
  else
  {
	...
	{   
	  ...
	  $ls_salida .='<td align="center" width="140">'.$la_datos["codper"][$i].'</td>';
	  $ls_salida .='<td align="center" width="230">'.$la_datos["nomper"][$i].'</td>';
	  $ls_salida .='<td align="center" width="230">'.$la_datos["apeper"][$i].'</td>';
	  $ls_salida .='</tr>';
    }
    $ls_salida.= '</table>';
  }
...
else
{
  ...
  $lb_hay = $lo_dao->getPersonal("ORDER BY $sort_col $sort_dir LIMIT $pagesize OFFSET $offset",$la_datos,$_GET["codper"],$_GET["nomper"],$_GET["apeper"]);
  if ($lb_hay)
  {
    $li_registros = (count($la_datos,COUNT_RECURSIVE)/count($la_datos))-1;  
