...
/* Función que rellena caracteres a la izquierda
   cadena: cadena original
   longitud: longitud de la cadena a ser rellenada
   caracter: tipo de caracter a rellenar
*/
function uf_padl(cajaTexto,longitud,caracter)
{ 
    var cadena = cajaTexto.value;
	cadena = trim(cadena);
    var ls_new_string="";
	var i;
	var ls_mystring = new String(cadena);
    ls_mystring = trim(ls_mystring);
	var li_longitud_actual = ls_mystring.length;
	var li_longitud_resta = (longitud-li_longitud_actual);
	for(i=1;i<=li_longitud_resta;i++)	
	{		
   	   ls_new_string=ls_new_string+caracter; 
    }
	ls_new_string=ls_new_string+cadena; 
    cajaTexto.value = ls_new_string;
}
