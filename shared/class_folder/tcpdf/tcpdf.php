...
if (!class_exists('TCPDF', false)) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 4.1.001 (http://www.tcpdf.org)');
	...
	*/
	class TCPDF {
		...
		*/
		 ...
		 * @access protected
		 */
		protected $header_logo_height = 30;
		...
		*/
		public function setHeaderData($ln="", $lw=0, $ht="", $hs="", $lh=0) {
			$this->header_logo = $ln;
			$this->header_logo_width = $lw*0.40;
			$this->header_logo_height = $lh*0.40;
		...
		*/
		...
		public function Header() {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
				$this->Image(K_PATH_IMAGES.$headerdata['logo'], $this->GetX(), $this->getHeaderMargin(), $headerdata['logo_width'],$this->header_logo_height);
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		protected function getHtmlDomArray($html) {
			...
			$html = preg_replace('/<li([^\>]*)><img/i', '<li\\1> <img', $html);
					...
					if (isset($startlinex)) {
						...
						if ((isset($plalign) AND ((($plalign == "C") OR ($plalign == "J") OR (($plalign == "R") AND (!$this->rtl)) OR (($plalign == "L") AND ($this->rtl))))) OR ($yshift < 0)) {
							...
							if ($plalign == "J") {
								...
								$ns = 0;
								// search spaces
								if (preg_match_all('/\[\(([^\)]*)\)\]/usi', $pmid, $string, PREG_PATTERN_ORDER)) {
									$maxkk = count($string[1]) - 1;
									foreach ($string[1] as $kk => $value) {
										if ($kk == $maxkk) {
										...
										$no += substr_count($value, ' ');
										$ns += substr_count($tvalue, ' ');
									}
									// calculate additional space to add to each space
									$spacewidth = (($w - $linew + (($no - $ns) * $this->GetStringWidth(" "))) / ($ns?$ns:1)) * $this->k;
									$spacewidthu = ($w - $linew + ($no * $this->GetStringWidth(" "))) / ($ns?$ns:1) / $this->FontSize / $this->k;
									$ns = 0;
									reset($string);
									$offset = 0;
									$strcount = 0;
									$prev_epsposbeg = 0;
									global $spacew;
									while (preg_match('/([0-9\.\+\-]*) (Td|cm|m|l|c|re)[\s]/us', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
										...
										switch ($strpiece[2][0]) {
											...
											case 'm':
											case 'l': {
												$pmid = preg_replace_callback('/([0-9\.\+\-]*) ('.$strpiece[1][0].') ('.$strpiece[2][0].')([\s]*)/us', 
													create_function('$matches', 'global $spacew; 
													$newx = sprintf("%.2f",(floatval($matches[1]) + $spacew));
													return "".$newx." ".$matches[2]." *#!#*".$matches[3].$matches[4];'), $pmid, 1);
												if (($strcount <= $maxkk) AND ($strpiece[2][0] == "Td")) {
													if ($strcount == $maxkk) {
														if ($this->rtl OR $this->tmprtl) {
															$tvalue = ltrim($string[1][$strcount]);
														} else {
															$tvalue = rtrim($string[1][$strcount]);
														}
													} else {
														$tvalue = $string[1][$strcount];
													}
													$ns += substr_count($tvalue, ' ');
													$strcount++;
												}
											...
											case 're': {
												$pmid = preg_replace_callback('/([0-9\.\+\-]*) ([0-9\.\+\-]*) ([0-9\.\+\-]*) ('.$strpiece[1][0].') ('.$strpiece[2][0].')([\s]*)/us', 
													create_function('$matches', 'global $spacew; 
													$newx = sprintf("%.2f",(floatval($matches[1]) + $spacew));
													return "".$newx." ".$matches[2]." ".$matches[3]." ".$matches[4]." *#!#*".$matches[5].$matches[6];'), $pmid, 1);
												break;
											}
											case 'c': {
												$pmid = preg_replace_callback('/([0-9\.\+\-]*) ([0-9\.\+\-]*) ([0-9\.\+\-]*) ([0-9\.\+\-]*) ([0-9\.\+\-]*) ('.$strpiece[1][0].') ('.$strpiece[2][0].')([\s]*)/us', 
													create_function('$matches', 'global $spacew; 
													...
													return "".$newx1." ".$matches[2]." ".$newx2." ".$matches[4]." ".$newx3." ".$matches[6]." *#!#*".$matches[7].$matches[8];'), $pmid, 1);
												break;
											}
										}
									}
									...
									if (($this->CurrentFont['type'] == "TrueTypeUnicode") OR ($this->CurrentFont['type'] == "cidfont0")) {
										...
										$spacew = $spacewidthu;
										$pmid = preg_replace_callback("/\[\(([^\)]*)\)\]/usi", 
													create_function('$matches', 'global $spacew;
													return "[(".str_replace(chr(0)." ", ") ".(-2830 * $spacew)." (", $matches[1]).")]";'), $pmid);
										$this->pages[$startlinepage] = $pstart."\n".$pmid."\n".$pend;
										$endlinepos = strlen($pstart."\n".$pmid."\n");
									} else {
										// non-unicode (single-byte characters)
										$rs = sprintf("%.3f Tw", $spacewidth);
										$pmid = preg_replace("/\[\(([^\)]*)\)\]/usi", $rs." [(\\1)]", $pmid);
										$this->pages[$startlinepage] = $pstart."\n".$pmid."\nBT 0 Tw ET\n".$pend;
										$endlinepos = strlen($pstart."\n".$pmid."\nBT 0 Tw ET\n");
									}
								}
