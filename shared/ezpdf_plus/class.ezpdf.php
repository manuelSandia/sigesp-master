...
//=======================================================================================
// Esta librería ha sido modificada por Edgar A. Quintero U. para añadirle una
// serie de funcionalidades en la generación de tablas y la justificación de texto.
// 1.- Las cabeceras de las tablas se pueden configurar de manera independiente del resto
//     de la tabla, es decir añadir color y justificación para el encabezado y otro para 
//     la tabla.
// 2.- Las lineas de las tablas tienen ahora 5 atributos distintos. Se le pueden 
//     configurar las lineas verticales y horizontales independientemente.
//     Se pueden tener las lineas horizontales sin que aparezcan las verticales,
//     se pueden tener las verticales sin que aparezcan las horizontales, se puede
//     tener solo las verticales, solo las horizontales, añadir un marco a la tabla etc.
// 3.- Se ha arreglado un problema con la justificación del texto, el cual ahora acepta
//     el atributo full, es decir lo que sería justify. Tanto dentro como fuera de las 
//     tablas.
//=======================================================================================
...
function ezPrvtTableDrawLines($pos,$gap,$x0,$x1,$y0,$y1,$y2,$col,$inner,$outer,$opt=1){
  ...
  foreach($pos as $x){
			...
			// si $omitir_vert = 1 Omite las lineas verticales para los campos internos esto permite crearle un marco a la tabla
			if($omitir_vert == 1){
						if($cnt==1 or $cnt==$n){
...
//  $y-=$gap-$decender;
  ...
  while ($ok==0){
    foreach($cols as $colName=>$colHeading){
	  ...
	  if(isset($optionsAll['cabecera_cols'][$colName]['justification'])){
	  
	  		$justification = $optionsAll['cabecera_cols'][$colName]['justification'];
	  }
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  global $omitir_vert,$extraer_datos;
  
  //INICIALIZACIÓN DE LAS VARIABLES PARA LA EXTRACCIÓN DE DATOS DE LA TABLA  (7/03/2009) MODIFICADO POR: Edgar A. Quintero U.
  $this->pos_linea = array(); 
  $this->pos_linea[][][0]['x'] = 0;
  $this->pos_linea[][][0]['y'] = 0;
  $this->contador_lineas = 0;
  
  $this->pos_fila = array();
  $this->pos_fila[0]['x'] = 0;
  $this->pos_fila[0]['y'] = 0;
  $this->contador_filas = 1;
 ...
 $defaults = array(
    'shaded'=>1,'showLines'=>1,'shadeCol'=>array(0.8,0.8,0.8),'shadeCol2'=>array(0.7,0.7,0.7),'extraer_datos'=>'','fontSize'=>10,'titleFontSize'=>12
    ,'titleGap'=>5,'lineCol'=>array(0,0,0),'gap'=>5,'xPos'=>'centre','xOrientation'=>'centre','TituloCol' => array(1,1,1),'Letra_Titulo' => array(0,0,0)
    ,'showHeadings'=>1,'textCol'=>array(0,0,0),'width'=>0,'maxWidth'=>0,'cols'=>array(),'minRowSpace'=>-100,'rowGap'=>2,'colGap'=>5
    ,'innerLineThickness'=>1,'outerLineThickness'=>1,'splitRows'=>0,'protectRows'=>1,'Titulo_Color'=>'no','cabecera_cols'=>array()
    );
  ...
  if ($options['showHeadings']){
   ...
   if($options['Titulo_Color'] == 'si'){
   		
		
		
		 $pos_vert_actual = $y; //GUARDO LA POSICIÓN VERTICAL ACTUAL
		 $this->setColor($options['Letra_Titulo'][0],$options['Letra_Titulo'][1],$options['Letra_Titulo'][2]);
		 //OBTENGO EL TAMAÑO DEL ENCABEZADO
		 $headingHeight = $this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$options['rowGap'],$options['fontSize'],$y,$options);		
	     $this->setColor($options['TituloCol'][0],$options['TituloCol'][1],$options['TituloCol'][2]);
	     //COLOREO EL FONDO DEL ENCABEZADO
		 $this->filledRectangle($x0-$options['gap']/2,$y0-$headingHeight-$decender,$x1-$x0,$headingHeight);		 
	      $this->setColor($options['Letra_Titulo'][0],$options['Letra_Titulo'][1],$options['Letra_Titulo'][2]);
		 //RESTITUYO LA POSICIÓN VERTICAL
		 $y = $pos_vert_actual;
		 //VUELVO A ESCRIBIR EL ENCABEZADO
		 $headingHeight = $this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$options['rowGap'],$options['fontSize'],$y,$options);
		 $this->setColor($options['textCol'][0],$options['textCol'][1],$options['textCol'][2]);
    ...
    while(!$abortTable && ($newPage || $newRow)){
      ...
      if ($newPage || $y<$this->ez['bottomMargin'] || (isset($options['minRowSpace']) && $y<($this->ez['bottomMargin']+$options['minRowSpace'])) ){
        ...
        $y2=$y-$mx+2*$height+$decender-$newRow*$height;
	   	...
	   	if($options['showLines'] == 4 or $options['showLines'] == 5){$omitir_vert = 1;} //Variable que permite crear borde externo a una tabla

	    if ($options['showLines'] == 1 or $options['showLines'] == 2 or $options['showLines'] == 4 or $options['showLines'] == 5){
          if (!$options['showHeadings']){
            $y0=$y1;
          }
...
//        $x1=$x1+$dm;
        ...
        if ($options['showHeadings']){
			  if($options['Titulo_Color'] == 'si'){
					  ...
					  // Ancho $x1-$x0
					  // 
					  // $descender: es la distancia que hay de la parte de abajo de las letras que descienden por debajo de la linea de base 
					  // como las letras g,y o q.
					  // $y0 es el punto vertical menos el descender
					  // $bajo es el retorno de la función ezPrvtTableColumnHeadings el cual indica la distancia que bajo despues de escribir el encbezado
					 ...
					 $this->setColor($options['textCol'][0],$options['textCol'][1],$options['textCol'][2]);
						...
						$bajo = $this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$options['rowGap'],$options['fontSize'],$y,$options);
				} 
          $y1=$y;
        } else {
          $y1=$y0;
	  ...
	  //EXTRAE LA INFORMACIÓN DE POSICIÓN DE LA FILA (7/03/2009) MODIFICADO POR: Edgar A. Quintero U.
	   if($options['extraer_datos']){
				$this->pos_fila[$this->contador_filas]['x'] = $x0;
				$this->pos_fila[$this->contador_filas]['y'] = $this->y-$height;
				$this->contador_filas ++;
		} 
  ...
  //            $this->ezNewPage();
            ...
            if ($colNewPage){
			  ...
			  //EXTRAE LOS DATOS DEL TEXTO DE LA LINEA (7/03/2009) MODIFICADO POR: Edgar A. Quintero U.
			  if($options['extraer_datos']){
			 		 $this->pos_linea[$this->contador_filas-1][$colName][$this->contador_lineas]['texto'] = $line;
             	}
			  
		  
		   //SE PUEDE ESPECIFICAR EL COLOR DE CADA COLUMNA POR SEPARADO
		   //AÑADIDO:  (09/03/2009) MODIFICADO POR: Edgar A. Quintero U.
		   if(isset($options['cols'][$colName]['color'])){
				
				$this->saveState();
				$this->setColor($options['cols'][$colName]['color'][0],$options['cols'][$colName]['color'][1],$options['cols'][$colName]['color'][2]);
			 }
			  
			  
			 //************************* IMPRIME LA LINEA DE TEXTO **************************************************
			 ...
			 //******************************************************************************************************
			  ...
			  if(isset($options['cols'][$colName]['color'])){
			   ...
			   //EXTRAE LOS DATOS DE POSICIÓN DE LA LINEA DE TEXTO (7/03/2009) MODIFICADO POR: Edgar A. Quintero U.			   
			   if($options['extraer_datos']){
					   $this->pos_linea[$this->contador_filas-1][$colName][$this->contador_lineas]['x'] = $pos[$colName];
					   $this->pos_linea[$this->contador_filas-1][$colName][$this->contador_lineas]['y'] = $this->y;
      ...
      if ($options['showLines'] == 2 or $options['showLines'] == 3 or $options['showLines'] == 5){
