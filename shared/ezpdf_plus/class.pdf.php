...
function PRVTcheckTextDirective1(&$text,$i,&$f,$final,&$x,&$y,$size=0,$angle=0,$wordSpaceAdjust=0){
  ...
  if ($text[$j]=='<'){
    $j++;
  if(isset($text[$j])){//SE AGREGO ESTA LINEA PARA QUE NO DE ERROR CUANDO CHEQUEA LAS DIRECTIVAS EN JUSTIFICACIÓN FULL
	   switch($text[$j]){
			  case '/':
				$j++;
				if (strlen($text) <= $j){
					...
					if ($k!==false && $text[$j]==':'){
					  ...
					  if (!isset($func) || !strlen(trim($func))){
						...
						if ($final){
						  ...
						  $info = array('x'=>$tmp[0],'y'=>$tmp[1],'angle'=>$angle,'status'=>'end','p'=>$parm,'nCallback'=>$this->nCallback,'adjust'=>$wordSpaceAdjust);
						  $x=$tmp[0];
						  $y=$tmp[1];
						  $ret = $this->$func($info);
						  if (is_array($ret)){
				...
				if ($k!==false && $text[$j]==':'){
				  ...
				  if (!isset($func) || !strlen(trim($func))){
					...
					if ($final){					 
					  ...
					  $tmp = $this->PRVTgetTextPosition($x,$y,$angle,$size,$wordSpaceAdjust,substr($text,0,$i));
					  $info = array('x'=>$tmp[0],'y'=>$tmp[1],'angle'=>$angle,'status'=>'start','p'=>$parm,'f'=>$func,'height'=>$this->getFontHeight($size),'decender'=>$this->getFontDecender($size),'adjust'=>$wordSpaceAdjust);					  
					  $x=$tmp[0];
					  $y=$tmp[1];
...
function covensol_color($info){
	...
	switch($info['status']){
		...
		case 'sol':
		  ...
		  if (!isset($this->ez['font_color'])){
			$this->ez['font_color']=array();
		  }
		  $i = $info['nCallback'];		  
		  $color = $this->decodifica_color($info['p']);
		  
		  $this->ez['font_color'][$i] = array('x'=>$info['x'],
		  									  'y'=>$info['y'],
											  'angle'=>$info['angle'],
		  ...
		  //echo  $this->ez['font_color'][$i]['x'].' '.$this->ez['font_color'][$i]['y'].'<br>';
		  $this->saveState();
		  break;
		case 'end':
		case 'eol':
...
function addText($x,$y,$size,$text,$angle=0,$wordSpaceAdjust=0){
  ...
  for ($i=0;$i<$len;$i++){
    ...
    if ($directive){
      ...
      if ($i>$start){
		...
		//echo  'add Text: '.$part.' '.$y.' '.$this->callback[$this->nCallback]['status'].'--> '.$this->ez['font_color'][$this->nCallback]['B'].'<br>';		
        
	     if ($this->nCallback>0){
		 
		 			if($this->callback[$this->nCallback]['f'] == 'covensol_color' and $this->callback[$this->nCallback]['status'] == 'start'){
					
								$this->setColor($this->ez['font_color'][$this->nCallback]['R'],$this->ez['font_color'][$this->nCallback]['G'],$this->ez['font_color'][$this->nCallback]['B']);
					
					}
		 
		 }
		 //echo  'add Text: '.$part.'--> '.$x.'<br>';		
  ...
  if ($this->nCallback>0){
     for ($i=$this->nCallback;$i>0;$i--){      // call each function
...
function getCaracterWidth($text){
  // this function should not change any of the settings, though it will need to
  // track any directives which change during calculation, so copy them at the start
  // and put them back at the end.
  $store_currentTextState = $this->currentTextState;
...
function PRVTadjustWrapText($text,$actual,$width,&$x,&$adjust,$justification){
   ...
   switch ($justification){
    ...
    case 'full':
      ...
      $text = $this->reemplazar_caracter($text);
	  
	  //$reemplazo = array(" "," ");
	  //$cadena = array("<c:uline>","</c:uline>");	 
	  //$text=str_replace($cadena,$reemplazo,$text); 
	  ...
	  $j=0;
	  $l=0;
	  $ns=0; //número de separador
	  $nl=1;//número de linea
	  $ls=0;
	  $textx = $text;
	  $w = 0;
	 ...
	 $cf = $this->currentFont;
	  for ($i=0;$i<$nb;$i++){ 		
								...
								$c=$text[$i]; //$s es la variable que almacena el texto
								...
								if ($directive){
									  ...
									  //SI ENCUENTRA UNA DIRECTIVA SUMA EL ANCHO EN NUMERO DE CARACTERES PARA OMITIRLA EN EL CONTEO
								  	  $i=$i+$directive-1;
								} 
								else {									  
									  $char=ord($text[$i]);
									  if (isset($this->fonts[$cf]['differences'][$char])){
											...
											$name = $this->fonts[$cf]['differences'][$char];
											if (isset($this->fonts[$cf]['C'][$name]['WX'])){
											  $l+=$this->fonts[$cf]['C'][$name]['WX'];
											}
									  } 
									  else if (isset($this->fonts[$cf]['C'][$char]['WX'])){
											$l+=$this->fonts[$cf]['C'][$char]['WX'];
									  }
								}
      ...
      if ($nspaces>0){
       ...
       $adjust = ($this->wmax-$l)/1000*$this->tamaño_letra/$nspaces;
	   //echo $text.'-->'.$l.'<br>';	  
      } else {
        $adjust=0;
      }
...
function reemplazar_caracter($texto){

	 //LA FUNCIÓN REEMPLAZAR CARACTER REEMPLAZA LAS LETRAS CON RETORNO COMO LOS ACENTOS PARA OBTENER UN ANCHO REAL DE LA LINEA
	 // (09/03/2009) MODIFICADO POR: Edgar A. Quintero U.

	  $texto=str_replace("\r",'',$texto);
	  $vocales = array("a", "e", "i", "o", "u", "n", "A", "E", "I", "O", "U", "N","","");
	  $vocales_acento = array("á", "é", "í", "ó", "ú", "ñ", "Á", "É", "Í", "Ó", "Ú", "Ñ");
	  $otras_letras = array("à", "è", "ì", "ò", "ù", "ñ", "À", "È", "Ì", "Ò", "Ù", "Ñ");
	  $texto=str_replace($vocales_acento,$vocales,$texto); //Sustituye las vocales con acento y la ñ
	  $texto=str_replace($otras_letras,$vocales,$texto);
...
function addTextWrap($x,$y,$width,$size,$text,$justification='left',$angle=0,$test=0){
  ...
  for ($i=0;$i<$len;$i++){
    ...
    if ($directive){
      ...
      $cOrd = ord($this->reemplazar_caracter($text[$i]));
      if (isset($this->fonts[$cf]['differences'][$cOrd])){
        // then this character is being replaced by another
        $cOrd2 = $this->fonts[$cf]['differences'][$this->reemplazar_caracter($cOrd)];
      } else {
        $cOrd2 = $cOrd;
      }
  	  //LA FUNCIÓN REEMPLAZAR CARACTER REEMPLAZA LAS LETRAS CON RETORNO COMO LOS ACENTOS OBTENER UN ANCHO REAL DE LA LINEA
	  // (09/03/2009) MODIFICADO POR: Edgar A. Quintero U.
      if (isset($this->fonts[$cf]['C'][$cOrd2]['WX'])){
        $w+=$this->fonts[$cf]['C'][$this->reemplazar_caracter($cOrd2)]['WX'];
      }
      if ($w>$tw){
        ...
        if ($break>0){
          ...
          $tmp = substr($text,0,$i);
          $adjust=0;
          $ctmp=ord($this->reemplazar_caracter($text[$i]));
          if (isset($this->fonts[$cf]['differences'][$ctmp])){
            $ctmp=$this->fonts[$cf]['differences'][$ctmp];
          }
          $tmpw=($w-$this->fonts[$cf]['C'][$this->reemplazar_caracter($ctmp)]['WX'])*$size/1000;
