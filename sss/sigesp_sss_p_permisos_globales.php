...
<?php
	...
	if($ls_operacion=="GUARDAR")
	{
		...
		if($lb_existe)
		{
			if(($ls_sistemas=="SNO")||($ls_sistemas=="SPG")||($ls_sistemas=="SEP")||($ls_sistemas=="SOC")||($ls_sistemas=="CXP"))
			{
				$lb_valido=$io_sss->uf_select_permisos_internos($ls_codemp,$ls_sistemas,$ls_usuario,$la_codintper);
				if($lb_valido)
				{
					
					$lb_valido=$io_sss->uf_sss_insert_derecho_usuario_defecto($ls_codemp,$ls_usuario,$ls_sistemas,$li_visible,$li_enabled,$li_leer,
																			  $li_incluir,$li_cambiar,$li_eliminar,$li_imprimir,$li_administrador,$li_anular,
																			  $li_ejecutar,$la_seguridad);
					if($lb_valido)
					{
						$lb_valido=$io_sss->uf_sss_update_derecho_usuario_defecto($ls_codemp,$ls_usuario,$ls_sistemas,$li_enabled,$li_leer,
																			  	  $li_incluir,$li_cambiar,$li_eliminar,$li_imprimir,$li_anular,
																			  	  $li_ejecutar,$la_seguridad);
					}
					if($lb_valido)
					{
						$lb_valido=$io_sss->uf_sss_delete_derecho_usuario2($ls_codemp,$ls_usuario,$ls_sistemas,$la_seguridad);
					}
					if($lb_valido)
					{
						$lb_valido=$io_sss->uf_sss_insert_derecho_usuario2($ls_codemp,$ls_usuario,$ls_sistemas,$la_seguridad);
					}
					if($lb_valido)
					{
						$lb_valido=$io_sss->uf_sss_update_derecho_usuario_defecto2($ls_codemp,$ls_usuario,$ls_sistemas,$li_enabled,$li_leer,
																			  	  $li_incluir,$li_cambiar,$li_eliminar,$li_imprimir,$li_anular,
																			  	  $li_ejecutar,$la_seguridad);
					}
				...
				else
				{
					...
					elseif($ls_sistemas=='SPG')
					{$io_msg->message("El usuario no tiene ninguna Estructura Presupuestaria asociada");}
					else
					{
						$io_msg->message("Por favor verifique que esten asociadas las unidades ejecutoras en el sistema indicado");	
					}
					$lb_valido=false;
				}
...
?>		  
  ...
  <div align="center">
    <table width="47%" height="32" border="0" cellpadding="0" cellspacing="0"  class="formato-blanco">
      <tr >
        <td height="17"  class="titulo-ventana"><span class="titulo-celdanew">Permisos por Sistemas</span></td>
      </tr>
      <tr>
        <td height="13"><table width="458" height="197" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
          ...
          <tr>
            <td height="19"><div align="right">Usuario</div></td>
            <td height="24" colspan="3" align="left"><?php  $io_sss->uf_pintar_combo_usuarios($la_usuarios,$ls_usuario);?>
              <div align="center"></div>
              <div align="center"></div></td>
          </tr>
          <tr>
            <td height="21" colspan="4" class="titulo-celdanew">Permisos</td>
            </tr>
          <tr>
