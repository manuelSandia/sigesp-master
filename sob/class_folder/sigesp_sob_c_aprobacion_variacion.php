<?php
class sigesp_sob_c_aprobacion_variacion
 {
	var $io_sql;
	var $io_mensajes;
	var $io_funciones;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function sigesp_sob_c_aprobacion_variacion($as_path)
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: sigesp_sob_c_aprobacion_variacion
		//		   Access: public 
	...
	}// end function sigesp_sob_c_aprobacion_variacion
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_variaciones($as_codvar,$ad_fecasides,$ad_fecasihas,$as_tipooperacion)
	{
		//////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_load_variaciones
		//		   Access: public
		...
		//////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT codcon, codvar, tipvar, monto, estapr".
				"  FROM sob_variacioncontrato ".
				" WHERE sob_variacioncontrato.codemp = '".$this->ls_codemp."'".
				"   AND sob_variacioncontrato.codvar LIKE '%".$as_codvar."%' ".
				"   AND sob_variacioncontrato.fecvar >= '".$ad_fecasides."' ".
				"   AND sob_variacioncontrato.fecvar <= '".$ad_fecasihas."' ".
				"   AND sob_variacioncontrato.estvar=1".
				"   AND sob_variacioncontrato.estapr='".$as_tipooperacion."'".
				" ORDER BY sob_variacioncontrato.codvar, sob_variacioncontrato.codcon ";
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_validar_estatus_variacion($as_codvar,$as_codcon,$as_estapr)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_validar_estatus_variacion
		//		   Access: private
		//		 Argument: as_codasi        // Numero de la Asignación
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Aprobacion MÉTODO->uf_validar_estatus_variacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
		...
		return $lb_existe;
	}// end function uf_validar_estatus_variacion
	//-----------------------------------------------------------------------------------------------------------------------------------	

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_validar_cuentas($as_codasi,$as_codcon)
	{
		//////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT codestpro1, codestpro2, codestpro3, codestpro4, codestpro5, spg_cuenta, monto, ".
				...
				"		   FROM spg_cuentas ".
				"		  WHERE spg_cuentas.codemp = sob_cuentavariacion.codemp ".
				"			AND spg_cuentas.codestpro1 = sob_cuentavariacion.codestpro1 ".
				"		    AND spg_cuentas.codestpro2 = sob_cuentavariacion.codestpro2 ".
				"		    AND spg_cuentas.codestpro3 = sob_cuentavariacion.codestpro3 ".
				"		    AND spg_cuentas.codestpro4 = sob_cuentavariacion.codestpro4 ".
				"		    AND spg_cuentas.codestpro5 = sob_cuentavariacion.codestpro5 ".
				"		    AND spg_cuentas.estcla = sob_cuentavariacion.estcla ".
				"			AND spg_cuentas.spg_cuenta = sob_cuentavariacion.spg_cuenta) AS disponibilidad, ".		
				"		(SELECT COUNT(codemp) ".
				"		   FROM spg_cuentas ".
				"		  WHERE spg_cuentas.codemp = sob_cuentavariacion.codemp ".
				"			AND spg_cuentas.codestpro1 = sob_cuentavariacion.codestpro1 ".
				...
				"		    AND spg_cuentas.codestpro5 = sob_cuentavariacion.codestpro5 ".
				"		    AND spg_cuentas.estcla = sob_cuentavariacion.estcla ".
				"			AND spg_cuentas.spg_cuenta = sob_cuentavariacion.spg_cuenta) AS existe ".		
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_update_estatus($as_codvar,$as_codcon,$as_estapr,$ad_fecapr,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
