<?php
	...
	function uf_print_cabecera($as_codcon,$as_fecinicon,$as_fecfincon,$as_fecact,$as_cedinsact,$as_civinsact,$as_inspector,$as_cedresact,$as_nomresact,$as_civresact,
							   $as_contratista,$as_representante,$as_cedrepresentante,$as_desobr,$as_ejecucion,$as_diaact,$as_mesact,$as_anioact,$as_feccon,&$io_pdf)

	{
		...
		$io_pdf->EzSetDy(-15);
		$la_data[1]=array('1'=>'     En el día de hoy, a los '.$as_diaact.' días del mes de '.$as_mesact.' de '.$as_anioact.'; reunidos, en el sitio donde se realizaran los trabajos de ejecución de la obra; por una parte el ciudadano, '.$as_inspector.' titular de la C.I. '.$as_cedinsact.', CIV '.$as_civinsact.', en su carácter de Ingeniero Inspector de la obra, en representación de '.$ls_nomemp.' y por la otra '.$as_representante.',titular de la C.I. '.$as_cedrepresentante.' y el ciudadano '.$as_nomresact.', Ingeniero Residente, titular de la C.I. '.$as_cedresact.', CIV '.$as_civresact.', en representación de la empresa contratista,   a los fines dar inicio a la obra de acuerdo a lo previsto  en el Artículo 103 Ley de Contrataciones Públicas,  publicado en Gaceta Oficial Nº 39165 de fecha 24 de Abril de 2009,   El lapso de ejecución de la obra es de '.$as_ejecucion.' según especificaciones del contrato.');
		$la_data[2]=array('1'=>' ');
		$la_data[3]=array('1'=>'     En prueba de conformidad, se firman (4) ejemplares de la presente Acta de Inicio.');
		$la_columnas=array('1'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
	...
	//----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
	 $ls_titulo='<b>ACTA DE INICIO</b>';
	//--------------------------------------------------------------------------------------------------------------------------------
	$lb_valido=uf_insert_seguridad($ls_titulo); // Seguridad de Reporte
	if($lb_valido)
	{

		$lb_valido=$io_report->uf_select_actacontrato($ls_codact,$ls_codcon,"1"); // Cargar el DS con los datos del reporte
...
//			$io_pdf->ezStartPageNumbers(570,47,8,'','',1); // Insertar el número de página
			...
			{			
				...
				$ls_ejecucion=$io_fecha->uf_restar_fechas($ls_fecinicon,$ls_fecfincon);
				
				if($ls_ejecucion>0)
				{
					if($ls_ejecucion==1)
					{
						$ls_ejecucion=$ls_ejecucion." dia ";
					}
					else
					{
						$ls_ejecucion=$ls_ejecucion." dias ";
					}
				}
				...
				uf_print_cabecera($ls_codcon,$ls_fecinicon,$ls_fecfincon,$ls_fecact,$ls_cedinsact,$ls_civinsact,$ls_inspector,$ls_cedresact,$ls_nomresact,$ls_civresact,
								  $ls_contratista,$ls_representante,$ls_cedrepresentante,$ls_desobr,$ls_ejecucion,$ls_diaact,$ls_mesact,$ls_anioact,$ls_feccon,&$io_pdf);
			}
		}
