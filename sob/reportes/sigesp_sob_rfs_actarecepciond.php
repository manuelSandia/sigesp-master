<?php
	...
	function uf_print_encabezado_pagina($as_titulo,&$io_pdf)
	{
		...
		$io_pdf->line(305,60,305,200);
		$io_pdf->line(160,60,160,180);
		$io_pdf->line(450,60,450,180);
		$io_pdf->addText(115,185,9,"Por la INSTITUCION"); // Agregar el título
		$io_pdf->addText(385,185,9,"Por la EMPRESA CONTRATISTA"); // Agregar el título
		$io_pdf->addText(30,165,9,"Coordinacion de Inspeccion"); // Agregar el título
		$io_pdf->addText(190,165,9,"Ingeniero Inspector"); // Agregar el título
		$io_pdf->addText(335,165,9,"Representante Legal"); // Agregar el título
		$io_pdf->addText(490,165,9,"Ing. Residente"); // Agregar el título
		$io_pdf->addText(50,105,8,"Nombre y Apellido"); // Agregar el título
		$io_pdf->addText(205,105,8,"Nombre y Apellido"); // Agregar el título
		$io_pdf->addText(345,105,8,"Nombre y Apellido"); // Agregar el título
		$io_pdf->addText(485,105,8,"Nombre y Apellido"); // Agregar el título
		$io_pdf->addText(30,85,8,"C.I."); // Agregar el título
		$io_pdf->addText(175,85,8,"C.I."); // Agregar el título
		$io_pdf->addText(320,85,8,"C.I."); // Agregar el título
		$io_pdf->addText(460,85,8,"C.I."); // Agregar el título
		$io_pdf->addText(30,65,8,"CIV"); // Agregar el título
		$io_pdf->addText(175,65,8,"CIV"); // Agregar el título
		$io_pdf->addText(460,65,8,"CIV"); // Agregar el título

		//LINEAS DE FIRMAS
		$io_pdf->line(25,115,150,115);
		$io_pdf->line(45,85,155,85);
		$io_pdf->line(50,65,155,65);

		$io_pdf->line(175,115,300,115);
		$io_pdf->line(190,85,295,85);
		$io_pdf->line(195,65,295,65);

		$io_pdf->line(320,115,440,115);
		$io_pdf->line(335,85,440,85);
		

		$io_pdf->line(460,115,580,115);
		$io_pdf->line(475,85,580,85);
		$io_pdf->line(480,65,580,65);
...

	{
		...
		$io_pdf->EzSetDy(-15);
		$la_data[1]=array('1'=>'     En el día de hoy, a los '.$as_diaact.' días del mes de '.$as_mesact.' de '.$as_anioact.'; concluido el lapso de garantía; reunidos '. 
		...
		'Ingeniero Residente, titular de la C.I. '.$as_cedresact.', CIV '.$as_civresact.', en representación de la empresa contratista '.$as_contratista.',   a '.
		'los fines previstos en el Artículo 125 de la Ley de Contrataciones Públicas publicado en Gaceta Oficial Nº 39.165 de fecha 24 de abril de 2009, se ha procedido '.
		'a la inspección de los trabajos ejecutados, los cuales fueron concluidos el día, '.$as_diafinact.' del mes de '.$as_mesfinact.' de '.$as_aniofinact.' determinándose '.
		'que su ejecución se adapta a las condiciones estipuladas en el Contrato No. '.$as_codcon.' de fecha '.$as_feccon.' suscrito entre ambas partes,  por lo cual  se ha acordado proceder a la  Recepción Definitiva.');
		$la_data[2]=array('1'=>' ');
		$la_data[3]=array('1'=>'     La presente acta no exime al contratista de cualquier obligación que pudiera derivarse del Contrato o Ley, para que conste se firma la presente Recepción Definitiva. ');
		$la_data[4]=array('1'=>' ');
		$la_data[5]=array('1'=>'     En prueba de conformidad se firman (N) ejemplares de  la presente Acta.');
		$la_columnas=array('1'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
	...
	//----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
	 $ls_titulo='<b>ACTA DE RECEPCIÓN DEFINITIVA</b>';
	//--------------------------------------------------------------------------------------------------------------------------------
	$lb_valido=uf_insert_seguridad($ls_titulo); // Seguridad de Reporte
	if($lb_valido)
	{

		$lb_valido=$io_report->uf_select_actacontrato($ls_codact,$ls_codcon,"4"); // Cargar el DS con los datos del reporte
