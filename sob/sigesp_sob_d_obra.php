<?php
	...
	function uf_limpiar_variables()
	{
		...
		global $ls_hidstatus,$ls_codtipest,$li_baseimp,$li_monimp;
...
if	(array_key_exists("txtbaseimp",$_POST)){		$li_baseimp=$_POST["txtbaseimp"]; }
else{$li_baseimp="0,00";}

if	(array_key_exists("txtbasimpasi",$_POST)){	$li_monimp=$_POST["txtbasimpasi"]; }
else{$li_monimp="0,00";}
...
if($ls_operacion=="ue_nuevo")//Abre una ficha de obra nueva
{
	...
	$ls_codobr= $io_keygen->uf_generar_numero_nuevo("SOB","sob_obra","codobr","SOBASI",15,"","","");
	unset($io_keygen);
/*	$ls_nomobr="";	    
	$ls_nompro="";	   
	$ls_resobr="";	
...
}elseif($ls_operacion=="ue_guardar")
{
	...
	if($ls_hidstatus!="C")
	{
		...
		$lb_valido=$io_obra->uf_guardar_obra(&$ls_codobr,$ls_codten,$ls_codtipest,$ls_codest,$ls_codmun,$ls_codpar,$ls_codcom,
											 $ls_codsiscon,$ls_codpro,$ls_codtob,$ls_nomobr,$ls_dirobr,$ls_obsobr,$ls_resobr,
											 $ld_dateinicio,$ld_datefin,$li_monto,$ld_datecreacion,$li_baseimp,$li_monimp,
											 $la_seguridad,$lb_existe);
		if ($lb_valido)
		{
			$lb_partida=$io_obra->uf_delete_dtpartidas($ls_codobr,$la_seguridad);
			...
			$lb_partida=$io_obra->uf_delete_dtfuentesfinanciamiento($ls_codobr,$la_seguridad);
	...
	else
	{
		...
		{			
			$ls_hidstatus=$_POST["hidstatus"];
			$io_sql->begin_transaction();
			$lb_valido=$io_obra->uf_update_obra($ls_codobr,$ls_codten,$ls_codtipest,$ls_codest,$ls_codmun,$ls_codpar,$ls_codcom,
												$ls_codsiscon,$ls_codpro,$ls_codtob,$ls_nomobr,$ls_dirobr,$ls_obsobr,$ls_resobr,
												$ld_dateinicio,$ld_datefin,$li_monto,$ld_datecreacion,$li_baseimp,$li_monimp,
												$la_seguridad);
			if($lb_valido)
...
}elseif ($ls_operacion=="ue_eliminar")
{
	...
	if($lb_valido)
	{
		if($li_estado==1)
		{
				...
				{
					$lb_valido=$io_obra->uf_delete_dtpartidas($ls_codobr,$la_seguridad);
					if($lb_valido)
					{
						$lb_valido=$io_obra->uf_delete_dtfuentesfinanciamiento($ls_codobr,$la_seguridad);
						if($lb_valido)
						{
							$lb_valido=$io_obra->uf_delete_obra($ls_codobr,$la_seguridad);
							if($lb_valido)
							{
								$io_msg->message("La Obra ha sido eliminada");
							}
						}
					}
					if(!$lb_valido)
					{
						$io_msg->message("Ha ocurrido un error al eliminar la obra");
					}
				}
				else
				{
		...
		else
		{
			...
			{
				$ls_estado=$io_funsob->uf_convertir_numeroestado($li_estado);
				$io_msg->message("La Obra no puede ser eliminada, su estado es ".$ls_estado);
			}
		}
	}
...
?>	
  ...
  <table width="743" border="0" align="center" cellpadding="0" cellspacing="0"  class="contorno">
      ...
      <tr class="formato-blanco">
        ...
        <td width="47"><input name="txtcodobr" type="text" id="txtcodobr" style="text-align:center " value="<?php print $ls_codobr ?>" size="20" maxlength="15" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnopqrstuvwxyz-')"  onBlur="javascript: ue_rellenarcampo(this,15);"></td>
      ...
      <tr class="formato-blanco">
        ...
        <td colspan="5"><textarea name="txtnomobr" cols="80"  onKeyDown="textCounter(this,254)"  onKeyUp="textCounter(this,254)"  rows="2" id="txtnomobr"  onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnopqrstuvwxyz-()=%#¿?!¡áéíóú ')"><?php print $ls_nomobr ?></textarea></td>
      </tr>
      <tr class="formato-blanco">
        <td colspan="7">&nbsp;</td>
      </tr>
      <tr class="formato-blanco">
        <td height="392" colspan="7"><table width="636" border="0" align="center" cellpadding="0" cellspacing="0"  class="sin-borde">
          ...
          <tr class="formato-blanco">
            ...
            <td colspan="6"><input name="txtresobr" type="text" id="txtresobr"   style="text-align:left;" value="<?php print $ls_resobr ?>" size="57" maxlength="50"  onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnopqrstuvwxyz-()=%#¿?!¡áéíóú ')"></td>
          </tr>
          <tr class="formato-blanco">
...
&nbsp;&nbsp;&nbsp; Fecha de Fin
      ...
      <input name="hidestado" type="hidden" id="hidestado" value="<?php print $ls_codest ?>"></td>
            ...
            <td width="96"  height="22"><?php
					if($ls_codest=="")
						...
						$lb_valido=$io_obra->uf_llenarcombo_municipio($ls_codpai,$ls_codest,&$la_municipio);
            ...
            <td align="right"><div align="left">
			    ...
			    if($ls_codmun=="")
					...
					$lb_valido=$io_obra->uf_llenarcombo_parroquia($ls_codpai,$ls_codest,$ls_codmun,$la_parroquia);
            ...
            <td><?php
			    if($ls_codpar=="")
					...
					$lb_valido=$io_obra->uf_llenarcombo_comunidad($ls_codpai,$ls_codest,$ls_codmun,$ls_codpar,$la_municipio);
          ...
          <tr class="formato-blanco">
            ...
            <td height="22" colspan="6" align="left"><div align="left">
                <input name="txtdirobr" type="text" id="txtdirobr" style="text-align:left" value="<?php print $ls_dirobr ?>" size="60" maxlength="200"   onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnopqrstuvwxyz-()=%#¿?!¡áéíóú ')">
            </div></td>
          </tr>
          <tr class="formato-blanco">
            <td height="16">&nbsp;</td>
          ...
          <tr class="formato-blanco">
            ...
            <td colspan="6"><textarea name="txtobsobr" cols="60" rows="2" id="textarea" onKeyDown="textCounter(this,254)"  onKeyUp="textCounter(this,254)" style="text-align:left"   onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnopqrstuvwxyz-()=%#¿?!¡áéíóú ')"><?php print $ls_obsobr ?></textarea></td>
          </tr>
          <tr class="formato-blanco">
            <td colspan="2">&nbsp;</td>
            <td colspan="6">&nbsp;</td>
            ...
            <tr class="formato-blanco">
              ...
              <td width="585"><div align="left">
			  	...
			  	<a href="javascript:ue_catpartida();">
			  		<img src="../shared/imagebank/mas.gif" width="9" height="17" border="0">				</a>
      ...
      <tr class="formato-blanco">
        ...
        <td colspan="2"><div align="left">
          <input name="txtbaseimp" type="text" id="txtbaseimp"  style="text-align: right" value="<?php print $li_baseimp; ?>" size="20" maxlength="20" readonly="true">
        </div></td>
      </tr>
      <tr class="formato-blanco">
        <td>&nbsp;</td>
        ...
        <td colspan="2"><div align="left">
          <input name="txtbasimpasi" type="text" id="txtbasimpasi"  style="text-align: right" value="<?php print $li_monimp; ?>" size="20" maxlength="20" onKeyPress="return(currencyFormat(this,'.',',',event))" onBlur="uf_validarbasimp();">
        </div></td>
      </tr>
      <tr class="formato-blanco">
        <td>&nbsp;</td>
        ...
        <td colspan="2"><input name="txtmonto" type="text" id="txtmonto"  style="text-align:right" value="<?php print $li_monto ?>"  readonly="true" size="20" maxlength="20"  ></td>
      </tr>
      <tr class="formato-blanco">
        <td colspan="7">&nbsp;</td>
      </tr>
        ...
        <td colspan="7"><div align="center">
          <table width="609" border="0" align="center" cellpadding="0" cellspacing="0"  class="sin-borde">
            ...
            <tr align="center" class="formato-blanco">
			  ...
			  <?php $io_grid->makegrid($li_filasfuentes,$la_columfuentes,$la_objectfuentes,$li_anchofuentes,$ls_titulofuentes,$ls_nametable);?>			  </td>
      ...
      <tr class="formato-blanco">
        <td colspan="7">            <input name="reporte" type="hidden" id="reporte" value="<?php print $ls_reporte;?>"></td>
      </tr>
      <tr class="formato-blanco">
...
{
	document.formulario.operacion.value="pais";
...
{		
	...
	if (f.txtcodobr.value!="")
	{
		...
		f.txtmonto.value=f.txtbaseimp.value;
...
{
	var opener="obra";
	var codigo=document.formulario.txtcodobr.value;
			  		   ...
			  		   ls_fechainicio,ls_fechafin,ls_nomten,ls_nomtipest,ls_nomsiscon,ls_nomtob,ls_estado,ls_codpais,monimp,basimp)
{
	...
	f.txtbasimpasi.value=uf_convertir(monimp);
	f.txtbaseimp.value=uf_convertir(basimp);
...
function valida_monto_total(txt,filaeliminar)
{
	...
	f.txtbaseimp.value=uf_convertir(montototal);
														...
														else
														{
															...
															{															
																	if(validamontolleno("fuente")==true)
																	{
																		...
																		{																		
																			if(ue_comparar_intervalos('txtfeciniobr','txtfecfinobr','La fecha de inicio de la Obra debe ser menor que la fecha de fin'))
																			{
																				if(parseFloat(li_monto)==parseFloat(li_montofuente))
																				{
																					if(parseFloat(li_monto)>=parseFloat(li_baseimp))
																					{
																						f.action="sigesp_sob_d_obra.php";
																					...
																					{
																						alert("Debe actualizar el monto de los otros creditos");
																					}
																				}
																				else
																				{
...
function ue_otroscreditos()
{
	f = document.formulario;
	codobr=f.txtcodobr.value;
	subtotal=ue_formato_calculo(f.txtbaseimp.value);
	totrowspg=ue_calcular_total_fila_local("txtcodcue");
	codasi="";
	if((codobr=="")||(parseFloat(subtotal)<=0))
	{
		alert("Debe Seleccionar una obra y el Monto del Subtotal Debe ser Mayor que Cero");
	}
	else
	{	
