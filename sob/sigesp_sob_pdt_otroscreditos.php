...
function ue_search()
{
	...
	if (codasi=='')
	   {
 	     codobr=document.formulario.codobr.value;
//	     subtotal=ue_formato_calculo(document.formulario.subtotal.value);
	     subtotal=document.formulario.subtotal.value;
	   }
	else 
	   {
...
function ue_aceptar()
{
	...
	if(codasi!="")
	{
		...
		for(i=1;i<=totrow;i++)
		{
			marcado=eval("f.chkcargos"+i+".checked");
			if(marcado)
			{
				check=true;
			}
	...
	else
	{
		...
		for(i=1;i<=totrow;i++)
		{
			monimp=eval("f.txtmonimp"+i+".value");
	        monimp=ue_formato_calculo(monimp);
			totimp=eval(totimp+"+"+monimp);
		}
	    subtotal=document.formulario.subtotal.value;
		total=eval(totimp+"+"+subtotal);
		opener.document.formulario.txtbasimpasi.value=uf_convertir(totimp);
		opener.document.formulario.txtmonto.value=uf_convertir(total);
	}
