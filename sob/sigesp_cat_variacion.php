...
<html>
<head>
<title>Cat&aacute;logo de Variaciones</title>
  ...
  <p align="center">
    <input name="operacion" type="hidden" id="operacion">
  	 ...
  	 <table width="650" border="0" align="center" cellpadding="1" cellspacing="1">
    	<tr>
     	 	<td width="600" colspan="2" class="titulo-celda">Cat&aacute;logo de Variaciones</td>
    	</tr>
	 </table>
	 <br>
	 <table width="646" border="0" cellpadding="0" cellspacing="0" class="formato-blanco" align="center">
      ...
      <tr>
        ...
        <td width="118" height="29">Estado de la Variacion: </td>
...
{ 
	$ls_sql="SELECT v.*,c.monreacon,a.codasi"; 
   
	if($ls_codobr!="%%")   
     {
      $ls_sql=$ls_sql." FROM sob_obra o,sob_asignacion a,sob_contrato c,sob_variacioncontrato v
	  ...
	  if($ls_codasi!="%%")   
       {
         if($ls_codcont!="%%")
		 {
		   $ls_sql=$ls_sql." FROM rpc_proveedor p,sob_asignacion a,sob_contrato c,sob_variacioncontrato v
		 ...
		 {
		   $ls_sql=$ls_sql." FROM sob_asignacion a,sob_contrato c,sob_variacioncontrato v
	   ...
	   else
	   {
	    if($ls_codcon!="%%")   
        {
		 ...
		 {
		   $ls_sql=$ls_sql." FROM sob_contrato c,sob_variacioncontrato v,sob_asignacion a
	   ...
	   else
	   {
		...
		{
		  $ls_sql=$ls_sql." FROM sob_variacioncontrato v,sob_asignacion a,sob_contrato c
	...
	if($ls_hidestado=="")
	{
       if($ls_estado!=0)
	   {
		$ls_sql=$ls_sql." AND v.estvar='".$ls_estado."'";
	   }
	}
	else
	{
		$ls_sql=$ls_sql." AND v.estvar<>3"; 
	}
	if ($ld_fecval!="")
	{
		$ls_sql=$ls_sql." AND v.fecvar='".$ld_fecval."'";
	}
	...
	if($rs_data===false)
	{
		$is_msg_error="Error en select catalogo variacion".$io_funcion->uf_convertirmsg($io_sql->message);
		print $is_msg_error;
		print $ls_sql;
	}
	else
	{
		if($row=$io_sql->fetch_row($rs_data))
		{
			...
			$li_totrow=$io_datastore->getRowCount("codvar");
			print "<table width=650 border=0 cellpadding=1 cellspacing=1 class=fondo-tabla align=center>";
			print "<tr class=titulo-celda>";
			print "<td>Cod. Contrato</td>";
			print "<td>Cod. Variacion</td>";
			print "<td>Fecha Variacion.</td>";
		   	print "<td>Estado</td>";
			print "</tr>";
			for($li_z=1;$li_z<=$li_totrow;$li_z++)
			{
				print "<tr class=celdas-blancas align=center>";				
                $ls_codvar=$data["codvar"][$li_z];
				$ls_codcon=$data["codcon"][$li_z];
				$ls_tipvar=$data["tipvar"][$li_z];
				$ls_motvar=$data["motvar"][$li_z];	
				$ls_fecha=$io_funcion->uf_convertirfecmostrar($data["fecvar"][$li_z]);
				$ls_monto=$data["monto"][$li_z];
				$ls_monco=$data["monreacon"][$li_z];
				$ls_codasi=$data["codasi"][$li_z];
				$ls_estapr=$data["estapr"][$li_z];
				$ls_nomestval=$io_funobr->uf_convertir_numeroestado($data["estvar"][$li_z]);
				
				print "<td><a href=\"javascript: aceptar('$ls_codvar','$ls_codcon','$ls_tipvar','$ls_motvar','$ls_fecha','$ls_monto','$ls_monco','$ls_nomestval','$ls_codasi','$ls_estapr');\">".$ls_codcon."</a></td>";
				print "<td>".$ls_codvar."</td>";
		  ...
		  {
		    $io_msg->message("No se han Registrado Variaciones");
		  }
...
function aceptar(ls_codvar,ls_codcon,ls_tipvar,ls_motvar,ls_fecha,ls_monto,ls_monco,ls_nomestval,ls_codasi,ls_estapr)
  {
    opener.ue_cargarvariacion(ls_codvar,ls_codcon,ls_tipvar,ls_motvar,ls_fecha,ls_monto,ls_monco,ls_nomestval,ls_codasi,ls_estapr);
	close();
  }
  ...
  f.action="sigesp_cat_variacion.php";
  f.submit();
  }
</script>
