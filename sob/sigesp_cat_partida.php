...
if($ls_operacion=="ue_buscar")
{
	$ls_sql=" SELECT s.codpar,s.nompar,s.despar,u.nomuni,u.coduni,s.prepar,s.codcovpar,s.codcatpar,c.descatpar,
			  (SELECT descov FROM sob_covenin WHERE s.codemp=sob_covenin.codemp AND s.codcovpar=sob_covenin.codcoven) as descov
	...
	if($row=$io_sql->fetch_row($rs_data))
	{
		...
		for($li_z=1;$li_z<=$li_totrow;$li_z++)
		{
			...
			$ls_descov=$data["descov"][$li_z];
			print "<td style=\"text-align:center \"><a href=\"javascript: aceptar('$ls_codigo','$ls_nombre','$ls_descripcion','$ls_codunidad','$ls_unidad','$ls_prepar','$ls_codcovpar','$ls_codcatpar','$ls_descatpar','$ls_descov');\">".$ls_codigo."</a></td>";
...
<script language="JavaScript">
  ...
  function aceptar(codigo,nombre,descripcion,codunidad,nomunidad,prepar,codcovpar,codcatpar,descatpar,descov)
  {
 	f=document.form1;
	opener.ue_cargarpartida(codigo,nombre,descripcion,codunidad,nomunidad,prepar,codcovpar,codcatpar,descatpar,descov);
