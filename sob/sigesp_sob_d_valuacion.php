...
if(array_key_exists("operacion",$_POST))
{
	...
	if($li_filascargos>1)
	{
		...
		$ld_subtot=number_format($ld_subtot,2,',','.');
	}
	...
	for($li_row=1;$li_row<=$li_filascuentas;$li_row++)
	{
		$ls_codestpro1=$_POST["codest1".$li_row];
		$ls_codestpro2=$_POST["codest2".$li_row];
		$ls_codestpro3=$_POST["codest3".$li_row];
		$ls_codestpro4=$_POST["codest4".$li_row];
		$ls_codestpro5=$_POST["codest5".$li_row];
		$ls_estcla=$_POST["txtestcla".$li_row];
		$ls_spgcuenta=$_POST["txtnomcue".$li_row];
		$ls_moncue=$_POST["txtmoncue".$li_row];
		$ls_codestpro=$ls_codestpro1.$ls_codestpro2.$ls_codestpro3.$ls_codestpro4.$ls_codestpro5;
		$io_fun_sob->uf_formatoprogramatica($ls_codestpro,&$as_programatica);
		$la_objectcuentas[$li_row][1]="<input name=txtcodcue".$li_row." type=text id=txtcodcue".$li_row." class=sin-borde style= text-align:center size=40 value='".$as_programatica."' readonly><input name=codest1".$li_row." type=hidden id=codest1".$li_row." value='".$ls_codestpro1."'><input name=codest2".$li_row." type=hidden id=codest2".$li_row." value='".$ls_codestpro2."'><input name=codest3".$li_row." type=hidden id=codest3".$li_row." value='".$ls_codestpro3."'><input name=codest4".$li_row." type=hidden id=codest4".$li_row." value='".$ls_codestpro4."'><input name=codest5".$li_row." type=hidden id=codest5".$li_row." value='".$ls_codestpro5."'>".
		...
		$la_objectcuentas[$li_row][3]="<input name=txtnomcue".$li_row." type=text id=txtnomcue".$li_row." class=sin-borde style= text-align:left size=10 value='".$ls_spgcuenta."' readonly >";
		$la_objectcuentas[$li_row][4]="<input name=txtmoncue".$li_row." type=text id=txtmoncue".$li_row." class=sin-borde size=20 style= text-align:right value='".$ls_moncue."' onKeyPress=return(currencyFormat(this,'.',',',event)) onBlur=javascript:uf_calcularamo()>";
...
elseif($ls_operacion=="ue_removerretenciones")
{
	...
	for($li_i=1;$li_i<=$li_filasretenciones;$li_i++)
	{
		...
		{		
			...
			$ls_cuenta=$_POST["txtcueret".$li_i];
			$ls_monret=$_POST["txtmonret".$li_i];
			$ls_monretaux=$io_funcsob->uf_convertir_cadenanumero($ls_monret);
			$ls_totreten=$ls_totreten+$ls_monretaux;
			$ls_totret=$_POST["txttotret".$li_i];
			$ls_deduccion=$_POST["txtdedret".$li_i];
			$ls_formula=$_POST["formula".$li_i];
...
elseif($ls_operacion=="ue_cargarcargo")
{
	$ls_subtotpar=$_POST["txtsubtotpar"]; 
	$ls_basimpval=$_POST["txtbasimpval"];
	$ls_baseimponible=$_POST["baseimponible"];
	...
	for($li_i=1;$li_i<$li_filascargos;$li_i++)
	{
		...
		$ls_spgcuenta=$_POST["spgcuenta".$li_i];
		$ls_estcla=$_POST["estcla".$li_i];
		$ld_result=$io_evalform->uf_evaluar($ls_formula,$ls_baseimponible,$lb_valido);
	...
	$la_objectcargos[$li_filascargos][4]="<input name=txtvacio type=text id=txtvacio class=sin-borde style= text-align:center size=5 readonly>";
	 ...
	 $ld_resultado=$ld_basimpval+$ld_subtot;  
	 $ls_subtot=$io_funcsob->uf_convertir_numerocadena($ld_resultado);
	 $ld_subtot=$io_funcsob->uf_convertir_numerocadena($ld_subtot);
...
elseif($ls_operacion=="ue_guardar")
{
  ...
  $lb_valido=$io_valuacion->uf_guardar_valuacion($ls_codval,$ls_codcon,$ld_fecha,$ld_fecinival,$ld_fecfinval,$ls_obsval,$ls_amoactual,$ls_amoobs,$ls_amoant,
												 ...
												 $ls_numrecdoc,$ls_numref,$ls_poramoactual);
  if($lb_valido)
  {
    /************************************PARTIDAS*******************************************/
	$li_partidas=1;
   ...
   /*****************************************************************************************/ 	 
	if($lb_valido)
	{
		...
		{
			$la_cuentas["codest1"][$li_i]=$_POST["codest1".$li_i];
			$la_cuentas["codest2"][$li_i]=$_POST["codest2".$li_i];
			$la_cuentas["codest3"][$li_i]=$_POST["codest3".$li_i];
			$la_cuentas["codest4"][$li_i]=$_POST["codest4".$li_i];
			$la_cuentas["codest5"][$li_i]=$_POST["codest5".$li_i];
			$la_cuentas["estcla"][$li_i]=$_POST["txtestcla".$li_i];
			$la_cuentas["nomcue"][$li_i]=$_POST["txtnomcue".$li_i];
			$la_cuentas["moncue"][$li_i]=$_POST["txtmoncue".$li_i];

		}
		$lb_valido=$io_valuacion->uf_update_dtcuentas($ls_codval,$ls_codcon,$la_cuentas,$li_filascuentas,$la_seguridad); 
		/*****************************************************************************************/ 	 	 
	 }
	if($lb_valido)
	{
...
//	$io_valuacion->uf_select_newcodigo($ls_codcon,&$ls_codval);
	...
	$lb_valido=$io_valuacion->uf_load_cuentaspresupuestarias($ls_codcon,&$rs_datacuentas,&$ai_rowscuentas);
	$li_row=0;
	if($ai_rowscuentas>0)
	{
		while(!$rs_datacuentas->EOF)
		{
			...
			$la_objectcuentas[$li_row][2]="<input name=txtestcla".$li_row." type=text id=txtestcla".$li_row." class=sin-borde style= text-align:center size=20 value='".$ls_estcla."' readonly>";
			$la_objectcuentas[$li_row][3]="<input name=txtnomcue".$li_row." type=text id=txtnomcue".$li_row." class=sin-borde style= text-align:left size=10 value='".$ls_spgcuenta."' readonly >";
			$la_objectcuentas[$li_row][4]="<input name=txtmoncue".$li_row." type=text id=txtmoncue".$li_row." class=sin-borde size=20 style= text-align:right value='0,00' onKeyPress=return(currencyFormat(this,'.',',',event)) onBlur=javascript:uf_calcularamo()>";
			$la_objectcuentas[$li_row][5]="";
...
{  
   ...
   { 
	 $lb_ultimavaluacion=$io_valuacion->uf_verificar_valuacioneliminar($ls_codcon,$ls_codval);
	 if($lb_ultimavaluacion)
	 {
		$io_valuacion->io_sql->begin_transaction();
		$lb_valido=$io_valuacion->uf_delete_partidas($ls_codcon,$ls_codval,$la_seguridad);
		if($lb_valido)
		{
			$lb_valido=$io_valuacion->uf_delete_retencionesvaluacion($ls_codcon,$ls_codval,$la_seguridad);
			if($lb_valido)
			{
				$lb_valido=$io_valuacion->uf_delete_cargos($ls_codcon,$ls_codval,$la_seguridad);
				if($lb_valido)
				{
					$lb_valido=$io_valuacion->uf_delete_cuentas($ls_codval,$ls_codcon);
					if($lb_valido)
					{
						$lb_valido=$io_valuacion->uf_delete_valuaciones($ls_codcon,$ls_codval,$la_seguridad);
						if($lb_valido)
						{
							for ($li_i=1;($li_i<$li_filaspartidas)&&($lb_valido);$li_i++)
							{
								if(!empty($_POST["flagpar".$li_i]))
								{
									...
									$lb_valido=$io_valuacion->uf_update_Actcantidaejecutada($ls_codasi,$ls_codparG,$ls_canparG,$ls_canejeG,$la_seguridad);
								}
							}
						}
					}
		...
		if($lb_valido)
		{
			$io_msg->message("La Valuacion fue Eliminada exitosamente!!");
			$io_valuacion->io_sql->commit();
		}
		else
		{
			$io_msg->message("Ocurrio un error al Eliminar la Valuacion");
			$io_valuacion->io_sql->rollback();
		}
	 ...
	 else
	 {
		 ...
		 if($lb_valido)
		 {
			$io_valuacion->uf_update_estado($ls_codval,3,$la_seguridad,$ls_codcon);
		 }
		 if($lb_valido)
		 {
		 ...
		 {
			$io_msg->message("Ocurrio un error al anular la Valuacion");
		 }
	  }
   }
   else
...
//   $ld_subtot=number_format($ld_subtot,2,',','.');
   ...
   /************************CARGANDO CUENTAS**************************************************/ 
	$lb_valido=$io_valuacion->uf_select_cuentaspresupuestarias($ls_codval,$ls_codcon,&$rs_datacuentas,&$ai_rowscuentas);
	$li_row=0;
	if($ai_rowscuentas>0)
	{
		while(!$rs_datacuentas->EOF)
		{
			...
			$ls_estcla=$rs_datacuentas->fields["estcla"];
			$ls_spgcuenta=$rs_datacuentas->fields["spg_cuenta"];
			$li_monto=number_format($rs_datacuentas->fields["monto"],2,',','.');
			$ls_codestpro=$ls_codestpro1.$ls_codestpro2.$ls_codestpro3.$ls_codestpro4.$ls_codestpro5;
			$io_fun_sob->uf_formatoprogramatica($ls_codestpro,&$as_programatica);
			$li_row++;
			$la_objectcuentas[$li_row][1]="<input name=txtcodcue".$li_row." type=text id=txtcodcue".$li_row." class=sin-borde style= text-align:center size=40 value='".$as_programatica."' readonly><input name=codest1".$li_row." type=hidden id=codest1".$li_row." value='".$ls_codestpro1."'><input name=codest2".$li_row." type=hidden id=codest2".$li_row." value='".$ls_codestpro2."'><input name=codest3".$li_row." type=hidden id=codest3".$li_row." value='".$ls_codestpro3."'><input name=codest4".$li_row." type=hidden id=codest4".$li_row." value='".$ls_codestpro4."'><input name=codest5".$li_row." type=hidden id=codest5".$li_row." value='".$ls_codestpro5."'>".
			...
			$la_objectcuentas[$li_row][2]="<input name=txtestcla".$li_row." type=text id=txtestcla".$li_row." class=sin-borde style= text-align:center size=20 value='".$ls_estcla."' readonly>";
			$la_objectcuentas[$li_row][3]="<input name=txtnomcue".$li_row." type=text id=txtnomcue".$li_row." class=sin-borde style= text-align:left size=10 value='".$ls_spgcuenta."' readonly >";
			$la_objectcuentas[$li_row][4]="<input name=txtmoncue".$li_row." type=text id=txtmoncue".$li_row." class=sin-borde size=20 style= text-align:right value='".$li_monto."' onKeyPress=return(currencyFormat(this,'.',',',event)) onBlur=javascript:uf_calcularamo()>";
			$la_objectcuentas[$li_row][5]="";
...
?>	
  ...
  <table width="780" border="0" align="center" cellpadding="0" cellspacing="0"  class="contorno">
      ...
      <tr class="formato-blanco">
        ...
        <td width="176"><span class="style6">
      ...
      <tr class="formato-blanco">
        <td width="87" height="22"><div align="right"></div></td>
	  ...
	  <tr class="formato-blanco">
        ...
        <td width="116"><input name="txtfecinival" type="text" id="txtfecinival"  style="text-align:left" value="<?php print $ls_fecinival; ?>" size="12" maxlength="11"   onKeyPress="javascript:ue_validafecha();" datepicker="true" readonly="true"></td>
        <td width="128"><div align="right">Fecha Fin </div></td>
      ...
      <tr class="formato-blanco">
        ...
        <td><div align="left">
          <input name="txtnumrecdoc" type="text" id="txtnumrecdoc"  onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnopqrstuvwxyz')" value="<?php print $ls_numrecdoc;?>" size="20" maxlength="15" style="text-align:center" >
        </div></td>
        <td><div align="right">N&uacute;mero de Control </div></td>
        <td><div align="left">
...
%            </span></td>
      ...
      <tr class="formato-blanco">
        <td colspan="6"><div align="center"><?php $io_grid->makegrid($li_filascuentas,$la_columcuentas,$la_objectcuentas,$li_anchocuentas,$ls_titulocuentas,$ls_nametable);?>	</div></td>
      </tr>
      <tr class="formato-blanco">
      ...
      <tr class="formato-blanco">
        <td colspan="6"><table width="609" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr class="formato-blanco">
            ...
            <td width="593"><a href="javascript:ue_catcargos();"><img src="../shared/imagebank/mas.gif" width="9" height="17" border="0"></a><a href="javascript:ue_catcargos();">Agregar Detalle 
              <input name="baseimponible" type="hidden" id="baseimponible" value="<?php print $ls_baseimponible; ?>">
            </a></td>
          </tr>
          <tr align="center" class="formato-blanco">
      ...
      <tr class="formato-blanco">
        ...
        <td height="22"><input name="txttotcar" type="text" id="txttotcar" value="<?php print $ld_subtot; ?>" size="20" style="text-align:right " readonly></td>
      </tr>
      <tr class="formato-blanco">
        <td>&nbsp;</td>
        <td>&nbsp;</td>
...
function ue_cargarvaluacion(ls_codval,ls_codasi,ls_codcon,ls_fecha,ls_fecinival,ls_fecfinval,ls_obsval,ls_amoval,ls_obsamoval,
							ls_amoantval,ls_amototval,ls_amoresval,ls_basimpval,ls_montotval,ls_subtotpar,ls_totreten,ls_subtot,
							ls_nomestval,ls_estapr,ls_moncont,ls_numrecdoc,ls_numref,ls_moncar,ls_poramo)
{
    f=document.form1;
	...
	f.txttotcar.value=uf_convertir(ls_moncar);
	f.txtporamoactual.value=uf_convertir(ls_poramo);
	f.txtestval.value=ls_nomestval;	
	f.hidcodasi.value=ls_codasi;
	f.hidestapr.value=ls_estapr;
	f.txtmoncon.value=ls_moncont;
...
function ue_cargarcargo(cod,nom,formula,codestpro,spg_cuenta,estcla)
{
	...
	if(ls_estapr!="1")
	{
		...
		if(!lb_existe)
		{
			...
			f.baseimponible.value=totaldet;
...
{
	f=document.form1;
	totded=f.txttotreten.value;
	totded=totded.replace(".",""); 
	totded=totded.replace(",","."); 
	...
	{
		alert("Debe eliminar las deducciones relacionadas a la valuacion");
	}
}
						...
						if(ue_comparar_intervalo("txtfecinival","txtfecfinval","La fecha de inicio de la Valuación debe ser menor de la de finalización!!!"))
						{
							...
							if(ld_montotval<=ld_moncon)
							{
							   ...
							   for(li_i=1;li_i<=filas;li_i++)
							   {
								...
								totalgen=f.txtsubtotpar.value;
								totalgen=totalgen.replace(".",""); 
								totalgen = totalgen.replace(",",".");
								totalgen = parseFloat(totalgen);
								basimpval=f.txtbasimpval.value;
								basimpval=basimpval.replace(".",""); 
								basimpval = basimpval.replace(",",".");
								basimpval = parseFloat(basimpval);
								if(totaldet==totalgen)
								{
									if(basimpval>0)
									{
									  if (ue_valida_null(f.txtnumrecdoc,"Numero de Documento")==false)
									   {
										 f.txtnumrecdoc.focus();
									   }
									   else
									   {
										  if (ue_valida_null(f.txtnumref,"Numero de Referencia")==false)
										   {
											 f.txtnumref.focus();
										   }
										   else
									...
									{
										alert("El monto total de la valuacion no puede ser menor a 0");
									}
								}
								else
								{
									alert("El monto total de las cuentas debe ser igual al total de las partidas");
								}
							}
							else
							{
   ...
   else
   {
	 ...
	 if((f.txtsubtotpar.value=="")||(f.txtsubtotpar.value=="0,00"))
      ...
      else
      {
        ...
        ld_totant=parseFloat(uf_convertir_monto(f.txttotant.value));
        ...
        if(ld_amoactual>ld_totant)
         ...
         else
	     {
	      if(f.hidamototbd.value=="")
		   ...
		   ld_totalamo=redondear(ld_totalamo,2);
		   ld_totant=redondear(ld_totant,2);
		   if(ld_totalamo>ld_totant)
		    ...
		    else
		    {
			  ...
			  if(totaldet>0)
			  {
				  ld_subto=totaldet-ld_amoactual;
