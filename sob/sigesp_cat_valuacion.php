...
{ 
	$ls_sql="SELECT v.*,a.codasi,c.monto AS moncont"; 
	...
	}else
	{
		if($row=$io_sql->fetch_row($rs_data))
		{
			...
			for($li_z=1;$li_z<=$li_totrow;$li_z++)
			{
				...
				$ls_moncont=$data["moncont"][$li_z];
				$ls_numrecdoc=$data["numrecdoc"][$li_z];
				$ls_numref=$data["numref"][$li_z];
				$ls_poramo=$data["poramo"][$li_z];
				$ls_nomestval=$io_funobr->uf_convertir_numeroestado($data["estval"][$li_z]);
				$ls_moncar=($ls_subtot-$ls_basimpval);
				
				print "<td><a href=\"javascript: aceptar('$ls_codval','$ls_codasi','$ls_codcon','$ls_fecha','$ls_fecinival',".
					  "             '$ls_fecfinval','$ls_obsval','$ls_amoval','$ls_obsamoval','$ls_amoantval','$ls_amototval',".
					  "             '$ls_amoresval','$ls_basimpval','$ls_montotval','$ls_subtotpar','$ls_totreten','$ls_subtot',".
					  "             '$ls_nomestval','$ls_estapr','$ls_moncont','$ls_numrecdoc','$ls_numref','$ls_moncar','$ls_poramo');\">".$ls_codcon."</a></td>";
...
function aceptar(ls_codval,ls_codasi,ls_codcon,ls_fecha,ls_fecinival,ls_fecfinval,ls_obsval,ls_amoval,ls_obsamoval,ls_amoantval,
				 ls_amototval,ls_amoresval,ls_basimpval,ls_montotval,ls_subtotpar,$ls_totreten,ls_subtot,ls_nomestval,ls_estapr,
				 ls_moncont,ls_numrecdoc,ls_numref,ls_moncar,ls_poramo)
  {
    opener.ue_cargarvaluacion(ls_codval,ls_codasi,ls_codcon,ls_fecha,ls_fecinival,ls_fecfinval,ls_obsval,ls_amoval,ls_obsamoval,
