<?php
	...
	function uf_print_encabezado_pagina($as_codsolvia,&$io_encabezado,&$io_pdf)
	{
		...
		$io_pdf->addText(210,740,11,"<b>SOLICITUD Y APROBACION DE</b>"); // Agregar NRO DE CONTROL
		$io_pdf->addText(240,725,11,"<b>GASTOS DE VIAJE</b>"); // Agregar NRO DE CONTROL
	...
	function uf_print_cabecera($as_cedper,$as_nomper,$as_desuniadm,$as_codcueban,$as_tipcuebanper,$as_desded,$as_destipper,
							  ...
							  $as_telefono,$ai_sueper,$as_denestpro1,$as_denestpro2,$as_desrut,$ai_solviaext,$as_obssolvia,$ls_transporte,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('desde'=>'<b>16. Tipo de Viático:</b>            Nacional    '.$ls_extno.'               Exterior    '.$ls_extsi.' ','dias'=>'<b>17. Pernocta:   </b>              Si '.$ls_pernoctasi.'            No '.$ls_pernoctano.' '));
...
/*		$la_data[1]=array('desde'=>'<b>18. Vehiculo Oficial:</b>           Si      No','hasta'=>'<b>19. Pasaje Terrestre</b>            Si      No','dias'=>'<b>20. Vehiculo Propio</b>          Si      No');
		...
		unset($la_columna);
		unset($la_config);*/
		$la_data=array(array('titulo'=>'<b>MEDIO DE TRANSPORTE</b>    '.$ls_transporte));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
...
/*		unset($la_data);
	...
	function uf_print_firmas(&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('elaborado'=>'<b>24. Solicitado Por:</b>','revisado'=>'<b>25. Jefe Unidad Solicitante:</b>','autorizado'=>'<b>26. Oficina de Administración:</b>'),
		...
		$la_data=array(array('elaborado'=>'<b>27. Director:</b>','revisado'=>'<b>28. Sub Director Administrativo:</b>','autorizado'=>'<b>29. Sub Director Académico:</b>'),
					   array('elaborado'=>'','revisado'=>'','autorizado'=>''),
					   array('elaborado'=>'','revisado'=>'','autorizado'=>''),
					   array('elaborado'=>'','revisado'=>'','autorizado'=>''),
					   array('elaborado'=>'','revisado'=>'','autorizado'=>''),
		...
		while($row=$io_report->io_sql->fetch_row($rs_data))
		{
			...
			if($lb_valido)
			{
				...
				for($li_s=1;$li_s<=$li_totrow_det;$li_s++)
				{
					...
					$ls_tiptra = $io_report->ds_detalle->data["codtiptra"][$li_s];
					
					if ((trim($ls_proasi)=='TRP')&&(!$lb_evaluado))
					{
						
						switch (trim($ls_tiptra))
						{ 
							case "01":
								$ls_transporte = '<b>1) Aéreo</b>   2) Marítimo   3) Terrestre    4) Institucional';  
							break;
						
							case "02":
								$ls_transporte = '1) Aéreo   <b>2) Marítimo</b>   3) Terrestre    4) Institucional';
							break;
						
							case "03":
								$ls_transporte = '1) Aéreo   2) Marítimo   <b>3) Terrestre</b>    4) Institucional';
							break;
						
							case "04":
								$ls_transporte = '1) Aéreo   2) Marítimo   3) Terrestre    <b>4) Institucional</b>';
							break;
				...
				if (empty($ls_transporte))
				{
					$ls_transporte = '1) Aéreo   2) Marítimo   3) Terrestre    4) Institucional';
				}
				uf_print_cabecera($ls_cedper,$ls_nomper,$ls_desuniadm,$ls_codcueban,$ls_tipcuebanper,$ls_desded,$ls_destipper,
							  ...
							  $ls_telefono,$li_sueper,$ls_denestpro1,$ls_denestpro2,$ls_desrut,$li_solviaext,$ls_obssolvia,$ls_transporte,&$io_pdf); // Imprimimos la cabecera del registro
