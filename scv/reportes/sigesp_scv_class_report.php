...
class sigesp_scv_class_report
{
	...
	function uf_select_solicitudpago_personal($as_codemp,$as_codsoldes,$as_codsolhas,$ad_fecdes,$ad_fechas,$ai_orden,$as_codsolvia,&$rs_data)	
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Report MÉTODO->uf_select_solicitudpago_personal ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if ($li_numrows>0)
	...
	function uf_select_listadosolicitudes($ad_fecregdes,$ad_fecreghas,$as_coduniadm,$as_orden)
	{
		...
		$ls_sql="SELECT scv_solicitudviatico.codsolvia,scv_solicitudviatico.fecsalvia,scv_solicitudviatico.fecregvia,scv_rutas.desrut,scv_dt_personal.codper,".
				...
				" scv_solicitudviatico.fecsalvia, scv_solicitudviatico.fecregvia, scv_rutas.desrut, ".
				" scv_dt_personal.codper,scv_dt_personal.codclavia,scv_dt_personal.codemp, ".
				" scv_dt_personal.codsolvia ".
				" ORDER BY ".$as_orden."";
