<?php
	...
	function uf_print_encabezado_pagina($as_codsolvia,&$io_encabezado,&$io_pdf)
	{
		...
		//Cambio del título del reporte
		$io_pdf->addText(195,735,11,"<b>SOLICITUD DE VIATICOS NACIONALES</b>"); // Agregar NRO DE CONTROL
							  ...
							  $as_telefono,$ai_sueper,$as_denestpro1,$as_denestpro2,$as_desrut,$ai_solviaext,$as_obssolvia,&$io_pdf)
	{
	    ...
	    // Se comentaron las lineas que imprimen los datos del solicitante y se copiaron mas abajo para eliminar la impresion del salario
		...
		$la_data=array(array('nombre'=>$as_nomper,'cedula'=>$as_cedper,'cargo'=>$as_cargo,'categoria'=>$as_telefono));
		$la_columna=array('nombre'=>'<b>2. Nombre del Funcionario</b>','cedula'=>'<b>3. Cédula de Identidad N°</b>',
						  'cargo'=>'<b>4. Denominación del Cargo</b>','categoria'=>'<b>5. Telefono Oficina / Celular</b>');
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 'fontSize' => 8, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
						 'shaded'=>0	, // Sombra entre líneas
						 ...
						 'cols'=>array('nombre'=>array('justification'=>'center','width'=>110),
									   ...
									   'categoria'=>array('justification'=>'center','width'=>130)));
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		unset($la_data);
		unset($la_columna);
		unset($la_config);
		...
		// Se cambio el correlativo de la etiqueta por la eliminacion del sueldo
		$la_data=array(array('titulo'=>'<b>6. N° de Cuenta Nómina</b>','corriente'=>$ls_corriente,'titulocorriente'=>'Corriente',
		...
		// Se cambio el correlativo de la etiqueta por la eliminacion del sueldo
		$la_data=array(array('dependencia'=>'<b>7. Dependencia Solicitante:</b> '.$as_desuniadm));
		...
		unset($la_config);
		// Se cambio el correlativo de la etiqueta por la eliminacion del sueldo
		$la_data=array(array('estpro1'=>'<b>8. Proyecto / Acción Centralizada:</b> '.$as_denestpro1,'estpro2'=>'<b>9. Acción Específica:</b> '.$as_denestpro2));
		...
		unset($la_config);
		// Se cambio el correlativo de la etiqueta por la eliminacion del sueldo
		$la_data=array(array('ruta'=>'<b>10. Ruta:</b> '.$as_desrut));
		...
		unset($la_config);
		// Se cambio el correlativo de la etiqueta por la eliminacion del sueldo
		$la_data=array(array('desde'=>'<b>11. Tiempo Estimado        Desde:</b> '.$ad_fecsalvia,'hasta'=>'<b>Hasta:</b> '.$ad_fecregvia,'dias'=>'<b>12. Total Días:</b> '.$ai_numdiavia));
		...
		unset($la_config);
		// Se cambio el correlativo de la etiqueta por la eliminacion del sueldo
		$la_data=array(array('mision'=>'<b>13. Conceptos de la Misión:</b> '.$as_denmis));
		...
		unset($la_config);
		// Se cambio el correlativo de la etiqueta por la eliminacion del sueldo
		$la_data=array(array('desde'=>'<b>14. Tipo de Viático:</b>            Nacional    '.$ls_extno.'               Exterior    '.$ls_extsi.' ','dias'=>'<b>15. Pernocta:   </b>              Si '.$ls_pernoctasi.'            No '.$ls_pernoctano.' '));
...
/*		$la_data[1]=array('desde'=>'<b>18. Vehiculo Oficial:</b>           Si      No','hasta'=>'<b>19. Pasaje Terrestre</b>            Si      No','dias'=>'<b>20. Vehiculo Propio</b>          Si      No');
		...
		unset($la_config);
		// Se cambio el correlativo de la etiqueta por la eliminacion del sueldo
		$la_data=array(array('titulo'=>'16. Pasaje Aéreo                       Si         No                                  Ruta:'));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
...
/*		unset($la_data);
		...
		unset($la_columna);
		unset($la_config);*/
		// Se cambio el titulo de este renglon del reporte
		$la_data=array(array('titulo'=>'<b>RELACION DE GASTOS</b>'));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
		...
		unset($la_config);
		// Se cambio el correlativo de la etiqueta por la eliminacion del sueldo
		$la_data=array(array('titulo'=>'17. '.$as_obssolvia));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
	...
	function uf_print_firmas(&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		// Se cambio el correlativo de la etiqueta por la eliminacion del sueldo y cambio de títulos de las firmas
		$la_data=array(array('elaborado'=>'<b>18. Solicitado Por:</b>','revisado'=>'<b>19. Autorizado Por: Jefe de Area / Gerente / Director Ejecutivo / Presidente</b>','autorizado'=>'<b>20. Recibido Por: Control y Ordenación de Pagos</b>'),
	...
	else // Imprimimos el reporte
	{
		...
		while($row=$io_report->io_sql->fetch_row($rs_data))
		{
			...
			if($lb_valido)
			{
				...
				uf_print_detalle_asignaciones($la_data,$li_total,$ls_totalletras,$io_pdf); 
				unset($la_data);
