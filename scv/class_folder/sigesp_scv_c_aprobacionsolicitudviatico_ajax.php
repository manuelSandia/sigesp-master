<?php
	...
	require_once("sigesp_scv_c_aprobacionsolicitudviatico.php");
	$io_anulacion=new sigesp_scv_c_aprobacionsolicitudviatico('../../');
	// proceso a ejecutar
	...
	{	
		...
		//	  Description: M�todo que impirme el grid de las solicitudes de viaticos para anular 
		...
		while($row=$io_anulacion->io_sql->fetch_row($rs_datasol))
		{
			...
			$lo_object[$li_fila][5]="<input type=text name=txtdenmis".$li_fila."    id=txtdenmis".$li_fila."    class=sin-borde style=text-align:left   size=50 value='".utf8_encode($ls_denmis)."'   readonly>";
			$lo_object[$li_fila][6]="<textarea input name=txtjustapro".$li_fila." id=txtjustapro".$li_fila."  type=text class=sin-borde  size=80   style=text-align:left       >$ls_justapro</textarea>";
		...
		if($li_fila==0)
		{
			...
			$lo_object[$li_fila][6]="<input type=text name=txtjustapro".$li_fila."     class=sin-borde style=text-align:right  size=15 readonly>";
